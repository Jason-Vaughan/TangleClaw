...
- [x] Car 6 — The channel teaches itself, and the rule comes down (#912, #1020, #904) — 2026-08-21: nudge names the send endpoint + the initiator-closes contract; both nudge and prime state the real API origin instead of a guide that never carried one; `_medusaSwitchboardLines` reaches all four engine generators, gated on the project's opt-in and scoped by the store's project name. **#912 layer 2 (initiator-side open-exchange tracking) NOT built** — it is the enforceable guarantee where the nudge line is a soft instruction, and it is its own feature. Suite green (6675 pass, exit 0, zero `not ok`); 5 mutations red.
  - **Completion test — the rule replacement is drafted, NOT applied:** `.tangleclaw/plans/818-rule5-replacement.md`. Session rule id 12 is operator-authored and goes live fleet-wide on save, so applying it is the operator's action.
