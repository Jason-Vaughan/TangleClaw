...
- [x] Chunk 03 (`2c65777d`, Critic fixes `cd7424a8`). Reviewed by cumulative `rev-20260925T191930Z-4d2dd300` (3 blocking, all fixed) and verify-resolutions `rev-20260925T192629Z-afb06cad` (0 blocking). Full suite on `cd7424a8` green. **STOPPED for the Architect** before chunk 04. Scope: wake transport seam, wake facts, and the watchdog timer (fake clock)
...
- **[DECISION, for the Architect's note] The R20 A3 readiness condition.** R20 allows an unconfirmed attempt to be re-armed only after a persisted readiness state change. As built, the watchdog re-arms after `rearmAfterMs` with nothing read. The re-nudge itself then waits for the full gates, including the pane reaching an idle, typeable prompt for the debounce streak. So no injection happens without that readiness change being observed. The persisted part is the re-arm fact plus the subsequent `wake_pending`/`wake_attempted` facts. This is recorded rather than silently read as equivalent.
