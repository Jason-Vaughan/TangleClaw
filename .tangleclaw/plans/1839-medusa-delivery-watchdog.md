...
- **[DECISION, for ruling] Unverified readers.** Agents mark mail handled with a plain `/read`, as the generated guide instructs, and that carries no launch binding. Those reads and acks are recorded with actor `unverified-reader`, never `recipient`. Like `operator-ui` (P3), an unverified ack auto-closes a no-reply exchange but never satisfies a reply-required one. Reads and acks are de-duplicated **per actor**, so a dashboard ack does not hide the agent's later ack. **Ruled by the Architect (message `e2956328`): keep it.** Handling is the only terminal act a no-reply exchange needs. An unverified ack never satisfies or closes a reply-required exchange, and `actor=unverified-reader` stays in the ledger so the limit is auditable.
...
- **[DECISION] Chunk 03: how a re-arm works.** A re-arm is a `rearmed` fact that returns the exchange to `wake_pending` (code `rearmed`) with a persisted `next_eligible_at`. A later blocked or pending verdict cannot erase it; only the next attempt consumes it. `rearm()` re-checks the state and the expected count inside its transaction, so duplicate ticks and restarts are idempotent. The wake monitor treats an owned edge as re-opened only while `rearmDue` holds, and then runs every gate from the top, including the idle debounce.
- **[RULING, Architect message `a06a43ac`] R20 A3: time-then-gates was REJECTED as equivalent; now built as ruled.** A re-arm needs a durable trigger newer than the attempt:
  - a negative receipt; or
  - a persisted `readiness_changed` fact, tied to the attempt's nonce. It is recorded only when an ineligible verdict (`wake_blocked`: busy pane, engine not idle, listener reconnecting, capture failed) is on record after the attempt and the gates then find the session eligible again.

  After a nudge, while an attempt is unsettled (no read, no receipt, no pending re-arm, no readiness change yet), the wake monitor keeps running the engine and pane gates on the owned edge in observe-only mode. It writes verdicts to the exchanges, not the ledger, and never injects. `rearmAfterMs` remains only as a minimum settle after the attempt. Elapsed time alone never re-arms or spends budget; such an exchange stays unconfirmed and chunk 04 escalates it by age. `readiness_changed` was added to v49's fact CHECK, since v49 is unreleased. The four required tests are in `test/medusa-watchdog.test.js` as (1)–(4).
- **Amendment Critic `rev-20260925T193452Z-beee9db9`** (1 blocking). Fixed in one commit:
  - **Blocking:** a restarted wake monitor, having lost its in-memory watermark, would nudge again outside the re-arm budget. `alreadyAttempted(workspace, unread)` now adopts the edge as nudged when every unread tracked exchange has an attempt and no pending re-arm, and together they cover the unread count. Unaccounted mail (for example an untracked arrival) is still a fresh edge. There is a test that restarts the monitor.
  - The trigger rule lives only in `medusa-exchanges#rearmTrigger`. `rearm()` refuses without a trigger and records it as the code; the watchdog adds only timing.
  - New tests cover the observe-only engine gate (Codex busy then idle) and a pane-capture failure.
  - `recordWakeFact` is deleted, stale comments are fixed, and the plan's transport text is corrected (below).
  - #1879 notes the fact volume.
  - A tc-verbs test pins the single closing instruction.
  - `server.js#_stillHasUnhandledMail` keeps its acknowledged-only rule on purpose: #1435's "handled" means acknowledged, not read.
- **[DECISION] Settled attempts are not watched.** Once an attempt has a readiness change, a receipt or a pending re-arm, the monitor stops observing that edge. A settled owned edge after a reconnect shows `wake_pending`/`awaiting-read` instead of a stale reason.
...
- **[DECISION] Chunk 04: the ladder is explicit facts.** The rungs are `aged`, `escalated` and `operator_alerted`, each recorded once, before any notice goes out. A notice's delivery facts (`escalation_queued`/`accepted`/`failed`/`undeliverable`) never move the rung; an early build that let them was caught by the tests. `escalated` was added to v49's fact CHECK, since v49 is unreleased.
- **[DECISION] Chunk 04: phases.** Unread is measured from the send and unanswered from the first ack; the ladder is one-way across both. Once a blocking exchange has escalated while unread, a later unanswered phase does not repeat it. Reply-phase blocking escalates `replyBlockingMs` (30 min) after the ack, and the operator is alerted `operatorBlockingMs` after that. Critical reaches aged and escalated at once and the operator after `operatorCriticalMs`. Normal mail only ages.
- **[DECISION] Chunk 04: the banner** shows only operator-rung or critical exchanges, so ordinary aged mail raises nothing. It rides the existing `/api/server-info` poll (`medusaEscalations`) rather than adding a timer. It is built with text nodes, since recipient names are project names, and uses the base amber banner tone (dark text) for contrast.
- **[DECISION] Chunk 04: the sender-side surface** is `tc message sent` (open exchanges, state, blocker, rung) plus the notices themselves. The recipient side is `GET …/medusa/exchanges?direction=received`.
- **[DECISION] Chunk 05: the isolated E2E** (`test/medusa-watchdog-e2e.test.js`) follows the #1861 exit-test precedent: an in-process server on an ephemeral port with a scratch store, never the live service. An ephemeral port binds no fixed port, so there was no PortHub lease to take. Its fake Hub copies the real Hub's event order.
- **[VERIFIED] The Hub-id assumption.** Read-only, from the Hub source `Medusa/src/medusa/medusa-server.js` (`/messages/direct`): one `crypto.randomUUID()` is the push frame's `messageId`, the pushed `message.id` and the `id` returned to the sender. The Hub pushes to an online recipient before it answers the sender, which is why arrival adoption (chunk 02) is the common path, not an edge case.
- **[DECISION] The operational guide** gains one Medusa line (priority, `inReplyTo`, close) in `lib/engines.js#_medusaDiscoveryLines`. It is mirrored into the committed `CLAUDE.md` as #1861 did, so a launch's regenerated block matches the repo copy.
- **Boundary cumulative `rev` at `7a6af992`** (0 blocking, 7 warnings). Fixed in one commit:
  - Reply-phase blocking now waits `replyBlockingMs` after the ack before the sender and the route are told together (it had told the sender at once).
  - Retirement leaves a replied exchange for its initiator.
  - The send path moved into `lib/medusa-send.js#sendTracked`, so routes stay thin, as `control-api` does for #1861, and #1873 can reuse it.
  - One closing instruction everywhere: a reply-required exchange is closed by its sender, and a no-reply one closes on the recipient's ack (`tc`, the guide line, `CLAUDE.md`, the docs).
  - Failed or undeliverable notices are logged, and the timer catches notice errors.
...
| C12 | Replaceable wake transport: receipt-bearing native preferred, guarded tmux as fallback; the durable inbox stays the source of truth | new `lib/wake-transports.js`: each transport is `{id, channel, receipts: 'positive'|'negative'|'none', deliver(ctx, seams) → {ok, error}, verify(ctx, seams) → receipt}`, chosen per session by `forSession` (Master or tmux today; a native adapter plugs in there per engine). *As built; rev 1's `deliver → {outcome}` and per-profile selection were superseded.* | a fake positive-receipt transport reaches `wake_accepted`; tmux tops out at `wake_attempted`, or `wake_not_accepted` on a negative receipt, and never records accepted |
...
**Tests (as built):** `test/medusa-exchanges.test.js` (facts, projection, validation, replay, triggers, retraction); `test/medusa-watchdog.test.js` (fake-clock re-arm triggers and budget, restarts, the wake monitor's observe-only and restart paths, and the transport cases planned for a separate `test/wake-transports.test.js`); `test/medusa-escalation.test.js` (the ladder, notices, routes, retirement, the dashboard banner, and the `escalation` key normalization); `test/api-medusa-exchanges.test.js` (send metadata, binding, `/read` facts, the #1435 regression, config and escalation routes); `test/store-medusa-exchange-migration.test.js` (v48→v49, fresh-install parity); `test/medusa-watchdog-e2e.test.js` (the isolated exit test). The existing `test/medusa-wake*.test.js` stay green **unmodified**, which is the C7 contract. The full suite runs on the worktree, never the main instance.
...
- **#1873: sender retraction.** B1 models the `retracted` terminal state, its v49 columns (as built, shared by every terminal outcome: `terminal_at`, `terminal_by`, `terminal_code` holding the retract reason, plus `replacement_hub_id`) and its guarded transition, and the watchdog never escalates it (C15). The API, UI, wake cancellation and retraction notice stay with #1873. The PR says `Refs #1873`; #1873 stays open.
