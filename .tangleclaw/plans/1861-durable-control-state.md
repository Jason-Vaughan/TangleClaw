...
- [x] Cumulative Critic
- [x] Draft PR opened: #1866 (2026-09-25). **STOP here**: the PM owns readiness and merge sequencing

**Recorded decisions made during the build** (each reported to the Architect over Medusa):

- **[DECISION] Store-failure governance memory (§2.4).** The plan said a governed launch records its `assignment_id` on the launch record. The build does not add a launch-record column. The assignment row already carries `bound_launch_id`, and the gate needs to know which *projects* are governed, not which launches are. `lib/control-gate.js` keeps that in memory: primed from `control_assignments` at boot, retried on every check until priming succeeds, and updated synchronously by every committed control command and every rebind. Per Architect rulings R3-C and R4-B: until priming succeeds, every check is `503`. After a successful prime, a store failure refuses a known-governed job or target subject and **every caller subject, the operator included**, with `503 CONTROL_STATE_UNAVAILABLE`, while a project affirmatively seen ungoverned passes. A verified operator passes a readable held lane, never an unreadable store.
- **[RULING R4-B / R5] No operator exception to unestablished control state.** A verified operator passes a READABLE held or stopped lane at restart and update-apply, but the gate first does a real read of current control state (`store.control.anyRestricted()`). A read failure, or priming that has not completed, is `503 CONTROL_STATE_UNAVAILABLE` for the operator as for every caller. `_knownToBeGoverned` has no operator carve-out. There is no A1 break-glass. Applied in `b7a163f8`. Operator regressions (both expect 503): `test/control-gate.test.js` for a thrown store and failed priming, and `test/control-surfaces.test.js` for restart and update-apply over HTTP. The readable-held operator pass is kept in both files. R5-C readiness requires an independent review bound to the exact PR head over the whole tree.
