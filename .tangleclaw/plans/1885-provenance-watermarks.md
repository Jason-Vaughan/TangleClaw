...
- [x] 03 — Project Settings UI, disposition, capabilities, docs, CHANGELOG. Built in 790d5464, fixed in 614f0e17; verify-resolutions rev-20260926T033429Z-61386ed9 clean (O-2, O-3, R-8, R-9 accepted on the record). Cumulative Critic rev-20260926T032840Z-d99ab744: R-1 (blocking, the settings control untested) fixed by moving the patch and preview into `tcProvenancePatch` / `tcProvenancePreview` in `public/api-helper.js`, tested behaviourally; the save now sends only the changed half, so a stored invalid template is never resent. R-4/R-5 fixed (drift is judged without the provenance line; the writer is handed unstamped content). R-6 fixed (in-place surfaces claim no staging stem). R-7 fixed (status line).
  - Verification beyond tests (R-3), run 2026-09-26 against scratch `TANGLECLAW_HOME`s, never the live instance, with a throwaway script:
    - **Off:** the codex, aider and claude carriers, the advisory and the prime (9 files) diff byte-identical against a checkout of `origin/main` (5f74a343).
    - **On:** exactly one line, directly beneath the `Generated by TangleClaw` header; unchanged, and no drift reported, on a second regeneration.
    - **Custom template, then null:** `Custom codex e2e-full`, then back to the default, byte-identical to the first write.
    - **Adversarial name:** the store refuses `-->` and newlines in a project name, so the reachable worst case is a project named `Generated by TangleClaw`. It renders the fixed `Built by TangleClaw` on both the carrier and the advisory, and no second header appears in the first three lines.
    - **`git add .codex.yaml` and regenerate:** the line is gone from the carrier and stays on the advisory. Untracked again: it returns.
    - **Off again:** the line is gone everywhere, with no drift reported.
  - Suite: 13232 pass, 2 fail, 1 skip at 790d5464 (TAP). Both failures are `test/dir-scanner.test.js` deadline tests, the known load-sensitive flake #1884, reproduced on the clean base c4e682a9 (1 fail in 12 isolated runs); no file of theirs is touched here.
  - Suite green at 614f0e17 by node's TAP reporter (0 fail; the #1884 flake did not fire), and recorded from the same run's JUnit report with `test-evidence record --from-junit`, so `test-status` is current. The recorder's total differs from TAP's because it counts the JUnit report differently; both report zero failures.
