...
- **Per-project wrap step overrides** (#643) — `project.json#wrapStepOverrides`, keyed by step id, lets a project disable or reconfigure (`enabled`/`blocker`/`prompt`) an individual wrap step. Order and membership stay framework-owned; `verifyChanged` is not overridable and a `commit`-kind step is not disableable. `lib/wrap-step-overrides.js`.
