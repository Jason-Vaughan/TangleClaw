...
- **Wrap step: `pr-merge`** — applies the gate's resolutions after the wrap commit, enqueueing GitHub auto-merge (`gh pr merge --auto --squash --delete-branch`) per `merge`. Never blocks; a failed enqueue surfaces as a warning with remediation. `lib/wrap-steps/pr-merge.js`.
