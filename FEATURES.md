...
- **Wrap step: `pr-check`** — surfaces open PRs and gates the wrap on them: a session-scoped open PR with no `merge`/`defer`/`ignore` resolution blocks, and a `merge` resolution enqueues GitHub auto-merge (`gh pr merge --auto --squash --delete-branch`). Degraded probes (no `gh`, no auth) still skip without blocking. `lib/wrap-steps/pr-check.js`.
