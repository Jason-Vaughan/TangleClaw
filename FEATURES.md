...
- **Session rules + self-improvement loop** (#347) — per-project session rules store (`store.sessionRules`; startup rules launch-inject, wrap rules inject into the wrap prompt via `lib/wrap-steps/ai-content.js#_appendWrapRules`), the Project Rules section of the Settings modal, and the wrap-time self-improvement loop. The standalone global-tier panel was retired. Docs: `docs/session-rules-self-improvement.md`.
