...
function buildUpdatePrompt(data) {
  ...
  return [
    ...
    '   `code` (dirty-tree, wrong-ref, no-update, no-tag, no-git, git-error,',
    '   provision-failed) and an `error`.',
    ...
    '   provision-failed is different from the guards: the checkout already moved to',
    '   the new release but its dependencies did not install, so the tree and',
    '   node_modules disagree. Do NOT restart the server in that state; the error',
    '   text carries the one-line recovery. Report it and STOP.',
    '   On success, the JSON also carries `provisioning`: if its `assetsChanged`',
    '   lists files under deploy/, tell the operator those need manual deploy steps',
    '   after the restart — the updater never rewrites launchd services itself.',
