...
- **Server restart required** after code changes — TangleClaw runs as a long-lived Node process (`node server.js`). Code changes (new routes, schema migrations, etc.) are NOT hot-reloaded. Kill the old process and relaunch.
- **API base URL**: `http://localhost:3102`
- **Version check**: `curl http://localhost:3102/api/version` — compare against `version.json` to confirm the running server matches the codebase.
