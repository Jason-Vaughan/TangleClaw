...
- **A signed-in account manages itself.** Global settings → Your account changes the password (`POST /api/auth/password`), which needs the current password — a session alone cannot change it — and ends every session the account holds, re-issuing this browser a fresh one so a copied cookie does not survive the change; two changes racing cannot both win (the second is refused, nothing written); recovery codes are untouched. **Sign out** ends this browser's session (`POST /api/auth/logout`), and **Sign out everywhere** (`POST /api/auth/logout-everywhere`, session + CSRF token required) ends every session the account holds.
