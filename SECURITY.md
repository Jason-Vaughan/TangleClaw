...
- **A signed-in account manages itself.** Global settings → Your account changes the password (`POST /api/auth/password`), which needs the current password — a session alone cannot change it — and ends the account's other sessions while keeping this one; recovery codes are untouched. **Sign out** ends this browser's session (`POST /api/auth/logout`), and **Sign out everywhere** (`POST /api/auth/logout-everywhere`, session + CSRF token required) ends every session the account holds.
