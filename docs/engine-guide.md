...
listed fails too. So this column is generated by nobody: wire a flag, and the guard tells you to
move it before the suite is green again. Wiring one is a feature decision per flag, not a cleanup.
...
the half that does not run. A row may declare both, and a row declaring neither does not belong in
the table.
