...
| `mergeStrategy` | `"whole-file"` (default) — TangleClaw owns the entire file — or `"managed-block"`, where it splices only the region between `BEGIN:tangleclaw` / `END:tangleclaw` and leaves the rest byte-identical. A broken marker set (misordered, unclosed, duplicated) is **repaired** into one well-formed block rather than refused or appended around — content between our own markers is regenerated, everything else is kept, and the result is stable across runs (`lib/managed-block.js`, shared with the wrap pipeline's priming roll). **Required** for a shared-convention carrier (`AGENTS.md`, `GEMINI.md`, `CONVENTIONS.md`): those are files operators commit and other tools also write, so a whole-file write destroys their content and is **refused at the write**, not merely warned about |
