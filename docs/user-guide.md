...
- **API**:
  ...
  `GET /api/launch-sequences?projectId=` carries `startupDelivery` on every row and, for the operator and the Master only, a `startupControl` block: `channel` (state, adapter, engine, opened/closed, close reason, teardown — never the adapter's own state), `fires` (every row, `denied` included) and `fireable`.
...
that was killed or crashed, or when TangleClaw can't tell whether one is running, the bar keeps its
10-second return to the landing page instead. If the launch is refused, the reason appears beside
