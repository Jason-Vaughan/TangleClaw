...
- **Edit**: change the text (at most 4096 bytes; line breaks are the only control characters allowed), tick the projects whose sessions may fire it, and tap **Save startup prompt**. Every save is a new **revision**, recording how well its author was proven: `operator-verified` with the username on an install with a login, `open-install-unverified` on one without. If someone else saved in the meantime, the save is refused with the server's reason, your edits stay in the editor, and it tells you the newer revision number: a second save then replaces that revision knowingly, never silently.
- **At launch, automatically**: a launch whose engine has a supported native channel and a `tc start` sequence to read selects the **native** startup path: TangleClaw types nothing into the pane (no prime paste, no kickoff line, no unready nudge) and, once the pane has rendered ready, fires the startup prompt once through the channel. The launch's own bootstrap is the caller (`launch`, clearance `launch-automatic`), and the fire row's receipt is what says the prompt arrived. A pane that never renders ready within the launch window records `blocked (pane_not_ready)` and nothing is sent; a blocker Codex reports (below) is recorded the same way. There is no paste fallback and no automatic retry: the **Launch readiness** panel names the reason and you may **Fire** by hand once it clears. Every other launch keeps the familiar path (`legacy`) and records, on the same panel, why it could not go native.
- **Fire on Codex**: a Codex session launched by TangleClaw runs against a per-launch app-server, and a fire delivers the prompt as an ordinary user turn in its pane. The fire record moves `pending → dispatching → accepted → applied`: *accepted* means the engine echoed this launch's digest with the prompt's exact bytes, *applied* means the turn completed. A turn waiting on an approval or a question in the pane stays `accepted` (`approval_pending` / `user_input_pending`) until you answer it there; TangleClaw never answers for you. A fire that cannot proceed is refused (`409 STARTUP_FIRE_BLOCKED`) and names why: the folder is not trusted in Codex (`trust_required`), Codex is signed out (`auth_required`), the usage window is exhausted with no credits (`quota_exhausted`), the pane has not opened its thread or is mid-turn (`engine_not_ready`), or Codex could not say (`readiness_unknown`). Nothing is sent in those cases and the same launch can be fired again once the blocker clears. A send whose answer was lost is recorded `indeterminate` and is never retried by itself. Other engines still answer `STARTUP_CONTROL_UNSUPPORTED`.
...
- **API**:
  ...
  `GET /api/launch-sequences?projectId=` carries `startupDelivery` on every row and, for the operator only, a `startupControl` block: `channel` (state, adapter, engine, opened/closed, close reason, teardown — never the adapter's own state), `fires` (every row, `denied` included) and `fireable`.
...
**Did the next session get this wrap?** A wrap that finishes normally publishes a handoff, and the next launch resumes from that publication. When a finished wrap's handoff was *not* published — a newer one was already current, the session ended before the wrap could record it, or the handoff step never ran — the drawer says **Wrap finished — handoff NOT published** with the reason, because the next session will start from an older handoff (#1675). A failed or stranded wrap PR keeps its own banner, which is worse, and names the handoff in its detail. The next session's resume proposal opens with where it came from: the publication's id, its session, the exact time it was staged and the launch verdict. When that is older than the latest session (a session crashed after it, for example), or when there is no publication and it comes from the unbound continuity index or a session summary, the proposal says so before suggesting anything. `tc start status` in that session prints the publication id and digest it read.
