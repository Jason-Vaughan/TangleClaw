...
- **Today**: Codex is the one engine with a native startup adapter (see **Fire on Codex** above). A fire at any other engine is refused as `STARTUP_CONTROL_UNSUPPORTED` and recorded. Nothing is ever typed into a pane as a fallback, and launches keep delivering their context as they do now; the automatic bootstrap through the adapter is the next chunk.
- **API**:
