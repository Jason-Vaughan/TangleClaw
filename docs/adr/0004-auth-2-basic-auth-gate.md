...
- **In-process Node auth (Better Auth / hand-rolled scrypt).** Rejected for the same reason as in
  ...
  **Revisited by ADR 0015 (accepted 2026-09-11)**, which adopts this mechanism and had to answer
  this objection rather than skip it — see its "Answering ADR 0004's rejection" section. The short of
  ...
  ADR 0016 settles how: a cookie, because a browser cannot set a header on a WebSocket handshake,
  and the upgrade is authenticated before the socket exists (#1419).
