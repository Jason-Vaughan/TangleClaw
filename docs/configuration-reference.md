...
**Both resolved WebSocket URLs must be loopback.** The WS path is unauthenticated
at the workspace layer — anything that can reach the port can register as any
workspace — so the override moves a *port*, not a host. A resolved host that is
not `localhost`, `127.0.0.0/8` or `::1` is refused, the loopback default is used,
and the refusal is logged once naming the rejected value. To reach a Bridge on
another machine, tunnel it to loopback. This is deliberately stricter than the
HTTP side, which the Bridge's own mesh layer authenticates.
