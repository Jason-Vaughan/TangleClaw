...
| `authEnabled` | boolean | `false` | The master switch for **both** doors this install can have. **(1) TangleClaw's own session gate** (#1418, #1420): enforced in-process on *every* ingress mode **whenever this flag is true**. With no account in the `users` table the install is `account-required` — closed, and a browser is shown a page that creates the first account (or run `node scripts/reset-admin.js --store --user <name>`). With accounts that are all disabled it is `locked` — closed, recovered with the same command. A store or config read failure also enforces. **(2) Caddy's `basic_auth` gate**, which exists only in caddy ingress mode and only once the cutover has regenerated the Caddyfile; there the *Caddyfile* is the authority on what is actually enforced, and it and this flag can legitimately disagree. So "is a login enforced here?" is answered by asking both — `GET /api/auth/me` reports (1) as `gateState` (`open`, `account-required`, `armed`, `locked` or `unreadable`), and its `gateActive` is true for every state except `open`. Setting this **false** in a readable config file is the documented way out of a misconfigured TangleClaw gate: it takes effect on the next request. **Not settable through `PATCH /api/config`** — that route answers `409 CREDENTIAL_ROUTE_MOVED` for this field and the two below. Change the login via the settings surface, or `node scripts/reset-admin.js`. Meaning ratified in ADR 0016 OQ3. |
