...
| `versionBumpEnabled` | boolean | `true` | Legacy switch `releaseMode` replaces, still read when `releaseMode` is unset: `false` means `off`. A settings save carrying either key writes both (the settings modal sends this one on every save), with `versionBumpEnabled` `true` only for `auto`, so an older TangleClaw reading only this key holds rather than cuts |
