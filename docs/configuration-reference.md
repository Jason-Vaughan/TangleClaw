...
```json
{
  ...
  "capabilities": {
    ...
    "wake": {
      ...
      "decorativePattern": "string — OPTIONAL. Regex source matching cells this engine ANIMATES while idle (decoration the operator did not type). Both idle gates discount them: the transcript digest blanks them to spaces, the composer scan skips them. Declared only where an idle pane was watched and seen to move — codex's braille shimmer (#1344). Refused unless it matches decoration only: a pattern matching the empty string or any printable ASCII character is rejected (that is the range an operator types), so it cannot disable both gates silently.",
