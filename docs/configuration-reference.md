...
| `authEnabled` | boolean | `false` | The master switch for **both** doors this install can have. **(1) TangleClaw's own session gate** (#1418): enforced in-process on *every* ingress mode, and live when this flag is true **and** at least one enabled account exists in the `users` table — create one with `node scripts/reset-admin.js --store --user <name>`. **(2) Caddy's `basic_auth` gate**, which exists only in caddy ingress mode and only once the cutover has regenerated the Caddyfile; there the *Caddyfile* is the authority on what is actually enforced, and it and this flag can legitimately disagree. So "is a login enforced here?" is answered by asking both — `GET /api/auth/me` reports `gateActive` for (1). Setting this **false** is the documented way out of a misconfigured TangleClaw gate: it takes effect on the next request. **Not settable through `PATCH /api/config`** — that route answers `409 CREDENTIAL_ROUTE_MOVED` for this field and the two below. Change the login via the settings surface, or `node scripts/reset-admin.js`. Meaning ratified in ADR 0016 OQ3. |
