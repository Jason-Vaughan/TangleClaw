...
function refreshStatusBars(deps = {}) {
  ...
  for (const session of list()) {
    try {
      ...
      // Not ours, or already correct. Re-stamping an unbranded bar would put
      // TangleClaw's name on a session someone else started.
      if (!current || !current.includes(STATUS_BRAND) || current.includes(`v${version}`)) {
        skipped++;
        continue;
      }
