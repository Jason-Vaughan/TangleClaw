...
function refreshStatusBars(deps = {}) {
  ...
  // Both outcomes are logged. "Touched nothing" is the shape a broken refresh
  // takes — a changed status-left format, or tmux answering in a way the guard
  // no longer recognizes — and it is indistinguishable from a healthy no-op
  // unless it says which one it was.
  if (updated > 0) {
    log.info('Refreshed session status bars', { updated, skipped, version });
  } else if (skipped > 0) {
    log.info('Session status bars already current', { skipped, version });
  }
