...
function refreshStatusBars(deps = {}) {
  ...
  for (const session of list()) {
    try {
      ...
      // Already correct. Matched on the whole label including its trailing
      // space, not on a bare `v<version>` — that substring is also contained in
      // every longer version sharing the prefix, so a `v4.3` bar would read as
      // already-current on a `v4.35.0` server and never be corrected. The
      // colour prefix is deliberately not compared: a bar is current when it
      // names the right version, whatever it is painted.
      if (current.includes(`${STATUS_BRAND} v${version} `)) {
        skipped++;
        continue;
      }
  ...
  if (updated > 0) log.info('Refreshed session status bars', { updated, skipped, version });
