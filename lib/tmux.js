...
function createSession(name, options = {}) {
  ...
  // Configure status bar — label left side, show time/date on right
  try {
    _exec(`tmux set-option -t ${_escapeArg(name)} status-left ${_escapeArg(' tmux: #{session_name} ')}`);
    _exec(`tmux set-option -t ${_escapeArg(name)} status-right ${_escapeArg(' %H:%M  %Y-%m-%d ')}`);
  } catch (err) {
