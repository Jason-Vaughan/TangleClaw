...
function createSession(name, options = {}) {
  ...
  // Configure status bar — show "TangleClaw" on left, time/date on right
  try {
    _exec(`tmux set-option -t ${_escapeArg(name)} status-left ${_escapeArg('#[fg=#8BC34A,bold] TangleClaw ')}`);
...
function _resolveMouseValue(sessionVal, globalVal) {
  const effective = sessionVal === '' ? globalVal : sessionVal;
  return effective === 'on';
}
...
function getMouse(session) {
  ...
  try {
    ...
    return _resolveMouseValue(sessionVal, globalVal);
  } catch {
    return false;
  }
}
