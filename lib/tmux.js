...
/**
 * Clear whatever is typed at a session's prompt, preserving it in the log first.
 *
 ...
 * line-kill simply ignores it, which is why the capture below is the part that
 * makes this honest — the draft survives as evidence either way. Never throws;
 * a failure to read or clear must not stop the caller's delivery.
 * @param {string} session - Session name
 * @returns {string} The prompt line as it stood before clearing (may be empty).
 */
function _clearPromptLine(session) {
  let before = '';
  try {
    const { lines } = capturePane(session, { lines: 5 });
    for (let i = lines.length - 1; i >= 0; i--) {
      const line = (lines[i] || '').trim();
      if (line) { before = line; break; }
    }
  } catch (err) {
    log.warn('Could not capture the prompt before clearing it', { session, error: err.message });
  }
  ...
  if (before) {
    log.info('Cleared the prompt before injecting', { session, promptBeforeClear: before });
  }
  return before;
}
 ...
 * idle with it queued.
 *
 * Whatever stood at the prompt is captured and logged before it is cleared, so
 * a discarded draft is recoverable from the log rather than silently destroyed.
