...
const _draftSeams = {
  ...
  saveDraft: (attemptKey, draft) => require('./draft-store').saveDraft(attemptKey, draft),
  sessionCreatedAt: (session) => sessionCreatedAt(session),
  // Lets the engine repaint after a line-kill before the composer is re-read.
  settle: () => execSync('sleep 0.2')
};

/**
 * The attempt a draft belongs to: the caller's session id when it has one, else
 * the pane's own identity (its name and tmux creation time), which a relaunch
 * under the same name changes. Null when neither can be established.
 *
 * @param {string} session - tmux session name.
 * @param {string|null} attemptKey - The caller's key (`draftStore.sessionAttemptKey`).
 * @returns {string|null}
 */
function _draftAttemptKey(session, attemptKey) {
  if (attemptKey) return attemptKey;
  const created = _draftSeams.sessionCreatedAt(session);
  return created && created.createdAt ? `${session}@${created.createdAt}` : null;
}
 ...
 * destroyed unrecorded. The reading itself is `medusa-wake.readComposerDraft`,
 * the one definition of where the composer is and what in it was typed; this
 * function only gathers the pane, the cursor and the engine's profile for it.
 *
...
function _readDraft(session, engineId) {
  const notLocated = (reason) => ({ state: 'not-located', text: '', reason, draftPresent: null, rows: 0, complete: false });
  ...
  return wake.readComposerDraft(lines, cursor, profile);
}
 ...
 * What that costs is recorded honestly — a draft that was observed is kept in
 * the private draft store (`lib/draft-store.js`) and the log says where, an
 * empty composer records nothing, and a composer that could not be read logs
 * that no draft was captured and why (#1507).
 *
 ...
 * @param {string|null} [attemptKey] - The attempt the pane belongs to, which keys
 *   the draft store; derived from the pane itself when absent.
 * @returns {{state: string, text: string, reason: (string|null), draftPresent: (boolean|null), rows: number, complete: boolean}} From `_readDraft`.
 */
function _clearPromptLine(session, engineId, attemptKey = null) {
  let draft;
  try {
    ...
    draft = { state: 'not-located', text: '', reason: `the pane could not be read: ${err.message}`, draftPresent: null, rows: 0, complete: false };
  }

  // A composer holding only the switchboard's own nudge, stranded by a lost
  // Enter, holds nothing the operator typed (#1621). Keeping it would file the
  // switchboard's text in the operator's draft store on every recovery.
  let ownNudge = false;
  if (draft.state === 'draft' && draft.complete) {
    try {
      ownNudge = require('./medusa-wake').isOwnNudge(draft.text);
    } catch (err) {
      // prawduct:allow prawduct/broad-except -- the check runs ahead of a delivery and must never stop one; an unchecked draft is kept, the safe default
      log.warn('Could not check whether the prompt holds a stranded switchboard nudge; keeping it as a draft', { session, error: err.message });
    }
  }

  // Kept BEFORE the clear, so the draft is on disk by the time `C-u` destroys
  // it. The text goes to the private draft store, never the log: logs carry
  // names, never payloads, and a half-typed line can hold a secret.
  let kept = null;
  if (draft.state === 'draft' && !ownNudge) {
    try {
      const key = _draftAttemptKey(session, attemptKey);
      if (!key) throw new Error('the pane\'s attempt could not be identified');
      kept = _draftSeams.saveDraft(key, { engineId: engineId || null, text: draft.text, rows: draft.rows, complete: draft.complete });
    } catch (err) {
      log.warn('A draft at the prompt could not be kept before clearing it', { session, engineId, error: err.message });
    }
  }
  ...
  if (ownNudge) {
    // A stranded nudge wraps across several rows, and whether one line-kill
    // clears them all is the engine's behaviour, not ours. Anything left would
    // be submitted together with the fresh paste as one garbled instruction, so
    // the composer is read again and the caller refuses to paste over leftovers.
    let after;
    try {
      _draftSeams.settle();
      after = _readDraft(session, engineId);
    } catch (err) {
      // prawduct:allow prawduct/broad-except -- an unreadable composer after the clear is treated as leftover text, the safe direction
      after = { state: 'not-located', reason: err.message };
    }
    if (after.state !== 'empty') {
      log.warn('A stranded switchboard nudge was not fully cleared from the prompt; not pasting after it', {
        session, engineId, rows: draft.rows, after: after.state, reason: after.reason || null
      });
      return { ...draft, strandedNudgeResidue: true };
    }
    log.info('Cleared a stranded switchboard nudge from the prompt before injecting', { session, engineId, rows: draft.rows });
  } else if (kept) {
    // An opaque reference and a size — never the text, nor anything derived
    // from it: a digest of a short draft confirms a guess at it.
    log.info('Cleared a draft from the prompt before injecting; kept in the draft store', {
      session, engineId, draftRef: kept.draftRef, rows: kept.rows, chars: kept.chars
    });
 ...
 * profile and kept in the private draft store before it is cleared, so a
 * discarded draft is recoverable; where the composer cannot be located, the log
 * says so rather than recording something else as the draft.
 ...
 * @param {string|null} [options.attemptKey] - The attempt the pane belongs to
 *   (`draftStore.sessionAttemptKey(sessionId)`), which keys the draft store.
 *   Without it the pane's own name and creation time are used.
 */
function sendKeys(session, text, options = {}) {
  ...
  if (options.clearPrompt !== false) {
    const cleared = _clearPromptLine(session, options.engineId, options.attemptKey || null);
    if (cleared && cleared.strandedNudgeResidue) {
      throw new Error('A stranded switchboard nudge was not fully cleared from the prompt; refusing to paste after it');
    }
  }
