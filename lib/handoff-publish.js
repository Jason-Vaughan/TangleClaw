...
/**
 * The answer for a replayed finalize of an attempt already published.
 *
 * Which publication it displaced when it was published is not recorded on its
 * own row, so it is answered as unknown (null) rather than guessed. A row that
 * has since been superseded names the publication that took over — the winner,
 * in the winner's slot. An earlier version returned that winner as
 * `supersededId`, which reads as the publication this one displaced: the
 * opposite direction.
 * @param {object} row - The published row
 * @returns {{published: true, reason: string, supersededId: null, supersededById: (string|null)}}
 */
function _alreadyPublished(row) {
  return { published: true, reason: 'already published', supersededId: null, supersededById: row.supersededBy || null };
}
 ...
 * @returns {{published: boolean, reason: string|null, supersededId: string|null, supersededById: string|null}}
 *   Directional (#1675): `supersededId` is the publication this one DISPLACED as current;
 *   `supersededById` is the newer publication this one LOST to. Never the same slot.
 */
function publishHandoff(project, publicationId) {
  const refuse = (reason) => {
    ...
    return { published: false, reason, supersededId: null, supersededById: null };
  };
  ...
  if (rowState && rowState.already) {
    return _alreadyPublished(rowState.already);
  }
  ...
  try {
    return store.handoffs.transaction(() => {
      ...
      if (fresh.state === 'published') {
        return _alreadyPublished(fresh);
      }
      if (fresh.state !== 'staged') {
        return { published: false, reason: `publication is ${fresh.state}`, supersededId: null, supersededById: null };
      }
      ...
      if (newer) {
        ...
        return {
          ...
          supersededId: null,
          supersededById: newer.publicationId
        };
      }
      ...
      return { published: true, reason: null, supersededId: previousId, supersededById: null };
    });
  } catch (err) {
...
function recordPromotedHandoff(project, publicationId) {
  ...
  try {
    return store.handoffs.transaction(() => {
      ...
      if (currentMismatch) return { published: false, reason: currentMismatch, supersededId: null, supersededById: null };
 ...
 * @returns {{outcomes: Array<{publicationId: string|null, action: string|null, applied: boolean, reason: string|null, supersededId: string|null, supersededById: string|null}>, appliedCount: number}}
 */
function applyHandoffRepairs(project, proposals) {
  ...
  for (const { publicationId, action } of publishable) {
    ...
    try {
      ...
      result = { published: false, reason: `repair failed: ${err.message}`, supersededId: null, supersededById: null };
    }
    outcomes.push({
      publicationId,
      action,
      ...
      supersededById: result.supersededById || null
    });
