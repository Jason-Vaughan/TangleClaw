  ...
  try {
    return store.handoffs.transaction(() => {
      ...
      if (current.outcome !== 'ok') {
        return { published: false, reason: `current.json is ${current.outcome}: ${current.reason || 'unreadable'}`, supersededId: null };
      }
      if (current.doc.publicationId !== publicationId) {
        return { published: false, reason: 'current.json names a different publication', supersededId: null };
      }
      if (current.digest !== row.fileDigest) {
        return { published: false, reason: 'current.json digest does not match the recorded digest', supersededId: null };
      }
      if (current.doc.kind !== row.kind) {
        return { published: false, reason: 'current.json kind does not match the recorded kind', supersededId: null };
      }
