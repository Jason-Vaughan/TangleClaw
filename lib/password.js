 ...
 *
 * The same `salt:hash` output as {@link hashPassword}, with the derivation on
 * libuv's threadpool. It exists for `POST /api/auth/set-password`, which anyone
 * who can reach an install with no account yet may call: a synchronous hash
 * there lets a burst of submissions stall every other request, the same reason
 * the login route verifies asynchronously.
 *
 * @param {string} password - Plaintext password
...
module.exports = { hashPassword, hashPasswordAsync, verifyPassword, verifyPasswordAsync, SALT_BYTES, KEY_BYTES };
