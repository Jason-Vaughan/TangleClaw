...
// Named `password_` because `password` reads as a value at every call site in
// this file; the trailing underscore keeps the module distinguishable from the
// plaintext arguments it is handed.
...
function _runMigrations() {
  ...
  if (currentVersion < CURRENT_SCHEMA_VERSION) {
    ...
    if (currentVersion < 36) {
      ...
      // already had.
      //
      // So what this block is FOR is the postcondition. It advances the schema
      // version only after reading the shape back out of sqlite_master, which
      // means a future edit that drops the UNIQUE from the DDL fails the
      // upgrade loudly instead of leaving a login table that accepts duplicate
      // usernames. The CREATE stays, idempotent, so the block is still correct
      // in isolation if `_createTables` ever stops carrying it.
      _db.exec('BEGIN IMMEDIATE');
      try {
        _db.exec(
          'CREATE TABLE IF NOT EXISTS users ('
        ...
        if (!ddl || !ddl.sql || !/username[^,]*UNIQUE/i.test(ddl.sql)) {
          throw new Error(
...
const usersApi = {
  ...
  create(username, password) {
    ...
    const hash = password_.hashPassword(password);
    try {
      const info = _db.prepare(
   ...
   * name is the one about to verify a password against it. Callers that just
   * want to know a name is taken should use `exists`.
   *
  ...
  /**
   * Whether a username is taken, disabled accounts included.
   * @param {string} username
   * @returns {boolean}
   */
  exists(username) {
    return usersApi.getByName(username) !== null;
  },
  ...
  setPassword(username, password) {
    ...
    const info = _db.prepare('UPDATE users SET password_hash = ? WHERE username = ?')
      .run(password_.hashPassword(password), name);
   ...
   * unknown account, disabled account — so a caller cannot accidentally
   * distinguish them and turn this into a username oracle. The reason is logged
   * at debug for the operator, never returned.
   *
  ...
  verify(username, password) {
    ...
    if (!row) {
      log.debug('User verify failed', { reason: 'no such user' });
      return null;
    }
    if (row.disabled_at) {
      log.debug('User verify failed', { username: row.username, reason: 'disabled' });
      return null;
    }
    if (!password_.verifyPassword(password, row.password_hash)) {
      log.debug('User verify failed', { username: row.username, reason: 'bad password' });
      return null;
    }
