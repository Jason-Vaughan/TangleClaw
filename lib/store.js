...
const DEFAULT_PROJECT_CONFIG = {
  ...
  // #139 Chunk 3 — wrap_pipeline runner opt-in. While `false` (default
  // through Chunk 10), `lib/sessions.js:triggerWrap` runs the legacy
  // NL-prompt path byte-equal to pre-#139 behavior. Flipping to `true`
  // routes through `lib/wrap-pipeline.js:runWrapPipeline`. Chunk 11 will
  // flip the default. Real step implementations land in Chunks 4–9; the
  // runner shipped here dispatches no-op stubs only — opting in early is
  // safe but produces no useful output until later chunks.
  wrapV2: false,
