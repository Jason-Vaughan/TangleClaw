  ...
  if (currentVersion < CURRENT_SCHEMA_VERSION) {
    ...
    if (currentVersion < 41) {
      ...
      try {
        ...
        if (!pubSql) {
          throw new Error('v40→41 did not create handoff_publications. Refusing to advance schema_version.');
        }
        ...
        if (!/UNIQUE\s*\(\s*session_id\s*,\s*wrap_run_id\s*\)/i.test(pubSql)) {
          throw new Error('v40→41 did not create handoff_publications with UNIQUE (session_id, wrap_run_id). Refusing to advance schema_version.');
        }
        ...
        if (!/seq[^,]*UNIQUE/i.test(pubSql)) {
          throw new Error('v40→41 did not create handoff_publications with a UNIQUE seq. Refusing to advance schema_version.');
        }
        ...
        if (!/UNIQUE/i.test(finalSql) || !/eligible_at IS NOT NULL/i.test(finalSql)) {
          throw new Error('v40→41 did not create the partial unique index that keeps one eligible final per session. Refusing to advance schema_version.');
        }
        _db.exec('COMMIT');
      } catch (err) {
        try { _db.exec('ROLLBACK'); } catch { /* nothing to roll back */ }
 ...
 * @param {object} seq - `{launchId, pageBudget, applicability, notApplicableReason, preflight, sourceManifest, steps}`
...
function _insertLaunchSequence(sessionId, projectId, engineId, seq) {
  ...
  _db.prepare(
    `INSERT INTO launch_sequences (launch_id, session_id, project_id, engine_id, page_budget,
       applicability, not_applicable_reason, preflight, source_manifest)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    seq.launchId,
    sessionId,
...
const launchSequencesApi = {
  ...
  revise(sequenceId, { revision, cursor, sourceManifest, steps }) {
    ...
    _db.prepare(
      'UPDATE launch_sequences SET revision = ?, cursor = ?, source_manifest = ? WHERE id = ?'
