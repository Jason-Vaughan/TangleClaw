...
/**
 * Run `fn` atomically: inside the caller's open transaction when there is one
 * (`insertFire` runs inside the fire service's), else in one of its own.
 * @param {() => *} fn - Synchronous work.
 * @returns {*}
 */
function _startupControlAtomic(fn) {
  return _db.isTransaction ? fn() : _startupControlTransaction(fn);
}
