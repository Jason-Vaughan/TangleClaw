...
const usersApi = {
   ...
   *
   * The only writer the first-account screen uses. `create` alone cannot serve
   * it: the gate offers that screen while no row exists, and a "check the table,
   ...
   *
   * The password is hashed BEFORE the transaction opens, so the write lock is
   * never held across a scrypt computation.
   *
