  ...
  if (currentVersion < CURRENT_SCHEMA_VERSION) {
    ...
    if (currentVersion < 23) {
      ...
      try {
        ...
        try { _db.exec('ROLLBACK'); } catch { /* no transaction in progress */ }
        // A fresh DB created by this version already carries the CHECK from
        // _createTables, so the rebuild is unnecessary; defer to the postcondition.
        log.debug('Migration v22→v23 recreate skipped (table may already carry the CHECK)', { error: err.message });
      }
      ...
      if (!ddl || !/CHECK\s*\(\s*op\s+IN/i.test(ddl.sql)) {
        throw new Error(
          ...
          `(found: ${ddl ? ddl.sql : 'table missing'}). Aborting — schema_version will NOT ` +
          `advance to 23 until this is resolved. See SR-3MW8.`
        );
      }
