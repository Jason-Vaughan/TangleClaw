 ...
 * @type {string[]}
 */
const SESSION_RULE_KINDS = ['startup', 'wrap'];
...
const sessionRulesApi = {
   ...
   * @param {string} [options.kind] - CC-6: filter by rule kind ('startup'|'wrap')
   * @returns {object[]}
   */
  list(options = {}) {
    _ensureDb();
  ...
  /**
   * Create a new session rule. Rules are always project-scoped — the former
   * global tier (projectId null) was retired in favor of the Global rules
   * document; cross-project directives belong there.
   * @param {object} data
   ...
   * @param {string} [data.kind] - CC-6: 'startup' (default) | 'wrap'
