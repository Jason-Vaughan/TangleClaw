 ...
 */
/**
 * Narrow the database file to owner-only.
 *
 * `new DatabaseSync` creates the file at the process umask, which on a default
 * macOS account is 0644 — world-readable. That was survivable while the file
 * held project metadata; it stopped being survivable when it started holding
 * password hashes. Run on every `init`, not only on create, so an install that
 * predates this gets narrowed on its next boot.
 *
 * Best-effort: a filesystem that cannot represent the mode (a network mount, a
 * Docker volume) must not stop the server starting. `_checkPermissions` warns
 * separately, so a failure here is still visible rather than silent.
 * @returns {void}
 */
function _tightenDbPermissions() {
  try {
    ...
    log.debug('Could not tighten database file permissions', {
...
function _checkPermissions() {
  try {
    ...
    // The database holds login credentials (ADR 0015), so unlike config.json it
    // has no 0644 allowance: group- or world-readable means any local account
    // can copy `users.password_hash` and attack it offline at leisure. Salted
    // scrypt makes that expensive rather than impossible, and the reach is
    // local-only, which is why this warns rather than refuses — but `init`
    // chmods it to 0600, so reaching this warning means something outside
    // TangleClaw reopened the file.
    if (fs.existsSync(_dbFile)) {
      const dbMode = fs.statSync(_dbFile).mode & 0o777;
      if (dbMode !== 0o600) {
        log.warn('Database file permissions are too open', {
          path: _dbFile,
          mode: '0' + dbMode.toString(8),
          recommended: '0600'
        });
      }
    }
...
const portLeasesApi = {
  ...
  lease(data) {
    ...
    _db.prepare(`
      INSERT INTO port_leases (host, port, project, service, status, permanent, ttl_ms, expires_at, last_heartbeat, description, auto_renew, reach)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, datetime('now'), ?, ?, ?)
    ...
    `).run(host, data.port, data.project, data.service, status, permanent, ttlMs, expiresAt, description, autoRenew, reach);
...
// A throwaway scrypt hash, computed once at module load, that `verify` compares
// against when there is no account to compare against. Its VALUE is irrelevant
// and nothing may ever match it — its only job is to cost what a real
// comparison costs, so a missing or disabled account takes the same time as a
// wrong password.
const ABSENT_USER_HASH = passwordHashing.hashPassword(crypto.randomBytes(32).toString('hex'));
...
const usersApi = {
  ...
  setPassword(username, password) {
    ...
    if (info.changes > 0) log.info('User password changed', { username: name });
   ...
   * gets sampled. `ABSENT_USER_HASH` exists only to be the thing those paths
   * compare against.
   *
  ...
  verify(username, password) {
    ...
    if (!row || row.disabled_at) {
      ...
      passwordHashing.verifyPassword(typeof password === 'string' ? password : '', ABSENT_USER_HASH);
  ...
  disable(username) {
    ...
    if (info.changes > 0) log.info('User disabled', { username: name });
    return info.changes > 0;
  },
  ...
  enable(username) {
    ...
    if (info.changes > 0) log.info('User enabled', { username: name });
    return info.changes > 0;
  },
 ...
 * the cap and rotates; a rare forensic type (`wrap.auto_pr`, 45 rows) never
 * reaches it and survives indefinitely — without anyone maintaining an exemption
 * list that the next `activity.log` call site would silently fall outside of.
 * Rarity exempts itself, and a brand-new event type is bounded on its first
 * insert with nothing to update.
 *
