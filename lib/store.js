...
function _copyBundledFiles(srcDir, destDir, _pattern) {
  ...
  for (const file of files) {
    if (!existing.has(file)) {
      ...
      // Merge new fields from bundled profile into existing — preserves user
      // customizations while backfilling fields added in newer versions (e.g.
      // preKeys, preKeyDelay, startupDelay added to codex.json).
      _mergeBundledProfile(path.join(srcDir, file), path.join(destDir, file));
    }
  }
}

/**
 * Recursively add missing keys from src into target. Never overwrites existing
 * values; recurses into plain objects only (arrays treated as opaque values).
 * @param {object} target
 * @param {object} src
 * @returns {boolean} Whether any keys were added
 */
function _addMissingKeys(target, src) {
  let added = false;
  for (const key of Object.keys(src)) {
    if (!(key in target)) {
      ...
      if (_addMissingKeys(target[key], src[key])) added = true;
    }
  }
  return added;
}
...
/**
 * Whether `arr` is an ordered subsequence of `full` — every element of `arr`
 * appears in `full` in the same relative order. Used to detect uncustomized
 * arrays (e.g. `wrap.steps`) so we can safely adopt the bundled version.
 * @param {Array} arr
 * @param {Array} full
 * @returns {boolean}
 */
function _isOrderedSubsequence(arr, full) {
  let j = 0;
  for (let i = 0; i < arr.length; i++) {
    while (j < full.length && full[j] !== arr[i]) j++;
...
/**
 * Deep-merge bundled profile fields into an existing profile on disk.
 * Adds keys/sub-keys that are missing — never overwrites or deletes user
 * values. One narrowly-scoped exception: a targeted prune for the stale
 * `["2","Enter"]` bypass-mode preKeys in `claude.json` (#119), guarded by
 * exact equality so user-customized preKeys are preserved.
 * @param {string} bundledPath - Path to bundled engine JSON
 * @param {string} livePath - Path to live engine JSON
 */
function _mergeBundledProfile(bundledPath, livePath) {
  let bundled, live;
  try {
  ...
  const merged = _addMissingKeys(live, bundled);
  changed = merged;

  // One-shot prune: remove stale ["2","Enter"] preKeys from claude bypass mode
  // (#119). Older bundled profiles dismissed Claude Code's "press 2 to confirm
  // dangerous mode" dialog with these keys, but the current CLI bypasses with
  // no prompt — so `2` + Enter lands in chat as a stray first user message.
  // Equality match preserves any user-customized preKeys; we only strip the
  // exact stale default. Bundled file no longer carries these fields, so the
  // recursive addMissing pass above will not re-add them. Filename guard
  // (rather than `live.id === 'claude'`) self-heals files whose `id` may have
  // drifted from a hand-edit.
  let pruned = false;
  if (
    path.basename(livePath) === 'claude.json' &&
    live && live.launchModes && live.launchModes.bypassPermissions
  ) {
    const bp = live.launchModes.bypassPermissions;
    if (
      Array.isArray(bp.preKeys) && bp.preKeys.length === 2 &&
      bp.preKeys[0] === '2' && bp.preKeys[1] === 'Enter'
    ) {
      delete bp.preKeys;
      delete bp.preKeyDelay;
      pruned = true;
      changed = true;
      log.info('Pruned stale bypass-mode preKeys (#119)', { file: path.basename(livePath) });
    }
  }
  ...
  if (changed) {
    ...
    if (merged) {
      log.info('Merged new bundled fields into engine profile', { file: path.basename(livePath) });
    } else if (pruned) {
      log.debug('Rewrote engine profile after prune', { file: path.basename(livePath) });
    }
  }
}

/**
 * Deep-merge bundled methodology template fields into a live template.json on
 * disk. Adds missing keys/sub-keys (e.g. fields introduced after the user's
 * install) without overwriting user values. Additionally reconciles
 * `wrap.steps` (#136): when the live array is an ordered subsequence of the
 * bundled array, the user has not customized it, so we adopt the bundled
 * version to pick up newly-added steps like `memory-update`. Any user-added
 * step (entry not in bundled) leaves the array untouched.
 *
 * **Contract (additive-only, intentional):** the merger cannot distinguish
 * "user is on a stale install" from "user deliberately removed an entry."
 * It treats the bundled template as the source of truth for the additive
 * direction — if the user has explicitly deleted a top-level key or trimmed
 * a wrap.steps entry that the bundled template still ships, the deletion is
 * silently re-added on next server start. This mirrors `_mergeBundledProfile`
 * (#119) and keeps existing installs reachable by future bundled fixes; users
 * who genuinely need to remove behavior should disable it via project config
 * rather than mutating the runtime template.
 * @param {string} bundledPath - Path to bundled template.json
 * @param {string} livePath - Path to live template.json
 */
function _mergeBundledTemplate(bundledPath, livePath) {
  ...
  if (_addMissingKeys(live, bundled)) {
    changed = true;
  }

  if (
    bundled.wrap && Array.isArray(bundled.wrap.steps) &&
    live.wrap && Array.isArray(live.wrap.steps)
  ) {
    const liveSteps = live.wrap.steps;
    const bundledSteps = bundled.wrap.steps;
    const stepsDiffer =
      liveSteps.length !== bundledSteps.length ||
      liveSteps.some((s, i) => s !== bundledSteps[i]);
    if (stepsDiffer && _isOrderedSubsequence(liveSteps, bundledSteps)) {
      const added = bundledSteps.filter((s) => !liveSteps.includes(s));
      live.wrap.steps = [...bundledSteps];
      changed = true;
      log.info('Adopted updated wrap.steps from bundled template (#136)', {
        id: live.id,
        added
      });
    }
  ...
  if (changed) {
    fs.writeFileSync(livePath, JSON.stringify(live, null, 2) + '\n', { mode: 0o600 });
    log.info('Merged bundled methodology template into runtime', { id: live.id });
  }
}

/**
 * Copy bundled methodology templates to target directory. New files (or fully
 * missing template directories) are copied as-is; an existing `template.json`
 * is reconciled in place via `_mergeBundledTemplate` so post-install changes
 * to bundled templates reach existing installs (#136).
  ...
  for (const dir of dirs) {
    ...
    for (const file of srcFiles) {
      ...
      if (!fs.existsSync(destFile)) {
        ...
        _mergeBundledTemplate(srcFile, destFile);
      }
    }
    if (copied > 0) {
...
const sessionsApi = {
  ...
  start(data) {
    ...
    const stmt = _db.prepare(
      `INSERT INTO sessions (project_id, engine_id, tmux_session, prime_prompt, methodology_phase, session_mode, launch_mode)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    );
    stmt.run(
      data.projectId,
...
const globalRulesApi = {
   ...
   * Creates from defaults if missing. Auto-heals legacy polluted files
   * by rewriting them in-place with normalized content (#100).
   * @returns {string}
   */
  load() {
    ...
    try {
      if (fs.existsSync(userFile)) {
        ...
        if (normalized !== raw) {
          try {
            fs.writeFileSync(userFile, normalized, 'utf8');
          } catch (err) {
            log.warn('Failed to auto-heal global rules', { path: userFile, error: err.message });
          }
        }
        return normalized;
      }
    ...
    try {
      const defaults = _normalizeRulesContent(fs.readFileSync(BUNDLED_GLOBAL_RULES, 'utf8'));
  ...
  /**
   * Save updated global rules content. Normalized before persisting (#100).
  ...
  save(content) {
    ...
    fs.mkdirSync(path.dirname(userFile), { recursive: true });
    fs.writeFileSync(userFile, normalized, 'utf8');
    activityApi.log({ eventType: 'rules.global_updated', detail: { length: normalized.length } });
  },
...
const openclawConnectionsApi = {
  ...
  create(data) {
    ...
    const bridgePort = data.bridgePort || 3201;
    const bridgeToken = data.bridgeToken || null;

    try {
      _db.prepare(
        `INSERT INTO openclaw_connections (id, name, host, port, ssh_user, ssh_key_path, gateway_token, cli_command, local_port, available_as_engine, default_mode, audit_secret, bridge_port, bridge_token)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
      ).run(id, name, host, port, sshUser, sshKeyPath, gatewayToken, cliCommand, localPort, availableAsEngine, defaultMode, auditSecret, bridgePort, bridgeToken);
    } catch (err) {
      if (err.message && err.message.includes('UNIQUE')) {
...
function _rowToConnection(row) {
  return {
    ...
    bridgePort: row.bridge_port || 3201,
