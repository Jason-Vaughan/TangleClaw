...
const usersApi = {
  ...
  disable(username) {
    ...
    if (info.changes > 0) {
      const killed = authSessionsApi.destroyForUser(name);
      log.info('User disabled', { username: name, sessionsDestroyed: killed });
    }
    return info.changes > 0;
  },
...
const authSessionsApi = {
   ...
   * for the rows nobody comes back for — a browser closed for good leaves a row
   * that is never resolved again. Called at boot rather than on a timer: the
   * table is tiny, and this repo has a standing rule against timer-driven
   * lifecycle.
   *
   * @param {number} [now] - Epoch ms; injectable for tests
  ...
  /**
   * Whether any account exists that could be logged into.
   *
   * This is the predicate the gate activates on, and it asks about ENABLED
   * accounts specifically. An install whose only account is disabled has no
   * key to its own door, so treating it as gated would lock the operator out
   * with no route back that does not need a shell on the machine.
   *
   * @returns {boolean}
   */
  anyLoginableUser() {
    const row = _db.prepare(
      'SELECT 1 AS present FROM users WHERE disabled_at IS NULL LIMIT 1'
    ).get();
    return Boolean(row);
  }
};
