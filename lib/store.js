 ...
 * refusal is logged, and the caller can read `.status` on what it gets back to
 * see the session was already over.
 *
...
const sessionsApi = {
   ...
   *
   * A session that has already ended is left alone — see `_transitionSession`.
   * The returned row is then the one already on disk, so a caller that needs to
   * know whether IT did the wrapping reads `.status`.
   * @param {number} id - Session id
   * @param {string} [summary] - Wrap summary markdown
   * @returns {object|null} - The session as it now stands
   */
  wrap(id, summary) {
    _ensureDb();
