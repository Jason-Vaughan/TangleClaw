...
const DEFAULT_PROJECT_CONFIG = {
  ...
  wrapSections: null,
  // Per-step wrap overrides, keyed by `wrap_pipeline.steps[].id` — the only
  // way a project can turn off or reconfigure an individual wrap step without
  // forking its whole methodology. It lives here rather than in the template
  // because `FRAMEWORK_OWNED_PATHS` clone-replaces `wrap_pipeline.steps`
  // wholesale on a framework revision bump, so a template edit is destroyed at
  // the next boot; nothing that syncs templates writes this file.
  //
 ...
 *     (e.g. #264 flipped a step's `blocker: false`→`"errors-only"` and
