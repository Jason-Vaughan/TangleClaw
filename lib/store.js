...
/**
 * Default per-eventType retention for `activity_log` (#869).
 * A per-type cap is the mechanical rule that distinguishes churn from forensic history:
 * high-volume types (e.g. `port.leased`) hit the cap and rotate, bounding the table's
 * growth, while rare forensic types (e.g. `wrap.auto_pr`) naturally stay below the cap
 * and survive indefinitely. This avoids a time-based TTL that would age out the exact
 * rare rows an operator needs when a stale branch is finally inspected, and requires
 * no hand-written whitelist that a new event type might miss.
 *
 * Scoped per `event_type`, not `project_id`, because the Project Master writes
 * NULL-project rows that a project-keyed prune would leave unbounded.
 * @type {number}
 */
...
/**
 * Test seam to override the per-eventType retention.
 * @param {number} n - Rows to keep per event_type; <= 0 keeps all
 */
...
/**
 * Trim activity_log history for a single event_type to the newest `keep` rows.
...
const activityApi = {
  ...
  log(event) {
    try {
      ...
      if (pruned > 0) {
        log.warn('Retention sweep pruned activity log', { eventType: event.eventType, count: pruned });
        if (event.eventType !== 'activity.pruned') {
          activityApi.log({
            eventType: 'activity.pruned',
            detail: { prunedType: event.eventType, count: pruned }
          });
        }
      }
    } catch (err) {
