...
const STARTUP_FIRE_REASON_CODES = Object.freeze([
  ...
  // Chunk B2 (the Codex adapter): what stopped a fire before a send, what an
...
function _createTables() {
  _db.exec(`
    ...
    -- The native channel a launch's engine was given (Chunk B2): the
    ...
    -- Codex contract (S4). One open channel per session; sequence_id names
 ...
 * engine echoes back as the receipt's binding (Chunk B2, E8).
...
/**
 * v45→v46 (#1825 Chunk B2): the receipt columns on `startup_prompt_fires`
 ...
 */
function _verifyStartupControlTables() {
  const ddl = (name) => {
    const row = _db.prepare('SELECT sql FROM sqlite_master WHERE name = ?').get(name);
    return row && row.sql ? row.sql : '';
  };
  const refuse = (what) => {
    throw new Error(`v45→v46 left ${what}. Refusing to advance schema_version.`);
  };
...
const startupPromptsApi = {
   ...
   * A fire's checks and its intent row are one decision; this keeps two
   * concurrent fires from both passing the checks.
...
/**
 * The native channels TangleClaw opened for launches (Chunk B2). The adapter
...
const startupControlChannelsApi = {
  ...
  /**
   * Run `fn` inside one `BEGIN IMMEDIATE` transaction.
   * @param {() => *} fn - Synchronous work.
   * @returns {*}
   */
  transaction(fn) {
