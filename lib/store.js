...
function _createTables() {
  _db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS projects (
      ...
      engine_id     TEXT    NOT NULL DEFAULT 'claude-code',
...
function _runMigrations() {
  ...
  if (currentVersion < CURRENT_SCHEMA_VERSION) {
    ...
    // Future migrations go here: if (currentVersion < 2) { migrate_v1_to_v2(); }
    _db.prepare('INSERT INTO schema_version (version) VALUES (?)').run(CURRENT_SCHEMA_VERSION);
  }
...
function _copyBundledTemplates(srcDir, destDir) {
  ...
  const existing = fs.existsSync(destDir)
    ? fs.readdirSync(destDir, { withFileTypes: true }).filter((d) => d.isDirectory())
    : [];
...
const projectsApi = {
  ...
  create(data) {
    ...
    const engineId = data.engine || 'claude-code';
