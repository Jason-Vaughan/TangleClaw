...
function _maybeWarnLegacyGlobalRulesFile() {
  ...
  try {
    ...
    const canonical = fs.existsSync(BUNDLED_GLOBAL_RULES)
      ...
      : '';
    if (legacy.trim() === canonical.trim()) return; // no drift, nothing to flag
    log.warn(
      'legacy global-rules file detected and IGNORED (#240); content differs from the tracked canonical source',
      {
        ...
        howToRecover: 'review the diff and re-apply intended changes via the landing-page Global Rules editor or by editing data/global-rules.md directly and committing'
      }
    );
  } catch (err) {
