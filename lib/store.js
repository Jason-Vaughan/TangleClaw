 ...
 * refusing to start.
 *
 * The check and the fix live together on purpose. They were briefly split, with
 * the report in `_checkPermissions` — which `init` calls BEFORE the database is
 * opened, so the first boot after an upgrade warned about a file the same init
 * was two lines from correcting. A report that fires on a condition its own
 * caller repairs teaches the operator to ignore it.
 *
