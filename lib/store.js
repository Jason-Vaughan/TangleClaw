 ...
 * Parsed result is cached; the cache is invalidated by
 * `orchestrationProfiles._clearCache()` (tests) and automatically on a
 * `_setBasePath()` switch.
 */
const orchestrationProfilesApi = {
   ...
   */
  load() {
    if (_orchProfilesCache) return _orchProfilesCache;
    try {
      const raw = fs.readFileSync(_orchProfilesFile, 'utf8');
      const parsed = JSON.parse(raw);
      _orchProfilesCache = parsed && typeof parsed === 'object' && parsed.profiles
        ? parsed
        : { profiles: {} };
    } catch (err) {
      if (err.code !== 'ENOENT') {
      ...
      _orchProfilesCache = { profiles: {} };
    }
    return _orchProfilesCache;
  },

  /** Invalidate the in-memory cache (tests / after a base-path switch). */
  _clearCache() {
    _orchProfilesCache = null;
  }
};
