...
const activityApi = {
  ...
  log(event) {
    try {
      ...
      if (pruned > 1 && event.eventType !== ACTIVITY_PRUNE_EVENT) {
        ...
        // Recorded in the table it just pruned, so the deletion outlives the
        // process that did it (Chunk 01's norm: a sweep that runs unattended
        // names what it displaced). Rare by the threshold above, so it never
        // approaches its own cap. The guard keeps termination obvious rather
        // than merely argued.
        activityApi.log({
