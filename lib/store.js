...
const controlApi = {
  ...
  /**
   * The open assignment a launch is bound to, if any.
   * @param {string} launchId - Launch id
   * @returns {object|null} The raw row
   */
  getOpenForLaunch(launchId) {
    _ensureDb();
    return _db.prepare('SELECT * FROM control_assignments WHERE bound_launch_id = ? AND closed_at IS NULL').get(launchId) || null;
  },
   ...
   * @param {{event_id: string, fact: string, outcome_code?: string|null, actor_principal?: string|null}} r
   * @returns {number} The receipt's `receipt_seq`
   */
  insertReceipt(r) {
    _ensureDb();
    const out = _db.prepare('INSERT INTO control_receipts (event_id, fact, outcome_code, actor_principal) VALUES (?, ?, ?, ?)')
      .run(r.event_id, r.fact, r.outcome_code ?? null, r.actor_principal ?? null);
