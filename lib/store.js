...
const awarenessReceiptsApi = {
  ...
  record(entry = {}) {
    ...
    const info = _db.prepare(
      `INSERT INTO awareness_receipts (project_id, session_id, workspace_id, verb, source)
       VALUES (?, ?, ?, ?, ?)`
    ).run(
      entry.projectId ?? null,
      entry.sessionId ?? null,
   ...
   *   - `unverified` — no receipt; the best delivery row is `unverified`:
   *                    something was pushed blind and nothing observed it land.
   ...
   *   - `unaware`    — no receipt and no delivered/unverified/no-rules row.
   *                    Precisely: no EVIDENCE awareness arrived — the state
   *                    that hid for 12 days in August 2026 and must not be
   *                    able to again. A `skipped` row deliberately does NOT
   *                    soften this: a skip says rules existed and nothing
   *                    reached the session (see the acceptance test).
   *
  ...
  sessionAwareness(sessionId) {
    ...
    const unverified = _db.prepare(
      "SELECT channel, skip_reason FROM session_rule_deliveries WHERE session_id = ? AND outcome = 'unverified' ORDER BY id DESC LIMIT 1"
    ).get(sessionId);
    if (unverified) {
      return {
        state: 'unverified',
        basis: `a blind send through ${unverified.channel} (${unverified.skip_reason || 'no reason recorded'}) and no tc invocation`,
        ...none
      };
    }
