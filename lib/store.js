...
const sessionRuleDeliveriesApi = {
  ...
  markDelivered(id, nowMs) {
    ...
    if (!Number.isFinite(writtenAtMs) || now - writtenAtMs > RECEIPT_MAX_AGE_MS) {
      log.warn('Refused a rules-hook receipt outside the freshness window', {
        deliveryId: numeric, writtenAt: row.created_at, ageMs: Number.isFinite(writtenAtMs) ? now - writtenAtMs : null
      });
      return null;
    }
    const info = _db.prepare(
