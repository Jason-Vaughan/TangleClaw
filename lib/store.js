...
const usersApi = {
  ...
  /**
   * Replace a signed-in account's password and end its OTHER sessions, keeping
   * the one the change was made from — atomically.
   *
   ...
   * other session is still ended, because the reason to change a password is
   * often that someone else may know the old one.
   *
   * The password arrives already HASHED so no scrypt derivation runs while
   * SQLite's write lock is held (as {@link recoveryCodesApi.redeem} does). The
   * account must still be enabled under the lock: a disable that lands between
   * the route's verification and this write must not be undone by it.
   *
   ...
   * @param {number} keepSessionId - The session row the change was made from
   * @returns {{ sessionsEnded: number }|null} null if no enabled account has that id
   */
  replacePasswordKeepingSession(userId, passwordHash, keepSessionId) {
    if (typeof passwordHash !== 'string' || !passwordHash) throw new Error('passwordHash is required');
    if (typeof userId !== 'number' || typeof keepSessionId !== 'number') {
      throw new Error('userId and keepSessionId are required');
    }
    _db.exec('BEGIN IMMEDIATE');
    let username;
    let ended;
    try {
      const row = _db.prepare('SELECT username FROM users WHERE id = ? AND disabled_at IS NULL').get(userId);
      if (!row) {
        _db.exec('ROLLBACK');
        return null;
      }
      username = row.username;
      _db.prepare('UPDATE users SET password_hash = ? WHERE id = ?').run(passwordHash, userId);
      ended = Number(_db.prepare('DELETE FROM auth_sessions WHERE user_id = ? AND id != ?')
    ...
    log.warn('User password changed from the dashboard', { username, otherSessionsEnded: ended });
