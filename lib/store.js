...
function _createTables() {
  _db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS session_rules (
      ...
      created_by  TEXT    NOT NULL DEFAULT 'operator',                -- 'operator' | 'ai' (AI path = D1b)
...
const sessionRulesApi = {
  /**
   * The injection query: active rules that apply to a project — global rules
   * (project_id IS NULL) plus that project's own rules. Used by
   * `engines._getRulesContent` to build the cross-model `## Session Rules`
   * section at session launch.
  ...
  create(data) {
    ...
    _db.prepare(
      'INSERT INTO session_rules (project_id, content, created_by, owner) VALUES (?, ?, ?, ?)'
    ).run(
      data.projectId ?? null,
      data.content.trim(),
    ...
    activityApi.log({
      ...
      detail: { scope: rule.projectId ? 'project' : 'global', createdBy: rule.createdBy, contentPreview: rule.content.slice(0, 80) }
    });
    return rule;
  },
