 ...
 * `init.directories`. Object-keyed arrays go through the Chunk 2
 * `mergeById` policy.
 *
 * @param {Array} live
 * @param {Array} bundled
 ...
 * needed.
 *
 * Policies (Chunk 1 of #155):
 *   - `_reconcileOrderedSubset` — ordered string lists; bundled wins
 *     only when live is a stale-older strict subset.
 *   - `_reconcileSetUnion` — unordered string sets; bundled adds are
 *     appended, live order is preserved.
 *
 * Hook arrays (`hooks.<engine>.<event>[]`) intentionally are NOT in this
 * table — they have match-by-matcher semantics handled separately by
 * `_mergeBundledHookEntries` (#158).
 *
 * Object-id-keyed arrays (`phases`, `evalDimensions.tier1/2/3`,
 * `actions`) are deferred to Chunk 2 along with the `_reconcileMergeById`
 * policy and the ADR 0001 update.
 *
 * @type {Array<{path: string, reconcile: Function, label: string}>}
 */
...
function _mergeBundledTemplate(bundledPath, livePath) {
  ...
  // Plain-array reconciliation via the policy table (#155 Chunk 1).
  // `wrap.steps` (originally inline #136) now flows through this same loop.
  for (const entry of ARRAY_RECONCILERS) {
    ...
    const reconciled = entry.reconcile(liveArr, bundledArr);
    if (reconciled === null) {
    ...
    log.info('Reconciled template array from bundled (#155)', {
      ...
      path: entry.path,
      policy: entry.label,
      added: reconciled.filter((v) => !liveArr.includes(v)),
    });
  }
