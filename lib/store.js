  ...
  if (currentVersion < CURRENT_SCHEMA_VERSION) {
    ...
    if (currentVersion < 29) {
      ...
      try {
        ...
        // exists to allow. Committing without checking would advance the
        // schema version past a CHECK that never changed.
        const probe = _db.prepare(
          "SELECT 1 FROM pragma_table_info('session_rule_deliveries') WHERE name = 'channel'"
        ).get();
        if (!probe) throw new Error('rebuilt session_rule_deliveries is missing the channel column');
        _db.exec('COMMIT');
      } catch (err) {
