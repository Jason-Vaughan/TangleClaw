...
function _createTables() {
  _db.exec(`
    ...
    --
    -- Rows record a delivery ATTEMPT, not only a success: a delivered = 0 row
    -- carrying a skip_reason is what makes "which projects never received
    -- their rules" and "why not, on this engine" answerable. A success-only
    -- ledger looks identical to a severed channel — the exact failure mode this
  ...
  if (currentVersion < CURRENT_SCHEMA_VERSION) {
    ...
    if (currentVersion < 26) {
      ...
      try {
        ...
        log.debug('Migration v25→v26 skipped (session_rule_deliveries may already exist)', { error: err.message });
      }
...
// Bundled methodology templates TC has retired. tilt: identity-first
// methodology retired 2026-07-17 — it carried no distinct value over
// `minimal`, and its two fleet projects were downgraded. Same tombstone
// semantics as RETIRED_ENGINE_IDS: without the removal pass, the synced
// runtime copy under ~/.tangleclaw/templates/ outlives the bundled
// deletion forever and keeps surfacing in methodology pickers.
const RETIRED_TEMPLATE_IDS = ['tilt'];
...
function _copyBundledTemplates(srcDir, destDir) {
  ...
  // Tombstone pass — remove runtime copies of retired bundled templates.
  for (const id of RETIRED_TEMPLATE_IDS) {
    const stalePath = path.join(destDir, id);
    if (!fs.existsSync(stalePath)) continue;
    try {
      fs.rmSync(stalePath, { recursive: true, force: true });
      log.warn('Removed retired methodology template', {
        template: id,
        path: stalePath,
        howToInvestigate: 'This methodology was retired from TangleClaw. If you need a custom methodology with this behavior, re-create it under a DIFFERENT id in ~/.tangleclaw/templates/ — this id is tombstoned and will be removed again on every boot.'
      });
    } catch (err) {
      log.warn('Could not remove retired methodology template', { template: id, error: err.message });
    }
  }
 ...
 * @returns {{id: number, sessionId: number|null, projectId: number|null, engineId: string, kind: string, channel: string, delivered: boolean, skipReason: string|null, ruleIds: number[], ruleCount: number, digest: string, createdAt: string}}
 */
...
const sessionRuleDeliveriesApi = {
   ...
   *
   * Both outcomes are recorded deliberately: a `delivered: false` row with a
   * `skipReason` is the evidence that distinguishes "this engine has no channel"
   * from "this project has no rules" — two states that were indistinguishable
   * before this ledger existed, which is how startup rules stayed severed on 13
   * projects without anyone noticing.
   *
   * @param {object} entry
   ...
   * @param {boolean} entry.delivered - Whether the block actually reached the engine
   * @param {string} [entry.skipReason] - Why not; required when delivered is false
   ...
   * @throws {StoreError} BAD_REQUEST on a missing engineId, bad channel, or an
   *   undelivered attempt with no reason
   */
  record(entry = {}) {
    _ensureDb();
    ...
    // An undelivered row with no reason is the useless case: it records that
    // something went wrong while discarding the only field that says what.
    if (!delivered && !entry.skipReason) {
      throw new StoreError('skipReason is required when delivered is false', 'BAD_REQUEST');
    }
    const ruleIds = Array.isArray(entry.ruleIds) ? entry.ruleIds : [];
    const info = _db.prepare(
      `INSERT INTO session_rule_deliveries
         (session_id, project_id, engine_id, kind, channel, delivered, skip_reason, rule_ids, rule_count, digest)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      ...
      entry.channel,
      delivered ? 1 : 0,
      delivered ? null : entry.skipReason,
