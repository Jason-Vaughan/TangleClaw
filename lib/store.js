...
function _createTables() {
  _db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS startup_prompt_revisions (
      ...
      created_by_kind   TEXT    NOT NULL CHECK (created_by_kind IN ('seed','operator')),
      created_at        TEXT    NOT NULL DEFAULT (datetime('now'))
    );

    -- Every authorized fire of the startup prompt, and what came of it. A
    -- durable table rather than activity_log, which is pruned. It names the
    -- target by session and launch-sequence ROW id, never by launch id: a
    -- launch id is a bearer credential and must not be stored where an audit
    -- reader can see it.
    CREATE TABLE IF NOT EXISTS startup_prompt_fires (
      id                INTEGER PRIMARY KEY AUTOINCREMENT,
      session_id        INTEGER NOT NULL,
      sequence_id       INTEGER NOT NULL,
      ...
      outcome           TEXT    NOT NULL CHECK (outcome IN ('pending','unsupported','accepted','applied','failed','interrupted','blocked')),
      reason            TEXT,
      created_at        TEXT    NOT NULL DEFAULT (datetime('now')),
      updated_at        TEXT    NOT NULL DEFAULT (datetime('now')),
      -- Idempotency: one fire per launch per prompt revision. A repeat returns
      -- this row instead of submitting the prompt a second time.
      UNIQUE (sequence_id, prompt_revision)
    );
    CREATE INDEX IF NOT EXISTS idx_startup_prompt_fires_session ON startup_prompt_fires(session_id);
    -- At most one fire in flight per launch, whatever revision it carries.
    CREATE UNIQUE INDEX IF NOT EXISTS idx_startup_prompt_fires_inflight
      ON startup_prompt_fires(sequence_id) WHERE outcome IN ('pending','accepted');
  `);

  // Seed revision 1 of the startup prompt. Here rather than in the v45
  // migration because a fresh install stamps the current schema version below
  // without running any migration, and would otherwise start with no prompt.
  // A no-op on every later boot.
  _db.prepare(
    'INSERT INTO startup_prompt_revisions (revision, text, digest, created_by_kind) '
    + "SELECT 1, ?, ?, 'seed' WHERE NOT EXISTS (SELECT 1 FROM startup_prompt_revisions)"
  ).run(STARTUP_PROMPT_SEED, _startupPromptDigest(STARTUP_PROMPT_SEED));
 ...
 * version if either table lacks a constraint its readers rely on: the outcome
 * CHECK, the idempotency key, the one-fire-in-flight index, and the seed.
 *
...
function _verifyStartupPromptTables() {
  ...
  if (!/created_by_kind[^,]*CHECK\s*\(\s*created_by_kind\s+IN/i.test(revisions)) {
    throw new Error('v44→v45 left startup_prompt_revisions without its created_by_kind CHECK. Refusing to advance schema_version.');
  }
  const fires = ddl('startup_prompt_fires');
  if (!/outcome[^,]*CHECK\s*\(\s*outcome\s+IN/i.test(fires) || !/UNIQUE\s*\(\s*sequence_id\s*,\s*prompt_revision\s*\)/i.test(fires)) {
    throw new Error('v44→v45 left startup_prompt_fires without its outcome CHECK or idempotency key. Refusing to advance schema_version.');
  }
  if (!ddl('idx_startup_prompt_fires_inflight')) {
    throw new Error('v44→v45 did not create the one-fire-in-flight index. Refusing to advance schema_version.');
  }
...
const startupPromptsApi = {
   ...
   * @returns {{revision: number, text: string, digest: string, createdByKind: string, createdAt: string}}
   */
  current() {
    _ensureDb();
  ...
  /**
   * Write a new revision, only if `expectedRevision` is still current.
   * @param {{text: string, firerProjectIds: number[], expectedRevision: number, byKind: string}} input
  ...
  update({ text, firerProjectIds, expectedRevision, byKind }) {
    ...
    try {
      ...
      if (revision !== expectedRevision) {
        _db.exec('ROLLBACK');
        return { ok: false, currentRevision: revision };
      }
      _db.prepare(
        'INSERT INTO startup_prompt_revisions (revision, text, digest, firer_project_ids, created_by_kind) '
        + 'VALUES (?, ?, ?, ?, ?)'
      ).run(revision + 1, text, _startupPromptDigest(text), JSON.stringify(firerProjectIds || []), byKind);
      _db.exec('COMMIT');
    } catch (err) {
      try { _db.exec('ROLLBACK'); } catch { /* nothing to roll back */ }
      throw err;
  ...
  /**
   * Record a fire, or return the one already recorded for this launch and
   * revision. Idempotent on (sequenceId, promptRevision).
   * @param {{sessionId: number, sequenceId: number, promptRevision: number, promptDigest: string, callerKind: string, callerProjectId: (number|null), outcome: string, reason: (string|null)}} fire
   * @returns {{fire: object, duplicate: boolean}}
   */
  recordFire(fire) {
    _ensureDb();
    const existing = this.getFire(fire.sequenceId, fire.promptRevision);
    if (existing) return { fire: existing, duplicate: true };
    _db.prepare(
      'INSERT INTO startup_prompt_fires '
      + '(session_id, sequence_id, prompt_revision, prompt_digest, caller_kind, caller_project_id, outcome, reason) '
      + 'VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
    ).run(fire.sessionId, fire.sequenceId, fire.promptRevision, fire.promptDigest,
      fire.callerKind, fire.callerProjectId, fire.outcome, fire.reason);
    return { fire: this.getFire(fire.sequenceId, fire.promptRevision), duplicate: false };
  },

  /**
   * The fire recorded for one launch and revision, or null.
   * @param {number} sequenceId - launch_sequences row id.
   * @param {number} promptRevision - Prompt revision.
   * @returns {object|null}
   */
  getFire(sequenceId, promptRevision) {
    _ensureDb();
    const row = _db.prepare(
      'SELECT * FROM startup_prompt_fires WHERE sequence_id = ? AND prompt_revision = ?'
...
function _insertLaunchSequence(sessionId, projectId, engineId, seq) {
  ...
  _db.prepare(
    `INSERT INTO launch_sequences (launch_id, session_id, project_id, engine_id, page_budget,
       applicability, not_applicable_reason, preflight, source_manifest, recovery, recovery_mode)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    seq.launchId,
    sessionId,
...
const sessionRuleDeliveriesApi = {
   ...
   * session's. It cannot prove that: the hook is registered `matcher: 'startup'`
   * in the project's own settings (`lib/engines.js`), so every `claude` opened
   * in that directory runs it, and the token names a row rather than a session.
