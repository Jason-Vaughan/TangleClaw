...
function _mergeBundledProfile(bundledPath, livePath) {
  ...
  const merged = addMissing(live, bundled);
  changed = merged;
 ...
 * possibly with intervening elements. Used by `_mergeBundledTemplate` to
 * decide whether a runtime `wrap.steps` array is a stale-older-version of
...
/**
 * Reconcile a single bundled template into a live runtime template. Mirrors
 * `_mergeBundledProfile` (engines, #119) for methodology templates (#136).
 *
 ...
 *   2. **`wrap.steps` array**: if the live array is an ordered subset of the
 *      bundled array (i.e. user is on an older version that's missing some
 *      bundled steps), replace with bundled. If the live array contains steps
 *      not in bundled, or appears in a different order than bundled, the user
 *      has customized — leave alone and log at debug level so the operator
 *      can see the divergence in verbose mode.
 *
...
function _mergeBundledTemplate(bundledPath, livePath) {
  ...
  /**
   * Recursively add missing keys from src into target. Same shape as the
   * helper inside `_mergeBundledProfile` (#119) — kept as a local closure
   * so the two reconcilers stay independently auditable.
   * @param {object} target
   * @param {object} src
   * @returns {boolean} Whether any keys were added
   */
  ...
  if (
    ...
    if (
      ...
      _isOrderedSubset(liveSteps, bundledSteps)
    ) {
      live.wrap.steps = [...bundledSteps];
      changed = true;
      log.info('Reconciled wrap.steps from bundled template (#136)', {
        file: path.basename(livePath),
        added: bundledSteps.filter((s) => !liveSteps.includes(s))
      });
    } else if (
      liveSteps.length > 0 &&
      !_isOrderedSubset(liveSteps, bundledSteps)
    ) {
      log.debug('wrap.steps customized; leaving runtime template as-is (#136)', {
        file: path.basename(livePath)
      });
    }
  }
