 ...
 *
 * A PR opened from the cleanup path (`lib/stranded-check.js#openPr`) is a
 * `wrap.strand_pr_opened` row with the same key, plus the PR URL and who opened
 * it. It neither clears nor acknowledges the item: an open PR has not landed,
 * and the GitHub check clears the item once it merges or goes green. Listed
 * items carry it as `prOpened`, so the same PR is not offered twice.
 */
...
const EVENT_PR_OPENED = 'wrap.strand_pr_opened';
...
/**
 * Record a PR opened for a listed item from the cleanup path. Never throws,
 * like {@link record}; the caller confirms the write by listing again.
 * @param {object} project - Project with `id`
 * @param {object} params
 * @param {string|null} params.remote - The item's remote, as listed
 * @param {string} params.branch
 * @param {string|null} params.headSha - The item's head, as listed (null for an older record)
 * @param {string} params.prUrl - The PR `gh pr create` printed
 * @param {string|null} params.by - Signed-in username, or null when not known
 * @param {string} params.at - ISO time the PR was opened
 */
function recordPrOpened(project, { remote, branch, headSha, prUrl, by, at }) {
  _internal.log({
    projectId: project.id,
    eventType: EVENT_PR_OPENED,
    detail: { remote: remote || null, branch, headSha: headSha || null, prUrl, by: by || null, at }
  });
}
 ...
 * @returns {string|null} e.g. `2026-09-16T21:39:10Z`; null for a missing value
 */
function isoFromSqlite(createdAt) {
 ...
 *   acknowledgedAt: string|null, prOpened: {url: string, by: string|null, at: string}|null}>}}
 */
function list(project) {
  ...
  for (const r of _rows(projectId, EVENT_ACK).reverse()) {
    const d = r.detail;
    acks.set(_key(d.remote, d.branch, d.headSha), { by: d.by ?? null, at: d.at || isoFromSqlite(r.createdAt) });
  }

  const prsOpened = new Map();
  for (const r of _rows(projectId, EVENT_PR_OPENED).reverse()) {
    const d = r.detail;
    if (typeof d.prUrl !== 'string' || !d.prUrl) continue;
    prsOpened.set(_key(d.remote, d.branch, d.headSha), { url: d.prUrl, by: d.by ?? null, at: d.at || isoFromSqlite(r.createdAt) });
  }
 ...
 * @param {() => object} [readGithub] - Reads the latest GitHub check status
 *   (`lib/stranded-check.js#status`). Passed in by the caller: the check module
 *   depends on this one, not the other way round.
 * @returns {string[]} Markdown lines (empty when there is no project id)
 */
function primeSection(project, readGithub) {
  if (!project || project.id == null) return [];
  try {
    const github = typeof readGithub === 'function' ? readGithub() : undefined;
