 ...
 * @param {number|null} [ctx.wrapRequestedAtMs] - When this session last requested a wrap, even if since acknowledged
...
function receiptCurrency(receipt, ctx) {
  ...
  if (Number.isFinite(ctx.wrapRequestedAtMs) && ctx.wrapRequestedAtMs >= at) {
    return { current: false, staleReason: 'wrap-requested' };
  }
