 ...
 *
 * @param {(cmd: string, opts?: object) => Promise<{ok: boolean, stdout: string, stderr: string, code: number}>} runRemote - Remote command seam.
 * @returns {Promise<{ok: boolean, bin: string|null, code: string, detail: string|null}>}
 */
async function resolveDockerBin(runRemote) {
  const probe = DOCKER_FALLBACK_PATHS.map((p) => `if [ -x ${p} ]; then echo ${p}; exit 0; fi`).join('; ');
  const r = await runRemote(`command -v docker 2>/dev/null || { ${probe}; exit 42; }`);
 ...
 * @returns {Promise<{ok: boolean, container: string|null, code: string, detail: string|null}>}
 */
async function findContainer(runRemote, dockerBin, port) {
  const r = await runRemote(`${dockerBin} ps --filter ${shellQuote(`publish=${port}`)} --format ${shellQuote('{{.Names}}')}`);
  if (!r.ok) {
 ...
 *
 * @param {object} opts
 * @param {Function} opts.runRemote - Runs one command on the gateway host; resolves `{ok, stdout, stderr, code}`.
 *   Async so a host that does not answer never holds the server's event loop.
 ...
 * @returns {Promise<{approved: boolean, code: string, reason: string, count: number, requestId: string|null, dockerBin: string|null}>}
 */
async function approvePending(opts) {
  const { runRemote, port, gatewayToken } = opts;

  const docker = await resolveDockerBin(runRemote);
  if (!docker.ok) {
    return { approved: false, code: docker.code, reason: docker.detail, count: 0, requestId: null, dockerBin: null };
  }

  const found = await findContainer(runRemote, docker.bin, port);
  if (!found.ok) {
    return { approved: false, code: found.code, reason: found.detail, count: 0, requestId: null, dockerBin: docker.bin };
  }

  const listed = await runRemote(`${docker.bin} exec ${shellQuote(found.container)} openclaw devices list --json`);
  if (!listed.ok) {
    return {
  ...
  const approved = await runRemote(
