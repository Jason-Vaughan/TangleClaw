...
// credential into: engine-private files this repo genuinely gitignores
// (`.gitignore:96-97`). Everything else is assumed COMMITTED (#1619).
//
...
function _medusaDiscoveryLines(format = 'md') {
  const body = [
    ...
    '`<project-name>` — `tc whoami`, or GET `$TANGLECLAW_API/api/tc/whoami?projectId=$TANGLECLAW_PROJECT_ID`. URL-encode it: names may contain spaces.',
...
function _buildSharedDocsSection(docs, options = {}) {
  ...
  //
  // The doc's path and inline contents deliberately stay. They are group
  // configuration: identical across every checkout of one repository, which is
  // the property the fix is actually about. Stripping them would disable inline
  // shared docs on the one carrier Claude reads — a capability loss, not a
  // neutrality gain.
