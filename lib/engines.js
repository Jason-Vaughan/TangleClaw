 ...
 *
 * When git cannot answer — not a repository, git missing, git erroring — the
 * conventional list decides instead. That is not a safety hole: a directory
 * that is not a repository cannot commit anything, so there is nothing to
 * protect, and blanket-withholding would break the legitimate private-carrier
 * case for every non-repo project. What the list cannot answer safely is an
 * UNKNOWN NAME, and `_isCommittedCarrier` still fails toward committed there.
 *
 * @param {string|null|undefined} projectPath - The project root.
 * @param {string|null|undefined} filename - The carrier's basename.
 * @returns {boolean} False only when git says this project ignores the file.
 */
function _carrierIsCommitted(projectPath, filename) {
  ...
  try {
    ...
    log.warn('Could not ask git whether the carrier is ignored; falling back to the conventional list', {
      projectPath, filename, error: err.message
    });
    return _isCommittedCarrier(filename);
  }
  if (status === 0) return false;
  if (status === 1) return true;
  // Git did not answer. Say so once — silence here is how the pre-#1619
  // hardcoded list would quietly become the decision again on a host without
  // git, which is exactly the shape of the PATH failure this issue documents.
  log.warn('git could not classify the carrier; falling back to the conventional list', {
  ...
  for (const [groupName, groupDocs] of byGroup) {
    ...
    for (const doc of groupDocs) {
      ...
      try {
        ...
        if (lock) {
          // Never in a committed carrier: it names another project and its
          // expiry is wall-clock, so the shared bytes would change every
          // time anything regenerated them.
          lockWarning = committed
            ? ' ⚠️ may be locked by another project — check before editing'
