 ...
 * @returns {{ written: boolean, preservedForeign: number, replacedOwn: number }}
 */
function _reconcileHooksFile(settingsFile, baselineHooks) {
    ...
    try {
      ...
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
        log.warn('Settings file is not a JSON object; leaving it untouched', {
          settingsFile, howToInvestigate: 'the file must hold a JSON object for TangleClaw to merge hooks into it'
        });
        return { written: false, preservedForeign: 0, replacedOwn: 0 };
      }
      settings = parsed;
    } catch (err) {
      ...
      log.warn('Failed to parse settings file; leaving it untouched', {
        settingsFile, error: err.message
      });
      return { written: false, preservedForeign: 0, replacedOwn: 0 };
    }
  }
  ...
  if (!existed && Object.keys(settings).length === 0) {
    return { written: false, preservedForeign, replacedOwn };
  }
  if (existed && next === raw) {
    return { written: false, preservedForeign, replacedOwn };
  }
  ...
  return { written: true, preservedForeign, replacedOwn };
}
...
function syncEngineHooks(projectPath, engineIdOverride) {
  ...
  if (synced.written || retiredShared.written) {
