...
function _medusaDiscoveryLines(format = 'md') {
  const body = [
    ...
    'If you cannot continue without an answer, send with `"priority": "blocking"` (it needs your launch headers; `tc message send --priority blocking` sends them) so an unanswered message escalates instead of waiting silently. Answer one with `"inReplyTo": "<message-id>"`, and close an exchange you started with `POST <base>/medusa/exchanges/<exchange-id>/close` (`tc message close`).',
