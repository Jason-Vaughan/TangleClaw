...
function _buildSharedDocsSection(docs, options = {}) {
  ...
  //
  // Nor the doc's PATH or its inline CONTENTS. I argued once that these were
  // group configuration and therefore identical everywhere; the Architect
  // disproved it with two same-policy fixtures differing only in a document
  // reference path, whose tracked carriers differed and contained that path.
  // A path is install state — it moves with the machine, the group's shared
  // directory and the operator's layout — and inline contents are that file's
  // bytes at generation time, so both reintroduce exactly the churn this fix
  // removes.
  //
