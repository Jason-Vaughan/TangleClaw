 ...
 *   default for engine-agnostic playbooks). The V1 `prawduct`
...
function _renderPlaybook(methodologyTemplate, projectConfig, engineId) {
  ...
  if (sections && typeof sections === 'object' && !Array.isArray(sections)) {
    ...
    for (const [rule, heading] of Object.entries(sections)) {
      if (extensions[rule] === false && typeof heading === 'string') {
        ...
        if (playbook === before) {
          // A disabled rule whose declared heading is missing (e.g. a drifted
          // user-owned playbook copy) would silently re-create the exact
          // render mismatch #536 fixes — surface it instead.
          log.warn('Playbook section for disabled rule not found — section may still render', {
            methodology: methodologyTemplate.id, rule, heading
          });
        }
      }
    }
  ...
  if (!playbook.trim()) {
    log.warn('Playbook empty after rule-section stripping — omitting it', {
      methodology: methodologyTemplate.id, engineId
    });
    return null;
  }
  return playbook;
 ...
 * methodology, and governance state.
 * @param {string} projectPath - Absolute path to project root
 * @param {object|null} methodologyTemplate - Resolved methodology template
 ...
 */
function syncEngineHooks(projectPath, methodologyTemplate, engineIdOverride) {
