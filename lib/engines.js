...
function writeEngineConfig(engineId, projectPath, projectConfig, engineProfile) {
  ...
  // A provenance line is part of what this write produces, so it is applied
  // before the drift comparison: comparing the file against content without
  // its line would report drift on every launch of a project that opted in.
  ...
  const provenanceCtx = provenanceSurface
    ...
    : null;
  if (provenanceSurface) content = provenance.applyProvenance(provenanceSurface, content, provenanceCtx);
