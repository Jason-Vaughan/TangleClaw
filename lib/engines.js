 ...
 * @returns {{written: boolean, drifted: boolean, configFilePath: string|null, error: string|null}}
 */
function writeEngineConfig(engineId, projectPath, projectConfig, engineProfile, methodologyTemplate) {
  ...
  if (!content) {
    return { written: false, drifted: false, configFilePath: null, error: 'generateConfig returned empty' };
  }
  if (!engineProfile || !engineProfile.configFormat || !engineProfile.configFormat.filename) {
    return { written: false, drifted: false, configFilePath: null, error: 'engine profile has no configFormat.filename' };
  }
  ...
  try {
    ...
    return { written: true, drifted, configFilePath, error: null };
  } catch (err) {
    return { written: false, drifted, configFilePath, error: err.message };
  }
}
