 ...
 * @param {boolean} [options.refresh] - Re-resolve the operator's login PATH
 *   before probing. What the wizard's "Check again" passes: the operator has
 *   just installed something, and an installer that edits their shell profile
 *   changes the PATH we cached.
 * @returns {{ id: string, available: boolean, path: string|null }[]}
 */
function detect(options) {
  const profiles = store.engines.list();
  const results = [];
  // One refresh for the whole cycle, not one per engine: the login PATH is a
  // property of the operator's shell, not of any single engine, and re-running
  // their profile once per profile is the cost this exists to avoid.
  const refresh = !!(options && options.refresh);
  if (refresh) resetDetectionCache();
 ...
 * @returns {string} The PATH it reported, or `''` if it did not answer usably.
 */
function _probeShellPath(shell, flags) {
  try {
    // Bounded: this runs the operator's own profile, which may do anything —
    // including block. A shell that does not answer leaves detection exactly
    // where it was, which is the pre-existing behaviour, not a new failure.
    const out = execSync(
      `${shell} ${flags} 'printf "${SHELL_PATH_START}%s${SHELL_PATH_END}" "$PATH"'`,
    ...
    return out.slice(start + SHELL_PATH_START.length, end).trim();
  } catch {
    return '';
  }
}
 ...
 *
 * @param {boolean} [refresh] - Re-run the login shell instead of reusing the
 *   cached answer. What "Check again" does after the operator installs
 *   something: a new binary usually lands on the PATH we already have, but an
 *   installer that edits the shell profile changes the PATH itself.
 * @returns {string} A PATH value; never empty, since it always contains this
 *   process's own.
 */
function _detectionPath(refresh) {
  if (refresh) _loginPathCache = null;
  ...
  // fails or hangs.
  const loginPath = _probeShellPath(shell, '-lic') || _probeShellPath(shell, '-lc');
 ...
 * @param {boolean} [refresh] - Re-resolve the login PATH first.
 * @returns {{ id: string, available: boolean, path: string|null }}
 */
function _detectWhich(id, target, refresh) {
  ...
  try {
    ...
    const binPath = execSync(`command -v ${target}`, {
      timeout: 2000,
      encoding: 'utf8',
      env: { ...process.env, PATH: _detectionPath(refresh) },
 ...
 * @param {object} [options]
 * @param {boolean} [options.refresh] - Re-resolve the operator's login PATH
 *   before probing (see `detect`). What the wizard's "Check again" passes after
 *   the operator installs something.
...
/**
 * Is at least one engine installed on this machine?
 *
 * The single predicate behind the setup gate, called from both routes that can
 * finish setup — `POST /api/setup/complete` and the Skip path's
 * `PATCH /api/config { setupComplete: true }`. One function rather than two
 * expressions on purpose: the last time these two coordinated around a
 * condition and each spelled it out for itself, one kept the old rule and
...
function anyEngineInstalled() {
  if (listWithAvailability().some((e) => e && e.available)) return true;
  return listWithAvailability({ refresh: true }).some((e) => e && e.available);
}
    ...
    for (let i = 1; i <= ruleShardCount; i += 1) {
      entries.push({
        ...
        hooks: [
          {
            type: 'command',
            // Quoted: the resolved TangleClaw directory is an absolute path the
            // operator chose, and an unquoted one breaks the moment it contains a
            // space — silently, since the hook simply fails to run.
            command: `"{{TANGLECLAW_DIR}}/data/hooks/sessionstart-rules.sh" ${i}`,
