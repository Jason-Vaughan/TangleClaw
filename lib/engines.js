...
const { execSync, execFile, spawnSync } = require('node:child_process');
const store = require('./store');
const provenance = require('./provenance');
const primeHookOutput = require('./prime-hook-output');
...
const { tcBootstrapLines, planDocsLine, contextReentryLine, controlStateLine, workloadLine } = require('./ecosystem-primer');
...
const legacyClaudeMd = require('./legacy-claude-md');
...
// The only carriers TangleClaw may write per-checkout identity or a live
// credential into: engine-private files that are conventionally gitignored
// (#1619).
//
// This list is the CONVENTION, and it is no longer what decides. Generation
// asks git for the carrier's real ignored state in the project itself
// (`_carrierIsCommitted`), because TangleClaw's own ignore file says nothing
// about a managed project that tracks its `.codex.yaml` — and for that project
// the list would call a tracked file private and inline the bearer token into
// it. The list answers only where there is no project to ask about, and an
// unrecognised name still resolves to committed.
//
// The direction of that default is the point. A carrier nobody classified
// must fail toward "shared", because the two failure modes are not
// comparable: withholding identity from a private file costs one API call,
// while writing it into a tracked one publishes a project name — or a live
// bearer token — to the repository and to everywhere that repository is
// pushed. `CLAUDE.md` sat on the wrong side of this for exactly that reason,
// classified by an assumption (`.gitignore:85` says the opposite).
const PRIVATE_ENGINE_CARRIERS = ['.codex.yaml', '.aider.conf.yml'];

/**
 * Whether a carrier file is one the operator commits, and so must contain no
 * checkout-specific identity and no live credential (#1619).
 *
 * @param {string|null|undefined} filename - The carrier's basename.
 * @returns {boolean} True unless the file is a known engine-private carrier.
 */
function _isCommittedCarrier(filename) {
  return !PRIVATE_ENGINE_CARRIERS.includes(filename);
}

/**
 * The carrier classification a generator uses: git's answer for this project
 * where there is one, the conventional list otherwise (#1619).
 *
 * A direct unit call passes no project path — git is never ASKED — and that is
 * the only case the list answers. Git failing to ANSWER is different and does
 * not come here: `_carrierIsCommitted` returns committed for it, because a
 * repository whose git is unavailable is still a repository.
 *
 * @param {string|null|undefined} projectPath
 * @param {string} carrierFilename
 * @returns {boolean}
 */
function _carrierCommitted(projectPath, carrierFilename) {
  if (!projectPath) return _isCommittedCarrier(carrierFilename);
  return _carrierIsCommitted(projectPath, carrierFilename);
}

/**
 * Whether THIS project's carrier is one the operator commits — asked of git
 * rather than assumed from a list (#1619, the chunk 04 gate).
 *
 * `_isCommittedCarrier` encodes the conventional case, and the convention is
 * this repo's. A managed project is free to track its own `.codex.yaml`, and
 * for that project the list is simply wrong — it would classify a tracked file
 * as private and keep inlining the live bearer token into it. Only the
 * project's own git can settle it.
 *
 * `git check-ignore` is the question that matters: a file git ignores will
 * never be committed, whatever it is called. Everything else is treated as
 * committed, which keeps the fail-safe direction — an untracked-but-not-ignored
 * file is one `git add` away from being in the repository.
 *
 * When git cannot answer, the carrier is treated as COMMITTED. It used to fall
 * back to the conventional list, and that was wrong for the reason the Architect
 * reproduced: **missing git is not a non-repository**. A project that tracks its
 * own `.codex.yaml` is a repository whose git happens to be unavailable, and the
 * list would call that tracked file private and inline the live token into it —
 * the defect this whole change exists to remove, reachable by breaking PATH.
 *
 * The cost of the safe direction is one API call for a fetch pointer a private
 * carrier did not strictly need. The cost of the other direction is a credential
 * in a repository. Those are not comparable, so an unknown state is not a
 * judgement call.
 *
 * Note git reports a TRACKED file as "not ignored" even when a rule would
 * otherwise match it — ignore rules do not apply to tracked files — so the
 * tracked-and-ignored carrier lands on `committed` through the status-1 branch,
 * verified rather than assumed.
 *
 * @param {string|null|undefined} projectPath - The project root.
 * @param {string|null|undefined} filename - The carrier's basename.
 * @returns {boolean} False ONLY when git positively says this project ignores it.
 */
function _carrierIsCommitted(projectPath, filename) {
  if (!projectPath || !filename) return _isCommittedCarrier(filename);
  let status = null;
  try {
    // exit 0 = ignored, 1 = not ignored, anything else = we did not learn.
    status = _internal.checkIgnore(projectPath, filename);
  } catch (err) {
    log.warn('Could not ask git whether the carrier is ignored; treating it as committed', {
      projectPath, filename, error: err.message
    });
    return true;
  }
  if (status === 0) return false;
  if (status === 1) return true;
  // Git did not answer (timeout, or a non-zero status that is neither verdict).
  // Said out loud, because silence here is how a broken PATH would quietly
  // decide a carrier's classification — the exact failure shape this issue
  // documents on this very host.
  log.warn('git could not classify the carrier; treating it as committed', {
    projectPath, filename, status
  });
  return true;
}
...
/**
 * Warn — never repair — when a governed carrier still holds legacy sections
 * its managed block duplicates (#1911). A matching heading does not prove the
 * operator left the body alone, so removal needs the operator's approval of a
 * previewed plan (`scripts/repair-governed-claude-md.js`).
 * @param {string} text - The carrier as just written.
 * @param {string} configFilePath
 * @param {string} projectPath
 * @param {string} syntax
 * @returns {void}
 */
function _reportLegacyDuplicates(text, configFilePath, projectPath, syntax) {
  const analysis = analyzeGovernedCarrier(text, syntax);
  const preview = legacyClaudeMd.repairCommand(projectPath);
  if (analysis.refused) {
    log.warn('governed carrier holds legacy TangleClaw content that cannot be bounded — left as is (#1911)', {
      configFilePath, reason: analysis.refused
    });
  } else if (analysis.eligible && analysis.candidates.length > 0) {
    log.warn('governed carrier duplicates TangleClaw operational sections above the Prawduct anchor — left as is (#1911)', {
      configFilePath, candidates: analysis.candidates.length, howToInvestigate: `preview a repair with: ${preview}`
    });
  }
}

/**
 * Analyze a governed carrier for the legacy whole-file sections its managed
 * block now duplicates (#1911). Read-only; see lib/legacy-claude-md.js.
 * @param {string} text - The carrier's contents.
 * @param {string} [syntax='markdown'] - The carrier's managed-block syntax.
 * @returns {object} The `analyzeLegacyCarrier` result.
 */
function analyzeGovernedCarrier(text, syntax = 'markdown') {
  return legacyClaudeMd.analyzeLegacyCarrier(text, legacyCarrierContext(syntax));
}

/**
 * The context lib/legacy-claude-md.js needs: marker lines and header forms.
 * @param {string} [syntax='markdown']
 * @returns {{markers: {begin: string, end: string}, headerMark: string, generatedHeader: string}}
 */
function legacyCarrierContext(syntax = 'markdown') {
  return { markers: _managedBlockMarkers(syntax), headerMark: GENERATED_HEADER_MARK, generatedHeader: CLAUDE_MD_GENERATED_HEADER };
}
 ...
 *   carries no complete pair (no block yet, or a malformed one).
 */
function _extractManagedBlock(text, syntax) {
  const markers = _managedBlockMarkers(syntax);
  if (!markers) return null;
  return managedBlock.extractManagedBlock(text, markers);
}

/**
 * Splice TangleClaw's generated content into an engine config file that another
 * writer also owns, touching only the region between our markers.
 *
 * The syntax-aware half of the managed-block mechanism: it resolves the comment
 * form for the host format, keeps the composed body from introducing a second
 * H1, and names the engine-side sources a stray marker literal can arrive from.
 * The splice itself — and, crucially, what a marker set means — lives in
 * `lib/managed-block.js`, shared with the wrap pipeline's priming roll so the
 * two cannot drift into different answers again. Only exactly-one-of-each or
 * none-of-either is actionable there; a misordered pair is repaired, and every
 * other count is refused, because a marker literal is not proof the marker is
 * ours.
 *
 ...
 *   `merged` is the full new file contents; `error` is set instead when the
 *   splice cannot be attempted, and then nothing should be written. `repaired`
 *   flags a rewrite outside the managed region, which the caller reports.
 */
function _mergeManagedBlock(existing, blockBody, syntax) {
  const markers = _managedBlockMarkers(syntax);
  if (!markers) {
    return { merged: null, error: `no managed-block comment form for syntax '${syntax}'`, repaired: false };
  }
  ...
  return managedBlock.spliceManagedBlock(existing, demoted, markers, {
    bodySourceHint: 'check global-rules.md and any injected shared document'
  });
}
...
// The exact header the whole-file CLAUDE.md generator writes. A governed
// carrier that still opens with it holds that generator's legacy output (#1911).
const CLAUDE_MD_GENERATED_HEADER = `# CLAUDE.md — ${GENERATED_HEADER_MARK}`;
...
function retireInactiveEngineConfig(projectPath, previousEngineId, activeEngineId) {
  ...
  // Ownership is one question, asked in one place. Testing for BOTH markers
  // read a carrier with a single unmatched one as marker-free, and control then
  // fell through to a whole-file write of the notice — destroying the operator's
  // content in the very file the managed block exists to share — or to a skip
  // whose stated reason ("no managed markers") was untrue of that file, leaving
  // the superseded config live as canon. A file with any marker goes to the
  // merge, which refuses what it cannot read and says so.
  const markers = _managedBlockMarkers(syntax);
  if (markers && managedBlock.hasManagedMarkers(existing, markers)) {
    const { merged, error, repaired } = _mergeManagedBlock(existing, notice, syntax);
    if (error) return failed(file, 'managed-block merge refused', error);
    if (repaired) {
      log.warn('managed-block markers were repaired while retiring the previous engine config', {
        file,
        howToInvestigate: 'the file\'s BEGIN/END:tangleclaw markers were out of order; they have been '
          + 'rewritten as one block and the text that sat between them kept below it'
      });
    }
    try {
...
function listWithAvailability(options = {}) {
  ...
  return profiles.map((profile) => {
    ...
    return engineClientPayload(profile, { available: det.available });
  });
}

/**
 * The capability block as the browser receives it.
 *
 * Passed through whole except for `wake`, whose presence is *derived* rather
 * than copied: a declaration is not a capability here. The monitor gates on the
 * block passing `medusaWake.wakeSignature`, so a browser gating on the raw key
 * would render a live Auto-wake control for an operator profile whose block is
 * present but malformed, and that session would then never be nudged. One
 * validity decision, three readers — this projection, the `medusaWake`
 * disposition row, and the monitor's own table.
 *
 * The valid block crosses **whole, `evidence` included**, though nothing in
 * `public/` reads the provenance. Trimming it would make the projected shape
 * one `wakeSignature` itself refuses — `evidence` is required — so the server
 * and the browser would disagree the moment either was handed a projected
 * profile, which is exactly what `test/setting-disposition.test.js` does on
 * purpose. If the payload size ever matters, the fix is to relax what the
 * signature demands of a *projected* block, not to trim here.
 *
 * @param {object} profile - The engine profile to project.
 * @returns {object} The client-facing capabilities.
 */
function _clientCapabilities(profile) {
  const caps = { ...(profile.capabilities || {}) };
  if (!require('./medusa-wake').wakeSignature(profile)) delete caps.wake;
  return caps;
}

/**
 * The engine as the browser receives it — the one definition of which profile
 * fields cross to `public/`.
 *
 * A profile carries more than the client needs, and every field it projects is
 * a field the browser may then read. Written twice — once here for the engine
 * roster, once per project in `enrichProject` — the two shapes drift, and a
 * predicate in `public/api-helper.js` reading a field only one of them sends is
 * indistinguishable from one reading a field nobody sends: it silently answers
 * for every engine as though the capability were absent. `configFormat` is here
 * because `tcSettingDisposition` gates the generated-config row on it, and the
 * routing makes the second shape the load-bearing one — `listWithAvailability`
 * drops `pickerHidden` profiles, so OpenClaw, the one engine that row exists
 * for, reaches the modal ONLY through the per-project payload.
 *
 * @param {object} profile - The engine profile to project.
 * @param {object} [overrides] - Fields the caller knows better than the profile
 *   does: `available` always, plus `id`/`name` for a connection-backed engine
 *   whose identity is the connection's rather than the base profile's.
 * @returns {object} The client-facing engine. Its `name` is always a non-empty
 *   string — the declared name, else the id, else the literal `'Unnamed engine'`
 *   when neither is usable. That is the invariant every render site used to
 *   re-establish for itself (#736), and it is total: a render site reading
 *   `engine.name` straight cannot get `''` or a non-string.
 */
function engineClientPayload(profile, overrides) {
  const payload = {
    id: profile.id,
    name: profile.name,
    ...
    // Whole, not a picked subset. Two disposition rows already read something
    // in here — `supportsSilentPrime` and the `wake` block — and a projection
    // that enumerated today's keys would silently drop the next one, which
    // reads in the browser exactly like an engine that lacks the capability.
    // The one derived key is `wake`: see `_clientCapabilities`.
    capabilities: _clientCapabilities(profile),
    // The carrier, and why it is absent where it is. The disposition keys on
    // `filename` rather than on `capabilities.supportsConfigFile` because
    // `writeEngineConfig` branches on `filename`: gating the sentence the
    // operator reads on a different field would let the two disagree, which is
    // the drift the cross-realm parity exists to prevent.
    configFormat: profile.configFormat || null,
    commands: profile.commands || [],
    launchModes: profile.launchModes || null,
    defaultLaunchMode: profile.defaultLaunchMode || null,
    ...(overrides || {})
  };
  // `name` is guaranteed a non-empty string here, so no render site has to
  // guard it (#736). Only `id` is validated when a profile is saved, and
  // `engines.get()` JSON-parses whatever is on disk, so a hand-added profile
  // can carry no name — or a truthy non-string, which `esc` renders as ''. The
  // result is a blank, unidentifiable control, and it fails silently: every
  // new surface that shows an engine name has to remember the guard, and
  // nothing goes red when one does not.
  //
  // Normalised AFTER the overrides so it covers the name they supply too — a
  // connection-backed engine's label is built from `conn.name`, which is
  // operator-authored like the rest.
  // Total, including the case where the id is unusable too — the guarantee the
  // render sites now rest on has to hold for every input, not for every input
  // anybody thought of.
  payload.name = _displayName(payload.name, payload.id) || 'Unnamed engine';
  return payload;
}

/**
 * The first usable display string for an engine — its name, else its id — or
 * `null` when neither is usable.
 *
 * `typeof` rather than `||`: a truthy non-string (a number, an object) passes
 * `||` and is then dropped by `esc`, which leaves exactly the blank label this
 * is here to prevent. **`id` gets the same test as `name`**: nothing validates
 * a hand-dropped profile, so falling back to an id that is itself a number
 * re-opens the hole one level down.
 *
 * Returns `null` rather than a fallback string because the two callers want
 * different words for that case — a rendered engine is "Unnamed engine", a
 * sentence about one is "This engine" — and a shared default would put the
 * wrong one in one of them.
 *
 * @param {*} name - The declared name.
 * @param {*} id - The engine id, used when the name is unusable.
 * @returns {string|null}
 */
function _displayName(name, id) {
 ...
 *
 * **A row declares `applies` or `caveat`.** `applies` is the all-or-nothing
 * gate: the setting does nothing here, so the control is inert and the `reason`
 * says why. `caveat` is the partial answer: the setting does take effect, and
 * part of what it promises does not. A setting with two halves — one
 * engine-agnostic, one gated — fits neither boolean honestly, and the two
 * toggles that seed an index file and then point a session at it are exactly
 * that shape (#1252). Reporting them as not applying would be false; reporting
 * them as applying, with nothing said, is the silence ADR 0013 forbids. A row
 * declaring neither is not engine-conditional and does not belong in this
 * table; `test/setting-disposition.test.js` fails one at the table rather than
 * letting it reach the operator as a control that says nothing.
 *
 * One or the other, not both — the server would compose them (a passed gate
 * falls through to the caveat) but the browser mirror cannot: membership in
 * `TC_SETTING_CAVEAT_FILES` answers before any gate runs. Wanting a row that
 * does both is a reason to give that table per-row caveat functions first, not
 * a reason to write the row.
 */
const ENGINE_CONDITIONAL_SETTINGS = {
  ...
  // The two index toggles have two halves each, and only one of them is
  // engine-agnostic (#1252). The wrap seeds and maintains the file on every
  // engine; the SessionStart pointer that tells the agent the file is there
  // rides the hidden prime, which four of five engines cannot deliver. So the
  // toggle is honest as a whole-setting gate on neither answer, and carries a
  // caveat instead.
  // An engine with no config file carries none of the generated content
  // (#1251): the project's rules block, the Global Rules document, and the
  // PortHub, shared-docs and session-memory guides all ride one file, so the
  // unit that applies or does not is the CARRIER, not each rule. Keyed on the
  // carrier for that reason, and named after the file the operator can go and
  // look at rather than after its contents — the settings modal already has a
  // "Project Rules" section, which is the free-text `session_rules` feature and
  // a different thing with the same word.
  generatedConfig: {
    // The row gates on a whole config block, and `chosen` is a scalar
    // comparison, so what it hands back is whether the operator moved any of
    // that block off what ships. `rules.core` is deliberately not consulted:
    // `updateProject` refuses to disable a core rule, so a core flag can never
    // be a choice and counting it would report every project as customized.
    read: (projConfig) => _rulesCustomized(projConfig),
    shippedDefault: () => false,
    applies: (value, engineProfile) => Boolean(
      engineProfile && engineProfile.configFormat && engineProfile.configFormat.filename
    ),
    // The engine-specific half of this sentence is declared in the profile, not
    // written here: a sixth engine with no config file states its own case in
    // its own file, and the browser mirror reads the same field rather than
    // carrying a second copy of the words.
    reason: (value, engineProfile) => {
      const name = engineDisplayName(engineProfile);
      const declared = engineProfile && engineProfile.configFormat
        && engineProfile.configFormat.absentReason;
      // Scoped to what the FILE carries. The modal also has a Project Rules
      // section a few groups down — free-text `session_rules`, a different
      // feature riding a different channel — and a sentence that read as
      // covering it would make a claim this row has not checked.
      return `${name} has no config file, so the project rule settings and TangleClaw guides it `
        + `would carry never reach a session here.${declared ? ` ${declared}` : ''}`;
    },
    evidence: () => 'configFormat.filename is null'
  },
  // A wake nudge is typed into the engine's own pane, so it needs that engine's
  // live-probed idle/busy signature — the `capabilities.wake` block the profile
  // declares (#1255). Before this row the only operator-facing string about the
  // feature was a hint reading "Claude sessions only for now", which had been
  // wrong since antigravity was profiled (#560): the log knew, the operator did
  // not.
  //
  // ADR 0013 asks that a row be checked for PARTIAL application before an
  // `applies` gate is written for it (#1252's lesson). Checked, and `applies`
  // is right: the monitor's transport/engine gate skips a session with no wake
  // profile before every other gate, so nothing about the setting runs — there
  // is no half to caveat. Recorded here because the next reader's cheapest
  // wrong move is assuming there must be one.
  medusaWake: {
    // Opt-in, and off is what ships: a wake spends a real turn, so both
    // `lib/projects.js` and the monitor require an explicit `true`.
    shippedDefault: () => false,
    // Validity, not presence. A block that is declared but malformed is
    // refused by the monitor, so gating on the raw key here would show a live
    // control for a session that is never nudged — this row's own silence.
    // Declaring badly and declaring nothing are one answer, decided once in
    // `medusaWake.wakeSignature` and read by this row, by the browser
    // projection, and by the monitor's table.
    applies: (value, engineProfile) => Boolean(
      require('./medusa-wake').wakeSignature(engineProfile)
    ),
    reason: (value, engineProfile) =>
      `${engineDisplayName(engineProfile)} has no measured idle signature, so TangleClaw cannot tell a `
      + 'busy pane from a resting one here and will never nudge this project\'s sessions. Typing into a '
      + 'pane on a guessed signature is the one thing the wake monitor refuses to do.',
    // Both halves of what `applies` refuses. A block that is present but
    // malformed is the same answer as none, so a fact naming only the absent
    // case would be false for exactly the profile the read guard exists for.
    evidence: () => 'capabilities.wake is not declared, or is declared malformed'
  },
  featureIndexEnabled: {
    shippedDefault: () => DEFAULT_PROJECT_CONFIG.featureIndexEnabled,
    caveat: (value, engineProfile, projConfig) =>
      _indexPointerCaveat(engineProfile, projConfig, 'FEATURES.md')
  },
  projectMapEnabled: {
    shippedDefault: () => DEFAULT_PROJECT_CONFIG.projectMapEnabled,
    caveat: (value, engineProfile, projConfig) =>
      _indexPointerCaveat(engineProfile, projConfig, 'PROJECT-MAP.md')
  }
};

/**
 * Record a config write that was skipped because the engine has no carrier for
 * one (#1251).
 *
 * A project whose rules and guides reach no session is owed a trace. The
 * settings modal is where the operator actually learns this — a log line is
 * explicitly not compliance under ADR 0013 — but the log still gets the event,
 * and the LEVEL is the disposition's to derive: losing rules the operator chose
 * is a warning, losing the stock set is a record. Deriving it here is what
 * keeps that asymmetry from depending on each author remembering it.
 *
 * Called by `writeEngineConfig` rather than by its callers, so no writer can
 * fail to report. Skips for any other reason — a governed project deferring to
 * its plugin, an empty generated body — pass through untouched and answer
 * `false`.
 *
 * @param {{skipped: boolean, skipReason: string|null}} writeResult - What
 *   `writeEngineConfig` answered.
 * @param {object|null} projConfig - The project's config (defaults merged).
 * @param {object|null} engineProfile - The engine the project runs on.
 * @param {object} [fields] - Extra log fields identifying the project/caller.
 * @returns {boolean} Whether this skip was the missing-carrier one.
 */
function reportMissingConfigCarrier(writeResult, projConfig, engineProfile, fields) {
  if (!writeResult || !writeResult.skipped) return false;
  const carrier = settingDisposition('generatedConfig', projConfig, engineProfile);
  if (carrier.applies) return false;
  log[carrier.level === 'warn' ? 'warn' : 'info'](carrier.reason, {
    ...(fields || {}),
    evidence: carrier.evidence,
    rulesCustomized: carrier.chosen,
...
/**
 * Whether the project has moved any `rules.extensions` value off what the
 * product ships — the provenance input for the generated-config row.
 *
 * A rules block has no single stored scalar to compare, and `chosen` is what
 * decides whether losing it warns or merely records. So the comparison is per
 * key against `DEFAULT_PROJECT_CONFIG`: a project that turned `independentCritic`
 * on and gets nothing has lost real intent, while one running stock rules never
 * expressed a preference for anyone to lose.
 *
 * A key the shipped defaults do not declare counts as customized — it is
 * something only an operator could have put there.
 *
 * @param {object|null} projConfig - The project's config (defaults merged).
 * @returns {boolean}
 */
function _rulesCustomized(projConfig) {
  const stored = projConfig && projConfig.rules && projConfig.rules.extensions;
  if (!stored || typeof stored !== 'object') return false;
  const shipped = DEFAULT_PROJECT_CONFIG.rules.extensions;
  return Object.entries(stored).some(([rule, value]) => value !== shipped[rule]);
}

/**
 * What is lost when an index toggle's SessionStart pointer cannot be delivered,
 * or null when the whole setting takes effect.
 *
 * Asks `silentPrimeDisposition` rather than testing the capability, because the
 * pointer this sentence is about is emitted under exactly that predicate in
 * `sessions.js` — a caveat that fired under a different condition would tell
 * the operator about a loss the launch path does not have, or stay quiet
 * through one it does. The gate is a triple (toggle, project `silentPrime`,
 * engine capability), and the two ways it fails read differently to the
 * operator: an engine that cannot deliver a hidden prime is nothing they can
 * change, while their own silent-prime switch is.
 *
 * Phrased as a property of the setting on this engine rather than of the
 * stored value, so it reads true whether the toggle is on or off — the
 * operator deciding whether to turn it on is the one who most needs it.
 *
 * @param {object|null} engineProfile - The engine the project runs on.
 * @param {object|null} projConfig - The project's config (defaults merged).
 ...
 * @returns {string|null}
 */
function _indexPointerCaveat(engineProfile, projConfig, file) {
  // No profile is not evidence that a capability is absent — the same rule the
  // disposition's own no-profile branch follows, and the reason a caveat row
  // cannot simply borrow that branch: the wrap half runs regardless, so
  // answering "does not apply" would report a live setting dead.
  if (!engineProfile) {
  ...
  const disposition = silentPrimeDisposition(projConfig, engineProfile);
  if (disposition === 'on') return null;
  const name = engineDisplayName(engineProfile);
  if (disposition === 'not-applicable') {
    return `Only half of this setting takes effect on ${name}: the wrap maintains ${file}, but `
      + `${name} delivers no hidden prime, so no session is told the file exists.`;
  }
 ...
 *
 * Shares `_displayName` with the client projection rather than repeating the
 * test. Its callers pass a RAW profile (`resolveProfile`, not
 * `engineClientPayload`), so this is the one place the unusable-`name` case is
 * still live server-side — and a `||` here while the projection tests the type
 * would let the two realms narrate the same profile differently.
 *
 * @param {object|null} engineProfile - Engine profile.
 * @returns {string}
 */
function engineDisplayName(engineProfile) {
  ...
  return _displayName(engineProfile.name, engineProfile.id) || 'This engine';
}
 ...
 *
 * **Three answers, not two.** `applies: false` with a `reason` is the setting
 * that does nothing here. `applies: true` with a `caveat` is the setting that
 * does part of what it promises. `applies: true` with neither is the whole
 * setting working. Deriving the third state here rather than at each call site
 * is the point: a caller that composed it would decide for itself what "partly"
 * means, and the two realms would drift on the sentence the operator reads.
 *
 ...
 *   level: 'warn'|'info'|null}} `reason` and `evidence` are null unless the
 *   setting does not apply; `caveat` is null unless it applies only partly;
 *   `level` is null only when there is nothing to say at all.
 */
function settingDisposition(setting, projConfig, engineProfile) {
  ...
  // an object is never `!==`-equal to its own default. So a row hands back a
  // scalar — the one it gates on where it gates on one, and otherwise a derived
  // answer to that same question: `generatedConfig` gates on a whole rules
  // block and reports whether any of it was moved off the shipped set.
  ...
  // Provenance, not importance — see above. One expression, so the caveat
  // branch cannot grow a second rule for the same question.
  const level = chosen ? 'warn' : 'info';

  // The all-or-nothing gate, for the rows that declare one. A row with no
  // `applies` is not engine-gated: it takes effect everywhere and only its
  // caveat varies, so it never reaches either branch below — including the
  // no-profile one, which would otherwise report a running setting as dead.
  if (spec.applies) {
  ...
  const caveat = spec.caveat ? spec.caveat(value, engineProfile, projConfig) : null;
  return {
    setting,
    value,
    applies: true,
    ...
    evidence: null,
    caveat,
    level: caveat ? level : null
  };
}

/**
 * The capability flags TangleClaw actually acts on, each mapped to what it
 * decides — the difference between a capability the product READS and one a
 * profile merely DECLARES (#1254).
 *
 * An engine profile declares more than the product consumes, and a reader
 * cannot tell which is which by looking: `supportsCoAuthor: true` on aider
 * reads exactly like `supportsConfigFile: true`, and only one of them changes
 * anything. Rendering the declared set as a capability panel would therefore
 * publish promises the product does not keep. Making the distinction exist is
 * what lets a panel show it; deciding whether to WIRE any particular unread
 * flag is a separate question per flag, and this answers none of them.
 *
 * **Nothing reads this at runtime yet — that is deliberate, not an orphan.**
 * Its consumer is the per-engine capability panel (#764), which is not built;
 * this is the input that panel renders from, and recording it here is what
 * stops the distinction being rebuilt beside it. It does not rot in the
 * meantime because the guard below keeps it true rather than leaving it as a
 * description of what somebody once believed — which is the defect #1254 was.
 *
 * `test/engine-capability-reads.test.js` holds the list to the code both ways:
 * a key here with no reader fails, and a key that gains one without being
 * added here fails too — so the set cannot quietly become a description of
 * what somebody once believed. Its reads are found by spelling, so a reader
 * must name the flag on a line that also names `capabilities` (or `caps`);
 * destructuring the flag onto its own line hides it from the guard.
 *
 * Deliberately not listed, and deliberately not deleted from the profiles:
 * `supportsSlashCommands`, `supportsCoAuthor` (with its `coAuthorFormat`
 * payload), `supportsRemote` and `supportsModes`, which describe engines
 * accurately but drive nothing; and openclaw's `capabilities.awareness`, whose
 * own `reason` text records the gap it stands for.
 */
const READ_CAPABILITIES = {
  supportsPrimePrompt: 'whether a launch may paste the prime into the pane at all',
  supportsConfigFile: 'whether TangleClaw generates this engine a project config file',
  supportsSilentPrime: 'whether the silent-prime setting applies to a project (ADR 0013)',
  startupInjection: 'the payload cap the startup-rules channel budgets the prime against',
  toolOutput: 'the tool-result cap a launch sequence pages `tc start` output against (Train 21)',
  launchSequence: 'whether a session on this engine is served its context as an acknowledged launch sequence',
  readOnlyModeMarker: 'how a wrap step recognises a pane parked in a read-only mode',
  wake: 'the live-probed pane signature that decides whether a session here can be judged idle, nudged, and readiness-gated before a prime paste',
  startupControl: 'whether the startup prompt can be fired through a registered native-channel adapter with a semantic receipt, or is refused as unsupported'
};

/**
 * The config file an engine reads from a project root, or null when it declares
 * none.
 *
 * One derivation, because three readers state it to a session — the prime's
 * rule-sources section, the pulled governance step, and the launch manifest that
 * hashes the file. Reading `configFormat.filename` in each place is how one of
 * them ends up naming a file the others do not.
 * @param {object|null} engineProfile - Engine profile
 * @returns {string|null} The filename, e.g. `CLAUDE.md`
 */
function configFilenameOf(engineProfile) {
  return (engineProfile && engineProfile.configFormat && engineProfile.configFormat.filename) || null;
}
...
function generateConfig(engineId, projectConfig, projectPath) {
  ...
  // The carrier is classified by the FILE it lands in, not by the generator's
  // format (#1619). An operator profile pairing a shared-convention carrier
  // with a private-format generator would otherwise inline a live bearer token
  // into a tracked file.
  const carrier = profile.configFormat.filename;

  switch (generator) {
    case 'claude-md':
      return _generateClaudeMd(projectConfig, projectPath, carrier);
    case 'codex-yaml':
      return _generateCodexYaml(projectConfig, projectPath, carrier);
    case 'aider-conf':
      return _generateAiderConf(projectConfig, projectPath, carrier);
    case 'gemini-md':
      return _generateGeminiMd(projectConfig, `# GEMINI.md — ${GENERATED_HEADER_MARK}`, projectPath, carrier);
    case 'antigravity-md':
      ...
      return _generateGeminiMd(projectConfig, '## TangleClaw — generated; edits inside the markers are overwritten', projectPath, carrier);
    default:
 ...
 * @param {object} projectConfig - Per-project config
 * @returns {{ coreRulesLines: string[], extensionRulesLines: string[], porthubGuide: string|null,
 *   sharedDocsGuide: string|null, globalRules: string|null, sharedDocsContent: string|null,
 *   sharedDocsContentShared: string|null, serverPort: number, serverProtocol: string }}
 *   `sharedDocsContent` is the full rendering, for an engine-private carrier;
 *   `sharedDocsContentShared` is the checkout-neutral one a committed carrier
 *   takes (#1619). Both are built; the generator selects by its carrier.
 */
function _getRulesContent(projectConfig, projectPath) {
  ...
  // Extension rules. `v === true` alone meant a rule whose value is not a
  // boolean could never reach the prose path: `loggingLevel` is the one the
  // product ships that way, and it read as a setting that did nothing on the
  // three markdown generators while two others read it straight off the config
  // (#1253). A rule that carries a value renders WITH its value.
  const extensions = projectConfig.rules && projectConfig.rules.extensions ? projectConfig.rules.extensions : {};
  const activeExtensions = Object.entries(extensions).filter(([, v]) => _rendersAsRule(v));
  const extensionRulesLines = activeExtensions.map(([rule, v]) => _ruleLabel(rule, v));
  ...
  // The committed-carrier rendering: names and descriptions only (#1619).
  let sharedDocsContentShared = null;
  if (projectConfig.id) {
    try {
      const injectableDocs = store.sharedDocs.getInjectableForProject(projectConfig.id);
      if (injectableDocs.length > 0) {
        // Explicit now that an omitted classification means COMMITTED: this is
        // the full rendering, for an engine-private carrier.
        sharedDocsContent = _buildSharedDocsSection(injectableDocs, { committedCarrier: false });
        sharedDocsContentShared = _buildSharedDocsSection(injectableDocs, { committedCarrier: true });
      }
    } catch (err) {
  ...
  return { coreRulesLines, extensionRulesLines, porthubGuide, sharedDocsGuide, sessionMemoryGuide, globalRules, sharedDocsContent, sharedDocsContentShared, serverPort, serverProtocol, serviceTokenEnabled, serviceToken, medusaEnabled, medusaProjectName };
}
...
function _serviceTokenAuthLines(rules, format = 'md', options = {}) {
  // An unclassified carrier fails toward SHARED — including `{}`, which the
  // object-shaped default did not cover (#1619).
  const committedCarrier = options.committedCarrier !== false;
  ...
  // Reaching here means a LIVE credential is about to be written to disk. That
  // is legitimate only for an engine-private carrier, and until #1619 it was
  // also happening for a tracked one — visible to nobody, because the only
  // evidence was the generated file itself. Say so once per write.
  log.info('Inlining the live service token into an engine-private carrier', {
    format, hint: 'a committed carrier must take the fetch pointer instead (#1619)'
  });
 ...
 * @returns {string[]} Lines to push into the config.
 */
function _apiOriginDiscoveryLines(format = 'md', rules = null) {
  // `rules` non-null means the carrier is engine-private and may hold the
  // literal origin; null means committed, and gets the pointer. One function
  // for both so a caller cannot reach the literal without saying which carrier
  // it is writing — the bypass the three-reviewer round found was two call
  // sites that formatted the origin inline and never consulted the classifier.
  if (rules) {
    const url = `${rules.serverProtocol}://localhost:${rules.serverPort}`;
    return format === 'comment'
      ? ['#', `# TangleClaw API base URL: ${url}`]
      : [`**TangleClaw API base URL**: \`${url}\``, ''];
  }
 ...
 * @param {{committedCarrier?: boolean}} [options] - A carrier the operator commits
 *   takes `_medusaDiscoveryLines` instead: same routes, no identity (#1619).
...
function _medusaSwitchboardLines(rules, format = 'md', options = {}) {
  ...
  // object-shaped default did not cover (#1619).
  const committedCarrier = options.committedCarrier !== false;
  if (!rules || !rules.medusaEnabled) return [];
  // A committed carrier's block is the same in every checkout, INCLUDING one
  // whose project is not registered here: gating it on a resolved name would
  // make the shared bytes depend on this machine's registry, so an unregistered
  // clone would regenerate the file with the section deleted (#1619).
  if (!committedCarrier && !rules.medusaProjectName) return [];
  ...
  if (format === 'comment') {
    return [
      ...
      `#   peer why  GET  ${api}/peers/<workspace-id>  (why a peer has not picked up: a reason code + meaning)`,
  ...
  return [
    ...
    `| peers | \`GET ${api}/roster\` |`,
    `| why a peer has not picked up | \`GET ${api}/peers/<workspace-id>\` — the wake monitor's latest reason code for a peer on this host, with its \`meaning\` (\`local: false\` for one it cannot see) |`,
    '',
 ...
 * @returns {string}
 */
function _generateClaudeMd(projectConfig, projectPath, carrierFilename = 'CLAUDE.md') {
  const committedCarrier = _carrierCommitted(projectPath, carrierFilename);
  const lines = [CLAUDE_MD_GENERATED_HEADER, ''];
  ...
  lines.push(...contextReentryLine('md'), '');
  lines.push(...controlStateLine('md'), '');
  lines.push(...workloadLine('md'), '');
  ...
  if (rules.porthubGuide) {
    ...
    lines.push(..._apiOriginDiscoveryLines('md', committedCarrier ? null : rules));
    for (const authLine of _serviceTokenAuthLines(rules, 'md', { committedCarrier })) lines.push(authLine);
    for (const line of _medusaSwitchboardLines(rules, 'md', { committedCarrier })) lines.push(line);
  }

  // Shared documents
  // Select, do not assume. After chunk 04 the private case is reachable here
  // too: a project that gitignores its CLAUDE.md gets `committedCarrier ===
  // false`, and it should then receive the full rendering it is entitled to
  // rather than silently lose the doc path, contents and existence check.
  // There is deliberately no `||` fallback between the two: on a committed
  // carrier that would fail open, toward the identity-bearing rendering.
  const docsForCarrier = committedCarrier ? rules.sharedDocsContentShared : rules.sharedDocsContent;
  if (docsForCarrier) {
    lines.push(docsForCarrier, '');
  }
 ...
 * — where to read the API base URL, the tc bootstrap line, PortHub guide,
 * service-token pointer, switchboard section, shared docs, session-memory
 * guide — and none of the rules tiers:
 ...
 *
 * Where to READ the base URL is unconditional: the origin itself is a launch
 * fact, so since #1619 the block says to read `TANGLECLAW_API` rather than
 * naming this install's URL — but it still says it on every governed project,
 * because its absence is the #1020 dangling pointer. The switchboard and auth
 * helpers already return `[]` when their feature is off, so gating them here
 * would only duplicate their own gates.
 *
 ...
 *
 * @param {object} projectConfig - Per-project config
 * @param {string} projectPath - Absolute project root. Also decides the carrier
 *   classification: `git check-ignore` is asked here, and the answer controls
 *   whether this block may carry an origin, a project name or a live token.
 * @param {string} [carrierFilename='CLAUDE.md'] - The file these bytes land in.
 *   Classified by the FILE, not by the generator's format (#1619).
 * @returns {string} Markdown body for the managed block
 */
function _generateOperationalBlock(projectConfig, projectPath, carrierFilename = 'CLAUDE.md') {
  const committedCarrier = _carrierCommitted(projectPath, carrierFilename);
  const rules = _getRulesContent(projectConfig, projectPath);
 ...
 * @returns {string}
 */
function _generateCodexYaml(projectConfig, projectPath, carrierFilename = '.codex.yaml') {
  const committedCarrier = _carrierCommitted(projectPath, carrierFilename);
  ...
  instrParts.push(...contextReentryLine('md'), '');
  instrParts.push(...controlStateLine('md'), '');
  instrParts.push(...workloadLine('md'), '');
  ...
  if (rules.porthubGuide) {
    ...
    instrParts.push('');
    for (const line of _apiOriginDiscoveryLines('md', committedCarrier ? null : rules)) instrParts.push(line);
    for (const line of _medusaSwitchboardLines(rules, 'md', { committedCarrier })) instrParts.push(line);
    for (const authLine of _serviceTokenAuthLines(rules, 'md', { committedCarrier })) instrParts.push(authLine);
  }
  const codexDocs = committedCarrier ? rules.sharedDocsContentShared : rules.sharedDocsContent;
  if (codexDocs) {
    for (const line of codexDocs.split('\n')) {
      instrParts.push(line);
    }
    instrParts.push('');
  }
 ...
 * @returns {string}
 */
function _generateAiderConf(projectConfig, projectPath, carrierFilename = '.aider.conf.yml') {
  const committedCarrier = _carrierCommitted(projectPath, carrierFilename);
  const rules = _getRulesContent(projectConfig, projectPath);
  ...
  lines.push(...contextReentryLine('comment'));
  lines.push(...controlStateLine('comment'));
  lines.push(...workloadLine('comment'));
  ...
  if (rules.porthubGuide) {
    ...
    for (const line of _apiOriginDiscoveryLines('comment', committedCarrier ? null : rules)) lines.push(line);
    for (const authLine of _serviceTokenAuthLines(rules, 'comment', { committedCarrier })) lines.push(authLine);
    for (const line of _medusaSwitchboardLines(rules, 'comment', { committedCarrier })) lines.push(line);
  }

  const aiderDocs = committedCarrier ? rules.sharedDocsContentShared : rules.sharedDocsContent;
  if (aiderDocs) {
    lines.push('#');
    lines.push('# Shared Documents:');
    for (const line of aiderDocs.split('\n')) {
      const trimmed = line.replace(/^#+\s*/, '').trim();
      if (trimmed) lines.push(`#   ${trimmed}`);
    }
  }
 ...
 * @returns {string}
 */
function _generateGeminiMd(projectConfig, header = `# GEMINI.md — ${GENERATED_HEADER_MARK}`, projectPath, carrierFilename = 'AGENTS.md') {
  const committedCarrier = _carrierCommitted(projectPath, carrierFilename);
  const lines = [header, ''];
  const rules = _getRulesContent(projectConfig, projectPath);
 ...
 * @returns {string}
 */
function _buildSharedDocsSection(docs, options = {}) {
  // #1619. A committed carrier must not carry the LOCK line: it names another
  // project and its `expiresAt` is wall-clock, so a single locked shared doc
  // makes the shared bytes differ on every regeneration — the same
  // silent-diff-then-silent-commit mechanism this fix exists to remove, with
  // another project's name riding along.
  //
  // Nor the doc's PATH or its inline CONTENTS. A path is install state — it
  // moves with the machine, the group's shared directory and the operator's
  // layout — and inline contents are that file's bytes at generation time. Two
  // projects on the same policy but different document paths produce different
  // tracked carriers, which is the churn this fix removes.
  //
  // Access is preserved rather than dropped (brief, point 4): the committed
  // carrier still NAMES every doc and its group, and points at the shared-docs
  // API, which serves the path and the content to whoever asks. What leaves
  // the shared bytes is the value, never the capability.
  // Fails toward shared for `{}` as well as for an omitted argument.
  const committed = options.committedCarrier !== false;
  ...
  const lines = committed
    ? ['## Shared Documents', '',
      'These are the group\'s shared documents. Their paths, contents and lock state are install state, so they are not written here — read them through the shared-docs API: `GET $TANGLECLAW_API/api/shared-docs?groupId=<id>`.', '',
    'To find `<id>`: `GET $TANGLECLAW_API/api/groups` lists the groups your project is in, with their ids, and `GET $TANGLECLAW_API/api/groups/<id>/members` says which projects are in one.', '',
    'Send your project binding on every one of these requests: `-H "x-tangleclaw-project-id: $TANGLECLAW_PROJECT_ID" -H "x-tangleclaw-launch-id: $TANGLECLAW_LAUNCH_ID"`. Both are exported into your pane; they identify which project is asking. They are required: a shared-docs or groups request without them is refused with `403`, and with them you can read and change documents only in the groups your project belongs to — on these routes, another project\'s group or document answers `404`, as if it did not exist. Changing a document\'s registration (its file path, name or injection settings) or deleting it, and creating, changing or deleting a group or its members, are the operator\'s alone (`403 OPERATOR_ONLY`): ask the operator. Editing a document\'s contents is not a registration change; lock it first. A pane with no `TANGLECLAW_LAUNCH_ID` predates launch binding: relaunch the session.', '',
    'Any of these may be locked by another project at any moment — the API reports lock state, this file cannot, so check it there before editing.', '',
    'Send `groupId` to name the group you mean. Without it the list holds the documents of every group your project is in, never another project\'s. Lock before editing a document\'s contents.', '']
    : ['## Shared Documents', ''];
  ...
  for (const [groupName, groupDocs] of byGroup) {
    ...
    for (const doc of groupDocs) {
      ...
      try {
        ...
        if (lock) {
          // A committed carrier gets NOTHING here — not even a generic
          // warning. Emitting one only while a lock happens to exist still
          // makes the shared bytes depend on live state: the same document
          // unlocked and then locked produces two different files, which is
          // the churn this fix removes, one step quieter than the holder's
          // name. The "lock before editing" instruction is unconditional
          // prose in the section header instead, where it is always true.
          lockWarning = committed
            ? ''
            : ` ⚠️ LOCKED by ${lock.lockedByProject} (expires ${lock.expiresAt})`;
        }
      } catch {
        // Ignore lock check errors
      }

      if (doc.injectMode === 'inline' && !committed) {
        ...
        if (committed) {
          // Name and description only. `fileStatus` is a per-machine existence
          // check, so it varies too.
          lines.push(`- **${doc.name}**${doc.description ? ` — ${doc.description}` : ''}${lockWarning}`);
        } else {
...
/**
 * Whether an extension rule's stored value is something to tell the agent.
 *
 * `true` is the switched-on rule. A string or a finite number is a rule that
 * carries a VALUE — `loggingLevel: 'info'` — and dropping those is what left a
 * shipped setting unrepresented in three of the five generated configs
 * (#1253). Everything else stays out: `false` and an absent key are the
 * off-switch this filter has always had, an empty string expresses nothing,
 * and an object has no one-line prose form to render.
 *
 * @param {*} value - The stored value of one `rules.extensions` key.
 * @returns {boolean}
 */
function _rendersAsRule(value) {
...
/**
 * Convert a rule key to the sentence the generated config carries.
 *
 * A label may be a function of the value, so a rule that carries one reads as
 * an instruction rather than a key-colon-value pair; a value-carrying rule
 * without an entry still names its value rather than silently dropping it.
 *
 * @param {string} rule - Rule key
 * @param {*} [value] - The rule's stored value, for the rules that carry one
 * @returns {string}
 */
function _ruleLabel(rule, value) {
  const labels = {
    ...
    loggingLevel: (level) => `Log at the "${level}" level`
  };
  const label = labels[rule];
  if (typeof label === 'function') return label(value);
  if (label) return label;
  return value === true ? rule : `${rule}: ${value}`;
}
 ...
 *
 * **The substitution arrives already quoted, so a caller must never add quotes
 * of its own.** The install path is the only part of these commands that comes
 * from outside the code, so making the substitution shell-safe is what makes
 * every command safe — as opposed to each emission site restating the rule,
 * which is how one site shipped unquoted while the site fifteen lines above it
 * carried a comment explaining the hazard (#759).
 *
 * `'<dir>'/data/hooks/x.sh` is one shell word: adjacent quoted and unquoted
 * segments concatenate, and the trailing segment is a literal this file writes.
 *
 * @param {string} str - Command string with placeholders
 * @param {string} [dir] - Install root; defaults to this checkout. A parameter
 *   so a test can pass a hostile path without relocating the install.
 * @returns {string}
 */
function _resolveHookPlaceholders(str, dir) {
  const tangleClawDir = dir === undefined ? path.join(__dirname, '..') : dir;
  return str.replace(/\{\{TANGLECLAW_DIR\}\}/g, () => shellWord(tangleClawDir));
}
 ...
 * @param {string} [dir] - Install root, forwarded to {@link _resolveHookPlaceholders};
 *   defaults to this checkout. A parameter so a test can drive the real write
 *   path with a hostile install path without relocating the install.
 * @returns {object}
 */
function _resolveHooksObject(hooks, dir) {
      ...
      if (entry.hooks && Array.isArray(entry.hooks)) {
        for (const hook of entry.hooks) {
          if (hook.command && typeof hook.command === 'string') {
            hook.command = _resolveHookPlaceholders(hook.command, dir);
          }
        }
      }
    }
...
// .claude/settings.local.json (the tracked settings.json is retired from, never
// written to — see the location constants below). Ownership is decided by these rather than by the resolved
 ...
 *
 * Both settings files are shared with the operator: `.claude/settings.json` is where
 * the Claude Code docs tell them to put a project-wide hook, and `settings.local.json`
 * is where their per-machine permissions live. TangleClaw owns only the entries it
 * writes and must be able to name them exactly in either, because the alternative
 * (replacing the whole block) silently discarded every hook the operator put there,
 * on every session launch (#752).
...
/**
 * The SessionStart sources TangleClaw's prime and rules hooks fire on (#1761).
 * An exact-alternatives matcher, which Claude Code matches against the hook's
 * `source`.
 * @type {string}
 */
const SESSION_START_REENTRY_MATCHER = 'startup|clear|compact';
 ...
 *
 * Both fire on `startup`, `clear` and `compact` (#1761): `/clear` and a
 * compaction drop the context these hooks delivered, and the engine names the
 * cause in the hook's `source`. `resume` and `fork` are left out because they
 * keep the transcript, so nothing was lost. The prime hook reads `source` to
 * put a re-entry preamble first; the rules hook reads it to post its delivery
 * receipt on `startup` only.
 *
...
function _buildBaselineHooks(projConfig, engineProfile, ruleShardCount = 0) {
  ...
  if (silentPrimeDisposition(projConfig, engineProfile) === 'on') {
    ...
    const entries = [
      {
        matcher: SESSION_START_REENTRY_MATCHER,
        hooks: [
          {
            type: 'command',
...
function writeEngineConfig(engineId, projectPath, projectConfig, engineProfile) {
  ...
  // Its text, and the provenance line it may carry, come from the module that
  // also budgets the prime against everything the SessionStart hook prints, so
  // what is written here and what the prime leaves room for cannot disagree.
  const uiWrapAdvisoryPath = tcProjectFiles.resolveIn(projectPath, tcProjectFiles.UI_WRAP_ADVISORY_RELPATH);
  try {
    provenance.writeOwnedFile(uiWrapAdvisoryPath, 'ui-wrap-advisory', primeHookOutput.UI_WRAP_ADVISORY_TEXT,
      _provenanceContext(projectPath, projectConfig, engineId, null));
  } catch (err) {
    // prawduct:allow prawduct/broad-except -- the advisory is a courtesy note; a
    // failed write must not stop the engine config below from being written.
    log.warn('Could not write the UI wrap advisory', { projectPath, error: err.message });
  }
  ...
  if (!engineProfile || !engineProfile.configFormat || !engineProfile.configFormat.filename) {
    const result = { written: false, skipped: true, skipReason: 'engine has no config file (configFormat.filename is null)', drifted: false, configFilePath: null, error: null };
    // Reported HERE rather than by each caller. Every one of the four writers
    // discarded this skip, which is #1251 — and a guard pairing a report with
    // each call site only holds until a fifth writer appears somewhere the
    // guard does not look. This function already holds both inputs the report
    // needs, so the obligation is discharged where it cannot be forgotten.
    reportMissingConfigCarrier(result, projectConfig, engineProfile, { project: projectPath, engine: engineId });
    return result;
  }
  let content = governed
    ? _generateOperationalBlock(projectConfig, projectPath, engineProfile.configFormat.filename)
  ...
  if (mergeStrategy === 'managed-block') {
    ...
    const { merged, error, repaired } = _mergeManagedBlock(existing, content, engineProfile.configFormat.syntax);
    if (error) {
    ...
    // A repair is the one write that changes bytes OUTSIDE the managed region,
    // so it is the one that must not happen quietly. The drift warning cannot
    // cover it: it is computed from the block a well-formed pair yields,
    // which a misordered pair does not have, so a repaired write would log
    // nothing at all.
    if (repaired) {
      log.warn('managed-block markers were repaired — content outside our region moved', {
        configFilePath,
        engineId,
        howToInvestigate: 'the BEGIN:tangleclaw / END:tangleclaw markers were out of order. They have '
          + 'been rewritten as one block in the first marker\'s place, and whatever sat between them is '
          + 'kept immediately below it — check that it landed where you want it'
      });
    }
    try {
      // Not atomic: a truncated write here loses the other owners' content too.
      // The tmp+rename fix has to keep a read-only carrier a reported failure,
      // which is a decision about operator intent — #1291.
      fs.mkdirSync(path.dirname(configFilePath), { recursive: true });
      fs.writeFileSync(configFilePath, merged);
      if (governed) _reportLegacyDuplicates(merged, configFilePath, projectPath, engineProfile.configFormat.syntax);
  ...
  // The provenance line is TangleClaw's, not the operator's, so drift is
  // judged with it left out of both sides. Otherwise every change TangleClaw
  // makes to the line — the setting turned on or off, a new template, a
  // renamed project, a carrier becoming tracked — would be reported as a
  // hand-edit the operator never made. `writeOwnedFile` applies the line.
  const provenanceSurface = _provenanceSurfaceFor(engineId, engineProfile.configFormat.filename);
  const provenanceCtx = provenanceSurface
    ? _provenanceContext(projectPath, projectConfig, engineId, engineProfile.configFormat.filename)
    : null;
  ...
  if (fs.existsSync(configFilePath)) {
    try {
      ...
      // With no config in the context the setting reads as off, which strips the line.
      const existing = provenanceSurface ? provenance.applyProvenance(provenanceSurface, onDisk, {}) : onDisk;
  ...
  try {
    if (provenanceSurface) {
      provenance.writeOwnedFile(configFilePath, provenanceSurface, content, provenanceCtx);
    } else {
      fs.mkdirSync(path.dirname(configFilePath), { recursive: true });
...
/**
 * The provenance surface a whole-file engine carrier is, or null.
 *
 * Named per bundled engine and its carrier file, never inferred from an
 * extension: an operator profile that reuses an id with a different carrier
 * gets no line, because nobody classified that file (ADR 0019).
 *
 * @param {string} engineId
 * @param {string} filename - The profile's `configFormat.filename`.
 * @returns {string|null} A `lib/provenance.js` REGISTRY key.
 */
function _provenanceSurfaceFor(engineId, filename) {
  const surfaces = { codex: 'codex-config', aider: 'aider-config' };
  const surfaceId = Object.prototype.hasOwnProperty.call(surfaces, engineId) ? surfaces[engineId] : null;
  // The carrier's filename is the registry's, never restated here.
  return surfaceId && provenance.REGISTRY[surfaceId].relPath === filename ? surfaceId : null;
}

/**
 * What a whole-file carrier's provenance line needs: the project's config,
 * its stored name, the engine, and whether the carrier is committed. The
 * last is answered by `_carrierCommitted`, the predicate that keeps identity
 * out of committed carriers (#1619), so the line follows the same answer. While
 * the project has the setting off, none of it is looked up.
 *
 * @param {string} projectPath
 * @param {object} projectConfig
 * @param {string} engineId
 * @param {string|null} filename - The carrier's name, or null for a surface
 *   with no tracked state, which then gets no `committed` answer.
 * @returns {{config: object, project: string|null, engine: string, committed?: boolean}}
 */
function _provenanceContext(projectPath, projectConfig, engineId, filename) {
  // Off means no side effects at all: no store lookup that could log, no git
  // call. Removing a line a file already carries needs neither answer, since
  // an unanswered `committed` counts as committed.
  if (!provenance.resolveProvenance(projectConfig).enabled) return { config: projectConfig, engine: engineId };
  let project = null;
  try {
    const row = store.projects.getByPath(projectPath);
    project = row ? row.name : null;
  } catch (err) {
    log.warn('Could not resolve the project name for the provenance line', { projectPath, error: err.message });
  }
  const ctx = { config: projectConfig, project, engine: engineId };
  // Only a carrier asks git: a private `.tangleclaw/` file has no tracked state to answer.
  if (filename) ctx.committed = _carrierCommitted(projectPath, filename);
  return ctx;
}

/**
 * Which provenance surfaces this project's files would carry the line on, and
 * why each of the others would not — asked by read-only reporting (`tc
 * capabilities`), never by a writer.
 *
 * Each answer is the same gate the writer applies: the prime and re-entry
 * files are written only while silent prime is on, and a whole-file carrier
 * gets the line only while `_carrierCommitted` says git ignores it. Only the
 * surfaces this engine has are listed; ids, never paths (ADR 0019).
 *
 * @param {string} projectPath
 * @param {object} projConfig - The project's config (defaults merged).
 * @param {string} engineId
 * @param {object|null} engineProfile
 * @returns {{enabled: boolean, stamped: string[], unstamped: Array<{surfaceId: string, reason: string}>}}
 */
function provenanceSurfaces(projectPath, projConfig, engineId, engineProfile) {
  const stamped = ['ui-wrap-advisory'];
  const unstamped = [];
  const prime = silentPrimeDisposition(projConfig, engineProfile);
  for (const surfaceId of ['session-prime', 'session-reentry']) {
    if (prime === 'on') stamped.push(surfaceId);
    else unstamped.push({ surfaceId, reason: prime === 'off' ? 'silent prime is off' : 'this engine delivers no hidden prime' });
  }
  const filename = engineProfile && engineProfile.configFormat ? engineProfile.configFormat.filename : null;
  const carrier = filename ? _provenanceSurfaceFor(engineId, filename) : null;
  if (carrier) {
    if (_carrierCommitted(projectPath, filename)) unstamped.push({ surfaceId: carrier, reason: 'git does not ignore it, so it may be committed' });
    else stamped.push(carrier);
  }
  return { enabled: provenance.resolveProvenance(projConfig).enabled, stamped, unstamped };
}

// Where TangleClaw's hooks live, and where they no longer do.
//
// `.claude/settings.json` is the SHARED, COMMITTABLE file: the Claude Code docs
// point operators at it, and it carries the plugin install reference
// (`enabledPlugins` + `extraKnownMarketplaces`) that `lib/governance-state.js`
// anchors governance detection on. TangleClaw's own hooks name an ABSOLUTE path
// to this machine's install, which is valid on exactly one machine — so writing
// them there forced every managed project to choose between a clone that works
// and committed governance, and there is no third option while one file holds
// both (#1022).
//
// `.claude/settings.local.json` is the machine-local half Claude Code already
// provides and already ignores, and it is merged over `settings.json` at load.
// That is where an absolute path belongs.
//
// The tracked file is not merely abandoned: TangleClaw wrote entries into it on
// every launch for as long as this function has existed, so every already-
// configured project holds one. `_retireHooksIn` below removes ours from it and
// leaves the operator's, because a relocation that does not retire the old site
// orphans an unresolvable hook in every project it ever touched — 25 entries
// across 24 projects the last time these same two scripts moved (#1007).
const SHARED_SETTINGS_BASENAME = 'settings.json';
const LOCAL_SETTINGS_BASENAME = 'settings.local.json';

/** The project-relative directory holding the hook settings files above. */
const HOOK_SETTINGS_DIR = '.claude';

/**
 * Tracked settings files TangleClaw writes hook entries into or retires them
 * from, repo-root-relative with forward slashes. The wrap's ownership check reads
 * this rather than naming an engine's directory itself, so which files carry
 * TangleClaw hooks stays this module's knowledge.
 */
const SHARED_HOOK_SETTINGS_PATHS = Object.freeze([`${HOOK_SETTINGS_DIR}/${SHARED_SETTINGS_BASENAME}`]);

/**
 * Reconcile TangleClaw's hook entries within ONE settings file.
 *
 * Reads, merges via `_mergeBaselineHooks` (so foreign hooks and every non-hook
 * key survive), and writes back ONLY when the serialized file actually changes.
 *
 * That last condition is load-bearing rather than an optimization. This function
 * runs on every launch, create, attach, PATCH and boot-sync; an unconditional
 * write left the tracked `settings.json` permanently modified in `git status`,
 * which trained everyone to ignore its diffs and, in this repo, fed the file
 * into the wrap's `git add -A` where the repo's own no-absolute-path guard
 * rejected it and stranded the wrap PR with auto-merge armed (#1242, #1275).
 * A sync with nothing to say now says nothing.
 *
 * A file that does not exist and would gain no hooks is NOT created: an empty
 * `{}` appearing in a project TangleClaw had no reason to touch is the same
 * class of unasked-for write this function exists to stop making.
 * @param {string} settingsFile - Absolute path to the settings file.
 * @param {object} baselineHooks - TangleClaw's current emission for this file
 *   (already placeholder-resolved). `{}` means "retire ours, add nothing".
 * @returns {{ written: boolean, refused: boolean, preservedForeign: number,
 *   replacedOwn: number }} `refused` is true when the file could not be merged
 *   into and was left untouched — distinct from `written: false`, which also
 *   covers the ordinary "nothing changed" case.
 */
function _reconcileHooksFile(settingsFile, baselineHooks) {
  ...
  if (existed) {
    try {
      ...
      // A JSON scalar or array is not a settings object. Starting fresh from `{}`
      // would DELETE it; refusing leaves the operator's file exactly as found and
      // says why, which is the honest degrade for a shape we cannot merge into.
      if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
        log.warn('Settings file is not a JSON object; leaving it untouched', {
          settingsFile,
          projectPath: path.dirname(path.dirname(settingsFile)),
          howToInvestigate: 'the file must hold a JSON object for TangleClaw to merge hooks into it'
        });
        return { written: false, refused: true, preservedForeign: 0, replacedOwn: 0 };
      }
      settings = parsed;
    } catch (err) {
      // Unparseable. The old code started from `{}` here, which silently
      // discarded whatever the operator had — including the install reference
      // governance detection reads. Refuse instead.
      log.warn('Failed to parse settings file; leaving it untouched', {
        settingsFile,
        projectPath: path.dirname(path.dirname(settingsFile)),
        error: err.message
      });
  ...
  // Compare the hooks BEFORE reconciling, and decide on that comparison alone.
  //
  // The tempting predicate — re-serialize the whole file and compare bytes to
  // what was read — answers the wrong question. It conflates "we changed
  // something" with "we would have formatted it differently", so a project whose
  // committed `settings.json` uses four-space indent, tabs, or no trailing
  // newline gets its tracked governance file rewritten by a sync that retired
  // nothing. That is the same unasked-for diff on a committable file this
  // function exists to stop making, one layer up, and it re-arms every time the
  // project's own formatter puts the file back. `repairOrphanHooks` accepts that
  // churn deliberately, but it can: it runs once, by operator request. This runs
  // on every launch, create, attach, PATCH and boot-sync.
  const hooksBefore = JSON.stringify(settings.hooks === undefined ? null : settings.hooks);
  const { hooks: merged, preservedForeign, replacedOwn } = _mergeBaselineHooks(
    settings.hooks, baselineHooks || {}
  );
  if (Object.keys(merged).length > 0) settings.hooks = merged;
  else delete settings.hooks;
  const hooksAfter = JSON.stringify(settings.hooks === undefined ? null : settings.hooks);

  if (hooksBefore === hooksAfter) {
    return { written: false, refused: false, preservedForeign, replacedOwn };
  }
  // Nothing to add and no file to add it to: creating an empty `{}` in a project
  // TangleClaw had no reason to touch is itself an unasked-for write. (Reached
  // only when the hooks DID change, i.e. a baseline exists.)
  if (!existed && Object.keys(settings).length === 0) {
    return { written: false, refused: false, preservedForeign, replacedOwn };
  }

  // tmp + rename, matching `repairOrphanHooks`'s sibling write. The relocation
  // changes who the concurrency partner is: `settings.local.json` is the file
  // Claude Code itself rewrites when the operator approves a permission
  // mid-session, and this function runs on attach and on engine/silentPrime
  // PATCH, either of which can land while a session is live. Same-directory
  // rename is atomic on POSIX, so a crash or a collision mid-write cannot leave
  // a half-written settings file behind.
  fs.mkdirSync(path.dirname(settingsFile), { recursive: true });
  const tmpPath = `${settingsFile}.tmp`;
  fs.writeFileSync(tmpPath, JSON.stringify(settings, null, 2) + '\n');
  fs.renameSync(tmpPath, settingsFile);
  return { written: true, refused: false, preservedForeign, replacedOwn };
}

/**
 * Remove TangleClaw's hook entries from a settings file, adding none.
 *
 * The retirement half of the relocation, and also how a non-claude engine's
 * stale entries are cleared. Never creates the file: nothing to retire in a file
 * that does not exist.
 * @param {string} settingsFile - Absolute path to the settings file.
 * @returns {{ written: boolean, refused: boolean, preservedForeign: number, replacedOwn: number }}
 */
function _retireHooksIn(settingsFile) {
  if (!fs.existsSync(settingsFile)) {
    return { written: false, refused: false, preservedForeign: 0, replacedOwn: 0 };
  }
  return _reconcileHooksFile(settingsFile, {});
}

/**
 * Make sure the project ignores the file TangleClaw is about to write its
 * machine-specific hooks into.
 *
 * The relocation only helps if the destination is actually untracked. On this
 * operator's machine every managed project already ignores it — but
 * `git check-ignore -v` attributes that to their USER-GLOBAL ignore file, not to
 * any project's own `.gitignore`. A user-global rule travels with a home
 * directory, not with a repository, so on a contributor's clone, a second
 * machine, or CI, those same projects do NOT ignore it. TangleClaw would then be
 * the first thing to create an untracked `settings.local.json` holding an
 * absolute path to one install, and a wrap's `git add -A` would commit it —
 * #1022's exact shape, moved to a file the project had no reason to expect.
 *
 * So the precondition is established rather than assumed. The line goes in
 * `.claude/.gitignore`, not the project root's: TangleClaw already owns that
 * directory, a nested `.gitignore` is ordinary git, and editing the operator's
 * root ignore file to suit ourselves is the kind of unasked-for write this whole
 * change is about. Written only when git says the path is not already ignored,
 * so a project covered by any existing rule is left alone.
 *
 * Best-effort throughout: a project that is not a git repository, or a `git`
 * that cannot be run, is not a reason to skip installing the hook — it just
 * means there is no index for the file to be committed into either.
 * @param {string} projectPath - Absolute path to the project root.
 * @param {string} localFile - Absolute path to the settings.local.json target.
 * @returns {boolean} True when this call wrote the ignore line.
 */
function _ensureLocalSettingsIgnored(projectPath, localFile) {
  const relative = path.relative(projectPath, localFile);
  try {
    // `check-ignore` exits 0 when the path IS ignored, 1 when it is not, and
    // 128 when this is not a git repository. Only a clean 1 means "add it".
    const probe = spawnSync('git', ['check-ignore', '-q', '--', relative], {
      cwd: projectPath, timeout: 5000
    });
    if (probe.error || probe.status !== 1) return false;
  } catch {
    return false;
  }

  const ignoreFile = path.join(path.dirname(localFile), '.gitignore');
  const line = path.basename(localFile);
  try {
    let existing = '';
    if (fs.existsSync(ignoreFile)) {
      existing = fs.readFileSync(ignoreFile, 'utf8');
      // Present but not effective (a negation below it, a different pattern
      // shape) is the operator's arrangement to reason about, not ours to
      // duplicate a line into.
      if (existing.split('\n').some((l) => l.trim() === line)) return false;
      if (existing.length > 0 && !existing.endsWith('\n')) existing += '\n';
    } else {
      existing = '# Written by TangleClaw. This file carries hook commands naming an\n'
        + '# absolute path to one machine\'s install, so it must not be committed.\n';
    }
    fs.writeFileSync(ignoreFile, existing + line + '\n');
    log.info('Added the machine-local settings file to the project\'s ignore rules', {
      projectPath, ignoreFile
    });
    return true;
  } catch (err) {
    log.warn('Could not ignore the machine-local settings file; it may be committed by a wrap', {
      projectPath,
      ignoreFile,
      error: err.message,
      howToInvestigate: `add \`${line}\` to \`${path.relative(projectPath, ignoreFile)}\` by hand — `
        + 'the file holds an absolute path to this machine\'s TangleClaw install and breaks any other clone'
    });
    return false;
  }
}
...
/**
 * Sync the project's TangleClaw hooks to match its engine.
 *
 * Writes them to `.claude/settings.local.json` and retires any TangleClaw entry
 * still sitting in the tracked `.claude/settings.json` (see the constants above
 * for why the two files are not interchangeable). The tracked file's non-hook
 * keys are never touched by either pass.
...
function syncEngineHooks(projectPath, engineIdOverride) {
  ...
  const settingsDir = path.join(projectPath, HOOK_SETTINGS_DIR);
  const sharedFile = path.join(settingsDir, SHARED_SETTINGS_BASENAME);
  const localFile = path.join(settingsDir, LOCAL_SETTINGS_BASENAME);
  ...
  // silently resolve as claude here and get baseline hooks written.
  ...
  // The tracked file is retired on EVERY sync, whatever the engine. A project
  // configured before the relocation holds an entry there that resolves on one
  // machine, and nothing else will ever remove it.
  const retiredShared = _retireHooksIn(sharedFile);

  // When the project's current engine is not claude, `.claude/` is not consulted
  // at runtime — but a stale hooks block left over from a prior claude+silentPrime
  // state is still orphan canon. Clear it from the local file too, so a future
  // engine flip back to claude (or any cross-engine audit) doesn't see a phantom
  // entry (#140). Symmetric-capability-gates: the engine PATCH branch must clean
  // up engine-specific state for the same reason silentPrime PATCH does (#137).
  if (engineId !== 'claude') {
    const retiredLocal = _retireHooksIn(localFile);
    if (retiredShared.written || retiredLocal.written) {
      log.info('Cleared stale TangleClaw hooks for non-claude engine', {
        projectPath,
        engine: engineId,
        sharedRetired: retiredShared.replacedOwn,
        localRetired: retiredLocal.replacedOwn,
        preservedForeign: retiredShared.preservedForeign + retiredLocal.preservedForeign
      });
    }
    return;
  }
  ...
  // legacy vendored Stop hook alongside the plugin's.
  //
  // Resolve the engine profile so baseline hooks gate on capability, not just
  // projConfig — keeps this in lockstep with launchSession's silentPrime
  // derivation.
  ...
  // Merge, never replace. The local file may hold hooks the OPERATOR wrote —
  // `settings.local.json` is where Claude Code puts their per-machine config, and
  // this function runs on every session launch, so a wholesale assignment would
  // discard them silently and repeatedly, which is the defect #752 fixed in the
  // shared file. Only the entries TangleClaw itself emits are reconciled.
  const synced = _reconcileHooksFile(localFile, _resolveHooksObject(baselineHooks));

  // Whenever the file exists there is something to keep out of a commit — not
  // only on the sync that created it. Gating on `written` would leave a project
  // whose local file is already in sync unprotected the moment its ignore rule
  // goes away (an operator deleting `.claude/.gitignore`, a `.gitignore` rewrite
  // upstream), and a settled sync is the common case forever after the first.
  if (fs.existsSync(localFile)) _ensureLocalSettingsIgnored(projectPath, localFile);

  if (synced.written || retiredShared.written || synced.refused) {
    log.info('Synced engine hooks', {
      projectPath,
      engine: engineId,
      target: LOCAL_SETTINGS_BASENAME,
      hasBaselineHooks: Object.keys(baselineHooks).length > 0,
      // Stated separately from `written`, because a refused local write beside a
      // successful shared retirement would otherwise read as an installed hook.
      // The refusal's own warn is a different line and easy to miss.
      localWritten: synced.written,
      localRefused: synced.refused,
      // Counted so a regression shows up in the log rather than only in a lost hook.
      preservedForeign: synced.preservedForeign,
      replacedOwn: synced.replacedOwn,
      // Non-zero exactly once per project: the migration off the tracked file.
      retiredFromSharedSettings: retiredShared.replacedOwn
    });
  } else {
    // The settled path writes nothing, which is the point — but it must not also
    // be indistinguishable from never having run. This repo's recurring defect
    // (ADR 0001, #137/#140) is a PATCH path that FORGOT to call this function,
    // and the old unconditional info line was what told those two apart.
    log.debug('Engine hooks already in sync; nothing written', { projectPath, engine: engineId });
  }
}
...
const _internal = {
  // Test seam: the git question `_carrierIsCommitted` asks.
  // Bounded, and its `error` is read: spawnSync REPORTS ENOENT on `{error}`
  // rather than throwing, so a host without git on PATH would otherwise fall
  // back to the hardcoded list in silence — with no warning logged, because
  // nothing threw. This module already records that misreading spawnSync this
  // way broke three earlier checks (#891, #894), and this host's PATH really is
  // stale (#1619), so it is not hypothetical.
  checkIgnore: (projectPath, filename) => {
    const probe = spawnSync(
      'git', ['-C', projectPath, 'check-ignore', '-q', '--', filename],
 ...
 * TC gates on it; the hook re-sync emits TC's L1 prime into
 * `.claude/settings.local.json` and retires any stale vendored governance hook
 * reference from the tracked file (no destructive delete of the project's
 * vendored file). Idempotent: a no-op when
...
module.exports = {
  ...
  // Public names for the two helpers the wrap's ownership check relies on to decide
  // what it may commit without asking (#1509) — a caller outside this module.
  isTangleClawHookEntry: _isTangleClawHookEntry,
  managedBlockMarkers: _managedBlockMarkers,
  _reconcileHooksFile,
  _retireHooksIn,
  TC_HOOK_SCRIPTS,
  TC_LEGACY_HOOK_MARKERS,
  // Exported as a pair so no consumer re-types either basename: a guard asserting
  // "our hooks are NOT in the shared file" must name the same string this module
  // writes, or it passes while pointing somewhere else.
  SHARED_SETTINGS_BASENAME,
  LOCAL_SETTINGS_BASENAME,
  SHARED_HOOK_SETTINGS_PATHS,
  detect,
  detectEngine,
  detectEngineAsync,
  anyEngineInstalled,
  ...
  _generateClaudeMd,
  // Exported so the identity pins can reach the generator that produced the
  // #1619 defect: a plugin-governed project's tracked CLAUDE.md comes from
  // here, not from `_generateClaudeMd`.
  _generateOperationalBlock,
  analyzeGovernedCarrier,
  legacyCarrierContext,
  CLAUDE_MD_GENERATED_HEADER,
  _isCommittedCarrier,
  // Exported so the identity matrix can build its fixtures from the real
  // emitters rather than from copied prose (#1619). The others it needs —
  // `_medusaSwitchboardLines`, `_buildSharedDocsSection` and
  // `GENERATED_HEADER_MARK` — are already exported from this same object.
  _apiOriginDiscoveryLines,
  _carrierIsCommitted,
  _generateCodexYaml,
  _generateAiderConf,
  _generateGeminiMd,
