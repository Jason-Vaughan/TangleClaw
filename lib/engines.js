...
const { tcBootstrapLines } = require('./ecosystem-primer');
 ...
 * the setting is neither honored nor offered there — the settings UI does not
 * render the toggle, so a stored value on such a project is indistinguishable
 * from the shipped default (`DEFAULT_PROJECT_CONFIG.silentPrime` is true) and
 * must not be reported as a preference the operator set and lost.
 *
 * One definition on purpose, like `honorsLaunchMode` above: the launch path,
 * the settings modal and the delivery ledger all answer this question, and a
 * predicate restated at each site is how one of them ends up warning about a
 * default (Train 11 car 6, Critic R-5).
 *
...
function silentPrimeDisposition(projConfig, engineProfile) {
  const supported = Boolean(engineProfile && engineProfile.capabilities
...
function generateConfig(engineId, projectConfig, projectPath) {
  ...
  switch (generator) {
    ...
    case 'gemini-md':
      return _generateGeminiMd(projectConfig, '# GEMINI.md — Generated by TangleClaw', projectPath);
    case 'antigravity-md':
 ...
 * @returns {string}
 */
function _generateGeminiMd(projectConfig, header = '# GEMINI.md — Generated by TangleClaw', projectPath) {
  const lines = [header, ''];
  const rules = _getRulesContent(projectConfig, projectPath);
 ...
 * form so it parses wherever the engine would have read it.
 ...
 * @returns {string} Notice text.
 */
function _inactiveNotice(previousEngineId, activeEngineId, activeFile, syntax) {
  const date = new Date().toISOString().slice(0, 10);
  const sentences = [
    `${INACTIVE_CONFIG_MARK} — this project's engine switched from ${previousEngineId} to ${activeEngineId} on ${date}.`,
  ...
  if (syntax === 'markdown') {
    return [`<!-- ${INACTIVE_CONFIG_MARK} -->`, `# ${sentences[0]}`, '', ...sentences.slice(1)].join('\n');
  }
 ...
 * stays), a whole-file config that carries the generated header is replaced
 ...
 * @returns {{retired: boolean, file: (string|null), reason: string}}
 */
function retireInactiveEngineConfig(projectPath, previousEngineId, activeEngineId) {
  ...
  if (!prev || !prev.configFormat || !prev.configFormat.filename) {
    return { retired: false, file: null, reason: `engine ${previousEngineId} has no config file` };
  }
  ...
  if (activeFile === filename) {
    return { retired: false, file, reason: 'both engines share one config file — the active engine regenerates it' };
  }
  if (!fs.existsSync(file)) return { retired: false, file, reason: 'no config file on disk' };
  if (previousEngineId === 'claude' && isPluginGoverned(projectPath)) {
    return { retired: false, file, reason: 'plugin-governed: the file is the plugin\'s, TangleClaw never owned it' };
  }
  let existing;
  try {
    existing = fs.readFileSync(file, 'utf8');
  } catch (err) {
    return { retired: false, file, reason: `could not read it: ${err.message}` };
  }
  if (existing.includes(INACTIVE_CONFIG_MARK)) return { retired: false, file, reason: 'already marked inactive' };
  ...
  if (markers && existing.includes(markers.begin) && existing.includes(markers.end)) {
    ...
    if (error) return { retired: false, file, reason: `managed-block merge refused: ${error}` };
    try {
      fs.writeFileSync(file, merged);
    } catch (err) {
      return { retired: false, file, reason: `could not write it: ${err.message}` };
    }
    return { retired: true, file, reason: 'managed block replaced by the inactive notice; content outside the markers kept' };
  }
  if (/Generated by TangleClaw/.test(existing.split('\n').slice(0, 3).join('\n'))) {
    try {
      fs.writeFileSync(file, `${notice}\n`);
    } catch (err) {
      return { retired: false, file, reason: `could not write it: ${err.message}` };
    }
    return { retired: true, file, reason: 'generated file replaced by the inactive notice' };
  }
  return { retired: false, file, reason: 'not TangleClaw-written (no generated header, no managed markers) — left alone' };
}
