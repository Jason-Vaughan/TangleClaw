...
function _mergeManagedBlock(existing, blockBody, syntax) {
  ...
  const demoted = syntax === 'markdown'
    ? blockBody.replace(/^# (?!#)/gm, '## ')
    : blockBody;
 ...
 * @param {string} [format] - 'md' or 'comment'
...
function _medusaSwitchboardLines(rules, format = 'md') {
  ...
  if (format === 'comment') {
    return [
      ...
      `#   reply     POST ${api}/send           {"to": "<workspace-id>", "message": "..."}`,
...
When signaling the stopping place, explicitly state which one the operator should use and why.`;
  ...
  const content = generateConfig(engineId, projectConfig, projectPath);
  if (!content) {
  ...
  // profile rather than inferred from the filename: whether a file is shared is
  // a fact about the engine's ecosystem, not about its name. Absent, the value
  // is 'whole-file', so every engine that has not opted in keeps byte-identical
  // behavior.
  const mergeStrategy = engineProfile.configFormat.mergeStrategy || 'whole-file';
