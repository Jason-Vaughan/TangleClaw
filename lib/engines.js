...
async function anyEngineInstalled() {
  ...
  // Still nothing, and we could not read the operator's shell PATH — so we did
  // not actually look where engines get installed. FAIL OPEN. The cost of being
  // wrong in this direction is a dashboard that cannot launch anything, which
  // the operator sees immediately and can fix. The cost of the other direction
  // is a first-run wizard that refuses to finish on a machine where the engine
  // IS installed, with a Check again button that keeps saying no because the
  // check itself is broken — an install with no way out at all. #346 is exactly
  // that wrong answer, and it was cosmetic until this gate made it a wall.
  if (!probe.probed) {
    log.warn('Allowing setup to finish without confirming an engine: the login shell PATH '
      + 'could not be read, so "no engine installed" is not a trustworthy answer', {
      howToInvestigate: 'Engine detection can only see the server\'s own PATH here. If '
        + 'sessions fail to launch, check that the engine binary is on the PATH launchd gives '
        + 'this service.'
    });
    return true;
  }
