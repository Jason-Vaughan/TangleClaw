...
function _generateClaudeMd(projectConfig, methodologyTemplate) {
  ...
  // PortHub guide (when porthubRegistration core rule is active)
  if (coreRules.porthubRegistration !== false) {
    const guidePath = path.join(__dirname, '..', 'data', 'porthub-guide.md');
    try {
      if (fs.existsSync(guidePath)) {
        const guideContent = fs.readFileSync(guidePath, 'utf8').trim();
        lines.push(guideContent, '');
      }
    } catch {
      // Guide file not available, skip silently
    }
    // Inject TangleClaw server URL so the AI knows where to call
    const config = store.config.load();
    const serverPort = config.serverPort || 3101;
    lines.push(`**TangleClaw API base URL**: \`http://localhost:${serverPort}\``, '');
  }
