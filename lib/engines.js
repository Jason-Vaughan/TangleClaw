 ...
 * launch path records the drop is `settingDisposition`'s to decide from the
 * stored value's provenance — a stored `true` is the shipped default
 * (`DEFAULT_PROJECT_CONFIG.silentPrime`) and cannot be reported as a preference
 * anyone set, while a stored `false` is a choice.
 *
