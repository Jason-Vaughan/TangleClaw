...
function _buildSharedDocsSection(docs, options = {}) {
  ...
  const lines = committed
    ...
    'To find `<id>`: `GET $TANGLECLAW_API/api/groups` lists the groups with their ids, and `GET $TANGLECLAW_API/api/groups/<id>/members` says which projects are in one — yours is the group whose members include project `$TANGLECLAW_PROJECT_ID`.', '',
    'Send your project binding on every one of these requests: `-H "x-tangleclaw-project-id: $TANGLECLAW_PROJECT_ID" -H "x-tangleclaw-launch-id: $TANGLECLAW_LAUNCH_ID"`. Both are exported into your pane; the server resolves the launch id to your live session so it can answer for your project. A pane with no `TANGLECLAW_LAUNCH_ID` predates launch binding: relaunch the session.', '',
    ...
    'ALWAYS send `groupId`. Omitting it is not a narrower query, it is an unfiltered one: it answers with every group\'s documents and their absolute paths across this whole install, which is other projects\' business, not yours. If you cannot determine your group, say so and stop rather than issuing the bare request. Lock before editing.', '']
    : ['## Shared Documents', ''];
