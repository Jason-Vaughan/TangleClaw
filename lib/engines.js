...
function _getRulesContent(projectConfig) {
  ...
  // Project version recording guide
  let projectVersionGuide = null;
  {
    const guidePath = path.join(__dirname, '..', 'data', 'project-version-recording-guide.md');
    try {
      if (fs.existsSync(guidePath)) {
        projectVersionGuide = fs.readFileSync(guidePath, 'utf8').trim();
      }
    } catch (err) {
      log.warn('Failed to read project version recording guide', { guidePath, error: err.message });
    }
  }
  ...
  return { coreRulesLines, extensionRulesLines, porthubGuide, sharedDocsGuide, sessionMemoryGuide, projectVersionGuide, globalRules, sharedDocsContent, serverPort, serverProtocol };
}
...
function _generateClaudeMd(projectConfig, methodologyTemplate) {
  ...
  // Project version recording guide
  if (rules.projectVersionGuide) {
    lines.push(rules.projectVersionGuide, '');
  }
...
function _generateCodexYaml(projectConfig, methodologyTemplate) {
  ...
  if (rules.projectVersionGuide) {
    for (const line of rules.projectVersionGuide.split('\n')) {
      instrParts.push(line);
    }
    instrParts.push('');
  }
...
function _generateAiderConf(projectConfig, methodologyTemplate) {
  ...
  if (rules.projectVersionGuide) {
    lines.push('#');
    lines.push('# Project Version Recording:');
    for (const line of rules.projectVersionGuide.split('\n')) {
      const trimmed = line.replace(/^#+\s*/, '').trim();
      if (trimmed) lines.push(`#   ${trimmed}`);
    }
...
function syncEngineHooks(projectPath, methodologyTemplate) {
  // Only sync hooks for Claude projects — .claude/settings.json is Claude-specific
