...
function listWithAvailability() {
  ...
  // Append OpenClaw connections marked as available-as-engine
  try {
    const openclawConns = store.openclawConnections.list({ availableAsEngine: true });
    const openclawProfile = store.engines.get('openclaw');

    for (const conn of openclawConns) {
      results.push({
        id: `openclaw:${conn.id}`,
        name: `${conn.name} (OpenClaw)`,
        interactionModel: openclawProfile ? openclawProfile.interactionModel : 'session',
        available: true,
        command: openclawProfile ? openclawProfile.command : 'ssh',
        capabilities: openclawProfile ? (openclawProfile.capabilities || {}) : {},
        commands: openclawProfile ? (openclawProfile.commands || []) : [],
        category: 'OpenClaw',
        connectionId: conn.id
      });
    }
  } catch (err) {
    log.warn('Failed to load OpenClaw connections for engine list', { error: err.message });
  }
...
function _getRulesContent(projectConfig) {
  ...
  const serverProtocol = config.httpsEnabled ? 'https' : 'http';

  return { coreRulesLines, extensionRulesLines, porthubGuide, sharedDocsGuide, sessionMemoryGuide, globalRules, sharedDocsContent, serverPort, serverProtocol };
}
 ...
 *     or `genesis`; or `generateConfig` returned empty). Callers should
...
function writeEngineConfig(engineId, projectPath, projectConfig, engineProfile, methodologyTemplate) {
  ...
  // Deliberate no-op: engine has no config file (openclaw, genesis).
...
function syncEngineHooks(projectPath, methodologyTemplate) {
  ...
  // Defer to the Prawduct V2 Claude Code plugin when it governs this project
  // (#330 hybrid). The plugin owns governance hooks; TC must NOT write its own
  // .hooks block — it would fire the legacy vendored Stop hook alongside the
  // plugin and break once the vendored script is removed (chunk C2). Strip any
  // TC-generated hooks block, preserving every non-hook key (enabledPlugins,
  // extraKnownMarketplaces, …) so the plugin install reference is untouched.
  if (isPluginGoverned(projectPath)) {
    if (fs.existsSync(settingsFile)) {
      try {
        ...
        if (existing && existing.hooks) {
          ...
          log.info('Cleared TC-generated hooks — project governed by the Prawduct V2 plugin', { projectPath });
        }
      } catch (err) {
        log.warn('Failed to clear hooks for plugin-governed project', { projectPath, error: err.message });
      }
    }
    return;
  }
  ...
  const rawMethodologyHooks = methodologyTemplate && methodologyTemplate.hooks && methodologyTemplate.hooks.claude
