 ...
 */
function _serviceTokenAuthLines(rules, format = 'md', options = { committedCarrier: true }) {
 ...
 */
function _medusaSwitchboardLines(rules, format = 'md', options = { committedCarrier: true }) {
  ...
  if (!options.committedCarrier && !rules.medusaProjectName) return [];
 ...
 * @returns {string}
 */
function _buildSharedDocsSection(docs, options = { committedCarrier: true }) {
  ...
  const committed = options.committedCarrier === true;
  ...
  const lines = committed
    ? ['## Shared Documents', '',
      'These are the group\'s shared documents. Their paths, contents and lock state are install state, so they are not written here — read them through the shared-docs API: `GET $TANGLECLAW_API/api/shared-docs?groupId=<id>`. Get `<id>` from `$TANGLECLAW_API/api/projects/$TANGLECLAW_PROJECT_ID` (its group), and always send it: a request without `groupId` is not filtered and returns every group\'s documents on this install. Lock before editing.', '']
    : ['## Shared Documents', ''];
