...
// (#1619).
//
// LIMIT, not yet closed: this is a default, not a guarantee. It is TangleClaw's
// own ignore file that lists these two, and a managed project is free to track
// its `.codex.yaml` — in which case this classifier would call a tracked file
// private and keep inlining the bearer token into it. The durable answer is to
// ask git for the carrier's actual tracked/ignored state at the write boundary;
// until then, treat the list as the conventional case and the fail-safe default
// below as what protects the unconventional one.
//
 ...
 *
 * A direct unit call passes no project path, and a project may not be a git
 * repository at all. Both land on the list, which is the conventional case;
 * the git question is what covers the project that departs from it.
 *
...
function _buildSharedDocsSection(docs, options = {}) {
  ...
  const lines = committed
    ...
    'Any of these may be locked by another project at any moment — the API reports lock state, this file cannot. Lock before editing, and check the lock first.', '',
