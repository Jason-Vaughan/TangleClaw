...
function _medusaSwitchboardLines(rules, format = 'md') {
  ...
  if (format === 'comment') {
    return [
      ...
      `#   peer why  GET  ${api}/peers/<workspace-id>  (why a peer has not picked up: a reason code)`,
  ...
  return [
    ...
    `| peers | \`GET ${api}/roster\` |`,
    `| why a peer has not picked up | \`GET ${api}/peers/<workspace-id>\` — the wake monitor's latest reason code for a peer on this host (\`local: false\` for one it cannot see) |`,
    '',
