 ...
 *
 * **A row declares `applies` or `caveat`, and may declare both.** `applies` is
 * the all-or-nothing gate: the setting does nothing here, so the control is
 * inert and the `reason` says why. `caveat` is the partial answer: the setting
 * does take effect, and part of what it promises does not. A setting with two
 * halves — one engine-agnostic, one gated — fits neither boolean honestly, and
 * the two toggles that seed an index file and then point a session at it are
 * exactly that shape (#1252). Reporting them as not applying would be false;
 * reporting them as applying, with nothing said, is the silence ADR 0013
 * forbids. A row declaring neither is not engine-conditional and does not
