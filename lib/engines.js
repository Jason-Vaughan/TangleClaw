...
function _generateClaudeMd(projectConfig, projectPath, carrierFilename = 'CLAUDE.md') {
  ...
  // Shared documents
  if (rules.sharedDocsContentShared) {
    // Committed carrier: the neutral rendering only — no fallback to the
    // identity-bearing one, which would fail open (#1619).
    lines.push(rules.sharedDocsContentShared, '');
  }
...
function _buildSharedDocsSection(docs, options = {}) {
  ...
  const lines = committed
    ? ['## Shared Documents', '',
      'These are the group\'s shared documents. Their paths, contents and lock state are install state, so they are not written here — read them through the shared-docs API at `$TANGLECLAW_API/api/shared-docs?groupId=<your group>`, and lock before editing.', '']
    : ['## Shared Documents', ''];
...
const _internal = {
  ...
  checkIgnore: (projectPath, filename) => spawnSync(
    'git', ['-C', projectPath, 'check-ignore', '-q', '--', filename], { stdio: 'ignore' }
  ).status,
