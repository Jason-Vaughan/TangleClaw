...
function _provenanceSurfaceFor(engineId, filename) {
  const surfaces = { codex: ['.codex.yaml', 'codex-config'], aider: ['.aider.conf.yml', 'aider-config'] };
  const entry = Object.prototype.hasOwnProperty.call(surfaces, engineId) ? surfaces[engineId] : null;
  return entry && entry[0] === filename ? entry[1] : null;
}
