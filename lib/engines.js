...
const ENGINE_CONDITIONAL_SETTINGS = {
  ...
  defaultLaunchMode: {
    ...
    applies: (value, engineProfile) => honorsLaunchMode(engineProfile, value),
    reason: (value, engineProfile) => {
      const name = engineDisplayName(engineProfile);
      const mode = typeof value === 'string' && value ? `"${value}"` : 'that launch mode';
      return _profileDeclaresMode(engineProfile, value)
    ...
    evidence: (value, engineProfile) => _profileDeclaresMode(engineProfile, value)
      ? 'mode is disabled'
...
/**
 * Whether a profile declares a launch-mode key at all — the difference between
 * "disabled here" and "never offered here", which is the whole content of the
 * reason the operator reads.
 * @param {object|null} engineProfile - Engine profile.
 * @param {*} mode - Launch mode key.
 * @returns {boolean}
 */
function _profileDeclaresMode(engineProfile, mode) {
  const modes = engineProfile && engineProfile.launchModes;
...
function settingDisposition(setting, projConfig, engineProfile) {
  ...
  const stored = projConfig && Object.prototype.hasOwnProperty.call(projConfig, setting)
    ? projConfig[setting]
    : undefined;
