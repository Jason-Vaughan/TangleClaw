 ...
 * one (#1251).
 *
 * `writeEngineConfig` reports its skip honestly and every call site discarded
 * it, so a project whose rules and guides reach no session produced no trace at
 * all. The settings modal is where the operator learns this — a log line is
 * explicitly not compliance under ADR 0013 — but the log is still owed the
 * event, and the LEVEL is the disposition's to derive: losing rules the
 * operator chose is a warning, losing the stock set is a record. A call site
 * picking its own level is how that asymmetry survives only as long as each
 * author remembers it.
 *
 * One owner for four writers (launch, create, engine PATCH, boot sync). Skips
 * for any other reason — a governed project deferring to its plugin, an empty
 * generated body — are the caller's own business and pass through untouched.
 *
