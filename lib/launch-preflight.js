...
function runPreflight(rawCtx) {
  ...
  if (current.row) {
    ...
    if (recorded) {
      ...
      // 14b — the handoff's git probe FAILED, and said so.
      //
      ...
      //
      // `stale` rather than recovery, and rather than `workspace-unavailable`:
      // the handoff itself is sound — it was published by the newest session,
      // which finished — and what cannot be believed is the tree it describes.
      // That is exactly what the reconciliation verdicts are for. A human says
      // whether the tree is what the document meant; nothing here can.
      evidence.worktreeChecks = 'unverified: the wrap could not read git';
      evidence.worktreeReadFailure = doc.worktreeProblem;
      return decide(VERDICTS.STALE, [
        `the handoff could not read its work tree when it was staged: ${doc.worktreeProblem}`,
        'it records no worktree for that reason, so this launch has nothing to compare against'
      ]);
    } else {
  ...
  if (evidence.worktreeHeadUnverified) {
    okFailures.push(evidence.worktreeReadFailure
