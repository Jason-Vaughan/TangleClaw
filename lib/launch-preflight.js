 ...
 * lost it. Reading `evidence.worktreeWasDirty` here keeps that single exception
...
function needsRecovery(result) {
  ...
  if (result.verdict === VERDICTS.WORKSPACE_UNAVAILABLE) {
    return (result.evidence && result.evidence.worktreeWasDirty) === true;
  }
...
function runPreflight(rawCtx) {
  ...
  // Proposals accumulate as the checks run and are attached to EVERY result.
  // A repair describes the store, not the verdict: a caller that got one is
  // holding a fact about a staged attempt whatever word came back with it, and
  // a path that dropped it would silently strand the attempt it names.
  const repairs = [];
   ...
   * @returns {object}
   */
  const decide = (verdict, reasons) => ({ verdict, reasons, repairs, evidence });
  ...
  for (const row of ctx.publications) {
    ...
    if (_repairable(row, ctx, staged && staged.digest, staged && staged.doc)) {
      repairs.push({ action: 'publish', publicationId: row.publicationId, seq: row.seq });
    }
  }
  ...
  if (current.row) {
    ...
    if (recorded) {
      ...
      if (!probe || probe.toplevelExists === false) {
        evidence.worktreeWasDirty = recorded.dirty === true;
      ...
      const movedHead = probe.headSha !== recorded.headSha;
      ...
      if (movedHead || movedBranch) {
        return decide(VERDICTS.STALE, [
          movedHead ? `HEAD is ${probe.headSha}, the handoff recorded ${recorded.headSha}` : null,
