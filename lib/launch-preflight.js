...
const RECOVERY_VERDICTS = Object.freeze(new Set([
  ...
  // A check that could not be performed is not a check that passed. Failing to
  // establish continuity is not evidence that continuity is sound, so the
  // launch owes recovery rather than proceeding on a verdict nobody reached.
...
/**
 * Is this a preflight result a caller can actually act on?
 *
 * Four sites used to answer this separately and none of them agreed: some
 * checked the verdict string, one checked nothing, and the gate read
 * `requiresRecovery`. That split is what let a partial object — a verdict with
 * no `requiresRecovery` — pass a verdict-only guard and then read falsy at the
 * gate, recording `none` for a launch nobody evaluated.
 *
 * Both fields are required, because both are load-bearing: the verdict is what
 * a reader is shown, and `requiresRecovery` is what the gate obeys. A result
 * carrying one without the other cannot be rendered and enforced consistently,
 * so it is not a result.
 *
 * @param {*} result - Anything a caller received in a preflight's place
 * @returns {boolean} True when it carries both load-bearing fields
 */
function isUsableResult(result) {
  return !!result
    && typeof result === 'object'
    && typeof result.verdict === 'string'
    && typeof result.requiresRecovery === 'boolean';
}
...
function needsRecovery(result) {
  // Missing or malformed evidence owes recovery, exactly as an evaluation that
  // was attempted and failed does. Answering `false` here would let a caller
  // obtain an open gate by handing over nothing, which is the same hole this
  // predicate exists to close — and it cannot be told apart from a real pass by
  // anything downstream.
  if (!result || typeof result.verdict !== 'string') return true;
...
function _normalize(raw) {
  ...
  return {
    ...
    file: {
      ...
      // Carried through, because a field this rebuild drops is gone for good —
      // the same last-hop death `handoffEpoch.present` had. The reason is the
      // only thing that tells an operator in recovery WHERE to look.
...
function runPreflight(rawCtx) {
  ...
  if (ctx.file.state === FILE_STATES.UNREADABLE || ctx.file.state === FILE_STATES.INVALID) {
    return decide(VERDICTS.HANDOFF_CORRUPT, [
      ctx.file.reason
        ? `current.json is ${ctx.file.state}: ${ctx.file.reason}`
        : `current.json is ${ctx.file.state}`
    ]);
  }
  ...
  if (current.row) {
    ...
    if (recorded) {
      ...
      //
      // Recorded evidence the probe could not read is STALE: `recorded` is
      // evidence, the probe is not, and a probe's silence does not retract it.
      // BOTH clauses key on their own recorded value for that reason — a guard
      // that also required the probe would let an unreadable probe turn a real
      ...
      // The SAME rule, and it has to be the same rule. `!!probe.branch` would
      // let an unreadable probe retract recorded evidence — which is precisely
      // what `movedHead` was changed to stop doing, so leaving it here keeps
      // the wrong verdict alive in the sibling clause: a document recording a
      // branch with no sha, against a probe that could read neither, would
      // answer OK for a tree nobody read on either side.
      const movedBranch = !!recorded.branch && probe.branch !== recorded.branch;
      // The RECORDED side is what decides this, not the pair. A probe that
      // succeeded says where the tree is NOW; it never says it is the tree the
      // handoff described. So a document that established neither a head nor a
      // branch is unverifiable against ANY probe, readable or not — and
      // `movedHead`/`movedBranch` are both false in that case, because each
      // guards on its own recorded value, which is how an unverified worktree
      // reached `ok`.
      //
      // Keyed on the recorded side alone, this covers all three inputs: both
      // absent, and recorded-absent against a probe that read fine. The fourth
      // — recorded present, probe unreadable — is a real mismatch and stays
      // STALE above, because the recorded value is evidence and the probe's
      // silence does not retract it.
      if (!recorded.headSha && !recorded.branch) {
        evidence.worktreeHeadUnverified = true;
      }
      // The producer says so outright when its reading went short, and that is
      // strictly better evidence than inferring a gap from a null: a null means
      // "unborn repository" just as readily as "git could not be read". A
      // document that names either tree field as unestablished cannot verify
      // the tree with the other one.
      const short = Array.isArray(recorded.unestablished) ? recorded.unestablished : [];
      if (short.includes('headSha') || short.includes('branch')) {
        evidence.worktreeHeadUnverified = true;
        evidence.worktreeReadFailure = recorded.readFailure || null;
      }
      if (movedHead || movedBranch) {
        return decide(VERDICTS.STALE, [
          ...
          movedHead
            ...
            : null,
          // The SAME two arms as the head clause above, for the same reason it
          // has them: "branch is null" states a reading nobody took and sends
          // an operator to look at a branch that was never reported.
          movedBranch
            ? (probe.branch === null || probe.branch === undefined
              ? `the recorded worktree's branch could not be read; the handoff recorded ${recorded.branch}`
              : `branch is ${probe.branch}, the handoff recorded ${recorded.branch}`)
            : null
        ].filter(Boolean));
      }
    } else if (doc.worktreeProblem) {
      // The handoff's git probe FAILED, and said so.
      //
      // A null `worktree` used to mean one thing here, and the producer wrote
      // it for two: a project with no git identity, and a probe that could not
      // run, timed out or was refused. Only the first has nothing to check.
      // The second is the most reassuring possible reading of a failure — it
      // skips 13 and 14 and satisfies a precondition of `ok`, so a wrap whose
      // git probe timed out handed the next launch a clean bill of health for a
      // tree nobody measured, in bytes frozen at staging and unrepairable after.
      //
      // Routed through `worktreeHeadUnverified` rather than returning a verdict
      // of its own, and BOTH halves of that matter.
      //
      // The mechanism, because this is the same fact that clause already
      // carries: a worktree neither end could establish is not a verified
      // worktree. Reusing it is also what gives the two evidence keys below a
      // reader — `worktreeReadFailure` is read by check 15's clause, which an
      // early return here would have skipped, leaving a key nothing consumes.
      //
      // The strength, because a verdict of its own got it backwards. This is
      // STRICTLY WEAKER evidence than the `unestablished` case twenty lines
      // above: there, git answered and only HEAD or the branch went short, and
      // that already forces recovery by falling through to 15. Here git never
      // answered at all. Reporting the total failure as reconciliation-only
      // while the partial one demands recovery inverts the ladder, and it is
      // the same "a failure reads as the reassuring outcome" defect this whole
      // check exists to end. Falling through also lets 15 collect its other
      // failed preconditions, so a handoff that is behind its session AND
      // unreadable names both rather than only the one that returned first.
      evidence.worktreeChecks = 'unverified: the wrap could not read git';
      evidence.worktreeHeadUnverified = true;
      evidence.worktreeReadFailure = doc.worktreeProblem;
    } else {
      // Recorded as a non-git project — and now provably that, rather than a
      // failure wearing the same null. 13 and 14 have nothing to check, and 15
  ...
  if (evidence.worktreeHeadUnverified) {
    // Three ways to be unverified, and they send a reader to different places:
    // the wrap could not run git at all, git ran and the reading went short, or
    // the document named neither field. Collapsing the first into the second
    // would tell an operator a probe went short when none ever ran.
    okFailures.push(evidence.worktreeChecks === 'unverified: the wrap could not read git'
      ? `the handoff could not read its work tree when it was staged (${evidence.worktreeReadFailure}), so it records no worktree and this launch has nothing to compare against`
      : evidence.worktreeReadFailure
        ? `the handoff's own git reading went short (${evidence.worktreeReadFailure}), so its worktree is unverified whatever this launch reads`
        : 'the handoff recorded neither a HEAD nor a branch for its worktree, so it is unverified whatever this launch reads');
  }
