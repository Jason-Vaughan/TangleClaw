...
/**
 * Assessment reasons that mean the engine is doing something. Every other
 * refusal (a filled composer, a dialog, a first unconfirmed observation) means
 * only that rest is not established.
 */
const BUSY_REASONS = new Set(['turn-in-flight', 'agents-running', 'not-at-rest', 'pane-writing']);
...
function createObserver(overrides = {}) {
  ...
  async function observe(session) {
    ...
    let cap;
    try {
      cap = await deps.capture(session.tmuxSession, CAPTURE_TIMEOUT_MS);
    } catch (err) {
  ...
  async function tick() {
    ...
    let i = 0;
    for (; i < n; i++) {
      if (deps.now() - start >= TICK_BUDGET_MS) break;
