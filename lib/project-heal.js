...
function healOnLaunch(projectPath, opts = {}) {
  ...
  if (!where.ok) {
    ...
    // Paths the operator chose to keep tracked are not raised again (#1512).
    const kept = new Set(wrapState.readUntrackDeclined(projectPath));
    result.trackedState = tracked.paths.filter((p) => !kept.has(p));
