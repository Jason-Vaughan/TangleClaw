 ...
 * 3. reports which state files git still tracks — an exclude cannot hide a
...
const _internal = {
  git: (cwd, args) => execFileSync('git', args, {
    cwd, encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'], timeout: GIT_TIMEOUT_MS, maxBuffer: 8 * 1024 * 1024
  }),
...
function healOnLaunch(projectPath, opts = {}) {
  ...
  if (!migration.migrated && /could not/.test(migration.reason)) problems.push(migration.reason);
  ...
  if (!where.ok) {
    ...
    result.trackedState = tracked.paths;
