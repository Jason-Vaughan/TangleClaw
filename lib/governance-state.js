...
/**
 * The engine a session's work runs on: the session's own engine (the attempt),
 * else the project row's. One definition, because every copy of this rule is a
 * chance to read a field the record does not have.
 *
 * @param {{engineId?: string|null}|null} session - The session being wrapped or launched
 * @param {{engineId?: string|null}|null} project - The project row
 * @returns {string|null}
 */
function sessionEngineId(session, project) {
  return (session && session.engineId) || (project && project.engineId) || null;
}
...
module.exports = {
  isPluginGoverned, governanceState, isOnboarded, methodologyCapability, withheldPrefixesFor, sessionEngineId,
