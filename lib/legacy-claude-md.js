...
/**
 * Whether a legacy section's body is the managed block's copy, as the
 * whole-file layout wrote it. That generator appended lines with no heading of
 * their own (the API base URL, the service-token pointer) straight after the
 * PortHub guide, so they fall inside the legacy PortHub section while the
 * block carries them in another section. Such trailing lines match when each
 * appears verbatim somewhere in the block; anything else is a difference.
 * @param {string[]} body - The legacy section's lines.
 * @param {string} managed - The block's copy of the section, trimmed.
 * @param {Set<string>} blockLines - Every line the block carries.
 * @returns {boolean}
 */
function _matchesManagedCopy(body, managed, blockLines) {
  const legacyLines = body.map(_bare).join('\n').trimEnd().split('\n');
  const managedLines = managed.split('\n');
  if (legacyLines.length < managedLines.length) return false;
  for (let i = 0; i < managedLines.length; i++) {
    if (legacyLines[i] !== managedLines[i]) return false;
  }
  return legacyLines.slice(managedLines.length).every((line) => line === '' || blockLines.has(line));
}
...
function analyzeLegacyCarrier(text, ctx) {
  ...
  for (let k = 0; k < heads.length; k++) {
    ...
    candidates.push({
      ...
      matchesManagedCopy: _matchesManagedCopy(body, block.sections.get(heading), blockLineSet)
    });
  }
