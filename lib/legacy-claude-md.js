...
function repairCommand(projectPath, digest) {
  const base = `node "${REPAIR_SCRIPT}" "${projectPath}"`;
  return digest ? `${base} --apply ${digest}` : base;
}
...
function applyLegacyRepair(filePath, expectedDigest, ctx) {
  ...
  try {
    ...
    if (_sha256(fs.readFileSync(filePath, 'utf8')) !== analysis.fileSha256) {
      fs.rmSync(tmp, { force: true });
      return refuse('the file changed while the repair was being written — nothing was replaced', analysis);
    }
