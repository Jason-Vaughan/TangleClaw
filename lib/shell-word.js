...
/**
 * Invert {@link shellWord} for a string that is exactly one word.
 *
 * A thin name over {@link firstWord} for callers that already hold the word and
 * want to say so.
 *
 * @param {string} word - One shell word, quoted or bare
 * @returns {string} The value the shell would pass along
 */
function unshellWord(word) {
  return firstWord(word);
}

module.exports = { shellWord, unshellWord, firstWord };
