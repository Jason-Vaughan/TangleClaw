...
/**
 * The Codex startupControl adapter (#1825 Chunk B2): TangleClaw's side of
...
/** How long a thread must stay idle before an absent turn means "never sent" (E7). */
 ...
 * spawns; the probes fill it. A failed probe EMPTIES it (E9): a stale value is
 * never reused.
 ...
 * bounded, and cache the answer (E9). Used on the launch path only, which is
 ...
 * recorded with), then recover every in-flight fire without resending (E1,
 * E5). A channel that does not answer is closed as lost; its fires are
...
async function recover(deps = {}) {
  ...
  try {
    ...
    log.warn('startupControl recovery could not list channels', { error: err.message });
    return out;
  }
  const live = new Map();
  for (const channel of channels) {
  ...
  try {
    ...
    log.warn('startupControl recovery could not list fires', { error: err.message });
    return out;
  }
  for (const fire of fires) {
    ...
    const onUpdate = (patch) => {
      const r = store.startupPrompts.updateFire(fire.id, patch);
      return r.fire || store.startupPrompts.getFireById(fire.id);
    };
...
async function start() {
  ...
  try { await recover(); } catch (err) { log.warn('startupControl recovery failed at boot', { error: err.message }); }
...
class AppServerConnection extends EventEmitter {
  ...
  _onMessage(text) {
    ...
    try {
      ...
      this.emit('error', new Error('app-server sent a message that is not JSON'));
      return;
    }
...
/**
 * Whether the account can run a turn (E4 as ruled): only an explicit
...
/**
 * Fire the prompt at a session's channel and drive the receipt (E5).
 *
...
async function _fire(input, deps, onAccepted) {
  ...
  if (!channel || channel.adapter !== NAME) {
    return onUpdate({ outcome: 'blocked', reasonCode: 'engine_not_ready', reason: 'this session has no native channel: it was launched without one' });
  }
  ...
  try {
    ...
    if (!row || row.outcome !== 'dispatching') return row;
...
class TurnWatcher {
  ...
  async consider(turn, fromReadBack = false) {
    ...
    if (!this.acceptedSeen && !fromReadBack) {
      const found = await this.readBack();
      if (found || this.finished) return;
    }
    if (this.finished) return;
   ...
   * @returns {Promise<boolean>} Whether the turn was found.
   */
  async readBack() {
    if (this.finished) return true;
    const { turns } = await _allTurns(this.conn, this.threadId);
    const turn = turns.find((t) => t && t.id === this.turnId);
    if (turn) await this.consider(turn, true);
    return !!turn;
  }
...
/**
 * Settle an indeterminate fire from the engine's own record (E7). Never
...
async function reconcile(input, deps = {}) {
  ...
  try {
    ...
    if (turn) {
      ...
      return onUpdate({ outcome: 'accepted', reason: null, engineTurnId: turn.id });
    }
