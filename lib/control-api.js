...
function _syncHooks(projectId) {
  try {
    const project = store.projects.get(projectId);
    if (project) require('./sessions')._syncControlHooks(project);
  } catch (err) {
...
function check(query) {
  ...
  // `blocked` is the gate's own answer, so a hook never re-derives it: a
  // stopped assignment an operator successor has superseded reads `closed`,
  // and work under it is still refused.
  const stopped = row.state === control.STATES.STOPPED || (row.state === control.STATES.CLOSED && !!row.stopped_at);
  const blocked = row.state === control.STATES.HELD || stopped;
  return {
    status: 200,
    body: {
      assignmentId: row.assignment_id, state: row.state, stateGeneration: row.state_generation,
      blocked, code: row.state === control.STATES.HELD ? 'CONTROL_HELD' : (stopped ? 'CONTROL_STOPPED' : null)
    },
    notify: null
  };
}
...
async function notify(change) {
  ...
  try {
    ...
    if (!to) {
      ...
      await _internal.sendSystemMessage({
        to,
  ...
  try {
    control.recordNotify(change.event.eventId, outcome);
  } catch (err) {
