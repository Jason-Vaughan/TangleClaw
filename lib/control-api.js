...
function closeAssignment(req, params, body) {
  const caller = _principal(req);
  const b = body || {};
  return _commandResponse(200, control.close({ assignmentId: params.id, requestId: b.requestId, reasonCode: b.reasonCode }, caller.actor));
}
...
function mine(req) {
  ...
  return { status: 200, body: { ...st, boundToThisLaunch: _isBoundTarget(caller, row) }, notify: null };
}
...
function check(query) {
  ...
  return { status: 200, body: { assignmentId: row.assignment_id, state: row.state, stateGeneration: row.state_generation }, notify: null };
}
