...
function launchSession(projectName, options = {}) {
  ...
  log.info('Session launched', {
    ...
    // this is the launch-time half of S3, and the fire route's later refusal
    // points back at it.
 ...
 * @param {string|null} enginePath - The resolved engine executable this launch runs (E9: the adapter probes exactly that binary).
...
function _prepareStartupChannel(engineId, project, engineProfile, launchCmd, enginePath) {
  const none = (reasonCode, reason) => ({ command: null, handle: null, adapter: null, reasonCode, reason });
  ...
  if (!resolved.supported) return none(resolved.reasonCode, resolved.reason);
  let prepared;
  try {
  ...
  if (!prepared || !prepared.ok) {
    ...
    return none(prepared && prepared.reasonCode ? prepared.reasonCode : 'channel_unavailable', prepared ? prepared.reason : 'no answer from the adapter');
  }
...
/**
 * End the native startup channel of a session that has ended. Every adapter
 * is asked, because the session row does not say which one owns the channel
 * and an adapter with no channel for the session answers null.
...
function _releaseStartupChannel(session, reason) {
  ...
  for (const adapter of Object.values(startupControl.ADAPTERS)) {
    if (typeof adapter.releaseSession !== 'function') continue;
    try {
