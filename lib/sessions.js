...
function _collectPrimeSections(project, engineProfile, options = {}) {
  ...
  // Which of the four carriers this session's rule TEXT rides. Decided once,
  // here, because the block that ships and the sentence that names its channel
  // must agree — a session told the wrong one goes looking where the rules are
  // not, which is #749 restated.
  //
  // `pull-pointer` requires BOTH the setting and a sequence that will actually
  // serve the rules (#1584). On an engine that declares no launch sequence, or
  // a launch whose prime is disabled, the paste stands: a pointer to a channel
  // this session does not have delivers nothing at all.
  const pasteRules = projectConfig.resolvePasteRules(projConfig);
  if (pasteRules.warning) log.warn('Project config falls back for the rules carrier', { project: project.name, warning: pasteRules.warning });
  const delivery = pull
    ? 'launch-step'
    : (viaStartupHook
      ? 'hook'
      : (pasteRules.mode === 'pull' && options.launchSequence === true ? 'pull-pointer' : 'prime'));
  // The one case where the setting asked for something this launch cannot do.
  // Said out loud, because from outside it looks identical to a project that
  // never set it: the rules are pasted either way, and the prime's rule-sources
  // line honestly says "in this prime" — which is the answer, not the reason.
  if (!pull && !viaStartupHook && pasteRules.mode === 'pull' && options.launchSequence !== true) {
    log.info('Project rules stay pasted: this launch has no sequence to pull them from', {
      project: project.name, engine: engineProfile && engineProfile.id
    });
  }
