 ...
 *
 * Strictly more conservative than `detectIdle`: it additionally requires the
 * pane to LOOK at rest (no busy marker, no running agent fleet, an empty
 * composer or a bare prompt) and to hold still across consecutive polls.
 *
...
function detectAtPrompt(tmuxSession, engineId) {
  ...
  const now = Date.now();
  const since = assessed.digest === (prev && prev.digest) && prev ? prev.since : now;
  ...
  return {
    ...
    lastOutputAge: Math.floor((now - since) / 1000)
  };
}
