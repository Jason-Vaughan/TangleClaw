...
function _legacyDuplicateNote(projectPath, filename, engineProfile) {
  ...
  return `\`${filename}\` still holds ${n} legacy TangleClaw section${n === 1 ? '' : 's'} above the Prawduct anchor that the managed block duplicates. Nothing was removed; tell the operator, who can preview a repair with \`node scripts/repair-governed-claude-md.js "${projectPath}"\` (#1911).`;
}
