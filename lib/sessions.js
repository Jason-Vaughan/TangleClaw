...
function _buildLaunchSnapshot(launchId, project, engineProfile, applicability, startupRules, primeOptions, preflightResult, projConfig) {
  ...
  // prawduct:allow prawduct/broad-except -- a launch must survive a failure to render its launch steps
    ...
    return {
      ...
      // discard a real answer and report that nothing was checked. The recovery
      // state rides with it for the same reason — a launch with no steps to
      // serve still has a handoff that was or was not sound, and the dashboard
      // reads that from this row.
      preflight,
