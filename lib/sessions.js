...
function launchSession(projectName, options = {}) {
  ...
  // The provenance line is added to the prime file after the prime is final,
  // so a silent prime is fitted to its hook budget minus that line: the engine
  // discards an over-budget payload whole, which would lose the prime rather
  // than the line.
  ...
  const provenanceReserve = silentPrime ? provenance.lineOverhead('session-prime', primeProvenance) : 0;
  ...
  if (silentPrime && primeText) {
    ...
    _writeReentryFile(project.path, project);
  } else {
 ...
 * @returns {string|null} - Absolute path to the written file, or null on failure
 */
function _writeReentryFile(projectPath, project) {
  try {
    const fs = require('node:fs');
    const path = require('node:path');
    const filePath = tcProjectFiles.resolveIn(projectPath, tcProjectFiles.SESSION_REENTRY_RELPATH);
    fs.mkdirSync(path.dirname(filePath), { recursive: true });
    fs.writeFileSync(filePath, renderReentryPreamble(project));
    return filePath;
  } catch (err) {
