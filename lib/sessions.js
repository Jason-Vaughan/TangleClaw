...
function launchSession(projectName, options = {}) {
  ...
  // #240 drift-aware write — the helper warns when the on-disk file
  // differs from regenerated content, surfacing silent-clobber bugs
  // (e.g. PR-driven CLAUDE.md edits that bypass the canonical store).
  const methodologyTemplate = store.templates.get(project.methodology);
  if (engineProfile.configFormat) {
