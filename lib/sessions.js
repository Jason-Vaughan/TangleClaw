...
function launchSession(projectName, options = {}) {
  ...
  if (wrapping) {
    ...
    if (ageMs > STALE_WRAPPING_THRESHOLD_MS) {
      ...
      if (probe.answered && !probe.live) {
        ...
        // This branch is a terminal path for the row, so the cached pane text has
        // no later consumer; without this it lives for the life of the process.
        _wrapPaneCache.delete(wrapping.id);
      }
  ...
  if (options.primePrompt !== false) {
    primeText = generatePrimePrompt(project, engineProfile, { medusaWorkspaceId, startupRules, continuityMode: options.continuityMode, operatorHost: options.operatorHost });
  }
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  _recordIndexPointerSkipped('featureIndexEnabled', projConfig, engineProfile, project);
  ...
  _recordIndexPointerSkipped('projectMapEnabled', projConfig, engineProfile, project);
...
/**
 * Build the wrapping answer.
 *
 * Three branches report a wrapping session — the probe could not answer, the
 * pane is live, the pane is gone — and each used to assemble the same eleven
 * keys by hand. They had already drifted: one omitted `lastEngineError`, and
 * "there is no idle reading" was encoded three different ways, which a consumer
 * cannot tell apart.
 *
 * One shape, one encoding: an unestablished reading is always `null`, and
 * `incomplete` names the returned fields the branch could not establish. A pane
 * that no longer exists leaves `incomplete` empty — those readings did not FAIL,
 * the question stopped applying — and `wrapFinished` is what says so.
 * @param {string} projectName - The project's name.
 * @param {object} wrapping - The wrapping session row.
 * @param {{idle?: number|boolean|null, lastOutputAge?: number|null,
 *   incomplete?: string[], cause?: string|null, wrapFinished?: boolean}} [readings]
 *   What this branch established. Anything omitted reports as unestablished.
 * @returns {object} The status payload.
 */
function _wrappingStatus(projectName, wrapping, readings = {}) {
  const out = {
    active: false,
    wrapping: true,
    ...
    // failure makes every consumer probe for its existence instead of reading
    // its value.
    incomplete: readings.incomplete || [],
    cause: readings.cause === undefined ? null : readings.cause,
    // `idle` is null, never false, when there is no reading. A definite "not
    // idle" is precisely the plausible default this family exists to stop
    // shipping, and the session page treats idle as a wrap-completion signal, so
    // a false there answers the wrong question. Null is falsy, so a consumer
    // that predates these states behaves exactly as it did before.
    idle: readings.idle === undefined ? null : readings.idle,
    lastOutputAge: readings.lastOutputAge === undefined ? null : readings.lastOutputAge,
    lastEngineError: engineErrors.get(wrapping.id)
  };
  if (readings.wrapFinished) out.wrapFinished = true;
  return out;
}
 ...
 *
 * @param {string} projectName - Project directory name
 * @returns {{ active: boolean|null, wrapping?: boolean, wrapFinished?: boolean,
 ...
 *   `wrapFinished` is the client's cue to POST `/wrap/complete`: this function
 *   reports that a wrap appears over and finalizes nothing itself (#910).
 */
function getSessionStatus(projectName) {
  ...
  if (active) {
    ...
    if (probe && probe.answered && !probe.live) {
      ...
      if (probe && probe.live) {
        ...
        // gate the wake monitor already uses. The wrapping-session site below
        // deliberately stays on `detectIdle`: it drives wrap-completion, which
        // was built against output-staleness.
  ...
  if (wrapping) {
    ...
    if (!wrapProbe.answered) {
      // A probe that could not establish the pane's state reports nothing about
      // it (#908). Reported as still wrapping with the liveness named, rather
      // than as finished: `wrapping: true` is what the row actually says, and
      // `live` is the field that could not be established. The launch path's age
      // recovery still claims the row within the hour, so nothing is stuck.
      //
      // `incomplete` names the RETURNED fields that could not be established,
      // which is the convention `session.active` and `git.dirty` already follow
      // — not the internal concept behind them. Reaching the pane is what
      // produces idle and lastOutputAge, so a pane we could not reach leaves
      // both unestablished; that the LIVENESS is unknown is what a non-null
      // `cause` on a wrapping answer means.
      return _wrappingStatus(projectName, wrapping, {
    ...
    // tmux ANSWERED that the pane is gone, so the wrap is over. Reporting that is
    // the whole of this function's job: completion is an action, so an action
    // performs it (#910). A caller that sees `wrapFinished` posts to
    // `/api/sessions/:project/wrap/complete`; the launch path below claims a
    // stale row on its own (#105), so a wrapping row cannot become unrecoverable
    // because nobody was watching the page.
    return _wrappingStatus(projectName, wrapping, {
      wrapFinished: true,
      // The pane is gone, so there is no reading to take. `incomplete` is empty
      // because these are not fields that FAILED to be established — the
      // question does not apply to a pane that no longer exists.
      idle: null,
      lastOutputAge: null
    });
  }
 ...
 * call that prints nothing reads as idle. Deliberately kept for the consumers
 * that were built against exactly that meaning and document it —
 * `lib/wrap-steps/ai-content.js` (which states its dependence on "≥10s of
 * unchanged pane output") and `lib/actions/invoke-critic.js`. Migrating those
 * is a separate behaviour change with its own risk, not a side effect of
 * fixing the chime.
 *
...
async function _triggerWrapV2(project, active, options) {
  ...
  try {
    pipelineResult = await wrapPipeline.runWrapPipeline(project.name, {
      ...
      onStepEvent: (event) => wrapRunRegistry.emit(project.name, claim.runId, event)
    });
  } catch (err) {
    ...
    wrapRunRegistry.finish(project.name, claim.runId, failed);
    return failed;
  }

  // Everything from here to `finish` is inside a try/finally, because
  // `finish` is the run's ONLY terminal transition: it clears `running`,
  // appends `run-done`, and ends every open stream subscriber (#185). A throw
  // between the pipeline returning and that call — `_completeV2Wrap` tearing
  // down tmux, `wrapShape()`, a logger — used to leave the project's
  // single-flight slot held forever, every SSE socket hanging open with no
  // terminal frame, and the next wrap refused with 409 until the 30-minute
  // stale takeover. The pipeline itself already ran; the reporting around it
  ...
  try {
    ...
    const result = {
      ok: pipelineResult.ok,
      // #185 — the handle for GET /wrap/stream/:runId. The POST returns it with
      // the final result; a client wanting it earlier reads it off /wrap/status.
      runId: claim.runId,
      sessionId: active.id,
    ...
    settled = wrapRunRegistry.finish(project.name, claim.runId, result);
    return result;
  } finally {
    // The pipeline ran and its result is lost to the throw, but the run must
    // still reach a terminal state — otherwise the slot and every subscriber
    // outlive it. The recorded outcome names the reporting failure rather
    // than claiming the wrap itself failed: the commit, if there was one,
    // landed.
    if (!settled) {
      wrapRunRegistry.finish(project.name, claim.runId, {
        ok: false,
        runId: claim.runId,
        sessionId: active.id,
        wrapCommand: null,
        ...
        captureFields: [],
        pipelineResult: null,
        error: 'wrap pipeline finished but reporting threw — check the server log; any commit it made still landed'
      });
    }
  }
}
 ...
 *   `runId` (#185) is the handle `GET /wrap/stream/:runId` takes; this route is
 *   where a client learns it, because the wrap POST does not return until the
 *   pipeline has ended.
 */
 ...
 *   blocked step.
 * @returns {Promise<{ ok: boolean, runId: string|null, sessionId: number|null, wrapCommand: string|null, wrapSteps: string[], captureFields: string[], pipelineResult?: object, error: string|null }>}
 *   `runId` (#185) rides every outcome — success and thrown pipeline alike —
 *   so a caller holding the POST's return can still address the run it just
 *   watched. `null` only on the paths that never claimed one (refused POST).
 */
...
function completeWrap(projectName, summary, expectedSessionId) {
  ...
  // Recover the summary from the pane cache when the caller supplies none, so a
  // finalize records what the pane said rather than nothing — that text becomes
  // the wrap commit subject, which is the permanent record (#910).
  //
  // BOTH of this route's callers post an empty body today, so the cache is what
  // actually decides and `summary` is a parameter for a caller that does not yet
  // exist. The fallback is gated on the cache HOLDING something rather than on
  // who is asking: with nothing cached a finalize records `null`, and that is
  // deliberate — `parseWrapSummary` falls back to the last 50 raw pane lines when
  // it finds no headings, and that text is injected into the next session's
  // prime. A wrong summary is worse than an absent one.
  let effectiveSummary = summary;
  if (effectiveSummary === undefined || effectiveSummary === null) {
    const cached = _wrapPaneCache.get(target.id);
    if (cached) {
      effectiveSummary = parseWrapSummary(cached, wrapDefaultPipeline.wrapShape().captureFields);
    }
  }

  // Update session record
  const session = store.sessions.wrap(target.id, effectiveSummary);
