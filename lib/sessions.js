 ...
 * legacy NL-prompt-via-tmux flow. Gated by `projConfig.wrapV2` (default
 * `false`). In Chunk 3 every step kind is a no-op stub, so this path's
 * user-visible effect is "pipeline ran, returned ok:true, nothing else
 * happened" — no tmux send, no session state transition. Real step
 * implementations land in Chunks 4–9; the state-transition + completion
 * flow lands alongside the `commit` step in Chunk 9 and the multi-step
 * UI in Chunk 10.
 ...
 */
async function _triggerWrapV2(project, active) {
 ...
 */
async function triggerWrap(projectName) {
  const project = store.projects.getByName(projectName);
  if (!project) {
  ...
  // #139 Chunk 3 — `wrapV2` opt-in routes to the new server-side pipeline
  // runner. Stays `false` by default through Chunk 10 so existing wrap
  // behavior is byte-equal to pre-#139. Chunk 11 flips the default.
  const projConfig = store.projectConfig.load(project.path);
  if (projConfig && projConfig.wrapV2 === true) {
    return _triggerWrapV2(project, active);
  }
