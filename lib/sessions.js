...
/**
 * Run a wrap whose slot is already claimed, to a settled registry entry. On a
 * successful pipeline run that produced a commit, the session record is
 * transitioned to `wrapped` + tmux is killed + doc locks are released
 * (#139 Chunk 11a). Halted / thrown / clean-session (ok + null SHA) paths leave
 * the session active so the user can retry or continue.
 *
 ...
 * @param {object} [options] - Per-wrap user choices forwarded to the runner
 *   (e.g. `{skipTests, prHandling}`), collected by the wrap modal and drawer.
 * @param {string} runId - The claimed run
 ...
 *   runner (`{skipTests, prHandling, skipAiContent, bumpLevel, …}`).
 * @returns {{ok: true, runId: string, sessionId: number, done: Promise<object>} | {ok: false, code?: string, sessionId: number|null, error: string}}
 *   `done` resolves to the run's `triggerWrap`-shaped outcome and never rejects.
 *   A refusal (no project, no active session, a run already in progress) claims
 *   nothing and carries no `done`.
 */
function startWrap(projectName, options) {
  const target = _wrapTarget(projectName);
  if (target.refusal) return target.refusal;
  return _startWrapPipeline(target.project, target.active, options);
}
 ...
 * @returns {Promise<{ ok: boolean, runId?: string, sessionId: number|null, wrapCommand: string|null, wrapSteps: string[], captureFields: string[], lifecycleCompleted?: boolean, pipelineResult?: object, error: string|null }>}
