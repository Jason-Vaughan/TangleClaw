...
function launchSession(projectName, options = {}) {
  ...
  // the setting real for EVERY caller — the landing page's hidden-picker
  // direct launch, ClawBridge, and raw API POSTs alike. 'default' adds no CLI
  // args downstream, so leaving launchMode unset for it preserves the
  // pre-setting launch command byte-for-byte. The membership guard keeps a
  // stale key (e.g. after an engine switch) from reaching _buildLaunchCommand.
  if (!options.launchMode
      && typeof projConfig.defaultLaunchMode === 'string'
      && projConfig.defaultLaunchMode !== 'default'
      && engineProfile.launchModes
      && engineProfile.launchModes[projConfig.defaultLaunchMode]) {
