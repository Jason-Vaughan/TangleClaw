...
function launchSession(projectName, options = {}) {
  ...
  let ruleShards = [];
  try {
    if (silentPrime && startupRules.rules && startupRules.rules.length > 0) {
...
function buildStartupRulesSection(projectId) {
  ...
  const manifestLines = ['## Project Rules', ''];
  manifestLines.push(
    `${usable.length} operator-authored rule${usable.length === 1 ? '' : 's'} `
    + `govern${usable.length === 1 ? 's' : ''} this session. They are delivered on their own `
    + 'startup channel and should already appear in your context as a "Project Rules" block. '
    + '**If no such block arrived, say so before acting** — it means delivery failed and you '
    + 'are working without rules the operator believes are in force.'
  );
  manifestLines.push('');
