...
const { summarizeFeatureIndexForPrime, countCuratedEntries } = require('./feature-index-prime');
const { createLogger } = require('./logger');

const log = createLogger('sessions');

/**
 * Size ceiling for a generated prime prompt, in tokens (converted to a rough
 * character budget at use). This was per-workflow config (2000 or 4000
 * depending on the project's template); code-owning it takes the LARGER of the
 * two deliberately, because the failure mode here is one-directional: the cap
 * truncates the prime's tail, and a tail cut silently drops directive sections
 * (#557 — a bypass-mode session once booted with its wait-for-confirmation
 * instruction cut off). A too-generous cap costs some hidden context; a too-tight
 * one costs a safety directive.
 */
...
function launchSession(projectName, options = {}) {
  ...
  if (!hasRules) {
    ...
    _recordRuleDelivery(primeFilePath
      ? { ...deliveryBase, channel: 'prime-file', outcome: 'delivered' }
      : { ...deliveryBase, channel: 'prime-file', outcome: 'skipped', skipReason: 'failed to write .tangleclaw/session-prime.md' });
 ...
 *
 * The digest is a sha256 over the rendered block, giving a rule *set* a stable
 * version identity that per-rule version numbers cannot express.
 *
...
function buildStartupRulesSection(projectId) {
  const empty = { lines: [], ruleIds: [], digest: '' };
  let rules;
  try {
  ...
  lines.push('Operator-authored rules for this project. They apply for the whole session:');
 ...
 */
function _resolvePrimeBudget(engineProfile) {
  const declared = engineProfile
    && engineProfile.capabilities
    && engineProfile.capabilities.startupInjection
    && engineProfile.capabilities.startupInjection.maxChars;
  if (Number.isFinite(declared) && declared > 0) return declared;
  return PRIME_MAX_TOKENS * 4;
}
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  sections.push(...startupRules.lines);
  ...
  if (projConfig.featureIndexEnabled === true
    ...
    try {
      ...
      const { curated, backlogEntries } = summarizeFeatureIndexForPrime(contents);
  ...
  // from the engine profile rather than a constant here.
  const maxChars = _resolvePrimeBudget(engineProfile);
  ...
  let prompt = _renderPrimeSections(sections, yielded);
  for (const section of yieldOrder) {
    if (prompt.length <= maxChars) break;
    yielded.add(section);
    prompt = _renderPrimeSections(sections, yielded);
  }
  if (yielded.size > 0) {
    log.info('Prime sections yielded to the size budget', {
      project: project.name, yielded: yielded.size, maxChars, length: prompt.length
    });
  }

  // Medusa consumer contract — appended LAST and budgeted to the space the
  // budget leaves (#557). Bulk reference material must yield to the directive
  // sections above (Resume wait-guard, wrap sentinel, scope guard), never
  // displace them: when the contract was embedded mid-prime, a blind tail
  // truncation silently cut every directive after it, and a bypass-mode
  ...
  // exactly the failure this replaced. The operator's next rule edit is now
  // costing them content, and the log line is how that becomes visible.
  if (Number.isFinite(maxChars) && prompt.length > maxChars) {
    log.warn('Prime exceeds the engine channel budget after all bulk sections yielded', {
      project: project.name, engine: engineProfile && engineProfile.id, maxChars, length: prompt.length
    });
    prompt += '\n\n_(This prime is ' + prompt.length + ' characters against a '
      + maxChars + '-character channel budget. Nothing was cut, but the consumer may '
      + 'truncate it; the complete text is on disk at `.tangleclaw/session-prime.md` '
      + 'in the project root.)_\n';
  }
