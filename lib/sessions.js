...
function launchSession(projectName, options = {}) {
  ...
  const channel = _prepareStartupChannel(engineId, project, launchProfile, launchCmd, det.path);
  ...
  _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText, effectiveMode, silentPrime,
    ...
    { sessionId: session.id, projectId: project.id, hasSequence: snapshot.applicability === 'applicable' });
 ...
 */
function _prepareStartupChannel(engineId, project, engineProfile, launchCmd, enginePath) {
  ...
  let prepared;
  try {
    prepared = resolved.adapter.prepareLaunch({ project, engineProfile, launchCmd, enginePath });
 ...
 *   to ask the session to read its context (#1635):
 *   `{sessionId, projectId, hasSequence}`. Null from callers that aren't
 *   launching a real session, which is why the branch tests it rather than
 *   testing `silentPrime` alone.
 */
