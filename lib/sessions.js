...
const launchKickoff = require('./launch-kickoff');
const launchBootstrap = require('./launch-bootstrap');
...
// The enum itself, not the context facade, which does not re-export it.
// `NOT_EVALUATED` is gate-bearing since it joined RECOVERY_VERDICTS, so it is
// imported rather than spelled.
const PREFLIGHT_MODULE = require('./launch-preflight');
const { VERDICTS: PREFLIGHT_VERDICTS } = PREFLIGHT_MODULE;
...
/**
 * The launch refusal for a project whose assignment is STOPPED (#1861), or
 * null. An unreadable control store refuses a project the gate knows is
 * governed, the same way every other governed action fails closed.
 * @param {object} project - Project record
 * @returns {object|null} A `launchSession` failure result
 */
function _stoppedLaunchRefusal(project) {
  let refusal = null;
  try {
    const row = store.control.getOpenForProject(project.id);
    if (row && row.state === controlState.STATES.STOPPED) {
      refusal = controlGate.checkMutation({ surface: 'launch', subject: { kind: 'target', projectId: project.id } });
    }
  } catch (err) { // prawduct:allow prawduct/broad-except -- the gate decides what an unreadable control store means for this project; the launch must not crash on it
    refusal = controlGate.checkMutation({ surface: 'launch', subject: { kind: 'target', projectId: project.id } });
  }
  if (!refusal) return null;
  return {
    session: null, primePrompt: null, ttydUrl: null,
    code: refusal.code, controlRefusal: refusal,
    error: `${refusal.code}: ${refusal.message}`
  };
}

/**
 * Move the project's open assignment to this launch (#1861), so the new
 * session is the one that may acknowledge it and the previous launch no
 * longer authorizes anything. A failure is logged, never fatal: the launch has
 * happened, and every gate still reads the assignment by project.
 * @param {object} project - Project record
 * @param {object} session - The new session row
 * @param {string} launchId - This launch's id
 * @returns {void}
 */
function _rebindControl(project, session, launchId) {
  try {
    controlState.rebind({ projectId: project.id, sessionId: session.id, launchId });
    // Keep the gate's governance memory in step with the rebind, synchronously.
    controlGate.remember(project.id, store.control.getOpenForProject(project.id));
  } catch (err) {
    log.warn('Could not rebind the control assignment to the new launch', { project: project.name, session: session.id, error: err.message });
  }
}

/**
 * Bring a project's managed control hooks in line with its governance (#1861):
 * a governed project's checkout is marked and its hooks installed; an
 * ungoverned one is unmarked (its hooks, if any, become a pass-through).
 * Best-effort and logged: the hooks are defense in depth, never the gate.
 * @param {object} project - Project record
 * @returns {object|null} The install result, or null when unmarked or failed
 */
function syncControlHooks(project) {
  try {
    const row = store.control.getOpenForProject(project.id);
    const api = _apiOrigin();
    if (row && api) {
      const result = controlHooks.install(project.path, { assignmentId: row.assignment_id, api });
      _isolateSiblingWorktreeProjects(project);
      return result;
    }
    if (row) log.warn('Control hooks not installed: no API origin to write into the marker', { project: project.name });
    else controlHooks.unmark(project.path);
  } catch (err) {
    log.warn('Could not sync managed control hooks', { project: project.name, code: err.code || null, error: err.message });
  }
  return null;
}

/**
 * Keep other registered projects that are linked worktrees of a governed
 * checkout's clone out of its lane (#1861). The governed main checkout marks
 * its common git dir so worktrees the Builder creates inherit the lane; a
 * registered project that lives in another worktree of that clone is a
 * different lane, and must say so explicitly whichever of the two launched
 * first. Ungoverned ones get an explicit ungoverned marker; governed ones keep
 * their own.
 * @param {object} project - The project that was just governed
 * @returns {void}
 */
function _isolateSiblingWorktreeProjects(project) {
  let commonDir;
  try {
    commonDir = controlHooks.locate(project.path).commonDir;
  } catch (err) { // prawduct:allow prawduct/broad-except -- a governed project that is not a readable git checkout has no clone siblings to isolate
    return;
  }
  for (const other of store.projects.list()) {
    if (other.id === project.id || !other.path) continue;
    try {
      const where = controlHooks.locate(other.path);
      if (where.commonDir !== commonDir || where.isMainWorktree) continue;
      if (store.control.getOpenForProject(other.id)) continue;
      controlHooks.unmark(other.path);
    } catch (err) { // prawduct:allow prawduct/broad-except -- a registered project that is not a readable git checkout shares no clone and needs no marker
      log.debug('Skipped isolating a project from a governed clone', { project: other.name, error: err.message });
    }
  }
}

/**
 * The launch state-step lines for a governed project (#1861); empty otherwise.
 * @param {object} project - Project record
 * @returns {string[]}
 */
function _controlPrimeLines(project) {
  let row;
  try {
    row = store.control.getOpenForProject(project.id);
  } catch (err) {
    return ['Control state: could not be read. Treat this lane as held and ask the operator before any mutating work.', ''];
  }
  if (!row) return [];
  const holds = store.control.listHolds(row.assignment_id, { activeOnly: true });
  const head = `Control state: assignment ${row.assignment_id}${row.issue_ref ? ` (${row.issue_ref})` : ''} is ${row.state.toUpperCase()} at generation ${row.state_generation}`;
  if (row.state === controlState.STATES.HELD) {
    return [
      `${head}, with ${holds.length} active hold${holds.length === 1 ? '' : 's'} (${holds.map((h) => `${h.hold_id} by ${h.issuer_principal}`).join(', ')}).`,
      'Do no mutating work. TangleClaw refuses governed mutations while held; shell git/gh is not blocked, so honour the hold there too. '
        + `Run \`tc control status\`, then acknowledge with \`tc control ack ${row.state_generation}\`.`,
      ''
    ];
  }
  return [`${head}. \`tc control status\` shows it at any time.`, ''];
}
 ...
 *   A project whose control assignment is STOPPED is refused with `CONTROL_STOPPED` and the
 *   gate's `controlRefusal` (#1861).
 */
function launchSession(projectName, options = {}) {
  ...
  // #1861 — STOP ends the assignment. An ordinary launch into a STOPPED project
  // is refused until the operator creates a successor assignment; the operator
  // can still read control status without starting the stopped Builder.
  const stopRefusal = _stoppedLaunchRefusal(project);
  if (stopRefusal) return stopRefusal;
  ...
  if (existing) {
    ...
    if (probe && !probe.live) {
      ...
      _releaseStartupChannel(existing, 'tmux session died before relaunch');
  ...
  // `handoffResume` (#1675) is the Resume drawn from the publication the verdict
  // read — threaded to both the pushed prime and the pull steps so neither falls
  // back to re-reading the mutable continuity index.
  const primeOptions = { medusaWorkspaceId, startupRules, continuityMode: options.continuityMode, operatorHost: options.operatorHost, healReport: heal.report, handoffResume: (preflightResult && preflightResult.handoffResume) || null };
  // A silent prime is printed by the SessionStart hook together with the UI
  // wrap advisory and, after `/clear` or a compaction, the re-entry preamble,
  // and the prime file gains its provenance line after the prime is final. The
  // engine caps the hook's WHOLE output and injects only a preview past it, so
  // the prime is fitted to the cap minus all of that, in its worst case (#1888).
  const primeProvenance = { config: projConfig, project: project.name, engine: engineId };
  // Said once, here, where the launch resolves the setting for every write it
  // makes: a hand-edited template the API would refuse renders the default
  // rather than blocking the prime, and the operator reads why.
  const provenanceWarning = provenance.resolveProvenance(projConfig).warning;
  if (provenanceWarning) log.warn('Provenance template ignored', { project: project.name, warning: provenanceWarning });
  const provenanceReserve = silentPrime ? primeHookOutput.primeReserve(project, primeProvenance) : 0;
  if (options.primePrompt !== false) {
    primeText = generatePrimePrompt(project, engineProfile, { ...primeOptions, launchSequence: sequenceApplicability.applicable, reserveChars: provenanceReserve });
  }
  ...
  if (silentPrime && primeText) {
    primeFilePath = _writePrimeFile(project.path, primeText, primeProvenance);
    // Beside the prime and under the same gate: the hook re-fires on `/clear`
    // and compaction (#1761), and this is what it puts ahead of the prime then.
    _writeReentryFile(project.path, project, primeProvenance);
  } else {
  ...
  const snapshot = _buildLaunchSnapshot(launchId, project, engineProfile, sequenceApplicability, startupRules, primeOptions, preflightResult, projConfig);
  ...
  if (primeText && sequenceApplicability.applicable && snapshot.applicability !== 'applicable') {
    ...
    primeText = generatePrimePrompt(project, engineProfile, { ...primeOptions, launchSequence: false, reserveChars: provenanceReserve });
    if (silentPrime && primeText) primeFilePath = _writePrimeFile(project.path, primeText, primeProvenance);
  }
  ...
  // The engine's native startup channel (#1825 B2). When the engine's profile
  // names a registered adapter on a verified version, the adapter starts the
  // per-launch server BEFORE the pane exists and rewrites the pane's command to
  // attach to it. A channel that cannot be started never fails the launch: the
  // pane runs today's command, and the reason is recorded on the launch entry
  // so the fire route's later `unsupported` has a cause.
  // The pane's environment is built first and handed to the adapter too: with
  // `--remote` the pane is a client, and the agent loop with its shell tools
  // runs in the app-server, so a `tc start next` the fired startup prompt asks
  // for would otherwise run with no `tc` on PATH and no launch identity. The
  // server inherits exactly what the pane would have (B3).
  const paneEnv = _paneEnvironment(project, launchProfile, launchId, medusaWorkspaceId);
  const channel = _prepareStartupChannel(engineId, project, launchProfile, launchCmd, det.path, paneEnv);
  const paneCommand = channel.command || _legacyStartupCommand(channel, launchCmd);
  // Which path puts this launch's context in front of the engine (#1825 B3):
  // native when the channel started AND there is a sequence to read through it,
  // else the keystroke path it always was. Frozen on the sequence row, in the
  // transaction that binds it, so every later reader — the deferred init, the
  // unready monitor, a restart — sees one answer and none can turn a native
  // launch back into a paste.
  snapshot.startupDelivery = channel.handle && snapshot.applicability === 'applicable' ? 'native' : 'legacy';
  ...
  try {
    const created = tmux.createSession(tmuxName, {
      cwd: project.path,
      command: _withPathFloor(paneCommand),
      env: paneEnv
    });

    if (!created) {
      _abandonStartupChannel(channel, 'tmux session not created');
      return { session: null, primePrompt: null, ttydUrl: null, error: `Failed to create tmux session "${tmuxName}"` };
    }
  } catch (err) {
    _abandonStartupChannel(channel, `tmux error: ${err.message}`);
  ...
  try {
    ...
    _abandonStartupChannel(channel, 'session row not created');
    return _abandonUnboundLaunch(project, tmuxName, err);
  }
  _attachStartupChannel(channel, session, engineId);
  _rebindControl(project, session, launchId);
  syncControlHooks(project);
  ...
  log.info('Session launched', {
    ...
    // Whether the pane was given a native startup channel, and if not, why:
    // an unsupported launch keeps today's path, and the fire route's later
    // refusal points back at the recorded reason.
    startupControl: channel.command ? 'channel' : `none (${channel.reasonCode}: ${channel.reason})`,
  ...
  // #1635 — the kickoff context. A silently primed session is handed everything
  // it needs as hidden context and never asked anything, so it sits at an empty
  // prompt; `hasSequence` is what it would be asked to read.
  _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText, effectiveMode, silentPrime,
    ...
    { sessionId: session.id, projectId: project.id, hasSequence: snapshot.applicability === 'applicable', startupDelivery: snapshot.startupDelivery });
...
/**
 * What changed in the governing text between the previous session's handoff and
 * this launch (car 21.10, §2.4 step 3).
 *
 * Never throws, and never answers a failure with the same value as "there was
 * nothing to compare". A first launch legitimately has no handoff; a launch
 * whose handoff was corrupt, or whose own computation threw, DID try and
 * failed, and an agent handed an identical blank in both cases cannot tell a
 * clean slate from a measurement that was attempted and lost.
 * @param {object} project - Project record
 * @param {object} [preflightResult] - From `launchPreflight.evaluate`; carries the handoff manifest
 * @returns {object|null} The drift, `{unavailable: reason}` when it could not be
 *   computed, or null when there was genuinely nothing to compare against
 */
function _launchRuleDrift(project, preflightResult) {
  if (!preflightResult) return null;
  const before = preflightResult.handoffManifest;
  if (!before) {
    // The preflight says WHY it has no manifest. A document that could not be
    // read is a failed measurement and says so; a project with no handoff at
    // all has nothing to say and renders no section.
    const reason = preflightResult.handoffManifestUnavailable;
    return reason ? { unavailable: reason } : null;
  }
  try {
    const after = launchSequence.manifestFingerprints(project);
    return ruleDriftOf.diffRuleManifests(before, after);
  // prawduct:allow prawduct/broad-except -- drift is advisory context; a launch must not fail on it
  } catch (err) {
    log.warn('Rule drift could not be computed for this launch', {
      project: project && project.name, error: err.message
    });
    return { unavailable: `this launch could not compare them (${err.message})` };
  }
}
 ...
 * @param {object|null} [projConfig] - The project's config, for the recovery mode this launch freezes
 * @returns {object} The `launchSequence` argument of `store.sessions.start`
 */
function _buildLaunchSnapshot(launchId, project, engineProfile, applicability, startupRules, primeOptions, preflightResult, projConfig) {
  const rules = startupRules.rules || [];
  // Resolved once, here, and frozen into the record below: a launch plays by the
  // mode it was told at step 1, and a setting edited mid-launch must not decide
  // the gate a pane is already standing at.
  const recovery = projectConfig.resolveRecoveryMode(projConfig);
  if (recovery.warning) {
    log.warn('Project config falls back for the recovery mode', { project: project.name, warning: recovery.warning });
  }
  // Only what a later reader of the sequence cannot recompute is stored: the
  // verdict, its reason, both predicates, and the one piece of evidence a
  // predicate depends on (`worktreeDirty`). The rest of `evidence` and the
  // repair outcomes are launch-time diagnosis and reach the log instead —
  ...
  // Computed once, here, and frozen into the snapshot below. The step-3 text and
  // the READY gate must state the SAME drift: recomputing at attest time would
  // let a rule edited between render and READY produce a requirement the agent
  // was never shown, which is the opposite of what a reconciliation is for.
  const ruleDrift = _launchRuleDrift(project, preflightResult);
  try {
    const rendered = applicability.applicable
      ? renderLaunchSteps(project, engineProfile, {
        ...primeOptions, preflight, ruleDrift,
        handoffMethodology: (preflightResult && preflightResult.handoffMethodology) || null
      })
      : null;
    ...
    const renderContext = {
      ...
      // #1675 — the publication the Resume was drawn from, so a revision
      // re-renders the same one and the manifest can name what was consumed.
      handoffResume: primeOptions.handoffResume || null,
      // #1738 — carried so a revision re-renders the same reattach directive.
      handoffMethodology: (preflightResult && preflightResult.handoffMethodology) || null
    };
    return launchSequence.buildSnapshot({ launchId, project, engineProfile, applicability, rendered, rules, preflight, recoveryMode: recovery.mode, renderContext, ruleDrift });
  // prawduct:allow prawduct/broad-except -- a launch must survive a failure to render its launch steps
    ...
    return {
      ...
      preflight,
      // `none`, whatever the verdict said. This launch has no steps, so there is
      // nothing to withhold and it can never attest — `_serve` and `ready` both
      // refuse it with SEQUENCE_NOT_APPLICABLE first. Recording `required` would
      // be a demand no gate enforces and no panel renders, and the clear route
      // would then write a clearance for a launch that gated nothing. The
      // verdict above is where the handoff's soundness is recorded.
      recovery: 'none',
 ...
 *
 * Both predicates are stored, and `requiresRecovery` is the one that MUST be.
 * It is not a function of the verdict: `needsRecovery` reads
 * `evidence.worktreeDirty` for `workspace-unavailable`, because a vanished
 * worktree that was measured clean has nothing to recover while one whose
 * dirtiness was never measured does. A later reader given only the verdict
 * would recompute it as `false` for the unmeasured case — the unsafe direction,
 * and precisely the flattening this car had to fix one layer down.
 * `requiresReconciliation` is stored for the adjacent reason: the demand must
 * describe the verdict this launch's step 3 actually stated, not one a rebuilt
 * result would classify differently.
 *
 * `worktreeDirty` rides along so the recovery answer stays auditable — a stored
 * boolean nobody can check against its input is a claim, not a record.
 *
 * @param {object} [result] - From `launchPreflight.evaluate`
 * @returns {{verdict: string, reason: string, requiresRecovery: boolean, requiresReconciliation: boolean, worktreeDirty: (boolean|null), evaluationFailed: boolean, evaluationMissing: boolean}}
 */
function _storedPreflight(result) {
  // Absent or malformed is MISSING EVIDENCE, and that constant owes recovery.
  // An applicable launch owes the check, so nothing arriving here cannot read
  // as nothing being wrong.
  //
  // This is the site production actually reaches — every launch routes its
  // evaluation through here before a snapshot is built — so the log belongs
  // HERE, not only at the snapshot boundary. A recovery demand whose cause is
  // recorded nowhere sends its reader looking for an entry that does not exist.
  if (!PREFLIGHT_MODULE.isUsableResult(result)) {
    log.warn('No usable preflight result to store for this launch — recording missing evidence, which owes recovery (this does not establish that none was produced)', {
      received: result === undefined ? 'nothing' : (result === null ? 'null' : typeof result)
    });
    return launchSequence.PREFLIGHT_NOT_EVALUATED;
  }
  ...
  return {
    ...
    // Provenance survives storage, and therefore survives a clearance. A
    // clearance is permission to proceed under the configured policy — it does
    // not turn a failed or absent evaluation into a successful one, so the row
    // must still say which of the two it was after READY.
    evaluationFailed: result.evaluationFailed === true,
    evaluationMissing: result.evaluationMissing === true
  };
}

/**
 * The environment a launched pane runs in, and that a native startup channel's
 * server inherits (#1825 B3).
 *
 ...
 *
 * `TANGLECLAW_API` is omitted, never faked: an unresolvable origin must not
 * become a sentence-shaped URL in the pane — `tc` reports a missing
 * TANGLECLAW_API loudly, which is the honest failure. The session id does not
 * exist yet (the row is written after tmux starts), and a running process
 * never sees a later env change, so the launch id is what `tc start` presents;
 * the server resolves it once the row is bound.
 *
 * @param {object} project - The project.
 * @param {object} launchProfile - The profile the launch command was built from.
 * @param {string} launchId - This launch's id.
 * @param {string|null} medusaWorkspaceId - The pre-minted switchboard id, when the project opted in.
 * @returns {Object<string, string>} The environment entries, profile env layered last.
 */
function _paneEnvironment(project, launchProfile, launchId, medusaWorkspaceId) {
  const ambientEnv = {
  ...
  return { ...ambientEnv, ...(launchProfile.launch ? launchProfile.launch.env : {}) };
}

/**
 * Start the engine's native startup channel for a launch, when its profile
 * names a registered adapter on a verified version (#1825 B2). Returns the
 * pane command to run, or the reason no channel was started. Never throws.
 * @param {string} engineId - Engine id.
 * @param {object} project - The project.
 * @param {object} engineProfile - The launch profile.
 * @param {string} launchCmd - The command `_buildLaunchCommand` produced.
 * @param {string|null} enginePath - The resolved engine executable this launch runs; the adapter probes exactly that binary.
 * @param {Object<string, string>} [env] - The pane's environment entries, which the channel's server inherits too (B3).
 * @returns {{command: (string|null), handle: (object|null), adapter: (object|null), reasonCode: (string|null), reason: (string|null)}}
 */
function _prepareStartupChannel(engineId, project, engineProfile, launchCmd, enginePath, env = {}) {
  const none = (reasonCode, reason, adapterName = null) => ({ command: null, handle: null, adapter: null, adapterName, reasonCode, reason });
  if (typeof launchCmd !== 'string' || launchCmd === '') return none('engine_declares_none', 'no launch command');
  let resolved;
  try {
    resolved = startupControl.resolveForLaunch(engineId, engines.resolveProfile, undefined, { enginePath });
  // prawduct:allow prawduct/broad-except -- a capability read must never fail a launch; the pane launches on today's path
  } catch (err) {
    return none('engine_profile_unreadable', err.message);
  }
  if (!resolved.supported) return none(resolved.reasonCode, resolved.reason, resolved.block ? resolved.block.adapter : null);
  let prepared;
  try {
    prepared = resolved.adapter.prepareLaunch({ project, engineProfile, launchCmd, enginePath, env });
  // prawduct:allow prawduct/broad-except -- an adapter fault is recorded as no channel, never thrown over the launch
  } catch (err) {
    prepared = { ok: false, reasonCode: 'channel_unavailable', reason: err.message };
  }
  if (!prepared || !prepared.ok) {
    log.warn('startupControl channel not started; launching on the legacy path', {
      project: project.name, engine: engineId, reason: prepared ? prepared.reason : 'no answer from the adapter'
    });
    return none(prepared && prepared.reasonCode ? prepared.reasonCode : 'channel_unavailable', prepared ? prepared.reason : 'no answer from the adapter', resolved.block.adapter);
  }
  return { command: prepared.command, handle: prepared.handle, adapter: resolved.adapter, adapterName: resolved.block.adapter, reasonCode: null, reason: null };
}

/**
 * Let an engine adapter harden its legacy launch after the native channel was
 * unavailable. The adapter already probed the exact binary in
 * `_prepareStartupChannel`, so it can make a version-aware choice without a
 * second subprocess or a guessed capability. A missing or broken optional
 * hook preserves the historical command.
 * @param {object} channel - Result from `_prepareStartupChannel`.
 * @param {string} launchCmd - Historical engine launch command.
 * @returns {string} The adapter-hardened fallback, or `launchCmd` unchanged.
 */
function _legacyStartupCommand(channel, launchCmd) {
  const name = channel && channel.adapterName;
  const adapter = name && Object.prototype.hasOwnProperty.call(startupControl.ADAPTERS, name)
    ? startupControl.ADAPTERS[name]
    : null;
  if (!adapter || typeof adapter.legacyLaunchCommand !== 'function') return launchCmd;
  try {
    const command = adapter.legacyLaunchCommand(launchCmd);
    return typeof command === 'string' && command ? command : launchCmd;
  } catch (err) {
    log.warn('startupControl adapter could not harden the legacy launch command', { adapter: name, error: err.message });
    return launchCmd;
  }
}

/**
 * Harden a launch that never attempts a native startup channel — the Project
 * Master's — exactly as a project launch's legacy fallback is hardened: the
 * engine's declared adapter probes the exact executable this launch runs, then
 * `_legacyStartupCommand` applies its version-bounded choice. An engine that
 * declares no adapter, or a resolution that fails, keeps the historical command.
 * @param {string} engineId - Engine id whose profile names the adapter.
 * @param {string} launchCmd - Historical engine launch command.
 * @param {string|null} enginePath - The resolved executable this launch runs.
 * @returns {string} The adapter-hardened command, or `launchCmd` unchanged.
 */
function _hardenedLegacyLaunchCommand(engineId, launchCmd, enginePath) {
  if (typeof launchCmd !== 'string' || launchCmd === '') return launchCmd;
  let resolved;
  try {
    resolved = startupControl.resolveForLaunch(engineId, engines.resolveProfile, undefined, { enginePath });
  // prawduct:allow prawduct/broad-except -- a capability read must never fail a launch; the pane launches on today's command
  } catch (err) {
    log.warn('startupControl could not resolve an adapter to harden the launch command', { engine: engineId, error: err.message });
    return launchCmd;
  }
  return _legacyStartupCommand({ adapterName: resolved && resolved.block ? resolved.block.adapter : null }, launchCmd);
}

/**
 * Record a started channel against the session that now owns it.
 * @param {object} channel - From `_prepareStartupChannel`.
 * @param {object} session - The session row.
 * @param {string} engineId - Engine id.
 * @returns {void}
 */
function _attachStartupChannel(channel, session, engineId) {
  try {
    const sequence = store.launchSequences.getBySession(session.id);
    if (!channel.handle) {
      // Only a launch that COULD have had a channel records why it did not:
      // an engine that declares none has nothing to explain.
      if (channel.reasonCode && channel.reasonCode !== 'engine_declares_none') {
        store.startupControlChannels.recordUnavailable({
          sessionId: session.id, sequenceId: sequence ? sequence.id : 0, engineId,
          adapter: channel.adapterName || 'none', reason: `${channel.reasonCode}: ${channel.reason}`
        });
      }
      return;
    }
    const row = channel.adapter.attachLaunch(channel.handle, { sessionId: session.id, sequenceId: sequence ? sequence.id : 0, engineId });
    if (!row) {
      // The adapter could not record the channel and has stopped the server it
      // started. The launch already selected the native path (frozen with the
      // sequence), so nothing will be typed into this pane and the bootstrap's
      // fire will find no channel; this closed row is what lets the panel name
      // the cause instead of showing a native launch with no channel at all.
      store.startupControlChannels.recordUnavailable({
        sessionId: session.id, sequenceId: sequence ? sequence.id : 0, engineId,
        adapter: channel.adapterName || (channel.adapter && channel.adapter.NAME) || 'unknown',
        reason: 'channel_unavailable: the channel row could not be recorded, so the server it named was stopped'
      });
    }
  // prawduct:allow prawduct/broad-except -- a bookkeeping fault must not fail a launch that is already running
  } catch (err) {
    log.warn('Could not record the startupControl channel', { session: session.id, error: err.message });
  }
}

/**
 * Stop a channel whose launch did not complete.
 * @param {object} channel - From `_prepareStartupChannel`.
 * @param {string} reason - Why.
 * @returns {void}
 */
function _abandonStartupChannel(channel, reason) {
  if (!channel || !channel.handle) return;
  try {
    channel.adapter.abandonLaunch(channel.handle, reason);
  // prawduct:allow prawduct/broad-except -- the launch is already failing; the channel is best-effort cleanup
  } catch (err) {
    log.warn('Could not stop an abandoned startupControl channel', { error: err.message });
  }
}

/**
 * End the native startup channel of a session that has ended. The channel row
 * names the adapter that owns it, and only that adapter is asked.
 * @param {object} session - The ended session row.
 * @param {string} reason - Why.
 * @returns {void}
 */
function _releaseStartupChannel(session, reason) {
  if (!session || !Number.isInteger(session.id)) return;
  try {
    const channel = store.startupControlChannels.getOpenBySession(session.id);
    if (!channel) return;
    const adapter = Object.prototype.hasOwnProperty.call(startupControl.ADAPTERS, channel.adapter) ? startupControl.ADAPTERS[channel.adapter] : null;
    if (!adapter || typeof adapter.releaseSession !== 'function') {
      log.warn('A channel names an adapter this TangleClaw cannot release', { session: session.id, adapter: channel.adapter });
      store.startupControlChannels.close(channel.id, reason, 'skipped: adapter not registered');
      return;
    }
    adapter.releaseSession(session.id, reason);
  // prawduct:allow prawduct/broad-except -- a session's end must complete even when its channel cannot be stopped
  } catch (err) {
    log.warn('Could not release a startupControl channel', { session: session.id, error: err.message });
  }
}
...
function buildStartupRulesSection(projectId) {
  ...
  // One definition, shared with the manifest the drift diff compares against.
  const usable = rules.filter(ruleDriftOf.isUsableRule);
...
/**
 * The launch note for a governed carrier that still holds legacy TangleClaw
 * sections its managed block duplicates (#1911), or null. Read-only: it names
 * the preview command and removes nothing, because a matching heading does not
 * prove the operator left the body alone.
 * @param {string} projectPath - Absolute project root.
 * @param {string|null} filename - The carrier's filename.
 * @param {object} engineProfile - Engine profile (for the marker syntax).
 * @returns {string|null}
 */
function _legacyDuplicateNote(projectPath, filename, engineProfile) {
  if (!filename) return null;
  let text;
  try {
    text = fs.readFileSync(path.join(projectPath, filename), 'utf8');
  } catch { // prawduct:allow prawduct/broad-except -- a missing or unreadable carrier has no note to give
    return null;
  }
  const syntax = engineProfile && engineProfile.configFormat && engineProfile.configFormat.syntax;
  const analysis = engines.analyzeGovernedCarrier(text, syntax || 'markdown');
  if (analysis.refused) {
    return `\`${filename}\` holds legacy TangleClaw content above the Prawduct anchor that cannot be bounded safely (${analysis.refused}). Nothing was removed; tell the operator, who must tidy it by hand (#1911).`;
  }
  if (!analysis.eligible || analysis.candidates.length === 0) return null;
  const n = analysis.candidates.length;
  return `\`${filename}\` still holds ${n} legacy TangleClaw item${n === 1 ? '' : 's'} (sections or bootstrap bullets) above the Prawduct anchor that the managed block duplicates. Nothing was removed; tell the operator, who can preview a repair with \`${legacyClaudeMd.repairCommand(projectPath)}\` (#1911).`;
}
...
function _ruleSourcesSection(project, engineProfile, startupRules, delivery) {
  ...
  if (governance === 'governed-plugin') {
    ...
    const legacyNote = _legacyDuplicateNote(project.path, declared, engineProfile);
...
/**
 * What tells an agent the launch sequence exists, and the order its own launch
 * runs in. It is the push prime's only addition for a launch that has a
 * sequence, and the engine-neutral form matters: `tc` is on PATH in every pane
 * TangleClaw launches, whatever the engine.
 *
 * The ordering half is here, in the FIRST step, because two other directives
 * each claim the agent's first turn: the banner block just above claims the
 * first visible reply, and the Resume section claims the first visible
 * *message*. A pull serves Resume FOURTH, so its claim arrives after the agent
 * has necessarily already spoken — unsatisfiable on arrival. This block sits
 * after the banner and before everything else for the same reason it exists:
 * the order has to be stated where it is read before either claim is acted
 * on. Read next to a wait-for-
 * confirmation rule, the pair resolves as "do not initialize without asking",
 * and a launch stopped to ask the operator for permission to run `tc start
 * next` — a question the operator has no way to answer, because initialization
 * is not a choice they were offered.
 *
 * Stating the order once, before any of those directives arrive, is what makes
 * them legible: each becomes a stage of a stated sequence instead of a rival
 * claim on the same turn. The authorization split is the load-bearing half.
 * Initialization is routine and pre-authorized; the project work proposed at
 * the end of it is not, and every confirmation gate still applies there.
 * @type {string[]}
 */
const LAUNCH_BOOTSTRAP_LINES = [
  ...
  '',
  // Lettered, not numbered: "step" already means a launch-sequence step in
  // every other line an agent reads here, so a second 1-4 beside the first
  // gives "step 4" two referents — and one of the readings puts the
  // confirmation gate in front of the task STEP, which is the misreading this
  // block exists to end.
  'Your opening runs in this order:',
  '(a) Emit the banner line this prime requires, and say in one line that you are reading your '
    + 'launch context. Keep it short: whatever you have been given, the freshness checks that '
    + 'decide what to propose have not run yet, so there is nothing to report here beyond having '
    + 'started.',
  '(b) Run the sequence to the end: `tc start next` for each step, acknowledging each one with the '
    + 'command that step prints.',
  '(c) Do the read-only freshness checks the task step asks for, then attest with `tc start ready`.',
  '(d) Propose the project action, and stop there.',
  '(a) through (c) are routine initialization and are already authorized. Do not ask the operator '
    + 'to approve them, and do not read a wait-for-confirmation rule anywhere in this context as '
    + 'gating them — those rules gate (d), the work you propose. They never gate reading a launch '
    + 'step, including the task step. Reading and attesting record only that the context arrived: '
    + 'they never authorize executing the recorded next action, editing another project, or '
    + 'dispatching work nobody asked for, and an explicit preflight, recovery or '
    + 'privileged-action approval requirement still applies in full.',
  ''
];
...
function _collectPrimeSections(project, engineProfile, options = {}) {
  ...
  add('identity',
    ...
    + 'honor any wait-for-confirmation directive below before acting on the '
    + 'task. Reading your launch context is not acting.'
  );
  add('identity', '');
  ...
  // Directly under the verdict, because the two answer one question between
  // them: the verdict says whether the previous session's handoff is sound,
  // and this says whether the rules it was written under still hold.
  if (pull) add('state', ..._ruleDriftLines(options.ruleDrift));

  // #1738 — the return path. A previous session whose engine could not run the
  // project's methodology left its Prawduct state dormant; a session that can
  // run it is told to revalidate before relying on it.
  if (pull) add('state', ..._methodologyReattachLines(options.handoffMethodology, project, engineProfile));
  ...
  // The checkout, against the upstream every related session shares (#1678):
  // branch, HEAD, how it stands against origin/main as observed once for the
  // repository, and uncommitted/untracked counts. Always one line, so an
  // unmeasured checkout is said rather than omitted. Cached reads only — the
  // launch route warms them first. Describes; authorizes nothing.
  add('state', ...checkoutFreshness.primeLines(checkoutFreshness.projectCheckout(project, { config: store.config.load() })));
  ...
  // #1861 — durable HOLD/STOP. Said before any work, so a successor launched
  // into a held lane knows it is held without waiting for a notice. Absent for
  // an ungoverned project, whose prime is unchanged.
  add('state', ..._controlPrimeLines(project));
  ...
  // When the previous wrap left a resume (#1675: the publication the launch
  // verdict read, or failing that a labelled fallback — `_resumeSource`),
  ...
  // "visible out" — emit the resume prompt after a freshness check, and wait
  // for the operator's go (no auto-execute).
  ...
  //
  // WHERE that proposal lands is deliberately relative, not absolute: it
  // closes initialization rather than opening the session. On a pull this
  // section IS the fourth step, so an instruction to make it the first visible
  // message could only ever be read too late to obey — and an agent that takes
  // it literally concludes it should not have spoken yet, which is how one
  // came to ask permission to initialize at all. The bootstrap block states
  // the whole order up front; this section names its own place in it.
  const source = _resumeSource(project, options);
  if (source && options.continuityMode !== 'fresh') {
    add('task', '## Resume — the proposal that closes your initialization');
    add('task',
      'This prime is hidden context; the operator does not see it, so what the last session '
      + 'recorded reaches them only if you say it. Emit a short, visible resume proposal — after '
      + "the launch sequence where this launch has one, otherwise as your first visible reply. You "
      + 'MUST NOT start the work it proposes until the operator confirms.'
    );
    add('task', '');
    // #1675 — where the text below came from, BEFORE the text: the one fact
    // that decides how far to trust it, and the one the proposal must repeat
    // when it is anything but the current publication.
    add('task', ..._resumeProvenanceLines(source));
    add('task', 'Last session recorded:');
    if (source.currentState) add('task', `- Where we are: ${source.currentState}`);
    if (source.nextAction) add('task', `- Next action: ${source.nextAction}`);
    const f = source.freshness || {};
    ...
    add('task',
      ...
      + 'written-at sha above. Anything merged or released after it was staged is not in it.'
    );
    add('task',
    ...
    add('task',
      '3. Lead the proposal with "We left off at <X>. Next: <Y>." (incorporating your diff). '
      + 'Name the source in that sentence when it is not a current publication. '
      + 'Where this is also your first visible reply, the required '
      + "`*TangleClaw'd into existence.*` banner comes first; where you have already emitted it, "
      + 'do not repeat it. '
    ...
    if (pull) add('task', ..._noResumeLines(source, options.continuityMode));
    ...
    if (lastSession && lastSession.wrapSummary) {
      ...
      // least load-bearing block in the prime. Labelled with its session
      // (#1675): it is not a handoff, and read unlabelled it passed for one.
      const label = `_A summary recorded on session ${lastSession.id} (${lastSession.status}), not a handoff: no publication vouches for it._`;
      add('state', _yieldable(1,
        ['## Last Session Summary', label, lastSession.wrapSummary, ''],
 ...
 * @param {number} [options.reserveChars] - Characters the caller will add to
 *   the delivered file after this returns, fitted against on the startup-hook
 *   channel only.
 * @returns {string}
 */
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  // Minus whatever the caller will add to the file after this returns (the
  // provenance line), on the hook channel only: that is the channel whose cap
  // the file is read against. Floored at 0 so a pathological reserve yields
  // everything rather than producing a negative budget.
  const reserve = viaStartupHook && Number.isFinite(options.reserveChars) && options.reserveChars > 0
    ? options.reserveChars : 0;
  const maxChars = Math.max(0, _resolvePrimeBudget(engineProfile, { viaStartupHook }) - reserve);
 ...
 * @param {object} engineProfile - Engine profile
 * @param {object} [options] - As for `generatePrimePrompt`, plus:
 *   `preflight` (the verdict step 3 states) and `ruleDrift` (what changed in the
 *   governing text since the handoff). **Both fail silent when omitted**: the step
 *   still renders, simply without that section, so a caller that forgets `ruleDrift`
 *   produces a step 3 indistinguishable from a launch with no drift to report —
 *   while the READY gate, reading the frozen value, still demands a reconciliation
 *   for drift whose explanation is missing from the text the agent was handed.
 *   A revision re-render must pass it through for exactly that reason.
...
function _preflightLines(preflight) {
  const verdict = preflight && typeof preflight.verdict === 'string'
    ? preflight.verdict
    : PREFLIGHT_VERDICTS.NOT_EVALUATED;
  const reason = preflight && preflight.reason ? preflight.reason : 'no reason was recorded';
  const meaning = verdict === PREFLIGHT_VERDICTS.NOT_EVALUATED
...
/**
 * The reattach directive for a launch that follows a dormant-methodology
 * session (#1738).
 *
 * Renders only when the previous handoff recorded `capability-unavailable` AND
 * this launch's engine can run the methodology for this project. Anything else
 * says nothing: a launch on another engine that cannot run it has nothing to
 * revalidate with, and a previous session that measured has nothing to repair.
 *
 * Instructional by design. TangleClaw cannot observe `/prawduct:doctor`
 * passing, so the section says the authority is withheld by the agent until it
 * does, rather than claiming a gate it does not hold.
 *
 * @param {{disposition: string, engineId: (string|null)}|null} record - From the handoff
 * @param {object} project - Project record
 * @param {object} engineProfile - This launch's engine profile
 * @returns {string[]}
 */
function _methodologyReattachLines(record, project, engineProfile) {
  if (!record || record.disposition !== 'capability-unavailable') return [];
  const now = governance.methodologyCapability(project.path, { engineId: engineProfile && engineProfile.id });
  if (now.disposition !== 'available') return [];
  const was = record.engineId || 'an unknown';
  return [
    '## Prawduct state was dormant',
    `The previous session ran on the ${was} engine, which cannot run the Prawduct plugin. Its wrap left this project's Prawduct onboarding, plans, history and evidence untouched, ran no Prawduct gates, and withheld merge and release authority.`,
    'Before any methodology work — building against a plan, the Critic, a PR, a merge or a release — run `/prawduct:doctor` to revalidate this already-onboarded project. Never run `/prawduct:onboard`: it is for a new project and would treat preserved state as absent. Apply only the repairs Doctor previews and the owner confirms. If Doctor cannot load because the plugin itself is not installed, restore it through Prawduct\'s supported installation path, keeping the existing project state, and then run Doctor.',
    'Treat methodology authority as withheld until Doctor passes. TangleClaw cannot see Doctor\'s result, so this is yours to hold, not a gate it enforces, and nothing here means Doctor has passed or authority is restored. The previous handoff keeps its record as written; a later wrap whose preflight measures on this engine records a new result.',
    ''
  ];
}

/**
 * What changed in the governing text since the previous session's handoff
 * (car 21.10, §2.4 step 3).
 *
 * Always renders when a comparison was possible, including when nothing moved:
 * "no drift" is a finding the agent should be able to rely on, and a section
 * that appears only on bad news is one whose absence means nothing. Same
 * reasoning as the stranded-wraps section one block down.
 * @param {object|null} drift - From `launch-rule-drift.diffRuleManifests`
 * @returns {string[]} The section, or an empty array when there was nothing to compare
 */
function _ruleDriftLines(drift) {
  if (!drift) return [];
  const lines = ['## Rule drift since the last handoff'];

  // A measurement that was attempted and lost is not a clean slate, and the
  // two used to render identically — as no section at all.
  if (drift.unavailable) {
    lines.push(`Not compared — ${drift.unavailable}. Treat the rules in force now as possibly different from the ones the previous session worked under.`);
    return [...lines, ''];
  }

  if (!drift.recorded) {
    lines.push('The previous session\'s handoff recorded no governing sources, so nothing here can be compared.');
    return [...lines, ''];
  }

  const label = (row) => `- **${row.label}** (${row.source})`;
  if (drift.hasDrift) {
    lines.push('These governing sources changed since the handoff was written. Read them before acting on the previous session\'s next action:');
    lines.push('');
    for (const row of drift.added) lines.push(`${label(row)} — **added**`);
    for (const row of drift.removed) lines.push(`${label(row)} — **removed**`);
    for (const row of drift.changed) {
      const versions = row.fromRevision != null && row.toRevision != null
        ? ` (revision ${row.fromRevision} → ${row.toRevision})`
        : '';
      lines.push(`${label(row)} — **changed**${versions}`);
    }
    lines.push('');
  } else {
    // Named, never blanket, and named from the VERDICT rather than from
    // `comparedSources`. The old wording spoke for all three sources whenever
    // `hasDrift` was false, and the next line retracted it; narrowing it to
    // the compared set was still too wide, because a source that was compared
    // but only partly readable cannot carry "nothing changed" either. Only a
    // source whose verdict is literally `unchanged` is spoken for here.
    const settled = Object.entries(drift.perSource)
      .filter(([, s]) => s === ruleDriftOf.SOURCE_STATES.UNCHANGED).map(([source]) => source);
    if (settled.length > 0) {
      lines.push(`Nothing changed in ${settled.join(', ')} since the handoff was written.`);
    }
  }

  // Said per source and per DIRECTION. A source the handoff never recorded and
  // one this launch could not read are both uncomparable, for opposite
  // reasons: one sends the operator to the previous session, the other to this
  // machine. Folding either into the sentence above states a measurement
  // nobody took.
  const bySource = (state) => Object.entries(drift.perSource)
    .filter(([, s]) => s === state).map(([source]) => source);

  const unrecorded = bySource(ruleDriftOf.SOURCE_STATES.NOT_RECORDED);
  if (unrecorded.length > 0) {
    lines.push(`Not compared — the handoff recorded nothing about ${unrecorded.join(', ')}, so this launch cannot say whether they changed.`);
  }

  // Two lines, because they send the reader to two different machines. Saying
  // "the server log names what failed" about a file that failed at the
  // PREVIOUS wrap points at a log that says nothing.
  const unmeasured = drift.unmeasured || { before: [], after: [] };
  const atWrap = [...new Set([...bySource(ruleDriftOf.SOURCE_STATES.UNREADABLE_AT_WRAP), ...unmeasured.before])];
  const now = [...new Set([...bySource(ruleDriftOf.SOURCE_STATES.UNREADABLE_NOW), ...unmeasured.after])];
  if (atWrap.length > 0) {
    lines.push(`Partly uncompared — the previous session's wrap recorded ${atWrap.join(', ')} but could not read part of it, so there is nothing on its side to compare against. Anything named above still changed; what is NOT named may have changed too.`);
  }
  if (now.length > 0) {
    lines.push(`Partly uncompared — this launch could not read part of ${now.join(', ')}, so drift there is unknown even though the handoff recorded it. This machine's server log names what failed. Anything named above still changed; what is NOT named may have changed too.`);
  }
  return [...lines, ''];
}
...
/**
 * Where the Resume block's text comes from (#1675), most trustworthy first.
 *
 * 1. The publication the launch verdict read (`options.handoffResume`, from
 *    `launch-preflight-context#evaluate`). Its text is the text that
 *    publication vouches for.
 * 2. The continuity index, UNBOUND: no publication this launch checked vouches
 *    for it, and every wrap that reaches its continuity step rewrites it —
 *    including one that then blocked, was cancelled or had its handoff
 *    abandoned. Shown, because it may be the only continuity there is, and
 *    labelled, because unlabelled it read as the last session's handoff.
 *
 * A publication with nothing to resume (no where-we-are, no next action)
 * yields no Resume block, as an empty index always has.
 *
 * @param {object} project - Project record
 * @param {object} options - Prime options; `handoffResume` when the launch evaluated one
 * @returns {object|null} `{origin, currentState, nextAction, freshness, ...provenance}`
 */
function _resumeSource(project, options) {
  const picked = options && options.handoffResume;
  if (picked && typeof picked === 'object') {
    const r = picked.resume || {};
    if (!r.currentState && !r.nextAction) return null;
    return {
      origin: 'publication',
      currentState: r.currentState || '',
      nextAction: r.nextAction || '',
      freshness: r.freshness || {},
      publicationId: picked.publicationId,
      digest: picked.digest || null,
      kind: picked.kind,
      sessionId: picked.sessionId,
      stagedAt: picked.stagedAt || null,
      verdict: picked.verdict || null,
      newerSession: picked.newerSession || null,
      note: picked.note || null
    };
  }
  const index = continuity.readIndex(project.path);
  if (!index) return null;
  return {
    origin: 'unbound-index',
...
/**
 * The provenance lines the Resume block opens with (#1675).
 *
 * Stated before the recorded text, and for every source, because it is what
 * decides how far that text can be trusted: a publication's identity and exact
 * staging time let a reader date it against what has happened since, and a
 * source that is older than the latest session, or bound to no publication at
 * all, has to say so before anything is proposed from it.
 * @param {object} source - From `_resumeSource`
 * @returns {string[]}
 */
function _resumeProvenanceLines(source) {
  if (source.origin !== 'publication') {
    return [
      'Source: the continuity index, UNBOUND — no published handoff vouches for it, and a later '
        + 'failed or cancelled wrap may have rewritten it. Verify it; it is not a handoff.',
      ''
    ];
  }
  const shortDigest = source.digest ? `, digest ${source.digest.slice(0, 12)}` : '';
  const lines = [
    `Source: handoff publication \`${source.publicationId}\` (${source.kind}${shortDigest}) from session ${source.sessionId}, `
      + `staged ${source.stagedAt || 'at an unrecorded time'}; launch verdict \`${source.verdict || 'unknown'}\`.`
  ];
  if (source.newerSession) {
    lines.push(`This is OLDER than the latest session: session ${source.newerSession.id} `
      + `(${source.newerSession.status || 'status unknown'}) ran after it, and that session's work is not in this handoff.`);
  }
 ...
 * @param {object|null} resume - The resume source (`_resumeSource`), if one exists
 ...
 *
 * It used to say "the TangleClaw API base URL in your project guide". Since
 * #1619 the guide does not carry that URL — a committed carrier names no
 * origin — so the old wording pointed a session at a value that is no longer
 * there, and did so exactly when the origin could not be resolved. It now
 * names the launch environment, which is where the value actually lives.
 */
const API_ORIGIN_PROSE_FALLBACK = 'the `TANGLECLAW_API` value your launch exported';
...
function getSessionStatus(projectName) {
  ...
  if (active) {
    ...
    if (probe && probe.answered && !probe.live) {
      ...
      _releaseStartupChannel(active, 'tmux session died');
 ...
 *   `projectName`.
 * @param {string} [options.controlExempt] - `'medusa-wake'` only: the switchboard
 *   nudge that carries a control notice is not refused by a HOLD (#1861).
 *   Server-internal; no route passes it from a request body.
 * @returns {{ ok: boolean, error: string|null, controlRefusal?: object }}
 */
function injectCommand(projectName, command, options = {}) {
  ...
  // #1861 — a held or stopped lane takes no injected work. Nothing is inferred
  // as harmless from the command's text. The one exemption is the switchboard
  // wake nudge, because it is how the control notice itself reaches the pane.
  if (options.controlExempt !== 'medusa-wake') {
    const refusal = controlGate.checkMutation({ surface: 'inject-command', subject: { kind: 'target', projectId: project.id } });
    if (refusal) return { ok: false, error: `${refusal.code}: ${refusal.message}`, controlRefusal: refusal };
  }
  ...
  try {
    tmux.sendKeys(active.tmuxSession, command, {
      enter: options.enter !== false,
      // Locates the composer, and names the attempt a cleared draft is kept
      // under, so a draft cleared before the paste is recoverable (#1507).
      engineId: governance.sessionEngineId(active, project),
      attemptKey: draftStore.sessionAttemptKey(active.id)
    });
...
function _startWrapPipeline(project, active, options) {
  ...
  // #1861 — the assignment this run is admitted under. The run checks THAT
  // assignment at every boundary for its whole life, so a later assignment can
  // never authorize it (the current one is checked too, and can only tighten).
  const controlAssignmentId = controlGate.captureAssignment(project.id);
  ...
  const done = _runClaimedWrap(project, active, options, claim.runId, previousRun, controlAssignmentId)
 ...
 * @param {string|null} [controlAssignmentId] - The control assignment captured when the run was admitted (#1861)
 ...
 */
async function _runClaimedWrap(project, active, options, runId, previousRun, controlAssignmentId = null) {
  let settled = false;
  ...
  try {
    ...
    try {
      pipelineResult = await wrapPipeline.runWrapPipeline(project.name, {
        ...
        wrapRunId: runId,
        // #1707 — the operator's cancel: one registry decision per step
        // boundary, plus the read that lets a cancel win over a halting step.
        // Same spread-order guarantee: only this server's registry can stop a
        // run, never a request body.
        admitStep: (step) => wrapRunRegistry.admitStep(project.name, runId, step),
        isCancelRequested: () => wrapRunRegistry.isCancelRequested(project.name, runId),
        // #1861 — durable HOLD/STOP for this run's subject. Same spread-order
        // guarantee: only this server can decide whether a run is held.
        controlGate: (surface) => controlGate.checkMutation({
          surface, subject: { kind: 'job', projectId: project.id, assignmentId: controlAssignmentId }
        })
      });
    } catch (err) {
    ...
    // #1675 — what became of this run's handoff, carried on the result so the
    // drawer and any caller can tell a wrap the next launch resumes from from
    // one it does not.
    const handoffPublication = handoff
      ? _finalizeHandoff(project, handoff, {
        lifecycleCompleted, publicationBound, keepRequested, pipelineOk: pipelineResult.ok
      })
      : _unstagedHandoff(pipelineResult);
    ...
    log.info('Wrap pipeline ran', {
      ...
      cancelledAt: pipelineResult.cancelledAt || null,
      stepCount: pipelineResult.results.length,
      commitSha: pipelineResult.commitSha,
      lifecycleCompleted,
      sessionKept,
      handoffPublication: handoffPublication.state,
      publication: handoffPublication.publicationId
    });
    ...
    const result = {
      ...
      sessionKept,
      handoffPublication,
      // #1707 — present only when an operator's cancel stopped the run: its
      // terminal outcome, distinct from a block (nothing to Retry or Skip).
      ...(pipelineResult.cancelledAt ? { outcome: 'cancelled' } : {}),
      pipelineResult,
 ...
 *
 * Returns what happened, because the wrap's result has to say it (#1675): a
 * finished wrap whose handoff did not publish is one the next launch will not
 * resume from, and a refusal that reached only the log let the drawer call it a
 * success.
 *
 * @param {object} project - Project record
 ...
 * @returns {{state: string, publicationId: string, digest: (string|null), kind: string, reason: (string|null), supersededId: (string|null), supersededById: (string|null)}}
 *   `state` is `published` (current at finalization — not a promise it is still current at the
 *   next launch), `not-published` (completed, but the publish was refused) or `abandoned`.
 *   Supersession is directional: `supersededId` is the publication this one displaced,
 *   `supersededById` the newer one it lost to.
 */
function _finalizeHandoff(project, handoff, verdicts) {
  ...
  const outcome = (state, reason, supersession = {}) => ({
    state,
    publicationId: handoff.publicationId,
    digest: _publicationDigest(handoff.publicationId),
    kind: handoff.kind,
    reason: reason || null,
    supersededId: supersession.supersededId || null,
    supersededById: supersession.supersededById || null
  });
  const abandon = (reason) => {
    handoffPublish.abandonHandoff(handoff.publicationId, reason);
    return outcome('abandoned', reason);
  };
  const publish = () => {
    const result = handoffPublish.publishHandoff(project, handoff.publicationId);
    if (result.published) {
      return outcome('published', null, { supersededId: result.supersededId, supersededById: result.supersededById });
    }
    ...
    // input car 21.8 reads as `handoff-unconfirmed`.
    ...
    return outcome('not-published', result.reason, { supersededById: result.supersededById });
  };

  // A kept session has no lifecycle transition, so the checkpoint binds itself.
  if (keepRequested) {
    if (!pipelineOk) return abandon('pipeline-failed');
    const bound = store.handoffs.markCheckpointComplete(
      handoff.publicationId, handoff.wrapRunId, new Date().toISOString()
    );
    if (!bound) return abandon('checkpoint-not-bound');
    return publish();
  }

  if (!pipelineOk) return abandon('pipeline-failed');
  // Kill won the race: the attempt never completed, so it is abandoned rather
  // than published.
  if (!lifecycleCompleted) return abandon('lifecycle-incomplete');
  if (publicationBound !== true) return abandon('eligibility-not-bound');
  return publish();
}

/**
 * The recorded digest of a publication's bytes, or null when the row cannot be
 * read. The digest is what a launch compares, so it comes from the row that
 * attests to the bytes, never recomputed from a file here.
 * @param {string} publicationId
 * @returns {string|null}
 */
function _publicationDigest(publicationId) {
  try {
    const row = store.handoffs.get(publicationId);
    return row && row.fileDigest ? row.fileDigest : null;
  } catch (err) {
    log.warn('Could not read a publication row for the wrap result', { publicationId, error: err.message });
    return null;
  }
}

/**
 * The wrap result's account of its handoff when no attempt was staged (#1675).
 *
 * Read from the run's own `handoff-stage` row: a step that skipped or blocked
 * says why, and a run that stopped before the step (blocked, cancelled) never
 * reached it.
 * @param {object} pipelineResult - The finished pipeline result
 * @returns {{state: 'not-staged', publicationId: null, digest: null, kind: null, reason: string, supersededId: null, supersededById: null}}
 */
function _unstagedHandoff(pipelineResult) {
  const row = ((pipelineResult && pipelineResult.results) || []).find((r) => r && r.kind === 'handoff-stage');
  let reason;
  if (!row || row.status === 'pending') {
    reason = 'the wrap stopped before it staged a handoff';
  } else {
    const said = row.output && typeof row.output.reason === 'string' ? row.output.reason : null;
    reason = said || `the handoff step ended ${row.status || 'without a status'}`;
  }
  return { state: 'not-staged', publicationId: null, digest: null, kind: null, reason, supersededId: null, supersededById: null };
}
...
function _completePipelineWrap(active, pipelineResult, handoff) {
  ...
  _releaseStartupChannel(active, 'session wrapped');
...
function startWrap(projectName, options) {
  ...
  const intent = _resolveWrapIntent(target.project, target.active, options);
  // A running wrap is refused as in progress (with its run id), never re-gated
  // by this project's setting, for the same reason the stranded gate above skips it.
  if (!intent.ok && !wrapRunRegistry.get(target.project.name).running) {
    return {
      ok: false,
      code: 'WRAP_KEEP_SETTING_INVALID',
      sessionId: target.active.id,
      wrapCommand: null,
      wrapSteps: [],
      captureFields: [],
  ...
  const started = _startWrapPipeline(target.project, target.active, intent.ok ? intent.options : options);
  return started.ok ? { ...started, strandedUnchecked, ...intent.planned } : started;
}

/**
 * Resolve, once and before the run is claimed, whether this wrap will end its
 * session if it completes (#1708). The answer is written into the options every
 * reader of the run sees: the registry (and so `GET /wrap/status` and a Retry),
 * the `handoff-stage` step, which stages a `checkpoint` or a `final` handoff
 * from it mid-run, and the kill decision after the pipeline. Resolved at any
 * one of those sites alone, a wrap inheriting "keep" from its project would
 * stage a final handoff for a session that is then kept.
 *
 * A Retry of this session's unfinished run keeps that run's server-resolved
 * answer and its source unless the request explicitly sends a different
 * boolean: the trusted record is the run, not whatever a page replays.
 * Caller-supplied `keepSource` and `sessionOutcomePlanned` are never trusted;
 * both are overwritten or dropped here.
 *
 * @param {object} project - Project record
 * @param {object} active - The session the wrap targets
 * @param {object} [options] - The request's per-wrap choices (already validated)
 * @returns {{ok: true, options: object, planned: {sessionOutcomePlanned: 'end'|'keep', keepSource: string}} | {ok: false, error: string}}
 */
function _resolveWrapIntent(project, active, options) {
  const requested = options ? options.keepSessionRunning : undefined;
  let resolved = _inheritedKeep(project, active, requested);
  if (!resolved) {
    let readFailed = false;
    const projConfig = typeof requested === 'boolean'
      ? null
      : store.projectConfig.load(project.path, { onError: () => { readFailed = true; } });
    resolved = projectConfig.resolveKeepSessionRunning(requested, projConfig, { readFailed });
  }
  if (!resolved.ok) {
    log.warn('Wrap refused: the keep-running setting cannot be resolved', { project: project.name, error: resolved.error });
    return resolved;
  }
  const { sessionOutcomePlanned: _ignored, ...rest } = options || {};
  const resolvedOptions = { ...rest, keepSessionRunning: resolved.keep, keepSource: resolved.source };
  return { ok: true, options: resolvedOptions, planned: projectConfig.plannedSessionOutcome(resolvedOptions) };
}

/**
 * The keep-running answer a Retry inherits, or null to resolve afresh.
 *
 * A Retry follows this session's last run when that run finished without
 * completing (blocked, cancelled or failed). Its recorded answer carries over
 * unless the request sends a boolean that differs from it.
 *
 * @param {object} project - Project record
 * @param {object} active - The session the wrap targets
 * @param {*} requested - The request's `keepSessionRunning`
 * @returns {{ok: true, keep: boolean, source: string}|null}
 */
function _inheritedKeep(project, active, requested) {
  const previous = wrapRunRegistry.get(project.name);
  const recorded = previous.options;
  if (previous.running || previous.sessionId !== active.id || !previous.result || previous.result.ok === true) return null;
  if (!recorded || typeof recorded.keepSessionRunning !== 'boolean' || typeof recorded.keepSource !== 'string') return null;
  if (typeof requested === 'boolean' && requested !== recorded.keepSessionRunning) return null;
  return { ok: true, keep: recorded.keepSessionRunning, source: recorded.keepSource };
}

/**
 * Ask a project's running wrap to stop at its next step boundary (#1707) — the
 * call behind `POST /api/sessions/:project/wrap/cancel`.
 *
 * Only before the run reaches its `commit` step; the running step always
 * finishes. A cancelled run ends with `cancelledAt` set and leaves the session
 * running (as a stopped run does). What a cancel guarantees is that the wrap
 * makes no commit, branch, push, PR or auto-merge. It does not undo what the
 * steps that already ran did: content steps can edit the working tree, and
 * others write TangleClaw's database or local methodology state. Those stay,
 * uncommitted, and the report lists the steps that ran.
 *
 * @param {string} projectName - Project name
 * @param {string} runId - The run the caller is watching (from the 202 or `/wrap/status`)
 * @returns {{ok: true, runId: string, willStopBefore: string|null, finishingStepId: string|null} | {ok: false, code: 'NOT_FOUND'|'WRAP_RUN_NOT_FOUND'|'WRAP_NOT_CANCELLABLE', currentStepId?: string|null, error: string}}
 */
function cancelWrap(projectName, runId) {
  const project = store.projects.getByName(projectName);
  if (!project) return { ok: false, code: 'NOT_FOUND', error: `Project "${projectName}" not found` };
  const answer = wrapRunRegistry.requestCancel(project.name, runId);
  if (answer.ok) {
    log.info('Wrap cancel requested', { project: project.name, runId, willStopBefore: answer.willStopBefore });
    return { ok: true, runId, willStopBefore: answer.willStopBefore, finishingStepId: answer.finishingStepId };
  }
  if (answer.code === 'WRAP_NOT_CANCELLABLE') {
    return {
      ok: false,
      code: answer.code,
      currentStepId: answer.currentStepId,
      error: `The wrap is past the point of cancelling (at step "${answer.currentStepId}"): `
        + 'from the commit step on it may already have branched, committed, pushed or opened a PR, so it runs to its end.'
    };
  }
  return {
    ok: false,
    code: answer.code,
    error: `No running wrap with that run id for "${project.name}". Read GET /api/sessions/:project/wrap/status for the current run.`
  };
}
...
function completeWrap(projectName, summary, expectedSessionId) {
  ...
  _releaseStartupChannel(target, 'session wrapped');
...
function killSession(projectName, reason) {
  ...
  // Forget this session's Medusa listener + workspace id (MED-2K9P Chunk 04).
  _teardownMedusa(project, target);
  _releaseStartupChannel(target, 'session killed');
 ...
 *   `readiness`, `onRecord(fields)`, and optional `attemptKey` (the draft store's
 *   key for this session) and `sleep` / `watch` seams.
 * @returns {Promise<void>}
 */
async function _pastePrime(ctx) {
  const { tmuxName, engineId, projectName, primeText, readiness, onRecord } = ctx;
  const attemptKey = ctx.attemptKey || null;
  const sleep = ctx.sleep || ((ms) => new Promise((r) => setTimeout(r, ms)));
  const watchOpts = ctx.watch || {};

  tmux.sendKeys(tmuxName, primeText, { enter: true, engineId, attemptKey });
 ...
 * @param {object|null} [kickoffCtx=null] - What the silent-prime kickoff needs
 *   to ask the session to read its context (#1635), and what the automatic
 *   bootstrap needs (#1825 B3):
 *   `{sessionId, projectId, hasSequence, startupDelivery}`. `startupDelivery`
 *   is the launch's frozen selection (`'native'` | `'legacy'`); `'native'`
 *   withholds the paste and the kickoff and hands the first turn to the
 *   bootstrap. Null from callers that aren't launching a real session, which
 *   is why the branches test it rather than testing `silentPrime` alone.
 */
function _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText, launchMode, silentPrime, deliveryBase = null, kickoffCtx = null) {
  let delay = 0;
  // #1825 B3 — a launch that selected its native startup channel is never
  // typed into: the paste (phase 2) and the kickoff (phase 3) are both
  // withheld, and the bootstrap below is its one first turn. Read from the
  // launch's frozen selection, not re-derived here.
  const nativeStartup = !!(kickoffCtx && kickoffCtx.startupDelivery === 'native');
  if (nativeStartup && deliveryBase) {
    // The prime with its inline rules is not pasted, so the rules ledger must
    // not read `delivered` for it. The rules ARE served: the governance step of
    // the sequence the fired prompt asks the engine to read carries them, which
    // is the same fact the pull-pointer launch records with the same reason.
    _recordRuleDelivery({ ...deliveryBase, channel: 'none', outcome: 'skipped', skipReason: store.RULES_SERVED_BY_LAUNCH_SEQUENCE });
  }
  // The attempt a draft cleared before a prime paste is kept under (#1507).
  const attemptKey = kickoffCtx && kickoffCtx.sessionId != null ? draftStore.sessionAttemptKey(kickoffCtx.sessionId) : null;
  ...
  //
  // Withheld on a native launch too (#1825 B3, Architect F2): a preKey is a
  // keystroke, and two Enters on a fresh trust or login dialog accept its
  // default before the adapter's own readiness check could ever refuse
  // `trust_required`. The native path's contract is that TangleClaw types
  // nothing into that pane — the dialog is the operator's to answer.
  const resolved = _resolvePreKeys(engineProfile, launchMode);
  const preKeys = nativeStartup ? null : resolved.preKeys;
  ...
  if (primeText && !silentPrime && !nativeStartup && engineProfile.capabilities && engineProfile.capabilities.supportsPrimePrompt) {
    ...
    const paste = async (readiness) => {
      ...
      try {
        ...
        await _pastePrime({
          tmuxName, engineId, projectName, primeText, readiness, attemptKey,
          onRecord: record
        });
      } catch (err) {
  ...
  // Phase 3: the kickoff (#1635). Silent prime delivers the whole launch context
  // as hidden model context and types nothing, so the engine boots fully briefed
  // with no turn to answer — it waits, and until this existed the only thing that
  // ever broke that wait was `launch-unready`'s nudge, minutes later, or a human.
  //
  // Deliberately NOT an `else` on the paste above: the two are guarded on
  // different questions. The paste is skipped when the prime rode the hook; the
  // kickoff is owed when there is a sequence to read. A launch can legitimately
  // be neither (a pasted prime is itself the first turn) and the module says so
  // rather than this branch guessing.
  //
  // So the ONLY thing tested here is whether this is a real launch with a
  // session to speak for — `kickoffCtx` is null from every caller that is not
  // launching one. Re-testing `silentPrime` here would make the module's own
  // `not-silent` and `no-pane` answers unreachable in production, leaving two
  // declared outcomes that only tests could ever see. Every guard it applies is
  // a cheap synchronous return, so asking it costs a function call.
  if (kickoffCtx && !nativeStartup) {
    setTimeout(() => {
      launchKickoff.kickoff({
        sessionId: kickoffCtx.sessionId,
        projectId: kickoffCtx.projectId,
        projectName,
        tmuxName,
        engineId,
        silentPrime,
        hasSequence: kickoffCtx.hasSequence
      }).then((outcome) => {
        // info, not debug: `lib/logger.js` defaults to info, and the symptom
        // this whole change removes is a session that sits there saying
        // nothing. If that ever recurs, the operator's first question is
        // whether the kickoff ran and declined or never ran at all — a line
        // only visible after raising the log level cannot answer it.
        log.info('Launch kickoff outcome', {
          project: projectName, session: kickoffCtx.sessionId,
          outcome, meaning: launchKickoff.OUTCOME_MEANINGS[outcome]
        });
      // prawduct:allow prawduct/broad-except -- a launch must never fail on the kickoff
      }).catch((err) => {
        log.warn('Launch kickoff failed', { project: projectName, error: err.message });
      });
    }, delay);
  }

  // Phase 4: the automatic bootstrap (#1825 B3). For a native launch it is the
  // first turn — one fire through the engine's channel after the pane gate.
  // For a legacy launch it records, through the same service, why this launch
  // could not go native (S3), and changes nothing about the paste or kickoff
  // above. The module answers `no-sequence` itself for a launch with nothing
  // to read, so the only guard here is the one the kickoff uses: a real launch.
  if (kickoffCtx) {
    setTimeout(() => {
      launchBootstrap.bootstrap({
        ...
        engineId,
        hasSequence: kickoffCtx.hasSequence,
        startupDelivery: kickoffCtx.startupDelivery === 'native' ? 'native' : 'legacy'
      // The module logs its own outcome line, with the reason code; only the
      // rejection that must never reach the launch is caught here.
      // prawduct:allow prawduct/broad-except -- a launch must never fail on its bootstrap
      }).catch((err) => {
        log.warn('Launch bootstrap failed', { project: projectName, error: err.message });
      });
    }, delay);
  }
}
 ...
 *
 * Written through the provenance writer as its `session-prime` surface, so a
 * project that opted in gets its provenance line here (ADR 0019); one that did
 * not gets exactly the prime text.
 * @param {string} projectPath - Absolute path to the project directory
 * @param {string} primeText - Full prime prompt text to write
 * @param {{config?: object, project?: string, engine?: string}} [provenanceCtx] -
 *   The project's loaded config and the provenance placeholder values.
 * @returns {string|null} - Absolute path to the written file, or null on failure
 */
function _writePrimeFile(projectPath, primeText, provenanceCtx = {}) {
  try {
    const filePath = tcProjectFiles.resolveIn(projectPath, tcProjectFiles.SESSION_PRIME_RELPATH);
    provenance.writeOwnedFile(filePath, 'session-prime', primeText, provenanceCtx);
...
/**
 * Write the context re-entry preamble the prime hook emits ahead of the prime
 * when the engine re-fires it after `/clear` or a compaction (#1761).
 * Non-throwing, like the prime: a missing preamble leaves the hook emitting
 * the prime alone, which is what it did before re-entry existed.
 *
 * Written through the provenance writer as its `session-reentry` surface. The
 * prime's budget reserves this file's length through `lib/prime-hook-output.js`.
 * @param {string} projectPath - Absolute path to the project directory
 * @param {object} project - The project record, for its name
 * @param {{config?: object, project?: string, engine?: string}} [provenanceCtx] -
 *   The project's loaded config and the provenance placeholder values.
 * @returns {string|null} - Absolute path to the written file, or null on failure
 */
function _writeReentryFile(projectPath, project, provenanceCtx = {}) {
  try {
    const filePath = tcProjectFiles.resolveIn(projectPath, tcProjectFiles.SESSION_REENTRY_RELPATH);
    provenance.writeOwnedFile(filePath, 'session-reentry', renderReentryPreamble(project), provenanceCtx);
    return filePath;
  } catch (err) {
    log.warn('Failed to write session re-entry file', { projectPath, error: err.message });
    return null;
  }
}
 ...
 * and the re-entry preamble written beside it (#1761), which must never outlive
 * the prime it introduces.
...
function _removePrimeFile(projectPath) {
  try {
    const fs = require('node:fs');
    const path = require('node:path');
    fs.rmSync(tcProjectFiles.resolveIn(projectPath, tcProjectFiles.SESSION_REENTRY_RELPATH), { force: true });
    const filePath = tcProjectFiles.resolveIn(projectPath, tcProjectFiles.SESSION_PRIME_RELPATH);
 ...
 *
 * #1839: forgetting the workspace retires it for good (the next session mints
 * a new id), so any open exchange addressed to it can never be delivered.
 * Those exchanges end as `recipient_retired` and each initiator is told,
 * rather than left waiting. Carrying the mail to a successor is #1806.
...
function _teardownMedusa(project, session) {
  ...
  try {
    workspaceId = require('./medusa-registry').getWorkspaceId(project.path, session.id);
  } catch (err) {
    log.warn('Could not read the retiring Medusa workspace; its open exchanges stay open', { sessionId: session.id, error: err.message });
  }
  medusa.forgetSession({ projectPath: project.path, sessionId: session.id });
  if (!workspaceId) return;
  try {
    require('./medusa-watchdog').retireRecipient(workspaceId).catch((err) => {
      log.warn('Could not tell initiators about a retired Medusa workspace', { workspaceId, error: err.message });
    });
  } catch (err) { // prawduct:allow prawduct/broad-except -- teardown must complete even if the exchange record cannot be updated
    log.warn('Could not end exchanges to a retired Medusa workspace', { workspaceId, error: err.message });
  }
}
...
module.exports = {
  ...
  renderLaunchSteps,
  _resumeSource,
  _resumeProvenanceLines,
  _ruleDriftLines,
  _controlPrimeLines,
  _stoppedLaunchRefusal,
  _rebindControl,
  syncControlHooks,
  _launchRuleDrift,
  LAUNCH_STEP_IDS,
  ...
  _apiOrigin,
  _buildLaunchCommand,
  _hardenedLegacyLaunchCommand,
  _withPathFloor,
  _resolvePreKeys,
  // Test seam. The kickoff's own gates are unit-tested in isolation; what this
  // export buys is the HOP between them — a launch that decides correctly and
  // never calls the thing that acts on the decision is the failure mode this
  // project has already paid for once (2026-09-14: widget and server both
  // worked, nothing connected them).
  _deferEngineInit,
  _awaitPaneReady,
