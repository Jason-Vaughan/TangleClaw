...
function launchSession(projectName, options = {}) {
  ...
  if (!primeText) {
    _recordRuleDelivery({ ...deliveryBase, channel: 'none', delivered: false, skipReason: 'prime prompt disabled for this launch' });
  } else if (silentPrime) {
    _recordRuleDelivery(primeFilePath
      ? { ...deliveryBase, channel: 'prime-file', delivered: true }
      : { ...deliveryBase, channel: 'prime-file', delivered: false, skipReason: 'failed to write .tangleclaw/session-prime.md' });
  } else if (!supportsPaste) {
    _recordRuleDelivery({ ...deliveryBase, channel: 'none', delivered: false, skipReason: `engine ${engineId} declares no prime channel (supportsPrimePrompt is false)` });
  }
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  // cannot derive from CLAUDE.md or the engine's own banner. Methodology name +
  // description, current phase, archive paths, extension-rule definitions, and
  // shared-doc pointers are intentionally omitted — all are already injected
  ...
  if (primeText && !silentPrime && engineProfile.capabilities && engineProfile.capabilities.supportsPrimePrompt) {
    ...
    setTimeout(() => {
      if (!tmux.hasSession(tmuxName)) {
        ...
        if (deliveryBase) {
          _recordRuleDelivery({ ...deliveryBase, channel: 'prime-paste', delivered: false, skipReason: 'tmux session ended before the prime was pasted' });
        }
        return;
      }
      try {
        ...
        if (deliveryBase) {
          _recordRuleDelivery({ ...deliveryBase, channel: 'prime-paste', delivered: true });
        }
      } catch (err) {
        log.warn('Failed to inject prime prompt', { project: projectName, error: err.message });
        if (deliveryBase) {
          _recordRuleDelivery({ ...deliveryBase, channel: 'prime-paste', delivered: false, skipReason: `tmux send-keys failed: ${err.message}` });
        }
      }
    }, delay);
  }
