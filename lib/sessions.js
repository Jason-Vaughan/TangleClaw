...
function launchSession(projectName, options = {}) {
  ...
  const snapshot = _buildLaunchSnapshot(launchId, project, engineProfile, sequenceApplicability, startupRules, primeOptions);
 ...
 */
function _buildLaunchSnapshot(launchId, project, engineProfile, applicability, startupRules, primeOptions) {
  const rules = startupRules.rules || [];
  try {
    const preflight = launchSequence.PREFLIGHT_NOT_EVALUATED;
    const rendered = applicability.applicable
      ? renderLaunchSteps(project, engineProfile, { ...primeOptions, preflight })
      : null;
    return launchSequence.buildSnapshot({ launchId, project, engineProfile, applicability, rendered, rules, preflight });
 ...
 * @param {boolean} [pulled=false] - The rules are served by a launch step, not the prime.
 * @returns {string[]} Prime lines.
 */
function _ruleSourcesSection(project, engineProfile, startupRules, viaStartupHook, pulled = false) {
  ...
  sources.push(ruleCount > 0
    ? `TangleClaw project rules — ${ruleCount} startup rule${ruleCount === 1 ? '' : 's'} for this project, delivered ${pulled ? 'in this launch step' : (viaStartupHook ? 'through the rules hook' : 'in this prime')}.`
...
function _collectPrimeSections(project, engineProfile, options = {}) {
  ...
  const rulesLines = viaStartupHook && !pull
  ...
  add('governance', ..._ruleSourcesSection(project, engineProfile, startupRules, viaStartupHook && !pull, pull));
...
/**
 * Render one launch step. A convenience over `renderLaunchSteps` for a caller
 * that needs only one; the launch path renders all four at once so they are
 * built from the same reads.
 * @param {string} step - One of `LAUNCH_STEP_IDS`
 * @param {object} ctx - `{project, engineProfile, options}`
 * @returns {string}
 */
function renderLaunchStep(step, ctx) {
  if (!LAUNCH_STEP_IDS.includes(step)) throw new Error(`Unknown launch step "${step}"`);
  return renderLaunchSteps(ctx.project, ctx.engineProfile, ctx.options || {})[step];
}
...
function _configAndSharedDocsLines(project, engineProfile) {
  const file = engineProfile && engineProfile.configFormat && engineProfile.configFormat.filename;
  ...
  try {
    const docs = store.projectGroups.getByProject(project.id)
...
function _planPointerLines(project) {
  ...
  // prawduct:allow prawduct/broad-except -- a launch step must render even when the plans directory cannot be read
    ...
    lines.push(`The plans directory could not be read (${err.message}).`);
  }
  if (plans.length === 0) {
