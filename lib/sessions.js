...
function _collectPrimeSections(project, engineProfile, options = {}) {
  ...
  // Live install checkout (#993): only for the project whose checkout the
  // server runs and serves, where its branch and uncommitted state are live.
  // Cached reads — the launch route warms the local read; an origin not yet
  // observed is said as unknown, never as current.
  if (checkoutFreshness.isLiveInstall(project.path, checkoutFreshness.LIVE_INSTALL_ROOT)) {
    add('state', ...checkoutFreshness.primeLines(checkoutFreshness.liveInstallSnapshot(store.config.load())));
  }
