...
function _prepareStartupChannel(engineId, project, engineProfile, launchCmd, enginePath, env = {}) {
  ...
  return { command: prepared.command, handle: prepared.handle, adapter: resolved.adapter, reasonCode: null, reason: null };
}
...
function _attachStartupChannel(channel, session, engineId) {
  try {
    ...
    channel.adapter.attachLaunch(channel.handle, { sessionId: session.id, sequenceId: sequence ? sequence.id : 0, engineId });
  ...
  if (kickoffCtx) {
    setTimeout(() => {
      launchBootstrap.bootstrap({
        ...
        log.info('Launch bootstrap ran', {
          project: projectName, session: kickoffCtx.sessionId, outcome, meaning: launchBootstrap.OUTCOME_MEANINGS[outcome]
        });
