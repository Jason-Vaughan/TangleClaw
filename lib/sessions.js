...
function launchSession(projectName, options = {}) {
  ...
  if (existing) {
    ...
    if (probe && !probe.live) {
      ...
      // The wrapping-recovery branch below has always done this; the crashed
      // branch was the omission.
  ...
  // means nothing on this engine. Recorded at info, not warn: a stored value
  // on such a project is indistinguishable from the shipped default, so an
  // alarm would fire on every non-Claude launch about a preference nobody
  // set — the settings modal is where the operator is told, in words.
  if (silentPrimeDisposition === 'not-applicable') {
    log.info('silentPrime does not apply on this engine — neither honored nor offered', {
      project: projectName,
      engine: baseEngineId,
      setting: 'silentPrime',
      reason: `engine ${engineId} does not support silent prime (capabilities.supportsSilentPrime is not true)`
    });
  }
  ...
  if (options.primePrompt !== false) {
    primeText = generatePrimePrompt(project, engineProfile, { medusaWorkspaceId, startupRules, continuityMode: options.continuityMode });
  }
 ...
 * @param {boolean} viaStartupHook - Whether project rules ride the hook channel.
 * @returns {string[]} Prime lines.
 */
function _ruleSourcesSection(project, engineProfile, startupRules, viaStartupHook) {
  const configFile = engineProfile && engineProfile.configFormat && engineProfile.configFormat.filename
    ? `\`${engineProfile.configFormat.filename}\``
  ...
  sources.push(ruleCount > 0
    ? `TangleClaw project rules — ${ruleCount} startup rule${ruleCount === 1 ? '' : 's'} for this project, delivered ${viaStartupHook ? 'through the rules hook' : 'in this prime'}.`
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  sections.push(...ciStatus.primeLines(ciStatus.readCached(project.path)));
  ...
  sections.push(..._ruleSourcesSection(project, engineProfile, startupRules, viaStartupHook));
  ...
  // project id (the #1121 name-vs-id trap), the MagicDNS link convention, the
  ...
  const primerCtx = {
    ...
    operatorHost: sessionOwnership._localHost()
  };
  sections.push(_yieldable(0,
  ...
  if (resume && options.continuityMode !== 'fresh') {
    ...
    sections.push('4. Wait for the operator\'s go. Do not auto-execute the next action.');
    sections.push('');
  } else {
 ...
 *
 * @param {string} projectName - Project directory name
 * @returns {{ active: boolean|null, sessionId?: number, project: string, engine?: string,
...
function getSessionStatus(projectName) {
  ...
  if (active) {
    ...
    if (probe && probe.answered && !probe.live) {
      ...
      //
      // The wrapping branch below already works this way; this is the same
      // treatment on the branch it was carved out of.
...
/** How long to watch for a pasted prime to land in the transcript (#1134). */
const PASTE_LANDED_TIMEOUT_MS = 20000;
/** Poll interval for that watch. */
const PASTE_LANDED_POLL_MS = 1000;
/**
 * How many characters of the pasted text to look for in the pane.
 *
 * Short enough to survive the TUI wrapping a long line, long enough that it
 * cannot collide with ordinary transcript prose.
 */
const PASTE_ECHO_CHARS = 24;
/**
 * Extra paste attempts after the first, when the first is observed NOT to land.
 *
 * Bounded and small. Re-pasting a prime is cheap and harmless — it is context,
 * not an action — whereas booting with no prime is the failure this exists to
 * catch, so the bias is toward one retry rather than zero. It is never
 * unbounded: a pane that swallows everything must not be pasted into forever.
 */
...
/**
 * Whitespace-normalize pane or prompt text so a comparison survives the TUI's
 * own wrapping and padding.
 * @param {string} text - Raw text.
 * @returns {string}
 */
function _normalizePane(text) {
  return String(text || '').replace(/\s+/g, ' ').trim();
}

/**
 * The distinctive slice of a pasted prime to look for in the pane (#1134).
 *
 * Taken from the first non-empty line, because that is what the TUI echoes at
 * the top of the submitted block.
 *
 * @param {string} primeText - The text that was pasted.
 * @returns {string} A normalized token, or '' when the text is too short to
 *   yield one that could not collide with ordinary prose.
 */
function _pasteEchoToken(primeText) {
  const firstLine = String(primeText || '').split('\n').find((l) => l.trim());
  const token = _normalizePane(firstLine).slice(0, PASTE_ECHO_CHARS);
  return token.length >= PASTE_ECHO_CHARS ? token : '';
}

/**
 * Whether this engine declares that its pastes should be watched (#1134).
 *
 * One owner for the predicate, read by both the watch itself and the paste
 * path that decides whether to await it — two places asking the same question
 * by hand is how one of them ends up asking a different one.
 *
 * @param {string} engineId - Engine id.
 * @returns {boolean}
 */
function _pasteObservationDeclared(engineId) {
  const profile = medusaWake.ENGINE_WAKE_PROFILES[engineId];
  return !!(profile && profile.promptRe && profile.observePaste);
}

/**
 * Watch a pane after a prime paste and answer whether the engine took it
 * (#1134).
 *
 ...
 * discards anything submitted ("Please try again shortly"). Both declared
 * signals passed, the paste went out, the ledger said `delivered`, and no agent
 * brain was ever born from it. There is no lexical tell separating the
 * swallowing state from the ready one, so the only honest evidence is what
 * happens AFTER the send.
 *
 * **What is measured, and why not the digest.** Measured on agy 1.1.24: typing
 * into the composer already moves the pane digest, before Enter — so
 * "the transcript changed" is satisfied by our own keystrokes and proves
 * nothing. A landed submission instead echoes our text into the transcript AND
 * returns the composer to a bare prompt; a swallowed one clears the composer
 * with our text nowhere. So landing is: **our echo present, composer bare.**
 *
 * Gated on the profile's `observePaste`, not merely on having a prompt
 * signature: an engine nobody has measured swallowing a paste does not get a
 * watch inferred for it. It records the honest "not observed" reason and its
 * paste behaves exactly as before — the same rule `idleMarker` follows, and the
 * reason the Claude blind-paste row still reads `unverified` rather than being
 * silently re-judged by a mechanism built from another engine's evidence.
 *
 * @param {string} tmuxName - tmux session.
 * @param {string} engineId - Engine id, for its wake profile.
 * @param {string} primeText - The text just pasted.
 * @param {object} [opts] - Test seams (`capture`, `now`, `sleep`, `timeoutMs`).
 * @returns {Promise<{observed: 'landed'|'not-landed'|'unmeasured', reason: string, waitedMs: number}>}
 */
async function _observePasteLanded(tmuxName, engineId, primeText, opts = {}) {
  const capture = opts.capture || ((name) => tmux.capturePane(name, { lines: 40 }));
  const now = opts.now || (() => Date.now());
  const sleep = opts.sleep || ((ms) => new Promise((r) => setTimeout(r, ms)));
  const timeoutMs = opts.timeoutMs || PASTE_LANDED_TIMEOUT_MS;

  const profile = medusaWake.ENGINE_WAKE_PROFILES[engineId];
  if (!_pasteObservationDeclared(engineId)) {
    return {
      observed: 'unmeasured',
      reason: `engine ${engineId} declares no post-paste observation (its swallowing behaviour has never been measured), so whether the paste landed was not observed`,
      waitedMs: 0
    };
  }
  const token = _pasteEchoToken(primeText);
  if (!token) {
    return {
      observed: 'unmeasured',
      reason: 'the pasted text was too short to yield an echo token that could not collide with ordinary transcript prose',
      waitedMs: 0
    };
  }
  ...
  while (now() - started < timeoutMs) {
    let lines = null;
    try {
      lines = (capture(tmuxName) || {}).lines || null;
    } catch (err) {
      lastError = err.message;
    }
    if (lines) {
      const composerBare = lines.some((l) => profile.promptRe.test(l));
      const echoPresent = _normalizePane(lines.join('\n')).includes(token);
      if (echoPresent && composerBare) {
        return { observed: 'landed', reason: 'the pasted text is in the transcript and the composer returned to its prompt', waitedMs: now() - started };
      }
    }
    await sleep(PASTE_LANDED_POLL_MS);
  }
  return {
    observed: 'not-landed',
    reason: lastError
      ? `the pane could not be read while watching for the paste to land (${lastError})`
      : `the pasted text never appeared in the transcript within ${timeoutMs}ms — the engine discarded it`,
    waitedMs: now() - started
  };
}
 ...
 *
 * **The post-paste observation outranks the gate (#1134).** A pane observed at
 * rest before the send is not evidence the send was taken: antigravity 1.1.22
 * renders the full at-rest UI while still verifying an account and discards
 * whatever is submitted. So a paste watched INTO the transcript is `delivered`
 * even where the gate was blind, and a paste observed NOT to land is
 * `unverified` even where the gate was satisfied — which is the case #1134
 * reported and the one this ordering exists for. When nothing could observe the
 * far side (an engine with no probed prompt signature), the gate's own verdict
 * stands, as before.
 *
 ...
 * @param {{observed: string, reason: string}} [landed] - Result of
 *   `_observePasteLanded`; absent on paths that do not watch.
 * @returns {{outcome: string, skipReason?: string}} Ledger fields for the row
 */
function _primePasteOutcome(readiness, landed) {
  if (landed && landed.observed === 'landed') {
    return { outcome: 'delivered' };
  }
  if (landed && landed.observed === 'not-landed') {
    return { outcome: 'unverified', skipReason: landed.reason };
  }
  ...
  return {
    outcome: 'unverified',
    skipReason: (readiness && readiness.reason) || 'paste was not readiness-gated and nothing observed the pane'
  };
}
  ...
  if (primeText && !silentPrime && engineProfile.capabilities && engineProfile.capabilities.supportsPrimePrompt) {
    ...
    const paste = async (readiness) => {
      ...
      try {
        ...
        // agent was ever born from it. Bounded retries because a re-pasted
        // prime is cheap (it is context, not an action) and a session booted
        // with no prime is the failure this exists to catch.
        const record = (landed) => {
          log.debug('Prime prompt injected', {
            project: projectName, landed: landed && landed.observed, waitedMs: landed && landed.waitedMs
          });
          if (deliveryBase) {
            _recordRuleDelivery({ ...deliveryBase, channel: 'prime-paste', ..._primePasteOutcome(readiness, landed) });
          }
        };

        tmux.sendKeys(tmuxName, primeText, { enter: true });

        // An engine that declares no post-paste observation keeps this path
        // exactly as it was — synchronous, one send, the gate's own verdict.
        // The await below is not free: it moves the ledger write into a
        // microtask, so making every engine take it would change WHEN the row
        // appears for engines this change has no evidence about.
        if (!_pasteObservationDeclared(engineId)) {
          record(null);
          return;
        }
        ...
        // the complete at-rest UI and still discards what is submitted, so the
        // gate passed, the paste went out, the ledger said `delivered`, and no
        // agent was ever born from it. Retries are bounded: a re-pasted prime
        // is cheap (it is context, not an action), a session booted with no
        // prime is the failure this exists to catch, and a pane that swallows
        // everything must not be pasted into forever.
        let landed = await _observePasteLanded(tmuxName, engineId, primeText);
        for (let attempt = 1; attempt <= PASTE_MAX_RETRIES && landed.observed === 'not-landed'; attempt++) {
          if (!tmux.probeSession(tmuxName).live) break;
          log.info('Re-pasting the prime — the previous attempt was not observed landing', {
            project: projectName, engine: engineId, attempt: attempt + 1, reason: landed.reason
          });
          await new Promise((r) => setTimeout(r, PASTE_RETRY_BACKOFF_MS * attempt));
          tmux.sendKeys(tmuxName, primeText, { enter: true });
          landed = await _observePasteLanded(tmuxName, engineId, primeText);
        }
        record(landed);
      } catch (err) {
