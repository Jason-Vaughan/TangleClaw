 ...
 * declares no limit of its own — see `_resolvePrimeBudget`, which prefers the
 * engine's declared `startupInjection.maxChars`.
 *
...
function _resolvePrimeBudget(engineProfile, options = {}) {
  const fallback = PRIME_MAX_TOKENS * 4;
  if (options.viaStartupHook === false) return fallback;

  const injection = engineProfile
    && engineProfile.capabilities
    && engineProfile.capabilities.startupInjection;
  if (!injection || injection.maxChars === undefined) return fallback;
  ...
  // "believed delivered, actually wasn't" failure this whole mechanism exists
  // to end. Say it out loud.
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  if (projConfig.featureIndexEnabled === true
    ...
    try {
      ...
      const { backlogEntries } = summarizeFeatureIndexForPrime(contents);
      ...
      // Absence is normal — the index is opt-in scaffolding and must never
      // block a launch. But the pointer is now the index's ONLY representation
      // in the prime, so a parse or read failure removes it completely rather
      // than degrading it. Log, so that disappearance is attributable instead
      // of looking like the feature was never enabled.
      // prawduct:allow prawduct/broad-except -- session launch must survive any
      // failure to read optional project scaffolding
