  ...
  if (primeText && !silentPrime && engineProfile.capabilities && engineProfile.capabilities.supportsPrimePrompt) {
    ...
    const paste = async (readiness) => {
      ...
      try {
        ...
        let watched = await _observePasteRejected(tmuxName, engineId);
        for (let attempt = 1; attempt <= PASTE_MAX_RETRIES && watched.observed === 'rejected'; attempt++) {
          if (!tmux.probeSession(tmuxName).live) break;
          // Do not paste over a turn in flight. The profile declares a busy
          // marker precisely so nothing injects mid-turn; a re-paste that
          // ignores it would interleave with whatever the engine is doing.
          const busy = _paneIsBusy(tmuxName, engineId);
          if (busy !== false) {
            watched = { observed: 'rejected', reason: `${watched.reason}; a retry was due but the pane was ${busy === true ? 'busy' : 'not readable as idle'}` };
            break;
          }
          ...
          tmux.sendKeys(tmuxName, primeText, { enter: true });
          watched = await _observePasteRejected(tmuxName, engineId);
        }
        if (watched.observed !== 'rejected') {
          // A declared watch that could not answer must not vanish: the row
          // still takes the gate's verdict (unchanged behaviour), but the fact
          // that nothing observed the send is said out loud rather than left in
