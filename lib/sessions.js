...
async function _runClaimedWrap(project, active, options, runId, previousRun) {
  ...
  try {
    ...
    const sessionKept = pipelineResult.ok === true && Boolean(options) && options.keepSessionRunning === true;
    if (pipelineResult.ok && !sessionKept) {
    ...
    const result = {
      ...
      lifecycleCompleted,
      // True when the run finished and the operator's `keepSessionRunning`
      // left the session open on purpose.
      sessionKept,
      pipelineResult,
