...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  sections.push(...strandedWraps.primeSection(project));
 ...
 *   runner (`{skipTests, prHandling, skipAiContent, bumpLevel, proceedPastStranded, …}`).
 *   `proceedPastStranded` lists the stranded wraps (`{remote, branch, headSha}`)
 *   the operator chose to wrap past (#1540).
 * @returns {{ok: true, runId: string, sessionId: number, done: Promise<object>} | {ok: false, code?: string, items?: object[], sessionId: number|null, error: string}}
 *   `done` resolves to the run's `triggerWrap`-shaped outcome and never rejects.
 *   A refusal (no project, no active session, a run already in progress, or
