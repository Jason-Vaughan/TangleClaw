...
async function _triggerWrapV2(project, active, options) {
  ...
  try {
    ...
    if (pipelineResult.ok && pipelineResult.commitSha) {
      lifecycleCompleted = _completeV2Wrap(active, pipelineResult);
    }
...
function killSession(projectName, reason) {
  ...
  clearIdleCache(target.tmuxSession);
  if (!killedByThisCall) {
    // The pane and the listener are gone either way, which is what the operator
    // pressed the button for — but the row was ended by something else, so
    // saying "killed" here would put a second ending in the log for one death.
    log.warn('Kill tore down a session that had already ended', {
      project: projectName, session: target.id, reason
    });
    return { session: store.sessions.get(target.id), reconciled: true, error: null };
  }
