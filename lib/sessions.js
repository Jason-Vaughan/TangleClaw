 ...
 *
 * Never throws: a launch that cannot compute drift says it could not, which the
 * renderer and the gate both read as "no claim", rather than losing the session.
 * @param {object} project - Project record
 * @param {Array<{id: number, content: string}>} rules - The project rules this launch read
 * @param {object} [preflightResult] - From `launchPreflight.evaluate`; carries the handoff manifest
 * @returns {object|null} The drift, or null when nothing could be compared
 */
function _launchRuleDrift(project, rules, preflightResult) {
  const before = preflightResult && preflightResult.handoffManifest;
  if (!before) return null;
  try {
    const after = launchSequence.manifestFingerprints(project, rules);
...
function _buildLaunchSnapshot(launchId, project, engineProfile, applicability, startupRules, primeOptions, preflightResult, projConfig) {
  ...
  const ruleDrift = _launchRuleDrift(project, rules, preflightResult);
  try {
...
function _ruleDriftLines(drift) {
  ...
  if (drift.hasDrift) {
    ...
    lines.push('No rule, global rule or shared document changed since the handoff was written.');
  }

  // Said per source, never folded into the sentence above: a source nobody
  // recorded is not a source that did not change, and an agent told "no drift"
  // about shared documents nobody ever hashed has been told something false.
  const unrecorded = Object.entries(drift.perSource)
