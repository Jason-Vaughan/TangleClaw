...
function _buildLaunchCommand(engineProfile, project, launchMode) {
  ...
  if (launchMode && engineProfile.launchModes) {
    const modeConfig = engineProfile.launchModes[launchMode];
    if (!modeConfig) {
      log.warn('Unknown launch mode for engine — launching with engine defaults', {
        engine: engineProfile.id,
        launchMode,
        available: Object.keys(engineProfile.launchModes).join(',')
      });
    } else if (modeConfig.args && modeConfig.args.length > 0) {
      cmd += ' ' + modeConfig.args.join(' ');
    }
  }
