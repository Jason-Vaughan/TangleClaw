...
function launchSession(projectName, options = {}) {
  ...
  _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText, options.launchMode || null);
...
function generatePrimePrompt(project, engineProfile) {
  ...
  // Project Version Recording — engine-agnostic, always included
  sections.push('## Project Version Recording');
  sections.push('');
  sections.push('At the start of every session, determine the project\'s current version and write it to `.tangleclaw/project-version.txt` so TangleClaw can display it.');
  sections.push('');
  sections.push('Look for the version in this order:');
  sections.push('1. The latest released entry in `CHANGELOG.md` (skip `[Unreleased]`)');
  sections.push('2. A `version.json` or `package.json` at the project root');
  sections.push('3. The latest git tag (`git describe --tags --abbrev=0`)');
  sections.push('4. Use `0.0.0-dev` as a placeholder if none of the above exist');
  sections.push('');
  sections.push('Write the file in this exact format (overwrite any existing):');
  sections.push('```');
  sections.push('version: <X.Y.Z>');
  sections.push('recorded_at: <ISO-8601 UTC timestamp>');
  sections.push('source: <where you got it from>');
  sections.push('```');
  sections.push('');
...
function triggerWrap(projectName) {
  ...
  // Engine-agnostic: always re-record project version on wrap
  fullCommand += '\n\nBefore completing the wrap, re-check the project version (it may have changed during this session) and rewrite `.tangleclaw/project-version.txt` with the current value using the same format as the session-start recording.';
 ...
 * preKeys (defined on engineProfile.launch). This allows specific modes (e.g.
 * Bypass with its "press 2 to continue" confirmation) to define their own
 * startup key sequence without affecting other modes.
 *
 ...
 */
function _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText, launchMode) {
  let delay = 0;
  ...
  // Phase 2: prime prompt injection
