 ...
 *
 * The ordering half is here, in the FIRST step, because two later directives
 * each claim the agent's first turn: the banner block claims the first visible
 * reply, and the Resume section claims the first visible *message*. A pull
 * serves Resume FOURTH, so its claim arrives after the agent has necessarily
 * already spoken — unsatisfiable on arrival. Read next to a wait-for-
...
const LAUNCH_BOOTSTRAP_LINES = [
  ...
  '1. Emit the banner line this prime requires, and say in one line that you are reading your '
    + 'launch context. Keep it short — the context has not arrived yet, so there is nothing to '
    + 'report beyond that.',
  '2. Run the sequence to the end: `tc start next` for each step, acknowledging each one with the '
    + 'command that step prints.',
  '3. Do the read-only freshness checks the task step asks for, then attest with `tc start ready`.',
  '4. Propose the project action, and stop there.',
  'Steps 1-3 are routine initialization and are already authorized. Do not ask the operator to '
    + 'approve them, and do not read a wait-for-confirmation rule anywhere in this context as '
    + 'gating them — those rules gate step 4. Reading and attesting record only that the context '
    + 'arrived: they never authorize executing the recorded next action, editing another project, '
    + 'or dispatching work nobody asked for, and an explicit preflight, recovery or '
