...
function launchSession(projectName, options = {}) {
  ...
  _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText);
 ...
 */
function _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText) {
  let delay = 0;

  // Phase 1: preKeys (dismiss trust dialogs, update prompts, etc.)
  if (engineProfile.launch && engineProfile.launch.preKeys && engineProfile.launch.preKeys.length > 0) {
    ...
    for (let i = 0; i < engineProfile.launch.preKeys.length; i++) {
      const key = engineProfile.launch.preKeys[i];
    ...
    delay += engineProfile.launch.preKeys.length * 500;
  }
