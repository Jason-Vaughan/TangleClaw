...
function launchSession(projectName, options = {}) {
  ...
  // Parity with the launch-mode drop below (#741): a configured value this
  // engine cannot honor is a dropped preference, and a dropped preference that
  // says nothing reads as "Codex sessions behave differently" rather than
  // "your setting was discarded". Same asymmetric-capability-gate shape as
  // ADR 0001. Warned, not failed: the prime still arrives, by paste.
  if (projConfig.silentPrime === true && !silentPrime) {
    log.warn('Configured silentPrime is not usable for this engine — the prime will be pasted into the pane instead', {
      project: projectName,
      engine: baseEngineId,
      setting: 'silentPrime',
      reason: `engine ${engineId} does not declare capabilities.supportsSilentPrime`
    });
  }
