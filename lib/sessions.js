...
function launchSession(projectName, options = {}) {
  ...
  // the floor must not silently override it. Known accepted risk: a pane shell
  // that re-exports PATH from its rc files can drop the prepend — the live
  // Antigravity probe this chunk owes is exactly the test of that.
  const ambientEnv = {
    PATH: `${path.join(__dirname, '..', 'bin')}:${process.env.PATH || ''}`,
    TANGLECLAW_API: _apiOrigin(),
...
/**
 * The origin a session should call for its own switchboard endpoints.
 *
 * The prime used to say the base URL was "in your project guide". It is not,
 * and for a plugin-governed project TangleClaw does not write that guide at
 * all, so the pointer dangled (#1020) — while the one concrete base URL in the
 * prime is MEDUSA's `:3009`, from the embedded consumer contract, which is a
 * different server. A session following the instruction faithfully got a
 * connection refused or a 404.
 *
 * Same helper pair the generated engine configs use, for the same reason: read
 * what the server ACTUALLY serves rather than config intent.
...
function _apiOrigin() {
  try {
    ...
    log.warn('Could not resolve the API origin for the prime', { error: err.message });
    return 'the TangleClaw API base URL in your project guide';
  }
}
