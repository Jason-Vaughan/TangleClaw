...
function launchSession(projectName, options = {}) {
  ...
  if (options.primePrompt !== false) {
    primeText = generatePrimePrompt(project, engineProfile, { medusaWorkspaceId, startupRules, continuityMode: options.continuityMode, operatorHost: options.operatorHost, healReport: heal.report });
  }
...
async function launchWebuiSession(projectName, conn, engineId, engineProfile, project, options = {}) {
  ...
  const baseline = launchBaseline.capture(project.path);
  // Same heal as the tmux path (#1511); this path has no prime, so the result is logged only.
  projectHeal.healOnLaunch(project.path);
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  if (options.healReport) {
    sections.push(options.healReport);
    sections.push('');
  }
