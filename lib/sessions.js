...
async function _triggerWrapPipeline(project, active, options) {
  ...
  try {
    pipelineResult = await wrapPipeline.runWrapPipeline(project.name, {
      ...
      // becomes the wrap commit's subject and the continuity record, so it may
      // only ever come from this server's own record of a previous run, never
      // from a request body. Always set, so a body-supplied value is replaced.
      resumeFrom: wrapPipeline.resumableContentResults(previousRun, {
        sessionId: active.id, now: Date.now()
      })
    });
  } catch (err) {
