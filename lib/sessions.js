...
function launchSession(projectName, options = {}) {
  ...
  if (options.operatorHost) {
    ambientEnv.TC_OPERATOR_HOST = options.operatorHost;
  }
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  if (options.operatorHost) {
    sections.push(`> [!NOTE]`);
    sections.push(`> **Operator Topology:** The operator is accessing this session remotely via \`${options.operatorHost}\`.`);
    sections.push(`> When generating clickable URLs for the operator (like the Porthub registry, shared docs, or local servers), use this host instead of hardcoding \`localhost\`.`);
    sections.push('');
  }
