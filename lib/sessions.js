...
function _ruleDriftLines(drift) {
  ...
  if (drift.hasDrift) {
    ...
    lines.push('');
  } else if (drift.comparedSources && drift.comparedSources.length > 0) {
    // Named, never blanket. The old wording spoke for all three sources
    // whenever `hasDrift` was false — including the first launch after this
    // ships for every project, where two of the three were never compared —
    // and the next line then retracted it. A sentence that speaks only for
    // what was compared cannot need retracting.
    lines.push(`Nothing changed in ${drift.comparedSources.join(', ')} since the handoff was written.`);
  }
  ...
  const unrecorded = bySource('not-recorded');
  ...
  const unreadable = bySource('unreadable');
  if (unreadable.length > 0) {
    lines.push(`Not compared — this launch could not read ${unreadable.join(', ')}, so drift in them is unknown even though the handoff recorded them. The server log names what failed.`);
  }
  return [...lines, ''];
}
