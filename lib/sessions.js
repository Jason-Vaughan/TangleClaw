...
function launchSession(projectName, options = {}) {
  ...
  if (primeText && engineProfile.capabilities && engineProfile.capabilities.supportsPrimePrompt) {
    try {
      const startupDelay = (engineProfile.launch && engineProfile.launch.startupDelay) || 500;
      _sleep(startupDelay);
...
function generatePrimePrompt(project, engineProfile) {
  ...
  // Methodology playbook (full operational procedures)
  if (template) {
    const playbook = store.templates.getPlaybook(project.methodology);
    if (playbook) {
