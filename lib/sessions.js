...
function launchSession(projectName, options = {}) {
  ...
  const requestedMode = options.launchMode || engineProfile.defaultLaunchMode || null;
  const effectiveMode = requestedMode && !engines.honorsLaunchMode(engineProfile, requestedMode)
    ? (engines.honorsLaunchMode(engineProfile, 'default') ? 'default' : null)
    : requestedMode;
  ...
  _deferEngineInit(tmuxName, projectName, engineId, engineProfile, primeText, options.launchMode || null, silentPrime, deliveryBase);
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  const maxChars = PRIME_MAX_TOKENS * 4; // rough token-to-char ratio
