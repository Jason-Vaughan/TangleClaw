 ...
 */
async function _triggerWrapPipeline(project, active, options) {
  ...
  // between the pipeline returning and that call — `_completePipelineWrap` tearing
  ...
  try {
    ...
    // #139 Chunk 11a — session-lifecycle transition. A wrap that
    ...
    if (pipelineResult.ok && pipelineResult.commitSha) {
      lifecycleCompleted = _completePipelineWrap(active, pipelineResult);
    }
    ...
    const result = {
      ...
      // caller and to a test; `server.js#_wrapResultPayload` deliberately does
      // not forward it, keeping the HTTP response shape unchanged.
      lifecycleCompleted,
      pipelineResult,
...
/**
 * Run the pipeline wrap teardown — symmetric with `completeWrap` minus
 * `_autoCommitIfDirty` (the pipeline's `commit` step already flushed staged
 ...
 * @param {object} pipelineResult - Output of `runWrapPipeline`
 * @returns {boolean} Whether THIS call recorded the wrap. False when the
 *   session had already ended — the teardown still ran, but no wrap was written.
 */
function _completePipelineWrap(active, pipelineResult) {
  ...
  let wrapped = false;
  try {
    wrapped = store.sessions.wrap(active.id, summary) !== null;
    if (!wrapped) {
      log.warn('The wrap pipeline finished, but the session had already ended — '
        + 'no wrap was recorded', { session: active.id });
    }
  } catch (err) {
    log.warn('store.sessions.wrap failed in the wrap lifecycle', { session: active.id, error: err.message });
  }
  ...
  if (active.tmuxSession) {
    try {
      ...
      log.warn('Failed to kill tmux session during wrap teardown', { session: active.id, error: err.message });
    }
    clearIdleCache(active.tmuxSession);
  }
  ...
  try {
    ...
    log.warn('Failed to tear down Medusa on wrap', { session: active.id, error: err.message });
  }

  try {
    const released = store.documentLocks.releaseBySession(active.id);
    if (released > 0) {
      log.info('Released document locks on wrap', { session: active.id, count: released });
    }
  } catch (err) {
    log.warn('Failed to release document locks on wrap', { session: active.id, error: err.message });
  }
 ...
 *   blocked step.
 * @returns {Promise<{ ok: boolean, runId: string|null, sessionId: number|null, wrapCommand: string|null, wrapSteps: string[], captureFields: string[], lifecycleCompleted?: boolean, pipelineResult?: object, error: string|null }>}
...
async function triggerWrap(projectName, options) {
  ...
  return _triggerWrapPipeline(project, active, options);
}
