...
function startWrap(projectName, options) {
  ...
  const started = _startWrapPipeline(target.project, target.active, intent.options);
...
function _resolveWrapIntent(project, active, options) {
  ...
  return {
    ok: true,
    options: { ...rest, keepSessionRunning: resolved.keep, keepSource: resolved.source },
    planned: { sessionOutcomePlanned: resolved.keep ? 'keep' : 'end', keepSource: resolved.source }
  };
}
