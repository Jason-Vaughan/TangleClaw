...
function getSessionStatus(projectName) {
  ...
  if (wrapping) {
    ...
    // saying so is the whole of this function's job (#910).
    //
    // This branch used to call `autoCompleteWrap`, which writes the wrap
    // complete, tears down the Medusa listener, and runs a real `git commit` in
    // the operator's repository. The session page POLLS this endpoint, so
    // reading a status finalized a wrap and wrote to the operator's git history;
    // nothing about the request said mutate. #908 made the trigger honest — only
    // a probe that ANSWERED may act — which was correct and left the class
    // standing, one bad predicate away from returning.
    //
    // Completion is an action, so an action performs it: the session page turns
    // this observation into a `POST /api/sessions/:project/wrap/complete`, and
    // the launch path below still claims a stale row on its own (#105), so a
    // wrapping row can never become unrecoverable just because nobody is
    // watching the page.
    return {
      active: false,
      // The row still says wrapping, because nothing has finalized it. Reporting
      // `false` here would be this function deciding the outcome by describing
      // it — the same conflation in a quieter form.
      wrapping: true,
      // The observation itself: the pane is gone, so the wrap appears finished.
      // Named as an appearance rather than a result, because whether it IS
      // finished is settled by the finalizer, not by the reader.
      wrapFinished: true,
      sessionId: wrapping.id,
      project: projectName,
      engine: wrapping.engineId,
      ...
      incomplete: [],
      cause: null,
      // The pane is gone, so there is no idle signal to read — and `false` is a
      // plausible default in the one function whose subject is not producing
      // plausible defaults (architecture.md Direction).
      idle: null,
      lastOutputAge: null
    };
  }
...
function completeWrap(projectName, summary) {
  ...
  // Recover the summary from the pane cache when the caller supplies none.
  //
  // This is what makes finalization an ACTION without losing anything (#910).
  // The dead-pane summary used to be parsed inside `getSessionStatus`, which is
  // why a read finalized wraps at all; moving the finalize to an explicit POST
  // has to bring the parse with it, or the same wrap records an empty summary
  // and the wrap commit subject goes blank. The cache is populated while the
  // pane is alive and consumed here, so a caller that DOES pass a summary — the
  // operator finalizing a still-live wrap from the idle modal — keeps overriding
  // it, unchanged.
