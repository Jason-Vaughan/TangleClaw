 ...
 * hard-coded `CLAUDE.md`; plugin governance is reported only where
 * `governanceState` says it applies, and says so honestly when it cannot
 * apply (a non-Claude engine has no plugin channel).
 *
...
function _ruleSourcesSection(project, engineProfile, startupRules, viaStartupHook) {
  ...
  const sources = [
    `TangleClaw global rules — \`data/global-rules.md\`, carried in the managed block of ${configFile} (every managed project).`,
    ruleCount > 0
  ...
  if (governance === 'governed-plugin') {
    sources.push('Plugin methodology (prawduct) — its own session-start channel; owns merge strategy, attribution, branch conventions and test requirements.');
  } else if (governance === 'governed-vendored') {
    sources.push('Vendored methodology hook (`tools/product-hook`) — its own session-start channel.');
  }
  return [
...
function generatePrimePrompt(project, engineProfile, options = {}) {
  ...
  // disagree — the `--squash` / merge-commit fork lived for two months across
  // hundreds of sessions that way. Naming the sources is the tractable half of
  // "detect contradictions"; the other half is the boundary guard on
  // `data/global-rules.md` (test/global-rules-boundary.test.js).
  // Part of the non-yielding floor: "how many authorities, and what to do
  // when they disagree" is a directive, not lookup bulk, and the section is
 ...
 */
function _primePasteOutcome(readiness) {
  ...
  if (primeText && !silentPrime && engineProfile.capabilities && engineProfile.capabilities.supportsPrimePrompt) {
    ...
    const paste = (readiness) => {
      ...
      try {
        ...
        if (deliveryBase) {
          _recordRuleDelivery({ ...deliveryBase, channel: 'prime-paste', ..._primePasteOutcome(readiness) });
        }
      } catch (err) {
