...
function getSessionStatus(projectName) {
  ...
  if (active) {
    ...
    if (probe && probe.answered && !probe.live) {
      ...
      if (probe && probe.live) {
        const idleInfo = detectIdle(active.tmuxSession, active.engine);
        idle = idleInfo.idle;
  ...
  if (wrapping) {
    ...
    if (wrapProbe.live) {
      ...
      const idleInfo = detectIdle(wrapping.tmuxSession, wrapping.engine);
 ...
 * @param {string} [engineId] - Engine ID for accurate Medusa at-rest detection
 * @returns {{ idle: boolean, lastOutputAge: number }}
 */
function detectIdle(tmuxSession, engineId) {
  try {
    const capture = tmux.capturePane(tmuxSession, { lines: 15 });
    ...
    if (engineId && medusaWake.ENGINE_WAKE_PROFILES[engineId]) {
      const profile = medusaWake.ENGINE_WAKE_PROFILES[engineId];
      const cursor = tmux.cursorInfo(tmuxSession);
      const assessment = medusaWake._assessPane(capture.lines, profile, cursor);
      isIdle = assessment.idle;
    } else {
      // Fallback for legacy engines without a wake profile
    ...
    // Always update cache for the fallback branch
    if (!cached || cached.output !== currentOutput) {
      _lastOutput.set(tmuxSession, { output: currentOutput, timestamp: now });
    }

    const age = cached ? Math.floor((now - cached.timestamp) / 1000) : 0;
    return { idle: isIdle, lastOutputAge: age };
  } catch {
    return { idle: false, lastOutputAge: 0 };
  }
}
