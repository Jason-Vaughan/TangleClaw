 ...
 * @param {string|null} keyRef - Reference, e.g. `file:~/.config/tangleclaw/x.key` or `env:OPENAI_API_KEY`.
...
/**
 * Assert that an orchestration-profile endpoint honors the OpenAI-compatible
 * contract (TB-4 #359).
 *
 * The TB orchestration contract PINS one request shape for every profile
 * endpoint: the harness POSTs `{baseUrl}/chat/completions` with an OpenAI-style
 * JSON body, and TC injects ONLY the three generic OpenAI knobs
 * (`OPENAI_API_BASE`, `OPENAI_API_KEY`, `--model`) — nothing engine-specific ever
 * leaks into the launch (see applyLaunchOverlay). That makes "every endpoint is
 * OpenAI-compat" a checked guarantee rather than a coincidence: a future
 * LangGraph endpoint (semantic-route, Monad #35) drops into the same profile
 * mechanism unchanged, and swapping a profile's baseUrl between two OpenAI-compat
 * endpoints requires no harness change.
 *
 * The invariant TC enforces at resolve time is that the endpoint is a well-formed
 * absolute http(s) base URL the harness can append `/chat/completions` to. The
 * `/v1` suffix is the near-universal OpenAI-compat convention (LiteLLM, vLLM,
 * OpenRouter, …) but is deliberately NOT hard-required here — refusing on it would
 * false-reject a valid compat server mounted at a different base path.
 *
 * @param {*} baseUrl - The profile's endpoint base URL.
 * @returns {{ok: true}|{ok: false, reason: string}} `ok`, or a typed reason it is not compat.
 */
function assertOpenAICompatEndpoint(baseUrl) {
  ...
  try {
    ...
    return { ok: false, reason: `baseUrl is not a valid URL: ${baseUrl}` };
  }
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    return { ok: false, reason: `baseUrl scheme must be http(s), got "${url.protocol}"` };
  }
  if (!url.hostname) {
...
function resolveLaunchProfile(project, projConfig, profilesConfig, deps = {}) {
  ...
  // TB-4 (#359) — pin the OpenAI-compat guarantee. A bound endpoint that is not a
  // well-formed OpenAI-compatible base URL refuses to inject (honest degradation,
  // no silent fallback), so every endpoint TC actually injects is OpenAI-compat by
  // construction. Distinct from the null-baseUrl case above (deliberately
  // not-yet-landed) — this catches a misconfigured/non-compat endpoint.
  const compat = assertOpenAICompatEndpoint(profile.baseUrl);
  if (!compat.ok) {
    return {
      refused: true,
      reason: `profile "${profileName}" baseUrl is not OpenAI-compat: ${compat.reason}`,
      profileName
    };
  }
  if (!profile.model) {
