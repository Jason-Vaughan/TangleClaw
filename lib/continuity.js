 ...
 * @returns {{currentState:string, nextAction:string, map:string, freshness:{sha:string, branch:string, writtenAt:string}}}
 */
 ...
 *
 * @param {object} fields
 * @param {object} [fields.meta] - Frontmatter: session, date, project, methodology, harness, branch,
 *   sha, tags, type, files. `type` + `files` (CC-5) feed the operator-search type / file-touched
 *   filters; `files` accepts a comma-string or an array (normalized to a comma-joined string).
...
function renderWrapSummary(fields = {}) {
  ...
  const metaKeys = ['session', 'date', 'project', 'methodology', 'harness', 'branch', 'sha', 'tags', 'type', 'files'];
  const lines = ['---'];
...
/**
 * Path-like token matcher for Map entries — same shape as the
 * `features-toc` tokenizer so an entry written `\`lib/foo.js:42\`` or
 * `lib/foo.js (added)` both register the file. Used to decide which
 * entries a deletion prunes.
 */
const MAP_PATH_TOKEN_RE = /([A-Za-z0-9_./-]+\.(?:js|jsx|ts|tsx|json|md|html|css|yaml|yml|sh))(?:\b|:)/gi;
...
function listSessions(projectPath) {
  ...
  arr.sort((a, b) => {
    const da = a.date || '';
    const db = b.date || '';
    if (da !== db) return db.localeCompare(da); // recency desc; '' (undated) sorts last
...
function searchSessions(projectPath, query, opts = {}) {
  ...
  return {
    ...
    meta: {
      ...
      // Forward-only fields: how many sessions can never match a type/file filter
...
async function searchProjectTranscripts(projectPath, query, opts = {}) {
  ...
  let results = [];
  if (q) {
    const searched = await Promise.all(candidates.map((s) =>
  ...
  return {
    ...
    meta: {
      ...
      returned: limited.length,
      unindexed: {
        type: all.filter((s) => !s.type).length,
        file: all.filter((s) => !s.files.length).length
      }
    }
  };
}
