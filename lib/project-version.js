 ...
 *   5. pyproject.toml — static `[project] version = "X.Y.Z"` (#1444)
 *   6. git tag — `git describe --tags --abbrev=0`
 *   7. fallback — `0.0.0-dev`
 *
...
function detectVersion(projectPath) {
  ...
  // version.json → package.json → pyproject.toml, shared with the self-heal
  // ladder so the two cannot drift, and stopping where the wrap's bump stops.
  const probed = versionFiles.readProbedVersion(projectPath);
  if (probed && probed.version) return { version: probed.version, source: probed.source };
