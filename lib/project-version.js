...
function _readConfiguredVersion(projectPath) {
  let cfg = null;
  try {
    // `./project-config`, NOT `./store`. The lazy require this replaces was safe
    // for the require CYCLE it was written for and unsafe for a reader that did
    // not exist yet: it loaded `lib/store.js` — and with it an open SQLite handle
    // — on first call, inside whatever process happened to be calling. The
    // scanner child (#884) is such a process, and it exists to be SIGKILLed. The
    // reader is pure `fs`, so it has its own module now and no cycle to break.
...
module.exports = {
  ...
  recordVersion,
  FALLBACK_VERSION,
  // Exported so `lib/projects.js`'s live-detection ladder reads a configured
  // `versionFilePath` through THIS implementation rather than growing a second
  // one. The two ladders legitimately differ in their tail (that one omits the
  // git-tag fallback on purpose), but they must not differ on where a project
  // says its version lives.
  _readConfiguredVersion,
  _readGitTagVersion,
  _formatCacheFile
};
