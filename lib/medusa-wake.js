...
function _judgeSession(session, st) {
  ...
  if (newest === st.lastNudgedKey) {
    ...
    // the watchdog has re-armed it because the nudge provably did not land or
    // went unanswered past its budgeted wait. A re-armed edge goes back through
    // every gate below; nothing here injects directly.
    let rearmed = false;
    try {
  ...
  Promise.resolve()
    ...
    .then((receipt) => {
      if (receipt && receipt.outcome === 'not-accepted') {
        _internal.recordWakeFacts(workspaceId, 'wake_not_accepted', { code: 'nonce-in-composer', detail: { nonce } });
      }
    })
    .catch((err) => {
