...
function _scanSession(session) {
  ...
  if (session.isMaster) {
    ...
    // Never type into a pane a wrap pipeline is driving. The at-prompt and
    // debounce gates below make a collision unlikely, not impossible: a wrap
    // pauses between steps, and a pane that looks at rest there is exactly the
    // shape this monitor acts on. The registry is the only thing that knows a
    // wrap is running — it is process-local module state in this same process,
    // and it is what replaced the session status that used to carry this.
    //
