 ...
 * observed, so a new gate that forgets to name itself fails loudly (an
 * undefined reason) instead of leaving the previous tick's verdict standing as
 * if it were current.
...
function _scanSession(session) {
  ...
  _noteVerdict(st, _judgeSession(session, st));
}
 ...
 *   reason: string, since: (string|null), observedAt: (string|null), monitorRunning: boolean}}
 */
function peerReachability(workspaceId) {
  ...
  const match = candidates.find((c) => _internal.getStatus(c.key).workspaceId === id)
    || candidates.find((c) => _internal.registeredWorkspaceId(c) === id);
  if (!id || !match) return { workspaceId: id, local: false };

  const st = _sessions.get(match.key);
  const verdict = st && st.verdict;
  return {
    workspaceId: id,
    local: true,
    reason: verdict ? verdict.reason : 'not-observed',
...
/**
 * The workspace id the registry holds for a scan candidate, or null. Never
 * throws: an unreadable registry is "no match here", which leaves the id
 * `local: false` unless a running listener claimed it first.
 * @param {{key: (string|number), record: (object|null)}} candidate - A live
 *   session record, or the Master (null record).
 * @returns {string|null}
 */
function _registeredWorkspaceId(candidate) {
  try {
    const registry = require('./medusa-registry');
    if (candidate.record === null) {
      const target = require('./master').masterMedusaTarget();
      return registry.getWorkspaceId(target.projectPath, target.sessionId);
    }
    const project = _internal.getProject(candidate.record.projectId);
    return project ? registry.getWorkspaceId(project.path, candidate.key) : null;
  } catch (err) {
    log.warn('medusa-wake: registry read failed while resolving a peer', { error: err.message });
    return null;
  }
}
