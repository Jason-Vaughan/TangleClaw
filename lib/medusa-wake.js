...
const ENGINE_WAKE_PROFILES = {
  ...
  antigravity: {
    ...
    // #1134 — watch the pane AFTER a prime paste and believe the transcript,
    // not the pre-send gate. Declared for antigravity because that is where the
    // discarding state was measured (1.1.22, sessions 899/900/901: the complete
    // at-rest UI rendered while the account was still verifying, and everything
    // submitted was dropped). An engine that has not been measured swallowing a
    // paste does not get a watch inferred for it — its paste records the honest
    // "not observed" reason and behaves exactly as before, which is the same
    // rule `idleMarker` follows.
    //
    // The tell is the REJECTION, not the landing. Landing was tried first and
    // does not work: a real prime is 37-225 lines on this machine, so once it
    // echoes and the engine answers, any part of it is far above a pane tail —
    // the check would read "not landed" on every healthy launch and re-paste
    // the whole prime into a session that already had it, which is worse than
    // the bug. `>` is no help either: this profile's own note records it
    // rendering mid-turn.
    //
    // `Please try again shortly` is quoted from the live #1134 capture (agy
    // 1.1.22, 2026-09-01, sessions 899/900/901) — the CLI's own words when it
    // discards a submission during account verification. NOT re-confirmed on
    // 1.1.24: that build's verify window could not be reproduced here (this
    // account is already verified), so this marker may be a no-op on newer
    // builds. That is the honest state and it is safe in the direction that
    // matters — an absent marker leaves the pre-existing verdict untouched and
    // never invents a retry.
...
function assessSessionIdle(opts) {
  ...
  const verdict = _assessPane(lines, profile, o.cursor === undefined ? null : o.cursor);
  if (!verdict.idle) {
  ...
  return { idle: idleTicks >= required, reason: 'at-prompt', digest, idleTicks };
}
...
function _scanSession(session) {
  ...
  if (!assessed.idle) {
    ...
    if (assessed.reason !== 'at-prompt') {
      record('skipped', 'none', assessed.reason === 'pane-writing' ? 'pane-writing' : `pane-${assessed.reason}`);
    }
    return;
  }
