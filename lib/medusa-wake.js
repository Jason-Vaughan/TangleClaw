...
const WAKE_FIELDS = {
  ...
  decorativePattern: { required: false, check: _compiles, expects: 'a string that compiles as a regex' }
};
...
function _composerEmpty(cursor, profile) {
  ...
  // A decorative cell is accepted as the separator too: an engine that
  // animates decoration can OVERWRITE its own pad, and reading that as typed
  // input is what kept codex unwakeable (#1344 — measured live as
  // `›⠁Ask Codex to do anything`, where the shimmer sits where the space was).
  const padded = pad && (sep === pad || sep === ' ' || _isDecorative(sep, profile));
