...
const WAKE_FIELDS = {
  ...
  decorativePattern: {
    ...
    // own half-written text. An earlier version of this check refused `.` and
    // `[\s\S]` but accepted `\S`, `\w` and `[a-z]`, which are exactly as
    // broad; enumerating bad patterns was the wrong shape.
    //
    // The rule instead: decoration must not match any printable ASCII, because
    // that is the range an operator types. Decoration lives outside it —
    ...
    expects: 'a regex matching decoration only — it must not match the empty string or any printable ASCII character'
  }
};
 ...
 *
 * Checked over every printable ASCII character rather than a list of known-bad
 * patterns: the question is what the regex ACCEPTS, and a blacklist only ever
 * covers the offenders someone thought of (#1344 — the first version refused
 * `.` but accepted `\S`). 95 tests, run once per engine at table build.
 *
 * @param {string} source - Regex source from `capabilities.wake.decorativePattern`.
 * @returns {boolean} True when it matches only non-typed characters.
 */
function _matchesNoTypedText(source) {
  const re = new RegExp(source);
  if (re.test('')) return false;
  const printable = [];
  for (let code = 0x20; code <= 0x7e; code++) printable.push(String.fromCharCode(code));
  for (const ch of printable) if (re.test(ch)) return false;
  // Also a run of typed text, not just single cells. `_isDecorative` only ever
  // tests one cell, but `_paneDigest` applies the pattern across a whole line,
  // where a multi-cell pattern such as `[a-z]{2}` would blank real text while
  // matching no single character.
  if (re.test(printable.join(''))) return false;
  return true;
}
...
function _buildWakeProfiles(profiles) {
  ...
  for (const profile of profiles) {
    ...
    table[profile.id] = {
      ...
      // Compiled here like `promptRe`, as TWO objects rather than one shared
      // global. A global regex carries `lastIndex` between calls, so a single
      // shared instance makes every `.test` caller responsible for resetting it
      // and silently returns alternating answers for anyone who forgets. The
      // non-global one is for single-cell tests, the global one for cleaning a
      // whole line in one pass; neither is stateful in the other's use.
      ...(block.decorativePattern === undefined
        ? {}
        : {
          decorativeRe: new RegExp(block.decorativePattern),
          decorativeReAll: new RegExp(block.decorativePattern, 'g')
        })
    };
  }
  return table;
...
function _paneDigest(lines, profile) {
  ...
  // Each decorative cell becomes a SPACE rather than being deleted. Deleting
  // shortens the line by however many cells the animation happens to be
  // drawing, so two frames of the same idle pane hash differently whenever the
  // decoration sits ahead of real text — which would let the permanent
  // `pane-writing` failure this exists to fix come straight back. Blanking
  // keeps every real character at its column, and the trailing trim below
  // absorbs a decorative tail.
  const decorative = profile && profile.decorativeReAll;
  const stripped = (lines || []).map((l) => {
    const text = _strip(l);
    return (decorative ? text.replace(decorative, ' ') : text).replace(/\s+$/, '');
  });
