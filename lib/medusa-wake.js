...
function _buildWakeProfiles(profiles) {
  ...
  for (const profile of profiles) {
    ...
    table[profile.id] = {
      ...
      // Compiled here like `promptRe`, and global so a line with several
      // decorative cells is cleaned in one pass.
      ...(block.decorativePattern === undefined
        ? {}
        : { decorativeRe: new RegExp(block.decorativePattern, 'g') })
    };
  }
  return table;
}
...
function _paneDigest(lines, profile) {
  ...
  const decorative = profile && profile.decorativeRe;
...
function _isDecorative(ch, profile) {
  ...
  // The compiled pattern is global, so `lastIndex` must not carry between tests.
  profile.decorativeRe.lastIndex = 0;
