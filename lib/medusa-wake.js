 ...
 * @type {Map<number, {idleTicks: number, lastNudgedKey: string|null, nudges: number, skipLogged: boolean}>}
 */
...
function _scanSession(session) {
  ...
  if (!st) {
    st = { idleTicks: 0, lastNudgedKey: null, nudges: 0, skipLogged: false };
  ...
  try {
    ...
    return; // unreadable config — treat as opted out this tick
  }
  if (!projConfig || projConfig.medusaWake !== true) return;

  // Listener + fresh-mail gates.
  const status = _internal.getStatus(sessionId);
  if (status.state !== 'listening' || status.unread === 0) {
