...
/** Built on first use: the template comes from `_nudgeLineFor` and `withNonce`. */
let _ownNudgeRe = null;
 ...
 *
 * The pattern is derived from `_nudgeLineFor` and `withNonce` rather than
 * restated, so a change to the nudge's wording cannot leave this behind. Every
 ...
 * base repeated in all three paths, and a 12-hex nonce. Whitespace is removed
 * from both sides before comparing, because the composer wraps the line
 * wherever its width falls, words included. Text before or after the nudge,
...
function isOwnNudge(text) {
  ...
  if (!_ownNudgeRe) {
    ...
    const template = wakeTransports.withNonce(_nudgeLineFor(S.base, S.unread, S.origin), S.nonce);
    ...
    const body = escaped
      ...
      .reduce((acc, part, i) => {
        if (i === 0) return part;
        const slot = seenBase ? '\\1' : '(/api/(?:sessions/[^/\\s]+|master)/medusa)';
        seenBase = true;
        return acc + slot + part;
      }, '');
    _ownNudgeRe = new RegExp(`^${body}$`);
  }
  return _ownNudgeRe.test(text.replace(/[\s ]+/g, ''));
}
...
function _judgeSession(session, st) {
  ...
  // record shows every unread tracked message already attempted, the edge is
  // already nudged, whatever this process remembers.
  if (newest !== st.lastNudgedKey) {
    try {
      if (_internal.alreadyAttempted(status.workspaceId, status.unread)) st.lastNudgedKey = newest;
    } catch (err) {
...
const _internal = {
  ...
  alreadyAttempted: (workspaceId, unread) => (workspaceId ? require('./medusa-exchanges').alreadyAttempted(workspaceId, unread) : false),
