...
function _judgeSession(session, st) {
  ...
  // What the engine's own protocol says, where it has a channel (#1628).
  // Asked only here — a live, opted-in session with fresh mail — so a quiet
  // fleet costs no socket traffic. With one, only a fresh `idle` goes on to
  // the pane gate (where it excuses the at-rest marker and nothing else):
  // `busy` holds whatever the pane shows, and anything unproven holds too
  // rather than being guessed from the pane. With none, an engine that is
  // MEANT to be judged by its channel holds as well: its pane marker has been
  // measured reading at rest on transcript prose that merely quotes it. Every
  // other engine keeps the pane gate below, as it always had.
  const engine = project ? _engineActivity(session, project, st) : { channel: 'absent' };
  if (engine.channel === 'absent' && _declaresObserver(session.engineId)) {
    st.idleTicks = 0;
    record('skipped', 'none', 'engine-channel-absent');
    return 'engine-channel-absent';
  }
  if (engine.channel === 'present' && engine.state !== 'idle') {
    const engineCode = engine.state === 'busy' ? 'engine-thread-busy' : 'engine-thread-unknown';
    st.idleTicks = 0;
  ...
  const assessed = assessSessionIdle({
    ...
    engineIdle: engine.channel === 'present'
  });
