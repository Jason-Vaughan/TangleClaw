...
const WAKE_FIELDS = {
  ...
  promptGlyph: { required: true, check: _isText, expects: 'a non-empty string' },
  promptPad: { required: true, check: (v) => v === null || _isText(v), expects: 'a non-empty string or null' },
  placeholderSgr: {
    required: true,
...
function _resolveWakeProfiles() {
  ...
  try {
    ...
    log.warn('medusa-wake: engine profiles unreadable — no session can be nudged until they are', {
      error: err.message
    });
    return {};
  }
...
// `server.js` runs AFTER requiring this module. Exposed as a property so all
// seven consumers keep writing `medusaWake.ENGINE_WAKE_PROFILES[engineId]` and
// pay the ordering no attention. The one thing that does NOT work is
