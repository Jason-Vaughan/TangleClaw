...
const WAKE_FIELDS = {
  ...
  decorativePattern: {
    required: false,
    // Not just 'compiles'. A pattern that also matches a space or an empty
    // string turns BOTH gates off silently: `_paneDigest` blanks every line to
    // nothing, so the pane always looks settled, and `_composerEmpty` discounts
    // every cell, so typed input reads as empty. The engine would then be
    // nudged mid-turn over the operator's own text. Same spirit as
    // `_isOneCell` above: refuse the declaration rather than ship a gate that
    // passes everything.
    check: (v) => _compiles(v) && !new RegExp(v).test(' ') && !new RegExp(v).test(''),
    expects: 'a regex that matches decoration only — it must not match a space or the empty string'
  }
};
...
function _composerEmpty(cursor, profile) {
  ...
  for (let i = cursor.x; i < cells.length; i++) {
    ...
    // the time). `decorativeRe` is global, so `lastIndex` is reset per test.
    if (_isDecorative(cells[i].ch, profile)) continue;
