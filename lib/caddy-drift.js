 ...
 *
 * A check that could NOT run still produces a notice. That is the point of
 * `not-measured`: on an install where a Caddyfile is expected, "TangleClaw did
 * not look" and "TangleClaw looked and found nothing" are different facts, and
 * collapsing them is the failure this check exists to avoid.
 *
 * @param {object} result - From `checkCaddyDrift`.
 * @returns {{setting: string, severity: string, message: string, findings: string[]}|null}
 *   Null when the check ran and every property holds.
 */
function describeDrift(result) {
  ...
  if (!result.measured) {
    return {
      setting: 'Caddyfile',
      severity: 'unknown',
      message: `TangleClaw could not check the live Caddyfile for drift: ${result.reason}`,
      findings: []
    };
  }
  ...
  return {
    ...
    message: `The live Caddyfile does not hold ${count} security `
      + `${count === 1 ? 'property' : 'properties'} TangleClaw would generate.`,
