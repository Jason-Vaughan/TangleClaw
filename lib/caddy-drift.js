 ...
 * @returns {{ status: string, findings: string[] }}
 */
function checkOffboxRefused(live) {
  const findings = [];
  for (const site of live.values()) {
...
function checkCaddyDrift(options = {}) {
  ...
  const offboxRefused = checkOffboxRefused(liveSummary.sites);
...
const PROPERTY_LABELS = {
  ...
  offboxRefused: 'every site with no gate refuses other machines'
};
...
/**
 * Work out whether, and how, the peer guard can be added to every ungated site
 * of an existing Caddyfile without changing anything else Caddy reads.
 *
 ...
 */
function planOffboxGuard(content, adaptContent = adaptCaddyfileContent) {
  const refuse = (reason) => ({
  ...
  if (checkOffboxRefused(summarizeConfig(before.config).sites).status === HOLDS) {
    return { status: GUARD_ALREADY, reason: null, content: null, guarded: [] };
  }

  let inserted;
  try {
    inserted = caddy.insertOffboxGuard(content);
  } catch (err) {
    return refuse(err.message);
  }
  if (inserted.guarded.length === 0) {
    return refuse(
      'a site proxies with no gate, but no block was found where the guard can be placed '
  ...
  const still = checkOffboxRefused(summarizeConfig(after.config).sites);
