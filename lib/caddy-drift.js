 ...
 *
 * **When TangleClaw's own gate guards the door** (`authGate.guardsTheDoor`:
 * `armed` or `locked`) the property holds without reading a single site.
 * TangleClaw is then the gate for every request that reaches it, whichever
 * Caddy site carried the request, so a live site with no `basic_auth` is not a
 * missing gate — and neither is a live file that still carries one. A site that
 * proxies to anything OTHER than TangleClaw is not this property's to judge:
 * TangleClaw's gate does not stand in front of it, and P3 reports it as an
 * upstream TangleClaw does not generate.
 *
 ...
 */
function checkGates(live, baseline, gateState = null) {
  if (authGate.guardsTheDoor(gateState)) return { status: HOLDS, findings: [] };
...
/**
 * Header names a `reverse_proxy` `headers.request` block touches, lower-cased.
 *
 * Caddy adapts `header_up Name value` to `set`, `+Name` to `add`, a search/
 * replace pair to `replace` (all keyed by name) and `-Name` to `delete` (a
 * list of names). Every one of them is a way to change what the upstream sees.
 *
 * @param {object|undefined} request - A handler's `headers.request`.
 * @returns {string[]}
 */
function requestHeaderNamesTouched(request) {
  if (!request || typeof request !== 'object') return [];
  const names = [];
  for (const op of ['set', 'add', 'replace']) {
    if (request[op] && typeof request[op] === 'object') names.push(...Object.keys(request[op]));
  }
  if (Array.isArray(request.delete)) names.push(...request.delete);
  return names.map((name) => String(name).toLowerCase());
}

/**
 * Walk every handler under a route list, into subroutes.
 * @param {Array} routes - Caddy JSON routes.
 * @param {(handler: object) => void} visit - Called for each handler.
 */
function eachHandler(routes, visit) {
  if (!Array.isArray(routes)) return;
  for (const route of routes) {
    for (const handler of (route && Array.isArray(route.handle)) ? route.handle : []) {
      if (!handler) continue;
      visit(handler);
      if (handler.handler === 'subroute') eachHandler(handler.routes, visit);
    }
  }
}

/**
 * P5 - nothing in the live file lets a client decide `X-Forwarded-For`.
 *
 * TangleClaw's fleet carve-out (`lib/auth-gate.js#isMachineClient`) reads "no
 * `X-Forwarded-For`" as "not forwarded by Caddy". That holds because Caddy's
 * `reverse_proxy` sets the header on every request it forwards and replaces a
 * client's value — verified against Caddy v2.11.4. Two settings end it:
 *   - `trusted_proxies`, on the server or on a `reverse_proxy`, which makes
 *     Caddy keep the value a peer in that range sent;
 *   - a `header_up` that sets, adds, rewrites or deletes `X-Forwarded-For`,
 *     which is how an off-box request could arrive without the header and look
 *     like a local process.
 * The generator emits neither, so this is an absolute and not a diff: any
 * occurrence in the live file is a divergence. Read in every gate state — the
 * carve-out applies whenever the gate enforces.
 *
 * `request_header` (the `headers` handler) is deliberately not flagged: it runs
 * before the proxy, which then overwrites the value for an untrusted peer, so
 * it cannot remove the fingerprint. Flagging it would be a false positive this
 * check exists to avoid.
 *
 * @param {object} adapted - The LIVE file's `caddy adapt` JSON.
 * @returns {{ status: string, findings: string[] }}
 */
function checkForwardedFor(adapted) {
  const servers = (adapted && adapted.apps && adapted.apps.http && adapted.apps.http.servers) || {};
  const findings = [];
  for (const server of Object.values(servers)) {
    const listen = Array.isArray(server.listen) ? [...server.listen].sort().join(',') : '?';
    if (server.trusted_proxies) {
      findings.push(`the server on ${listen} sets trusted_proxies, so Caddy keeps a client's X-Forwarded-For`);
    }
    eachHandler(server.routes, (handler) => {
      if (handler.handler !== 'reverse_proxy') return;
      const upstream = (handler.upstreams || []).map((u) => u && u.dial).filter(Boolean).join(', ') || '?';
      if (handler.trusted_proxies) {
        findings.push(
          `a reverse_proxy on ${listen} to ${upstream} sets trusted_proxies, so Caddy keeps a client's X-Forwarded-For`
        );
      }
      const request = handler.headers && handler.headers.request;
      if (requestHeaderNamesTouched(request).includes('x-forwarded-for')) {
        findings.push(`a reverse_proxy on ${listen} to ${upstream} rewrites X-Forwarded-For with header_up`);
      }
    });
  }
  return { status: findings.length ? DIVERGED : HOLDS, findings: [...new Set(findings)] };
}
...
/**
 * P6 - every site that proxies with no gate refuses every peer but this machine.
 *
 * Absolute, not a diff against the baseline: the generator never writes a site
 * that proxies to TangleClaw with none of `basic_auth`, the peer guard, or
 * TangleClaw's own gate guarding the door, so there is no config under which an
 * ungated site answering other machines is intended.
 *
 * **When TangleClaw's own gate guards the door** (`authGate.guardsTheDoor`) the
 * property holds without reading a site, for P1's reason: TangleClaw is then the
 * gate for every request that reaches it, and its sites must answer other
 * machines — the login is what admits them.
 *
 ...
 * @returns {{ status: string, findings: string[] }}
 */
function checkOffboxRefused(live, tangleclawUpstreams, gateState = null) {
  if (authGate.guardsTheDoor(gateState)) return { status: HOLDS, findings: [] };
 ...
 *
 * The gate state goes to the generator too, so the baseline carries
 * `basic_auth` exactly when a cutover would write it.
 *
 ...
 */
function buildBaseline(config, liveContent, gateState = null) {
 ...
 *   (`lib/auth-gate.js#resolveGateState`). Omitted, the baseline keeps
 *   `basic_auth` whenever config carries a credential — the generator's own
 *   fail-safe — so an armed install's missing `basic_auth` would read as drift.
...
function checkCaddyDrift(options = {}) {
  ...
  const gateState = options.gateState === undefined ? null : options.gateState;
  ...
  const baseline = buildBaseline(config, liveContent, gateState);
  if (!baseline.ok) {
  ...
  const gatedProxies = checkGates(liveSummary.sites, baseSummary.sites, gateState);
  ...
  const forwardedFor = checkForwardedFor(liveAdapted.config);
  const offboxRefused = checkOffboxRefused(
    liveSummary.sites, upstreamsOf(baseSummary.sites), gateState
  );

  const properties = {
    gatedProxies, httpsProtocols, knownUpstreams, leaseReach, forwardedFor, offboxRefused
  };
  const findings = [];
...
const PROPERTY_LABELS = {
  ...
  forwardedFor: 'no Caddy setting lets a client decide X-Forwarded-For',
 ...
 *
 * Also refuses when TangleClaw's own gate guards the door
 * (`authGate.guardsTheDoor`): those sites carry no `basic_auth` on purpose and
 * must answer other machines, so a peer guard would lock the operator out of
 * their own install from anywhere but the box.
 *
 * @param {string} content - The live Caddyfile text.
 ...
 */
function planOffboxGuard(content, serverPort, adaptContent = adaptCaddyfileContent, gateState = null) {
  const refuse = (reason) => ({
  ...
  if (authGate.guardsTheDoor(gateState)) {
    return refuse(
      'TangleClaw\'s own login guards this install, so its sites answer other machines on purpose; '
      + 'a peer guard would lock you out of it from anywhere but this machine'
    );
  }
