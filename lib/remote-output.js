 ...
 *
 * Scope is the PROPERTY, not a directory: every caller that builds text from a
 * remote-touching `git`/`gh` failure, wherever it lives. This module started
 * under `lib/wrap-steps/` and the guarantee was written as repo-wide while the
 * sweep behind it had only covered that one glob — five callers outside it
 * (`wrap-pr-status`, `ci-status`, `update-checker`, `update-applier`,
 * `behind-origin`) were still unredacted, two of them logging. A module whose
 * contract is a property has to sit where every holder of that property can
 * reach it.
 *
 * Local `git` (`status`, `add`, `checkout`, `commit`, `rev-parse`) stays out:
 * it reaches no network and carries no credential, and routing it through here
 * would say those strings are equally dangerous when they are not.
 */
 ...
 * are diagnostics, not archives — the first line is what anyone reads, and a
 * rare row type can sit in its table indefinitely, so an unbounded remote error
 * grows storage with text nobody looks at.
 */
const MAX_CHARS = 200;

/**
 * Opening of the string {@link redactRemoteOutput} substitutes when it replaces
 * text wholesale. Exported so a caller can tell "the command said nothing" from
 * "the command said something we could not show", which are different problems
 * and want different follow-up.
 */
...
function redactRemoteOutput(text) {
  ...
  if (scan.flagged) {
    return `${REDACTED_PREFIX}${scan.types.join(', ')} detected in remote error output]`;
  }
...
function reasonFromFailure(res) {
  const detail = detailFromFailure(res);
  if (!detail) return `exit ${res ? res.exitCode : 'unknown'}`;
  // A wholesale replacement takes the exit code away with the secret. At these
  // call sites the redacted text is the ENTIRE explanation — `commit.js` keeps
  // its own `(exit N)` prefix, but a standalone reason that reads only
  // "[redacted — …]" tells the operator nothing about what failed, and the exit
  // code is not a credential.
  if (detail.startsWith(REDACTED_PREFIX)) return `${detail} (exit ${res ? res.exitCode : 'unknown'})`;
  return detail;
}

/**
 * A remote URL with its userinfo removed, for keeping the URL itself — as a
 * stored key, or in an API response — rather than error text.
 *
 * Same pattern as step 1 of {@link redactRemoteOutput}, and for the same
 * reasons, but the userinfo is removed instead of masked: the result is a
 * repository identity, and `https://***@host/x.git` would be a different
 * identity for the same repository. No scan or length cap, because the input is
 * a URL git reported for `origin`, not free text.
 *
 * @param {string|null|undefined} url - Remote URL as git reports it
 * @returns {string|null} The URL without credentials; null when absent or blank
 */
...
module.exports = { redactRemoteOutput, detailFromFailure, reasonFromFailure, stripRemoteCredentials, MAX_CHARS, REDACTED_PREFIX };
