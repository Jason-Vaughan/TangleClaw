 ...
 * @returns {{supported: boolean, reason: string, adapter: (object|null), block: (object|null)}}
 */
function resolve(profile, adapters = ADAPTERS) {
  ...
  if (declared === undefined) {
    return { supported: false, reason: `engine ${engine} declares no startupControl channel`, adapter: null, block: null };
  }
  ...
  if (errors.length > 0) {
    ...
    return { supported: false, reason: `engine ${engine} declares a malformed startupControl block`, adapter: null, block: null };
  }
  ...
  return { supported: true, reason: `adapter "${declared.adapter}"`, adapter, block: declared };
}
