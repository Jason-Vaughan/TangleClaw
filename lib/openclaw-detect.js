...
// Shape guards for the SSH-command-interpolated fields (defense-in-depth: these
// arrive from the Add/Edit form before any connection record exists).
const SAFE_HOST = /^[A-Za-z0-9._:-]+$/; // hostname / IPv4 / IPv6 / tailscale name
...
/**
 * Whether the SSH-target fields are all shape-safe to interpolate.
 * @param {object} conn - { host, sshUser, sshKeyPath }
 * @returns {string|null} An error reason, or null if all safe.
 */
function unsafeReason(conn) {
  if (!conn || !conn.host || !conn.sshUser || !conn.sshKeyPath) {
    return 'host, sshUser, and sshKeyPath are all required to detect the instance dir';
  }
