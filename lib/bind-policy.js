 ...
 *   that binding in a log line. `reason` is one of `'caddy'`, `'opt-in'`,
 *   `'grace'`, or `'default'`, and `grace` is true for the `'grace'` case — an
 *   install still bound wide because narrowing it would strand its operator. `refusedOptIn` is true when the operator asked for a wide bind
