 ...
 * wide — provided something shows it was used; a config written by hand before
 * the first boot lacks the key too, and is recorded as closed. That absence is a
 * one-shot signal — the next whole-object config save destroys it — so
 * `migrateLegacyBind()` converts it to an explicit value at boot (`null` for a
 * legacy install, `false` for a fresh one), and every later decision reads that
...
/**
 * Record, once, whether an install whose config lacks the key is a legacy install
 * that was bound wide (grace, `null`) or a fresh one that never was (`false`).
 *
 ...
 *
 * Absence alone is not enough to grant grace, though: a config written by hand
 * before the first boot lacks the key too, and granting it grace would listen on
 * every interface before any login exists. So grace needs evidence the install
 * was used. Without it the install is recorded as closed (`false`), because
 * there is no remote operator to strand (ADR 0009: an install that never had
 * remote reach narrows immediately). The evidence is:
 * - `setupComplete === false` means fresh. After `load()` that value can only come
 *   from a file that persists it, because `load()` reads a MISSING
 *   `setupComplete` as `true` for legacy installs that predate the field. That is
 *   also why "setupComplete missing" cannot be the fresh-install signal.
 * - `priorUse === false` means fresh: no project, session or user exists and no
 *   project was ever deleted.
 * - `priorUse` omitted keeps the legacy answer, so a caller that cannot say
 *   strands no one.
 *
 * @param {object} config - Global config (defaults-merged).
 * @param {boolean} keyPersisted - Whether the config FILE carries the key.
 * @param {boolean} [priorUse] - Whether the store shows the install was used
 *   (`store.hasPriorUse()`).
 * @returns {{migrated: boolean, reason: string, value: (null|false|undefined)}}
 *   `migrated` true means the caller must persist `config` — this function does
 *   not write. `reason` is `'legacy-direct-install'` (grace) or `'fresh-install'`
 *   (closed) when it migrated.
 */
function migrateLegacyBind(config, keyPersisted, priorUse) {
  const cfg = config || {};
  ...
  if (cfg.setupComplete === false || priorUse === false) {
    cfg[OPT_IN_KEY] = false;
    return { migrated: true, reason: 'fresh-install', value: false };
  }
  cfg[OPT_IN_KEY] = null;
