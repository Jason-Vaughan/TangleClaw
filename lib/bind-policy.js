 ...
 *   and caddy mode overrode it — the caller is expected to log that.
 */
function resolveBind(config) {
  const cfg = config || {};
  const optIn = cfg[OPT_IN_KEY] === true;

  if (cfg.ingressMode === 'caddy') {
    return { host: LOOPBACK, label: LOOPBACK, reason: 'caddy', refusedOptIn: optIn };
  }
  if (optIn) {
    return { host: null, label: '*', reason: 'opt-in', refusedOptIn: false };
  }
  return { host: LOOPBACK, label: LOOPBACK, reason: 'default', refusedOptIn: false };
}
