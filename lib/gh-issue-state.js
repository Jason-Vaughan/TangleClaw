...
function _reasonFrom(r) {
  if (r.errorCode === 'ENOENT') return 'gh is not installed';
  if (r.timedOut) return `gh api timed out after ${EXEC_TIMEOUT_MS}ms`;
...
function _isNotFound(r) {
  if (r.errorCode === 'ENOENT' || r.timedOut) return false;
