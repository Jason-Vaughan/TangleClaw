 ...
 *
 * A caller that needs the tree's state AS OF NOW — rather than a view of it
 * that may be up to the TTL old — passes `fresh`. The cache exists to keep
 * repeated dashboard polls off the event loop; a caller recording a fact into
 * a frozen document is not polling, and a stale `dirty` there is written down
 * as though it had been measured. A fresh read still refreshes the entry, so
 * it costs the pollers nothing.
 *
 * @param {string} dir - Project directory path
 * @param {object} [options] - Read options.
 * @param {boolean} [options.fresh=false] - Skip the cached entry and measure now.
 * @param {number} [options.budgetMs] - Total wall clock for all git work here.
 * @returns {{ branch: string, dirty: boolean|null, headSha: string|null, lastCommit: string, lastCommitAge: string, latestTag: string|null, incomplete: string[], cause: string|null }|null}
 *   `branch` is never null: it answers the sentinels `'unknown'` and `'HEAD'`. Use
 *   `measuredBranch()` where an unestablished branch must read as absent.
 */
...
function _fetchInfo(dir, options = {}) {
  ...
  if (hasCommits) {
    // THREE fields in one invocation, and the count matters: everything this
    // function reports about the tree has to describe ONE moment. A caller that
    // freezes these into a document — the handoff does — pairs whatever it gets
    // here, so a branch and a sha from different reads can name a branch that
    // does not contain its own commit.
    //
    // `%H` leads because it is fixed-width and cannot be ambiguous. `%s` is the
    // subject and is always exactly one line — git folds a multi-line first
    // paragraph into it — so no commit message can pull the format out of
    // alignment.
    const logged = step('lastCommit', null,
      (t) => exec('git log -1 --format=%H%n%s%n%cr', dir, t));
    if (logged.ok) {
      const lines = String(logged.value).split('\n');
      headSha = (lines[0] || '').trim() || null;
      lastCommit = (lines[1] || '').trim();
      lastCommitAge = (lines[2] || '').trim();
    } else {
      // THREE fields did not arrive, not two. `headSha` joined this invocation
      // and must join its failure: a caller that freezes the result reads an
      // absent `incomplete` entry as "measured", and this is the one field
      // whose absence a frozen document cannot recover from.
      if (!incomplete.includes('headSha')) incomplete.push('headSha');
  ...
  return {
    branch,
    dirty,
    // Measured in the SAME invocation as `lastCommit` and beside the same
    // `status` read that produced `branch` and `dirty`, so a caller freezing all
    // three describes one tree at one moment. `null` when the repository is
    // unborn or the log could not be read — never a sha carried from elsewhere,
    // and `incomplete` names it when a read was attempted and failed.
    //
    // A caller that FREEZES these must also know `branch` is not null-on-failure:
    // it answers `'unknown'` when the status read failed or could not be parsed,
    // and `'HEAD'` when the tree is detached. Both are truthy, so `branch || null`
    // stores a sentinel as a measured branch name. `measuredBranch()` below is
    // the reader for anyone who needs "the branch, or nothing".
    headSha,
    lastCommit,
    lastCommitAge,
    latestTag,
...
/**
 * The branch a reading actually established, or null.
 *
 * `getInfo().branch` is never null: it answers `'unknown'` when the status read
 * failed or could not be parsed, and `'HEAD'` when the tree is detached. Both
 * are truthy, so the obvious `info.branch || null` freezes a sentinel into
 * whatever the caller is writing — which for the handoff document means a
 * launch reading `branch: "unknown"` as a fact and refusing READY over it,
 * unrepairably, because the bytes are frozen.
 *
 * A detached tree genuinely has no branch, so it answers null too rather than
 * reporting the literal string git prints.
 * @param {object|null} info - A `getInfo` result
 * @returns {string|null} The branch name, or null when none was established
 */
function measuredBranch(info) {
  if (!info || typeof info.branch !== 'string') return null;
  if (info.branch === 'unknown' || info.branch === 'HEAD') return null;
