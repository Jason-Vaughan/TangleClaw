...
function _fetchInfo(dir, options = {}) {
  ...
  if (hasCommits) {
    ...
    // function reports about the tree has to describe ONE moment. `headSha`
    // joined them for #1648, where the handoff froze a branch probed at wrap
    // scope beside a sha carried from the session's launch — a document
    // claiming `main` at a commit that had never been on main.
    //
  ...
  return {
    ...
    // three into a document is describing one tree at one moment (#1648). `null`
    // when the repository is unborn or the log could not be read — never a sha
