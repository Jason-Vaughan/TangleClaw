 ...
 * hand-rolling a check. Each deviation is named where its option is documented
 * below — `PLAN_POINTER_CONTAINMENT` in `lib/wrap-steps/priming-roll.js` does not
 * follow symlinks, because worktree governance state is symlinked out of the
 * worktree by convention; `lib/checkout-layout.js` counts the root as inside,
 * because it asks about a directory. The deviations are listed, their TALLY is
 * not: a count goes stale the moment one lands, and a stale inventory is exactly
 * what tells the next maintainer to stop looking.
 *
 ...
 *
 * The root is never inside BY DEFAULT: a caller naming a FILE is asking about
 * something the root, a directory, can never be. That was the disagreement this
 * function exists to end — one caller's hand-rolled check counted the root as
 * inside, long read as a deliberate difference for a caller validating
 * directories.
 *
 * Such a caller now exists, and it asks here with an option rather than
 * hand-rolling its own check, which is the whole contract of this module.
 * `lib/checkout-layout.js` tests a DIRECTORY — "does this git command act on the
 * primary checkout" — and there the root is precisely the answer that matters:
 * `git checkout` run in the primary root itself is the case the #798 guard
 * exists for, and a worktree root must count as inside its own worktree or the
 * subtraction that keeps the guard off every worktree fails.
 *
 ...
 *   For callers asking about a DIRECTORY rather than a file; leave it off for
 *   anything that will be opened for writing, because the root cannot be.
...
function _containsPath(projectRoot, absolutePath, options = {}) {
  ...
  // `allowRoot` binds on BOTH passes or it binds on neither: a caller asking
  // about the root would clear the lexical test and then be rejected here for
  // the same reason it was exempted there.
  const realRel = path.relative(realRoot, realResolved);
  if ((realRel === '' && !allowRoot) || realRel.startsWith('..') || path.isAbsolute(realRel)) {
