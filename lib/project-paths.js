...
function resolveWithinProject(projectRoot, relativePath) {
  ...
  if (path.isAbsolute(raw)) {
    return { ok: false, reason: 'path must be relative to the project root, not absolute' };
  }
  ...
  if (rel === '') {
    return { ok: false, reason: 'path resolves to the project root itself, not a file inside it' };
  }
