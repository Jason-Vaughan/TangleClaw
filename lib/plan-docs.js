...
function _isSafeHref(href) {
  // The browser's URL parser strips leading C0 controls and space before it
  // reads the scheme, and tabs/newlines anywhere — so `\x01javascript:...`
  // is `javascript:` to the browser while a scheme regex saw no scheme at
  // all. Refuse the whole class outright rather than mirror the stripping.
  ...
  if (!SAFE_HREF_SCHEMES.has(url.protocol)) return false;
  // http(s) either written out, or a same-document/relative reference that
  // still resolves to the dummy base — a protocol-relative `//host/x` (or
  // its backslash spelling) resolves to another host and is refused.
  return /^https?:/i.test(href) || url.host === 'x';
}
...
/** One table-separator cell: dashes with optional alignment colons. */
const TABLE_SEP_CELL_RE = /^:?-+:?$/;

/**
 * Whether a line is a GFM table separator row (`|---|:-:|`).
 *
 * Split-and-check rather than one regex: the earlier single pattern had
 * optional whitespace on both sides of an optional pipe, which backtracks
 * quadratically on a long run of spaces — a session-authored line of 60k
 * spaces held the request for seconds. Splitting on `|` and trimming each
 * cell is linear by construction.
 *
...
/**
 * How deep blockquotes may nest before further `>` markers are flattened
 * into the innermost quote. Each level is one renderer frame, so an unbounded
 * `>>>>…` line was a stack overflow waiting in session-authored text.
 */
 ...
 * @param {number} [depth] - Blockquote nesting depth of this call (internal).
 * @returns {string} HTML, safe to place in a document body.
 */
function renderPlanBody(markdown, depth = 0) {
  ...
  while (i < lines.length) {
    ...
    if (line.trim().startsWith('|') && i + 1 < lines.length && _isTableSeparator(lines[i + 1])) {
      flushParagraph();
      const header = line;
    ...
    if (line.startsWith('>')) {
      ...
      // Past the depth cap, remaining `>` markers are content of this quote,
      // not further nesting — bounded frames whatever the input.
      const inner = depth >= MAX_BLOCKQUOTE_DEPTH
        ? quoted.map((q) => q.replace(/^(?:>\s?)+/, ''))
        : quoted;
      out.push(`<blockquote>${renderPlanBody(inner.join('\n'), depth + 1)}</blockquote>`);
      continue;
    }
