 ...
 * because the peer only ever sends JSON; extensions are not negotiated. A
 * request path and a subprotocol can be named for peers that route on them
 * (ttyd serves its terminal at `/ws` under the `tty` subprotocol).
 *
...
class WsUnixClient extends EventEmitter {
   ...
   * @param {string} [options.path='/'] - The request target, with any query string.
   * @param {string} [options.protocol] - A subprotocol to request (`Sec-WebSocket-Protocol`).
   */
