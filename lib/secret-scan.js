 ...
 * density of non-text control bytes) marks it binary — PNG/PDF/zip content
 * trips this and is skipped. Cheap and conservative: false negatives (a binary
 * we skip) are acceptable for flag-only detection; false positives just waste
 * a regex pass. Shared by every caller that scans file bytes (uploads, the wrap's
 * secret check) so they agree on what "binary" means.
 * @param {Buffer} buffer
 * @returns {boolean} `false` for an empty buffer, which holds nothing to scan.
 */
...
/**
 * Largest file worth scanning, in bytes. Lives here beside `looksLikeText`
 * because both answer the same question — is this content worth a regex pass —
 * and every scanner (uploads, the wrap's secret check) must agree on it. Past
 * this size content is almost always generated or binary.
 */
const SCAN_SIZE_CAP = 1024 * 1024; // 1 MB

module.exports = { scanText, looksLikeText, SCAN_SIZE_CAP, PATTERNS };
