...
const { runPreflight, FILE_STATES } = require('./launch-preflight');
...
function buildContext(project, options = {}) {
  ...
  const publications = store.handoffs.listByProject(project.id, SESSION_WINDOW);
  ...
  return {
    ...
    worktreeProbe: probeWorktree(currentDoc && currentDoc.worktree, exec),
    // Diagnosis only, and only worth taking when a worktree was recorded and is
    // gone — it answers "where does this project actually sit now", and it never
    // promotes a verdict.
 ...
 * @param {Function} [options.exec] - `execFileSync` replacement, for tests
 * @returns {{verdict: string, reason: string, reasons: string[], evidence: object, repairOutcomes: object[], repaired: boolean, evaluationFailed: boolean}}
 */
