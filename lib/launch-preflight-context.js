...
const { digestOf, resumeState } = require('./handoff-publication');
...
function _handoffResume(project, ctx, result) {
  ...
  const fromDoc = (note) => ({
    ...base,
    resume: { currentState: null, nextAction: doc.nextAction || null, freshness: {} },
    note
  });
  switch (resumeState(doc)) {
    case 'valid':
      return { ...base, resume: doc.resume, note: null };
    case 'none':
...
function _legacyResume(project, doc, base) {
  ...
  return {
    ...
    resume: {
      ...
      nextAction: doc.nextAction || parsed.nextAction || null,
      freshness: {
        sha: parsed.freshness.sha || null,
        branch: parsed.freshness.branch || null,
        writtenAt: parsed.freshness.writtenAt || null,
        tier: parsed.freshness.tier || null
      }
    },
    note: null
  };
