...
function start(projectName, body) {
  ...
  if (!injected.ok && injected.controlRefusal) {
    // #1861: a held lane takes no handback prompt; answer with the gate's own refusal.
    const c = injected.controlRefusal;
    log.warn('handback prompt refused: the lane is held', { project: projectName, stepId, code: c.code });
    return _refuse(c.status, c.code, c.message);
  }
  if (!injected.ok) {
