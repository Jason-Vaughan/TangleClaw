 ...
 *   - `quiet` — still quiet when `QUIET_WATCH_CAP_MS` runs out, so an unanswered
 *     question does not keep a watch alive for the life of the server;
...
async function _watch(projectName, entry) {
  ...
  for (;;) {
    ...
    if (entry.state === 'quiet' && _internal.now() - entry.startedAt >= QUIET_WATCH_CAP_MS) {
