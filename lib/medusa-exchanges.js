...
const ONCE_FACTS = Object.freeze(['send_pending', 'hub_accepted', 'arrived', 'read', 'acknowledged', 'replied', 'aged', 'operator_alerted']);
 ...
 * escalates at once, so there is nothing to shorten. The watchdog reads its
 * configured values through `resolveThresholds`; these are the defaults.
 */
...
function project(row, facts) {
  ...
  for (const f of facts) {
    ...
    switch (f.fact) {
      ...
      case 'escalation_undeliverable': raiseEsc('escalated'); break;
  ...
  // Once a send's outcome is known, "unknown" is no longer the truth about it.
  if (progressState === 'send_unknown' && hubId) progressState = 'stored';
  if (wake && rank <= PROGRESS.wake) { rank = PROGRESS.wake; progressState = wake.state; }

  let state = terminal ? terminal.state : progressState;
  if (row.tracking === 'untracked') state = 'untracked';
  return {
    hub_id: hubId,
    state,
 ...
 * second read of the same message advances nothing.
 * @param {string} exchangeId - Exchange id
...
function _append(exchangeId, fact, opts = {}) {
  if (ONCE_FACTS.includes(fact) && store.medusaExchanges.facts(exchangeId).some((f) => f.fact === fact)) {
    return { row: store.medusaExchanges.get(exchangeId), recorded: false };
  }
...
/**
 * Record a send's intent before the Hub is called. Idempotent on `requestId`:
 * the same request returns the exchange it already created.
 * @param {object} input
 ...
 * @throws {ExchangeError} 400 on a malformed recipient, 404 on an unknown reply target, 429 over the blocking limit
 */
 ...
 * @param {{hubStatus?: string}} [opts] - The Hub's `received`/`queued` answer
 * @returns {object} The exchange row
 */
function bindHubId(exchangeId, hubId, opts = {}) {
  if (typeof hubId !== 'string' || !HUB_ID_RE.test(hubId)) {
    throw new ExchangeError(502, 'HUB_ID_INVALID', 'The Hub returned no usable message id');
  }
  return store.medusaExchanges.transaction(() => {
    ...
    let out = _append(exchangeId, 'hub_accepted', { code: hubStatus, actor: 'system', detail: { hubId }, at }).row;
 ...
 * made it, else the untracked arrival.
 * @param {string} hubId - Hub message id
 * @returns {object|null}
 */
function _forRecipientFact(hubId) {
  if (typeof hubId !== 'string' || !HUB_ID_RE.test(hubId)) return null;
  return store.medusaExchanges.getByHubId(hubId, 'send') || store.medusaExchanges.getByHubId(hubId, 'arrival');
}

/**
 * Record that the recipient fetched these messages from its inbox.
 * @param {string[]} hubIds - Hub ids actually returned to the reader
 * @returns {number} How many exchanges gained a `read` fact
 */
function recordRead(hubIds) {
  let n = 0;
  for (const hubId of Array.isArray(hubIds) ? hubIds : []) {
    const x = _forRecipientFact(hubId);
    if (!x) continue;
    store.medusaExchanges.transaction(() => {
      if (_append(x.exchange_id, 'read', { actor: 'recipient' }).recorded) n += 1;
    });
  }
  return n;
}
 ...
 */
function recordAcknowledged(hubIds, caller) {
  const rec = caller && caller.kind === 'project' ? { actor: 'recipient', proof: 'launch' }
    : caller && caller.kind === 'operator' ? { actor: 'operator', proof: caller.proof || null }
      : caller && caller.kind === 'operator-ui' ? { actor: 'operator-ui', proof: null }
        : { actor: 'recipient', proof: null };
  let n = 0;
  for (const hubId of Array.isArray(hubIds) ? hubIds : []) {
    const x = _forRecipientFact(hubId);
    if (!x) continue;
    store.medusaExchanges.transaction(() => {
      const { row, recorded } = _append(x.exchange_id, 'acknowledged', rec);
      if (recorded) n += 1;
      if (recorded && row.tracking === 'tracked' && !row.reply_required && !row.terminal_at) {
        _append(x.exchange_id, 'closed', {
          code: rec.actor === 'operator-ui' ? 'acknowledged-in-dashboard' : 'acknowledged',
          actor: 'system'
        });
      }
    });
...
/**
 * Record a wake-path fact (pending, blocked, attempted, not accepted,
 * accepted, rearmed) on the exchange a Hub id names.
 * @param {string} hubId - Hub message id
 * @param {string} fact - One of the wake facts or `rearmed`
 * @param {{code?: string|null, detail?: object|null}} [opts]
 * @returns {object|null} The exchange row, or null when no exchange matches
 */
function recordWakeFact(hubId, fact, opts = {}) {
  if (!WAKE_FACTS.includes(fact) && fact !== 'rearmed') throw new Error(`not a wake fact: ${fact}`);
  const x = _forRecipientFact(hubId);
  if (!x) return null;
  return store.medusaExchanges.transaction(() => _append(x.exchange_id, fact, {
    code: opts.code == null ? null : _code(opts.code), actor: 'system', detail: opts.detail || null
  }).row);
}
...
function recordEscalationFact(exchangeId, fact, opts = {}) {
  const allowed = ['aged', 'escalation_queued', 'escalation_accepted', 'escalation_failed', 'escalation_undeliverable', 'operator_alerted'];
...
function _terminal(exchangeId, fact, opts) {
  return store.medusaExchanges.transaction(() => {
    ...
    if (current.terminal_at) {
      if (current.state === wanted || (current.tracking === 'untracked' && fact === 'closed')) return current;
