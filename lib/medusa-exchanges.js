 ...
 * @param {string} hubId - Hub message id
 * @param {string} recipientWorkspaceId - The workspace being woken
 ...
 */
function recordWakeFact(hubId, recipientWorkspaceId, fact, opts = {}) {
 ...
 * one attempt concerns them all. A blocked or pending verdict that has not
 * changed since the last one is not recorded again, because the monitor
 * re-judges every few seconds; every attempt is recorded, each with its own nonce.
 ...
 * @param {{code?: string|null, detail?: object|null, at?: string}} [opts]
...
function recordWakeForRecipient(workspaceId, fact, opts = {}) {
  ...
  for (const x of _awaitingRead(workspaceId)) {
    store.medusaExchanges.transaction(() => {
      ...
      if (repeat) return;
      // The wake verdict that follows a re-arm must not erase it before the
      // retry happens; only an attempt consumes a re-arm.
      if (row.state === 'wake_pending' && row.wake_code === 'rearmed' && fact !== 'wake_attempted') return;
 ...
 * attempted yet. The wake monitor asks this before treating an already-nudged
 * inbox as done.
 * @param {string} workspaceId - Recipient workspace id
 * @returns {boolean}
 */
function rearmDue(workspaceId) {
  return _awaitingRead(workspaceId).some((x) => x.state === 'wake_pending' && x.wake_code === 'rearmed');
}

/**
 * Re-arm one exchange's wake, once, if it is still in the state the watchdog
 * judged it in. Decided inside a transaction, so a duplicate tick or a restart
 * mid-backoff cannot re-arm twice. The re-arm only asks the wake monitor to try
 * again; the monitor still applies every gate before it injects anything.
 * @param {string} exchangeId - Exchange id
 * @param {{expectRearmCount: number, nextEligibleAt: string, code: string, at?: string}} opts
...
function rearm(exchangeId, opts) {
  return store.medusaExchanges.transaction(() => {
    ...
    if (row.state !== 'wake_attempted' && row.state !== 'wake_not_accepted') return null;
    if (row.rearm_count !== opts.expectRearmCount) return null;
    return _append(exchangeId, 'rearmed', {
      code: _code(opts.code), actor: 'system', detail: { nextEligibleAt: opts.nextEligibleAt }, at: opts.at
    }).row;
  });
}
