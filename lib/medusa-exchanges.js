 ...
 * deliver to that id again, so waiting would be silence. Exchanges already
 * answered are left for their initiator to close.
...
function markRecipientRetired(workspaceId) {
  ...
  for (const x of store.medusaExchanges.listOpenForRecipient(workspaceId)) {
    // An answered exchange is waiting on its initiator, not its recipient:
    // the recipient leaving changes nothing about it.
    if (x.state === 'replied') continue;
    try {
