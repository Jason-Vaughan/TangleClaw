 ...
 * accepted) on the exchange a Hub id names.
 * @param {string} hubId - Hub message id
...
function recordWakeFact(hubId, recipientWorkspaceId, fact, opts = {}) {
  // Re-arms go only through `rearm`, which re-checks the budget.
 ...
 * @returns {{attempt: (object|null), outcome: (string|null), rearmPending: boolean}}
 *   `outcome` is `wake_not_accepted` or `wake_accepted` when a receipt for the
 *   newest attempt has been recorded.
 */
function wakeStanding(facts) {
  ...
  if (attempt) {
    for (const f of facts) {
      if (f.fact_seq > attempt.fact_seq && (f.fact === 'wake_not_accepted' || f.fact === 'wake_accepted')) outcome = f.fact;
    }
  }
  return {
    attempt,
...
/**
 * Re-arm one exchange's wake, once, if it is still where the watchdog judged
 * it: attempted, not read, not accepted, not already re-armed, and at the
 * expected count. Decided inside a transaction, so a duplicate tick or a
...
function rearm(exchangeId, opts) {
  return store.medusaExchanges.transaction(() => {
    ...
    const facts = store.medusaExchanges.facts(exchangeId);
    if (facts.some((f) => f.fact === 'read' || f.fact === 'acknowledged')) return null;
    const standing = wakeStanding(facts);
    if (!standing.attempt || standing.rearmPending || standing.outcome === 'wake_accepted') return null;
