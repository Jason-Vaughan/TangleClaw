...
async function runWrapPipeline(projectName, options = {}) {
  ...
  // Advisory: a project with no readable config runs the template unmodified,
  ...
  try {
    ...
    log.warn('project config unreadable — running the methodology pipeline with no step overrides', {
      project: projectName,
      error: err.message
    });
    stepOverrides = null;
  ...
  for (const templateStep of pipeline.steps) {
    if (blockedAt !== null) {
    ...
    const resolution = wrapStepOverrides.resolveStep(templateStep, stepOverrides);
  ...
  // future methodology with a renamed commit step still surfaces the
