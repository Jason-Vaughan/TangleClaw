 ...
 */
function _buildStepContext(project, session, step, runState) {
  return {
    project,
    session,
    step,
 ...
 *     enum string (e.g. `"errors-only"` for `lint`). The runner halts
 *     the pipeline on `!ok` only when the boolean `true` form is set;
 *     enum-style blockers are resolved inside the handler (it returns
 *     `ok: false` if the enum tripped).
...
async function runWrapPipeline(projectName, options = {}) {
  ...
  for (const step of pipeline.steps) {
    ...
    if (!handler) {
      ...
      const context = _buildStepContext(project, session, step, runState);
      try {
    ...
    // Block-true semantics: halt the pipeline on this step's failure.
    // Enum-style blockers (e.g. `"errors-only"`) are converted to an
    // `ok: false` inside the handler — the runner only knows boolean.
    if (step.blocker === true && !stepResult.ok) {
      blockedAt = step.id;
    }
  }
