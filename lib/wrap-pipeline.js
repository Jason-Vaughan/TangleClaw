 ...
 * closed — another run's content describing this session's commit:
 ...
 *     capture (`capturedText`), and was captured within `RESUME_WINDOW_MS`.
 *
 * Pure: the caller supplies the previous run and the clock, so the rule is
 * testable as data. The result is only ever built server-side — see
 * `lib/sessions.js#_triggerWrapPipeline`, which sets it AFTER spreading the
 * request's options so an HTTP body cannot supply its own captured content.
 *
 ...
 * @returns {Record<string, {output: object, capturedAt: number}>|null} Reusable results by step id, or null
 */
function resumableContentResults(previousRun, { sessionId, now }) {
  if (!previousRun || previousRun.running) return null;
  if (previousRun.sessionId == null || previousRun.sessionId !== sessionId) return null;
  if (typeof previousRun.finishedAt !== 'number') return null;
  const pipelineResult = previousRun.result && previousRun.result.pipelineResult;
  if (!pipelineResult || pipelineResult.ok !== false) return null;
  if (typeof pipelineResult.blockedAt !== 'string' || pipelineResult.commitSha) return null;
  ...
  for (const r of pipelineResult.results || []) {
    ...
    // A result that was itself resumed keeps its ORIGINAL capture time.
    const capturedAt = typeof r.output.capturedAt === 'number' ? r.output.capturedAt : previousRun.finishedAt;
    if (now < capturedAt || now - capturedAt > RESUME_WINDOW_MS) continue;
    reusable[r.stepId] = { output: r.output, capturedAt };
  }
  return Object.keys(reusable).length > 0 ? reusable : null;
}
 ...
 * capture from a fresh one — and marks the result `resumed`, so the drawer and
 * the run record can. An operator's "Skip & note" for the step wins over reuse:
...
function _resumedContentResult(step, runState, options) {
  ...
  const prior = resumeFrom && Object.prototype.hasOwnProperty.call(resumeFrom, step.id)
    ? resumeFrom[step.id] : null;
  if (!prior || !prior.output) return null;
  if (options.skipAiContent && options.skipAiContent[step.id] === true) return null;
  const parsedFields = prior.output.parsedFields === undefined ? null : prior.output.parsedFields;
  runState.staged[step.id] = { capturedText: prior.output.capturedText, parsedFields };
  return {
    ok: true,
    status: 'done',
