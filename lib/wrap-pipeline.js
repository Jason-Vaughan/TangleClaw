...
async function runWrapPipeline(projectName, options = {}) {
  ...
  _notifyStepEvent(options, projectName, {
    ...
    // Named only when it differs from the registered checkout, so the drawer can
    // say which tree is being wrapped at the moment it matters (#1469).
    workTree: scope.worktreeTarget ? scope.workTree : null
  });
