...
function normalizeAuthority(raw, projectId) {
  ...
  const allowed = new Set(['hold', 'stop', 'lifecycle', 'releaseDelegations', 'escalation']);
  ...
  // #1839: who hears when a blocking or critical message to this assignment's
  // project goes unanswered. A visibility route, never authority: nothing
  // checks it before a mutation. Kept only when given, so an assignment made
  // without it reads exactly as before.
  if (raw.escalation !== undefined) {
    const esc = raw.escalation;
    if (typeof esc !== 'object' || esc === null || Array.isArray(esc)) _malformed('authority.escalation must be an object');
    for (const key of Object.keys(esc)) {
      if (key !== 'blocking' && key !== 'critical') _malformed(`authority.escalation has an unknown key: ${key}`);
    }
    out.escalation = {
      blocking: _principalList(esc.blocking, 'escalation.blocking'),
      critical: _principalList(esc.critical, 'escalation.critical')
    };
    if (out.escalation.blocking.includes(target) || out.escalation.critical.includes(target)) {
      _malformed('the target project cannot be its own escalation route');
    }
  }
  return out;
}
...
/**
 * Where an unanswered message to a project escalates, from the project's
 * open assignment (#1839). The escalation route is set by the operator when
 * the assignment is created, never inferred from a role name or workspace.
 * An ungoverned project, or an assignment with no route for this priority,
 * escalates to the operator only.
 * @param {number} projectId - Recipient project id
 * @param {'blocking'|'critical'} priority - Message priority
 * @returns {{controlState: string, stateGeneration: (number|null), principals: string[]}} `controlState`
 *   is the assignment's state, or `ungoverned`; `principals` excludes the
 *   operator, who always sees escalations on the dashboard
 */
function escalationRouteFor(projectId, priority) {
  const row = Number.isInteger(projectId) ? store.control.getOpenForProject(projectId) : null;
  if (!row) return { controlState: 'ungoverned', stateGeneration: null, principals: [] };
  const route = (_authorityOf(row).escalation || {})[priority] || [];
  return { controlState: row.state, stateGeneration: row.state_generation, principals: route.filter((p) => p !== OPERATOR) };
}
