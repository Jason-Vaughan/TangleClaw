 ...
 * @returns {{controlState: string, principals: string[]}} `controlState` is the
 *   assignment's state, or `ungoverned`; `principals` excludes the operator,
 *   who always sees escalations on the dashboard
 */
function escalationRouteFor(projectId, priority) {
  ...
  if (!row) return { controlState: 'ungoverned', principals: [] };
  const route = (_authorityOf(row).escalation || {})[priority] || [];
  return { controlState: row.state, principals: route.filter((p) => p !== OPERATOR) };
}
