...
/** How an assignment's target was observed to have seen its state. */
 ...
 * @returns {void}
 */
function recordNotify(eventId, outcome) {
  store.control.insertReceipt({
    event_id: eventId, fact: 'notify_attempted', outcome_code: NOTIFY_OUTCOMES.includes(outcome) ? outcome : 'failed', actor_principal: SYSTEM
  });
}
