...
function validateTemplate(template) {
  ...
  // wrap_contract validation (CC-8, #386) — a methodology's default wrap-section
  // depth. `sections` is a subset of the fixed continuity vocabulary; the renderer
  // always forces `Next action` in, so omitting it is harmless. Absent
  // wrap_contract ⇒ the deep default (all 8 sections), so Prawduct omits the field.
  // Validated against the SAME vocabulary as the per-project gate (lib/projects.js)
  // so the methodology-default and project-override layers stay symmetric.
  if (template.wrap_contract !== undefined && template.wrap_contract !== null) {
    const wc = template.wrap_contract;
    if (typeof wc !== 'object') {
      errors.push('wrap_contract must be an object');
    } else if (wc.sections !== undefined) {
      if (!Array.isArray(wc.sections)) {
        errors.push('wrap_contract.sections must be an array');
      } else {
        const invalid = wc.sections.filter((s) => !continuity.WRAP_SECTIONS.includes(s));
        if (invalid.length) {
          errors.push(
            `wrap_contract.sections must be a subset of valid section names ` +
            `(${continuity.WRAP_SECTIONS.join(', ')}); invalid: ${invalid.join(', ')}`
          );
        }
      }
    }
 ...
 * Deprecated templates (e.g. the V1 `prawduct` methodology superseded by the
 * V2 plugin, #536) carry `deprecated: true` plus a human-readable
 * `deprecationNote` so pickers can badge them; they remain fully functional.
 * @returns {object[]}
 */
function listTemplates() {
  ...
  return templates.map((t) => {
    ...
    return {
      ...
      type: t.type,
      version: t.version,
      deprecated: (full && full.deprecated) === true,
      deprecationNote: full && full.deprecated === true ? (full.deprecationNote || null) : null,
