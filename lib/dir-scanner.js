...
function createScanner(options = {}) {
  ...
  /**
   * Re-emit one line the child printed to stderr into the server's log, AT THE
   * LEVEL THE CHILD CHOSE.
   *
   * Preserving the level is the whole point. Logging every child line at `info`
   * reads as harmless until someone runs the server at `logLevel: 'warn'` — a
   * supported setting — at which point every warning the child emits is dropped
   * again and the silent-failure defect this re-emission exists to fix is back,
   * under a configuration nobody would think to test. The child's own formatter
   * puts the level in the line (`[ISO] [WARN] [module] …`), so it is parsed back
   * out rather than guessed at.
   *
   * A line that does not match the format is not discarded — it is anything the
   * child's runtime wrote directly, a V8 stack or a native warning, which is
   * exactly the output worth keeping. It goes to `warn`: unattributable output
   * from a process that is supposed to speak over IPC is a mild anomaly in
   * itself.
   *
   * @param {number|undefined} pid - The child's pid, so a line can be tied to a process.
   * @param {string} line - One complete line, newline already stripped.
   * @returns {void}
   */
  function _reemit(pid, line) {
    const m = line.match(/^\[[^\]]+\] \[(DEBUG|INFO|WARN|ERROR)\] /);
    const level = m ? m[1].toLowerCase() : 'warn';
    const emit = log[level] || log.warn;
    emit('scanner child', { pid, line });
  }
  ...
  function _ensureChild() {
    ...
    // Kept as a bounded TAIL rather than the first 4096 bytes it used to be.
    // That inversion is required, not cosmetic: this stream now also carries
    // every routine warning the child emits, so a head-bounded buffer fills up
    // with healthy `versionFilePath` notices during a long life and has no room
    // left for the output that explains the death. What was said LAST is what
    // explains an exit; for a child that dies at startup, last and first are the
    // same lines anyway, so nothing is lost in the case the buffer was written
    // for.
    let stderr = '';
    // The buffer above answers "why did it die". It cannot answer "what did it
    // warn about while alive", and that is a distinct question with real weight:
    // the child owns diagnostics nothing else can produce — an unwritable
    // version cache, a `versionFilePath` pointing at a file that is not there.
    // The child has no log file of its own (opening the server's from a process
    // that gets SIGKILLed mid-write is not a trade worth making), so it prints
    // to stderr and the supervisor re-emits each COMPLETE line into the server's
    // log, which is the only sink an operator ever reads. Buffered to a newline
    // because a pipe splits writes wherever it likes, and half a warning
    // attributed to the scanner is worse than none.
    let pending = '';
    if (spawned.stderr) {
      ...
      spawned.stderr.on('data', (chunk) => {
        ...
        pending = lines.pop(); // the tail is a partial line until a later chunk
        for (const line of lines) {
          if (line.trim()) _reemit(spawned.pid, line);
        }
        // A child that logs in a loop must not grow this either. Dropping the
        // partial line loses at most one message and cannot leak.
...
//
// A THIRD CONSUMER JOINED THIS CHILD, AND THE ANSWER IS "SHARE" — recorded here
// because the paragraph above says to revisit rather than assume. Uploads (#889)
// send `saveUpload` and `listUploads` through this scanner, and one of them is
// unlike anything else here: it carries up to the body cap as a base64 string
// and performs a write, where every other request reads a few small files. Two
// consequences, both accepted rather than overlooked. The child's handlers run
// synchronously, so a multi-megabyte write blocks it for the duration — hundreds
// of milliseconds on a slow external disk, bounded by the caller's own deadline
// and not by anything here. And a sibling riding this child when an upload's
// path stops answering dies with it as `tcAborted`, which is the same collateral
// property `projectFacts` already documents and reports honestly.
//
// A third scanner would remove both, and is not worth a permanent extra node
// process: these are all operator-initiated, human-paced actions that rarely
// overlap, and the failure they share is a directory refusing to answer — which
// is exactly the case the collateral vocabulary exists to describe. What WOULD
// justify splitting is a consumer that is neither rare nor human-paced.
