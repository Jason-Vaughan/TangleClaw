 ...
 *
 * The governance state is passed in by callers that already have it, because
 * deriving it here reads the project's directory on the caller's thread — and on
 * the dashboard's ten-second poll that was one of the reads that let a single
 * unreadable folder stop the server answering anything (#884). Callers without
 * it still get a correct answer; they just pay the read.
 *
 * @param {object} project - Project record (DB row; `path` and `engineId` read)
 * @param {string} [governanceState] - Precomputed governance state. Derived from
 *   disk when omitted.
...
function availableActions(project, governanceState) {
  ...
  const state = governanceState !== undefined
    ? governanceState
...
async function runAction(projectName, command, options) {
  ...
  const { governanceState } = await readProjectFacts(project);
  if (!availableActions(project, governanceState).some((a) => a.command === command)) {
    return {
      ok: false,
      output: null,
