 ...
 * the project's live `governanceState` (`lib/governance-state.js`, read in the
 * scanner child so this file never touches the operator's path), so the button
 ...
 * nothing at all.
 *
 * **Who may run one.** `POST /api/projects/:name/actions/:command` admits the
 * operator or a session bound to that project (#1752), because every action
 * here acts on one project's own checkout. That gate is an admission point for
 * project-scoped actions only. An action that reaches across the fleet, changes
 * identity or access, destroys or reshapes a project's lifecycle, or is the
 * operator's by nature must be classified operator-only explicitly; it does not
 * inherit this gate. `test/api-actions.test.js` pins the command list so adding
 * one forces that decision.
 *
 ...
 *
 * The governance state is REQUIRED, not derived. Deriving it here read the
 * project's directory on the caller's thread, which on the dashboard's
 * ten-second poll was one of the reads that let a single unreadable folder stop
 * the server answering anything (#884). Both call sites now obtain it from the
 * scanner child, so the fallback that made this convenient had no production
 * caller left — and a convenience that silently reintroduces the defect the
 * surrounding work removed is a trap, not a convenience. An omitted argument
 * offers nothing, which is the intent.
 *
 * @param {object} project - Project record (DB row; `path` and `engineId` read)
 * @param {string} governanceState - Governance state, from `readProjectFacts`.
 * @returns {Array<{label: string, command: string, confirm: boolean, confirmMessage?: string, successToast?: string}>}
 */
function availableActions(project, governanceState) {
...
async function runAction(projectName, command, options) {
  ...
  // Same predicate the button renders from, AND from the same source of truth.
  // Deriving the state here with a synchronous disk read would reintroduce both
  // faults at once: a route-reachable read of an operator-chosen path on the
  // event loop (#884), and a second source of truth for one predicate — the
  // button rendered from the scanner's answer while the gate enforces its own,
  // so a degraded read hides an action the API still allows (ADR 0001, and this
  // file's header). This is an operator-pressed path, so the read is interactive:
  // no backoff, and not sharing a child with the dashboard's poll.
  const facts = await readProjectFacts(project);
  // A directory that could not be READ is not a project whose governance says no.
  // Reporting it as UNAVAILABLE told the operator their action does not apply to
  // this project, when the truth is that the server could not look — the exact
  // misdiagnosis `lib/project-facts.js` exists to prevent, arriving through the
  // one path that already had the real reason in hand.
  if (facts.unreadable) {
    return {
      ok: false,
      output: null,
      error: `Could not read "${projectName}" to decide whether "${command}" is available: `
        + facts.unreadable + (facts.unreadableHint ? ` — ${facts.unreadableHint}` : ''),
      code: 'PROJECT_UNREADABLE'
    };
  }
  if (!availableActions(project, facts.governanceState).some((a) => a.command === command)) {
    return {
      ok: false,
      output: null,
