...
function refresh(dir) {
  ...
  // `measure` resolves rather than rejects; the catch is for a defect, so a
  // throw cannot pin this directory's slot in flight and its answer `pending`.
  const p = measure(dir).catch((err) => ({
    ...snapshotPending(), state: 'unknown', reason: `measurement failed: ${gitProbe.scrubLocations(err && err.message)}`,
    measuredAt: new Date(_internal.now()).toISOString(), incomplete: ['status']
  })).then((value) => {
...
/**
 * The payload before a directory's first measurement: a fact about the probe,
 * never about the checkout.
 *
 * @returns {object}
 */
...
function impactSnapshot(dir, fromSha, toSha) {
  ...
  if (!_impactInFlight.has(key)) {
    const p = classifyRange(dir, fromSha, toSha).catch((err) => ({
      impact: 'unknown', reason: `classification failed: ${gitProbe.scrubLocations(err && err.message)}`,
      fromSha: fromSha || null, toSha: toSha || null, executablePaths: [], recordsPaths: [], truncated: false
    })).then((value) => {
...
function compareRefresh(dir, headSha, upstreamSha) {
  ...
  if (pending) return pending;
  const p = compareToUpstream(dir, headSha, upstreamSha).catch((err) => ({
    ahead: null, behind: null, relation: 'unknown', reason: `comparison failed: ${gitProbe.scrubLocations(err && err.message)}`
  })).then((value) => {
