...
process.stdin.on('end', () => {
  ...
  try {
    // Follow the leaf with lstat, NOT existsSync. existsSync follows links, so
    // it answers false for a link whose target does not exist yet — and a write
    // through a DANGLING link still creates the file at the link's destination.
    // Deciding leaf-ness by existence therefore missed exactly the shape an
    // attacker controls: create memory/evil -> /outside/new.md, and the target's
    // absence was what let it through.
    let leaf = lexical;
    for (let hop = 0; hop < 16; hop++) {
      ...
      try {
        ...
        break; // nothing there at all — not a link, resolve the ancestors below
      }
      if (!st.isSymbolicLink()) break;
      leaf = path.resolve(path.dirname(leaf), fs.readlinkSync(leaf));
    }
    // Then the ancestor pass, for ".." and for a linked DIRECTORY somewhere
    // above a target that does not exist yet.
