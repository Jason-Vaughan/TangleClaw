...
function refreshMasterIdentity(options = {}) {
  ...
  if (enforcement === 'structural') {
    try {
      ...
      // The level IS on disk by this point — the guard reads it per call — so a
      // failure here means "the new level is in force but a tampered guard may
      // not have been restored", which is a different sentence from "nothing
      // changed". Flagged rather than described, so the route can say the true
      // one instead of guessing which failure it caught. Lives here rather than
      // at the change path because it is equally true of an ensure.
 ...
 * - `null` — tmux did not answer, so this could not be established. It is NOT
 *   folded into `false`: a caller that renders "all good" from a read that never
 *   happened writes a fact nobody has. Same discipline as `exists` (#905).
 *
 * @param {object} [options]
 * @param {string} [options.home] - Master home override (tests).
 * @param {object} [options.tmuxLib] - tmux module override (tests).
...
function readMasterIdentityFreshness(options = {}) {
  ...
  try {
    ...
    // No identity on disk means nothing has been generated for a Master to be
    // behind. An operator who has never opened it must not see a stale badge.
    return none;
  }
  const writtenAt = new Date(writtenAtMs).toISOString();

  const started = t.sessionCreatedAt(MASTER_TMUX_SESSION);
  ...
  if (!started.answered || started.cause === 'unparseable') {
...
function getMasterStatus(options = {}) {
  ...
  return {
    ...
    settings: {
      ...
      ...readMasterIdentityFreshness({ home, tmuxLib: t })
    }
  };
}
