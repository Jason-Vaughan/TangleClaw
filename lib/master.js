...
function _renderAccessLevelSection(level, enforcement) {
  ...
  return [
    ...
    'The guard covers the file-editing tools. Do not reach for shell commands to do',
    'what it refused — that is working around the boundary, not around a bug.',
    '',
    structural
      ? 'Your level is checked by a write guard on every attempt, so'
      : 'On this engine there is no write guard — nothing mechanically stops a write, so',
    structural
      ? 'a change the operator makes takes effect on your very next tool call.'
      : 'this boundary holds only because you honor it. A change the operator makes',
    ...(structural ? [] : ['reaches you when this file is regenerated, not immediately.']),
    ''
  ];
}
...
process.stdin.on('end', () => {
  ...
  const control = [LEVEL_FILE, path.join(HOME, '.claude')];
