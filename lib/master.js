 ...
 * @returns {{applied: boolean, home: string}} `applied: false` when no master
 *   home exists yet.
 */
function applyMasterAccessLevel(level, options = {}) {
  ...
  //
  // Keyed on the guard already existing rather than on engine resolution: this
  // is the change path, and it must not newly provision structural enforcement
  // for a master running instructionally. Regeneration is idempotent, so the
  // common case (nothing was touched) rewrites identical bytes.
  if (fs.existsSync(path.join(home, '.claude', 'hooks', 'guard-writes.js'))) {
    _writeMasterGuardrails(home);
  }
  return { applied: true, home };
}
...
function readLevel() {
  try {
    ...
    if (token === 'write' || token === 'suggest') return token;
    return 'read-only';
  } catch (err) {
    return 'read-only';
  }
