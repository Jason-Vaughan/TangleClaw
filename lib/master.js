 ...
 * file on EVERY invocation, so the GUARD applies a change on the master's next
 * tool call — the RUNNING master reads its identity only at launch and must be
 * restarted before it acts on one (#755, #968): outside `memory/` — the sole write carve-out, and the
...
/**
 * The key the Master's Medusa listener is held under in `lib/medusa.js`.
 *
 * Project listeners are keyed by their numeric session id; the Master has no
 * session row, so it needs a key no project can ever mint. A string cannot
 * collide with a number, and the registry pins the workspace id to the
 * Master's HOME rather than to this key — which is what keeps the Master
 * addressable across the kill/restart cycle #968 made routine. A fresh id per
 * restart would force every peer to re-fetch the roster before every send.
 */
const MASTER_MEDUSA_KEY = 'master';

/** The name peers see on the roster, and the slug the workspace id is minted from. */
const MASTER_MEDUSA_NAME = 'Project Master';

/** API base every Master switchboard call goes through — the Master's analogue
 *  of `/api/sessions/:project/medusa`. Rendered into the identity and the wake
 *  nudge, so it lives in one place. */
const MASTER_MEDUSA_API_BASE = '/api/master/medusa';
 ...
 */
function masterHome() {
  return path.join(tangleclawHome.baseDir(), 'master');
}
...
function masterSettings(config) {
  ...
  return {
    ...
    // Same opt-in shape as a project's `medusaEnabled` / `medusaWake`: explicit
    // `true` only. A Master that is a switchboard participant is addressable by
    // every project session, so it is a choice the operator makes, not a default.
    medusaEnabled: raw.medusaEnabled === true,
    medusaWake: raw.medusaWake === true
  };
}
...
function buildMasterClaudeMd(config, extras = {}) {
  ...
  if (extras.medusaWorkspaceId) {
    lines.push(..._renderMasterMedusaSection(extras.medusaWorkspaceId, resolved.level));
  }

  lines.push(
    '## Read API quick reference',
    '',
    `- \`GET /api/projects\` — all projects (engine, archived, groups, path). `
      + 'Send `x-tangleclaw-role: master` and `x-tangleclaw-launch-id: $TANGLECLAW_LAUNCH_ID` with it: '
      + 'without them every row comes back as its public projection (`restricted: true`, no path, git or groups).',
    `- \`GET /api/sessions/:project/status\` — a project's live-session state.`,
    `- \`GET /api/awareness\` — per-session awareness state (confirmed/sent/unverified/no-rules/unaware) plus a top-level \`master\` entry — YOU appear there; an 'unaware' recent session means that project's sessions may not know TangleClaw exists.`,
    ...
    `- \`GET /api/shared-docs?groupId=<id>\` — a group's shared docs${tokenActive ? ' (bearer token)' : ''}. `
      + 'Send `x-tangleclaw-role: master` and `x-tangleclaw-launch-id: $TANGLECLAW_LAUNCH_ID` with it: '
      + 'they are how the server tells your pane from a project session, and `tc docs` sends both for you.',
    ...
    '',
    '**A project session reporting "port unreachable" is not proof of outage.** Managed sandboxes',
    '(Codex among them) block loopback while the host service is healthy. Before relaying an outage,',
    'check from the host: `launchctl print gui/$UID/com.tangleclaw.server` and',
    `\`lsof -nP -iTCP:${serverPort} -sTCP:LISTEN\`, or an unsandboxed \`GET /api/health\`.`,
    ''
  );
...
/**
 * Whether a Master at `level` may SEND on the switchboard (direct messages and
 * loops). Receiving is never gated: an inbox is a read.
 *
 * Sending an instruction to another session is the Master acting on the fleet,
 * which is what the access level governs — a `read-only` Master that could
 * still broadcast "do X" to every project would have a write path the level
 * claimed not to exist. `suggest` sends: a message is a proposal the recipient
 * session decides on, which is what that tier means.
 *
 * Read from CONFIG per call rather than from the guard's `.access-level` file.
 * `PATCH /api/config` writes both in the same request, so a flip binds here on
 * the very next call with no restart — and config is the authority the
 * operator actually set, where the file is the guard's copy of it.
 * @param {string} level - One of `MASTER_ACCESS_LEVELS`.
 * @returns {{allowed: boolean, reason: string|null}} `reason` is operator-facing
 *   and names the remedy when refused.
 */
function masterMedusaOutbound(level) {
  if (level === 'suggest' || level === 'write') return { allowed: true, reason: null };
  return {
    allowed: false,
    reason: `The Project Master is at access level "${level}", which can receive switchboard messages but not send them. Raise it to "suggest" or "write" (Master settings, or the READ/WRITE toggle on its control bar) to let it message other sessions.`
  };
}

/**
 * The Master's switchboard section of its identity — the Master-side twin of
 * `lib/sessions.js#_medusaPrimeSection`, carrying the same three things a
 * participant needs: its identity, its role, and how to reach the API.
 *
 * The loop-discipline line is the one that matters most here. A project
 * session mostly answers; a Master that can address the whole fleet mostly
 * ASKS, and an initiator that does not close its exchanges leaves loops parked
 * on every project it touched.
 * @param {string} workspaceId - The Master's registered workspace id.
 * @param {string} level - The access level in force, which decides whether the
 *   section promises a send path at all.
 * @returns {string[]} Identity lines.
 */
function _renderMasterMedusaSection(workspaceId, level) {
  const outbound = masterMedusaOutbound(level);
  const base = MASTER_MEDUSA_API_BASE;
  const lines = [
    '## Medusa Switchboard — session messaging',
    '',
    'You are a Medusa participant: project sessions can message you, and —',
    'subject to your access level — you can message them. This is the one part',
    'of the TangleClaw API you use with POST as well as GET.',
    '',
    `- **Your workspace id:** \`${workspaceId}\` — you send and receive under this`,
    '  identity. TangleClaw registers it for you and keeps it stable across your',
    '  restarts, so peers can address you without re-fetching the roster.',
    '- **Role:** when a message arrives in your inbox, read it, act on it within',
    '  your access level, and reply to the sender over the same channel.',
    '- **You are usually the initiator, so you close the loop.** The switchboard',
    '  convention is that the session that opened an exchange ends it: when a',
    '  peer has answered what you asked, send the closing acknowledgement rather',
    '  than leaving the exchange parked. When a peer initiated, keep responding',
    '  until THEY close it — never end an exchange you did not open.',
    ...
    '  unprompted. Participate when a message arrives (TangleClaw nudges you',
    '  when one does) or when the operator asks.',
    '- **How to interact:** TangleClaw already runs your WebSocket listener — do',
    '  NOT register your own connection for this workspace id (two consumers on',
    '  one id fight over the queue). Use the API at the base URL above:',
    `  inbox \`GET ${base}/messages\`, mark read \`POST ${base}/read\`,`,
    `  peers \`GET ${base}/roster\`, send \`POST ${base}/send\``,
    '  (`{"to","message"}`), why a peer has not picked up',
    `  \`GET ${base}/peers/<workspace-id>\` (a reason code with its \`meaning\`, or \`local: false\`).`
  ];
  if (outbound.allowed) {
    lines.push(
      `- **Sending is enabled** at your current access level (\`${level}\`). A`,
      '  send answers `received` (delivered live) or `queued` (the peer is',
      '  offline) — never assume delivery from a 200.'
    );
  } else {
    lines.push(
      `- **Sending is disabled** at your current access level (\`${level}\`): the`,
      `  send and loop routes answer 403 \`ACCESS_LEVEL\`. You can still read your`,
      '  inbox and the roster. Say so if asked to message a session, rather than',
      '  working around it.'
    );
  }
  lines.push('');
 ...
 * @param {boolean} [options.medusaEnabled] - Whether the Master is a switchboard
 *   participant; decides which of the two true sentences HOWTO.md carries.
 */
function _refreshMasterMemory(home, options = {}) {
  ...
  fs.writeFileSync(path.join(memDir, 'HOWTO.md'), [
    ...
    ...(options.medusaEnabled
      ? [
        '- **Medusa switchboard**: you are a participant — your workspace id, role',
        `  and the \`${MASTER_MEDUSA_API_BASE}/*\` routes are in the switchboard`,
        '  section of your identity file. Do not open your own WebSocket.'
      ]
      : [
        '- **Medusa switchboard**: sessions message each other via the switchboard;',
        '  you observe through the activity feed — you are not a participant. The',
        '  operator can make you one from the Medusa control on your control bar.'
      ]),
...
function refreshMasterIdentity(options = {}) {
  ...
  try {
    seedBaselineMasterRules();
    // The workspace id is ensured HERE, before the identity is rendered, so the
    // id the Master reads about itself is the id its listener registers under
    // — the same identity-before-listener ordering the project launch path
    // uses (`lib/sessions.js` pre-mints for the prime). Pinned to the home via
    // the registry, so this is idempotent across every ensure and restart.
    const medusaWorkspaceId = settings.medusaEnabled
      ? medusaRegistry.ensureWorkspaceId(home, MASTER_MEDUSA_KEY, MASTER_MEDUSA_NAME)
      : null;
    ...
    _refreshMasterMemory(home, { generatedAt: new Date().toISOString(), medusaEnabled: settings.medusaEnabled });
...
function ensureMasterSession(options = {}) {
  ...
  if (probe.live) {
    // The listener follows the SETTING on every ensure, not only on creation:
    // the operator flips `medusaEnabled` with the Master already running, and
    // "open the drawer" is the ensure that should make it true.
    const medusaStatus = syncMasterMedusa({ home, live: true, enabled: settings.medusaEnabled, wsFactory: options.wsFactory });
    return { created: false, ...base, medusa: medusaStatus };
  }
  if (!probe.answered) {
  ...
  const baseLaunchCmd = sessions._buildLaunchCommand(engineProfile, null, launchMode);
  if (!baseLaunchCmd) {
    return { created: false, ...base, error: `Engine "${engineId}" has no launch command` };
  }
  // The Master carries its own launch identity, so an engine whose shared
  // background process can hand one launch's environment to the next needs the
  // same version-bounded isolation a project pane gets. The Master never opens
  // a native startup channel, so this is its only path to it.
  const launchCmd = sessions._hardenedLegacyLaunchCommand(engineId, baseLaunchCmd, det.path || null);

  // Resolved the same idempotent way refreshMasterIdentity minted it moments
  // ago (registry-pinned to the home), rather than threaded back through a
  // return value — the two reads cannot disagree.
  const medusaWorkspaceIdForEnv = settings.medusaEnabled
    ? medusaRegistry.ensureWorkspaceId(home, MASTER_MEDUSA_KEY, MASTER_MEDUSA_NAME)
    : null;

  // Ambient-awareness floor for the Master (#1141): the same shell substrate
  // every project pane gets, minus a project identity it must not claim.
  // TANGLECLAW_ROLE carries the actor instead of TANGLECLAW_PROJECT_ID, so
  // `tc whoami` answers the master identity rather than "unresolved". The
  // API origin follows the sessions rule: omitted when unresolvable, never a
  // sentence-shaped fake. Engine-profile env wins on collision, as in
  // launchSession. The command is wrapped with the same PATH floor as project
  // panes — rc processing rebuilds PATH before the command body runs (#1140).
  const ambientEnv = { TANGLECLAW_ROLE: 'master' };
  const masterApiOrigin = sessions._apiOrigin();
  if (masterApiOrigin) ambientEnv.TANGLECLAW_API = masterApiOrigin;
  if (medusaWorkspaceIdForEnv) ambientEnv.TANGLECLAW_WORKSPACE_ID = medusaWorkspaceIdForEnv;
  // The Master's binding for surfaces that answer per caller (#1626): minted
  // like a project pane's, but recorded nowhere else. `new-session -e` keeps it
  // in the tmux session environment, where `liveMasterLaunchId` reads it back,
  // so a relaunch replaces it and a Master that ends takes it with it.
  ambientEnv.TANGLECLAW_LAUNCH_ID = launchSequence.mintLaunchId();

  try {
    const created = t.createSession(MASTER_TMUX_SESSION, {
      cwd: home,
      command: sessions._withPathFloor(launchCmd),
      env: { ...ambientEnv, ...(engineProfile.launch ? engineProfile.launch.env : {}) }
    });
    if (!created) {
  ...
  const medusaStatus = syncMasterMedusa({ home, live: true, enabled: settings.medusaEnabled, wsFactory: options.wsFactory });
  return { created: true, ...base, medusa: medusaStatus };
}

/**
 * The launch id of the live Project Master, read from its tmux session
 * environment. This is the only record of it: see the note where
 * `ensureMasterSession` mints it.
 *
 * `answered: false` means tmux did not reply, and a caller deciding access on
 * this must refuse. `launchId: null` with `answered: true` means there is no
 * live Master, or it was launched before it carried a launch id.
 * @param {object} [options]
 * @param {object} [options.tmuxLib] - tmux module override (tests).
 * @param {number} [options.timeout] - How long to wait for tmux, in ms; tmux's own default when omitted.
 * @returns {{launchId: string|null, answered: boolean, cause: string|null}}
 */
function liveMasterLaunchId(options = {}) {
  const t = options.tmuxLib || tmux;
  const readOptions = options.timeout ? { timeout: options.timeout } : {};
  const read = t.readSessionEnv(MASTER_TMUX_SESSION, 'TANGLECLAW_LAUNCH_ID', readOptions);
  return { launchId: read.value, answered: read.answered, cause: read.cause };
}

/**
 * The Master's Medusa target — what `lib/medusa.js` needs to run a listener
 * for it, in the shape every project session already passes.
 * @param {object} [options]
 * @param {string} [options.home] - Master home override (tests).
 * @returns {{projectPath: string, sessionId: string, name: string}}
 */
function masterMedusaTarget(options = {}) {
  return {
    projectPath: options.home || masterHome(),
    sessionId: MASTER_MEDUSA_KEY,
    name: MASTER_MEDUSA_NAME
  };
}

/**
 * The Master's Medusa listener status, in the same shape a project session's
 * `GET /medusa/status` returns — `off` when there is no listener.
 * @returns {{state: string, workspaceId: (string|null), unread: number, lastError: (string|null)}}
 */
function getMasterMedusaStatus() {
  return medusa.getStatus(MASTER_MEDUSA_KEY);
}

/**
 * Make the Master's listener match the one rule it follows: a listener exists
 * exactly when the Master is LIVE and the setting is ON. Every lifecycle
 * point — ensure, kill, boot resync, the bar's toggle — calls this rather than
 * starting or stopping directly, so no path can leave a listener registered
 * for a Master that is gone (#836's shape) or a live Master unregistered with
 * the setting on.
 *
 * Idempotent both ways: `medusa.startSession` returns the existing listener's
 * status, and `stopSession` on nothing is a no-op.
 * @param {object} options
 * @param {boolean} options.live - Whether the Master's tmux session is live.
 * @param {boolean} options.enabled - The `medusaEnabled` setting.
 * @param {string} [options.home] - Master home override (tests).
 * @param {(url: string) => object} [options.wsFactory] - Socket factory seam (tests).
 * @returns {{state: string, workspaceId: (string|null), unread: number, lastError: (string|null)}}
 *   The listener status after the sync.
 */
function syncMasterMedusa(options) {
  if (options.live && options.enabled) {
    return medusa.startSession({ ...masterMedusaTarget(options), wsFactory: options.wsFactory });
  }
  medusa.stopSession(MASTER_MEDUSA_KEY);
  return medusa.getStatus(MASTER_MEDUSA_KEY);
}

/**
 * Re-sync the Master's listener after a server restart — the Master's half of
 * `lib/sessions.js#resyncMedusaListeners`. Listeners are in-memory, so without
 * this a TangleClaw restart under a running Master silently dropped it from
 * the switchboard until the next ensure. `autoStart` already ensures on boot,
 * but a Master the operator started by hand and left running is not covered
 * by that path.
 * @param {object} [options]
 * @param {object} [options.tmuxLib] - tmux module override (tests).
 * @param {string} [options.home] - Master home override (tests).
 * @param {object} [options.config] - Config override (tests).
 * @param {(url: string) => object} [options.wsFactory] - Socket factory seam (tests).
 * @returns {{resynced: boolean, live: boolean|null}} `live: null` when tmux did
 *   not answer — in which case nothing is started, because a listener for a
 *   Master that may not exist is the defect this function exists to avoid.
 */
function resyncMasterMedusa(options = {}) {
  const t = options.tmuxLib || tmux;
  const settings = masterSettings(options.config || store.config.load());
  if (!settings.medusaEnabled) return { resynced: false, live: null };
  const probe = t.probeSession(MASTER_TMUX_SESSION);
  if (!probe.answered) {
    log.warn('Master Medusa re-sync skipped — tmux did not answer', { cause: probe.cause });
    return { resynced: false, live: null };
  }
  const status = syncMasterMedusa({ home: options.home, live: probe.live, enabled: true, wsFactory: options.wsFactory });
  if (probe.live) log.info('Re-synced Master Medusa listener after server restart', { workspaceId: status.workspaceId });
  return { resynced: probe.live, live: probe.live };
}

/**
 * Probe the Master's liveness — the one call the switchboard routes make
 * before resolving the Master as a target, kept as its own export so a route
 * test can stand in for tmux the way `medusa-wake`'s `_internal` seams do.
 * @param {object} [options]
 * @param {object} [options.tmuxLib] - tmux module override (tests).
 * @returns {{live: boolean, answered: boolean, cause: string|null}}
 */
function masterLiveness(options = {}) {
  return (options.tmuxLib || tmux).probeSession(MASTER_TMUX_SESSION);
}

/**
 * The Master as `medusa-wake` sees it: a session-shaped record the monitor can
 * scan with the same gates it applies to a project session, or `null` when
 * there is nothing to scan.
 *
 * `null` for an unanswered probe as well as an absent Master — a pane the
 * monitor cannot confirm exists is not one it may type into.
 * @param {object} [options]
 * @param {object} [options.tmuxLib] - tmux module override (tests).
 * @param {object} [options.config] - Config override (tests).
 * @param {object} [options.enginesLib] - engines module override (tests).
 * @returns {null|{id: string, isMaster: true, name: string, tmuxSession: string,
 *   engineId: (string|null), sessionMode: string, status: string,
 *   medusaWake: boolean, apiBase: string}}
 */
function masterWakeRecord(options = {}) {
  const probe = (options.tmuxLib || tmux).probeSession(MASTER_TMUX_SESSION);
  if (!probe.answered || !probe.live) return null;
  const config = options.config || store.config.load();
  const { settings, engineId } = _masterRuntime(config, options.enginesLib);
  return {
    id: MASTER_MEDUSA_KEY,
    isMaster: true,
    name: MASTER_MEDUSA_NAME,
    ...
    sessionMode: 'tmux',
    status: 'active',
    medusaWake: settings.medusaWake,
    apiBase: MASTER_MEDUSA_API_BASE
  };
}

/**
 * Type a line into the Master's pane — the Master's `injectCommand`, with the
 * same two refusals that function applies to a project session: a length cap
 * (`security-model.md`: 4096) and a pane that must be confirmed live first.
 * Never throws; the monitor that calls it retries on the next tick.
 * @param {string} command - Text to inject.
 * @param {object} [options]
 * @param {object} [options.tmuxLib] - tmux module override (tests).
 * @param {boolean} [options.enter] - Send Enter after the text (default true).
 * @param {object} [options.config] - Config override (tests).
 * @param {object} [options.enginesLib] - engines module override (tests).
 * @returns {{ok: boolean, error: string|null}}
 */
function injectMasterCommand(command, options = {}) {
  const t = options.tmuxLib || tmux;
  ...
  const probe = t.probeSession(MASTER_TMUX_SESSION);
  if (!probe.live) {
    return { ok: false, error: probe.answered ? 'The Project Master is not running' : `tmux did not answer (${probe.cause})` };
  }
  // The Master's engine locates its composer, so a draft cleared before the
  // paste is recorded (#1507). A config that cannot be read leaves it unknown,
  // which the clear reports honestly; it never stops the delivery.
  let engineId = null;
  try {
    engineId = _masterRuntime(options.config || store.config.load(), options.enginesLib).engineId;
  } catch (err) {
    log.debug('Master engine unresolved for the prompt clear', { error: err.message });
  }
  try {
    t.sendKeys(MASTER_TMUX_SESSION, command, { enter: options.enter !== false, engineId });
...
function killMasterSession(options = {}) {
  ...
  if (!probe.answered) {
    // Logged, like the other refusal. This was the only branch here that
    // returned an error silently, which makes the one condition an operator most
    // needs to reconstruct afterwards the one leaving no trace.
    log.warn('Project Master kill refused — tmux liveness could not be established', {
      tmuxSession: MASTER_TMUX_SESSION, cause: probe.cause
    });
    return {
      ...base,
      killed: false,
  ...
  if (!probe.live) {
    // Success. The operator wanted it not running; it is not running. A
    // listener for it is torn down here too: the session it spoke for is gone,
    // and a registered workspace with nobody behind it is what makes a roster
    // lie (#836).
    syncMasterMedusa({ live: false, enabled: false });
  ...
  // Confirmed dead → the listener goes with it. Only on the CONFIRMED path: the
  // unconfirmed branch above keeps the listener, because it may still be
  // speaking for a Master that is still running.
  syncMasterMedusa({ live: false, enabled: false });
...
function getMasterStatus(options = {}) {
  ...
  return {
    ...
    settings: {
      ...
      medusaEnabled: settings.medusaEnabled,
      medusaWake: settings.medusaWake,
      enforcement,
      // Train 21 (#1589) — the Master pane's launch-sequence applicability,
      // RECORDED rather than merely absent.
      //
      // Master never calls `resolveApplicability`, so until now a reader asking
      // "does the Master pane get a launch sequence?" found nothing, and an
      // absence reads the same whether the answer is no or whether nobody ever
      // asked. That is the confusion this Train exists to remove, so the answer
      // is stated in the same `{applicable, reason}` shape a project launch uses.
      //
      // The reason is NOT about the engine. Master usually resolves an engine
      // that declares `launchSequence.supported: true` — the sequence does not
      // apply because Master is not a project session: it has no project, no
      // previous handoff to preflight, and no project rules to serve. Deriving
      // this from the engine would answer "applicable" for exactly the pane
      // where it is least true.
      launchSequence: {
        applicable: false,
        reason: 'the Master pane is not a project session — it has no project, no previous handoff to '
          + 'preflight and no project rules to serve, so there is no launch sequence to run whatever '
          + 'engine it resolves'
      },
    ...
    // The Master's switchboard listener, in the shape the project status route
    // returns, so the bar's Medusa control reads one contract on both surfaces.
    // `off` whenever there is no listener — including a live Master with the
    // setting off, which is the resting state of a fresh install.
    // `outbound` rides along so the bar's control can render a receive-only
    // Master with its reason from THIS fetch, rather than paying a second
    // request to `/api/master/medusa/status` for a verdict this payload already
    // has every input for.
    medusa: { ...getMasterMedusaStatus(), outbound: masterMedusaOutbound(settings.accessLevel) }
  };
}

/**
 * The Master's awareness state, composed at read time (#1141) — the same
 * no-persisted-state rule as `store.awarenessReceipts.sessionAwareness`, with
 * the Master's own facts: the live pane (tmux), the identity file on disk
 * (its one carrier), and role='master' receipts scoped to the CURRENT pane's
 * start time, so a receipt from a previous master run cannot confirm this one.
 *
 * Vocabulary matches the fleet view, plus the two states only the Master has:
 *   - `not-running` — no live pane: nothing launched, nothing to be aware.
 *   - `unknown`     — tmux could not answer; said so rather than guessed.
 *   - `confirmed`   — ≥1 master receipt since the current pane started.
 *   - `sent`        — pane live, identity file present: the carrier was
 *                     written; nothing was demonstrated.
 *   - `unaware`     — pane live, identity file MISSING — the severed carrier.
 *
 * @param {object} [options] - `tmuxLib` injectable like the other entry points
 * @returns {{state: string, basis: string, receiptCount: number,
 *   lastVerb: string|null, lastReceiptAt: string|null, startedAt: string|null}}
 */
function getMasterAwareness(options = {}) {
  const t = options.tmuxLib || tmux;
  const home = options.home || masterHome();
  ...
  if (!probe.answered) {
    return {
      state: 'unknown',
      basis: `tmux did not answer (${probe.cause || 'no cause recorded'}) — whether a Master is running could not be established`,
      ...none, startedAt: null
    };
  }
  if (!probe.live) {
    return {
      state: 'not-running',
      basis: 'no Master session is running — nothing launched means nothing to be aware',
      ...none, startedAt: null
    };
  }
  const started = t.sessionCreatedAt(MASTER_TMUX_SESSION, { probe });
  // A live pane whose start time could not be read still gets scored — with
  // an UNSCOPED receipt read, said in the basis, rather than a fabricated
  // scope or a refusal to answer.
  const sinceUtc = started.createdAt
    ? new Date(started.createdAt * 1000).toISOString().slice(0, 19).replace('T', ' ')
    : null;
  const startedAt = started.createdAt ? new Date(started.createdAt * 1000).toISOString() : null;
  const receipts = store.awarenessReceipts.listForMaster(sinceUtc);
  if (receipts.length > 0) {
    return {
      state: 'confirmed',
      basis: `the Master invoked tc itself (${receipts.length} receipt${receipts.length === 1 ? '' : 's'}${sinceUtc ? ' since this pane started' : ', pane start time unreadable so ALL master receipts counted'}, last verb '${receipts[0].verb}')`,
      receiptCount: receipts.length,
      lastVerb: receipts[0].verb,
      lastReceiptAt: receipts[0].createdAt,
      startedAt
    };
  }
  if (fs.existsSync(masterIdentityPath(home))) {
    return {
      state: 'sent',
      basis: 'the identity file (the Master\'s one carrier) is on disk but the Master never demonstrated awareness',
      ...none, startedAt
    };
  }
  return {
    state: 'unaware',
    basis: 'the Master is running with NO identity file on disk and no tc invocation — there is no evidence awareness ever arrived',
    ...none, startedAt
  };
}
...
module.exports = {
  ...
  MASTER_MEDUSA_KEY,
  MASTER_MEDUSA_NAME,
  MASTER_MEDUSA_API_BASE,
  masterMedusaTarget,
  masterMedusaOutbound,
  getMasterMedusaStatus,
  syncMasterMedusa,
  resyncMasterMedusa,
  masterLiveness,
  masterWakeRecord,
