...
function _masterRuntime(config) {
  ...
  const engineId = settings.engine || engines.resolveDefaultEngine(config);
...
function ensureMasterSession(options = {}) {
  ...
  if (!engineId) {
    ...
    // engine installed.
    return { created: false, ...base, error: 'No AI engine is installed — install one (Claude Code, Codex, Gemini CLI, …) before starting the Project Master' };
  }
