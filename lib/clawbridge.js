 ...
 * issue #364 needs. The `ai-content`-over-gateway *consumer* is deferred
 * pending the capture-back design (see `.prawduct/artifacts/cc-7-degraded-wrap.md`
 * Slice B spike: the remote AI's `captureFile` lands on a different filesystem
 * and the PTY output renders `##` away).
 *
 ...
 *
 * CC-7 Slice B foundation (the gateway analog of `tmux.sendKeys`). The
 * `ai-content`-over-gateway consumer is deferred pending the capture-back
 * design (see `.prawduct/artifacts/cc-7-degraded-wrap.md` Slice B spike).
 *
 * @param {object} opts
...
module.exports = { startSession, send, getOutput, getStatus, _requestJson, _queryString };
