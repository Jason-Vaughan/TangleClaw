...
const { requestOnce, ABORT_REASONS } = require('./http-settle');
...
async function startSession(opts) {
  ...
  try {
    res = await requestOnce(
      { host: '127.0.0.1', port: localPort, path: '/v2/session/start' },
      { method: 'POST', headers, body, timeoutMs }
    );
  } catch (err) {
    return { ok: false, sessionId: null, attached: false, status: 0, error: _terminalError(err, timeoutMs) };
  }
  ...
  if (status >= 200 && status < 300) {
    ...
    return { ok: true, sessionId, attached, status, error: null };
  }

  const message = (parsed && (parsed.error || parsed.message)) || `bridge returned status ${status}`;
  return { ok: false, sessionId: null, attached: false, status, error: message };
}

/**
 * Render a `requestOnce` rejection as this module's caller-facing error string.
 *
 * A timeout keeps its long-standing bridge-specific wording (operators read it
 * in wrap logs and it names the budget that was exceeded); every other
 * termination — a socket dying mid-body, a refused connect — carries the
 * underlying message, which is more specific than anything synthesized here.
 *
 * @param {Error & {reason?: string}} err - The rejection from `requestOnce`.
 * @param {number} timeoutMs - The budget that was in force, for the timeout wording.
 * @returns {string}
 */
function _terminalError(err, timeoutMs) {
  return err.reason === ABORT_REASONS.TIMEOUT
 ...
 */
async function _requestJson(opts) {
  ...
  try {
    res = await requestOnce(
      { host: '127.0.0.1', port: localPort, path },
      { method, headers, body: payload, timeoutMs }
    );
  } catch (err) {
    return { ok2xx: false, status: 0, parsed: null, error: _terminalError(err, timeoutMs) };
  }
  ...
  try { parsed = raw ? JSON.parse(raw) : null; } catch (_) { /* non-JSON response */ }
  const status = res.status;
  return { ok2xx: status >= 200 && status < 300, status, parsed, error: null };
}
