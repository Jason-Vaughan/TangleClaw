...
/**
 * The checkout this server process runs from. Taken from `lib/behind-origin.js`
 * rather than computed again, so the local read and the origin observation it
 * is joined with can never name two different checkouts; a moved runtime
 * (#1672) changes it there. (`lib/server-info.js` computes the same directory
 * for the runtime-vs-disk check, which is a separate question.)
 */
const LIVE_INSTALL_ROOT = require('./behind-origin').REPO_ROOT;
 ...
 * One deliberate exception to "both halves": when the operator has turned the
 * origin check off, the checkout can still be `ok`, carrying an `info` finding
 * that the relation is not measured. That was their choice, and a permanent
 * "unknown" banner would only teach them to ignore it.
 *
...
function assess(local, origin) {
  ...
  const originFindings = _originFindings(local, origin);
  let localFindings = _localFindings(local);
  // A branch tracking the very ref the observation measured would report the
  // same commits twice — once from the status header's last-fetch counts, once
  // from the observation. The observation is the fresher one, so it speaks
  // whenever it could be used for this HEAD.
  const measuredHere = origin && (origin.evidence === 'fresh' || origin.evidence === 'stale')
    && origin.headSha === local.headSha;
  if (measuredHere && local.upstream === origin.upstreamRef) {
    localFindings = localFindings.filter((f) => f.code !== 'unpushed');
  }
  const findings = [...localFindings, ...originFindings];
  let status = 'ok';
...
/** @type {Map<string, string>} repoRoot → its resolved forms, computed once. */
const _rootForms = new Map();

/**
 * The spellings a root may be registered under: as given, and with symlinks
 * resolved. Computed once per root — the root is this server's own directory,
 * so resolving it is a read of a path the process is already running from.
 *
 * @param {string} repoRoot
 * @returns {string[]}
 */
function _forms(repoRoot) {
  if (!_rootForms.has(repoRoot)) {
    const given = path.resolve(repoRoot);
    let real = given;
    try { real = fs.realpathSync(given); } catch { /* keep the given form */ }
    _rootForms.set(repoRoot, [given, real]);
  }
  return _rootForms.get(repoRoot);
}

/**
 * Whether `projectPath` is the live install's checkout.
 *
 * A string comparison against the root's own resolved forms. The project path
 * itself is never resolved: it is operator-chosen, this runs on the request
 * path and at every launch, and a filesystem call on a path macOS privacy
 * protection holds can block the whole event loop. So a project registered
 * through a symlink the root does not itself resolve through is not recognised
 * — a missed banner line, not a hung server.
 *
...
function isLiveInstall(projectPath, repoRoot) {
  ...
  return _forms(repoRoot).includes(path.resolve(projectPath));
}
...
function liveInstallSnapshot(config) {
  ...
  // Keep the origin half moving on its own: an expired observation starts one
  // background fetch here, whichever surface is asking, so a finding that
  // says "a refresh has been started" is true on every path. Never waits.
  behindOrigin.snapshot(config);
  const snap = snapshot(LIVE_INSTALL_ROOT, behindOrigin.observation(config));
  // HEAD moved since the observation: re-measure the relation against the
  // origin/main already fetched (local git only, no network), so "being
  // re-measured" is also true rather than waiting out the fetch TTL.
  if (snap.findings.some((f) => f.code === 'origin-outdated')) behindOrigin.remeasureRelation();
  return snap;
}

/**
 * Warm the live install's local read before a launch renders its prime, and
 * start (never await) the origin observation. Only for the live install's own
 * project; resolves without doing anything for any other. Never rejects.
 *
 * @param {string} projectPath - The launching project's path.
 * @param {object|null|undefined} config - Loaded global config.
 * @returns {Promise<boolean>} Whether this was the live install.
 */
async function warmForLaunch(projectPath, config) {
  if (!isLiveInstall(projectPath, LIVE_INSTALL_ROOT)) return false;
  await refresh(LIVE_INSTALL_ROOT);
  require('./behind-origin').snapshot(config);
  return true;
}
