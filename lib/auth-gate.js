...
// opens the door. See `resolveGateState`.
 ...
 *
 * Only `open` lets a request through without a question. The other four all
 * ENFORCE; they differ in what a refused person is shown.
 */
...
// The one Caddy exemption TangleClaw's own gate does NOT honour.
//
// `caddy.AUTH_BYPASS_PATHS` exempts `/openclaw-direct/*` from `basic_auth`, and
// the reason it records is that the gateway "enforces its own token auth". That
// is not true of the path as built: `server.js#_openclawProxyHeaders` and
// `#_openclawWsRequestLines` strip the caller's `Authorization` and INJECT the
// stored gateway token, so the gateway's check is satisfied by TangleClaw on
// behalf of whoever asked. What Caddy's exemption actually fixes is a prompt
// loop — the gateway UI's own `Authorization` header displaces the browser's
// cached Basic credential (#472). A session cookie does not ride
// `Authorization`, so this gate has no such loop and no reason to wave the path
// through. Caddy keeps its exemption for as long as `basic_auth` is up.
//
// A PREFIX on the canonical path, never a match on the raw target, for the same
// reason `isLoginSurfacePath` canonicalises: `//openclaw-direct/x` and
// `/openclaw-direct%2Fx` are this path to the canonicaliser Caddy's bypass uses.
const GATE_DOES_NOT_BYPASS_PREFIX = '/openclaw-direct/';

/**
 * Whether TangleClaw's gate waves a path through unauthenticated.
 *
 * Caddy's exemption list, reused rather than restated (ADR 0015: "one
 * definition, moved, not duplicated") — minus `/openclaw-direct/*`, for the
 * reason argued at {@link GATE_DOES_NOT_BYPASS_PREFIX}. Subtractive by
 * construction: this can never exempt a path Caddy's list does not.
 *
 * @param {string} rawUrl - The raw request target (`req.url`)
 * @returns {boolean}
 */
function isGateBypassPath(rawUrl) {
  if (!caddy.isCaddyAuthBypassPath(rawUrl)) return false;
  return !caddy.caddyCanonicalPath(rawUrl).startsWith(GATE_DOES_NOT_BYPASS_PREFIX);
}
 ...
 * reads successfully stays the recovery lever, because that is a successful
 ...
 * `stat`. The earlier dormant predicate skipped the config read when no account
 * existed; that is no longer possible, because an install with no accounts is
 * now either `open` or `account-required`, and only `authEnabled` says which.
 *
 ...
 */
function resolveGateState(loadConfig, sessions) {
  let config;
  try {
    config = loadConfig();
  } catch (err) {
  ...
  if (!config || config.authEnabled !== true) return GATE_STATES.OPEN;
 ...
 *      one carrying `header_up`. That holds while no `trusted_proxies` is
 *      configured, which is what would make Caddy keep a client's value; the
 *      generator emits none. The fleet (`bin/tc`, PortHub, the switchboard)
 *      never sends the header. A local process tunnelling remote traffic without
 *      adding it (an `ssh -L` forward) is treated as local, which is right only
 *      because holding such a tunnel already means holding a shell here;
 ...
 *   1. Gate `open` → allow. Nothing below can run on an install with no door.
 *      Any other value, including one this module does not know, enforces.
 ...
 *      `account-required` (see {@link ACCOUNT_SETUP_PATH}).
...
function evaluate({ method, rawUrl, pathname, gateState, session, submittedCsrf, machineClient }) {
  if (isOpen(gateState)) return { action: 'allow' };
  ...
  // Caddy's exemptions minus the one this gate cannot honour — see
  // `isGateBypassPath`. Built on `caddy.isCaddyAuthBypassPath`, the function
  // `server.js`'s fail-closed parity guard also uses, so the two cannot drift
  // into disagreeing about how a path is canonicalised.
  if (isGateBypassPath(rawUrl)) return { action: 'allow' };
  if (isLoginSurfacePath(rawUrl)) return { action: 'allow' };
  const accountRequired = gateState === GATE_STATES.ACCOUNT_REQUIRED;
  if (accountRequired && isAccountSetupPath(rawUrl)) return { action: 'allow' };
 ...
 * the HTTP exemptions — the bypass list, the login surface and the
 * first-account route — and none of them is a WebSocket route. Routing the
 ...
 *
 * Order:
 *   1. Gate `open` → allow, by {@link isOpen}: on the path to a shell, a value
 *      that is not a definite "open" must not read as one.
