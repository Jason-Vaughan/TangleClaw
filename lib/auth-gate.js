 ...
 * WHOEVER REACHES IT — while the install has never had an account. Anyone who
 * can reach the screen then could already reach this install's dashboard
 * ungated, or had already passed Caddy's gate in front of it. Reach is exactly
 * the authority the operator already had, which is ADR 0016's argument, and it
 * holds for a direct-mode install with a wide bind too. No code path deletes a
 * user row, but a database can still be lost outside the code, and by then the
 * Caddyfile may carry no gate of its own. So once
 * `store.users#accountsEstablished` says an account existed, the first-account
 * route takes a claim only from this machine (`server.js`, set-password). A
 * delete verb added to the store must answer the same question first.
 *
 ...
 * the file on disk is observably not such a door (`loadIngress`, answered by
 * `lib/ingress-door.js`): Caddy's parser reads the file and every site that
 * serves beyond `localhost` passes `basic_auth` before it forwards, or the file
 * does not exist. Otherwise the
 * accounts decide, exactly as if `authEnabled` were on. One case in between: a
 * file whose only door is a `localhost` site with neither `basic_auth` nor the
 * peer guard (`unguardedLocalSite`) — local in name, reachable by any machine
 * that asks for `localhost` — keeps the opt-out on an install with no account,
 * and closes it on one with accounts, where the accounts then decide. This is ADR 0016's rule
 ...
 *   - `store.authSessions`
 * @param {(() => { ungatedRemoteSite: boolean, unguardedLocalSite: boolean })|null} [loadIngress] -
 *   Thunk describing the Caddyfile on disk, asked only in caddy mode with
 *   `authEnabled` off. Expected to THROW when the file exists but cannot be
 *   read, which enforces; an answer missing either boolean enforces too.
 ...
 * file and agrees.
 *
 * The failure directions of `resolveGateState` that apply are kept: an
 * unreadable config answers `unreadable`, which keeps Caddy's gate, and so does
 * an unreadable store while `authEnabled` is on. With `authEnabled` off this
 * answers `open` without asking the store at all — there is no Caddyfile to
 * weigh here, so nothing makes the accounts decide — and a store fault cannot
 * change that answer.
 *
