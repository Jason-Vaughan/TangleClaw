 ...
 *
 * ⚠ CHUNK 04 MUST REVISIT THIS. The carve-out is sound only where something
 * else fronts the remote path — Caddy's `basic_auth` does for every path EXCEPT
 * its bypass list, and one of those, `/openclaw-direct/*`, is a proxy that
 * injects the gateway token: an off-box, non-browser request to it passes Caddy
 * and arrives here in this exact shape, so on that path the carve-out is
 * already the only thing deciding (bounded by the connection's UUID). Once #1420 removes it, a loopback socket no longer implies a local
 ...
 * the HTTP exemptions — the bypass list and the login surface — and none of
 * them is a WebSocket route. Routing the handshake through them would mean a
 * future addition to either list silently becomes a way onto a socket that, for
 * `/terminal/*`, is a `--writable` shell. So this takes no path at all: an
 * upgrade is allowed for who is asking, never for where it is going.
 *
 ...
 *
 * Order:
 *   1. Gate not active → allow. Compared with `=== false`, not `!gateActive`:
 *      on the path to a shell, a value that is not a definite "off" must not
 *      read as one.
 *   2. A machine client → allow, by the same `isMachineClient` predicate HTTP
 *      uses, so the two transports share one carve-out and one revisit.
 ...
 */
function evaluateUpgrade({ gateActive, session, machineClient }) {
  if (gateActive === false) return { action: 'allow' };
  if (machineClient === true) return { action: 'allow' };
  if (session) return { action: 'allow' };
