 ...
 *
 * Asked by the Caddyfile generator (whether `basic_auth` is still needed, and
 * whether a remote site may be emitted without it), the drift check, and the
 * bind policy. One definition, so none of them can drop a gate on a state the
 * others would keep it for.
 *
 * @param {string} gateState - From {@link resolveGateState}
 * @returns {boolean}
 */
