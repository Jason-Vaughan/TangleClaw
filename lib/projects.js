...
async function listAllProjects(options = {}) {
  ...
  let entries;
  try {
    entries = await _withTimeout(
      fsp.readdir(projectsDir, { withFileTypes: true }),
 ...
 */
async function _scanDirectoryEntries(dir) {
  ...
  for (const entry of entries) {
    ...
    // git.getInfo shells out synchronously. Each call is capped by lib/git's own
    // 5s execSync timeout, so it cannot wedge the loop the way an unanswered
    // open() does, but it does block for as long as it runs — which is why it
    // stays inside the deadline below rather than being treated as free.
    let gitInfo = null;
    try {
      gitInfo = git.getInfo(dirPath);
    } catch {
    ...
    const markerHits = await Promise.all(
      PROJECT_MARKERS.map(m => _exists(path.join(dirPath, m)))
    );
    ...
    detected.push({
      ...
      detected: !!((gitInfo && gitInfo.branch) || hasTangleclawConfig || markerHits.some(Boolean))
    });
  }
...
async function scanDirectoryForProjects(directory) {
  ...
  try {
    ...
    return {
      ok: false,
      code: 'SCAN_FAILED',
      error: `Could not read ${directory}: ${err && err.message}${hint ? ` — ${hint}` : ''}`
    };
  }
}
