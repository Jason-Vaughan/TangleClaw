...
function createProject(data) {
  ...
  // The helper logs a warning if the existing on-disk file differs from
  // what we're about to write (surfaces silent-clobber bugs). For
  // createProject the project directory is fresh, so drift should never
  // fire here — but we route through the helper for uniformity with
  // the other three write sites and so a future caller that writes
  // into an existing directory automatically benefits.
  const writeResult = engines.writeEngineConfig(engineId, projectPath, projectConfig, engineProfile, methodologyTemplate);
  if (writeResult.error && !writeResult.written) {
...
function updateProject(name, updates) {
  ...
  if (updates.engine && updates.engine !== project.engineId) {
    ...
    // operator hand-edits being lost during engine switches.
