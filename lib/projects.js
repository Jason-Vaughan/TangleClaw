 ...
 * cost of a list is the slowest single directory rather than their sum. That
 * caller is also the only one that reads on the POLLED scanner; an omitted
 ...
 * @param {{exists: boolean, governanceState: string, unreadable: string|null,
...
async function listProjects(options = {}) {
  ...
  const facts = await Promise.all(
    projects.map((p) => readProjectFacts(p, { interactive: false })));
