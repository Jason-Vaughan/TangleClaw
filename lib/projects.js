...
function enrichProject(project) {
  ...
  try {
    ...
    versionFilePath = typeof projConfig.versionFilePath === 'string' && projConfig.versionFilePath.trim() !== ''
      ? projConfig.versionFilePath.trim()
      : null;
...
function updateProject(name, updates) {
  ...
  if (updates.versionFilePath !== undefined && updates.versionFilePath !== null) {
    ...
    const v = updates.versionFilePath.trim();
    // Resolution-based, matching what `version-bump` enforces at the write site.
    // A lexical `..` scan would disagree with it in both directions: it rejects
    // `a/../b.json` that the write site accepts, and it accepts `.`, which the
    // write site then refuses forever — a setting that saves cleanly and
    // silently skips every wrap.
    if (v !== '') {
      const resolved = path.resolve('/project-root', v);
      const rel = path.relative('/project-root', resolved);
      if (path.isAbsolute(v) || rel === '' || rel.startsWith('..')) {
        return { project: null, methodologySwitch: null, errors: ['versionFilePath must be a relative path to a file inside the project (no absolute paths, no "..", not the project root itself)'] };
      }
    }
  }
