 ...
 *
 * @param {string} directory - Operator-supplied path, `~` allowed.
 * @returns {{ok: true, path: string, created: boolean}|{ok: false, code: string, error: string}}
 */
function createProjectsDir(directory) {
  const dir = resolveProjectsDir(directory);
  ...
  try {
    ...
    return {
      ok: false,
      code: 'CREATE_FAILED',
      error: `Could not create ${directory}: ${err.message}`
    };
  }
