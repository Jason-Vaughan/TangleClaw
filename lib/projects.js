...
function updateProject(name, updates) {
  ...
  if (updates.activePlan !== undefined
    ...
    if (ap.includes('/') || ap.includes(path.sep) || path.isAbsolute(ap)) {
      return { project: null, methodologySwitch: null, errors: ['activePlan must be a bare plan filename under .claude/plans/ (no path separators)'] };
    }
    if (!ap.endsWith('.md') || !fs.existsSync(path.join(project.path, '.claude', 'plans', ap))) {
      return { project: null, methodologySwitch: null, errors: [`activePlan "${ap}" not found under .claude/plans/`] };
    }
  }
