 ...
 *
 * - `{step, since}` — a run is going, with the step it is on and when it began.
 ...
 * @returns {false|null|{step: string|null, since: number|null}}
 */
function _wrapState(projectName) {
  try {
    const run = _internal.wrapRun(projectName);
    if (!run || !run.running) return false;
    return { step: run.currentStepId || null, since: run.startedAt || null };
  } catch (err) {
 ...
 * @param {false|null|{step: string|null, since: number|null}} wrapping - This
 ...
 *   wrapping: false|null|{step: string|null, since: number|null}}}
 */
