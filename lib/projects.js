 ...
 * comment inline for why registering it is worse than merely offering it.
 *
...
function detectExistingProjects() {
  ...
  for (const entry of entries) {
    ...
    if (hasTangleclawConfig || hasPrawductDir) {
      ...
      // markers — so a scan would hand back the tool itself as a candidate
      // project. Worse than #708's pre-checked wizard box: this path registers
      // what it finds, writing `.tangleclaw/project.json` into the clone, which
      // dirties it and strands the self-updater behind its dirty-tree guard.
      //
      ...
      if (realDirPath === OWN_INSTALL_REALPATH) {
