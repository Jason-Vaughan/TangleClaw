...
function updateProject(name, updates) {
  ...
  if (updates.wrapStepOverrides !== undefined) {
    // Resolve the project's pipeline so a step that must not be disabled is
    // refused here rather than only at wrap time. Advisory: an unreadable
    // template still validates shape and fields, and the runner re-checks.
    let pipelineSteps;
    try {
      const methodologyId = updates.methodology || project.methodology;
      const template = methodologyId ? store.templates.get(methodologyId) : null;
      if (template && template.wrap_pipeline && Array.isArray(template.wrap_pipeline.steps)) {
        pipelineSteps = template.wrap_pipeline.steps;
      }
    } catch (err) { // prawduct:allow prawduct/broad-except -- an unreadable template must not block a settings save; the runner still enforces, and the failure is reported
      log.warn('methodology template unreadable — validating wrap step overrides without it', {
        project: project.name,
        error: err.message
      });
    }
    const verdict = wrapStepOverrides.validateOverrides(updates.wrapStepOverrides, pipelineSteps);
    if (!verdict.ok) {
