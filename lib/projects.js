  ...
  for (const project of allProjects) {
    try {
      ...
      for (const basename of HOOK_SETTINGS_BASENAMES) {
        const settingsPath = path.join(project.path, '.claude', basename);
        if (!fs.existsSync(settingsPath)) continue;
        let settings;
        try {
          settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
        } catch (err) {
          result.errors.push({ name: project.name, error: `Failed to parse .claude/${basename}: ${err.message}` });
          continue;
        }
          ...
          for (let i = 0; i < entries.length; i++) {
            ...
            if (missing.length > 0) {
              orphans.push({
                ...
                // the same event are indistinguishable, and the repair below could
                // not say which file it rewrote.
                file: basename,
...
function repairOrphanHooks(projectName = null) {
  ...
  for (const project of candidates) {
    try {
      ...
      const removed = [];
      let sawSettingsFile = false;
      let sawHooksBlock = false;
      for (const basename of HOOK_SETTINGS_BASENAMES) {
        const settingsPath = path.join(project.path, '.claude', basename);
        if (!fs.existsSync(settingsPath)) continue;
        ...
        try {
          ...
          result.errors.push({ name: project.name, error: `parse failed (${basename}): ${err.message}` });
          continue;
        }
        if (!settings || !settings.hooks || typeof settings.hooks !== 'object') continue;
        sawHooksBlock = true;
        ...
        // Only rewrite a file this pass actually changed. A project whose orphan is
        // in one file must not have the other rewritten for nothing — that is a
        // gratuitous mtime bump on the tracked, committable file.
