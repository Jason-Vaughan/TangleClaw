...
function enrichProject(project) {
  ...
  if (methodologyTemplate) {
    ...
    methodology = {
      ...
      // (e.g. prawduct's `Run Critic`) so the frontend can render them
      // in the session banner. Server-side dispatch is gated by the
      // template's `actions[]`, so leaking only label/command/confirm
      // is safe (the methodology authoritatively decides what's
      // dispatchable).
