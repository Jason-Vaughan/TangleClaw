...
function createProject(data) {
  ...
  // new project can't be created pointing at an engine that isn't on the machine
  // (the failure would otherwise surface at first launch, far from its cause).
  const engineId = data.engine || engines.resolveDefaultEngine(config);
  if (!engineId) {
    return { project: null, errors: ['No AI engine is installed — install one (Claude Code, Codex, Gemini CLI, …) before creating a project'] };
  }
...
function attachProject(name) {
  ...
  // default naming a binary this machine doesn't have.
  let engineId = engines.resolveDefaultEngine(config);
  ...
  if (!engineId) {
    // Nothing installed and the project carries no engine of its own. Refuse
    // rather than write a row naming an engine that doesn't exist here — the
    // DB column's 'claude' default would otherwise supply exactly that.
    return { project: null, errors: [...errors, 'No AI engine is installed — install one (Claude Code, Codex, Gemini CLI, …) before attaching a project'] };
  }
