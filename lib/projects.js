...
const launchAgentScan = require('./launchagent-scan');
const engineErrors = require('./engine-errors');
const { readProjectFacts } = require('./project-facts');
const {
  DEFAULT_PROJECT_CONFIG, RELEASE_MODES, resolveReleaseMode, nextReleaseMode,
  PASTE_RULES_MODES, RECOVERY_MODES, UNREADY_WINDOW_MIN_MINUTES, UNREADY_WINDOW_MAX_MINUTES
...
// Re-exported, not defined here. TangleClaw's own front door needs the same
// scrypt hash/verify this module used for the delete guard, and two copies of a
// hash function is a security defect rather than a duplication one. `lib/password.js`
// owns them; the names stay so every existing `projects.hashPassword` caller is
// unchanged.
...
// bullet's whole list item — continuation lines included — is preserved
// verbatim on refresh so descriptions survive (`_bulletItemEnd`, #1363).
...
/**
 * Where the markdown list item starting at `lines[start]` ends: the index of
 * the first line that is not part of it. A description that wraps is written
 * as continuation lines indented under the bullet, and those belong to the
 * item — reading only the bullet's own line is what cut wrapped descriptions
 * off mid-sentence (#1363). The item continues through:
 *
 * - any text line that does not open a new block at column 0: indented
 *   continuation text, a nested list, or a wrapped line written without
 *   indentation (which markdown also renders as part of the item);
 * - blank lines, but only when an indented line follows them, since a list
 *   item can hold more than one paragraph.
 *
 * It ends at the next list item, heading or horizontal rule, at a blank line
 * followed by an unindented line, or at the end of the file. Prose written
 * directly under a bullet with no blank line between is part of that bullet,
 * as markdown renders it, so it moves and goes with that directory. Trailing blank lines are never
 * part of the item, so the section's own spacing is not duplicated.
 *
 * @param {string[]} lines - The file, split on newlines
 * @param {number} start - Index of the item's `- ` line
 * @returns {number} Index one past the item's last line
 */
function _bulletItemEnd(lines, start) {
  let end = start + 1;
  while (end < lines.length) {
    const line = lines[end];
    // Any text that does not open a new top-level block at column 0. Indented
    // lines, including a nested list, never match those, so they continue too.
    // A `#1363`-style issue reference at the start of a wrapped line is text,
    // not a heading, and a `---` / `***` rule is a block of its own.
    if (line.trim() !== '' && !/^([-*+]|\d+[.)])\s/.test(line) && !/^#{1,6}(\s|$)/.test(line)
 ...
 * existing bullet is kept verbatim, continuation lines included), adding `<!-- describe -->` stubs for
 ...
 *
 * Only recognized dir bullets carry across, each as its whole list item: a
 * description that wraps onto further lines keeps them (`_bulletItemEnd`).
 * Prose in the Structure section that is not part of a dir bullet is NOT
 * preserved — it belongs in an operator-owned section, which the
 * section-scoped refresh leaves untouched.
 *
...
function _mergeStructureBody(existingContent, currentDirs) {
  ...
  if (start !== -1) {
    for (let i = start + 1; i < lines.length; i++) {
      ...
      const m = lines[i].match(PROJECT_MAP_DIR_LINE_RE);
      if (!m) continue;
      const end = _bulletItemEnd(lines, i);
      existingByDir[m[1]] = lines.slice(i, end).join('\n');
    }
  }
...
/**
 * Validate the SHAPE of a project's `tags` value.
 *
 * One predicate with two call sites, deliberately. #1338 exists because the
 * create and update paths held the same rule in two places and then disagreed:
 * `PATCH` refused a value that `POST` accepted, so a project could be created
 * with a `tags` value no edit could repair. A create-side copy of the update-side
 * rule would re-create exactly that, so both paths call this.
 *
 * The PER-ELEMENT rules — tag length, character set, a count cap, whether an
 * empty string is a tag — are deliberately still OPEN and are not invented
 * here; this is the shape floor and nothing more. They are tracked in #1374,
 * NOT in #1287, which raised them and is closed — a deferral whose only record
 * is a closed issue is how #1338 came to exist in the first place.
 *
 * #1374 also carries the structural half this does not do: `createProject`
 * still hand-copies four rules the update table owns, and running the table
 * itself is what stops the two drifting again.
 *
 * @param {unknown} tags - Candidate value for a project's tags.
 * @returns {string|null} Error message, or `null` when the shape is acceptable.
 */
function validateTagsShape(tags) {
...
function createProject(data) {
  ...
  // Launch posture (#626). Validated BEFORE the directory is created, like
  // every other create-time check above, so a refusal leaves nothing on disk.
  //
  // `updateProject` has enforced both of these since the settings existed; the
  // create path never has, so the one route that establishes a project's
  // posture was the one route that did not check it.
  if (data.showLaunchModePicker !== undefined && typeof data.showLaunchModePicker !== 'boolean') {
    return { project: null, errors: ['showLaunchModePicker must be a boolean'] };
  }
  // `silentPrime` was stored raw here while PATCH refused both of these. Every
  // reader tests `=== true`, so a stored `"true"` runs with silent prime OFF
  // against a shipped default of on — a project born quietly wrong.
  if (data.silentPrime !== undefined) {
    if (typeof data.silentPrime !== 'boolean') {
      return { project: null, errors: ['silentPrime must be a boolean'] };
    }
    if (data.silentPrime === true) {
      const fit = engines.settingDisposition('silentPrime', {}, engineProfile);
      if (!fit.applies) {
        return { project: null, errors: [`Cannot enable silentPrime — ${fit.reason}`] };
      }
    }
  }
  if (data.defaultLaunchMode !== undefined) {
    const fit = engines.settingDisposition('defaultLaunchMode',
      { defaultLaunchMode: data.defaultLaunchMode }, engineProfile);
    if (!fit.applies) {
      return { project: null, errors: [`Cannot set defaultLaunchMode — ${fit.reason}`] };
    }
  }
  // Eyes-open guard — the same one an edit gets, from the same function.
  // Creation is the one route it had never run on, and it is where the
  // combination is most likely to be established.
  const createGuard = bypassHiddenRefusal({
    show: data.showLaunchModePicker,
    mode: data.defaultLaunchMode,
    engineProfile,
    confirmed: data.confirmBypassHidden === true
  });
  if (createGuard) return { project: null, errors: [createGuard] };

  // #1338 — `createProject` wrote `tags` with no verdict at all, so the exact
  // shape `updateProject` refuses was reachable through `POST /api/projects`.
  // It is checked HERE, with the other create-time shape checks and before the
  // first `mkdirSync`, so a refusal leaves nothing on disk — the property the
  // checks above already hold.
  if (data.tags !== undefined) {
    const tagsError = validateTagsShape(data.tags);
    if (tagsError) return { project: null, errors: [tagsError] };
  }
  ...
  // #626: the pair of `defaultLaunchMode`, and the last of the settings the
  // issue calls first-session-load-bearing. Without it a project created with a
  // hidden picker had to be created, then edited, to reach the posture the
  // wizard was already asking about.
  if (data.showLaunchModePicker !== undefined) projectConfig.showLaunchModePicker = data.showLaunchModePicker;
  ...
  // openclaw) and is not a project-creation error, so only real `error`
  // strings reach the operator here. It is not swallowed either: the helper
  // reports the missing carrier itself.
...
/**
 * Injectable seams. The require is LAZY for the same reason `lib/medusa-wake.js`
 * keeps its own that way: `lib/sessions.js` already requires the registry at
 * module top and this module sits in the `projects -> sessions ->
 * project-version -> projects` cycle, where a module-top require has already
 * cost this codebase a partial-exports casualty. Lazy keeps the edge out of the
 * graph, and the indirection is what lets a test drive the three answers
 * `_wrapState` can give without standing up a real pipeline.
 */
const _internal = {
...
/**
 * This project's wrap state, as the run registry has it.
 *
 * Sourced from `lib/wrap-run-registry.js` rather than a database column
 * because a running pipeline lives in the registry and nowhere else: the
 * column that used to carry it could outlive the process that set it, which
 * is what made a restart leave a session wrapping forever (#1034).
 *
 * Four answers, and the differences between them are the point:
 *
 * - `{step, since, stale: false}` — a run is going, with the step it is on and
 *   when it began.
 * - `{step, since, stale: true}` — a run was claimed and never settled. Not
 *   folded into `false`: that is "no wrap is running", and a wedged pipeline is
 *   a fault worth seeing rather than a plausible absence to hide it behind
 *   (#1314). `step` is the step it wedged ON, which is the most useful thing
 *   the card can say about it.
 * - `false` — ESTABLISHED that none is. The registry is process-local, so an
 *   empty registry after a restart is the truth, not a gap.
 * - `null` — the read itself failed. Named in the payload's `incomplete`, never
 *   reported as `false`, because "no wrap" is an answer this did not get.
 *
 * @param {string} projectName - Registry key, which is the project's name.
 * @returns {false|null|{step: string|null, since: number|null, stale: boolean}}
 */
function _wrapState(projectName) {
  try {
    const run = _internal.wrapRun(projectName);
    if (!run || (!run.running && !run.stale)) return false;
    return {
      step: run.currentStepId || null,
      since: run.startedAt || null,
      stale: Boolean(run.stale)
    };
  } catch (err) {
    // Deliberately NOT fatal, and deliberately not `false`. `lib/medusa-wake.js`
    // reads the same registry and fails CLOSED — it assumes a wrap is running,
    // because withholding a keystroke is its safe error. This is a display, so
    // the safe error points the other way: `renderCard` shows no pinwheel for a
    // null, rather than painting one onto every card because one read threw.
    log.warn('Wrap-run registry unreadable — the card reports its wrap state as unknown', {
 ...
 * few lines below. `wrapping` obeys it as well: `false` is a real answer and is
 * always present, never omitted to mean "not wrapping".
 *
 * @param {object} row - Session row from the store.
 * @param {false|null|{step: string|null, since: number|null, stale: boolean}} wrapping - This
 *   project's wrap state from `_wrapState`; `null` means the read failed and is
 *   named in `incomplete`.
 * @returns {{active: true, status: string, startedAt: string, tmuxSession: string,
 *   sessionMode: string, incomplete: string[], cause: null, lastEngineError: object|null,
 *   wrapping: false|null|{step: string|null, since: number|null, stale: boolean}}}
 */
function _liveSession(row, wrapping) {
  return {
    active: true,
    status: row.status,
    startedAt: row.startedAt,
    tmuxSession: row.tmuxSession,
    // What a kill will tear down — a tmux pane or a webui SSH tunnel. The card's
    // Kill modal words its confirmation from this (#1311). Passed through as the
    // store gives it: `_rowToSession` owns the `'tmux'` default.
    sessionMode: row.sessionMode,
    // A wrap state we could not establish names itself here, the same way an
    // unestablished liveness names `active`. It is not folded into `false`:
    // that is the answer "no wrap is running", and a read that threw did not
    // give it.
    incomplete: wrapping === null ? ['wrapping'] : [],
    cause: null,
    // The engine's own last API error, as the wrap sentinel's pane scan saw
    // it (#261) — null on the healthy path, so the card can read the field.
    lastEngineError: engineErrors.get(row.id),
    wrapping
  };
}
 ...
 * @param {false|null|{step: string|null, since: number|null, stale: boolean}} wrapping - This
 *   project's wrap state from `_wrapState`. Carried here too: the registry read
 *   is independent of the tmux read and answers whether or not tmux did.
 * @returns {{active: null, status: string, startedAt: string, tmuxSession: string,
 *   sessionMode: string, incomplete: string[], cause: string|null,
 *   wrapping: false|null|{step: string|null, since: number|null, stale: boolean}}}
 */
function _unknownSession(row, cause, wrapping) {
  return {
    active: null,
    status: row.status,
    startedAt: row.startedAt,
    tmuxSession: row.tmuxSession,
    sessionMode: row.sessionMode,
    incomplete: wrapping === null ? ['active', 'wrapping'] : ['active'],
    cause: cause || null,
    // Carried even here, because the two reads are independent: the registry
    // is in THIS process and answers whether or not tmux did. The card still
    // renders the unknown glyph over the pinwheel — see `renderCard` — but the
    // payload does not throw away an answer it has.
    wrapping
  };
}
...
async function enrichProject(project, facts, context) {
  ...
  if (project.engineId && project.engineId.startsWith('openclaw:')) {
    ...
    if (conn) {
      // Projected through the one client-payload definition rather than
      // hand-built, so a field the browser reads cannot be present in the
      // engine roster and missing here — the drift that made the
      // generated-config row answer for the wrong engines. The connection owns
      // the identity; everything else is the base profile's.
      engine = engines.engineClientPayload(baseProfile || { id: 'openclaw' }, {
        id: project.engineId,
    ...
    if (engineProfile) {
      ...
      engine = engines.engineClientPayload(engineProfile, { available: det.available });
    }
  }

  // Session info — reported as live only when tmux confirmed the pane, and as
  // unknown when it could not be asked. A session mid-wrap is included: it holds
  // `active` for the whole of its wrap, which is why one lookup covers both.
  //
  // A wrap running over this session IS reported, sourced from
  // `lib/wrap-run-registry.js` — where a running pipeline actually lives —
  // rather than from the `wrapping` status column that used to carry it and
  // could outlive the process that set it (#1034). `_wrapState` explains the
  // answers it can give.
  //
  ...
  if (activeSession) {
    ...
    // One registry read for the branch, not one per arm: the answer is the
    // same either way, and `_wrapState` is the only thing here that can log.
    const wrapping = _wrapState(project.name);
    if (!verdict || verdict.names.has(activeSession.tmuxSession)) {
      session = _liveSession(activeSession, wrapping);
    } else if (!verdict.answered) {
      session = _unknownSession(activeSession, verdict.cause, wrapping);
    }
    // tmux answered and this pane is gone: don't report. The row is claimed by
    // an action — the session page's finalize, or the kill button — never by a
    // read (#910).
  }
  ...
  let evalAudit = { enabled: false, storedEnabled: false, openIncidents: 0, judgeModel: null, costCapPerSession: null };
  let silentPrime = false;
  let featureIndexEnabled = false;
  let projectMapEnabled = false;
  // #1492: whether the wrap may cut a release, resolved (legacy flag migrated).
  // `versionBumpEnabled` is its boolean shadow, reported for API callers that
  // still read the legacy key: on for every mode but `off`.
  let releaseMode = 'auto';
  ...
  // #1885: the opt-in provenance line. Default off, with the default template.
  let provenanceWatermark = { enabled: false, template: null, defaultTemplate: provenance.DEFAULT_TEMPLATE, warning: null };
  // #1708: whether a wrap that does not say inherits "keep the session
  // running". The pages pre-tick their keep checkbox from it.
  let wrapKeepSessionRunning = false;
  ...
  try {
    ...
    // Reported for every project, enabled or not (#1236): a modal cannot render
    // a toggle whose current value it cannot read, and `null` for "off" made
    // the setting invisible to the only surface that could turn it on. The
    // incident count stays behind the flag — it is a DB query per project on a
    // ten-second poll, and a disabled project has none by construction.
    const auditCfg = projConfig.evalAuditMode || {};
    // `enabled` reported here means "on AND able to do anything", because every
    // consumer treats it as exactly that: the dashboard lists the project, the
    // prime tells the agent it is being scored, the card paints a badge. A
    // project hand-edited to `true` on an engine no exchange can reach is on in
    // storage and inert in fact, and three surfaces answering that differently
    // is the incoherence this train exists to end. `storedEnabled` keeps the
    // raw value for the one surface that must see it — the settings modal,
    // which is where such a project gets switched back off.
    // `resolveProfile`, not the `engine` object built above: that one is the
    // API's shape (id, name, available, capabilities) and carries no
    // `launchModes`, so handing it to the mechanism works only for a row that
    // reads nothing but the id — which is the fabricated-profile trap this
    // chunk already had to close once.
    const auditFits = engines.settingDisposition('evalAuditMode', projConfig,
      engines.resolveProfile(project.engineId)).applies;
    evalAudit = {
      storedEnabled: auditCfg.enabled === true,
      enabled: auditFits && auditCfg.enabled === true,
      openIncidents: auditFits && auditCfg.enabled === true
        ? store.evalIncidents.countByStatus(project.name, 'open')
        : 0,
      // The two numbers the settings control needs to state the cost BEFORE
      // the switch is flipped, read from this project's own config rather than
      // hardcoded in the browser — an operator who has edited them must see
      // what they set, not what ships.
      judgeModel: auditCfg.judgeModel || null,
      costCapPerSession: typeof auditCfg.costCapPerSession === 'number' ? auditCfg.costCapPerSession : null
    };
    ...
    releaseMode = resolveReleaseMode(projConfig).mode;
    ...
    // The stored override, not the resolved line: the modal edits what is
    // stored, and one a hand edit made invalid is shown so it can be fixed,
    // with the warning that says the default renders instead.
    const resolvedProvenance = provenance.resolveProvenance(projConfig);
    const storedProvenance = projConfig.provenanceWatermark;
    provenanceWatermark = {
      enabled: resolvedProvenance.enabled,
      template: resolvedProvenance.templateSource === 'default' ? null
        : String(storedProvenance.template).trim(),
      defaultTemplate: provenance.DEFAULT_TEMPLATE,
      warning: resolvedProvenance.warning || null
    };
    wrapKeepSessionRunning = projConfig.wrapKeepSessionRunning === true;
  ...
  return {
    ...
    // Stranded-wrap counts for the card badge (#1541); `blocking` is what holds a
    // launch. The items themselves are one request away, on the stranded-wraps
    // route. A failed read is `null` with the reason, never a zero.
    ...strandedSummary(project),
    // Whether the last session was killed or crashed and left work in the
    // checkout (#1544). Null when a session is active or the last one wrapped.
    // The answer is read in the background, so a first read says `checking`.
    sessionHealth: sessionHealthSummary(project, facts)
  };
}

/**
 * The project list's `sessionHealth`. Never throws: a store that cannot be
 * read is reported as an unknown answer with the reason, never as null, which
 * would read as "nothing to report".
 * @param {object} project - Project record with `id`, `name` and `path`
 * @param {{exists?: boolean, unreadable?: string|null}} [facts] - The scanner's
 *   facts, so a folder it could not find or read is not spawned into
 * @returns {object|null} `lib/session-leftovers.js#read`, or an unknown answer
 */
function sessionHealthSummary(project, facts) {
  try {
    return sessionLeftovers.read(project, facts ? { exists: facts.exists !== false, unreadable: facts.unreadable || null } : null);
  } catch (err) { // prawduct:allow prawduct/broad-except -- the project list renders whatever the store throws, and reports the answer as unknown
    const message = err && err.message ? err.message : String(err);
    log.warn('Last-session state could not be read for the project list', { project: project.name, error: message });
    return {
      scope: 'session', sessionId: null, status: null, endedAt: null, state: 'unknown', checkedAt: null,
      reason: `the session record could not be read: ${message}`,
...
/**
 * A project's stranded-wrap counts for the project list. Never throws: the list
 * must render whatever the store does, and a count that could not be read is
 * reported as unknown rather than as zero.
 * @param {object} project - Project record with `id` and `name`
 * `stranded.github` carries the latest GitHub check's counts (#1542, #1543);
 * the findings themselves are on the stranded-wraps route.
 * @returns {{stranded: {total: number, unacknowledged: number, grandfathered: number, blocking: number,
 *   github: object}|null, strandedError: string|null}}
 */
function strandedSummary(project) {
  try {
    const counts = strandedWraps.counts(strandedWraps.list(project).items);
    return {
      stranded: { ...counts, github: strandedCheck.summary(strandedCheck.status(project)) },
      strandedError: null
    };
  } catch (err) { // prawduct:allow prawduct/broad-except -- the project list renders whatever the store throws, and reports the count as unknown
    const message = err && err.message ? err.message : String(err);
    log.warn('Stranded-wrap counts could not be read for the project list', { project: project.name, error: message });
    return { stranded: null, strandedError: message };
  }
}
...
function syncAllProjects() {
  ...
  // #1022 — retire any TangleClaw hook still sitting in the tracked, shared
  // `.claude/settings.json`, for EVERY project including ARCHIVED ones.
  //
  // Its own pass rather than a line in the loop below, because that loop reads
  // `store.projects.list()`, which excludes archived projects — so a sweep inside
  // it could not reach the projects that need it most. The relocation retires
  // from the old file on each sync, but a sync only happens on launch, create,
  // attach, PATCH, or a governed project's boot. A project that is archived, or
  // simply never opened again, would keep an entry naming an absolute path to one
  // install in a file its clones are meant to commit, permanently — and
  // `scanForOrphanHooks` cannot backstop it: on this machine that path still
  // resolves, so it is not an orphan by that predicate, and that scan filters
  // archived projects out too.
  //
  // This repo already ruled the same way once, under #247 — filtering on
  // `{archived: false}` would leave orphan hooks on archived projects — so this is
  // consistency rather than new policy. Retire-only, so it can never install a
  // hook into a project that should not have one; an existsSync plus a parse per
  // project, and `_retireHooksIn` writes only when it actually removes something.
  for (const project of store.projects.list({ archived: true })) {
    if (!project.path) continue;
    try {
      engines._retireHooksIn(path.join(project.path, '.claude', engines.SHARED_SETTINGS_BASENAME));
    } catch (err) {
      // A migration sweep must never be the thing that stops a boot.
      log.warn('Could not retire legacy hooks from the tracked settings file', {
...
//
// Bulk-repair pathway for projects whose .claude hook settings files have entries
...
// Both files Claude Code loads hooks from, in the order it merges them. The scan
// and the repair below read every one, because an orphaned hook is orphaned
// wherever it sits: the operator may write to either, and TangleClaw's own entries
// live in `settings.local.json` since they name an absolute install path (#1022).
// Scanning only the shared file would have gone quiet about exactly the hooks this
// scanner exists to catch — TangleClaw's, the ones whose script paths move.
const HOOK_SETTINGS_BASENAMES = [
  engines.SHARED_SETTINGS_BASENAME,
  engines.LOCAL_SETTINGS_BASENAME
];

/**
 * Classify one project's hook files, yielding the orphans found in each.
 *
 * The single owner of "which files hold hooks, how they are read, and what an
 * orphan record looks like". Both `scanForOrphanHooks` (read-only) and
 * `repairOrphanHooks` (rewrites) call it, so the pair cannot disagree about what
 * counts as an orphan — the failure #145 exists to prevent is a scan that reports
 * one the repair then declines to remove, leaving the operator's only non-launch
 * escape from an infinite hook-failure loop pointing at nothing.
 *
 * Yields one entry per file that exists and parses, whether or not it holds
 * orphans, because the callers' skip reasons need to tell "no file" from "a file
 * with nothing wrong in it".
 * @param {string} projectPath - Absolute path to the project root.
 * @returns {{ files: Array<{basename: string, settingsPath: string, settings: object,
 *   orphans: Array<{event: string, index: number, file: string, matcher: string|undefined,
 *   missing: string[], commands: string[]}>}>, errors: string[] }}
 */
function _classifyHookFiles(projectPath) {
  const files = [];
  const errors = [];
  for (const basename of HOOK_SETTINGS_BASENAMES) {
    const settingsPath = path.join(projectPath, '.claude', basename);
    if (!fs.existsSync(settingsPath)) continue;
    let settings;
    try {
      settings = JSON.parse(fs.readFileSync(settingsPath, 'utf8'));
    } catch (err) {
      errors.push(`Failed to parse .claude/${basename}: ${err.message}`);
      continue;
    }
    const orphans = [];
    if (settings && settings.hooks && typeof settings.hooks === 'object') {
      for (const [eventName, entries] of Object.entries(settings.hooks)) {
        ...
        for (let i = 0; i < entries.length; i++) {
          ...
          const missing = _hookEntryOrphanMissing(entry, projectPath);
          if (missing.length > 0) {
            orphans.push({
              event: eventName,
              index: i,
              // Which file the entry is in. Without it two orphans at index 0 of
              // the same event are indistinguishable — and that became the common
              // case when TangleClaw's own entries moved to a different file from
              // the operator's (#1022).
              file: basename,
    ...
    files.push({ basename, settingsPath, settings, orphans });
  }
  return { files, errors };
}
 ...
 * `.claude/settings.json` or `.claude/settings.local.json` that reference
 * `$CLAUDE_PROJECT_DIR/<path>` targets that don't exist. Read-only: does not
 * write. Returns inventory; each orphan names the `file` it was found in.
 *
 * @returns {{
 *   scanned: number,
 ...
 *     name: string,
 *     path: string,
 *     orphans: Array<{ event: string, index: number, file: string, matcher: string|undefined, missing: string[], commands: string[] }>
 *   }>,
...
function scanForOrphanHooks() {
  ...
  for (const project of allProjects) {
    try {
      if (!project.path || !fs.existsSync(project.path)) continue;
      result.scanned++;
      const classified = _classifyHookFiles(project.path);
      for (const error of classified.errors) {
        result.errors.push({ name: project.name, error });
      }
      const orphans = classified.files.flatMap((f) => f.orphans);
      if (orphans.length > 0) {
...
/**
 * Repair orphan hook entries in a project's hook settings files. Iterates the same
...
function repairOrphanHooks(projectName = null) {
  ...
  for (const project of candidates) {
    try {
      ...
      // Read BEFORE the loop below: it deletes `settings.hooks` from the very
      // files it repairs, so asking afterwards whether any file HAD a hooks block
      // answers about the post-repair state and reports a successful repair as
      // "no hooks block".
      const hadHooksBlock = classified.files.some(
        (f) => f.settings && f.settings.hooks && typeof f.settings.hooks === 'object'
      );
      const removed = [];
      const rewritten = [];
      for (const file of classified.files) {
        // Only rewrite a file this pass actually changed. A project whose orphan
        // is in one file must not have the other rewritten for nothing — that is
        // a gratuitous diff on the tracked, committable file.
        if (file.orphans.length === 0) continue;
        const doomed = new Set(file.orphans.map((o) => `${o.event}\u0000${o.index}`));
        const newHooks = {};
        for (const [eventName, entries] of Object.entries(file.settings.hooks)) {
          if (!Array.isArray(entries)) {
            newHooks[eventName] = entries;
            continue;
          }
          // Keyed by the classifier's own (event, index), so the repair removes
          // exactly what the scan reported rather than re-deciding it.
          const kept = entries.filter((_, i) => !doomed.has(`${eventName}\u0000${i}`));
          if (kept.length > 0) newHooks[eventName] = kept;
        }
        if (Object.keys(newHooks).length > 0) {
          file.settings.hooks = newHooks;
        } else {
          delete file.settings.hooks;
        }
        // Atomic write: tmp + rename. Same-directory rename is atomic on POSIX.
        const tmpPath = file.settingsPath + '.tmp';
        fs.writeFileSync(tmpPath, JSON.stringify(file.settings, null, 2) + '\n');
        fs.renameSync(tmpPath, file.settingsPath);
        removed.push(...file.orphans);
        rewritten.push(file.basename);
      }
      // A project that produced an error is reported as an error and nothing
      // else. Before both files were read it could only do one or the other;
      // now a parse failure in one file leaves the other looking merely empty,
      // which would report the same project as both errored and skipped.
      const errored = classified.errors.length > 0;
      if (classified.files.length === 0) {
        // Names both files, because both are looked at — a reason naming only
        // the shared one under-reports what was checked.
        if (!errored) {
          result.skipped.push({ name: project.name, reason: 'no .claude/settings.json or settings.local.json' });
        }
        continue;
      }
      if (!hadHooksBlock) {
        if (!errored) result.skipped.push({ name: project.name, reason: 'no hooks block' });
        continue;
      }
      if (removed.length === 0) {
        if (!errored) result.skipped.push({ name: project.name, reason: 'no orphan hooks' });
        continue;
      }
      ...
      log.info('Repaired orphan hooks (#145)', {
        ...
        // Which file(s) were rewritten — `removedCount` alone cannot say, and
        // after #1022 the two files hold different owners' hooks.
        files: rewritten
      });
    } catch (err) {
      result.errors.push({ name: project.name, error: err.message });
...
//
// `settings.local.json` is on the list because TangleClaw's own SessionStart hooks
// live there (#1022), and Claude Code merges it over `settings.json` from the same
// directory walk. Listing only the shared file would have made this guard blind to
// exactly the stranded hooks it was built to surface — the move would have quietly
// narrowed the guard's coverage while leaving it looking unchanged.
const STRANDED_CONFIG_FILES = [
  'CLAUDE.md',
  path.join('.claude', engines.SHARED_SETTINGS_BASENAME),
  path.join('.claude', engines.LOCAL_SETTINGS_BASENAME)
];
 ...
 * flagging any directory that holds a governance file (`STRANDED_CONFIG_FILES`)
 * and is not itself a registered project root.
 ...
 * operator-chosen paths synchronously — `detectExistingProjects` and the sites
 * #1350 enumerates are the known remainder — so "the dashboard list
...
/**
 * Why a hidden launch-mode picker over this default mode must be refused, or
 ...
 * combination needs an explicit confirm. The guard is for the WARNING, not for
 * the hidden picker: refusing every hidden picker would train the operator to
 * confirm without reading.
 *
 * One function for both routes. Written twice — once for an edit, once for a
 * create — it was two copies of a refusal sentence with inputs derived
 * differently, which is the same shape as the gap #626 closed by making the
 * create path run the guard at all.
 *
 * @param {object} opts - `show` (picker visibility, post-update), `mode` (the
 *   mode the engine will actually keep), `engineProfile`, `confirmed`.
 * @returns {string|null} The refusal, or null.
 */
function bypassHiddenRefusal({ show, mode, engineProfile, confirmed }) {
  if (show !== false || confirmed) return null;
  const key = typeof mode === 'string' && mode ? mode : store.DEFAULT_PROJECT_CONFIG.defaultLaunchMode;
  const modeConfig = engineProfile && engineProfile.launchModes && engineProfile.launchModes[key];
  if (!modeConfig || !modeConfig.warning) return null;
  return `hiding the launch-mode picker with default mode "${key}" removes its warning from the launch flow — resend with confirmBypassHidden: true to confirm`;
}

/**
 * The settings that only take effect at launch, and what a running session
 * therefore does about each.
 *
 * Changing one of these while a session is running stores the new value without
 * the running session picking it up. That is the worst shape a settings control
 * can have: it accepts the input, reports success, and changes nothing
 * observable, so the operator reasonably concludes it took effect (#758).
 *
 * Declared as one list rather than checked per key on purpose. Warning for a
 * single setting while its siblings stay silent is worse than warning for none:
 * it teaches that no warning means "this applied", which would then be false for
 * the rest. A setting added here without a `current` reader cannot be compared,
 * so the list is the contract the test derives from — never a copy of it.
 *
 * `divergence` is what separates two genuinely different situations, and getting
 * it wrong would replace one lie with another:
 *
 * - `running` — the live process behaves according to the OLD value, so the
 *   stored setting and the running session actually disagree. Only a relaunch
 *   reconciles them.
 * - `next-launch` — nothing is pending and nothing is stale. `launchSession`
 *   reads these fresh at the start of every launch, so a running session never
 *   held a copy to go stale. Telling the operator to relaunch would ask them to
 *   kill live work for no gain; what they may need correcting is only the
 *   expectation that it affects the session in front of them.
 *
 * `current` reads the value as it stands at the moment of this update, from
 * either the project row or the on-disk config.
 */
const LAUNCH_TIME_ONLY_SETTINGS = [
  { key: 'engine', label: 'Engine', divergence: 'running', current: (project) => project.engineId },
  { key: 'silentPrime', label: 'Silent prime', divergence: 'running', current: (project, cfg) => cfg.silentPrime },
  { key: 'defaultLaunchMode', label: 'Default launch mode', divergence: 'next-launch', current: (project, cfg) => cfg.defaultLaunchMode },
  { key: 'showLaunchModePicker', label: 'Show launch mode picker', divergence: 'next-launch', current: (project, cfg) => cfg.showLaunchModePicker }
];

/** How each `divergence` group ends its sentence. */
const LAUNCH_TIME_TAILS = {
  running: 'The running session is still using what it launched with — close and relaunch it to apply.',
  'next-launch': 'This applies to the next launch; the running session is unaffected.'
};

/**
 * Which launch-time-only settings this update actually CHANGES.
 *
 * A PATCH carrying a key at the value currently stored changes nothing and must
 * stay silent — the settings modal sends `engine` on every save, so comparing
 * presence rather than value would warn on a tag edit.
 *
 * @param {object} updates - The requested update.
 * @param {object} project - The project row, as stored now.
 * @param {object} cfg - Its on-disk config, as stored now.
 * @returns {object[]} The changed entries of `LAUNCH_TIME_ONLY_SETTINGS`, in declaration order.
 */
function launchTimeOnlyChanges(updates, project, cfg) {
  return LAUNCH_TIME_ONLY_SETTINGS
    .filter(s => updates[s.key] !== undefined && updates[s.key] !== s.current(project, cfg));
}

/**
 * The operator-facing sentences for a set of changed launch-time-only settings —
 * one per `divergence` group present, because the two groups need opposite
 * advice and a single sentence covering both could only be true of one.
 *
 * @param {object[]} changed - Entries from `launchTimeOnlyChanges`.
 * @returns {string[]} One warning per group, in declaration order.
 */
function formatLaunchTimeWarnings(changed) {
  const out = [];
  for (const group of Object.keys(LAUNCH_TIME_TAILS)) {
    const labels = changed.filter(s => s.divergence === group).map(s => s.label);
    if (labels.length === 0) continue;
    const subject = labels.length === 1
      ? `${labels[0]} is saved.`
      : `${labels.slice(0, -1).join(', ')} and ${labels[labels.length - 1]} are saved.`;
    out.push(`${subject} ${LAUNCH_TIME_TAILS[group]}`);
  }
  return out;
}

/**
 * @typedef {object} ProjectUpdateContext
 * @property {string} name - The name the project was looked up by (the PATCH's
 *   subject before any rename), which is what a rename compares against.
 * @property {object} project - The project row as stored now.
 * @property {object} updates - The requested update, verbatim.
 * @property {object} projConfig - The project's on-disk config as stored now.
 */

/**
 * @typedef {object} ProjectUpdateValidator
 * @property {string[]} keys - The PATCH fields whose PRESENCE runs this entry.
 *   Also the roster a coverage test reads, so a field validated by an entry that
 *   does not name it is invisible to that test.
 * @property {string[]} [reads] - Fields the entry consults but that do not by
 *   themselves run it. `confirmBypassHidden` is one: it answers the eyes-open
 *   guard, and making it a trigger would run the guard on a body carrying
 *   nothing else.
 * @property {(ctx: ProjectUpdateContext) => (string|null)} validate - The
 *   rejection reason, or null to accept. It must not write anything.
 */

/**
 * Every validation an incoming PATCH is judged by, as one ordered table.
 *
 * The table exists so the two phases can be separated at all: `updateProject`
 * runs EVERY entry to a verdict before `_applyProjectUpdates` writes anything,
 * which is the property a caller actually needs — a rejected field leaves
 * neither the DB row nor the on-disk config touched. The shape this replaced
 * validated most fields up front by convention and left three behind inside the
 * blocks that write, so `{ name, engine: <unknown> }` renamed the directory on
 * disk and THEN refused, leaving the projects row pointing at a path that no
 * longer existed (#1033).
 *
 * Order is precedence: the first entry to reject is the error the caller sees,
 * and it is the declaration order the previous inline gates had.
 *
 * A validator reads; it never writes. `projConfig` is loaded once and handed to
 * every entry rather than re-read per gate, because nothing between them can
 * change it — validation is the phase with no writes in it.
 *
 * @type {ProjectUpdateValidator[]}
 */
const PROJECT_UPDATE_VALIDATORS = [
  {
    // #103 — validated against the *intended* post-update engine, because
    // `engine` and `silentPrime` arrive in one PATCH from the settings modal.
    keys: ['silentPrime'],
    reads: ['engine'],
    validate: ({ updates, project, projConfig }) => {
      if (typeof updates.silentPrime !== 'boolean') return 'silentPrime must be a boolean';
      if (updates.silentPrime !== true) return null;
      const intendedEngineId = updates.engine || projConfig.engine || project.engineId;
      // `resolveProfile`, not `store.engines.get`: the latter returns null for a
      // connection-backed `openclaw:<id>`, so an OpenClaw project was refused
      // with "TangleClaw has no profile for this engine" about an engine the
      // rest of the product resolves fine.
      const intendedProfile = engines.resolveProfile(intendedEngineId);
      // The same owner the modal and the launch path ask, so a rejection here
      // and the greyed row there can never disagree — and the operator reads
      // the reason rather than a second wording of it.
      const fit = engines.settingDisposition('silentPrime', {}, intendedProfile);
      // Names the field it rejected, then hands over the same sentence the
      // modal greys the row with — an API error that says only "no" leaves
      // the caller to invent its own wording, which is how the two drift.
      return fit.applies ? null : `Cannot enable silentPrime — ${fit.reason}`;
    }
  },
  {
    // Eval Audit (#1236) — the first write path this setting has ever had; it
    // was hand-edit-only, which is why the dashboard panel is empty on every
    // install and #1227 was filed to delete the feature as dead.
    keys: ['evalAuditMode'],
    reads: ['engine'],
    validate: ({ updates, project, projConfig }) => {
      const patch = updates.evalAuditMode;
      if (!patch || typeof patch !== 'object' || Array.isArray(patch)) {
        return 'evalAuditMode must be an object';
      }
      // `enabled` is the only settable key. The other fifteen — judgeModel,
      // costCapPerSession, the sampling block — are hand-edit-only for now, and
      // an unknown key is REFUSED rather than dropped: silently accepting input
      // that does nothing is the defect ADR 0013 exists to end, and it would be
      // a poor joke to commit it inside the fix for it.
      const settable = ['enabled'];
      const unknown = Object.keys(patch).filter((k) => !settable.includes(k));
      if (unknown.length > 0) {
        return `evalAuditMode: ${unknown.join(', ')} cannot be set through the API (settable: ${settable.join(', ')}); edit project.json for the scoring tunables`;
      }
      if (patch.enabled !== undefined && typeof patch.enabled !== 'boolean') {
        return 'evalAuditMode.enabled must be a boolean';
      }
      if (patch.enabled !== true) return null;
      const intendedEngineId = updates.engine || projConfig.engine || project.engineId;
      // Resolved, never synthesized. A `{ id }` stub answers this row correctly
      // only because the row reads nothing but the id — the day #1251-#1255 add
      // a row reading `capabilities`, a stub answers from an empty profile. It
      // also has no `name`, so the API said "claude does not feed Eval Audit"
      // where the modal says "Claude Code does not feed", which is exactly the
      // one-sentence guarantee this chunk is built on.
      const intendedProfile = engines.resolveProfile(intendedEngineId);
      const fit = engines.settingDisposition('evalAuditMode', {}, intendedProfile);
      return fit.applies ? null : `Cannot enable Eval Audit — ${fit.reason}`;
    }
  },
  {
    // Feature Index toggle (#207). Unlike silentPrime, no engine capability
    // check is needed at this layer because the FEATURES.md file and wrap-step
    // parity are engine-agnostic; the engine capability gate lives at the
    // SessionStart injection site where it actually matters.
    keys: ['featureIndexEnabled'],
    validate: ({ updates }) => (typeof updates.featureIndexEnabled === 'boolean'
      ? null : 'featureIndexEnabled must be a boolean')
  },
  {
    // PIDX (#360, #356): per-project Project Map opt-in (engine-agnostic).
    keys: ['projectMapEnabled'],
    validate: ({ updates }) => (typeof updates.projectMapEnabled === 'boolean'
      ? null : 'projectMapEnabled must be a boolean')
  },
  {
    // #1492: whether the wrap may cut a release (engine-agnostic). The legacy
    // #318 boolean is still accepted, because the settings modal sends it on
    // every save; a PATCH carrying both must not contradict itself.
    keys: ['releaseMode', 'versionBumpEnabled'],
    validate: ({ updates }) => {
      const { releaseMode, versionBumpEnabled } = updates;
      if (versionBumpEnabled !== undefined && typeof versionBumpEnabled !== 'boolean') {
        return 'versionBumpEnabled must be a boolean';
      }
      if (releaseMode !== undefined && !RELEASE_MODES.includes(releaseMode)) {
        return `releaseMode must be one of ${RELEASE_MODES.join(', ')}`;
      }
      if (releaseMode !== undefined && versionBumpEnabled !== undefined
          && (releaseMode === 'off') === versionBumpEnabled) {
        return `releaseMode ${JSON.stringify(releaseMode)} contradicts versionBumpEnabled ${versionBumpEnabled}; send releaseMode alone`;
      }
      return null;
    }
  },
  {
    ...
    keys: ['versionFilePath'],
    validate: ({ updates, project }) => {
      if (updates.versionFilePath === null) return null;
      if (typeof updates.versionFilePath !== 'string') {
        return 'versionFilePath must be a string or null';
      }
      ...
      return contained.ok ? null : `versionFilePath ${contained.reason}`;
    }
  },
  {
    // MED-2K9P Chunk 02: per-project Medusa session-comms auto-enable
    // (engine-agnostic; the listener is TC-server-side and works regardless of
    // engine).
    keys: ['medusaEnabled'],
    validate: ({ updates }) => (typeof updates.medusaEnabled === 'boolean'
      ? null : 'medusaEnabled must be a boolean')
  },
  {
    // MED-2K9P v2 T2: per-project idle-gated wake opt-in (same shape as medusaEnabled).
    keys: ['medusaWake'],
    validate: ({ updates }) => (typeof updates.medusaWake === 'boolean'
      ? null : 'medusaWake must be a boolean')
  },
  {
    // Provenance line (ADR 0019). null resets the whole setting to off with the
    // default template. Otherwise `enabled` and `template` are the settable
    // keys; anything else, or an object setting neither, is refused rather than
    // stored to do nothing (ADR 0013). The template goes through the same
    // `validateTemplate` the renderer's resolver trusts, so a value this saves
    // can never be one the renderer then discards.
    keys: ['provenanceWatermark'],
    validate: ({ updates }) => {
      const patch = updates.provenanceWatermark;
      if (patch === null) return null;
      if (!patch || typeof patch !== 'object' || Array.isArray(patch)) {
        return 'provenanceWatermark must be an object or null';
      }
      const settable = ['enabled', 'template'];
      const unknown = Object.keys(patch).filter((k) => !settable.includes(k));
      if (unknown.length > 0) {
        return `provenanceWatermark: ${unknown.join(', ')} cannot be set (settable: ${settable.join(', ')})`;
      }
      if (patch.enabled === undefined && patch.template === undefined) {
        return 'provenanceWatermark must set enabled or template';
      }
      if (patch.enabled !== undefined && typeof patch.enabled !== 'boolean') {
        return 'provenanceWatermark.enabled must be a boolean';
      }
      const problem = provenance.validateTemplate(patch.template);
      return problem ? `provenanceWatermark.${problem}` : null;
    }
  },
  {
    // #1708: what a wrap that does not say inherits for "keep the session
    // running" (engine-agnostic). A boolean only: the wrap refuses to start on
    // a malformed value rather than guess, so one must never be saved.
    keys: ['wrapKeepSessionRunning'],
    validate: ({ updates }) => (typeof updates.wrapKeepSessionRunning === 'boolean'
      ? null : 'wrapKeepSessionRunning must be a boolean')
  },
  {
    ...
    // names (subset of continuity.WRAP_SECTIONS).
    keys: ['wrapSections'],
    validate: ({ updates }) => {
      if (updates.wrapSections === null) return null;
      if (!Array.isArray(updates.wrapSections)
          || updates.wrapSections.some((s) => !continuity.WRAP_SECTIONS.includes(s))) {
        return `wrapSections must be null or an array of valid section names (${continuity.WRAP_SECTIONS.join(', ')})`;
      }
      return null;
    }
  },
  {
    // Per-step wrap overrides. The runner re-checks every field at the point of
    // use — a hand-edited `.tangleclaw/project.json` never passes through this
    // validator, so this is the friendly error, not the safety guarantee.
    keys: ['wrapStepOverrides'],
    validate: ({ updates }) => {
  ...
  {
    // Train 21 (#1584, #1583): the launch sequence's per-project settings.
    // Unknown keys are refused rather than stored: a typo in a setting that
    // decides whether rule TEXT reaches a session would otherwise sit in
    // project.json reading like a configured value while changing nothing.
    keys: ['launchSequence'],
    validate: ({ updates }) => {
      const value = updates.launchSequence;
      if (value === null || typeof value !== 'object' || Array.isArray(value)) {
        return 'launchSequence must be an object';
      }
      const allowed = Object.keys(DEFAULT_PROJECT_CONFIG.launchSequence);
      const unknown = Object.keys(value).filter((k) => !allowed.includes(k));
      if (unknown.length > 0) {
        return `launchSequence has no setting${unknown.length === 1 ? '' : 's'} ${unknown.join(', ')}; it takes ${allowed.join(', ')}`;
      }
      if (value.pasteRules !== undefined && !PASTE_RULES_MODES.includes(value.pasteRules)) {
        return `launchSequence.pasteRules must be one of ${PASTE_RULES_MODES.join(', ')}`;
      }
      if (value.unreadyWindowMinutes !== undefined) {
        const m = value.unreadyWindowMinutes;
        if (typeof m !== 'number' || !Number.isFinite(m)
            || m < UNREADY_WINDOW_MIN_MINUTES || m > UNREADY_WINDOW_MAX_MINUTES) {
          return `launchSequence.unreadyWindowMinutes must be a number of minutes between ${UNREADY_WINDOW_MIN_MINUTES} and ${UNREADY_WINDOW_MAX_MINUTES}`;
        }
      }
      if (value.recoveryMode !== undefined && !RECOVERY_MODES.includes(value.recoveryMode)) {
        return `launchSequence.recoveryMode must be one of ${RECOVERY_MODES.join(', ')}`;
      }
      return null;
    }
  },
  {
    // Launch-mode posture (Phase A settings retask).
    keys: ['showLaunchModePicker'],
    validate: ({ updates }) => (typeof updates.showLaunchModePicker === 'boolean'
      ? null : 'showLaunchModePicker must be a boolean')
  },
  {
    keys: ['defaultLaunchMode'],
    reads: ['engine'],
    validate: ({ updates, project, projConfig }) => {
      if (typeof updates.defaultLaunchMode !== 'string' || !updates.defaultLaunchMode.trim()) {
        return 'defaultLaunchMode must be a non-empty string (an engine launch-mode key)';
      }
      ...
      if (!modes) {
        return updates.defaultLaunchMode === 'default'
          ? null
          : `engine "${intendedEngineId}" defines no launch modes — defaultLaunchMode can only be 'default'`;
      }
      if (!Object.prototype.hasOwnProperty.call(modes, updates.defaultLaunchMode)) {
        return `defaultLaunchMode "${updates.defaultLaunchMode}" is not a launch mode of engine "${intendedEngineId}" (valid: ${Object.keys(modes).join(', ')})`;
      }
      ...
      if (modes[updates.defaultLaunchMode].disabled === true) {
        return `defaultLaunchMode "${updates.defaultLaunchMode}" is disabled for engine "${intendedEngineId}"`;
      }
      return null;
    }
  },
  {
    ...
    // which is why `confirmBypassHidden` is read here and not a trigger.
    keys: ['defaultLaunchMode', 'showLaunchModePicker'],
    reads: ['confirmBypassHidden', 'engine'],
    validate: ({ updates, project, projConfig }) => {
      ...
      return bypassHiddenRefusal({
        show: effectiveShow,
        mode: reconcileLaunchMode(requestedMode, guardProfile),
        engineProfile: guardProfile,
        confirmed: updates.confirmBypassHidden === true
      }) || null;
    }
  },
  {
    ...
    // must be a BARE `.md` filename that exists under the project's plans
    // directory. Path separators are rejected outright — traversal-safe by
    // construction and matches the picker's contract (candidates are always bare
    // filenames); power users can still hand-edit any form into project.json
    // (read by priming-roll._readActivePlan).
    keys: ['activePlan'],
    validate: ({ updates, project }) => {
      const ap = updates.activePlan;
      if (ap === null || ap === '') return null;
      if (typeof ap !== 'string') return 'activePlan must be a string, null, or ""';
      if (ap.includes('/') || ap.includes(path.sep) || path.isAbsolute(ap)) {
        return 'activePlan must be a bare plan filename in the project\'s plans directory (no path separators)';
      }
      ...
      if (!ap.endsWith('.md') || !plansDir || !fs.existsSync(path.join(plansDir.dir, ap))) {
        ...
        return `activePlan "${ap}" not found under ${where}`;
      }
      return null;
    }
  },
  {
    // Rename. Every reason a rename cannot happen is decided here, so the
    // `fs.renameSync` in the apply phase is the only thing left that can fail —
    // and it fails as I/O, not as a verdict. This is the entry #1033 was filed
    // on: these checks used to sit inside the block that renames, one gate below
    // an `engine` rejection that had not been asked yet.
    keys: ['name'],
    validate: ({ name, updates, project }) => {
      if (!updates.name || updates.name === name) return null;
      // Block rename if session is active
      if (store.sessions.getActive(project.id)) {
        return 'Cannot rename while a session is active';
      }
      ...
      if (existing && existing.id !== project.id) {
        return existing.name === updates.name
          ? `Project "${updates.name}" already exists`
          : `Project "${existing.name}" already exists (case-insensitive match for "${updates.name}")`;
      }
      if (fs.existsSync(path.join(path.dirname(project.path), updates.name))) {
        return `Directory "${updates.name}" already exists on disk`;
      }
      return null;
    }
  },
  {
    // Engine switch. Asked here rather than inside the switch block: a PATCH
    // carrying both a rename and an unknown engine used to rename the directory
    // and then refuse, leaving the projects row pointing at a path that no
    // longer existed (#1033).
    //
    // Still gated on the engine CHANGING, exactly as the switch block is. The
    // settings modal posts `engine` on every save, so validating an unchanged
    // value would start refusing ordinary saves (a tag edit, a toggle) for any
    // project whose engine profile has since gone missing — a field the
    // operator did not touch failing the save of one they did.
    keys: ['engine'],
    validate: ({ updates, project }) => {
      if (!updates.engine || updates.engine === project.engineId) return null;
      return store.engines.get(updates.engine) ? null : `Engine "${updates.engine}" not found`;
    }
  },
  {
    // Rules — extensions only, core cannot be disabled. Also hoisted out of its
    // write block for #1033: this refusal used to land after the rename AND the
    // whole engine switch (config written, hooks re-synced) had already run.
    keys: ['rules'],
    validate: ({ updates }) => {
      if (!updates.rules || !updates.rules.core) return null;
      const coreDisabled = Object.entries(updates.rules.core).some(([, v]) => v === false);
      return coreDisabled ? 'Core rules cannot be disabled' : null;
    }
  },
  {
    // #1287 — `store.projects` writes `JSON.stringify(data.tags)` into a TEXT
    // column and `_rowToProject` reads it back with `_jsonParse(row.tags, [])`,
    // so a non-array round-trips as whatever it was: `PATCH {"tags":"a,b"}`
    // comes back as the STRING "a,b" where every reader expects `string[]`.
    // `data-model.md` declares the field `string[]`; this is the write path
    // holding the type the document already promises.
    keys: ['tags'],
    validate: ({ updates }) => validateTagsShape(updates.tags)
  },
  {
    // #1287 — `_applyProjectUpdates` writes `quickCommands` straight into
    // `project.json` with no verdict on its shape.
    //
    // This floor is PROSPECTIVE, and saying so matters because the obvious
    // justification is false: nothing reads the per-project `quickCommands`.
    // `public/session.js` renders `sessionState.config.quickCommands` — the
    // GLOBAL config from `GET /api/config`, a different field — so a malformed
    // per-project value reaches no browser today. #1287's body claims otherwise
    // and it is wrong on that point. The entry earns its place by holding the
    // type `data-model.md` declares before a reader exists to be broken by it,
    // and by closing the declared-unvalidated allowlist, which is the actual
    // subject of the issue.
    //
    // The globally-configured list that IS rendered is allowlisted for
    // `PATCH /api/config` (`server.js`) with no type check at all — a live
    // defect this entry does not reach, tracked as #1339.
    //
    // The floor is the shape `data-model.md` declares (`QuickCommand[]`) and the
    // shape `lib/store.js`'s own default is written in: `{label, command}`, both
    // strings. Per-element limits — tag length and character set, a count cap,
    // whether an empty string is a tag, whether a command is bounded — are named
    // as OPEN in #1374 (raised in #1287, which is closed) and are deliberately
    // not invented here; deciding them
    // silently inside a validation fix is how an undocumented rule gets born.
    // Extra keys are permitted for the same reason: refusing them would decide
    // that question by omission.
    keys: ['quickCommands'],
    validate: ({ updates }) => {
      // `cmd !== null` is load-bearing and not defensive noise: `typeof null` is
      // `'object'`, so without it a `[null]` element reaches `cmd.label` and
      // throws out of a function whose contract is to return a message. An array
      // element needs no separate check — parsed from a request body it cannot
      // carry string `label`/`command` properties, so the type checks below
      // already refuse it, and a clause that can never fire is not a guard.
      const ok = Array.isArray(updates.quickCommands) && updates.quickCommands.every((cmd) => (
        cmd !== null
        && typeof cmd === 'object'
        && typeof cmd.label === 'string'
        && typeof cmd.command === 'string'
      ));
      return ok ? null : 'quickCommands must be an array of objects with string label and command';
    }
  }
];

/**
 * Run every validator against an incoming PATCH and return the first rejection.
 *
 * Phase one of `updateProject`. It reads and it decides; it writes nothing, so
 * a rejection here leaves the project exactly as it was found — which is the
 * whole point of the split.
 *
 * @param {string} name - The name the project was looked up by.
 * @param {object} project - The project row as stored now.
 * @param {object} updates - Fields to update.
 * @returns {string|null} The rejection reason, or null when every field is acceptable.
 */
function _validateProjectUpdates(name, project, updates) {
  const ctx = { name, project, updates, projConfig: store.projectConfig.load(project.path) };
  for (const validator of PROJECT_UPDATE_VALIDATORS) {
    if (!validator.keys.some((k) => updates[k] !== undefined)) continue;
 ...
 *
 * Two phases, in this order and for this reason: every field is validated
 * before any field is written, so a PATCH whose last field is bad leaves the
 * projects row and the on-disk config untouched rather than half applied
 * (#1033). Phase one is `PROJECT_UPDATE_VALIDATORS`; phase two is
 * `_applyProjectUpdates`, which may still report I/O failures in `errors` but
 * never a verdict on the input.
 *
 * `errors` on a non-null project are partial failures of the update itself
 * (an engine config that did not write); `warnings` are true of the world
 * after a successful update and need the operator's hand — the LaunchAgents
 * that still name a renamed project's old path (#1148), and a launch-time-only
 * setting changed under a running session (#758).
 * @param {string} name - Project name
 * @param {object} updates - Fields to update
 * @returns {Promise<{ project: object|null, errors: string[], warnings?: string[] }>}
 */
async function updateProject(name, updates) {
  ...
  const rejection = _validateProjectUpdates(name, project, updates);
  if (rejection) {
    return { project: null, errors: [rejection] };
  }

  return _applyProjectUpdates(name, project, updates);
}

/**
 * Write an already-validated PATCH: disk, then the batched DB row.
 *
 * Phase two. Every field reaching this function has passed
 * `PROJECT_UPDATE_VALIDATORS`, so nothing here rejects the input — an `errors`
 * entry pushed below is an operation that failed (an engine config that would
 * not write), not a field that was refused, and the project is returned rather
 * than nulled. Adding a rejection here would put back the very shape #1033 was
 * filed on; the new field's verdict belongs in the table.
 *
 * Not everything that fails here is reported through `errors`, and the
 * difference matters: `store.projectConfig.save` is a bare `mkdirSync`/
 * `writeFileSync` and THROWS, so a disk failure leaves this function as an
 * exception rather than a return value. That is why the rename writes its row
 * immediately instead of waiting for the batched write at the end.
 *
 * Ordering IS load-bearing, unlike the validators' (which is only precedence):
 * the rename moves `project.path` that every later write resolves against, and
 * the engine switch must land before the hooks that are re-synced from it.
 *
 * @param {string} name - The name the project was looked up by.
 * @param {object} project - The project row; a rename updates its `name` and `path` in place,
 *   so later blocks resolve against the renamed project.
 * @param {object} updates - Fields to update, already validated.
 * @returns {Promise<{ project: object|null, errors: string[], warnings: string[] }>}
 */
async function _applyProjectUpdates(name, project, updates) {
  const errors = [];
  const warnings = [];
  const storeUpdates = {};
  /** The new name once a rename has landed, so the final read looks it up. */
  let renamedTo = null;

  // #758 — measured BEFORE any mutation, because this update is about to
  // overwrite the very values it compares against. The basis is what is STORED
  // now, which is what the session launched with until someone changes a setting
  // twice within one session; the difference is benign (a change and a change
  // back would warn), and reading the live process is not something this layer
  // can do. The warning itself is pushed at the end, so an update that fails
  // never claims something was saved.
  const launchTimeChanged = launchTimeOnlyChanges(
    updates, project, store.projectConfig.load(project.path)
  );

  // Name change — rename directory, DB record, and port leases. Every reason to
  // REFUSE a rename was decided in the validator; what is left is the move
  // itself, which can still fail as I/O.
  if (updates.name && updates.name !== name) {
    ...
    // The row moves WITH the directory rather than in the batched write at the
    // end of this function. Everything between here and there can throw —
    // `store.projectConfig.save` is a bare `mkdirSync`/`writeFileSync`, and the
    // seed helpers touch the project directory — and a throw with the rename
    // done but the row unwritten leaves the row naming a path that no longer
    // exists: the unopenable project this chunk exists to prevent, reached by an
    // exception instead of a verdict. Two stores cannot be written atomically,
    // so this does not close the window; it shrinks it to this one statement
    // from roughly fifteen. Same reason the port-lease rename below is immediate.
    store.projects.update(project.id, { name: updates.name, path: newPath });
    renamedTo = updates.name;
    // Both fields, for subsequent operations in this function: the row was just
    // written, and an object that still answers the old name disagrees with it.
    // What reads `name` below is the log context of the engine and silentPrime
    // branches — which now say the new name, where the launch-time warning at
    // the end still says the one the caller passed — and
    // `_syncLiveMedusaListener`, the only one that carries it out of this
    // process into a listener registration. That one cannot see the difference:
    // it returns early without an active session, and the rename validator
    // refuses outright when there is one. So this is the object kept level with
    // its row rather than a bug being fixed.
    ...
    // #1148 — a user LaunchAgent that runs something inside the project (the
    // supported pattern for a macOS automation that needs a TCC grant; see
    // docs/macos-tcc-automations.md) carries the OLD absolute path in its
    // plist, and launchd does not follow a rename: the job fails on its next
    // run with nothing pointing back here. Name the plists and leave the edit
    // to the operator — rewriting a LaunchAgent TangleClaw did not write is
    // not ours to do. The scan never throws (the rename already happened).
    const laScan = launchAgentScan.scanLaunchAgents(oldPath, launchAgentScan.defaultLaunchAgentsDir());
    const laWarning = launchAgentScan.formatWarning(oldPath, laScan);
    if (laWarning) {
      warnings.push(laWarning);
      log.warn('LaunchAgents still reference the renamed project path', {
        from: oldPath, to: newPath,
        matches: laScan.matches.map(m => m.file), unreadable: laScan.unreadable
      });
    }
  }

  // Engine change — the profile was resolved in the validator, so this reads it
  // back knowing it exists.
  if (updates.engine && updates.engine !== project.engineId) {
    ...
    if (writeResult.error && !writeResult.written && !writeResult.skipped) {
      // The live file was not written; a notice pointing at it would name a
      // file that does not exist. Leave the previous config as it is and say so.
      log.warn('Previous engine config left in place — the new engine config was not written', {
        project: project.name, from: project.engineId, to: updates.engine, error: writeResult.error
      });
    } else {
      try {
        const retire = engines.retireInactiveEngineConfig(project.path, project.engineId, updates.engine);
        const ctx = { project: project.name, from: project.engineId, to: updates.engine, file: retire.file, reason: retire.reason };
        if (retire.error) {
          log.warn('Failed to retire the previous engine config — it is still on disk as live canon', { ...ctx, error: retire.error });
        } else {
          log.info(retire.retired ? 'Previous engine config marked inactive' : 'Previous engine config left alone', ctx);
        }
      } catch (err) {
  ...
  // Rules update (extensions only — the core-rule refusal is in the validator)
  if (updates.rules) {
    const projConfig = store.projectConfig.load(project.path);
  ...
  // Silent prime opt-in (#103) — capability/type validation already happened in
  // the validator table so we know the value is safe to persist here. Re-sync
  // the .claude hook settings immediately so the SessionStart entry materializes
  ...
  // Eval Audit (#1236) — type- and disposition-validated in the validator table.
  // MERGED into the stored object, never assigned over it: `evalAuditMode`
  // carries fifteen scoring tunables beside `enabled` (judgeModel,
  // costCapPerSession, the whole sampling block), and a PATCH that replaced the
  // object would silently discard a configured cost cap — a settings save that
  // quietly destroys settings.
  if (updates.evalAuditMode !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
    projConfig.evalAuditMode = { ...(projConfig.evalAuditMode || {}), ...updates.evalAuditMode };
    store.projectConfig.save(project.path, projConfig);
  }

  // Feature Index toggle (#207) — type-validated in the validator table.
  ...
  // #1492: persist the release mode (validated in the table).
  if (updates.releaseMode !== undefined || updates.versionBumpEnabled !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
    const mode = nextReleaseMode(resolveReleaseMode(projConfig).mode, updates);
    projConfig.releaseMode = mode;
    // The legacy key is kept in step rather than deleted, for a TangleClaw
    // rolled back to a version that reads only it. `ask` writes false, so an
    // older reader holds instead of cutting a release nobody chose.
    projConfig.versionBumpEnabled = mode === 'auto';
    store.projectConfig.save(project.path, projConfig);
  }

  // Persist the explicit version-file path (validated in the table). An empty
  // string clears it back to the built-in probe order.
  if (updates.versionFilePath !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
    // The SAME helper the validator normalized with, not a second spelling of
    // it. What was judged and what is persisted must be the same value on a
    // path whose whole point is that a settings field cannot become an
    // arbitrary-file write.
    projConfig.versionFilePath = projectPaths.normalizeConfiguredPath(updates.versionFilePath);
    store.projectConfig.save(project.path, projConfig);
  }

  // MED-2K9P Chunk 02: persist the Medusa auto-enable pref (validated in the
  // table). Read at session launch (lib/sessions.js) to auto-start the listener.
  // TC#549 (v2 T3): the toggle also takes effect LIVE — a running session's
  // listener is started/stopped immediately, so the session doesn't stay
  // invisible in every roster until relaunch. The banner control remains the
  ...
  // Provenance line (validated in the table). MERGED, so a PATCH carrying only
  // `enabled` keeps the stored template. A blank template is stored as null:
  // that is the operator's reset to the default, and storing '' would leave a
  // value that means the same thing spelled a second way. Nothing is rewritten
  // here — each file picks the change up at its own next regeneration.
  if (updates.provenanceWatermark !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
    if (updates.provenanceWatermark === null) {
      projConfig.provenanceWatermark = null;
    } else {
      const stored = projConfig.provenanceWatermark && typeof projConfig.provenanceWatermark === 'object'
        ? projConfig.provenanceWatermark : {};
      const next = { enabled: stored.enabled === true, template: stored.template ?? null, ...updates.provenanceWatermark };
      if (typeof next.template === 'string') next.template = next.template.trim() || null;
      if (next.template === undefined) next.template = null;
      projConfig.provenanceWatermark = next;
    }
    store.projectConfig.save(project.path, projConfig);
  }

  // MED-2K9P v2 T2: persist the idle-gated wake opt-in (validated in the table).
  // Read each monitor tick (lib/medusa-wake.js), so the flag takes effect live —
  // no relaunch needed.
  ...
  // #1708: persist the wrap keep-running default (validated in the table). Read
  // by each wrap as it starts, so it applies to the next wrap.
  if (updates.wrapKeepSessionRunning !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
    projConfig.wrapKeepSessionRunning = updates.wrapKeepSessionRunning;
    store.projectConfig.save(project.path, projConfig);
  }

  // Launch-mode posture (validated + guard-checked in the table). Read by the
  // landing page (picker gate) and by sessions.launchSession (default-mode
  // resolution), so the setting takes effect on the next launch — no regen needed.
  ...
  // CC-6 (#381): persist the per-project wrap-section selection (validated in
  // the table). null clears the override back to the deep default (all 8 sections).
  ...
  // Launch-sequence settings (validated in the table). Merged rather than
  // replaced, so a save that sends one key does not silently reset the other.
  if (updates.launchSequence !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
    projConfig.launchSequence = { ...(projConfig.launchSequence || {}), ...updates.launchSequence };
    store.projectConfig.save(project.path, projConfig);
  }

  // Per-step wrap overrides (validated in the table). `{}` clears every override
  // and returns the project to the default pipeline.
  ...
  // #428: persist the active-plan pick (validated in the table). null/'' clears
  // the escape hatch; priming-roll._readActivePlan reads it back next wrap.
  ...
  // #758 — the change is saved and the running session did not pick it up. Said
  // here rather than in the settings modal so every caller gets it (the API and
  // the session page's own engine picker, not just the dashboard), and said
  // without naming an engine: settings resolve at launch on every engine.
  if (launchTimeChanged.length > 0 && store.sessions.getActive(project.id)) {
    for (const w of formatLaunchTimeWarnings(launchTimeChanged)) warnings.push(w);
    // Logged for the same reason the #1148 warning above is: a later "I changed
    // the engine and nothing happened" report has to be reconcilable against the
    // server log, and a log that only ever shows rename warnings reads as though
    // this path never fires.
    log.warn('Launch-time-only settings changed under a running session', {
      project: name,
      settings: launchTimeChanged.map(s => s.key)
    });
  }

  const finalName = renamedTo || name;
  const updated = await enrichProject(store.projects.getByName(finalName));
  return { project: updated, errors, warnings };
}
 ...
 * - **Session-safety:** if the project has a live session (`active`,
...
async function migrateProjectToPlugin(name) {
  ...
  // `active` DB row, so gate on its computed `.live` flag (a real tmux
...
module.exports = {
  ...
  syncAllProjects,
  listAllProjects,
  // Exported for tests: the registry read is a seam so a test can drive the
  // running / established-absent / unreadable answers directly, and
  // `_wrapState` is the mapping from the registry's shape to the card's.
  _internal,
  _wrapState,
  // The two card projections, exported so a test can assert that an
  // unestablished wrap state names itself in `incomplete` rather than
  // collapsing into the established `false`.
  _liveSession,
  _unknownSession,
  ...
  getProjectRow,
  updateProject,
  _PROJECT_UPDATE_VALIDATORS: PROJECT_UPDATE_VALIDATORS,
  // #758 — the launch-time-only class and its change detector. Exported so the
  // guard DERIVES the roster from the producer instead of restating it: a
  // setting added to the list with no warning path must go red on its own.
  LAUNCH_TIME_ONLY_SETTINGS,
  launchTimeOnlyChanges,
  formatLaunchTimeWarnings,
  deleteProject,
  archiveProject,
  unarchiveProject,
