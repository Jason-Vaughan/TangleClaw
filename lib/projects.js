 ...
 *
 * It ends at the next list item or heading, at a blank line followed by an
 * unindented line, or at the end of the file. Trailing blank lines are never
...
function _bulletItemEnd(lines, start) {
  ...
  while (end < lines.length) {
    ...
    if (line.trim() !== '' && !/^([-*+]|\d+[.)])\s/.test(line) && !/^#/.test(line)) {
      end += 1;
      continue;
    }
    if (line.trim() === '') {
