...
/**
 * The settings a session resolves ONCE, at launch, and never reads again.
 *
 * Changing one of these while a session is running stores the new value and
 * leaves the running process on the old one. That is the worst shape a settings
 * control can have: it accepts the input, reports success, and changes nothing
 ...
 *
 * `current` reads the value as it stands BEFORE the update, from either the
 * project row or its on-disk config.
 */
const LAUNCH_TIME_ONLY_SETTINGS = [
  { key: 'engine', label: 'Engine', current: (project) => project.engineId },
  { key: 'silentPrime', label: 'Silent prime', current: (project, cfg) => cfg.silentPrime },
  { key: 'defaultLaunchMode', label: 'Default launch mode', current: (project, cfg) => cfg.defaultLaunchMode },
  { key: 'showLaunchModePicker', label: 'Show launch mode picker', current: (project, cfg) => cfg.showLaunchModePicker }
];

/**
 * Which launch-time-only settings this update actually CHANGES.
 *
 * A PATCH carrying a key at its existing value changes nothing and must stay
 * silent — the settings modal sends `engine` on every save, so comparing
 * presence rather than value would warn on a tag edit.
 *
 * @param {object} updates - The requested update.
 * @param {object} project - The project row, pre-update.
 * @param {object} cfg - Its on-disk config, pre-update.
 * @returns {string[]} Human-readable labels of the changed settings, in declaration order.
 */
...
async function updateProject(name, updates) {
  ...
  // #758 — measured BEFORE any mutation, because the comparison is against the
  // values the running session actually launched with. The warning itself is
  // pushed at the end, so an update that fails never claims something was saved.
  ...
  // #758 — the change is saved and the running session will not see it. Said
  ...
  // without naming an engine: "settings apply at next launch" is how TangleClaw
  // launches all of them.
  if (launchTimeChanged.length > 0 && store.sessions.getActive(project.id)) {
    const which = launchTimeChanged.length === 1
      ? `${launchTimeChanged[0]} is`
      : `${launchTimeChanged.slice(0, -1).join(', ')} and ${launchTimeChanged[launchTimeChanged.length - 1]} are`;
    warnings.push(
      `${which} saved, but this project has a running session that resolved `
      + 'this at launch — it keeps what it started with. Close and relaunch the '
      + 'session to apply.'
    );
  }
