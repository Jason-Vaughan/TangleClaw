...
function createProject(data) {
  ...
  engines.reportMissingConfigCarrier(writeResult, projectConfig, engineProfile,
    { project: projectPath, engine: engineId, at: 'create' });
  ...
  for (const project of allProjects) {
    try {
      ...
      if (engineProfile && engineProfile.configFormat) {
        ...
        engines.reportMissingConfigCarrier(cfgResult, projConfig, engineProfile,
          { project: project.name, engine: engineId, at: 'boot-sync' });
...
async function updateProject(name, updates) {
  ...
  if (updates.engine && updates.engine !== project.engineId) {
    ...
    // Switching TO an engine with no config carrier is the moment the rules
    // stop being delivered, so it is the moment worth recording (#1251).
    engines.reportMissingConfigCarrier(writeResult, projConfig, engineProfile,
      { project: project.name, from: project.engineId, to: updates.engine, at: 'engine-switch' });
