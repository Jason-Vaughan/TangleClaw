 ...
 * @returns {string}
 */
function _buildProjectMapContent(projectPath) {
...
<!-- Auto-populated from this project's shared-doc group membership. -->
`;
}
 ...
 * @returns {boolean} true if a file was created, false if pre-existing or failed
 */
function _seedProjectMapFile(projectPath) {
  try {
    const filePath = path.join(projectPath, PROJECT_MAP_FILENAME);
    if (fs.existsSync(filePath)) {
      return false;
    }
    fs.writeFileSync(filePath, _buildProjectMapContent(projectPath));
...
function updateProject(name, updates) {
  ...
  if (updates.projectMapEnabled !== undefined) {
    ...
    if (updates.projectMapEnabled === true) {
      _seedProjectMapFile(project.path);
    }
  }
