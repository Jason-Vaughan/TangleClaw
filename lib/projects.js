...
function updateProject(name, updates) {
  ...
  if (updates.versionFilePath !== undefined && updates.versionFilePath !== null) {
    ...
    if (v !== '' && (path.isAbsolute(v) || v.split(/[\\/]/).includes('..'))) {
      return { project: null, methodologySwitch: null, errors: ['versionFilePath must be a relative path inside the project (no absolute paths, no "..")'] };
    }
  }
