...
/**
 * Collapse a `$HOME`-prefixed absolute path to `~/…` for display in a COMMITTED
 * file.
 *
 * `PROJECT-MAP.md` is tracked and routinely pushed to a public remote, so
 * writing a shared directory's absolute path into it publishes the operator's OS
 * username — and the layout of their private work — to everyone who clones the
 * repo. The path is written by a machine-refreshed section, so it re-appears on
 * every wrap; sanitizing at the render boundary is what makes the fix stick.
 *
 ...
 * paths expand it back (`resolveProjectsDir`, `lib/orchestration.js`), so this
 * loses no resolvability for the machine that wrote it.
 *
 * Only a prefix ending at a path boundary is collapsed: with `$HOME` of
 * `/Users/jane`, `/Users/jane-old/x` is left alone rather than becoming
 * `~-old/x`.
 *
 ...
 */
function _tildeHomePath(p) {
  if (typeof p !== 'string' || p === '') return p;
  const home = os.homedir() || process.env.HOME || '';
  // An empty or `/` home would collapse unrelated absolute paths — never sanitize
  // against it; leaving the path intact is the safe failure here.
 ...
 *
 * Shared directories render `~`-relative (`_tildeHomePath`) because this section
 * lands in a committed, often-public file and an absolute path would publish the
...
function _buildSharedDirsSection(groups) {
  ...
  return groups.map((g) => {
    const dir = g.sharedDir ? `\`${_tildeHomePath(g.sharedDir)}\`` : '_(no shared directory)_';
