...
async function updateProject(name, updates) {
  ...
  //
  // Validated up here with silentPrime, and for the same reason: a rejection
  // must drop before anything is written.
  if (updates.evalAuditMode !== undefined) {
    const patch = updates.evalAuditMode;
    if (!patch || typeof patch !== 'object' || Array.isArray(patch)) {
      return { project: null, errors: ['evalAuditMode must be an object'] };
    }
    ...
    if (unknown.length > 0) {
      return { project: null, errors: [`evalAuditMode: ${unknown.join(', ')} cannot be set through the API (settable: ${settable.join(', ')}); edit project.json for the scoring tunables`] };
    }
    if (patch.enabled !== undefined && typeof patch.enabled !== 'boolean') {
      return { project: null, errors: ['evalAuditMode.enabled must be a boolean'] };
    }
    if (patch.enabled === true) {
      ...
      if (!fit.applies) {
        return { project: null, errors: [`Cannot enable Eval Audit — ${fit.reason}`] };
      }
    }
  }
  ...
  // Eval Audit (#1236) — type- and disposition-validated at the top of the
  // function. MERGED into the stored object, never assigned over it:
  // `evalAuditMode` carries fifteen scoring tunables beside `enabled`
  // (judgeModel, costCapPerSession, the whole sampling block), and a PATCH that
  // replaced the object would silently discard a configured cost cap — a
  // settings save that quietly destroys settings.
  if (updates.evalAuditMode !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
