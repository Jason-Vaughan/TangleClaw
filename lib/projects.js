...
function createProject(data) {
  ...
  // Eyes-open guard, the same one `updateProject` applies to an edit: hiding
  // the picker while the default mode carries a warning (bypassPermissions /
  // fullAuto / yesAlways) removes the red isolated-environments warning from
  // the launch flow entirely. Creation is the one place it had never run, and
  // it is the place the combination is most likely to be established — a
  // project made this way launches straight into a warned mode having never
  // shown the warning to anybody.
  if (data.showLaunchModePicker === false) {
    const mode = typeof data.defaultLaunchMode === 'string' && data.defaultLaunchMode
      ? data.defaultLaunchMode
      : store.DEFAULT_PROJECT_CONFIG.defaultLaunchMode;
    const modeConfig = engineProfile.launchModes && engineProfile.launchModes[mode];
    if (modeConfig && modeConfig.warning && data.confirmBypassHidden !== true) {
      return { project: null, errors: [`hiding the launch-mode picker with default mode "${mode}" removes its warning from the launch flow — resend with confirmBypassHidden: true to confirm`] };
    }
  }
...
async function enrichProject(project, facts, context) {
  ...
  let evalAudit = { enabled: false, openIncidents: 0, judgeModel: null, costCapPerSession: null };
  ...
  try {
    ...
    evalAudit = {
      enabled: auditCfg.enabled === true,
      openIncidents: auditCfg.enabled === true
  ...
  if (updates.evalAuditMode !== undefined) {
    ...
    if (patch.enabled === true) {
      const intendedEngineId = updates.engine
        || store.projectConfig.load(project.path).engine
        || project.engineId;
      const intendedProfile = intendedEngineId ? { id: intendedEngineId } : null;
