...
function updateProject(name, updates) {
  ...
  if (updates.versionFilePath !== undefined && updates.versionFilePath !== null) {
    ...
    // Same predicate the wrap step enforces at its write site — shared rather
    // than re-derived, so the validator can never accept what the writer later
    // refuses (a setting that saves cleanly and then silently does nothing).
    if (v !== '') {
