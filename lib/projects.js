...
async function enrichProject(project, facts) {
  ...
  return {
    ...
    // SCAN_TIMEOUT | SCAN_ABORTED | SCAN_FAILED | EACCES, or null when healthy.
 ...
 * there are none.
 *
 * THE DEGRADATION IS STILL NOT COMPLETE, but the gap has narrowed to one call.
 * `enrichProject` no longer touches a project's directory for existence,
 * governance, git or config — all four come from the scanner child. It DOES
 * still call `_detectProjectVersion`, which reads the operator's path
 * synchronously and can block this process's event loop on a TCC-protected
 * directory. Until that moves, do not read "degrades to the registered projects"
 * as "cannot hang".
 *
