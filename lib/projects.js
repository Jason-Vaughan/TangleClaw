...
const PROJECT_UPDATE_VALIDATORS = [
  ...
  {
    // #1287: project tags must remain a string array.
    keys: ['tags'],
    validate: ({ updates }) => {
      if (!Array.isArray(updates.tags)
          || updates.tags.some((tag) => typeof tag !== 'string')) {
  ...
  {
    // #1287: project quick commands must remain an array of command objects.
    keys: ['quickCommands'],
    validate: ({ updates }) => {
      if (!Array.isArray(updates.quickCommands)
          || updates.quickCommands.some((cmd) => (
            !cmd
            || typeof cmd !== 'object'
            || Array.isArray(cmd)
        ...
        return 'quickCommands must be an array of objects with string label and command';
      }
      return null;
    }
  }
