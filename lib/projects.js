...
function validateProjectTags(tags) {
...
function createProject(data) {
  ...
  // #1338 — creation must enforce the same tags type as PATCH.
  // `createProject` does not pass through PROJECT_UPDATE_VALIDATORS, so validate
  // before creating anything on disk or writing the project config.
  if (data.tags !== undefined) {
    const tagsError = validateProjectTags(data.tags);
    if (tagsError) {
      return { project: null, errors: [tagsError] };
    }
  }
...
const PROJECT_UPDATE_VALIDATORS = [
  ...
  },
  {
    ...
    keys: ['tags'],
    validate: ({ updates }) => validateProjectTags(updates.tags)
  },
  {
