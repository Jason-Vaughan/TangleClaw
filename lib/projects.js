...
async function enrichProject(project, facts) {
  ...
  //
  // `undefined` rather than a fallback when the child could not say: `facts`
  // from a degraded read carries `version: null`, which is the same answer the
  // in-process chain gave for a directory it could not read.
