 ...
 *
 * @param {string} name - The name the project was looked up by.
 * @param {object} project - The project row; its `path` is updated in place by a rename.
...
async function _applyProjectUpdates(name, project, updates) {
  ...
  if (updates.name && updates.name !== name) {
    ...
    // No consumer below can observe the difference today — the only reader of
    // `project.name` that leaves this process is `_syncLiveMedusaListener`, and
    // it returns early without an active session, which the rename validator
    // refuses outright — so this is the object kept level with its row, not a
    // bug being fixed.
