...
// SYNCHRONOUS readdir, which is the defect fixed in `listAllProjects` — a
// TCC-protected path on macOS does not error, it never returns, and on the event
// loop that stops the whole server. It is left as-is only because every caller
// today is a test; wiring this to a route without converting it to the bounded
// async form above reintroduces the wedge silently.
