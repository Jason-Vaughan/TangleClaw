 ...
 */
function _unknownSession(row, cause) {
  return {
    active: null,
    status: row.status,
    startedAt: row.startedAt,
...
async function enrichProject(project, facts, context) {
  ...
  //
  // WHAT THIS DOES NOT SAY is that a wrap is running. The card reported that
  // from a `wrapping` DB row until the row stopped existing; re-sourcing it from
  // `lib/wrap-run-registry.js`, which is where a running pipeline actually lives,
  // is the next chunk of #1034.
  //
  ...
  if (activeSession) {
    ...
    if (!verdict || verdict.names.has(activeSession.tmuxSession)) {
      session = _liveSession(activeSession);
    } else if (!verdict.answered) {
      session = _unknownSession(activeSession, verdict.cause);
    }
    // tmux answered and this pane is gone: don't report. The row is claimed by
    // an action — the session page's finalize, or the launch path's stale
    // recovery — never by a read (#910).
  }
