...
function _scanFailureHint(err) {
  ...
  return 'the directory did not respond; on macOS this is what a TCC-protected path does when '
...
async function listAllProjects(options = {}) {
  ...
  let unregistered;
  try {
    unregistered = await _withTimeout(
...
/**
 * Abandon a directory walk that has outlived its budget.
 *
 * `_withTimeout` answers the REQUEST at the deadline; it cannot stop the walk it
 * raced, because a promise cannot be cancelled. Without this check the loop
 * keeps issuing filesystem calls into a path already known not to answer, long
 * after nobody is waiting for them — and each of those calls holds a libuv
 * threadpool slot (four by default) until the kernel returns, which it may never
 * do. A few repeat attempts and the pool is gone, taking every other async
 * filesystem call in the process with it. Checking between entries is what keeps
 * an abandoned walk from becoming a leak.
 *
 * @param {number} deadlineAt - Epoch ms after which the walk must stop.
 * @param {string} what - Short description used in the error message.
 * @returns {void}
 * @throws {Error & {tcTimedOut: boolean}} Once the deadline has passed.
 */
function _checkDeadline(deadlineAt, what) {
  if (Date.now() >= deadlineAt) {
    const err = new Error(`timed out after ${PROJECT_SCAN_TIMEOUT_MS}ms ${what}`);
    err.tcTimedOut = true;
    throw err;
  }
}
...
async function _listUnregisteredDirs(projectsDir, registeredNames, deadlineAt) {
  ...
  for (const entry of entries) {
    ...
    _checkDeadline(deadlineAt, `reading ${projectsDir}`);
...
async function _scanDirectoryEntries(dir, deadlineAt) {
  ...
  for (const entry of entries) {
    ...
    _checkDeadline(deadlineAt, `scanning ${dir}`);
 ...
 * asked about one specific directory — so a failure is REPORTED rather than
 * silently shortened, and a deadline failure carries the remedy.
 *
...
async function scanDirectoryForProjects(directory) {
  ...
  try {
    ...
    if (err && err.code === 'ENOENT') {
      return { ok: false, code: 'BAD_REQUEST', error: `Directory does not exist: ${directory}` };
    }
