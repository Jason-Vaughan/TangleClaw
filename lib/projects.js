 ...
 * Will be written by the AI on session start and wrap once the
 * session-lifecycle hook lands in #55 chunks 2+. In chunk 1 the
 * reader is live but no writer exists yet.
 ...
 * Returns the first hit from this ordered list, or null if none match:
 *   1. `.tangleclaw/project-version.txt` (AI-recorded cache, highest priority)
 *   2. `CHANGELOG.md` first non-Unreleased version header
 *   3. `version.json` at root (TangleClaw convention)
 *   4. `package.json` at root (Node convention)
 * See issue #55 for the design rationale.
 * @param {string} projectPath - Absolute project root path
 * @returns {string|null}
 */
function _detectProjectVersion(projectPath) {
  ...
  return (
    _readVersionCacheFile(projectPath) ||
    _readChangelogVersion(projectPath) ||
    _readVersionJsonVersion(projectPath) ||
    _readPackageJsonVersion(projectPath)
  );
}
...
function updateProject(name, updates) {
  ...
  if (updates.hasOwnProperty('methodology') && updates.methodology !== project.methodology) {
    if (updates.methodology === null) {
      ...
      // `currentTemplate` is hoisted to this scope so the extension-rule reset
      // below (which mutates `projConfig.rules.extensions`) can still see it;
      // the previous block-local `const` was a latent `ReferenceError` in the
      // `project.methodology` truthy branch — never reached because no test
      // exercised `PATCH methodology: null` end-to-end (#145, chunk 3 audit).
      let currentTemplate = null;
      if (project.methodology) {
        currentTemplate = store.templates.get(project.methodology);
      ...
      // Reset extension rules that the old methodology enabled
      if (currentTemplate && currentTemplate.defaultRules) {
        for (const [rule, value] of Object.entries(currentTemplate.defaultRules)) {
