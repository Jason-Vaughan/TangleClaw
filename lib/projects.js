 ...
 *
 * Facts about the project's OWN directory come from `_projectFacts`, which reads
 * them in a child process a deadline can kill — reading them on this process's
 * event loop is what let one unreadable folder stop the server answering
 * anything (#884). That is why this function is async.
 *
 * `facts` is an optional injection for the ONE caller that has already gathered
 * them: `listProjects` fetches all of its projects' facts concurrently, so the
 * cost of a list is the slowest single directory rather than their sum. It is
 ...
 * @param {{exists: boolean, governanceState: string, unreadable?: string}} [facts]
 *   Pre-gathered directory facts. Read here when omitted.
 * @returns {Promise<object>} - Enriched project
 */
async function enrichProject(project, facts) {
  if (!facts) facts = await _projectFacts(project);
  ...
  // Git info. Still read on this thread — moving it into the scanner child is
  // #884's next step; the existence test that gates it is already off the loop.
  let gitInfo = null;
  if (facts.exists) {
  ...
  return {
    ...
    // Why this project's directory could not be read, when that is what
    // happened. Absent on the healthy path so the field's presence is the
    // signal. Nothing renders it yet — that is #885.
 ...
 *
 * Shorter than the projects-directory walk's deadline because this is a probe of
 * a single known path, not a walk of unknown size: there is nothing here that is
 * legitimately slow, so a long wait only delays the degraded answer. The
 * dashboard polls every ten seconds, and these requests run concurrently, so the
 * route's added latency in the worst case is this value once — not once per
 * project.
 */
const PROJECT_FACTS_TIMEOUT_MS = 2000;

/**
 * Read what `enrichProject` needs from a project's own directory, in the child
 * process that can be killed if the directory never answers.
 *
 * ONE REQUEST PER PROJECT, NOT ONE PER POLL. A batch would be a single round
 * trip, but the supervisor's deadline kills the child rather than a request, so
 * a batch makes every project collateral for the worst directory in it. Per
 * project also means each request carries its own `pathKey`, which is what lets
 * the supervisor's backoff skip a directory that has already refused to answer
 * instead of spending another process on it every ten seconds.
 *
 * Never rejects. A project whose directory does not answer degrades to
 * `exists: false, governanceState: 'not-applicable'` and carries `unreadable`
 * with the reason — the same shape a genuinely missing directory produces, plus
 * the fact that we could not tell. Nothing renders `unreadable` yet; it is the
 * seam #885 needs to stop showing a silently-degraded list as a healthy one.
 *
 * @param {object} project - Project DB row; `path` and `engineId` are read.
 * @returns {Promise<{exists: boolean, governanceState: string, unreadable?: string}>}
 */
async function _projectFacts(project) {
  if (!project || !project.path) {
    return { exists: false, governanceState: 'not-applicable' };
  }
  try {
    return await dirScanner.request(
      'projectFacts',
      { dir: project.path, engineId: project.engineId },
      {
        timeoutMs: PROJECT_FACTS_TIMEOUT_MS,
        what: `reading ${project.path}`,
        // Opted in for the same reason the projects-directory walk is: nobody
        // asked for this read, it is a poll. Without the backoff an unreadable
        // project directory costs a killed child every ten seconds forever, and
        // a child blocked in the kernel may never leave the process table.
        pathKey: project.path
      }
    );
  } catch (err) {
    // A remembered refusal is not news — the scanner already logged a WARN
    // naming this path when it really failed, and logs another as the backoff
    // escalates. Warning again per poll would bury those under six identical
    // lines a minute.
    const say = (err && err.tcCached) ? log.debug : log.warn;
    say('Could not read a project directory — the project is listed without its git '
      + 'and governance detail', {
    ...
    return {
      ...
      unreadable: (err && err.message) || 'the directory did not answer'
    };
  }
}
 ...
 *
 * Facts are gathered concurrently, one scanner request per project, so the
 * route's cost is the slowest single directory rather than their sum. Callers
 * MUST await — this returned an array directly before #884.
 *
...
async function listProjects(options = {}) {
  ...
  const facts = await Promise.all(projects.map(_projectFacts));
 ...
 * there are none.
 *
 * THE DEGRADATION IS STILL NOT COMPLETE, but the gap has moved. `enrichProject`
 * no longer touches a project's directory for existence or governance — those
 * come from the scanner child. It DOES still read three more things from the
 * operator's path synchronously: `git.getInfo` (which shells out via `execSync`),
 * `_detectProjectVersion`, and `store.projectConfig.load`. Any of those can still
 * block this process's event loop on a TCC-protected directory, so do not read
 * "degrades to the registered projects" as "cannot hang" yet.
 *
 * Tracked as #884, which covers the whole family rather than this call site
 * alone. Note that the family is NOT well described by grepping
