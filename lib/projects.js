...
function createProject(data) {
  ...
  // A project born `minimal` must wrap commit-only from its first session,
  // not run the full pipeline until the next server restart happens to seed
  // it — the seeder is idempotent, so boot sync then finds the marker and
  // does nothing.
  seedCommitOnlyWrapOverrides({ name: data.name, path: projectPath, methodology: methodologyId }, projectConfig);
  ...
  // Sync engine hooks to match methodology. Engine passed explicitly — the
  // DB row is inserted below, after this call, so a DB-first resolution
  // inside syncEngineHooks would find nothing yet.
  try {
    engines.syncEngineHooks(projectPath, methodologyTemplate, engineId);
  } catch (err) {
...
function enrichProject(project) {
  ...
  // Per-step wrap overrides, keyed by step id. `{}` = run the methodology's
  // pipeline unmodified.
  let stepOverrides = {};
  ...
  return {
    ...
    // Governance state for the C2 visible-drift indicator (#353). Derived live
    // from on-disk config (engine + methodology from the canonical DB row), so
    // it self-clears the moment a project migrates. A missing project dir can't
    // be in live drift → not-applicable.
    governanceState: fs.existsSync(project.path)
      ? engines.governanceState(project.path, {
...
/**
 * One-shot wrap-config seeding for projects labeled with the `minimal`
 * methodology, run at boot sync while the label still exists.
 *
 * The wrap pipeline is code-owned (`lib/wrap-default-pipeline.js`) and
 * full-featured; `minimal`'s template pipeline was effectively commit-only
 * (its two ai-content steps shipped empty prompts, which self-skip). Without
 * seeding, retiring the per-methodology pipeline would silently flip every
 * `minimal` project from a commit-only wrap to the full 14-step wrap —
 * blocking changelog/memory steps on projects never configured for them. So
 * each `minimal` project gets `wrapStepOverrides` disabling every step except
 * the undisableable `commit`, preserving its wrap behavior exactly; operators
 * re-enable steps per project when wanted.
 *
 * `wrapOverridesSeeded` makes the seeding one-shot: without it, an operator
 * who later CLEARS the overrides map to opt into the full pipeline would be
 * silently re-seeded back to commit-only at the next boot. A `minimal`
 * project that already carries hand-authored overrides keeps them untouched
 * (the operator's intent stands) and is only stamped.
 *
 * @param {object} project - Project record (DB row — canonical for `methodology`)
 * @param {object} projConfig - Loaded `.tangleclaw/project.json` config (mutated on seed)
 * @returns {boolean} Whether the config was modified (and saved)
 */
function seedCommitOnlyWrapOverrides(project, projConfig) {
  if (project.methodology !== 'minimal') return false;
  if (projConfig.wrapOverridesSeeded) return false;

  const existing = projConfig.wrapStepOverrides || {};
  if (Object.keys(existing).length === 0) {
    ...
    projConfig.wrapStepOverrides = overrides;
    log.info('Seeded commit-only wrap overrides for minimal-methodology project', {
      project: project.name,
      disabledSteps: Object.keys(overrides).length
    });
  } else {
    log.info('Minimal-methodology project already has wrap overrides — stamping seed marker only', {
      project: project.name
    });
  }
  projConfig.wrapOverridesSeeded = true;
...
function syncAllProjects() {
  // archived:true means "include archived", not "only archived" — archived
  // rows are needed here so the wrap-config seed below can reach them; the
  // in-loop archived check still skips them for the full sync.
  ...
  for (const project of allProjects) {
    try {
      ...
      if (project.archived) {
        // Archived projects skip the full sync but still get the wrap-config
        // seed (it only writes .tangleclaw/project.json): an archived
        // `minimal` project left unseeded would silently flip to the full
        // pipeline on unarchive once the methodology field that identifies
        // it is gone.
        seedCommitOnlyWrapOverrides(project, store.projectConfig.load(project.path));
        continue;
      }
      ...
      // Wrap-config migration for `minimal`-labeled projects — see the
      // helper's doc for why this must happen before the per-methodology
      // pipeline retires. Runs before config regen so the first post-boot
      // wrap already honors the seeded overrides.
      seedCommitOnlyWrapOverrides(project, projConfig);

      // DB is the single source of truth for the engine, same as methodology
      // below (#320) and the session-launch path (sessions.js). The old
      ...
      if (engineProfile && engineProfile.configFormat) {
        // #320 — resolve methodology from the DB (`project.methodology`), the
        // single source of truth, NOT from `projConfig.methodology`. The old
        // `projConfig.methodology || project.methodology` precedence let a
        // stale, git-tracked project.json string override the server: TiLT v2
        // shipped a legacy `methodology: "minimal"` that contradicted the DB's
        // `prawduct`, and this startup-sync path would have regenerated
        // CLAUDE.md from the wrong template. This now matches the session-launch
        // path (sessions.js:198), closing the asymmetric-resolution gap
...
function attachProject(name) {
  ...
  if (!fs.existsSync(projConfigPath)) {
    ...
    // Apply methodology default rules, same as createProject (#536 Critic
    // finding): the config skeleton ships extension rules false, and rule
    // state now gates generated-config content (playbookRuleSections), so an
    // attached project must not silently carry skeleton-false as an opt-out
    // (asymmetric-gate class — see feedback_symmetric_capability_gates).
    if (methTemplate && methTemplate.defaultRules) {
      for (const [rule, value] of Object.entries(methTemplate.defaultRules)) {
  ...
  // Same birth-time seeding as createProject: a `minimal` attach must wrap
  // commit-only from its first session rather than flip at the next restart.
  // Loaded fresh because the config may have pre-existed the attach.
  try {
    seedCommitOnlyWrapOverrides(
      { name, path: projPath, methodology: methodologyId },
      store.projectConfig.load(projPath)
    );
  } catch (err) { // prawduct:allow prawduct/broad-except -- an unseedable config must not fail the attach; boot sync retries, and the failure is reported
    log.warn('Failed to seed wrap overrides during project attach', { project: name, error: err.message });
  }
...
function updateProject(name, updates) {
  ...
  if (updates.projectMapEnabled !== undefined && typeof updates.projectMapEnabled !== 'boolean') {
    return { project: null, methodologySwitch: null, errors: ['projectMapEnabled must be a boolean'] };
  }
  ...
  if (updates.versionBumpEnabled !== undefined && typeof updates.versionBumpEnabled !== 'boolean') {
    return { project: null, methodologySwitch: null, errors: ['versionBumpEnabled must be a boolean'] };
  }
  ...
  if (updates.versionFilePath !== undefined && updates.versionFilePath !== null) {
    if (typeof updates.versionFilePath !== 'string') {
      return { project: null, methodologySwitch: null, errors: ['versionFilePath must be a string or null'] };
    }
    ...
    if (v !== null) {
      ...
      if (!contained.ok) {
        return { project: null, methodologySwitch: null, errors: [`versionFilePath ${contained.reason}`] };
      }
    }
  }
  ...
  if (updates.medusaEnabled !== undefined && typeof updates.medusaEnabled !== 'boolean') {
    return { project: null, methodologySwitch: null, errors: ['medusaEnabled must be a boolean'] };
  }
  ...
  if (updates.medusaWake !== undefined && typeof updates.medusaWake !== 'boolean') {
    return { project: null, methodologySwitch: null, errors: ['medusaWake must be a boolean'] };
  }
  ...
  if (updates.wrapStepOverrides !== undefined) {
    ...
    if (!verdict.ok) {
      return { project: null, methodologySwitch: null, errors: [verdict.error] };
    }
  }
  ...
  if (updates.showLaunchModePicker !== undefined && typeof updates.showLaunchModePicker !== 'boolean') {
    return { project: null, methodologySwitch: null, errors: ['showLaunchModePicker must be a boolean'] };
  }
  if (updates.defaultLaunchMode !== undefined) {
    if (typeof updates.defaultLaunchMode !== 'string' || !updates.defaultLaunchMode.trim()) {
      return { project: null, methodologySwitch: null, errors: ['defaultLaunchMode must be a non-empty string (an engine launch-mode key)'] };
    }
    ...
    if (modes) {
      if (!Object.prototype.hasOwnProperty.call(modes, updates.defaultLaunchMode)) {
        return { project: null, methodologySwitch: null, errors: [`defaultLaunchMode "${updates.defaultLaunchMode}" is not a launch mode of engine "${intendedEngineId}" (valid: ${Object.keys(modes).join(', ')})`] };
      }
      ...
      if (modes[updates.defaultLaunchMode].disabled === true) {
        return { project: null, methodologySwitch: null, errors: [`defaultLaunchMode "${updates.defaultLaunchMode}" is disabled for engine "${intendedEngineId}"`] };
      }
    } else if (updates.defaultLaunchMode !== 'default') {
      return { project: null, methodologySwitch: null, errors: [`engine "${intendedEngineId}" defines no launch modes — defaultLaunchMode can only be 'default'`] };
    }
  }
  ...
  if (updates.defaultLaunchMode !== undefined || updates.showLaunchModePicker !== undefined) {
    ...
    if (effectiveShow === false && modeConfig && modeConfig.warning && updates.confirmBypassHidden !== true) {
      return { project: null, methodologySwitch: null, errors: [`hiding the launch-mode picker with default mode "${effectiveMode}" removes its warning from the launch flow — resend with confirmBypassHidden: true to confirm`] };
    }
  }
  ...
  if (updates.activePlan !== undefined
    ...
    if (typeof ap !== 'string') {
      return { project: null, methodologySwitch: null, errors: ['activePlan must be a string, null, or ""'] };
    }
    if (ap.includes('/') || ap.includes(path.sep) || path.isAbsolute(ap)) {
      return { project: null, methodologySwitch: null, errors: ['activePlan must be a bare plan filename in the project\'s plans directory (no path separators)'] };
    }
    ...
    if (!ap.endsWith('.md') || !plansDir || !fs.existsSync(path.join(plansDir.dir, ap))) {
      ...
      return { project: null, methodologySwitch: null, errors: [`activePlan "${ap}" not found under ${where}`] };
    }
  }
  ...
  if (updates.engine && updates.engine !== project.engineId) {
    ...
    try {
      ...
      engines.syncEngineHooks(project.path, methodologyTemplate, updates.engine);
    } catch (err) {
  ...
  if (updates.hasOwnProperty('methodology') && updates.methodology !== project.methodology) {
    ...
    if (!switchResult.success) {
      ...
      // #247 — methodology PATCH is a hook-affecting mutation; re-sync
      // git hooks to maintain the symmetric-capability-gates contract.
      try {
        gitHooks.syncGitHooks(project.path, store.config.load());
      } catch (err) {
        log.warn('Failed to sync git hooks after methodology switch', { error: err.message });
      }
    }
  }
  ...
  if (updates.phase !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
    // #320 — DB is the single source of truth for methodology; do not let a
    // stale project.json string override it (see syncAllProjects above).
    // For a combined `{methodology, phase}` PATCH, validate the phase against
    // the incoming methodology — `project.methodology` isn't refreshed until
    // store.projects.update below, and `updates.methodology` is already
    // guaranteed non-null (rejected at the top) and a valid id (validated in
    // the methodology branch above).
    const methTemplate = store.templates.get(updates.methodology || project.methodology);
  ...
  // returns the project to the methodology's own pipeline.
  if (updates.wrapStepOverrides !== undefined) {
    const projConfig = store.projectConfig.load(project.path);
...
function migrateProjectToPlugin(name) {
  ...
  const methodologyTemplate = store.templates.get(project.methodology);
  const result = engines.migrateToPlugin(project.path, methodologyTemplate);
  if (result.error) {
    return {
      project: enrichProject(project),
      status: project.migrationStatus || null,
