 ...
 *   is the caller's own service. Pass it only when that listener is up and is
 *   yours (bootstrap's ttyd/server/Caddy, a live tunnel). A caller registering
 *   before anything binds — OpenClaw connection create/update — must not: a
 *   listener there is a stranger's.
