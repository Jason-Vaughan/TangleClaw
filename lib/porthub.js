 ...
 *   whole answer.
 */
function _checkListener(port, host, projectName, adoptListener) {
  ...
  if (live) return { listenerCheck: null, refuse: false, listener: null };
...
function registerPort(port, projectName, service, options = {}) {
  ...
  try {
    ...
    check = _checkListener(port, host, projectName, options.adoptListener === true);
    if (check.listenerCheck === null && options.force === true) check.listenerCheck = 'takeover';
    if (check.refuse) {
...
function bootstrap(config) {
  ...
  if (fullConfigForIngress.ingressMode === 'caddy') {
    ...
    registerPort(httpsPort, selfName, 'caddy-https-ingress', caddyLease);
    registerPort(httpPort, selfName, 'caddy-http-ingress', caddyLease);
  }
