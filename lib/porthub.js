...
function registerPort(port, projectName, service, options = {}) {
  ...
  try {
    ...
    log.info('Port registered', { host, port, project: projectName, service, reach: options.reach || 'loopback' });
...
function _cleanupOrphanLeases() {
  ...
  for (const project of orphanProjects) {
    const released = store.portLeases.releaseByProject(project, { reason: 'orphan-sweep' });
