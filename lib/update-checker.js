...
function startChecker(intervalMs, initialDelayMs) {
  ...
  // included — for up to the full 15s timeout. That is why every
  // request-triggered path already uses the async form, and a timer firing
  // unattended is no more entitled to block the loop than a request is. It is
  // also what makes the interval free to shorten: while this blocked, the
  // interval was the only thing limiting how often it could.
  //
