...
function checkForUpdate() {
  ...
  try {
    ...
    // This is `update-applier`'s pre-flight path — the check that decides
    // whether an update can be applied at all — so a failure here is the
    // difference between "no update available" and "could not tell". Reported
    // through the shared reporter, which decides the level: see
    // `_reportCheckOutcome` for why the transition warns and the repeat does not.
    _reportCheckOutcome(err);
  }
