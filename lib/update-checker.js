...
function checkForUpdate() {
  ...
  try {
    ...
    // warn, not debug. This is `update-applier`'s pre-flight path — the check
    // that decides whether an update can be applied at all — so a failure here
    // is the difference between "no update available" and "could not tell".
    // `lib/logger.js` defaults to `info`, so at debug an install that had
    // silently stopped being able to detect releases left no trace an operator
    // would ever find. The async form carries the same level for the same
    // reason; the unattended-server case moved there with the periodic timer.
