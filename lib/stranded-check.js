 ...
 *
 * It also opens a pull request for a listed item when the operator asks
 * (`#openPr`, #1545), after reading GitHub to confirm the branch is still as
 * listed, and records who opened it.
 *
 * Nothing here reads the project folder on the event loop: a blocked read there
 * stops the whole server. Every read of the folder is a spawned `git`, which its
 * timeout kills.
 *
 ...
 * Other modules that read git borrow it as `exec`.
...
function defaultExec(file, args, options) {
  return execFileArgs(file, args, { cwd: options && options.cwd, timeoutMs: EXEC_TIMEOUT_MS, maxBufferBytes: 5 * 1024 * 1024 });
}
...
/**
 * Why a `git` spawn in `cwd` did not start. `execFile` reports a missing working
 * directory as ENOENT, exactly as it reports a missing binary, so this asks
 * whether `git` runs with no working directory at all. The question is a spawn,
 * not a filesystem read, so it cannot stall the server.
 * @param {string|null|undefined} cwd - The directory the failed spawn ran in
 * @param {Function} [exec] - `execFile` wrapper; defaults to this module's
 * @returns {Promise<string>}
 */
async function spawnFailure(cwd, exec = _internal.exec) {
  const probe = await exec('git', ['--version'], {});
  if (probe.exitCode === 0 && !probe.error) return `the project folder is missing (${cwd || 'no path'})`;
  return 'git is not installed';
}

/**
 * Read `origin` for a project directory.
 * @param {string} cwd
 * @returns {Promise<{kind: 'ok', url: string, remote: string, repo: string|null} |
 *   {kind: 'none', noSuchRemote: boolean, reason: string}>}
 *   `none` is a definite answer from git: no such remote (exit 2), or another
 *   refusal such as "not a git repository", with its reason.
 * @throws {ReadFailed} when git could not run or did not finish
 */
async function _readOrigin(cwd) {
  if (!cwd) throw new ReadFailed('the project has no folder path');
  const r = await _internal.exec('git', ['remote', 'get-url', 'origin'], { cwd });
  if (r.errorCode === 'ENOENT') throw new ReadFailed(await spawnFailure(cwd));
  if (didNotRun(r)) {
    // Could not run, or did not finish: that is not an answer about the remote.
    throw new ReadFailed(describeFailure(r, 'git remote get-url'));
  }
  if (r.exitCode !== 0 || !r.stdout.trim()) {
    return { kind: 'none', noSuchRemote: r.exitCode === 2 || r.exitCode === 0, reason: describeFailure(r, 'git remote get-url') };
  }
  const url = r.stdout.trim();
  return { kind: 'ok', url, remote: stripRemoteCredentials(url), repo: repoOf(url) };
}
...
async function _ghJson(cwd, args, what) {
  ...
  if (r.exitCode !== 0 || r.error) throw new ReadFailed(describeFailure(r, what));
  let data;
  try {
...
async function _remoteBranches(cwd) {
  ...
  if (r.exitCode !== 0 || r.error) throw new ReadFailed(describeFailure(r, 'git ls-remote'));
...
async function _examine(project, at) {
  ...
  const origin = await _readOrigin(cwd);
  // No such remote, and any other refusal (not a repository, a broken config),
  // is a definite "nothing to check here".
  if (origin.kind === 'none') {
    return { state: 'none', remote: null, reason: 'no origin remote' };
  }
  const { remote, repo } = origin;
...
async function openPr(project, request, by) {
  ...
  let result;
  try {
    result = await _openPr(project, item, by);
  } finally {
    _opening.delete(key);
  }
  if (!result.ok && ['READ_FAILED', 'CREATE_FAILED', 'WRITE_FAILED'].includes(result.code)) {
    log.warn('Could not open a PR for a stranded wrap', {
      project: project.name, branch: item.branch, code: result.code, reason: result.error
    });
  }
  return result;
}
...
async function _openPr(project, item, by) {
  ...
  let origin;
  try {
    origin = await _readOrigin(cwd);
  } catch (err) {
    if (err instanceof ReadFailed) return _refuse('READ_FAILED', err.message);
    throw err;
  }
  if (origin.kind === 'none') {
    return origin.noSuchRemote
      ? _refuse('NOT_GITHUB', 'the project has no origin remote, so there is nowhere to open a pull request')
      : _refuse('READ_FAILED', origin.reason);
  }
  const { remote, repo } = origin;
  ...
  if (ls.exitCode !== 0 || ls.error) return _refuse('READ_FAILED', describeFailure(ls, 'git ls-remote'));
  ...
  // #1861 — opening a PR is a governed mutation: a held or stopped lane opens
  // none, whoever asks. Required here, not at the top, because the gate pulls
  // in the store-backed control modules this file does not otherwise need.
  const refusal = require('./control-gate').checkMutation({
    surface: 'stranded-open-pr', subject: { kind: 'job', projectId: project.id, assignmentId: null }
  });
  if (refusal) return _refuse(refusal.code, refusal.message, { controlRefusal: refusal });
  ...
  if (created.exitCode !== 0 || created.error) {
    const reason = describeFailure(created, 'gh pr create');
    const stopped = created.timedOut || !!created.signal;
