 ...
 * @param {{exitCode: number, stderr: string, stdout: string, errorCode: (string|null), timedOut: boolean}} r
...
function _reasonFrom(r, what) {
  ...
  if (r.timedOut) return `${what} timed out after ${EXEC_TIMEOUT_MS}ms`;
...
async function _readOrigin(cwd) {
  ...
  if (r.timedOut || r.errorCode) {
    // Could not run, or did not finish: that is not an answer about the remote.
    throw new ReadFailed(_reasonFrom(r, 'git remote get-url'));
  }
  if (r.exitCode !== 0 || !r.stdout.trim()) {
    return { kind: 'none', noSuchRemote: r.exitCode === 2 || r.exitCode === 0, reason: _reasonFrom(r, 'git remote get-url') };
  }
...
async function _openPr(project, item, by) {
  ...
  if (created.exitCode !== 0 || created.error) {
    ...
    const stopped = created.timedOut;
