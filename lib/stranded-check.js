...
async function _examine(project, at) {
  ...
  if (origin.error && origin.error.code === 'ENOENT') throw new ReadFailed('git is not installed');
...
async function _run(project) {
  ...
  try {
    strandedWraps._internal.log({ projectId: project.id, eventType: EVENT_CHECK, detail });
...
function _checkRows(project) {
  return strandedWraps._internal.query({ projectId: project.id, eventType: EVENT_CHECK, limit: QUERY_LIMIT })
