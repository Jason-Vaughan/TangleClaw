 ...
 *   paneCwd:(string|null), workTreeReason:string, workToplevel:(string|null), workGitDir:(string|null), workTreeProblem:(string|null),
...
async function resolve(project, session, deps = {}) {
  ...
  const scope = {
    ...
    // The git common dir, already computed by `_repoIdentity` and previously
    // dropped here. Published because the handoff records it (#1585) and
    // re-deriving it would be a second reader of the same git call.
    workGitDir: workIdentity ? workIdentity.commonDir : null,
