 ...
 * file — `ProgramArguments`, `WorkingDirectory`, the stdout/stderr paths, a
 * shell one-liner — so the plist's layout does not matter. A match must end
 * at a path boundary: `/Users/me/Projects/app` does not match a plist that
 * names `/Users/me/Projects/app-2`, but does match `/Users/me/Projects/app/run.sh`.
 *
...
function scanLaunchAgents(needle, dir) {
  ...
  const re = new RegExp(`${escapeRegExp(needle)}(?![A-Za-z0-9._-])`);
