 ...
 * @param {boolean} [caddyAvailable] - Whether the `caddy` binary is on PATH,
 *   from `caddy.detectCaddy().available`. Defaults to true so a caller that has
 *   not looked is not silently told "yes" about a machine it did not check —
 *   callers that can look are expected to pass what they found.
