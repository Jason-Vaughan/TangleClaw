...
const CREDENTIAL_CODES = Object.freeze({
  ...
  // TangleClaw's own login guards the door, and Caddy's password is not in
  // front of it: there is no Caddy password to change, and building one would put
  // a second login in front of the one that works. Both sends the operator to
  // the account instead.
  ACCOUNT_LOGIN: 'account-login',
  // The gate state was not given or could not be read, so whether a Caddy gate
  // is wanted cannot be decided.
  GATE_STATE_UNKNOWN: 'gate-state-unknown'
});

/**
 * Where an account's password is reset, said the same way by every refusal that
 * sends the operator there.
 * @type {string}
 */
const ACCOUNT_REMEDY = 'Reset an account\'s password with a recovery code from the sign-in page, or run '
  + '`node scripts/reset-admin.js --store --user <name>` at a terminal on this machine.';
 ...
 *    authenticated whoever is asking — **but only for traffic that came through
 *    Caddy.** In caddy mode the server still binds an ungated loopback listener,
 *    so a browser on the operator's own machine reaches this route without
 *    crossing the gate at all. That is why condition 1 asks the socket, and why
 *    `handleRequest` refuses cross-site state changes before routing: this
 *    premise is sound about the perimeter and silent about the browser, and on
 *    its own it authorized a cross-site form POST that changed the credential
 *    (fixed 2026-08-03; remainder at #860).
 ...
 * @param {string|null} [gateState] - TangleClaw's gate state for this request
 *   (`lib/auth-gate.js#resolveGateState`). When it guards the door and no Caddy
 *   password is in force, the refusal says the login is the TangleClaw account
 *   and where that password is reset — rather than "nothing enforces a login",
 *   which is false on that install. Omitted, the refusals are the Caddy-only
 *   ones; nothing is allowed that would otherwise be refused, because this
 *   parameter only chooses between refusals.
...
function canChangeCredential(config, ingressState, caddyAvailable = true, fromLoopback = false,
  ...
  //
  // Deliberately not an `X-Auth-User` check: that header is a claim anyone able
  // to make the request can set, and `server.js` deletes it at request entry
  // (`authIdentity.refuseInboundIdentity`), so it proves nothing here.
  if (!fromLoopback) {
    return {
      allowed: false,
  ...
  // Before the Caddy-shaped refusals below, each of which would describe this
  // install wrongly: in direct mode "nothing enforces a login", and in caddy mode
  // after the state-driven cutover "the Caddy config carries no login". Both are
  // true of Caddy and false of the install. With Caddy's password still in front
  // (a caddy-mode file that carries one), that password IS changeable here and
  // this does not fire.
  if (authGate.guardsTheDoor(gateState) && (config.ingressMode !== 'caddy' || !(ingressState && ingressState.user))) {
    return {
      allowed: false,
      code: CREDENTIAL_CODES.ACCOUNT_LOGIN,
      reason: 'This install\'s login is its TangleClaw account, and no Caddy password stands in front of it, '
        + 'so there is no Caddy password to change here.',
      remedy: ACCOUNT_REMEDY
    };
  }
  if (config.ingressMode !== 'caddy') {
    return {
      ...
      reason: 'This install is not in caddy ingress mode, so there is no Caddy password in force to change.',
      remedy: 'TangleClaw\'s own login is managed under Your account. To put Caddy in front for remote access, run '
        + '`node scripts/ingress-cutover.js --to caddy` at a terminal.'
    };
  }
 ...
 * @param {string} gateState - The state the install is configured for
 *   (`lib/auth-gate.js#resolveIntendedGateState` — this predicate guards a
 *   writer). REQUIRED, for the reason `config` is: a caller that omits it would
 *   put `basic_auth` in front of an install TangleClaw's own login already guards.
 * @returns {{allowed: boolean, code: string, reason: (string|null)}}
 */
function canCreateGate(config, content, user, gateState) {
  ...
  // Asked before the file is looked at: when TangleClaw's login guards the door
  // an ungated Caddyfile is the intended shape, not a missing gate, and adding
  // `basic_auth` would put a second password in front of the one that works.
  if (authGate.guardsTheDoor(gateState)) {
    return {
      allowed: false,
      code: CREDENTIAL_CODES.ACCOUNT_LOGIN,
      reason: `TangleClaw's own login guards this install (${gateState}), so it needs no Caddy password in `
        + 'front of it. For a forgotten account password, use a recovery code or '
        + '`reset-admin.js --store --user <name>`; for a login that is broken, `gate-fallback.js`'
    };
  }
  // The two states left in which a Caddy gate is what this install wants: `open`
  // (building one is the operator turning a login on) and `account-required`
  // (Caddy's gate stays in front of the first-account screen). `unreadable`
  // keeps Caddy's gate everywhere else, but here it would mean writing one on a
  // guess and recording `authEnabled: true` from it; `fallback` is never an
  // intended state; anything else was not given.
  if (gateState !== authGate.GATE_STATES.OPEN && gateState !== authGate.GATE_STATES.ACCOUNT_REQUIRED) {
    return {
      allowed: false,
      code: CREDENTIAL_CODES.GATE_STATE_UNKNOWN,
      reason: gateState === authGate.GATE_STATES.UNREADABLE
        ? 'TangleClaw\'s config or account store could not be read, so whether this install wants a Caddy '
          + 'password cannot be decided. Fix the read error its log names, then run this again'
        : `the gate state was not given (${String(gateState)}), so whether this install wants a Caddy `
          + 'password cannot be decided'
    };
  }
  ...
  //
  // Two exact forms count as a round trip: what the generator writes today, and
  // what it wrote before sites without a password gained the peer guard
  // (`offboxGuard: false`). The second is the population most likely to be here
  // — an install with no login whose file predates the guard — and refusing it
  // would strand it on the one tool that adds a login. Neither form is written:
  // the rebuild below adds `basic_auth`, and a gated site carries no guard.
  //
  // Built for the gate state the rebuild will be written for, so the round trip
  // proves the same generator call that writes.
  let roundTrip;
  let legacyRoundTrip;
  try {
    roundTrip = caddy.buildCaddyfileContent({ ...derived, gateState });
    legacyRoundTrip = caddy.buildCaddyfileContent({ ...derived, gateState, offboxGuard: false });
  } catch (err) {
    return { allowed: false, code: CREDENTIAL_CODES.UNRECOGNIZED_SHAPE, reason: err.message };
  }
  if (roundTrip !== content && legacyRoundTrip !== content) {
    return {
      allowed: false,
      code: CREDENTIAL_CODES.UNRECOGNIZED_SHAPE,
 ...
 *
 * Refuses unless ALL of: the configured gate state wants a Caddy gate (`open` or
 * `account-required` — see {@link canCreateGate}), the install is in caddy mode (read from config here — a
 ...
 * @param {string} opts.hash - New bcrypt hash.
 * @param {string} opts.gateState - The configured gate state
 *   (`lib/auth-gate.js#resolveIntendedGateState`); see {@link canCreateGate}.
 * @param {number} opts.uid - Numeric uid for the launchctl target.
 * @param {string} opts.stamp - Filename-safe timestamp for the backup.
...
function createGate(opts) {
  const {
    caddyfilePath, user, hash, uid, stamp, gateState,
    reload = true,
    execFn = execFileSync,
    validateFn = caddy.validateCaddyfile
  } = opts;
  ...
  const verdict = canCreateGate(store.config.load(), original, user, gateState);
...
/**
 * The tail every terminal tool that rewrites the live Caddyfile IN PLACE shares:
 * write with a backup, `caddy validate` (restoring on failure), restart Caddy,
 * and tell the operator what is true afterwards — in the same words from every
 * tool, because the operator reading them may already be locked out.
 *
 * Three outcomes, each with its exit code:
 *   - `1` — nothing written: the original was restored, or (said loudly) could
 *     not be, with where the copy is and what to do before restarting Caddy;
 *   - `2` — written but Caddy could not be restarted, so the change is NOT live;
 *   - `0` — written and live.
 * The success lines are the caller's (`onWritten`, printed before the restart,
 * so a tool's summary reads in the order things happened).
 *
 * @param {object} opts
 * @param {string} opts.caddyfilePath - The live Caddyfile.
 * @param {string} opts.content - Replacement content.
 * @param {(p: string) => {ok: boolean, error?: string}} opts.validate - `caddy.validateCaddyfile`.
 * @param {(uid: number) => {ok: boolean, error: (string|null), command: string}} opts.reload - {@link reloadCaddy}.
 * @param {number} opts.uid - Numeric uid for the launchctl target.
 * @param {string} opts.stamp - Filename-safe timestamp for the backup.
 * @param {string} opts.change - What was written, for the not-live warning (e.g. `the pin`).
 * @param {string} [opts.nextStep] - A sentence appended after the restart line on
 *   both written outcomes (e.g. "Restart TangleClaw too, …").
 * @param {(backup: string) => void} [opts.onWritten] - Called after a good write.
 * @param {{write: Function}} opts.stdout
 * @param {{write: Function}} opts.stderr
 * @returns {{ code: 0|1|2, backup: (string|null), written: boolean }}
 */
function applyCaddyfileInPlace(opts) {
  const {
    caddyfilePath, content, validate, reload, uid, stamp, change, nextStep = null,
    onWritten = () => {}, stdout, stderr
  } = opts;
  const written = writeValidatedCaddyfile(caddyfilePath, content, validate, stamp);
  if (!written.ok) {
    ...
    return { code: 1, backup: written.backup, written: false };
  }
  onWritten(written.backup);
  const reloaded = reload(uid);
  if (reloaded.ok) {
    stdout.write(`  ✓ Caddy restarted.${nextStep ? ` ${nextStep}` : ''}\n\n`);
    return { code: 0, backup: written.backup, written: true };
  }
  stderr.write(`WARNING: ${change} is written but NOT live — Caddy could not be restarted automatically.\n`
    + `  Why: ${caddy.redactHashes(reloaded.error || 'no reason given')}\n`
    + `  Run: ${reloaded.command}\n`
    + (nextStep ? `  Then ${nextStep.charAt(0).toLowerCase()}${nextStep.slice(1)}\n` : ''));
  return { code: 2, backup: written.backup, written: true };
}
