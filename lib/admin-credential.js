...
function applyCredentialChange(opts) {
  ...
  if (!written.ok) {
    return {
      ok: false,
      // Restored and unrestorable are different facts, and collapsing them makes
      // the caller say "nothing was changed" about a machine whose live gate is
      // whatever a failed write left behind. The credential genuinely did not
      // change in either case — but in one of them the ingress may not load, and
      // an operator told all is well would not think to look.
