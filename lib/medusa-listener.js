 ...
 *
 * Sized above the number of distinct shared documents a group realistically
 * watches, so that per-`(doc, reader)` coalescing — not this cap — is what
 * bounds the ordinary case, and this stays a backstop against an inbox no
 * consumer will ever drain (an engine with no wake profile, by design).
 */
const SYSTEM_MESSAGE_RETENTION = 20;
 ...
 *
 * Keyed on the ONE property that makes it safe: the Bridge stamps `from` from
 * the sender, and `system` means no peer sent it, so no peer is waiting on a
 * reply. The switchboard's contract is that the initiator closes the loop —
 * expiring a peer's message would close it for them, silently. This is
 * deliberately not keyed on the payload's current shape (`type: 'system'` in
 * the body, an `event` name): a peer can type any body it likes.
 *
...
class MedusaListener extends EventEmitter {
  ...
  /**
   * The un-handled messages, oldest first. A message leaves this list only when
   * a consumer reports it handled (`markHandled`) — never on a badge clear, and
   * never on a mere read of this property.
   ...
   *
   * This is the one irreversible verb. Anything not handled stays in the inbox
...
module.exports = { MedusaListener, SYSTEM_MESSAGE_RETENTION, _setSystemMessageRetention };
