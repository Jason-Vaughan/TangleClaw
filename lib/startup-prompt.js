...
/** The caller kind of TangleClaw's own automatic bootstrap at launch (#1825 B3). */
const LAUNCH_CALLER_KIND = 'launch';

/** The clearance an automatic bootstrap records: proven by the launch itself, not a credential. */
const LAUNCH_CLEARANCE = 'launch-automatic';

/**
 * The internal caller for a launch's automatic bootstrap.
 *
 * Only `lib/launch-bootstrap.js` builds one, and no HTTP request can: the
 * routes hand `resolveAccess`'s result to `fire`, and its kinds do not include
 * `launch` (pinned by a test). What the caller CLAIMS is proven inside the fire
 * transaction by {@link canFire}: the session it names must be the target, in
 * the project it names, on its current launch.
 *
 * @param {{sessionId: number, projectId: number}} launch - The launch firing at itself.
 * @returns {{kind: 'launch', sessionId: number, projectId: number, groupIds: []}}
 */
function launchCaller(launch) {
  return { kind: LAUNCH_CALLER_KIND, sessionId: launch.sessionId, projectId: launch.projectId, groupIds: [] };
}
 ...
 * unbound caller are denied. The automatic bootstrap (`launch`) may fire only
 * at itself: the exact session and project it names must be the target's, and
 * it needs no place on the firer list because it fires nowhere else.
 *
 * @param {{kind: string, projectId: (number|null), groupIds: string[], sessionId?: number}} caller - A resolved caller.
 * @param {number} targetProjectId - The target session's project.
 * @param {object} prompt - The current revision.
 * @param {object} deps - Lookups.
 * @param {{sessionId: number}} [target] - The target session, which a `launch` caller must BE.
 * @returns {boolean}
 */
function canFire(caller, targetProjectId, prompt, deps, target) {
  if (!caller) return false;
  if (caller.kind === 'operator') return true;
  if (caller.kind === LAUNCH_CALLER_KIND) {
    return !!target && Number.isInteger(caller.sessionId) && caller.sessionId === target.sessionId
      && Number.isInteger(caller.projectId) && caller.projectId === targetProjectId;
  }
 ...
 * @param {object} input.caller - Resolved caller (`resolveAccess`), or the bootstrap's `launchCaller`.
 * @param {string} input.clearance - 'operator-verified', 'open-install-unverified' or 'project-binding'.
 * @param {{ready: boolean, reason: string}} [input.paneGate] - The automatic bootstrap's own pre-send
 *   gate (#1825 B3, Architect F1): what `_awaitPaneReady` found. Honoured for a `launch` caller only.
 *   When it did not find the pane ready, the attempt is still recorded (the durable intent row) and
 *   then settled `blocked` with `pane_not_ready` — the adapter is never asked, so no protocol fact is
 *   claimed about a pane TangleClaw could not see ready.
 * @param {object} [deps=DEFAULT_DEPS] - Lookups.
 * @returns {Promise<{status: number, body: object}>}
 */
async function fire(input, deps = DEFAULT_DEPS) {
  const { projectName, sessionId, sequenceId, expectedRevision, idempotencyKey, caller, clearance, paneGate } = input;
  ...
  const decided = deps.prompts.transaction(() => {
    ...
    const record = {
      ...
      promptTextDigest: prompt.textDigest,
      policyDigest: prompt.policyDigest,
      callerKind: caller.kind === 'operator' ? 'operator' : (caller.kind === LAUNCH_CALLER_KIND ? LAUNCH_CALLER_KIND : 'project'),
      callerClearance: caller.kind === LAUNCH_CALLER_KIND ? LAUNCH_CLEARANCE : clearance,
      // The bootstrap is attributed to the project it fires at, which is its own.
      callerProjectId: caller.kind === 'project' || caller.kind === LAUNCH_CALLER_KIND ? caller.projectId : null
    };

    // The scope proof and the target checks share this transaction (F3): a
    // `launch` caller's claim is tested against the session and project the
    // same read found, and the current-launch check below completes it.
    if (!canFire(caller, project.id, prompt, deps, { sessionId: session.id })) {
  ...
  if (caller.kind === LAUNCH_CALLER_KIND && paneGate && paneGate.ready !== true) {
    // The bootstrap's gate held the send. The intent row above is the audited
    // attempt; this settles it without the adapter, so the panel sees exactly
    // why the native path sent nothing, and nothing else may act on the pane.
    const reason = `the pane did not become ready within the launch window, so nothing was sent: ${paneGate.reason || 'no reason recorded'}`;
    const row = applyFireTransition(decided.dispatch.row.id, { outcome: 'blocked', reasonCode: 'pane_not_ready', reason }, deps);
    return _fireResult(row, false, decided.dispatch.session.engineId);
  }
...
module.exports = {
  ...
  LAUNCH_CALLER_KIND, LAUNCH_CLEARANCE,
  textProblem, firersProblem, read, update, canFire, launchCaller, fire, buildLaunchPayload, canonicalJson, applyFireTransition
};
