...
function fire(input, deps = DEFAULT_DEPS) {
  ...
  return deps.prompts.transaction(() => {
    ...
    // Dispatch through a registered adapter is not built yet: it needs the
    // intent → dispatching → receipt transitions defined against a live
    // engine receipt (#1825). The registry ships empty, so this is unreachable in
