 ...
 */
function outsideManagedBlock(text, markers) {
  if (_markerFault(markers)) return null;
  if (_countOccurrences(text, markers.begin) !== 1) return null;
  if (_countOccurrences(text, markers.end) !== 1) return null;
  const start = text.indexOf(markers.begin);
  const stop = text.indexOf(markers.end);
...
/**
 * Whether two versions of a file differ ONLY inside the managed block.
 *
 * Fails closed: a missing, duplicated or inverted marker pair on either side
 * answers no, because a wrong yes lets a hand edit be treated as TangleClaw's.
 * Untrimmed on both sides — a trailing-whitespace change outside the block is
 * still someone's edit.
 *
 * @param {string} before - The committed copy.
 * @param {string} after - The working copy.
 * @param {{begin: string, end: string}} markers - Managed-block delimiters.
 * @returns {{onlyInside: boolean, reason: string}} `reason` says why, for the
 *   caller's log line, on both answers.
 */
function differsOnlyInsideManagedBlock(before, after, markers) {
  const beforeOutside = outsideManagedBlock(before, markers);
  const afterOutside = outsideManagedBlock(after, markers);
  if (beforeOutside === null) return { onlyInside: false, reason: 'committed copy has no single well-formed managed block' };
  if (afterOutside === null) return { onlyInside: false, reason: 'working copy has no single well-formed managed block' };
  if (beforeOutside !== afterOutside) return { onlyInside: false, reason: 'the file differs outside the managed block' };
  return { onlyInside: true, reason: 'differs only inside the managed block' };
}

module.exports = {
  spliceManagedBlock,
  outsideManagedBlock,
  differsOnlyInsideManagedBlock,
  extractManagedBlock,
  hasManagedMarkers,
  _markerFault,
  _countOccurrences
