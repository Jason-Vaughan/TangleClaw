...
// The pure-fs half of uploads. Deliberately NOT `./uploads`, which owns the
// public name for this and requires the scanner supervisor — importing it here
// would put a supervisor inside the process the supervisor kills.
const uploadsFs = require('./uploads-fs');
...
const HANDLERS = {
  ...
  /**
   * Every upload a project holds — the legacy flat `<project>/.uploads/` pile
   * and every per-session `sessions/<sid>/uploads/` directory — read here so the
   * server never walks the operator's project directory itself (#889).
   *
   * `unreadable` and `code` ride the SUCCESS value rather than failing the
   * request, the way `projectFacts` reports a refused directory. A directory
   * that refuses to be read is a fact about the listing, not a failure to
   * produce one: the other directories were still walked, and answering with an
   * error would throw away every upload that WAS found in order to report the
   * one that was not.
   *
   * @param {object} payload - Request payload.
   * @param {string} payload.projectPath - Absolute project root.
   * @returns {Promise<{uploads: object[], unreadable: string|null, code: string|null}>}
   */
  async listUploads({ projectPath }) {
    return uploadsFs.listUploads(projectPath);
  },

  /**
   * Write one uploaded file into the project's uploads store.
   *
   * THE SECOND OP THAT WRITES, and it inherits `projectFacts`'s requirement
   * rather than re-deciding it: a writer inside a process that is SIGKILLed
   * mid-syscall must stage and rename, because writing in place truncates the
   * destination first. `lib/uploads-fs.js` owns that, for the upload and for
   * the `_scan.json` manifest alike.
   *
   * A MISSING PROJECT DIRECTORY IS REPORTED, NEVER CREATED. `saveUpload` builds
   * the uploads directory with `recursive: true`, which would happily
   * materialise a whole project tree at a path the operator has deleted or
   * unmounted, and then report the upload as saved into it. The check belongs
   * here rather than at the route because it is a read of an operator-chosen
   * path like any other: performed on the server it is one more call that a
   * stalled mount can decline to answer, which is the whole reason this op
   * exists.
   *
   * @param {object} payload - Request payload.
   * @param {string} payload.projectPath - Absolute project root.
   * @param {string} payload.filename - Operator-supplied name, sanitised downstream.
   * @param {string} payload.base64Data - Base64-encoded file content.
   * @param {string|number|null} [payload.sid] - Active session id, or null.
   * @returns {Promise<{status: string, upload?: object}>}
   */
  async saveUpload({ projectPath, filename, base64Data, sid = null }) {
    // `_probe`, not `_exists`. `_exists` collapses "not there" and "there and
    // refused" into one false, and answering a refusal with 'project-missing'
    // tells the operator their project was deleted when it was only unreadable —
    // the misdiagnosis this whole family of changes exists to remove, and the
    // distinction `_probe`'s own JSDoc was written for.
    const probe = await _probe(projectPath);
    if (!probe.exists) return { status: 'project-missing' };
    if (probe.refused) {
      return { status: 'project-refused', code: 'EACCES' };
    }
    return { status: 'saved', upload: uploadsFs.saveUpload(projectPath, filename, base64Data, sid) };
  }
};
