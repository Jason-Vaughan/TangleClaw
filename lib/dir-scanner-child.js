...
const HANDLERS = {
   ...
   *
   * `governanceState` runs synchronously here on purpose: this process exists to
   * be killed, so a blocking read costs it and nothing else. It runs only after
   * `_exists` has already answered, so the common protected-path case is caught
   * by the async probe before any synchronous read is attempted.
   *
