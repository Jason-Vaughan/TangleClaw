...
const HANDLERS = {
  ...
  /**
   * What the dashboard needs to know about ONE registered project's directory:
   * whether it is there, and what governs it.
   *
   ...
   * kills the CHILD, not a request — so one unreadable directory in a batch
   * takes every other project's answer down with it, and the whole list degrades
   * on account of one folder. Per-project requests keep the blast radius at the
   * project that actually failed, and let each one carry its own `pathKey` so a
   * directory that has already refused to answer is skipped by the supervisor's
   * backoff instead of costing another process every ten seconds.
   *
   * `governanceState` is reported as `not-applicable` for a directory that is
   * not there, matching what a caller could otherwise only learn by testing
   * existence itself — which is the read this op exists to take off the event
   * loop.
   *
   ...
   * @returns {Promise<{exists: boolean, governanceState: string}>}
   */
  async projectFacts({ dir, engineId }) {
    const exists = await _exists(dir);
    return {
      exists,
      governanceState: exists ? governance.governanceState(dir, { engineId }) : 'not-applicable'
    };
  },
