...
const HANDLERS = {
   ...
   *
   * The reads below `_exists` run SYNCHRONOUSLY here on purpose: this process
   * exists to be killed, so a blocking read costs it and nothing else. They run
   * only after `_exists` has answered, so the common protected-path case is
   * caught by the async probe before any synchronous read is attempted.
   *
   * A directory that is not there reports `not-applicable` governance and null
   * git and config — matching what a caller could otherwise only learn by
   * testing existence itself, which is the read this op exists to take off the
   * event loop.
   *
   * NOT here yet: the version-detection chain, which still runs in the server.
   * It reaches `lib/project-version.js` and back into `lib/projects.js`, and its
   * #165 self-heal WRITES — a writer in a process that gets SIGKILLed needs to
   * be atomic first. That is its own chunk.
   *
   ...
   *   git: object|null, config: object|null}>}
   */
  async projectFacts({ dir, engineId }) {
    ...
    if (!exists) {
      ...
      return { exists: false, governanceState: 'not-applicable', git: null, config: null };
    }
    return {
      exists: true,
