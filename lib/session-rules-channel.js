 ...
 * is rewritten on every launch and lives beside the shards it describes, and
 * the hook DELETES it on a successful post — single use. That deletion is
 * load-bearing rather than tidiness: the hook is registered on `startup` in the
 * project's own settings, so an unconsumed token is replayed by any later
 * `claude` opened in this directory, and in the one case that matters (the row
 * still `written` because this session's hook never ran) that replay credits
 * one session's delivery to another. A failed post leaves the token, so a
