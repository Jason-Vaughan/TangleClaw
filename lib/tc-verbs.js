...
async function runMessage(ctx) {
  ...
  // ack (ids validated above)
  await ctx.postJson(`${base}/read`, { ids });
  return { code: 0, stdout: `Marked ${ids.length} message(s) handled — they leave the inbox and the Hub drops its durable copies.\n` };
}
