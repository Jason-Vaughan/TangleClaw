...
function renderWhoami(d) {
  ...
  // #993: the live install's checkout state, for its own project only. The
  // server renders the sentences so the pane, the prime and the dashboard say
  // the same thing.
  if (d.liveInstall && Array.isArray(d.liveInstall.lines) && d.liveInstall.lines.length) {
    lines.push('');
    lines.push(...d.liveInstall.lines.filter((l) => l !== ''));
  }
  lines.push('');
