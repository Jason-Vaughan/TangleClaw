...
async function runMessage(ctx) {
  ...
  const usage = 'usage: tc message send [--priority normal|blocking] [--reason <code>] [--in-reply-to <message-id>] '
    + '[--reply-required|--no-reply] [--escalate-after <minutes>] <workspace-id> <text…> | tc message read | '
    + 'tc message ack <id> [<id>…] | tc message status <workspace-id> | tc message close <exchange-id>\n';
  ...
  if (sub === 'send') {
    ...
    const exchangeLine = x
      ? ` Exchange ${x.exchangeId} (${x.priority}, ${x.label}).`
        + (x.replyRequired ? ` Close it with \`tc message close ${x.exchangeId}\` once answered.` : '')
      : '';
    return {
      code: 0,
      stdout: `Message to ${result.to || to}: ${result.status || JSON.stringify(result)}.${retarget}${exchangeLine} `
...
const MESSAGE_SUBVERBS = ['send', 'read', 'ack', 'status', 'close'];
...
const VERB_ROSTER = [
  ...
  {
    id: 'message',
    usage: 'tc message send [--priority …] <workspace-id> <text…> | read | ack <id…> | status <workspace-id> | close <exchange-id>',
