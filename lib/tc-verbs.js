...
const START_SUBVERBS = ['next', 'status'];
...
function renderStartStatus(d) {
  ...
  if (d.pending) lines.push(`Not in this version: ${d.pending.stages.join(', ')} — ${d.pending.reason}.`);
...
/**
 * The `tc start` verb family: next | status (Train 21).
 * @param {TcContext} ctx
 * @returns {Promise<TcResult>}
 */
async function runStart(ctx) {
  const sub = ctx.argv[0];
  const usage = 'usage: tc start next [--ack <step>:<revision>:<digest>] [--page <n>] | tc start status\n';
...
const VERB_ROSTER = [
  ...
  {
    id: 'start',
    usage: 'tc start next [--ack <step>:<revision>:<digest>] [--page <n>] | tc start status',
    summary: "this session's launch sequence: pull each step of your context and acknowledge it",
    run: runStart
  },
  {
    id: 'ports',
