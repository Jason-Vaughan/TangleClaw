...
/**
 * What each wake-monitor reason code means for a SENDER — what the peer is
 * doing, as far as the monitor observed, and so what waiting will or will not
 * fix. Codes with a variable tail (`session-<status>`, `listener-<state>`) are
...
function renderPeerStatus(d) {
  ...
  const prefixed = /^session-/.test(reason)
    ? `the session is in state ${reason.slice('session-'.length)}, not active`
    : /^listener-/.test(reason)
      ? `the session's switchboard listener is in state ${reason.slice('listener-'.length)}, not listening, so its mail is not arriving`
      : null;
  const meaning = PEER_REASONS[reason] || prefixed
    || 'no description for this code — the server\'s answer stands as given';
  const lines = [`${d.workspaceId} (a TangleClaw session on this host): ${reason} — ${meaning}.`];
