...
async function runMessage(ctx) {
  ...
  if (sub === 'send') {
    ...
    return {
      code: 0,
      stdout: `Message to ${to}: ${result.status || JSON.stringify(result)}. `
  ...
  if (sub === 'read') {
    ...
    if ((data.messages || []).length === 0) {
      ...
      if (status.state === 'off') {
        return {
          code: 2,
          stderr: 'tc: your switchboard listener is not running, so your mail (if any) is not visible from here — '
            + 'an empty view proves nothing. Say the listener is off rather than reporting an empty inbox.\n'
        };
      }
    }
  ...
  // the response proves: the listener state it returns.
  const ackStatus = await ctx.postJson(`${base}/read`, { ids });
  if (ackStatus && ackStatus.state === 'off') {
    return {
      code: 2,
      stderr: 'tc: no switchboard listener is running for this project — NOTHING was marked handled. '
