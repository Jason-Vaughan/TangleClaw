...
function readResult() {
  ...
  try {
    ...
    // prawduct:allow prawduct/broad-except -- JSON.parse throws SyntaxError for
    // every malformation and the distinction between them changes nothing here;
    // the file is not an outcome, which is what the caller is told.
    return { present: true, malformed: true, result: null };
  }
}
