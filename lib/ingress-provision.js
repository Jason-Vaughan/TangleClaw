...
// answers without authentication — so callers are given the relative form. It is
// a LITERAL rather than a collapse of `cutoverLogPath()` because the guarantee
// this owes is "never an absolute path", and a derived form only holds that while
// the base sits under $HOME; `test/setup-provisioning.test.js` pins it.
//
// Safe to write out despite `TANGLECLAW_HOME` making the base relocatable: the
// cutover REFUSES to run while that variable is set (scripts/ingress-cutover.js),
// so a base this literal would misname is a base no cutover ever logs into.
...
// The protection states in which a gate is positively in force: `existing` —
// adoption read the live Caddyfile and matched it; `account` — TangleClaw's own
// login guards the door (`authGate.guardsTheDoor`), which needs no Caddy at all.
// Every other state is either "a credential exists but nothing here can say it
// is enforced" or "no answer yet" — neither of which may be reported to an
// operator as protected.
const PROTECTION_CONFIRMED = new Set(['existing', 'account']);
// States in which a credential EXISTS, whatever is or is not enforcing it —
// including the confirmed ones, because a gate known to be in force obviously has
// a credential behind it. Deliberately not narrowed to "stored but unconfirmed":
...
const PROTECTION_CREDENTIAL_STORED = new Set(['existing', 'account', 'unchanged', 'existing-unverified']);
// Values this build knows how to classify. Used only to notice one it does not.
const KNOWN_PROTECTION_STATES = new Set([
  'none', 'pending', 'existing', 'account', 'unchanged', 'existing-unverified'
]);
 ...
 * nothing is enforcing a login. Naming the confirmed states instead makes
...
function deriveProtectionFlags(protection) {
  ...
  return {
    confirmedProtection: PROTECTION_CONFIRMED.has(protection),
