...
/**
 * Characters no workload text field may contain (Architect ruling A29). Each
 * of these fields is shown to coordinators as one line of plain text, so
 * anything that can break the line, hide text or reorder what a reader sees is
 * refused:
 * - C0 controls and DEL (U+0000–U+001F, U+007F): newlines, tabs, escapes.
 * - C1 controls (U+0080–U+009F), including NEL (U+0085), which some
 *   renderers treat as a line break.
 * - Unicode bidirectional controls: ALM (U+061C), LRM/RLM (U+200E, U+200F),
 *   the embeddings and overrides (U+202A–U+202E) and the isolates
 *   (U+2066–U+2069). They let displayed text read differently from what is
 *   stored ("Trojan Source").
 * - LINE SEPARATOR and PARAGRAPH SEPARATOR (U+2028, U+2029), which are line
 *   breaks outside ASCII.
 * Ordinary letters in any script, right-to-left ones included, are allowed.
 */
// eslint-disable-next-line no-control-regex -- matching control characters is the point
const UNSAFE_TEXT = /[\u0000-\u001f\u007f-\u009f\u061c\u200e\u200f\u202a-\u202e\u2066-\u2069\u2028\u2029]/;

/**
 * Whether a string is safe as one line of plain workload text (A29).
 * @param {string} text - Candidate text
 * @returns {boolean} False when it contains any {@link UNSAFE_TEXT} character
 */
function isSafeText(text) {
  return typeof text === 'string' && !UNSAFE_TEXT.test(text);
}
...
function validate(body) {
  ...
  if (typeof body.summary !== 'string' || body.summary.length < 1 || body.summary.length > LIMITS.summary
    ...
    return bad('WORKLOAD_BAD_FIELD',
      `summary must be one line of 1–${LIMITS.summary} characters with no control, bidi or line-separator characters.`);
  }
  ...
  if (body.waitDetail !== undefined) {
    if (typeof body.waitDetail !== 'string' || body.waitDetail.length > LIMITS.waitDetail
      ...
      return bad('WORKLOAD_BAD_FIELD',
        `waitDetail must be at most ${LIMITS.waitDetail} characters with no control, bidi or line-separator characters.`);
    }
  ...
  if (body.tasks !== undefined) {
    ...
    for (const t of body.tasks) {
      if (typeof t !== 'string' || t === '' || t.length > LIMITS.task || !isSafeText(t)) {
        return bad('WORKLOAD_BAD_FIELD', `each task id must be 1–${LIMITS.task} characters with no control, bidi or line-separator characters.`);
      }
    }
...
function recordNarrowing({ caller, body, nowMs = Date.now() }) {
  ...
  if (typeof b.reason !== 'string' || b.reason.trim() === '' || b.reason.length > 200 || !isSafeText(b.reason)) {
    return _refuse(400, 'WORKLOAD_BAD_BODY',
      'reason must be one line of 1–200 characters, with no control, bidi or line-separator characters.');
  }
