...
/**
 * Characters no workload text field may contain: the display-safety rule of
 * ADR 0020 §3 (Architect rulings A29 and A30). Every free-text field is shown
 * to coordinators as one line of plain text, so anything that can break the
 * line, hide itself or change what a reader sees is refused, by Unicode
 * property rather than by a hand-kept list:
 * - `Cc` controls: C0 (tab, newline, escape...), DEL and C1 (including NEL).
 * - `Cf` format characters: bidi controls (U+061C, U+200E/F, U+202A–U+202E,
 *   U+2066–U+2069), zero-width characters (U+200B–U+200D, U+2060), U+FEFF,
 *   the soft hyphen U+00AD and the tag characters U+E0000–U+E007F.
 * - `Zl` / `Zp`: U+2028 LINE SEPARATOR and U+2029 PARAGRAPH SEPARATOR.
 * - `Default_Ignorable_Code_Point`: everything a renderer may draw as nothing,
 *   including the variation selectors (U+FE00–U+FE0F, U+E0100–U+E01EF),
 *   U+034F and the Hangul fillers.
 * A consequence: emoji written with a variation selector or a zero-width
 * joiner are refused too; single-code-point emoji and symbols are not.
 * This is display integrity only. It does not normalize text or detect
 * look-alike characters (homoglyphs), which ADR 0020 leaves out of scope.
 */
const UNSAFE_TEXT = /[\p{Cc}\p{Cf}\p{Zl}\p{Zp}\p{Default_Ignorable_Code_Point}]/u;

/**
 * A character a reader can see: a letter, number, punctuation mark or symbol.
 * A text field must contain at least one, so a value made only of combining
 * marks or spaces cannot display as empty.
 */
const VISIBLE_TEXT = /[\p{L}\p{N}\p{P}\p{S}]/u;

/**
 * Whether a string is safe as one line of plain workload text (ADR 0020 §3):
 * no {@link UNSAFE_TEXT} character, and at least one {@link VISIBLE_TEXT} one.
 * @param {string} text - Candidate text
 * @returns {boolean}
 */
function isSafeText(text) {
  return typeof text === 'string' && !UNSAFE_TEXT.test(text) && VISIBLE_TEXT.test(text);
}
...
function isValidBranchName(name) {
  ...
  if (!isSafeText(name) || /\s/.test(name)) return false;
...
function validate(body) {
  ...
  if (typeof body.summary !== 'string' || body.summary.length < 1 || body.summary.length > LIMITS.summary
      || !isSafeText(body.summary) || body.summary.trim() === '') {
    return bad('WORKLOAD_BAD_FIELD',
      `summary must be one line of 1–${LIMITS.summary} characters with no control, format, invisible or line-separator characters, and at least one visible character.`);
  }
  ...
  if (body.waitDetail !== undefined) {
    if (typeof body.waitDetail !== 'string' || body.waitDetail.length > LIMITS.waitDetail
        || !isSafeText(body.waitDetail)) {
      return bad('WORKLOAD_BAD_FIELD',
        `waitDetail must be at most ${LIMITS.waitDetail} characters with no control, format, invisible or line-separator characters, and at least one visible character.`);
    }
  ...
  if (body.tasks !== undefined) {
    ...
    for (const t of body.tasks) {
      if (typeof t !== 'string' || t === '' || t.length > LIMITS.task || !isSafeText(t)) {
        return bad('WORKLOAD_BAD_FIELD', `each task id must be 1–${LIMITS.task} characters with no control, format, invisible or line-separator characters, and at least one visible character.`);
      }
    }
...
function record({ req, body, access = sharedDocsAccess.resolveAccess(req), nowMs = Date.now() }) {
  ...
  if (result.busy) {
    log.warn('workload receipt refused: the store stayed locked through the retry', {
      projectId: access.projectId, sessionId: access.sessionId, launchId: access.launchId
    });
...
/**
 * Record an operator narrowing of a lane's composed verdict, or clear one
 * (ADR 0020 §7), from `POST /api/tc/workload/narrowing`. Operator only, with
 * the same proof tiers as control: a verified account session, or an install
 * whose auth gate is deliberately open. A narrowing only ever lowers a verdict;
 * this route has no way to raise one.
 * @param {{caller: object, body: *, nowMs?: number}} input - `caller` from
 *   `control-auth#resolveControlCaller`; the parsed body; a clock for tests
 * @returns {{status: number, body: object}}
 */
function recordNarrowing({ caller, body, nowMs = Date.now() }) {
  ...
  if (caller.kind !== 'operator') {
    return _refuse(403, 'OPERATOR_ONLY',
      'Only the operator can narrow a lane\'s workload verdict. No session, ProjectManager or Architect can.');
  }
  const b = body && typeof body === 'object' && !Array.isArray(body) ? body : null;
  const allowed = ['sessionId', 'capClearance', 'forceUnknown', 'clear', 'reason'];
  if (!b) return _refuse(400, 'WORKLOAD_BAD_BODY', 'The body must be a JSON object.');
  for (const key of Object.keys(b)) {
    if (!allowed.includes(key)) return _refuse(400, 'WORKLOAD_BAD_BODY', `"${key}" is not a narrowing field. Allowed: ${allowed.join(', ')}.`);
  }
  if (!Number.isInteger(b.sessionId) || b.sessionId < 1) {
    return _refuse(400, 'WORKLOAD_BAD_BODY', 'sessionId must be the target session\'s id.');
  }
  for (const flag of ['capClearance', 'forceUnknown', 'clear']) {
    if (b[flag] !== undefined && typeof b[flag] !== 'boolean') {
      return _refuse(400, 'WORKLOAD_BAD_BODY', `${flag} must be true or false.`);
    }
  }
  if (typeof b.reason !== 'string' || b.reason.trim() === '' || b.reason.length > 200 || !isSafeText(b.reason)) {
    return _refuse(400, 'WORKLOAD_BAD_BODY',
      'reason must be one line of 1–200 characters, with no control, format, invisible or line-separator characters, and at least one visible character.');
  }
  const narrows = b.capClearance === true || b.forceUnknown === true;
  if (b.clear === true && narrows) {
    return _refuse(400, 'WORKLOAD_BAD_BODY', 'Send clear, or capClearance/forceUnknown, not both.');
  }
  if (b.clear !== true && !narrows) {
    return _refuse(400, 'WORKLOAD_BAD_BODY', 'Say what to narrow (capClearance and/or forceUnknown), or clear: true.');
  }
  const session = store.sessions.get(b.sessionId);
  if (!session) return _refuse(404, 'SESSION_NOT_FOUND', `No session ${b.sessionId}.`);
  if (session.status !== store.SESSION_STATUS.ACTIVE) {
    return _refuse(409, 'SESSION_NOT_ACTIVE', 'A narrowing applies to a live launch; this session has ended.');
  }
  const sequence = store.launchSequences.getBySession(session.id);
  if (!sequence) return _refuse(409, 'SESSION_NOT_BOUND', 'This session has no launch to narrow.');
  const row = store.workloadReceipts.appendNarrowing({
    project_id: session.projectId,
    session_id: session.id,
    launch_id: sequence.launchId,
    action: b.clear === true ? 'clear' : 'narrow',
    cap_clearance: b.capClearance === true,
    force_unknown: b.forceUnknown === true,
    reason: b.reason,
    operator_proof: caller.actor.operatorProof,
  ...
  return {
    ...
    body: {
      narrowing: {
        sessionId: row.session_id,
        action: row.action,
        capClearance: row.cap_clearance === 1,
        forceUnknown: row.force_unknown === 1,
        reason: row.reason,
