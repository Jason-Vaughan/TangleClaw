 ...
 *
 * The tailnet name comes from `host-inventory#resolveTailnetHost`, the one
 * source every consumer reads (#1905): the configured `caddyTailnetHost`, else
 * the name the overlay probe observed. Reading the config key alone left the
 * detected MagicDNS name out of the certificate and the allowlist on a default
 * install, while the operator links named it.
 *
 * @param {string|null} certPath - Existing cert to carry names forward from.
 * @param {object} config - Loaded TangleClaw config.
 * @param {object} [options]
 * @param {object} [options.observation] - Overlay observation to resolve the
 *   tailnet name against. Defaults to the shared memo.
 * @returns {string[]} Deduplicated host list for mkcert.
 */
function certHostUnion(certPath, config, options = {}) {
  ...
  return [...new Set([
    ...
    hostInventory.resolveTailnetHost(config, options.observation).host,
    (config && config.publicDomain) || null
  ].filter(Boolean))];
}

/**
 * The certificate names a consumer depends on, which no request may remove (#1905).
 *
 * - The mkcert defaults: the direct listener and Caddy's local site answer to them.
 * - The mDNS name: Caddy's LAN site and the served-Host allowlist use it.
 * - The canonical tailnet host, when there is one: the Host allowlist, the
 *   operator links and (when configured) the Caddy tailnet site name it.
 *
 * `publicDomain` is deliberately absent: its site uses an ACME certificate, not
 * this one.
 *
 * @param {object} config - Loaded TangleClaw config.
 * @param {object} [options]
 * @param {object} [options.observation] - Overlay observation; defaults to the shared memo.
 * @param {string} [options.hostname] - Override for `os.hostname()` (tests).
 * @returns {string[]} Deduplicated, lower-cased names.
 */
function mandatoryCertHosts(config, options = {}) {
  const rawHostname = Object.prototype.hasOwnProperty.call(options, 'hostname')
    ? options.hostname
    : os.hostname();
  return [...new Set([
    ...MKCERT_HOSTS_DEFAULT,
    mdnsHostFor(rawHostname),
    hostInventory.resolveTailnetHost(config, options.observation).host
...
/**
 * Path of the installed server's launchd plist, in the one per-user
 * `~/Library/LaunchAgents` launchd reads (never under a relocated TangleClaw home).
 *
 * @returns {string}
 */
function installedServerPlistPath() {
  return path.join(require('./tangleclaw-home').userHome(), 'Library', 'LaunchAgents', 'com.tangleclaw.server.plist');
}

/**
 * The port the INSTALLED TangleClaw service listens on, for a script running
 * out of process: the server plist's `TANGLECLAW_PORT`, else config, else the
 * shipped default.
 *
 * Not {@link effectiveServerPort}: a script's own `TANGLECLAW_PORT` describes
 * whoever launched its shell (a TangleClaw-spawned pane inherits the server's),
 * not the service. And not `config.serverPort` alone: the installed plist sets
 * 3102 and never touches config, which stays at 3101, so every tool that must
 * find TangleClaw's routes in the Caddyfile, or ask the running server a
 * question, would look at a port nothing listens on — and a Caddyfile route to
 * the real port reads as "not TangleClaw's".
 *
 * @param {string} [serverPlistPath] - Defaults to {@link installedServerPlistPath}.
 * @param {object} config - Global config (`store.config.load()`).
 * @returns {number}
 */
function installedServerPort(serverPlistPath, config) {
  try {
    const xml = fs.readFileSync(serverPlistPath || installedServerPlistPath(), 'utf8');
    const m = xml.match(/<key>TANGLECLAW_PORT<\/key>\s*<string>(\d+)<\/string>/);
    if (m) return Number(m[1]);
  } catch (err) {
    // Not installed (a direct run, a test home): config is the next authority.
    if (err.code !== 'ENOENT') log.warn('Could not read the server plist for its port; using config', { error: err.message });
  }
  return (config && config.serverPort) || store.DEFAULT_CONFIG.serverPort;
}
