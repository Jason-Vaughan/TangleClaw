...
class ProcessLedger {
  ...
  constructor(rootPid, rootStart = null) {
    this.root = rootPid;
    // Identity is PID + start time: a PID alone may since belong to someone else.
    this.procs = new Map([[rootPid, rootStart]]);
    // Group id → its leader's start time (null when the leader was never seen).
    this.groups = new Map();
  ...
  record(table) {
    ...
    if (rootRow && this.procs.get(this.root) === null) this.procs.set(this.root, rootRow.lstart);
    for (const r of descendantsOf(table, this.root)) {
      if (!this.procs.has(r.pid)) this.procs.set(r.pid, r.lstart);
      this.pids.add(r.pid);
      if (r.pgid !== this.root) {
        this.pgids.add(r.pgid);
        const leader = table.find((x) => x.pid === r.pgid);
        if (!this.groups.has(r.pgid) || (this.groups.get(r.pgid) === null && leader)) {
          this.groups.set(r.pgid, leader ? leader.lstart : null);
        }
      }
    }
  }
  ...
  /**
   * Processes in a table that belong to this run: a recorded process (same PID
   * AND start time), or a member of a recorded process group — which catches a
   * child forked after the last sample — unless that group id has visibly been
   * reused (a live leader with a different start time).
   * @param {Array<object>} table - From `parseProcTable`.
   * @returns {Array<object>}
   */
  survivors(table) {
    const leaderOf = new Map(table.map((r) => [r.pid, r]));
    return table.filter((r) => {
      if (this.procs.has(r.pid)) {
        const start = this.procs.get(r.pid);
        if (start === null || start === r.lstart) return true;
      }
      if (this.groups.has(r.pgid)) {
        const recorded = this.groups.get(r.pgid);
        const leader = leaderOf.get(r.pgid);
        if (!leader || recorded === null || leader.lstart === recorded) return true;
      }
      return false;
    });
  }
...
/**
 * Whether an `lsof` result can be read as a (possibly partial) true reading.
 * lsof exits 1 when a listed process vanished or could not be read, and what
 * it printed is still true of the rest. A timeout, a signal or an overflowing
 * buffer means the output was CUT OFF, and a cut-off reading must never pass a
 * run: it is unmeasured.
 * @param {Error|null} err - `execFile`'s error.
 * @param {string} stdout - What lsof printed.
 * @returns {string|null} The output to parse, or null when unmeasured.
 */
function lsofOutput(err, stdout) {
