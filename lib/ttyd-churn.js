 ...
 * reading ONLY in lsof's recognized ordinary case: some listed processes
 * vanished before lsof reached them. That is checked, not assumed — every
 * requested process missing from the output must now be gone, and the output
 * must name no process that was not requested. Any other exit 1 (a permission
 * failure, a malformed request) is unmeasured, never an empty measurement.
 ...
 * @param {(pid: number) => boolean} isAlive - Whether a PID is still running.
 * @returns {string|null} The output to parse, or null when unmeasured.
 */
function lsofOutput(err, stdout, requested, isAlive) {
  const out = stdout || '';
  if (!err) return out;
  ...
  for (const pid of requested) {
    if (!reported.has(pid) && isAlive(pid)) return null;
  }
  return out;
}
