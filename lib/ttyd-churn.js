...
/**
 * Parse `ps -A -o pid=,ppid=,pgid=,stat=,etime=` into a process table.
 * @param {string} out - The `ps` stdout.
 * @returns {Array<{pid: number, ppid: number, pgid: number, stat: string, etime: string}>}
 */
function parseProcTable(out) {
  const rows = [];
  for (const line of String(out).split('\n')) {
    const m = line.trim().match(/^(\d+)\s+(\d+)\s+(\d+)\s+(\S+)\s+(\S+)$/);
    if (m) rows.push({ pid: +m[1], ppid: +m[2], pgid: +m[3], stat: m[4], etime: m[5] });
  }
  return rows;
}
...
class ProcessLedger {
  ...
  record(table) {
    for (const r of descendantsOf(table, this.root)) {
      this.pids.add(r.pid);
      if (r.pgid !== this.root) this.pgids.add(r.pgid);
    }
  }

  /**
   * Processes in a table that belong to this run: a recorded PID, or anything in
   * a recorded process group (which catches a child forked after the last sample).
   * @param {Array<object>} table - From `parseProcTable`.
   * @returns {Array<object>}
   */
  survivors(table) {
    return table.filter((r) => this.pids.has(r.pid) || this.pgids.has(r.pgid));
  }
}
