 ...
 * was asked about. A clean exit is. A timeout, a signal or an overflowing
 * buffer means the output was CUT OFF and is never a reading. Exit 1 is a
 * reading ONLY in lsof's recognized ordinary cases: a listed process vanished
 * before lsof reached it, or is exiting / a zombie, whose file table the kernel
 * has already torn down so lsof has nothing to list (the PTY masters its parent
 * ttyd still holds for it ARE listed, under ttyd). That is checked, not
 * assumed — every requested process missing from the output must be gone or
 * exiting, and the output must name no process that was not requested. Any
 * other exit 1 (a permission failure, a malformed request) is unmeasured,
 * never an empty measurement.
 * @param {Error|null} err - `execFile`'s error.
 * @param {string} stdout - What lsof printed (`-F pn`).
 * @param {number[]} requested - The PIDs lsof was asked about.
 * @param {(pid: number) => boolean} mustBeReported - True for a PID lsof should
 *   have listed: running, and not exiting or a zombie.
 * @returns {string|null} The output to parse, or null when unmeasured.
 */
function lsofOutput(err, stdout, requested, mustBeReported) {
  const out = stdout || '';
  if (!err) return out;
  ...
  for (const pid of requested) {
    if (!reported.has(pid) && mustBeReported(pid)) return null;
  }
  return out;
}
