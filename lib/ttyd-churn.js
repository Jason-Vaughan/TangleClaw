...
// The watcher's own test for "exiting", so the harness counts exactly the
// children the watcher would.
const { _isExiting } = require('./ttyd-watcher');

// Host guards. A run shares the machine's PTY pool with the live service, so it
// is capped hard and stopped early.
...
// Acceptance for a candidate that may ship: this many guarded cycles, then this
// long a quiet soak, with zero confirmed wedges, zero restarts, and the run's
// resources back at baseline within the return window.
 ...
 * the watcher's wedge age from data rather than a guess. A child counts as
 * exiting by the watcher's own predicate (`_isExiting`), so the two agree on
 * every state string.
 */
...
class ProcessLedger {
  /**
   * @param {number} rootPid - The scratch ttyd, launched as its own process-group leader.
   * @param {object} [opts]
   * @param {Array<{pid: number, lstart: string}>} [opts.notOwned] - Exact identities
   *   that are provably NOT the run's (the harness itself), excluded by PID AND
   *   start time only — never by group, which could hide a real late leak.
   */
  constructor(rootPid, opts = {}) {
    this.root = rootPid;
    ...
    this.procs = new Map([[rootPid, new Set()]]);
    // Group id → every start time seen for its leader. The root's OWN group is
    // recorded too: the scratch ttyd leads it, so a child caught before it
    // moved into a session of its own — and anything it left behind — is found
    // there after ttyd is gone.
    this.groups = new Map();
    this.pids = new Set([rootPid]);
    this.pgids = new Set();
    this.notOwned = (opts.notOwned || []).map((r) => `${r.pid}@${r.lstart}`);
  }
  ...
  /**
   * Record the scratch ttyd and every current descendant, with their groups.
   * @param {Array<object>} table - From `parseProcTable`.
   * @returns {void}
   */
  record(table) {
    const byPid = new Map(table.map((r) => [r.pid, r]));
    const rootRow = byPid.get(this.root);
    if (rootRow) {
      ProcessLedger._add(this.procs, this.root, rootRow.lstart);
      if (rootRow.pgid === this.root) {
        this.pgids.add(this.root);
        ProcessLedger._add(this.groups, this.root, rootRow.lstart);
      }
    }
    for (const r of descendantsOf(table, this.root)) {
      if (this._excluded(r)) continue;
  ...
  /**
   * Whether a row is one of the exact identities declared not owned.
   * @param {object} r - A process-table row.
   * @returns {boolean}
   */
  _excluded(r) {
    return this.notOwned.includes(`${r.pid}@${r.lstart}`);
  }
   ...
   */
  owns(r) {
    if (this._excluded(r)) return false;
   ...
   * reused (a live leader whose start time was never seen for it). Exact
   * identities declared not owned are never survivors.
   * @param {Array<object>} table - From `parseProcTable`.
   * @returns {Array<object>}
   */
  survivors(table) {
 ...
 * was asked about. A clean exit is. A
...
function lsofOutput(err, stdout, requested, after) {
  ...
  for (const r of omitted) {
    ...
    // The watcher's own exiting predicate: in macOS `ps` state strings,
    // uppercase E and Z mean only "trying to exit" and "zombie".
    if (!_isExiting(now.stat)) return null;
  }
  return out;
}

/**
 * Name the cause of a failed probe as `<probe>: <message>`, the way the
 * watcher's readings name theirs, so a run that could not measure says which
 * measurement failed and why. Multi-line messages (an `execFile` error carries
 * the command's stderr) are folded onto one line.
 * @param {string} probe - The probe: `pool`, `ps`, `lsof`, `which`, `health`.
 * @param {Error|string|null|undefined} err - What it failed with.
 * @returns {string}
 */
function probeError(probe, err) {
  const msg = err && typeof err === 'object' && 'message' in err ? err.message : String(err);
  return `${probe}: ${String(msg).trim().replace(/\s*\n\s*/g, ' | ') || 'failed with no message'}`;
}

/**
 * Why `lsofOutput` refused an lsof result, as a `probeError` cause. Only
 * meaningful when `lsofOutput` returned null, which needs an `err`.
 * @param {Error} err - `execFile`'s error.
 * @param {Map<number, object>|null} after - The post-lsof state read `lsofOutput` was given.
 * @returns {string}
 */
function lsofRefusal(err, after) {
  if (err.killed || err.signal || err.code !== 1) return probeError('lsof', err);
  if (!after) return probeError('lsof', 'exit 1 omitted requested processes, and their state could not be read after it');
  return probeError('lsof', 'exit 1 named a process not requested, or omitted one that is still running');
}
 ...
 * - `candidate` is a fix: it must meet the acceptance above in full
 *   (`ACCEPT_CYCLES`, `ACCEPT_SOAK_MS`, zero wedges and restarts, and its
 *   resources measured back at baseline).
 *
 * @param {object} r
 ...
 * @param {string[]} [r.stopErrors] - For an `aborted-unmeasured` stop, the cause of each
 *   measurement that failed (`probeError`), named in the reason.
 * @param {string[]} [r.resourceErrors] - The cause of each failed resource-return
 *   measurement, named when a candidate is inconclusive for want of one.
...
function verdict(r) {
  ...
  if (r.stop === 'aborted-unmeasured') return { verdict: 'inconclusive', why: [...why, withCauses('a measurement failed mid-run', r.stopErrors)] };
  ...
  if (r.ownedPtysReturned === null || r.fdsReturned === null || r.lingering === null) {
    short.push(withCauses('resource return was not measured', r.resourceErrors));
  }
...
/**
 * A reason, followed by the causes behind it when any were recorded.
 * @param {string} reason
 * @param {string[]|undefined} causes - `probeError` strings.
 * @returns {string}
 */
function withCauses(reason, causes) {
  return causes && causes.length ? `${reason}: ${causes.join('; ')}` : reason;
}
