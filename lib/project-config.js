...
const DEFAULT_PROJECT_CONFIG = {
  ...
  // written reconciliation. `operator` is the shipped default, per the
  // operator's R3 ruling.
  launchSequence: {
    pasteRules: 'pull',
 ...
 *
 * An unrecognised value reads as `operator`, the BLOCKING side — the mirror of
 * `resolvePasteRules`, which falls back to `paste` for the same reason and not
 * to the same word. The shared rule is that a typo falls back to whichever side
 * fails safe for the question that setting asks. There, the question is whether
 * rule text is dropped, so the safe side is the one that still delivers it;
 * here, the question is whether a launch with a bad handoff can walk through
 * unattended, so the safe side is the one that stops and asks a person.
 * @param {object|null|undefined} projConfig - A config as returned by `load`
 * @returns {{mode: 'operator'|'advisory', source: 'launchSequence'|'default'|'invalid', warning?: string}}
 */
function resolveRecoveryMode(projConfig) {
  const value = projConfig && projConfig.launchSequence ? projConfig.launchSequence.recoveryMode : undefined;
  if (value === undefined || value === null) {
    return { mode: DEFAULT_PROJECT_CONFIG.launchSequence.recoveryMode, source: 'default' };
  }
  if (RECOVERY_MODES.includes(value)) return { mode: value, source: 'launchSequence' };
  return {
    mode: 'operator',
    source: 'invalid',
    warning: `launchSequence.recoveryMode ${JSON.stringify(value)} is not one of ${RECOVERY_MODES.join(', ')}; `
