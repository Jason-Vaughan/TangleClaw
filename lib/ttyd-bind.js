...
/**
 * Is this `--interface` value a unix-socket path (caddy-mode wiring)?
 * @param {string} iface - The bound interface.
 * @returns {boolean}
 */
function isSocketBind(iface) {
  return iface.startsWith('/');
}
 ...
 * socket — never by listing the wide shapes, because the wide shapes are
 * open-ended (a LAN address, an interface name, `0.0.0.0`, no `--interface`)
 * and a list misses one (#1056). A bind this function cannot classify is
 * treated as exposed: a false alarm costs a notice, silence costs an open
 * shell.
 *
...
function describeInstalledBind(args) {
  ...
  const wide = iface === null || !(isLoopbackBind(iface) || isSocketBind(iface));
 ...
 *   describes the job AS IT STANDS — whether it is currently reachable from
 *   off the machine — never the state a successful apply would produce. Callers use it
...
function planReconcile(xml, config) {
  ...
  if (current.iface && isSocketBind(current.iface)) {
    return {
      action: 'refuse',
