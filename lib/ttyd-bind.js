 ...
 * @returns {{action: string, reason: string, rolledBack?: boolean}}
 */
function reconcileInstalledJob({ home, config, deps }) {
  ...
  if (!fs.existsSync(plistPath)) return { action: 'none', reason: 'no-installed-plist' };

  let plan;
  try {
    plan = planReconcile(fs.readFileSync(plistPath, 'utf8'), config);
  } catch (err) {
    return { action: 'refuse', reason: `unreadable: ${err.message}` };
  }
  ...
  try {
    ...
    return { action: 'refuse', reason: `staging-failed: ${err.message}` };
  }
  ...
  try {
    ...
    return { action: 'rewrite', reason: plan.reason };
  } catch (err) {
    ...
    try {
      ...
      return { action: 'refuse', reason: `reload-failed: ${err.message}`, rolledBack: true };
    } catch (restoreErr) {
      log.error(
        restored
