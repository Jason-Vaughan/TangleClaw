...
function planReconcile(xml, config) {
  ...
  return {
    ...
    // The rewrite closes it, so there is nothing left to warn about. This was
    // `undefined` until a test asked every path for a boolean — and `undefined`
    // is exactly the value the caller's `!== false` guard would have read as
    // "still wide", raising an exposure warning about a door it had just shut.
    stillWide: false
  };
}
