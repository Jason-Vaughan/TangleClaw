...
async function _probe(projectPath, exec, base) {
  ...
  if (head.exitCode === 0 && head.stdout.trim()) {
    ...
    if (view.exitCode === 0 && view.stdout.trim()) {
      ...
      return {
        ...base, state: 'unknown',
        reason: `origin/HEAD is not set locally (git remote set-head origin -a) and gh could not name the default branch: ${describeFailure(view, 'gh repo view')}`
      };
    }
  }
  ...
  if (list.exitCode !== 0) return { ...base, branch, state: 'unknown', reason: describeFailure(list, 'gh run list') };
  let runs;
  try {
...
function primeLines(result) {
  ...
  if (result.state === 'failing') {
    return [
      ...
      + 'Tell the operator in your opening: in the resume proposal where this launch has a sequence, '
      + 'otherwise in your first message.',
      ''
    ];
  }
  ...
  if (result.state === 'unknown') {
    return [
      ...
      + 'Mention it to the operator in your opening — in the resume proposal where this launch has '
      + 'a sequence, otherwise your first message; they are the one who can fix the probe._',
      ''
    ];
  }
  return [];
