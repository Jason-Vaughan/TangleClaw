 ...
 * @param {{stderr: string, errorCode: (string|null), timedOut: boolean}} r - The exec result.
 * @param {string} what - The command, for the fallback sentence.
 * @returns {string}
 */
function _reasonFrom(r, what) {
  if (r.errorCode === 'ENOENT') return `${what.split(' ')[0]} is not installed`;
  if (r.timedOut) return `${what} timed out after ${EXEC_TIMEOUT_MS}ms`;
  const line = (r.stderr || '').split('\n').map((l) => l.trim()).find((l) => l.length > 0);
  // `gh` reaches a remote, so its stderr can echo a tokenised URL and this
  // reason is rendered to the operator.
  const safe = redactRemoteOutput(line);
  // A wholesale replacement is TRUTHY, so `safe || <fallback>` would return a
  // bare `[redacted — …]` and drop the command name and exit code with the
  // secret — leaving the operator a reason that says nothing about what failed.
  // The exit code is not a credential; name it alongside.
  if (safe && safe.startsWith(REDACTED_PREFIX)) return `${what} failed (exit ${r.exitCode}): ${safe}`;
  return safe || `${what} failed (exit ${r.exitCode})`;
}
...
async function _probe(projectPath, exec, base) {
  ...
  if (remote.exitCode !== 0) {
    ...
    if (remote.errorCode === 'ENOENT') return { ...base, state: 'unknown', reason: 'git is not installed' };
