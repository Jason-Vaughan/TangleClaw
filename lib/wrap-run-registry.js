 ...
 *
 * The run keeps a copy of the operator's `options` so a reloaded page can
 * pick up the choices a Retry must replay (#1492). Held only in page memory,
 * a Hold chosen before a reload became Auto after it, and on a ready `auto`
 * project the next Retry cut the release the operator had refused.
 *
 ...
 *
 * @param {string} projectName - Registry key
 * @returns {{runId: string|null, running: boolean, stale: boolean, sessionId: number|null, startedAt: number|null, currentStepId: string|null, currentStepStartedAt: number|null, finishedAt: number|null, result: object|null, options: object|null}}
 */
function get(projectName) {
  const run = _runs.get(projectName);
  if (!run) {
    return { runId: null, running: false, stale: false, sessionId: null, startedAt: null, currentStepId: null, currentStepStartedAt: null, finishedAt: null, result: null, options: null };
  }
  return {
    runId: run.runId,
    running: _isLive(run),
...
module.exports = { begin, emit, finish, get, subscribe, anyRunning, STALE_RUN_MS, _resetForTests, _internal };
