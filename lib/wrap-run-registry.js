...
function updateStep(projectName, stepId) {
  ...
  events.emit(projectName, { type: 'step-start', stepId });
}
 ...
 * @returns {void}
 */
function stepDone(projectName, stepId, result) {
  const run = _runs.get(projectName);
  if (!run || !run.running) return;
  events.emit(projectName, { type: 'step-done', stepId, result });
}
...
function finish(projectName, result) {
  ...
  events.emit(projectName, { type: 'pipeline-done', result: result == null ? null : result });
}
...
module.exports = { begin, updateStep, stepDone, finish, get, anyRunning, STALE_RUN_MS, events, _resetForTests, _internal };
