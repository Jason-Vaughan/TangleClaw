 ...
 *
 * @param {string} projectName - Registry key
 * @returns {{runId: string|null, running: boolean, stale: boolean, sessionId: number|null, startedAt: number|null, currentStepId: string|null, currentStepStartedAt: number|null, finishedAt: number|null, result: object|null}}
 */
function get(projectName) {
  const run = _runs.get(projectName);
  if (!run) {
    return { runId: null, running: false, stale: false, sessionId: null, startedAt: null, currentStepId: null, currentStepStartedAt: null, finishedAt: null, result: null };
  }
  return {
    runId: run.runId,
    running: _isLive(run),
