 ...
 * @param {string} [baseDir] - TangleClaw's base directory. Defaults to the
 *   install's own (`lib/tangleclaw-home.js`); tests pass a scratch directory.
 * @returns {string} Absolute path to the installed attach script.
 */
function attachScriptPath(baseDir) {
  return path.join(baseDir || tangleclawHome.baseDir(), 'deploy', ATTACH_BASENAME);
}
 ...
 * @param {string} [opts.baseDir] - TangleClaw's base directory; defaults to
 *   the install's own.
 ...
 */
function syncAttachScript({ repoDir, baseDir }) {
  const dest = attachScriptPath(baseDir);
  try {
