...
async function resolve(cwd, prRef) {
  ...
  if (!isValidPrRef(prRef)) {
    return { ...base, reason: 'not a valid PR reference (expected a github.com PR URL or a number)' };
  }
  let res;
  try {
    res = await _internal.exec('gh', ['pr', 'view', prRef, '--json', GH_JSON_FIELDS], { cwd });
  } catch (err) {
    return { ...base, reason: `gh pr view threw: ${err.message}` };
  }
  if (res.exitCode !== 0) {
    const detail = (res.stderr || res.stdout || '').trim().slice(0, 300);
    // `gh` missing, not authed, or the PR not found — all indeterminate, not
    // a failed release.
    return { ...base, reason: `gh pr view failed (exit ${res.exitCode}): ${detail}` };
  }
  let data;
  try {
    data = JSON.parse(res.stdout);
  } catch (err) {
    return { ...base, reason: `could not parse gh output: ${err.message}` };
  }
