...
/**
 * Terminal check outcomes that count as passed. GitHub treats a neutral or
 * skipped required check as satisfied, so they do not hold a PR back; at least
 * one real success is still required (see {@link allChecksPassed}).
 * @type {Set<string>}
 */
const PASSING_CHECK_STATES = new Set(['SUCCESS', 'NEUTRAL', 'SKIPPED']);

/**
 * Whether a `statusCheckRollup` shows the PR's CI finished green: at least one
 * check, every check completed with a passing outcome, and at least one of them
 * an outright success. A PR with no checks, or with any check still running,
 * has not shown that it will land. Reads both rollup shapes, as
 * {@link hasFailingCheck} does.
 *
 * @param {Array<object>|undefined|null} rollup - The `statusCheckRollup` array
 * @returns {boolean}
 */
function allChecksPassed(rollup) {
  if (!Array.isArray(rollup) || rollup.length === 0) return false;
  let sawSuccess = false;
  for (const c of rollup) {
    if (!c || typeof c !== 'object') return false;
    const isContext = c.state !== undefined && c.conclusion === undefined;
    let outcome;
    if (isContext) {
      outcome = String(c.state || '').toUpperCase();
    } else {
      if (String(c.status || '').toUpperCase() !== 'COMPLETED') return false;
      outcome = String(c.conclusion || '').toUpperCase();
    }
    if (!PASSING_CHECK_STATES.has(outcome)) return false;
...
async function resolve(cwd, prRef) {
  ...
  if (res.exitCode !== 0) {
    // `gh` missing, stopped, not authed, or the PR not found — all
    // indeterminate, not a failed release. The shared wording redacts, since
    // `gh pr view` reaches a remote.
    return indeterminate(describeFailure(res, 'gh pr view'));
  }
  let data;
  try {
...
module.exports = { resolve, classify, hasFailingCheck, allChecksPassed, isValidPrRef, _internal, PR_URL_RE };
