 ...
 * to push the error line out of the last `TMUX_TAIL_LINES` rows. An error
...
function validatePatterns(patterns) {
  ...
  patterns.forEach((entry, i) => {
    ...
    if (typeof entry.regex !== 'string' || entry.regex.length === 0) {
      errors.push(`errorPatterns[${i}].regex must be a non-empty string`);
    } else {
      try {
        new RegExp(entry.regex);
      } catch (err) {
        errors.push(`errorPatterns[${i}].regex does not compile: ${err.message}`);
      }
    }
...
function compilePatterns(profile) {
  ...
  for (const entry of profile.errorPatterns) {
    if (!entry || typeof entry.regex !== 'string' || !PARSERS[entry.parser]) continue;
    try {
      compiled.push({ regex: new RegExp(entry.regex), parser: PARSERS[entry.parser], parserName: entry.parser });
    } catch (err) {
      log.warn('errorPatterns entry skipped — regex does not compile', {
        engine: profile.id, regex: entry.regex, error: err.message
      });
    }
  }
...
/**
 * Strip ANSI escapes and CR from a captured row.
 * @param {*} line - Raw row.
 * @returns {string}
 */
