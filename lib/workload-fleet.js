 ...
 * @returns {{wrapRun: function(string): object, wrapRequested: function(string): boolean, wrapRequestedAt: function(number): (number|null)}}
 */
function defaultWrapDeps() {
  return {
    ...
    wrapRequestedAt: (sessionId) => require('./wrap-sentinel').wrapRequestedAt(sessionId)
  };
}
...
function laneFor(session, { observer, projectName, nowMs = Date.now(), wrap = defaultWrapDeps() }) {
  ...
  const { composed, workload: block } = compose.composeLane({
    ...
    wrapRequestedAtMs: wrap.wrapRequestedAt ? wrap.wrapRequestedAt(session.id) : null,
    controlState,
    engine,
