...
const log = createLogger('upstream-observer');
...
function refresh(identity, member) {
  ...
  // `observe` resolves rather than rejects; the catch is for a defect, so a
  // throw cannot leave this identity's slot in flight — and `pending` — forever.
  const p = observe(member).catch((err) => ({
    state: 'unknown', sha: null, observedAt: null, observedFrom: member.name,
    reason: `observation failed: ${gitProbe.scrubLocations(err && err.message)}`
  })).then((value) => {
    if (value.state === 'unknown') {
      log.info('Upstream observation failed — related sessions will read it as unknown', { identity, from: member.name, reason: value.reason });
    }
