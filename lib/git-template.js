...
/**
 * The install's git-template root, under TangleClaw's one base directory
 * (`lib/tangleclaw-home.js`).
 *
 * A function rather than a constant so `__setTemplateDir(null)` re-derives it
 * instead of restoring a value frozen at module load — the reset has to land
 * back on the base directory the rest of the install is using.
 *
 * @returns {string} Absolute path to `<base>/git-template`.
 */
function _defaultTemplateDir() {
  return path.join(tangleclawHome.baseDir(), 'git-template');
}
...
// `null` until overridden: the default is derived on every read, so setting
// TANGLECLAW_HOME cannot leave this module pointing at a base nothing else uses.
let _templateDir = null;
...
/**
 * Where this install puts its git template.
 *
 * Exported because "which directory is this install actually using" is a
 * question with an answer only this module holds, and the answer has to be
 * checkable from outside: it is one of the sites that derives TangleClaw's base
 * directory, and the guarantee that they all derive it the same way is worth
 * nothing if it cannot be observed.
 *
 * @returns {string} Absolute path to the git-template root.
 */
function templateDir() {
  return _templateDir || _defaultTemplateDir();
}

function _hookPath() {
  return path.join(templateDir(), 'hooks', 'commit-msg');
}
...
function _sentinelPath() {
  ...
  return path.join(templateDir(), '.tc-init-templatedir-owned');
}
...
function installGlobalTemplate() {
  ...
  let existingPreRefresh = null;
  if (fs.existsSync(hookPath)) {
    try {
      existingPreRefresh = fs.readFileSync(hookPath, 'utf8');
      if (existingPreRefresh === source) {
        hookAction = 'idempotent';
      }
    } catch (_) { /* fall through to refresh */ }
  }
  if (hookAction === 'refreshed') {
    // PR-review (#253 Finding 2) — symmetric with `lib/git-hooks.js`'s
    // per-project refresh path: emit a `log.warn` BEFORE clobbering a
    // pre-existing template hook so operator hand-edits (against the
    // unusual but legal case of someone customizing TC's template dir
    // in place) leave a breadcrumb. `~/.tangleclaw/` is unambiguously
    // TC's namespace so this is lower-probability than the per-project
    // case, but the visibility cost is zero and the symmetry matches
    // the #240 drift-aware-write contract.
    if (existingPreRefresh !== null) {
      log.warn('Template-dir commit-msg hook drifted from shipped content; refreshing (operator edits will be overwritten)', {
        hookPath
      });
    }
  ...
  if (!currentValue) {
    if (!_setGlobalTemplateDir(templateDir())) {
      ...
      //
      // PR-review (#253 Finding 3) — surface a second-order orphan if
      // the rollback ALSO fails. Both writes failing means the host has
      // a serious problem (disk full, git binary disappeared
      // mid-sequence, perms revoked), and the operator's gitconfig is
      // now stranded with no way back. `log.error` so the breadcrumb
      // names this exact path; a stale `init.templateDir` pointing at a
      // missing TC dir is benign for `git init` (git falls back to its
      // built-in default), but it's wrong-state we want surfaced.
      if (!_unsetGlobalTemplateDir()) {
        log.error('Failed to roll back init.templateDir after sentinel-write failure — gitconfig left pointing at TC template dir without sentinel; future uninstall cannot revert', {
          tcPath: templateDir()
        });
      }
  ...
  } else if (_samePath(currentValue, templateDir())) {
    ...
    log.warn('git config --global init.templateDir already set to a non-TC path; global template install skipped (per-project hooks still work)', {
      existingValue: currentValue, tcPath: templateDir()
    });
...
function uninstallGlobalTemplate() {
  ...
  if (_hasSentinel()) {
    ...
    const currentValue = _readGlobalTemplateDir();
    if (currentValue && _samePath(currentValue, templateDir())) {
      if (_unsetGlobalTemplateDir()) {
