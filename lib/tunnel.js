...
function _registerTunnelPort(localPort, projectName) {
  // The tunnel is already listening when this runs, and it is TangleClaw's own
  // SSH child, so the listener is adopted rather than refused.
  porthub.registerPort(localPort, projectName, 'openclaw-tunnel', { permanent: false, ttlMs: 86400000, adoptListener: true });
}
