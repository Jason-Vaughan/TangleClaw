 ...
 * @param {number} localPort - Local tunnel port (probed on 127.0.0.1, where ssh binds locally)
 * @param {number} [timeoutMs=4000] - Abort the probe after this long
...
async function httpRoundTrip(localPort, timeoutMs = 4000) {
  ...
  try {
    ...
    // Any status code means headers arrived — transport is live end-to-end.
    return typeof res.status === 'number';
  } catch {
    // AbortError (timeout) / ECONNRESET / ECONNREFUSED / socket hang → dead.
    return false;
  } finally {
    clearTimeout(timer);
  }
...
async function ensureTunnel(projectName, config) {
  ...
  const bound = await tcpProbe(localPort);
...
async function killTunnelByPort(localPort, host) {
  ...
  if (!pid) {
    ...
    const released = !(await tcpProbe(localPort));
