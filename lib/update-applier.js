...
function _managedRegionOnlyDiff(relPath) {
  ...
  try {
    ...
    if (headOutside === null) return decline('committed copy has no single well-formed managed block');
    if (workOutside === null) return decline('working copy has no single well-formed managed block');
    if (headOutside !== workOutside) return decline('the file differs outside the managed block');
 ...
 *
 * #1022 moved TangleClaw's hooks OUT of `.claude/settings.json` and into
 * `settings.local.json`, which narrows but does not invalidate the line's claim
 * about the shared file: TangleClaw still writes it once per project to RETIRE
 * the entry an older version left there, and that migration write is exactly the
 * dirt a self-update must be able to discard rather than refuse over. The two
 * remain on opposite sides for the reason stated above — the local file is where
 * the operator's per-machine permissions live, so it stays theirs even though
 * TangleClaw now writes into it too, and discarding it would take those with it.
 * Narrowing the set once every project has migrated is a ruling to make then,
 * with an inventory behind it, not an inference to make now.
 *
...
module.exports = {
  ...
  _managedRegionOnlyDiff, _outsideManagedBlock,
  MANAGED_BLOCK_CARRIERS, MANAGED_BLOCK_SYNTAX
};
