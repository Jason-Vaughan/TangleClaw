 ...
 *
 * Nothing here reads the folder on the event loop, where a blocked read stops
 * the whole server: the folder is read by spawned `git` (killed by its
 * timeout), and whether it exists at all comes from the project list's scanner.
 *
...
async function _git(cwd, args) {
  ...
  if (r.error && r.error.code === 'ENOENT') throw new ReadFailed(await strandedCheck.spawnFailure(cwd, _internal.exec));
 ...
 */
async function _probe(project, session, folder) {
