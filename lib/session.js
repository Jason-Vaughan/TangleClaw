...
<body>
  ...
  <script>
    ...
    // ── Prompt Chime (SSE-based) ──
    var _chimeEnabled = false;
    var _chimeAudioCtx = null;
    var _chimeEventSource = null;
    ...
    function connectChimeSSE() {
      if (_chimeEventSource) _chimeEventSource.close();
      _chimeEventSource = new EventSource('/api/events');
      _chimeEventSource.onmessage = function(e) {
        try {
          var data = JSON.parse(e.data);
          if (data.event === 'idle' && data.session === _chimeSessionName) {
            playChime();
          }
        } catch {}
      };
    ...
    function disconnectChimeSSE() {
      if (_chimeEventSource) {
        _chimeEventSource.close();
        _chimeEventSource = null;
      }
    }
    ...
    async function toggleChime(enabled) {
      ...
      localStorage.setItem(_chimeSettingsKey, JSON.stringify({ enabled: enabled }));
    }
      ...
      try {
        ...
        if (saved && saved.enabled) {
          ...
          var _chimeResumeHandler = function() {
            ...
            if (_chimeEnabled && !_chimeEventSource) connectChimeSSE();
