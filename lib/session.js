...
<body>
  ...
  <script>
    ...
    var _lastPeekText = null;
    var _stableCount = 0;        // how many consecutive polls returned same text
    var _wasChanging = false;     // true when output was actively changing
    ...
    async function chimePoll() {
      ...
      try {
        ...
        if (_lastPeekText === null) {
          _lastPeekText = text;
          return;
        }

        if (text !== _lastPeekText) {
          _wasChanging = true;
          _stableCount = 0;
        } else {
          _stableCount++;
          if (_wasChanging && _stableCount >= 1) {
            playChime();
            _wasChanging = false;
          }
        }
    ...
    async function toggleChime(enabled) {
      ...
      if (enabled) {
        ...
        // Must await resume — iOS Safari won't play until context is running
