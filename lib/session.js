...
<body>
  ...
  <div class="modal-overlay" id="settings-modal">
    <div class="settings-modal">
      ...
      <div class="settings-row">
        ...
        <select id="setting-poll-interval" onchange="updatePollInterval(this.value)" style="background:var(--bg);color:var(--text);border:1px solid #333;border-radius:4px;padding:4px 8px;font-size:13px;">
  ...
  <script>
    ...
    var _chimePollInterval = 10;
    var _lastPeekState = 'unknown'; // 'busy' or 'waiting'
    var _chimeSettingsKey = 'tc_chime_' + projectName;

    // Prompt patterns that indicate the terminal is waiting for input
    var _promptPatterns = [
      /\$\s*$/m,                   // bash prompt ending with $
      />\s*$/m,                    // generic prompt ending with >
      /❯\s*$/m,                   // starship/claude prompt
      /╰─\s*$/m,                  // oh-my-zsh style
      /\]\s*$/m,                   // zsh prompt ending with ]
      />>>\s*$/m,                  // python REPL
      /irb.*>\s*$/m,              // ruby REPL
    ];
    ...
    function isPromptWaiting(text) {
      ...
      for (var i = 0; i < _promptPatterns.length; i++) {
        if (_promptPatterns[i].test(lastLine)) return true;
      }
      return false;
    }

    async function chimePoll() {
      if (!_chimeEnabled) return;
      var sessionName = projectName === '__root__' ? 'Projects' : projectName.replace(/[^a-zA-Z0-9_-]/g, '-');
      try {
        var res = await fetch('/api/sessions/' + encodeURIComponent(sessionName) + '/peek');
        var data = await res.json();
        var nowWaiting = isPromptWaiting(data.lines);

        if (nowWaiting && _lastPeekState === 'busy') {
          playChime();
        }
        _lastPeekState = nowWaiting ? 'waiting' : 'busy';
      } catch {
        _lastPeekState = 'unknown';
      }
    }
    ...
    function startChimePoll() {
      stopChimePoll();
      chimePoll();
      _chimePollTimer = setInterval(chimePoll, _chimePollInterval * 1000);
    }
    ...
    function toggleChime(enabled) {
      ...
      if (enabled) {
        // Create AudioContext on user gesture (required by Safari)
        if (!_chimeAudioCtx) {
      ...
      localStorage.setItem(_chimeSettingsKey, JSON.stringify({
        enabled: enabled,
        interval: _chimePollInterval,
      }));
    }

    function updatePollInterval(val) {
      _chimePollInterval = parseInt(val, 10) || 10;
      if (_chimeEnabled) startChimePoll();
      localStorage.setItem(_chimeSettingsKey, JSON.stringify({
        enabled: _chimeEnabled,
        interval: _chimePollInterval,
      }));
    }
    ...
    (function() {
      try {
        ...
        if (saved) {
          if (saved.interval) {
            _chimePollInterval = saved.interval;
            document.getElementById('setting-poll-interval').value = String(saved.interval);
          }
          if (saved.enabled) {
            document.getElementById('setting-chime').checked = true;
            // Delay enabling — AudioContext needs user gesture on iOS
            // We mark it visually but don't start polling until first interaction
            document.getElementById('btn-settings').classList.add('chime-active');
            _chimeEnabled = true;
            // Try to resume on any user tap
