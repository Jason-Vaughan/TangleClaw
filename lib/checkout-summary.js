...
function _relationText(c) {
  ...
  switch (v.relation) {
    ...
    case 'not-compared': return `related repo ${up.identity} (via ${up.groupName ? `group ${up.groupName}` : 'a project group'}) at ${target}${seen}, no checkout comparison`;
    default: {
...
function describe(c) {
  ...
  if (c.state === 'no-git') {
    ...
    // A group member that could not be read may name a second repository, so
    // the relation is undetermined, not absent.
    if (up.state === 'unknown') return [`Checkout: not a git checkout; related repository not determined (${up.reason || 'no reason given'}).`];
