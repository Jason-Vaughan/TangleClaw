 ...
 *
 * `wrap_pipeline.steps` lives in the methodology template, which is shared by
 * every project on that methodology and is framework-owned: `store.js`'s
 * `FRAMEWORK_OWNED_PATHS` deep-clone-REPLACES the whole `steps` subtree
 * whenever a bundled template ships a higher `schemaRevision`. So a project
 * that hand-edits its template to turn one step off loses that edit, silently,
 * at the next boot after a framework bump.
 *
 * Overrides therefore live in the project's own `.tangleclaw/project.json`,
 * which no template-sync path writes. The split mirrors the one already drawn
 ...
 * pinned by one test against the bundled template; if projects could reorder,
 * each project would need its own pin and the guarantee would quietly become
 * per-project. Projects needing genuinely different step order fork a
 * methodology id via `templates.save`, which is unaffected by any of this.
 *
 ...
 *
 * An override keyed to a step id this methodology doesn't contain is inert —
 * it is not an error, because a project may legitimately carry overrides for
 * steps that exist under a methodology it switches back to.
 *
...
module.exports = { OVERRIDABLE_FIELDS, validateOverrides, resolveStep };
