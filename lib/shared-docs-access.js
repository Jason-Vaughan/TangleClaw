 ...
 * re-derives the caller on its own. It answers one of five kinds:
 *
...
const DEFAULT_DEPS = Object.freeze({
  ...
  liveMasterLaunch: () => require('./master').liveMasterLaunchId()
});
 ...
 * @returns {{kind: string, projectId: null, groupIds: string[], reason: (string|null)}}
 */
function _resolveMaster(launchId, deps) {
  const live = deps.liveMasterLaunch();
  if (!live || !live.answered) return _invalid(INVALID_REASONS.MASTER_UNVERIFIABLE);
