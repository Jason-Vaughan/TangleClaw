 ...
 * shared-docs and groups route asks it before any lookup, and states what it
 * needs (see {@link NEEDS}): to read, to write within a group, or the operator.
 ...
 *
 * What each kind may do: the operator anything; a project read and write its
 * own groups (register, lock, notify, sync); the Master read every group and
 * write nothing. Editing or deleting a document and managing groups and their
 * members are the operator's alone: `filePath` decides which file is injected
 * into every member project's engine config, so a caller that could change it
 * could choose another project's hidden context.
 *
 ...
 * lookup. Group membership is checked after the lookup: {@link canSeeGroup}
 * for a read, {@link canWriteGroup} for a write. `operator` admits no one else.
 * @type {Readonly<{READ: string, WRITE: string, OPERATOR: string}>}
 */
...
function refusalFor(access, need = NEEDS.READ) {
  ...
  const bindingRefusal = _bindingRefusal(access);
 ...
 * @returns {{status: number, code: string, message: string}|null}
 */
function _bindingRefusal(access) {
