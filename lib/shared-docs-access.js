 ...
 * own ({@link canChangeProject}). The Master changes no project through the
 * API, whatever its access level: that level governs the files the Master may
 * edit (its write guard, `lib/master.js`), and API authority for the Master is
 * reserved for a scoped grant (#966, ADR 0008).
 *
...
/**
 * Which route family a binding refusal is for. The codes and the wording
 * differ, so an agent refused by a project route is not told about shared
 * documents. {@link refusalFor} answers for shared docs and groups,
 * {@link projectRefusalFor} for projects.
 ...
 * @returns {{kind: string, projectId: (number|null), groupIds: string[], reason: (string|null), sessionId?: number, launchId?: string}}
 ...
 *   `sessionId` and `launchId` are set only for a `project` caller: the verified session and launch.
 */
function resolveAccess(req, deps = DEFAULT_DEPS) {
  ...
  // The session and launch this binding verified, for callers that stamp them
  // (the workload receipts, #1912). The launch id is the one the store just
  // matched, so it is no longer a claim; every existing caller ignores both.
  return {
    kind: KINDS.PROJECT, projectId: launch.projectId, groupIds, reason: null,
    sessionId: launch.sessionId, launchId
  };
}
...
/**
 * The refusal a shared-docs or groups route sends when its caller cannot do
 * what the route needs, or null when it can. A binding refusal names the
 * headers and the environment variables that carry them, so an agent can
 * recover without a relaunch; an operator-only refusal says no binding would
 * help.
 *
 * Group membership is not checked here: a route that finds the group or
 * document outside the caller's groups answers 404, as it does for one that
 * does not exist. Project routes use {@link projectRefusalFor}.
 * @param {{kind: string, reason: (string|null)}} access - From {@link resolveAccess}
 * @param {string} [need] - `read`, `write` or `operator` from {@link NEEDS}; defaults to `read`
 * @returns {{status: number, code: string, message: string}|null}
 */
function refusalFor(access, need = NEEDS.READ) {
...
/**
 * The refusal a project write route sends when its caller cannot do what the
 * route needs, or null when it can. Only two needs exist here, `operator` and
 * `own-project`; any other is a programming error and throws, so a project
 * read route cannot be gated through this by mistake and silently refuse its
 * readers.
 *
 * Whether the caller owns the named project is not checked here: the route
 * asks {@link canChangeProject} once it has found the project.
 * @param {{kind: string, reason: (string|null)}} access - From {@link resolveAccess}
 * @param {string} need - `NEEDS.OPERATOR` or `NEEDS.OWN_PROJECT`
 * @param {string} [action] - What an operator-only route does, completing
 *   "Only the operator can …", e.g. "create a project"
 * @returns {{status: number, code: string, message: string}|null}
 * @throws {TypeError} For any other need
 */
function projectRefusalFor(access, need, action) {
  if (need !== NEEDS.OPERATOR && need !== NEEDS.OWN_PROJECT) {
    throw new TypeError(`projectRefusalFor: unsupported need "${need}"; project routes need operator or own-project`);
  }
  if (need === NEEDS.OPERATOR) {
    if (access.kind === KINDS.OPERATOR) return null;
    return {
  ...
  if (access.kind === KINDS.MASTER) {
    return {
      ...
      message: 'The Project Master cannot change projects through the TangleClaw API. Its access level '
        + '(read-only, suggest or write) governs the files it may edit, not API authority; a scoped API '
        + 'grant for the Master is tracked in #966. A project\'s settings, actions and stranded wraps are '
        + 'changed by that project\'s own session, or by the operator.'
    };
  }
  return null;
}
