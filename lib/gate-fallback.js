...
/**
 * Every top-level route of an adapted config, site lists and error lists alike,
 * with what a gate check needs to know about each — or why the config cannot be
 * read at all.
 *
 * The ONE walk over a config's servers for the two questions asked of it: may
 * TangleClaw stand down behind this door ({@link checkFallbackDoor}), and does
 * this door keep `authEnabled: false` from opening the install
 * (`lib/ingress-door.js`). A shape added here is added to both; two copies would
 * each learn it separately, and a missed one answers "no door".
 *
 * `anyHost` is true when some request can reach the route whatever host it
 * names: the route has no matcher, or one of its matcher sets (they are OR'd)
 * carries no `host` list.
 *
 * @param {object} adapted - `caddy adapt` JSON.
 * @returns {{ unreadable: string|null, routes: Array<{ kind: 'site'|'error', listen: string[],
 *   tls: boolean, route: object, hosts: string[], anyHost: boolean }> }}
 *   `unreadable` names what this cannot follow (then `routes` is empty).
 */
function eachTopLevelRoute(adapted) {
  const unreadable = (reason) => ({ unreadable: reason, routes: [] });
  if (!adapted || typeof adapted !== 'object') return unreadable('no adapted config to read');
  const apps = adapted.apps || {};
  const unknownApps = Object.keys(apps).filter((name) => !PASSIVE_APPS.has(name));
  if (unknownApps.length) {
    return unreadable(`the Caddyfile runs Caddy apps this check cannot read (${unknownApps.join(', ')})`);
  }
  ...
  for (const server of Object.values(servers)) {
    ...
    if (server.named_routes && Object.keys(server.named_routes).length) {
      return unreadable(`the server on ${listen.join(', ')} has named routes, which this check cannot follow`);
    }
    ...
    for (const [kind, list] of lists) {
      for (const route of Array.isArray(list) ? list : []) {
        if (!route) continue;
        const sets = Array.isArray(route.match) ? route.match : [];
        const hosts = [];
        let anyHost = sets.length === 0;
        for (const set of sets) {
          const setHosts = set && Array.isArray(set.host) ? set.host : [];
          if (setHosts.length === 0) anyHost = true;
          hosts.push(...setHosts);
        }
        routes.push({ kind, listen, tls, route, hosts, anyHost });
      }
    }
  }
  return { unreadable: null, routes };
}
...
function checkFallbackDoor(adapted, upstreamPort) {
  ...
  const walk = eachTopLevelRoute(adapted);
  if (walk.unreadable) return refuse(walk.unreadable);
  const probes = [];
  for (const { kind, listen, tls, route, hosts, anyHost } of walk.routes) {
    if (!_mayReachTangleclaw(route, upstreamPort)) continue;
    const where = `the ${kind} route on ${listen.join(', ') || 'no listener'} for ${anyHost ? 'any host' : hosts.join(', ')}`;
    ...
    for (const address of listen) {
      ...
      // A route some request reaches whatever host it names is probed with no host too.
      if (anyHost) probes.push({ port, tls: probeTls, host: null });
