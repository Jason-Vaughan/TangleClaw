...
function refuseInboundIdentity(headers) {
  ...
  return { present: true, proxied: headers[PROXY_EVIDENCE_HEADER] !== undefined };
}

/**
 * Valid `authStatus` values. See {@link resolveAuthStatus}.
 * @type {string[]}
 */
const AUTH_STATUSES = ['off', 'live', 'account-required', 'locked', 'unreadable'];

/**
 * The dashboard's `authStatus`, derived from TangleClaw's gate state — the one
 * owner of whether a login is enforced (`lib/auth-gate.js#resolveGateState`).
 *
 * | gate state | `authStatus` | meaning |
 * |---|---|---|
 * | `open` | `off` | no login required (ADR 0009's opt-out) |
 * | `armed` | `live` | TangleClaw enforces the login |
 * | `account-required` | `account-required` | login on, no account yet — closed |
 * | `locked` | `locked` | accounts exist, none enabled — closed |
 * | `unreadable` | `unreadable` | the gate could not read its state — closed |
 *
 ...
 *
 * An unknown gate state answers `unreadable`, never `off`: a status that fails
 * toward "no login required" would tell the operator the door is open when the
 * code cannot say so.
 *
 * @param {string} gateState - From `lib/auth-gate.js#resolveGateState`
 * @returns {'off'|'live'|'account-required'|'locked'|'unreadable'}
 */
function resolveAuthStatus(gateState) {
  switch (gateState) {
    case GATE_STATES.OPEN: return 'off';
    case GATE_STATES.ARMED: return 'live';
    case GATE_STATES.ACCOUNT_REQUIRED: return 'account-required';
    case GATE_STATES.LOCKED: return 'locked';
