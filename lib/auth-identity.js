 ...
 *
 * The condition is the gate being live — caddy ingress AND the basic_auth gate
 * enabled. Outside that nothing trustworthy sits in front of this server, so an
 * `x-…` header is just a string the caller chose, and believing one lets an
 * unauthenticated caller steer whatever reads it.
 *
 * Exported because more than one header answers to this same condition: the
 * identity header, and the forwarded host the operator-topology fact derives
 * from. Spelling the conjunction twice is how the two drift apart.
 *
...
function resolveRequestUser(headers, config) {
  ...
  if (!isProxyHeaderTrusted(config)) {
    return null;
  }
 ...
 * | `live` | `authEnabled` && TangleClaw's own session gate is active | #1418: TangleClaw enforces the login itself, on ANY ingress mode — checked FIRST, because it is true regardless of what Caddy is doing |
 * | `configured-inert` | `authEnabled` && no TC gate && `ingressMode !== 'caddy'` | AUTH-2: settable-but-inert — nothing is enforcing anything |
 ...
 * @param {boolean} [tcGateActive] - Whether TangleClaw's OWN session gate is
 *   enforcing this request (`lib/auth-gate.js#isGateActive`, carried on the
 *   request as `req.tcGateActive`). Defaults to false so every existing caller
 *   keeps its answer.
 * @returns {'off'|'live'|'configured-inert'|'configured-no-identity'|'configured-bypassed'}
 */
function resolveAuthStatus(headers, config, tcGateActive) {
  if (!config || !config.authEnabled) return 'off';
  // TangleClaw's own gate answers FIRST, because it is the one gate that does
  // not care about ingress mode (#1418). Without this the dashboard told a
  // gated DIRECT-mode install "auth enabled but direct mode is not enforcing
  // it — run the Caddy cutover": a denial of a gate that is live, plus a
  // remedy the operator does not need. That combination is reachable by the
  // shipped path, not a corner — the setup wizard writes `authEnabled: true`
  // in direct mode, and `scripts/reset-admin.js --store` creates the account
  // that arms the gate.
  //
  // It is the same two-predicates-one-flag shape this module already warns
  // about: `isGateExpected` asks authEnabled AND caddy mode, while
  // `auth-gate.js#isGateActive` asks authEnabled AND an enabled account. Both
  // are right about their own door; the bug was letting only one of them speak.
  if (tcGateActive === true) return 'live';
  // authEnabled is true and TangleClaw is not gating. In any non-caddy ingress
  // the flag is then inert — only the Caddy cutover installs a gate (AUTH-2).
...
module.exports = { resolveRequestUser, resolveAuthStatus, isProxyHeaderTrusted, IDENTITY_HEADER, PROXY_EVIDENCE_HEADER, AUTH_STATUSES };
