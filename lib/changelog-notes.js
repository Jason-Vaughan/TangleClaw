 ...
 * page readable once an operator follows the update beacon to it.
 *
...
// A fenced code block opener/closer. Entries in this project's changelog quote
// shell and markdown, so a `## ` line inside a fence is content, not a heading —
// treating it as one would silently truncate the release notes at that point.
const FENCE_RE = /^\s*(```|~~~)/;
...
function extractReleaseNotes(changelogText, version) {
  ...
  for (let i = start; i < lines.length; i++) {
    if (FENCE_RE.test(lines[i])) { inFence = !inFence; continue; }
    if (!inFence && ANY_H2_RE.test(lines[i])) {
      end = i;
      break;
    }
  }
...
//   node lib/changelog-notes.js <version> [changelog-path] > notes.md
//
// Exit codes are the workflow's contract — it keys on them to decide whether to
// publish, so they are meaningful and must stay stable:
//   0  notes written to stdout
//   1  no notes for this version (the version was bumped without promoting the
//      [Unreleased] section) — the release is incomplete, so publishing must stop
//   2  bad invocation: no version given, or the changelog could not be read
//
// 1 and 2 are distinguished because they need different fixes: 1 is a real
// authoring gap in the release, 2 means the workflow itself is wired wrong.
