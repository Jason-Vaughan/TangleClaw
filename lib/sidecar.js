...
const { requestOnce } = require('./http-settle');
 ...
 * on `_pollers.has()`, which stays true). `requestOnce` owns that guarantee and
 * documents why `res.'end'` alone is not enough.
 *
 ...
 */
async function _getWithAgent(url, headers, timeoutMs) {
  const res = await requestOnce(url, { headers, agent: _agent, timeoutMs });
...
function stopAllPolling() {
  ...
  // Every id with a POLLER, not just those already carrying failures: a healthy
  // connection has no _failures entry, so keying on that map let its in-flight
  // poll keep a matching epoch and write a count back after the clear — the exact
  // residue the epoch exists to prevent.
  for (const id of _pollers.keys()) _epoch.set(id, (_epoch.get(id) || 0) + 1);
  _pollers.clear();
  _failures.clear();
}
