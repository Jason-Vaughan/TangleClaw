 ...
 *
 * A project with no assignment is ungoverned and passes. Control state that
 * cannot be established refuses with 503 CONTROL_STATE_UNAVAILABLE, and that
 * includes the operator: a verified operator passes a READABLE held lane, but
 * nobody passes a store that cannot answer, because A1 has no override.
 * - Until the governed memory has been primed from the store, every subject is
 *   refused: an unread project cannot be told from an ungoverned one.
 * - After priming, a store failure refuses a subject known to be governed, and
 *   every caller subject. A project affirmatively seen ungoverned passes.
 * "Known" rests on memory primed from the store and updated synchronously as
 * governance is created or rebound.
 *
...
function _checkCaller(subject) {
  ...
  if (caller.kind === 'operator') {
    // Not held by any lane, but only once the store has answered: a read that
    // throws lands in checkMutation's fail-closed path.
    store.control.anyRestricted();
    return null;
  }
...
function _knownToBeGoverned(subject) {
  ...
  // Every caller subject acts install-wide (restart, update-apply): with the
  // store unreadable nobody, the operator included, can be shown the lanes
  // are clear.
  return true;
}
...
function checkMutation({ surface, subject }) {
  if (!_primed && !prime()) {
    log.error('Control state not established (governed memory unprimed); refusing', { surface });
    return _unavailable();
  }
  let refusal;
  try {
