...
/**
 * The transcript ABOVE the composer region.
 *
 ...
 * sitting in the box". Two earlier attempts at this exclusion were wrong, and
 * both are worth stating because the third has to survive them:
 *
 * 1. **By row index** — `lines[cursor.y]`. `capturePane` issues `-S -N` off the
 *    alternate screen, so row 0 is N rows ABOVE the visible pane top while
 *    `cursor_y` is pane-relative. They share no origin.
 * 2. **By matching the cursor's ONE row.** A wrap prompt is thousands of
 *    characters and renders across MANY composer rows. The nonce is not last in
 *    it — `_completionInstruction` puts ~110 characters after it — so the nonce
 *    routinely lands on a composer row that is not the cursor's, survives a
 *    one-row filter, and reads as a transcript echo.
 *
 * So the whole REGION goes. The composer sits at the bottom of the pane, and
 * its first row is the one bearing the prompt glyph; everything from there down
 * is composer or chrome, never transcript. Excluding too much can only cost a
 * missed echo (answering `unknown`); excluding too little costs a false
 ...
 * @returns {string} Transcript text above the composer, or '' when the composer
 *   cannot be located — in which case no echo may be trusted.
 */
function _transcriptOutsideComposer(lines, cursor, profile) {
  const rows = lines || [];
  if (!cursor || typeof cursor.line !== 'string' || !profile) return '';
  ...
  // LAST match: the composer is at the bottom, so a coincidental earlier match
  // in scrollback must not win.
  let cursorRow = -1;
  ...
  // Walk up to the composer's first row — the nearest glyph-bearing row at or
  // above the cursor. A continuation row carries no glyph, which is exactly why
  // the cursor's row alone is not the boundary.
  let start = cursorRow;
  if (profile.promptGlyph) {
    for (let i = cursorRow; i >= 0; i--) {
      if (medusaWake._strip(String(rows[i])).includes(profile.promptGlyph)) { start = i; break; }
    }
  }
  return rows.slice(0, start).join('\n');
}

/**
 * The composer region's own text — the counterpart of `_transcriptOutsideComposer`.
 *
 * Splitting the pane at the same boundary from both sides is what makes the
 * two answers consistent: whatever is not transcript is composer.
 *
 * @param {Array<string>} lines - Captured pane lines.
 * @param {{line: string}|null} cursor - `cursorInfo` result.
 * @param {object} profile - Engine wake profile, for `promptGlyph`.
 * @returns {string} The composer region's text, or '' when it cannot be located.
 */
function _composerRegion(lines, cursor, profile) {
  const rows = lines || [];
  const above = _transcriptOutsideComposer(rows, cursor, profile);
  if (!cursor || typeof cursor.line !== 'string' || !profile) return '';
  const composerLine = medusaWake._strip(cursor.line).trim();
  if (!composerLine) return '';
  // `_transcriptOutsideComposer` returns '' both when the composer starts at row
  // 0 and when it could not be located. Distinguish by re-locating the cursor.
  const found = rows.some((r) => medusaWake._strip(String(r)).trim() === composerLine);
  if (!found) return '';
  const aboveRows = above ? above.split('\n').length : 0;
  return rows.slice(aboveRows).join('\n');
}
...
async function verifySubmission(tmuxName, engineId, nonce, opts = {}) {
  ...
  while (deps.now() - startedAt < windowMs) {
    ...
    if (error) {
      ...
      const composerText = needle ? _composerRegion(lines, cursor, profile) : '';
      const nonceInComposer = Boolean(needle && composerText
        && medusaWake._strip(composerText).includes(needle));
      const composerHasInput = nonceInComposer || composerEmpty === false;
      ...
      // 2. Our text is sitting in the composer. BEFORE the busy check, because
      //    busy does not mean OUR prompt was taken, and before the echo check,
      //    because the composer is what the echo would be reading.
      if (composerHasInput) {
        composerHolds += 1;
        if (composerHolds >= COMPOSER_CONFIRM_POLLS) {
          return {
            outcome: 'not-accepted',
            reason: nonceInComposer
              ? `this send's own text is still in the composer on ${composerHolds} consecutive reads — `
                + 'the prompt was pasted but never submitted'
              : `the composer is holding input on ${composerHolds} consecutive reads — `
                + 'the prompt was pasted but never submitted'
          };
        }
        // Deliberately NO echo check on this frame: the nonce is in the composer
        // we just found holding input, and reading it here is the false
        // `accepted` this module keeps being bitten by.
        lastReason = 'the composer is holding input, awaiting a confirming read';
      } else {
        composerHolds = 0;

        // 3. The engine is working. ONLY the two positive activity signals:
        //    `_assessActivity` also answers working for `not-at-rest`, which
        //    means merely that the idle marker was absent from the tail — a
        //    scrolled pane produces that too, and absence is not evidence.
        if (activity.working
        ...
        // 4. Our nonce reached the transcript ABOVE the composer region.
        if (needle && cursor) {
          const body = _transcriptOutsideComposer(lines, cursor, profile);
          if (body && medusaWake._strip(body).includes(needle)) {
            return { outcome: 'accepted', reason: "this send's nonce is echoed in the pane transcript" };
          }
        }

        lastReason = composerEmpty === true
          ...
          : activity.working
            ...
            : 'the composer could not be located in the pane';
      }
    }
