...
/**
 * The captured tail with the cursor's own line removed.
 *
 ...
 * sitting in the box". `cursorInfo` addresses the line by `cursor_y`, which is
 * the same origin the capture uses, so the row index lines up.
 *
 * @param {Array<string>} lines - Captured pane lines.
 * @param {{y: number}|null} cursor - `cursorInfo` result.
 * @returns {string} The tail text with the composer line excluded.
 */
function _transcriptOutsideComposer(lines, cursor) {
  const rows = lines || [];
  if (!cursor || !Number.isInteger(cursor.y)) return '';
  // `capturePane` returns the visible pane top-down, so cursor_y indexes it.
  return rows.filter((_l, i) => i !== cursor.y).join('\n');
}
