 ...
 *   is echoed in the transcript OUTSIDE the composer region.
...
async function verifySubmission(tmuxName, engineId, nonce, opts = {}) {
  ...
  while (deps.now() - startedAt < windowMs) {
    ...
    if (error) {
      ...
      // A busy engine says SOMETHING is running. It does not say OUR prompt was
      // taken — a previous turn can still be running while our paste sits
      // unsubmitted, which is the consecutive-step case by definition. Checks 2
      // and 3 normally separate those, but both need a located boundary; when
      // the composer head has scrolled out of the bounded tail they cannot fire,
      // and activity would answer `accepted` for a prompt still in the composer.
      //
      // So: an unlocated boundary WITH our nonce somewhere in the capture is not
      // an accept. Deliberately narrow — it costs nothing on the normal accept
      // path, nor on an engine that hides its composer while working, because
      // both of those either locate the boundary or do not show the nonce.
      const nonceSomewhere = Boolean(needle && text.includes(needle));
      if (activity.working
          && (activity.reason === 'turn-in-flight' || activity.reason === 'agents-running')) {
        if (!split.located && nonceSomewhere) {
          lastReason = 'the engine is working, but the composer could not be located and this '
            + "send's text is somewhere in the capture — activity is not evidence that THIS prompt "
            + 'was the thing taken';
        } else {
          composerHolds = 0;
