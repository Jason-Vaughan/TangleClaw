...
/**
 * The composer locator, from its one home (`medusa-wake.locateComposer`). Kept
 * under this name because the receipt's own tests exercise it here; the
 * history of why the boundary is found the way it is lives with the function.
 * @type {typeof medusaWake.locateComposer}
 */
const _splitAtComposer = (lines, cursor, profile) => medusaWake.locateComposer(lines, cursor, profile);
...
async function verifySubmission(tmuxName, engineId, nonce, opts = {}) {
  ...
  // from the cursor's own rendered line, so without it `_splitAtComposer`
