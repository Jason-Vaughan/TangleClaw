...
const VERDICT_MEANINGS = Object.freeze({
  ...
  'native-startup': 'this launch selected its engine\'s native startup channel, so nothing is ever typed into its pane; the operator can Fire from the launch panel',
  ...
  'control-held': 'the lane is held or stopped by its control assignment, so nothing is typed into it; the next tick checks again'
});
 ...
 *   warnedWindow: boolean, loggedUnprofiled: boolean, cursorWarnLogged: boolean, loggedNative: boolean}>}
 */
...
function _judge(sequence, now) {
  ...
  if (!st) {
    ...
    st = {
      ...
      warnedCreatedAt: false, warnedWindow: false, loggedUnprofiled: false, cursorWarnLogged: false, loggedNative: false
    };
  ...
  // A launch that selected its native startup channel is never typed into
  // (#1825 B3, Architect F2): this nudge is a keystroke fallback, and the pane
  // may be sitting on the very trust or login dialog the native path refused to
  // type through. The stamp above still stands — the operator sees the launch
  // is unready — and the launch panel's Fire is the human's recourse.
  if (sequence.startupDelivery === 'native') {
    if (!st.loggedNative) {
      st.loggedNative = true;
      log.info('An unready session is not nudged: its launch selected native startup control', {
  ...
  if (!sent.ok && sent.controlRefusal) {
    // #1861: a held lane refused the nudge before anything was typed. That is
    // the control state working, not a pane fault.
    log.debug('An unready session was not nudged: its lane is held', { sequence: sequence.id, code: sent.controlRefusal.code });
    return 'control-held';
  }
  if (!sent.ok) {
