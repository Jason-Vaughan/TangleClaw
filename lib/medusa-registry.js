...
function _registryPath(projectPath) {
  return tcProjectFiles.resolveIn(projectPath, tcProjectFiles.MEDUSA_REGISTRY_RELPATH);
}
...
/**
 * Every persisted workspace id for a project, read in one pass — for a caller
 * resolving many sessions of the same project, which would otherwise read (and,
 * for an unreadable file, warn about) the same file once per session.
 * @param {string} projectPath - Absolute path to the project directory.
 * @returns {Object<string, string>} TC session id (as a string) → workspace id.
 */
function readWorkspaceIds(projectPath) {
  const ids = {};
  for (const [sessionId, entry] of Object.entries(_load(projectPath))) {
    if (entry && entry.workspaceId) ids[sessionId] = entry.workspaceId;
  }
  return ids;
}
