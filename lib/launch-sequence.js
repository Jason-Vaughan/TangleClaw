 ...
 *
 * `evaluationMissing` distinguishes this from the attempted-and-failed shape
 * `evaluate` returns, which sets `evaluationFailed`. Neither satisfies a
 * required successful check; they differ in where a reader goes to look.
...
const PREFLIGHT_NOT_EVALUATED = {
  ...
  reason: 'no preflight evaluation reached this launch, so the previous session\'s handoff is unchecked',
...
function buildSnapshot({ launchId, project, engineProfile, applicability, rendered, rules, preflight = PREFLIGHT_NOT_EVALUATED, recoveryMode = 'operator', renderContext = null, ruleDrift = null }) {
  ...
  // result that never arrived means.
  ...
  // evidence.
  const usablePreflight = preflight && typeof preflight === 'object'
    && typeof preflight.verdict === 'string'
    && typeof preflight.requiresRecovery === 'boolean';
  ...
  if (!usablePreflight) {
    ...
    // the reader it sends looking. Says which of the two shapes arrived, since
    // "nothing was passed" and "something malformed was passed" are different
    // bugs at the call site.
    log.warn('No usable preflight reached this launch snapshot — recording missing evidence, which owes recovery', {
