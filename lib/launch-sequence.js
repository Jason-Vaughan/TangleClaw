...
const PENDING_STAGES = {
  stages: ['ready', 'unready', 'recovery'],
  reason: 'this TangleClaw version serves and acknowledges the steps; READY, the unready window and recovery arrive in later versions'
};
 ...
 * @returns {object}
 */
function buildSourceManifest(project, engineProfile, rules, toolOutput) {
...
function buildSnapshot({ launchId, project, engineProfile, applicability, rendered, rules, preflight = PREFLIGHT_NOT_EVALUATED }) {
  ...
  const base = {
    ...
    pageBudget,
    preflight,
    sourceManifest: buildSourceManifest(project, engineProfile, rules || [], tool)
  };
...
module.exports = {
  ...
  // and advance it. Everything below `status` is reached only by tests, which
  // pin the pieces (pagination, budgets, applicability) that no single caller
