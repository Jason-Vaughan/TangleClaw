 ...
 * @param {Array<{id: number, content: string}>} [projectRules] - Already-read project rules,
 *   so a caller that has them does not read them twice; read here when omitted
 * @returns {{rules: object[], manifestSources: string[]}} The manifest
 */
function manifestFingerprints(project, projectRules = null) {
  const sha = (text) => crypto.createHash('sha256').update(text).digest('hex');
  const rules = [];
  const sources = [];

  let projectRows = projectRules;
  if (projectRows === null) {
    try {
      projectRows = store.sessionRules.listActiveForProject(project.id);
    // prawduct:allow prawduct/broad-except -- a source that cannot be read is left UNDECLARED, so
    // the diff reports it as not-recorded rather than as an empty set that reads like "none exist"
    } catch (err) {
  ...
  if (projectRows !== null) {
    rules.push(...ruleFingerprints(projectRows, 'project'));
  ...
  try {
    ...
    // is no per-rule identity to diff below the set.
  ...
  try {
    for (const doc of store.sharedDocs.listForProject(project.id)) {
      rules.push({
        ...
        source: 'shared',
        revision: null,
        contentHash: _fileHash(doc.filePath),
 ...
 *
 * The third is an uncleared recovery in `advisory` mode, which is the only mode
 * where a reconciliation can substitute for an operator's clear. In `operator`
 * mode an uncleared recovery is not a reconciliation requirement at all — it is
 * a refusal (`RECOVERY_UNCLEARED`), and reporting it here would tell an agent to
 * write its way past a gate only a person can open.
 * @param {object} sequence - The sequence
