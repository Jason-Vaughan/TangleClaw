...
const { pageOverhead, ackCommand, reviewCommand, REVISION_REASONS } = require('./launch-page');
const ruleDriftOf = require('./launch-rule-drift');
// `NOT_EVALUATED` became gate-bearing when it joined RECOVERY_VERDICTS, so it is
// imported rather than spelled. `launch-preflight` requires nothing, so this is
// cycle-free.
const PREFLIGHT_VERDICTS_MODULE = require('./launch-preflight');
const { VERDICTS: PREFLIGHT_VERDICTS } = PREFLIGHT_VERDICTS_MODULE;
...
/** The envelope schema `review` answers with: a re-read, never a serve. */
const REVIEW_SCHEMA = 'tc.launch-review/1';
...
/**
 * What a launch records when NO evaluation evidence reached it at all.
 *
 * This is missing evidence, not a benign default. An applicable launch owes the
 * check, so a result that is absent, null or malformed at the snapshot boundary
 * **requires recovery**: failing to establish current continuity is not evidence
 * that continuity is sound, and a launch that checked nothing must not reach an
 * ungated READY.
 *
 * It is NOT the shape of a legitimate first launch. The evaluator reaches
 * `first-launch` only after establishing no sessions, no publication rows, no
 * continuity index and an absent handoff, with integrity checks preceding it —
 * a positively evaluated verdict that happens to be benign. A failed read or an
 * omitted argument cannot establish those absences, so a healthy launch passes
 * its explicit successful evaluation rather than leaving `preflight` out. That
 * is also why there is ONE default and not a permissive constructor case beside
 * a restrictive fallback: the permissive one would be the hole, reopened.
 *
 * `evaluationMissing` says only that NO USABLE RESULT IS AVAILABLE. It is not a
 * claim that nothing ran — a result may have been produced and then lost,
 * dropped or malformed on its way here, and this constant cannot tell which.
 * `evaluationFailed` is reserved for a POSITIVELY OBSERVED failure: the
 * evaluator was called and it threw. Neither satisfies a required successful
 * check, and treating the absence of evidence as evidence of absence is the
 * very mistake this whole constant exists to stop.
 * @type {{verdict: string, reason: string, requiresRecovery: boolean, requiresReconciliation: boolean, worktreeDirty: null, evaluationFailed: boolean, evaluationMissing: boolean}}
 */
const PREFLIGHT_NOT_EVALUATED = {
  verdict: PREFLIGHT_VERDICTS.NOT_EVALUATED,
  reason: 'no usable preflight result is available for this launch, so the previous session\'s handoff is unchecked',
...
const ALL_ACKED_CONTENT = 'All four launch steps are acknowledged. Your session context is complete.\n\n'
  + 'Do the read-only freshness checks the task step asked for, then attest — so the operator can '
  + 'see the context arrived and was read:\n'
  ...
  + 'and every confirmation rule in your context still applies. Then emit the resume proposal the '
  + 'task step describes, and stop there for the operator.';
...
function manifestFingerprints(project) {
  ...
  try {
    const rows = store.sessionRules.listActiveForProject(project.id)
      // The same usability predicate the prime applies, imported rather than
      // restated: a rule counted by one population and not the other reads as
      // drift nobody caused.
      .filter(ruleDriftOf.isUsableRule);
  ...
  try {
    // `loadMeasured`, not `load`: the latter answers `''` for a document that is
    // missing, unreadable AND genuinely empty, and `sha('')` is a real-looking
    // hash for a measurement nobody took. Unreadable on both sides it compares
    // equal and prints "nothing changed"; unreadable on one side it reports a
    // global-rules edit nobody made and refuses READY over it. The try/catch
    // below cannot cover this: `load` catches its own errors and the missing
    // file, so it does not throw for either realistic failure.
    const { text, measured } = store.globalRules.loadMeasured();
    ...
    rules.push({
      ...
      source: 'global',
      revision: null,
      // null, not a hash of the empty string — the same treatment `_fileHash`
      // gives an unreadable shared document.
      contentHash: measured ? sha(String(text || '').trim()) : null,
      measured,
      label: 'the global rules'
    });
    sources.push('global');
    if (!measured) {
      log.warn('Global rules could not be read for the governing manifest — drift cannot speak for them', {
...
function buildSourceManifest(project, engineProfile, rules, toolOutput, renderContext = null) {
  ...
  return {
    ...
    // #1675 — the publication this launch's Resume was drawn from, so it can be
    // compared with the one a wrap's result named. Null when the launch read
    // none. Taken from the render context rather than re-read: a second read of
    // `current.json` could name a different document than the one rendered.
    handoffDigest: _consumedHandoff(renderContext).digest,
    handoffPublicationId: _consumedHandoff(renderContext).publicationId
  };
}

/**
 * Which publication a launch consumed, from its render context (#1675).
 * @param {object|null} renderContext
 * @returns {{digest: (string|null), publicationId: (string|null)}}
 */
function _consumedHandoff(renderContext) {
  const h = renderContext && renderContext.handoffResume;
  return {
    digest: h && typeof h.digest === 'string' ? h.digest : null,
    publicationId: h && typeof h.publicationId === 'string' ? h.publicationId : null
  };
}
 ...
 * @param {{verdict: string, requiresRecovery: boolean, reason: string}} [args.preflight] -
 *   The verdict step 3 was rendered with. BOTH `verdict` and a boolean
 *   `requiresRecovery` are required: a result carrying only one is not usable
 *   (`launchPreflight.isUsableResult`) and is normalized to missing evidence,
 *   which owes recovery. Omitting the argument does the same — a healthy launch
 *   states its successful evaluation rather than leaving this out.
...
function buildSnapshot({ launchId, project, engineProfile, applicability, rendered, rules, preflight = PREFLIGHT_NOT_EVALUATED, recoveryMode = 'operator', renderContext = null, ruleDrift = null }) {
  ...
  // Normalized HERE, not left to the default parameter, because a default only
  // fires for `undefined`. An explicit `null` or a malformed object slips past
  // it and then reads as falsy at the gate below — which is the same open gate
  // an omitted argument used to give, reached by a different route. One
  // normalization, so storage, renderer and gate cannot disagree about what a
  // result we do not have means.
  // A verdict STRING is not a usable result. The gate below reads
  // `requiresRecovery`, so an object carrying a verdict and nothing else passes
  // a verdict-only guard and then reads falsy — recording `recovery: 'none'` for
  // a launch nobody evaluated. Both fields are required, or this is missing
  // evidence.
  const usablePreflight = PREFLIGHT_VERDICTS_MODULE.isUsableResult(preflight);
  const measuredPreflight = usablePreflight ? preflight : PREFLIGHT_NOT_EVALUATED;
  if (!usablePreflight) {
    // The attempted-failure path logs from its catch; without this, the
    // missing-evidence half is the only recovery demand with no destination for
    // the reader it sends looking. Says WHAT ARRIVED rather than what happened
    // upstream: "nothing was passed" and "something malformed was passed" are
    // different bugs at the call site, and neither is proof that no evaluation
    // ran.
    log.warn('No usable preflight result for this launch snapshot — recording missing evidence, which owes recovery', {
      project: project && project.name,
      received: preflight === undefined ? 'nothing' : (preflight === null ? 'null' : typeof preflight)
    });
  }
  const base = {
    launchId,
    ...
    recovery: measuredPreflight.requiresRecovery === true ? 'required' : 'none',
  ...
  if (sequence.cursor >= steps.length) {
    ...
    const content = sequence.readyAt
      ? `All four launch steps are acknowledged and this launch attested READY at ${sequence.readyAt}. `
        + 'There is nothing left to pull. Emit the resume proposal the task step describes, and stop '
        + 'there for the operator — READY recorded that your context arrived, not that the work is approved.'
      : ALL_ACKED_CONTENT;
    return {
      status: 200,
      body: {
...
function status({ launchId, projectId }) {
  ...
  return {
    ...
    body: {
      ...
      // #1675 — the handoff publication this launch's Resume came from, to set
      // beside the one a wrap's result named. Both null when it read none, or
      // for a sequence recorded before this was.
      handoff: {
        publicationId: (sequence.sourceManifest && sequence.sourceManifest.handoffPublicationId) || null,
        digest: (sequence.sourceManifest && sequence.sourceManifest.handoffDigest) || null
      },
      readiness: {
...
/**
 * Which step a review asks for: a 1-based index or a step id, the first step
 * when none is named.
 * @param {object[]} steps - The sequence's steps at its current revision
 * @param {string|number|undefined} requested - What the caller named
 * @returns {object|null} The step, or null when nothing matches
 */
function _reviewTarget(steps, requested) {
  if (requested === undefined || requested === null || requested === '') return steps[0] || null;
  if (Number.isInteger(requested)) return steps[requested - 1] || null;
  return steps.find((st) => st.id === requested) || null;
}

/**
 * `GET /api/tc/start/review`: re-serve a page of an ATTESTED launch, read-only.
 *
 * The read a session needs after `/clear` or a compaction has dropped the
 * context it attested: `next` has nothing left to pull once READY is recorded,
 * so without this the launch context is unrecoverable mid-session.
 *
 * It changes nothing — no page is marked served, no ack is taken, the snapshot
 * is never revised against live rules, and the cursor and `readyAt` stay as
 * they are. That is what makes it safe to call at any point after READY, and
 * it is also why it refuses BEFORE READY: a page shown here and then
 * acknowledged through `next` would look served when `next` never served it.
 *
 * It serves the frozen snapshot the session attested, not the current rules:
 * after READY a rule change does not reopen the launch, and the rules channel
 * delivers it instead.
 * @param {object} args
 * @param {string|null} args.launchId - The pane's launch id
 * @param {number|null} args.projectId - The pane's project id
 * @param {string|number} [args.step] - A 1-based step index or a step id; the first step when omitted
 * @param {number} [args.page] - A page of that step, from 0; the first when omitted
 * @returns {{status: number, body: object}}
 */
function review({ launchId, projectId, step, page }) {
  const resolved = _resolve({ launchId, projectId, mutating: false });
  if (resolved.legacy) {
    return _refuse(409, 'LAUNCH_ID_REQUIRED',
      'This pane carries no TANGLECLAW_LAUNCH_ID, so it predates phased launch and has no attested sequence to '
      + 're-read. Its context arrived in the pushed prime; `tc rules` still re-reads the project rules.');
  }
  if (resolved.refusal) return resolved.refusal;
  const sequence = resolved.sequence;
  if (sequence.applicability !== 'applicable') {
    return _refuse(409, 'SEQUENCE_NOT_APPLICABLE',
      `This session has no launch steps to re-read: ${sequence.notApplicableReason}.`,
      { reason: sequence.notApplicableReason });
  }
  if (!sequence.readyAt) {
    return _refuse(409, 'NOT_READY',
      'Your launch is not attested yet, so there is nothing to re-read: `tc start next` still serves it. '
      + 'Nothing was served or changed.',
      { cursor: sequence.cursor });
  }
  const steps = store.launchSequences.listSteps(sequence.id, sequence.revision);
  const target = _reviewTarget(steps, step);
  if (!target) {
    return _refuse(400, 'UNKNOWN_STEP',
      `There is no launch step "${step}". Name one by number (1-${steps.length}) or by id (${steps.map((st) => st.id).join(', ')}).`,
      { stepCount: steps.length });
  }
  const pageCount = target.pageOffsets.length;
  const pageIndex = page === undefined || page === null ? 0 : page;
  if (!Number.isInteger(pageIndex) || pageIndex < 0 || pageIndex >= pageCount) {
    return _refuse(400, 'PAGE_OUT_OF_RANGE',
      `Step ${target.id} has ${pageCount} page(s), numbered from 0; page ${pageIndex} does not exist.`,
      { step: target.id, pageCount });
  }
  const [start, end] = target.pageOffsets[pageIndex];
  let nextRef = null;
  if (pageIndex < pageCount - 1) {
    nextRef = { step: target.id, page: pageIndex + 1, command: reviewCommand(target.id, pageIndex + 1) };
  } else if (target.index < steps.length - 1) {
    const following = steps[target.index + 1];
    nextRef = { step: following.id, page: 0, command: reviewCommand(following.id, 0) };
  }
  const artifact = sequence.readyArtifact || {};
  return {
    status: 200,
    body: {
      ...
      sequenceId: sequence.id,
      revision: sequence.revision,
      step: { index: target.index, id: target.id, of: steps.length },
      page: { index: pageIndex, of: pageCount, continued: pageIsContinued(target.content, start) },
      content: target.content.slice(start, end),
      review: {
        readyAt: sequence.readyAt,
        attestedVerdict: artifact.preflightVerdict || null,
        attestedFirstAction: artifact.proposedFirstAction || null
      },
      status: _statusBlock(sequence),
      next: nextRef ? (nextRef.step === target.id ? 'page' : 'step') : 'done',
      nextRef
    }
  };
}
