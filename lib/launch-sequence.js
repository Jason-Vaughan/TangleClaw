...
const PENDING_STAGES = {
  stages: ['recovery'],
  reason: 'this TangleClaw version serves, acknowledges and attests the steps; the handoff preflight and its recovery gate arrive in a later version'
};
...
function _reconciliationRequired(sequence) {
  if (sequence.revision > 1) {
    return `this launch's snapshot was revised to revision ${sequence.revision} after it began, so part of what you acknowledged has been replaced`;
  }
...
function _validateReady(sequence, steps, artifact) {
  if (!artifact || typeof artifact !== 'object') {
    return { refusal: _refuse(400, 'BAD_READY', `A READY attestation is a ${READY_SCHEMA} object; none was sent.`) };
  }
  if (artifact.schema !== READY_SCHEMA) {
    return { refusal: _refuse(400, 'BAD_READY', `A READY attestation carries "schema": "${READY_SCHEMA}".`) };
  }
  ...
  if (typeof artifact.proposedFirstAction !== 'string' || artifact.proposedFirstAction.trim().length === 0) {
    return {
      refusal: _refuse(400, 'BAD_READY',
        'A READY attestation states the first action you PROPOSE to take. It is a proposal, not an authorization: the operator\'s confirmation rules are unchanged.')
    };
  }
...
function ready({ launchId, projectId, artifact }) {
  return store.launchSequences.transaction(() => {
    ...
    // Before the rule check, so a duplicate of an accepted attestation stays
    // idempotent instead of being refused by a revision it predates.
    if (sequence.readyAt) {
