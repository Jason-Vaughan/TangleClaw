...
/**
 * Classify a `git symbolic-ref -q HEAD` failure. Exit status 1 is the
 * documented, silent answer for a detached HEAD. A missing binary or a
 * directory that is not a repository is the no-git opt-out. Anything else —
 * a timeout (the child is killed, so there is no exit status 1), a corrupt
 * repository — is a failure.
 *
 * @param {Error & {code?: number|string, killed?: boolean, stderr?: string}} err
 * @returns {'detached'|'no-git'|'failed'}
 */
function _symbolicRefFailure(err) {
  if (!err) return 'failed';
  if (gitProbe.isNoGit(err)) return 'no-git';
  if (err.code === 1 && !err.killed) return 'detached';
  return 'failed';
}

/**
 * Parse `git rev-list --count` output, or null when it is not a non-negative
 * integer. Garbage is not collapsed to 0: the caller reports it as an unknown
 * measurement, never as "nothing upstream", and the banner never renders it.
 *
 * @param {string|Buffer|null|undefined} out - Raw stdout.
 * @returns {number|null}
 */
function _parseCountStrict(out) {
 ...
 *
 * `state` says which: `'measured'` (a fetch and count succeeded),
 * `'skipped'` (detached HEAD, no fetch made) or `'unknown'` (a step failed;
 * `reason` names it and `commitsAhead` is 0 only for older consumers).
 *
 * @returns {Promise<{commitsAhead: number, skipped: string|null, state: 'measured'|'skipped'|'unknown', reason: string|null}>}
 */
function measure() {
  return new Promise((resolve) => {
    ...
    // A failed step still counts 0 so `commitsAhead` keeps its meaning for
    // older consumers, but `state: 'unknown'` and the reason say it was never
    // measured: a failed check must not read as "up to date" (#1678).
    const zero = (reason) => done({ commitsAhead: 0, skipped: null, state: 'unknown', reason }, reason);
    const call = (fn, onOk) => {
      try {
    ...
    call(_internal.gitSymbolicRef, (refErr) => {
      if (refErr) {
        // Every failure here skips the fetch, as before. What differs is what
        // the payload claims: `symbolic-ref -q` exits 1, silently, only on a
        // detached HEAD; no git or no repository is the designed opt-out; and
        // anything else (a timeout, a git error) is a check that did not run,
        // which must read as unknown rather than as a tagged install.
        const kind = _symbolicRefFailure(refErr);
        if (kind === 'detached') return done({ commitsAhead: 0, skipped: 'detached-head', state: 'skipped', reason: 'HEAD is not on a branch' });
        if (kind === 'no-git') return done({ commitsAhead: 0, skipped: 'no-git', state: 'skipped', reason: 'not a git checkout' });
        return zero(`branch check failed: ${redactRemoteOutput(String(refErr.message || refErr))}`);
      }
      call(_internal.gitFetch, (fetchErr) => {
        ...
        call(_internal.gitRevList, (revErr, stdout) => {
          ...
          const n = _parseCountStrict(stdout);
          if (n === null) return zero('rev-list: unparseable output');
          done({ commitsAhead: n, skipped: null, state: 'measured', reason: null });
        });
      });
    });
  });
 ...
 *
 * @returns {Promise<{commitsAhead: number, skipped: string|null, state: string, reason: string|null, checkedAt: string}>}
 */
function refresh() {
 ...
 * say.
 *
 * `state` is the honest reading of the count (#1678): `'measured'`,
 * `'skipped'`, `'unknown'` (the last attempt failed — `commitsAhead: 0` then
 * means "not measured", never "up to date"), `'pending'` (never measured) or
 * `'disabled'`. `checkedAt` is when the last attempt finished, successful or
 * not; only a `'measured'` state makes it the time origin was last observed.
 *
 * @param {object|null|undefined} config - Loaded global config.
 * @returns {{enabled: boolean, commitsAhead: number, skipped: string|null, checkedAt: string|null,
 *   state: 'measured'|'skipped'|'unknown'|'pending'|'disabled', reason: string|null}}
 */
function snapshot(config) {
  if (!isCheckEnabled(config)) {
    return { enabled: false, commitsAhead: 0, skipped: null, checkedAt: null, state: 'disabled', reason: 'check turned off' };
  }
  ...
  return {
    ...
    state: _cache ? _cache.state : 'pending',
    reason: _cache ? _cache.reason : 'not measured yet'
  };
}
