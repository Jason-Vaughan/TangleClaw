...
// Promise of the one in-flight local re-measure, or null when idle.
let _remeasuring = null;

/**
 * Re-measure HEAD's relation to the origin/main this process last fetched,
 * without fetching again. Used when HEAD has moved since the observation: the
 * upstream it was compared with is unchanged (and keeps its `checkedAt`), so
 * only the local half needs taking again. Local git only; single-flight; never
 * rejects. A no-op when there is no successful observation to re-measure.
 *
 * @returns {Promise<object|null>} The updated observation record, or null.
 */
function remeasureRelation() {
  if (_remeasuring) return _remeasuring;
  if (!_observation || !_observation.ok) return Promise.resolve(null);
  _remeasuring = (async () => {
    const base = _observation;
    const head = await _run(_internal.gitRevParseHead);
    const lr = await _run(_internal.gitLeftRight);
    const counts = lr.err ? null : _parseLeftRight(lr.out);
    // Replace only if nothing newer landed meanwhile, and only with a complete
    // measurement; a failed local read leaves the (visibly outdated) record.
    if (_observation === base && !head.err && head.out.trim() && counts) {
      _observation = { ...base, headSha: head.out.trim(), ahead: counts.ahead, behind: counts.behind };
    }
    _remeasuring = null;
    return _observation;
  })();
