...
function detectTtydLeak(probes = _probes) {
  ...
  if (poolFired || orphanFired) {
    ...
    // #1245 — a restart makes every terminal reconnect at once and that churn
    // leaks, so a young ttyd's orphan count may BE the last restart's cost. The
    // watcher holds its orphan gate down until ttyd is old enough;
    // say so here too, because this row's remedy is another restart and the
    // operator should know it may buy nothing. Pool exhaustion is never
    // qualified this way — a full pool means no terminal can attach at all.
    if (!poolFired && orphanFired && m.uptimeMs !== null && m.uptimeMs < m.minTtydAgeMs) {
      parts.push(`ttyd restarted ${Math.round(m.uptimeMs / 60000)} min ago, so these may be that restart's reconnect burst rather than a new leak — the watcher is holding off until it is ${Math.round(m.minTtydAgeMs / 60000)} min old`);
    }
 ...
 * @param {() => number} [deps.count] - Live-listener count.
 * @param {() => Promise<object>} [deps.check] - Bridge preflight.
 * @returns {Promise<{id: string, title: string, state: string, detail: string, remediation: string, hint?: string}>}
 */
async function detectMedusaBridge(deps = {}) {
  const title = 'Medusa Bridge reachable';
  const count = deps.count || medusa.activeListenerCount;
  const check = deps.check || medusa.checkBridgeHealth;
  let listenerCount;
  try {
    listenerCount = count();
  } catch (err) {
