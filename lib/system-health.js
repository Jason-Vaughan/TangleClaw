...
const _defaultProbes = {
  ...
  // Where the owned ttyd runtime lives (#1245, ADR 0018), so the ttyd row can
  // say when launchd is running some other binary and the fix is not in force.
  managedTtydPath: () => require('./ttyd-runtime').runtimePaths(require('./tangleclaw-home').baseDir()).ttyd,
...
function detectTtydLeak(probes = _probes) {
  ...
  const condition = _withReading(_classifyTtyd(m, title), m);
  return _noteBinary(condition, m, probes);
}

/**
 * Say, on the ttyd row, when launchd is running a binary other than the owned
 * runtime: the #1245 fix lives in that runtime, so a machine still on the
 * Homebrew ttyd (not yet cut over, or deliberately rolled back) is leaking
 * whatever its counts say today. A note, not a state change: the counts are
 * still what they are.
 * @param {object} condition - The ttyd verdict.
 * @param {object} m - The `measureLeak` summary.
 * @param {object} probes - The probes in force.
 * @returns {object} The same condition.
 */
function _noteBinary(condition, m, probes) {
  let managed = null;
  try { managed = probes.managedTtydPath ? probes.managedTtydPath() : null; } catch { managed = null; }
  condition.reading.binary = m.binary || null;
  // Unknown is not "not managed": a probe that could not say which binary is
  // running establishes nothing about whether the fix is in force.
  condition.reading.managed = (!managed || !m.binary) ? null : m.binary === managed;
  if (managed && !m.binary && m.pid !== null) {
    condition.detail += '; could not read which ttyd binary launchd is running, so whether the #1245 fix is in force is unknown';
  }
  if (managed && m.binary && m.binary !== managed) {
    condition.detail += `; ttyd is running ${m.binary}, not the owned runtime ${managed}, so the #1245 leak fix is not in force`;
  }
  return condition;
}
