...
function _classifyTtyd(m, title) {
  // Whatever the counts say, a watcher switched off is not keeping this machine
  // safe, so the row can never read as clear while it is off.
  if (m.disabled) {
    const counts = m.pool && m.orphans !== null
      ? `; PTY pool ${m.pool.used}/${m.pool.cap} slots in use, ${m.orphans} leaked tmux clients`
      : '';
    return _condition('ttyd-leak', title, STATE_UNKNOWN,
      `the ttyd watcher is disabled (${m.disabledBy}), so nothing will restart a leaking ttyd${counts}`,
      TTYD_REMEDIATION);
  }
  const poolFired = !!(m.pool && m.pool.exhausted);
  const orphanFired = m.orphans !== null && m.orphans >= m.orphanThreshold;
  // Children exiting right now but not yet confirmed wedged: the normal cost of
  // tabs closing. Named so a reconnect burst is visible without being counted
  // as a leak, which is also exactly what the watcher does with them.
  const transientNote = m.transient > 0
    ? `${m.transient} more exiting but not yet confirmed wedged (younger than ${Math.round(m.wedgeAgeMs / 1000)}s and not seen twice)`
    : null;
