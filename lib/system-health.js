...
function detectTtydLeak(probes = _probes) {
  ...
  if (poolFired || orphanFired) {
    ...
    // watcher holds its orphan gate down until ttyd is older than the cooldown;
    ...
    if (!poolFired && orphanFired && m.uptimeMs !== null && m.uptimeMs < m.cooldownMs) {
      parts.push(`ttyd restarted ${Math.round(m.uptimeMs / 60000)} min ago, so these may be that restart's reconnect burst rather than a new leak — the watcher is holding off until it is ${Math.round(m.cooldownMs / 60000)} min old`);
    }
