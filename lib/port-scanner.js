...
function _socketTableListener(port) {
  ...
  try {
    if (process.platform === 'darwin') {
      for (const line of run('netstat -anv -p tcp').split('\n')) {
        const parts = line.trim().split(/\s+/);
        // Proto Recv-Q Send-Q Local Foreign (state) rxbytes txbytes rhiwat shiwat pid ...
        if (parts[5] !== 'LISTEN') continue;
        const m = /\.(\d+)$/.exec(parts[3] || '');
        if (!m || Number(m[1]) !== port) continue;
        const pid = parseInt(parts[10], 10);
        return { found: true, pid: Number.isInteger(pid) ? pid : null, command: Number.isInteger(pid) ? _commandOf(pid) : null };
      }
      return { found: false, pid: null, command: null };
    }
    if (process.platform === 'linux') {
      for (const line of run('ss -Hltn').split('\n')) {
        const parts = line.trim().split(/\s+/);
        const m = /:(\d+)$/.exec(parts[3] || '');
        if (m && Number(m[1]) === port) return { found: true, pid: null, command: null };
      }
      return { found: false, pid: null, command: null };
    }
