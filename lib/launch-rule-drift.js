 ...
 *
 * The last two were one value for exactly one review round, and the renderer
 * spent that round telling operators "this launch could not read it — the
 * server log names what failed" about files that had failed at the previous
 * wrap, on a machine whose log says nothing. Which side was silent is not a
 * detail of the verdict; it IS the verdict, because it decides where the
 * person reading it goes to look.
 */
const SOURCE_STATES = {
  CHANGED: 'changed',
  UNCHANGED: 'unchanged',
