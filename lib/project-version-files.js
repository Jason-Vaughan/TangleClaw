...
/**
 * How old a staging file must be before the sweep will claim it as abandoned.
 *
 * THIS IS NOT A TIDINESS MARGIN, it is what stops the sweep corrupting a healthy
 * concurrent write. Two processes legitimately write this cache — the scanner
 * child on a poll, the server at session launch and wrap — so a staging file the
 * sweep finds may not be stranded at all; it may be another writer's, staged
 * milliseconds ago and about to be renamed. Unlinking it makes that writer's
 * rename fail for no reason. A live staging file exists for microseconds, so any
 * threshold in this range separates the two cases without ambiguity.
 */
 ...
 * the write that just succeeded — every branch here swallows and moves on.
 *
 * @param {string} dir - The `.tangleclaw` directory holding the cache.
 * @param {string} mine - Absolute path of the staging file this writer used, and
 *   already renamed. Skipped because the name may have been reused by another
 *   process since.
 * @returns {void}
 */
function _sweepStagingFiles(dir, mine) {
  ...
  for (const name of entries) {
    ...
    try {
      ...
      // It vanished between the readdir and here — which is the normal outcome
      // when the writer that owns it finished its rename — or it is a permission
      // we do not have. Either way the next successful write tries again.
    }
  }
}
...
function writeVersionCacheFile(projectPath, version, source) {
  ...
  const tmp = path.join(dir, `${STAGING_PREFIX}${process.pid}.${crypto.randomBytes(4).toString('hex')}.tmp`);
