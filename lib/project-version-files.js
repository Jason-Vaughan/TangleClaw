 ...
 * `package.json`, then `pyproject.toml` (#1444).
 *
 * Reading past an unusable `version.json` to `package.json` is this reader's
 * long-standing behaviour (#58), kept as it was: the dashboard shows the
 * package version even though the wrap's bump refuses that version.json.
 *
 * `pyproject.toml` is only reached where the wrap's bump would reach it too, so
 * the dashboard never shows a Python version the bump would not touch:
 * - a `version.json` that exists without a usable version stops the probe
 *   short of `pyproject.toml`, because its name says it is the version file;
 * - a `package.json` that is valid JSON with no `version` field is passed over
 *   (a Python repo often carries one only for tooling), and any other unusable
 *   `package.json` stops the probe short of `pyproject.toml`.
 *
 * @param {string} projectPath - Absolute project root path
 * @returns {{version:string, source:string}|{stopped:string}|null} - The version and its file,
 *   the file that stopped the probe short of pyproject.toml, or null when nothing was found
 */
function readProbedVersion(projectPath) {
  ...
  if (fs.existsSync(path.join(projectPath, 'version.json'))) {
    ...
    stoppedAt = 'version.json';
  }
  if (fs.existsSync(path.join(projectPath, 'package.json'))) {
    const v = readPackageJsonVersion(projectPath);
    if (v) return { version: v, source: 'package.json' };
    if (!stoppedAt && !_isVersionlessPackageJson(projectPath)) stoppedAt = 'package.json';
  }
  if (stoppedAt) return { stopped: stoppedAt };
...
/**
 * Parse a `package.json`'s text the way both the reader and the wrap's writer
 * must: a UTF-8 BOM is dropped before parsing, so a file an editor saved with
 * one is readable on both sides rather than on one only.
 *
 * @param {string} text - The file's content, unmodified
 * @returns {*} - The parsed value (throws on invalid JSON)
 */
function parsePackageJsonText(text) {
  return JSON.parse(String(text).replace(/^\uFEFF/, ''));
}

/**
 * Whether a parsed `package.json` is an object with no `version` field, the one
 * unusable shape the version probe passes over. The reader and the wrap's
 * writer both ask this, so they cannot disagree about which files they skip.
 *
 * @param {*} pkg - The parsed file
 * @returns {boolean}
 */
function isVersionlessPackageJson(pkg) {
  return !!pkg && typeof pkg === 'object' && !Array.isArray(pkg)
    && (pkg.version === undefined || pkg.version === null || pkg.version === '');
}
...
function _isVersionlessPackageJson(projectPath) {
  try {
    ...
    return isVersionlessPackageJson(parsePackageJsonText(content));
  } catch {
    return false;
  }
}
