 ...
 * @returns {{ok:true, version:string, start:number, end:number}|{ok:false, reason:string}}
 */
function parsePyprojectVersion(text) {
  ...
  for (const rawLine of lines) {
    ...
    if (inMultiline) {
      if (_countDelimiter(line, inMultiline) % 2 === 1) inMultiline = null;
      continue;
    }
    ...
    if (!inProject && /^project\s*=\s*\{/.test(trimmed)) inlineProject = true;
    ...
    for (const delim of ['"""', "'''"]) {
      if (_countDelimiter(line, delim) % 2 === 1) {
        inMultiline = delim;
        break;
      }
    }
  ...
  if (projectHeaders === 0) {
    return inlineProject
      ...
      : { ok: false, reason: 'has no [project] table' };
  }
  ...
  if (!found || found.version.trim() === '') return { ok: false, reason: 'has no version in [project] — nothing to bump' };
  return { ok: true, ...found };
}

/**
 * Count occurrences of a multi-line string delimiter on one line.
 *
 * @param {string} line - One line of the file
 * @param {string} delim - `"""` or `'''`
 * @returns {number}
 */
function _countDelimiter(line, delim) {
