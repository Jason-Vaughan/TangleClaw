 ...
 * @type {Map<number, {armed: boolean, prevHadSentinel: boolean, cursor: number|null, requested: boolean, requestedAt?: number, projectName: string|null}>}
 */
...
function _flag(st, sessionId, projectName) {
  ...
  st.requested = true;
  // Kept on the session, not only on the pending entry the drawer's ack
  // deletes: a wrap request supersedes the session's workload receipt
  // (#1912, ADR 0020 §4) whether or not the drawer has opened since.
  st.requestedAt = now;
  _pending.set(projectName, { sessionId, detectedAt: now });
...
async function _scanSession(session) {
  ...
  // Engine API error detection (#261) rides this same capture — the one
  // periodic read of every live pane — rather than adding a second loop.
  // It runs BEFORE the one-nudge latch below: a session that already asked to
  // wrap is still a session whose engine can be failing.
  try {
    _internal.observeEngineErrors(session, lines, _profileFor(session.engineId));
  } catch (err) {
    // Detection is a passenger on this scan, never its driver: a failure here
    // must not cost the session its typed-wrap trigger.
    log.warn('wrap-sentinel: engine-error detection failed', { sessionId, engine: session.engineId, error: err.message });
  }
  if (st.requested) return; // one wrap nudge per session — already flagged
  const hadSentinel = _hasSentinel(lines.join('\n'));
  if (firstSight) {
...
function _profileFor(engineId) {
  ...
  try {
    ...
    log.warn('wrap-sentinel: engine profile unreadable — error detection off for this tick', { engine: engineId, error: err.message });
  }
...
/**
 * When a session's typed-wrap request was detected, whether or not the drawer
 * has acknowledged it since (#1912). The workload composition reads this to
 * supersede a receipt the request came after.
 * @param {number} sessionId
 * @returns {number|null} Epoch ms, or null when the session has made no request
 */
function wrapRequestedAt(sessionId) {
  const st = _sessions.get(sessionId);
  return st && Number.isFinite(st.requestedAt) ? st.requestedAt : null;
}
