 ...
 * shares the `getBridgeContext` connection-resolution shape).
 */
...
/**
 * Resolve the ClawBridge sidecar for a webui session — mirrors
 * `wrap-steps/ai-content.js#defaultGetBridgeContext`. Lazily requires the store.
 ...
 */
function defaultGetBridgeContext(session, projectName) {
...
async function _scanSession(session) {
  ...
  if (session.sessionMode === 'webui') {
    ...
    if (firstSight) {
      ...
      // is never scanned — only genuinely new output can flag.
      const status = await _internal.getStatus(bridge);
      st.cursor = (status && status.ok && typeof status.cursor === 'number') ? status.cursor : 0;
      st.armed = true;
      return;
    }
