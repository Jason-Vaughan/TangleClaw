...
// deterministic without shelling out. `execSync` is an accessor onto
// `host-inventory`'s seam, because that module owns the one overlay probe;
// stubbing it here stubs the probe every consumer shares.
const _internal = {
  get execSync() { return hostInventory._internal.execSync; },
  set execSync(fn) { hostInventory._internal.execSync = fn; },
...
// Memoized local host. undefined = not yet resolved; a string once resolved
// (the machine's host identity doesn't change within a process).
let _localHostCache;

/**
 * Detect the machine's Tailscale Magic DNS name (`.Self.DNSName`, e.g.
 * `your-host.tailnet-name.ts.net.`), normalized: trailing dot stripped and
 * lowercased. Returns null when tailscale is absent, not running, or its output
 * can't be parsed — a best-effort probe, never throws. A fresh probe through
 * `host-inventory`, which owns the one overlay probe.
 *
 * @returns {string|null}
 */
function _detectMagicDnsName() {
  return hostInventory.probeOverlayDns().host;
}

/**
 * The one sentence that tells a reader how to compose an operator-facing URL.
 *
 * Exported and shared because this directive had grown four hand-written
 * copies — two in the ecosystem primer, two in `tc whoami` — and the moment
 * `resolveOperatorHost` became nullable, only the primer's pair learned the
 * unknown branch. The others interpolated it anyway and produced
 * `http(s)://null:<port>`: a fabricated URL, in generated instruction text, of
 * exactly the kind this whole change exists to stop. One producer means the
 * next reader cannot get a branch of it wrong.
 *
 * @param {string|null} host - A `resolveOperatorHost().host` value.
 * @returns {string} The directive, in both the known and the unknown case.
 */
function operatorLinkDirective(host) {
  return host
    ? `use \`http(s)://${host}:<port>\` for operator links`
    : 'TangleClaw could not establish which host they reach it on — ask, do not guess';
}
...
/**
 * Whether a candidate may be handed to the operator as a link target.
 *
 * One predicate for every path into `resolveOperatorHost`. The header path and
 * the probe path were checking different things — the probe skipped the shape
 * test, and only the header path stripped IPv6 brackets — so `[::1]` could get
 * through one door and not the other. Same question, same answer, one place.
 *
 * @param {string|null|undefined} candidate - A bare hostname.
 * @returns {boolean}
 */
function _isOperatorUsableHost(candidate) {
  if (typeof candidate !== 'string' || !candidate) return false;
  const lowered = candidate.toLowerCase();
  if (!HOST_SHAPE.test(lowered)) return false;
  return !NON_OPERATOR_HOSTS.includes(lowered.replace(/^\[|\]$/g, ''));
}
...
function _hostFromHeader(raw) {
  ...
  // An UNBRACKETED value with more than one colon is a bare IPv6 literal (RFC
  // 3986 requires brackets in a Host, but a malformed sender can omit them).
  // Splitting it would silently truncate `2001:db8::1` to `2001`, which is
  // hostname-shaped and would sail through every check below — refuse instead.
  let host;
  if (first.startsWith('[')) {
    host = first.slice(0, first.indexOf(']') + 1);
  } else if (first.split(':').length > 2) {
    return null;
  } else {
    host = first.split(':')[0];
  }
  const lowered = host.toLowerCase();
  return _isOperatorUsableHost(lowered) ? lowered : null;
}
 ...
 *
 * `x-forwarded-host` is believed ONLY under `authIdentity.isProxyHeaderTrusted`
 * — caddy ingress, where Caddy replaces a client's value, with the login on —
 * because what reads this ends up in hidden model context, and a value the
 * caller chose would let them steer the agent's links. `Host` is used
 * otherwise: it is what the operator's own client sent.
 *
 * With no usable header, the probe answer is the canonical tailnet host from
 * `host-inventory` (configured, else detected), then `_localHost()`. With no
 * `config` argument, the saved config supplies the configured name.
 *
 ...
 *   and the configured tailnet host.
...
function resolveOperatorHost(headers, config) {
  ...
  // The canonical tailnet host is the one the certificate and the served-Host
  // allowlist carry (#1905), so a link built from it is one the server accepts.
  // A caller with no config in hand (the primer's no-request path) still gets
  // the CONFIGURED name: resolving without it would fall back to the observed
  // name, which under drift is exactly the one the server refuses. The loaded
  // config is used for this answer only; proxy trust above keeps the caller's.
  const tailnet = hostInventory.resolveTailnetHost(
    config === undefined ? _loadConfigOrNull() : config).host;
  if (_isOperatorUsableHost(tailnet)) return { host: tailnet, source: 'local-probe' };

  const probed = _localHost();
  if (_isOperatorUsableHost(probed)) {
    return { host: String(probed).toLowerCase(), source: 'local-probe' };
  }
  return { host: null, source: null };
}

/**
 * The saved config, or null when it cannot be read.
 *
 * Only the tailnet answer in `resolveOperatorHost` uses it, and a null there
 * means "nothing configured", which falls back to the observed name.
 *
 * @returns {object|null}
 */
function _loadConfigOrNull() {
  try {
    ...
    log.debug('Could not load config for the operator host', { error: err.message });
    return null;
  }
}
 ...
 *
 * Prefers the machine's Tailscale Magic DNS name where there is one, falling
 * back to the OS hostname and finally
...
function _localHost() {
  ...
  _localHostCache = hostInventory.observeOverlayDns().host || _internal.hostname() || 'localhost';
 ...
 * paneless local webui transport fall back to the DB status (`active`, the
 * only non-terminal one). This is the SYNCHRONOUS signal — kept on the hot paths
...
function _liveness(session) {
  ...
  return {
    live: session.status === store.SESSION_STATUS.ACTIVE,
    source: 'db',
    incomplete: [],
    cause: null
  };
...
function _toOwnership(session, project) {
  ...
  return {
    ...
    owner: session.owner || null  // the signed-in TangleClaw user who launched it (null when no one was signed in)
  };
}
 ...
 *
 * `active` is the whole of live, and it spans the wrap — the agent is still
 * running mid-wrap, exactly the case #340's scope guard must handle — so this
 * matches `listLive` and `resolveBySessionId` with one lookup.
 *
...
function primeSection(project) {
  ...
  return [
    ...
    `- Host: \`${host}\` — this machine's own name, used to build session handles. It is NOT necessarily where the operator reaches you; the ecosystem primer carries that fact.`,
    `- Transport: \`${transport}\``,
 ...
 * (`tmux.probeSession`), so a stale `active` DB row whose pane is
 * confirmed gone is dropped rather than named as a phantom tab; remote
 ...
 * session that is still finishing. It's a snapshot — a tab opened mid-session won't
