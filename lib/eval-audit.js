 ...
 * Keys match the step IDs used in the methodology template's wrap pipeline
 * (`wrap_pipeline.steps[].id` post-#139 Chunk 2; legacy `wrap.steps`
 * entries before that). Unknown ids get benefit-of-doubt — see
 * `scoreWrapQuality`. Extend this table whenever a new step kind ships
 * a recognizable narrative marker.
 ...
 * @param {object} [methodology] - Methodology template object (has either
 *   the new `wrap_pipeline` schema or the legacy `wrap` block)
...
function scoreWrapQuality(exchanges, methodology) {
  // Route through the skills shim so eval-audit and triggerWrap read the
  // same conceptual state (#139 Chunk 2; ADR 0001 — symmetric capability gates).
  const wrapShape = skills.wrapShapeFromTemplate(methodology);
  const wrapSteps = (wrapShape && wrapShape.steps) || [];
