...
// ── Eval Dimensions ──
//
// The one dimension set every audited project is scored against. These were
// once overridable per methodology template; with that layer retired (#538)
// the audit scores one governance-agnostic contract, so a score from one
// project is comparable with a score from any other.
...
/**
 * Get the eval dimensions every audited exchange is scored against.
 * @returns {object} evalDimensions
 */
function getEvalDimensions() {
 ...
 * Assembled from the judge context + dimension definitions.
 *
 * @param {string} judgeContext - Judge context string
 ...
 * Keys match the wrap pipeline's step IDs (`lib/wrap-default-pipeline.js`
 * `steps[].id`). Unknown ids get benefit-of-doubt — see `scoreWrapQuality`.
 * Extend this table whenever a new step kind ships a recognizable
 * narrative marker.
...
const WRAP_STEP_PATTERNS = {
  ...
  'handoff-stage': /handoff|publication|tc\.handoff/i,
  'preflight': /preflight|prawduct|governance\s*(gate|verdict)|stop\s*hook/i,
  'session-files': /session-files|uncommitted\s+files?|include\s+or\s+leave/i
};
 ...
 * @param {string[]} [wrapStepIds] - The step ids this project's wrap actually
 *   runs (the code-owned pipeline minus override-disabled steps) — pass the
 *   effective set so a commit-only project isn't scored against steps it has
 *   deliberately turned off. Empty/absent scores 1.0 (nothing expected).
 * @returns {{ score: number, stepsFound: string[], stepsMissing: string[], totalSteps: number }}
 */
function scoreWrapQuality(exchanges, wrapStepIds) {
  const wrapSteps = Array.isArray(wrapStepIds) ? wrapStepIds : [];
...
// ── Bidirectional (Human) Scoring ──

/**
 * Validate a human score submission.
 * @param {object} data - { score, comment?, dimensions? }
 * @returns {{ valid: boolean, error?: string }}
 */
function validateHumanScore(data) {
  if (!data || typeof data !== 'object') {
    return { valid: false, error: 'Data must be a non-null object' };
  }
  if (data.score === undefined || data.score === null) {
    return { valid: false, error: 'Missing required field: score' };
  }
  if (typeof data.score !== 'number' || data.score < 1 || data.score > 5) {
    return { valid: false, error: 'score must be a number between 1 and 5' };
  }
...
/**
 * Check whether a session has exceeded its cost cap.
 * @param {number} sessionCost - Current accumulated cost in USD
 * @param {number} capUsd - Cost cap in USD
 * @returns {{ exceeded: boolean, currentCost: number, cap: number }}
 */
function checkCostCap(sessionCost, capUsd) {
  return {
    exceeded: sessionCost >= capUsd,
...
/**
 * Run the retention policy: purge exchanges and scores older than retentionDays.
 * @param {object} store - Store instance
 * @param {number} retentionDays - Number of days to retain data
 * @returns {{ exchangesPurged: number, scoresPurged: number, cutoffDate: string }}
 */
function runRetentionPolicy(store, retentionDays) {
  if (!retentionDays || retentionDays <= 0) {
    return { exchangesPurged: 0, scoresPurged: 0, cutoffDate: null };
  }

  const cutoffDate = new Date(Date.now() - retentionDays * 86400000).toISOString();
  const result = store.evalExchanges.purgeOlderThan(cutoffDate);

  if (result.exchangesPurged > 0 || result.scoresPurged > 0) {
    log.info('Retention policy executed', {
      retentionDays,
      cutoffDate,
      exchangesPurged: result.exchangesPurged,
      scoresPurged: result.scoresPurged
    });
  }

  return { ...result, cutoffDate };
}
