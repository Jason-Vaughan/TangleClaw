...
function _considerEscalation(x, now, settings) {
  ...
  if (rung < RUNG.escalated && t.escalate !== null && (age >= t.escalate || budgetSpent)) {
    ...
    for (const principal of route.principals) {
      const body = _noticeBody(x, 'escalated', now, { to: 'escalation', controlState: route.controlState, controlGeneration: route.stateGeneration });
...
function start(opts = {}) {
  ...
  _timer = setInterval(() => {
    try {
      tick().notices.catch((err) => log.warn('Medusa watchdog notices failed', { error: err.message }));
...
module.exports = {
  DEFAULTS, BOUNDS, validatePatch, resolveSettings, sendThresholds, tick, retireRecipient, listEscalations, escalationSummary,
