 ...
 *
 * Re-arm rules:
 * - Only an exchange at `wake_attempted` or `wake_not_accepted` with nothing
 *   read or acknowledged.
 * - A `wake_not_accepted` (the nonce was left in the composer) is eligible at
 *   once. An unconfirmed attempt becomes eligible `rearmAfterMs` after it.
...
function _considerRearm(x, now, settings) {
  if (x.state !== 'wake_attempted' && x.state !== 'wake_not_accepted') return false;
  ...
  const attempts = facts.filter((f) => f.fact === 'wake_attempted');
  const last = attempts[attempts.length - 1];
  if (!last) return false;
  const eligibleAt = Date.parse(last.at) + (x.state === 'wake_not_accepted' ? 0 : settings.rearmAfterMs);
  ...
  const row = exchanges.rearm(x.exchange_id, {
    ...
    code: x.state === 'wake_not_accepted' ? 'not-accepted' : 'unconfirmed',
