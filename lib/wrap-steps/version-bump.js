 ...
 *    `ready` verdict. `ask` never cuts by itself. In both, a bump level the
 *    operator picked in the wrap modal cuts whatever the verdict.
 *  - A hold is a skip (never blocks). Its output carries `held:true`,
 *    `releaseMode`, `readiness` (`verdict`, `reason`, `signals`),
 *    `needsOperator` (true for `ask`, and for `auto` on an `unknown` verdict)
 *    and `wouldBump` (`from`, `to`, `bumpLevel`). A cut carries `releaseMode`,
 *    `readiness` and `decidedBy` (`readiness` or `operator`).
 *
...
async function run(context) {
  ...
  const gate = _releaseGate(releaseMode.mode, readiness, requestedLevel);
  const releaseRecord = {
  ...
  if (!gate.cut) {
    return _skipped(gate.reason, project.name, {
      ...releaseRecord,
      held: true,
  ...
  if (gate.decidedBy === 'operator' && readiness.verdict !== 'ready') {
    output.detail += `; cut on the operator's ${bumpLevel} pick (readiness: ${readiness.verdict})`;
  }
 ...
 * and any bump level the operator picked in the wrap modal.
 *
 * A picked level is the operator's decision to cut, so it wins in `auto` and
 * `ask` whatever the verdict. The modal sends no level for its "auto" choice,
 * so its absence is never read as a decision. `off` never reaches here.
 *
 * @param {'auto'|'ask'} mode
 * @param {{verdict:string, reason:string}} readiness
 * @param {string|null|undefined} requestedLevel - `options.bumpLevel`, already validated
 * @returns {{cut:true, decidedBy:'operator'|'readiness'}|{cut:false, needsOperator:boolean, reason:string}}
 */
function _releaseGate(mode, readiness, requestedLevel) {
  if (requestedLevel !== undefined && requestedLevel !== null) {
    return { cut: true, decidedBy: 'operator' };
  }
  if (mode === 'auto' && readiness.verdict === 'ready') {
  ...
  if (mode === 'ask') {
    return {
      cut: false,
      needsOperator: true,
      reason: `release held: releaseMode is ask, so this wrap cuts only on a bump level picked in the wrap modal (readiness: ${readiness.verdict} — ${readiness.reason})`
    };
  }
  // auto, and the verdict isn't `ready`. An `unknown` verdict is a signal that
  // couldn't be read, which is the operator's call rather than a plain hold.
  return {
    cut: false,
    needsOperator: readiness.verdict !== 'not-ready',
    reason: `release held: readiness is ${readiness.verdict} — ${readiness.reason}. Pick a bump level in the wrap modal to cut anyway.`
  };
}
