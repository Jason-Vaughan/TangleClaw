 ...
 *  - A CHANGELOG carrying release headings that don't parse as
 *    `## [MAJOR.MINOR.PATCH] - YYYY-MM-DD` skips: the project's scheme isn't
 *    one this step can extend. (A changelog with NO release headings is the
 *    unrelated first-release case and still bumps — see
 *    {@link _hasReleaseHeadings}.)
 *
...
/**
 * Whether the CHANGELOG carries any released-version heading at all — any
 * `## [...]` line that isn't `[Unreleased]`, regardless of whether its version
 * parses.
 *
 * This exists to disambiguate the two very different reasons
 * {@link _topReleasedVersion} returns `null`:
 *
 *  - **No release headings** (`false` here) — a first-ever release. The drift
 *    guard has nothing to compare against and the bump should proceed.
 *  - **Release headings that don't parse** (`true` here) — the project uses a
 *    versioning scheme this step doesn't understand (a 4-octet counter, a date
 *    stamp, a calendar version). Bumping anyway means writing a heading in a
 *    scheme the project doesn't use, above the real one.
 *
 * Conflating those two is what let the guard fail open: it was written as
 * `if (topReleased && …)`, so an unparseable scheme skipped the check entirely
 * instead of tripping it.
 *
 * @param {string} changelogText
 * @returns {boolean}
 */
function _hasReleaseHeadings(changelogText) {
  const lines = String(changelogText || '').split('\n');
  for (const line of lines) {
    if (!NEXT_HEADING_RE.test(line)) continue;
    if (UNRELEASED_LINE_RE.test(line)) continue;
    return true;
  }
  return false;
}
...
async function run(context) {
  ...
  // proceeds.
  if (!topReleased && _hasReleaseHeadings(changelogText)) {
