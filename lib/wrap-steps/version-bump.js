...
async function run(context) {
  ...
  // API validator applies the same shared predicate, but this is the actual
  // write site and a hand-edited `.tangleclaw/project.json` never passes
  // through the validator.
  let configuredPath = null;
  if (configuredRel) {
    const contained = projectPaths.resolveWithinProject(project.path, configuredRel);
    if (!contained.ok) {
      return skip(`versionFilePath ${JSON.stringify(configuredRel)} ${contained.reason} — refusing to read or write it`);
    }
    configuredPath = contained.path;
  }
