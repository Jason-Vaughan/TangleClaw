...
function _resolveVersionJson(versionPath) {
  ...
  if (json.version === undefined || json.version === null || json.version === '') {
    return { skip: 'version.json has no "version" field — nothing to bump', noVersion: true };
  }
...
function _resolveVersionSource(versionPath, pkgPath, configuredPath) {
  ...
  // The built-in probe, in the same order the version reader walks
  // (`lib/project-version-files.js:detectLiveVersion`). A file that exists but
  // carries no version at all is passed over, as the reader passes over it: a
  // Python repo's tooling-only package.json must not stop the probe short of
  // its pyproject.toml. Anything else wrong with a file (unreadable, not an
  // object, a version that isn't semver, a shape the TOML scanner can't edit)
  // still stops here, because that file is the one the project meant.
