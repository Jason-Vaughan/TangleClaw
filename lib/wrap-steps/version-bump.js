...
async function run(context) {
  ...
  // entirely: no version.json, no CHANGELOG promotion, and no stamp on
  // `.prawduct/change-log.md` (Prawduct-owned state a dormant project keeps
  // untouched). `[Unreleased]` carries to the next wrap whose engine can run
  // it. Held rather than bumped-but-unmerged, because a bump rides any later
  // manual merge into a tagged release that never had its gates run.
