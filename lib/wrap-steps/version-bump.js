 ...
 *    `reason`, `signals`), `needsOperator` and `wouldBump` (`from`, `to`,
 *    `bumpLevel`), plus `decidedBy:'operator'` for a Hold. A plain hold is a
 *    skip. A hold that needs the operator (`ask`, or `auto` on an `unknown`
 *    verdict) with no decision made is the one case this step stops the wrap:
...
async function run(context) {
  ...
  const gate = _releaseGate(releaseMode.mode, readiness, { level: requestedLevel, release: requestedRelease });
  const releaseRecord = {
 ...
 * and whatever the operator decided in the wrap modal or the drawer.
 *
 ...
 * @returns {{cut:true, decidedBy:'operator'|'readiness'}|{cut:false, needsOperator:boolean, reason:string, decidedBy?:'operator'}}
 */
function _releaseGate(mode, readiness, decision) {
  ...
  if (release === 'hold') {
    return {
      ...
      needsOperator: false,
      decidedBy: 'operator',
      reason: `release held on the operator's decision (readiness: ${readiness.verdict} — ${readiness.reason})`
    };
  }
  ...
  if (mode === 'ask') {
    return {
      cut: false,
      needsOperator: true,
      reason: `release decision needed: releaseMode is ask, so this wrap cuts only when you choose Cut (readiness: ${readiness.verdict} — ${readiness.reason})`
    };
  }
  ...
  if (readiness.verdict !== 'not-ready') {
    return {
      cut: false,
      needsOperator: true,
      reason: `release decision needed: readiness is ${readiness.verdict} — ${readiness.reason}`
    };
  }
  return {
    cut: false,
    needsOperator: false,
    reason: `release held: readiness is ${readiness.verdict} — ${readiness.reason}. Choose Release: Cut in the wrap modal to cut anyway.`
  };
}
