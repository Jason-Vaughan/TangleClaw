...
function _classifyTopRelease(changelogText) {
  ...
  for (const line of lines) {
    ...
    // unreleased heading, and must not read as a release.
    if (/^## \[Unreleased\]/.test(line)) continue;
    const raw = line.trim();
    const plain = /^## \[(\d+)\.(\d+)\.(\d+)\]/.exec(line);
    if (plain) {
      return {
        kind: 'released',
        version: { major: Number(plain[1]), minor: Number(plain[2]), patch: Number(plain[3]) },
        raw
      };
    }
    // `[^\]]*` rather than an explicit character class so a combined
    // prerelease+build suffix (`1.0.0-rc.1+build.5`) is recognized too.
    if (/^## \[\d+\.\d+\.\d+[-+][^\]]*\]/.test(line)) {
...
/**
 * The top released version when it is one this step can compare against, else
 * null. Thin accessor over {@link _classifyTopRelease} — kept because the #203
 * drift-guard tests and any external caller speak in these terms.
 *
 * @param {string} changelogText
 * @returns {{major:number,minor:number,patch:number}|null}
 */
function _topReleasedVersion(changelogText) {
  ...
  return top.kind === 'released' ? top.version : null;
}
...
async function run(context) {
  ...
  if (topRelease.kind === 'foreign') {
    return _skipped(`refusing to bump: CHANGELOG.md's newest release heading (${topRelease.raw}) isn't MAJOR.MINOR.PATCH, so this project's versioning scheme isn't one this step can extend safely. Set versionBumpEnabled:false to silence this, or reconcile the changelog format.`);
  }

  if (topRelease.kind === 'unbumpable') {
    return _skipped(`refusing to bump: CHANGELOG.md's newest release heading (${topRelease.raw}) carries a prerelease or build suffix, so whether ${newVersion} supersedes it is ambiguous. Promote or reconcile that release manually before re-wrapping.`);
  }

  if (topRelease.kind === 'released'
      && _compareSemver(_parseSemver(newVersion), topRelease.version) <= 0) {
