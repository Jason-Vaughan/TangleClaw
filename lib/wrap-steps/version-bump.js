 ...
 * emits UTC, which produces a wrong date on the new release heading
 * when the operator's local day boundary has not yet crossed midnight
 * UTC (e.g. evening PT = next-day UTC). Every prior CHANGELOG entry
 * in this repo is local-zoned; using `getFullYear` / `getMonth` /
 * `getDate` matches that convention without pulling in a tz library.
 *
