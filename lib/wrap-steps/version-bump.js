 ...
 *  - A CHANGELOG whose release headings carry a version in some other scheme
 *    (`## [2.85.0.41]`, a calendar version) skips: it isn't one this step can
 *    extend. A changelog with no release headings yet is the unrelated
 *    first-release case and still bumps, and a 3-octet heading is recognized
 *    regardless of what follows the bracket — see
 *    {@link _hasForeignSchemeHeadings}.
 *
...
/**
 * Whether the CHANGELOG carries a release heading whose bracketed version is in
 * some scheme other than 3-octet semver — a 4-octet counter (`## [2.85.0.41]`),
 * a calendar version, a date stamp.
 *
 * This exists to disambiguate the two very different reasons
 * {@link _topReleasedVersion} returns `null`:
 *
 *  - **A first-ever release** (`false` here) — nothing to compare against, so
 *    the bump should proceed.
 *  - **A scheme this step doesn't recognize** (`true` here) — bumping anyway
 *    means writing a heading in a scheme the project doesn't use, above its
 *    real one.
 *
 * Conflating those two is what let the drift guard fail open: it was written as
 * `if (topReleased && …)`, so an unrecognized scheme skipped the check entirely
 * instead of tripping it.
 *
 * Deliberately narrower than "any heading `_topReleasedVersion` rejects".
 * That parser additionally requires ` - YYYY-MM-DD`, so keying the guard on it
 * would hard-skip every wrap for a project whose versions are ordinary semver
 * but whose headings omit the date or use an en-dash — and blame their
 * "versioning scheme", which would be the wrong diagnosis. A heading whose
 * version IS 3-octet semver is recognized here regardless of what follows it;
 * only the version's *shape* decides.
 *
 * @param {string} changelogText
 * @returns {boolean}
 */
function _hasForeignSchemeHeadings(changelogText) {
  const lines = String(changelogText || '').split('\n');
  for (const line of lines) {
    if (!NEXT_HEADING_RE.test(line)) continue;
    // Prefix match, not full-line: `## [Unreleased] - TBD` is still the
    // unreleased heading, and must not read as a foreign release scheme.
    if (/^## \[Unreleased\]/.test(line)) continue;
    // Prerelease / build metadata (`1.0.0-beta.1`, `1.0.0+build.5`) is valid
    // semver and must not read as a foreign scheme — `_parseSemver` rejects it
    // for *bumping*, but that is a different question from whether the project
    // versions in a scheme this step recognizes.
    if (/^## \[\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?\]/.test(line)) continue;
    return true;
  }
  return false;
}
...
async function run(context) {
  ...
  if (requestedLevel !== undefined && requestedLevel !== null
    ...
    return _skipped(`invalid bumpLevel override ${JSON.stringify(requestedLevel)} — expected one of ${BUMP_LEVELS.join(', ')}`);
  }
  ...
  const configuredRel = projConfig && typeof projConfig.versionFilePath === 'string'
    && projConfig.versionFilePath.trim() !== ''
    ? projConfig.versionFilePath.trim()
    : null;
  const configuredPath = configuredRel
    ? path.resolve(project.path, configuredRel)
    : null;
  // The commit step flushes whatever this resolves to, so a value that escapes
  // the project root would make a settings field an arbitrary-file write. The
  // API validator rejects those, but this is the actual write site and a
  // hand-edited `.tangleclaw/project.json` never passes through the validator.
  if (configuredPath) {
    const root = path.resolve(project.path);
    const rel = path.relative(root, configuredPath);
    if (rel === '' || rel.startsWith('..') || path.isAbsolute(rel)) {
      return _skipped(`versionFilePath ${JSON.stringify(configuredRel)} resolves outside the project root — refusing to read or write it`);
    }
  }
  ...
  if (!newVersion) {
    ...
    return _skipped(`refusing to bump: ${JSON.stringify(currentVersion)} in ${source.kind} isn't MAJOR.MINOR.PATCH semver, so this step can't derive the next version. Set versionBumpEnabled:false if this project manages its own versioning.`);
  }
  ...
  // The guard above only fires when a top version parsed. `_topReleasedVersion`
  // returns null for any scheme that isn't 3-octet `## [X.Y.Z] - YYYY-MM-DD`,
  // so on a 4-octet (or calendar, or date-stamped) changelog it used to skip
  // ITSELF and let the bump through — the fail-open hole that made the
  // wrong-file bump above reachable. Headings that exist but don't parse mean
  // "this project's scheme isn't one I understand", which is a reason to stop.
  // No headings at all is the other case entirely: a first release, which
  // proceeds.
  if (!topReleased && _hasForeignSchemeHeadings(changelogText)) {
    return _skipped(`refusing to bump: CHANGELOG.md has release headings but none parse as "## [MAJOR.MINOR.PATCH] - YYYY-MM-DD", so this project's versioning scheme isn't one this step can extend safely. Set versionBumpEnabled:false to silence this, or reconcile the changelog format.`);
  }
  ...
  if (topReleased && _compareSemver(_parseSemver(newVersion), topReleased) <= 0) {
    ...
    return _skipped(`refusing to bump: newVersion (${newVersion}) is not strictly greater than CHANGELOG top released (${tr}). ${source.kind} may have drifted; reconcile manually before re-wrapping.`);
  }
...
function _skipped(reason) {
  ...
  // identical in the log — which is the question #540 was filed to answer. The
  // pipeline runner logs project/step around this, so the reason is the part
  // that was missing.
  log.info('version bump skipped', { reason });
