...
function ignoreSuggestions(keptLocal, { dirty, hasTracked, classOf }) {
  ...
  for (const p of keptLocal || []) {
    // A line break in a path would split one suggestion into several rules, the
    // second of them the path's own text. No line is safer than that.
    if (/[\r\n]/.test(p)) continue;
...
/**
 * Escape a path so gitignore reads it as that one literal name. Gitignore drops
 * unescaped trailing spaces, so each one is escaped too, or `/name ` would
 * ignore `/name` instead.
 *
 * @param {string} rel - Repo-root-relative path, free of line breaks.
 * @returns {string}
 */
