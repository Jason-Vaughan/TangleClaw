 ...
 *   - **Self-heal when `PROJECT-MAP.md` is missing** (#423): the toggle is the
 *     operator's stated intent ("I want a project map") and the toggle — not
 *     file deletion — is the off-switch, so a missing file is CREATED from the
 *     current skeleton + membership rather than skipped forever. This covers a
 *     fresh clone where the file wasn't committed, a delete-to-regenerate, or a
 *     toggle enabled through a path that didn't seed. Create is as cheap as a
 *     refresh (one top-level `readdir`, no tree walk).
 ...
 *     created, addedDirs, removedDirs}` under `staged['project-map:refresh']`.
 ...
 *     is emitted from `_buildBodyLines` against `{mapRefresh, created, addedDirs, removedDirs}`.
 *
...
async function run(context) {
  ...
  let created;
  if (existing === null) {
    // Self-heal (#423): toggle on but file missing — create it from the current
    // skeleton + membership. The toggle is the off-switch, not file deletion, so
    // a missing index is an anomaly to recover, not a permanent skip.
    newContent = projects._buildProjectMapContent(project.path, groups);
    addedDirs = currentDirs.slice();
    removedDirs = [];
    created = true;
  } else {
    newContent = projects._refreshProjectMapContent(existing, currentDirs, groups);
    ...
    addedDirs = currentDirs.filter((d) => !existingDirs.includes(d));
    removedDirs = existingDirs.filter((d) => !currentDirs.includes(d));
    created = false;
  }
  ...
  log.info(created ? 'project-map staged create (self-heal)' : 'project-map staged refresh', {
  ...
  return {
    ...
    output: {
      ...
      removedDirs,
      detail: created
        ? `${projects.PROJECT_MAP_FILENAME} created (${addedDirs.length} dir(s))`
        : `${projects.PROJECT_MAP_FILENAME} refreshed (+${addedDirs.length}/-${removedDirs.length} dir(s))`
    },
    blockers: []
  };
}
