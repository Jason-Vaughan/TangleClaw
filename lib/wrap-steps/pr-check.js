 ...
 *     session-scoped PRs
 * The handler validates the entries, stages them, and blocks until every
 * session-scoped PR has one. Applying a `merge` is deliberately NOT done
 * here: it must happen after `commit` (see `pr-merge.js`), or the wrap
 * would merge a PR that is missing its own wrap commit. `commit` reads
 * `context.staged[step.id]` to embed the choices in the wrap commit body.
 *
 * **Step config.**
 *   - `step.includeDrafts` — boolean; default `false`. Open draft
 *     PRs are excluded unless the step is configured to include them.
 *
 * **Single-transaction discipline.** This handler reads only — it never
 * mutates git, never writes the filesystem, and never touches the remote.
 * `context.staged[step.id]` is its sole side effect. The one outward-facing
 * action in this pair, enqueueing auto-merge, lives in `pr-merge.js`.
 */

const { createLogger } = require('../logger');
const { execShell } = require('./_exec-shell');
const { reasonFromFailure } = require('./_remote-output');
...
async function defaultListOpenPrs(cwd) {
  ...
  if (r.exitCode !== 0) {
    ...
    // gh prints actionable text on stderr ("no auth", "not a repo", etc), so
    // surface it and the UI can render the recovery hint inline rather than
    // just "gh failed." Bounded and credential-stripped by the shared helper:
    // `gh` reaches a remote, so this is the class of text that can echo a
    // tokenised URL.
    return { ok: false, prs: [], reason: reasonFromFailure(r), exitCode: r.exitCode };
  }
  let parsed;
  try {
...
async function run(context) {
  ...
  try {
    ...
    if (invalid.length > 0 || unresolved.length > 0) {
      ...
      staged[step.id] = { branch, sessionScoped, resolutions, invalidHandling: invalid };
