...
async function run(context) {
  ...
  if (provenance.refresh === 'failed' || provenance.state === upstream.STATES.UNAVAILABLE) {
    log.warn('upstream provenance is not established; Include advice is withheld for files needing a decision', {
      project: project.name, state: provenance.state, ref: provenance.ref, refresh: provenance.refresh,
      refreshProblem: provenance.refreshProblem, problem: provenance.problem
    });
  }
  ...
  const summary = {
    ...
    // Paths this branch and upstream both changed: committed as the branch's
    // own work, and named so the divergence is visible.
    provenanceDiverged: upstream.divergedOf(provenance),
 ...
 *   safetyWithheld?:string[], refusedIncludes?:string[], alreadyUpstream?:string[], provenanceDiverged?:string[],
 *   provenance?:object}} s
 * @returns {string}
 */
function _detail(s) {
  ...
  const diverged = (s.provenanceDiverged || []).length;
  if (diverged) {
    parts.push(`${diverged} changed on both this branch and ${(s.provenance && s.provenance.ref) || 'upstream'}: ${s.provenanceDiverged.join(', ')}`);
  }
