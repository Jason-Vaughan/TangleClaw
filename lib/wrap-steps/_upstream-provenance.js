 ...
 * that must decide what is committed (the commit step). It makes no network
 * call, re-reads the work tree, and compares against that same recorded commit.
 * The changelog check, which only counts work, reuses `session-files`' verdicts
 * instead ({@link fromPreviousResults}); the commit step stays the authority. It also reads the local ref again:
...
/** Longest refresh failure reason shown to the operator; the log keeps the rest. */
const REFRESH_PROBLEM_CHARS = 160;
...
async function _factsAgainst(exec, toplevel, dirty, sha, state, opts = {}) {
  ...
  for (const f of dirty) {
    ...
    // Commits unique to this branch changed the path since it forked (proven
    // from history: the blob at HEAD differs from the blob at the merge-base).
    // Then an uncommitted edit is the branch's own line of work, not a stale
    // copy of upstream's, and the branch's merge is where the two meet. A path
    // HEAD does not hold never qualifies, whatever its history: an untracked
    // file is exactly what the stale-root case looks like.
    const branchChanged = !f.newToRepo && baseBlobs && headBlobs
      ? (baseBlobs.get(p) || null) !== (headBlobs.get(p) || null)
      : false;
    const entry = {
      upstream: fact,
      upstreamChanged,
...
/**
 * A failed refresh's reason, fit for the drawer. Remote error text can echo the
 * remote's URL, and a URL can carry a credential, so it is redacted exactly as
 * the behind-origin check redacts its own, then shortened: the operator needs
 * the cause in one line, and the full text is in the server log.
 *
 * @param {string} detail - First line of git's stderr, or ''.
 * @returns {string}
 */
function _refreshProblemOf(detail) {
  const text = String(detail || '').replace(/^fatal:\s*/i, '').trim();
  if (!text) return 'the fetch failed';
  // Judged on the WHOLE text, so no secret escapes by being cut in half. The
  // helper also caps its output from the end, which would drop the cause, so
  // its verdict is used but the shortening below is done here.
  const verdict = redactRemoteOutput(text);
  if (verdict && verdict.startsWith(REDACTED_PREFIX)) return verdict;
  const redacted = text.replace(/\/\/[^/\s@]+@/g, '//***@');
  if (redacted.length <= REFRESH_PROBLEM_CHARS) return redacted;
  // The middle goes: git names the remote first and the cause last, and a long
  // remote path must not push the cause off the end.
  const head = Math.floor(REFRESH_PROBLEM_CHARS * 0.4);
  const tail = REFRESH_PROBLEM_CHARS - head - 1;
  return `${redacted.slice(0, head)}…${redacted.slice(-tail)}`;
}
...
async function capture(toplevel, dirty, opts = {}) {
  ...
  if (opts.refresh !== false) {
    ...
    if (fetched.ok) {
      ...
      refresh = 'failed';
      refreshProblem = _refreshProblemOf(fetched.detail);
    }
  }
  if (!target) {
...
function explain(entry, summary, status = {}) {
  ...
  switch (entry.verdict) {
    ...
    case VERDICTS.UNVERIFIED:
      // The reason is said once, in the headline; the row says what it means here.
      if (summary.state === STATES.STALE && entry.upstream !== FACTS.UNKNOWN) {
        return `${ref} was not refreshed this wrap, so it may already have this file. It isn't recommended for the commit`;
      }
      return `couldn't confirm what ${ref} holds for this file, so it isn't recommended for the commit`;
    default:
      if (entry.branchChanged && entry.upstream === FACTS.EQUAL) {
        return `this matches ${ref} again, undoing a change this branch committed`;
      }
      if (entry.branchChanged && entry.upstreamChanged === true) {
        return `this branch and ${ref} have both changed this file since they split; the branch's merge is where they meet`;
      }
...
/**
 * Paths both this branch's own commits and upstream changed since they split,
 * judged as the branch's own work (verdict `none`) but reported so the
 * divergence is visible.
 *
 * @param {object|null} provenance
 * @returns {string[]}
 */
function divergedOf(provenance) {
  return Object.entries((provenance && provenance.paths) || {})
    .filter(([, e]) => e && e.branchChanged && e.upstreamChanged === true)
    .map(([p]) => p);
}
...
module.exports = {
  ...
  VERDICTS,
  VERDICT_RANK,
  _internal: { _verdict, _countLines, _literalPathspec, _refreshProblemOf }
};
