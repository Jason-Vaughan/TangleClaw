...
/**
 * Local-zoned YYYY-MM-DD formatter (#205 parity). `Date#toISOString`
 * always emits UTC, which produces a heading dated next-day on hosts
 * with negative UTC offsets running late local-evening. The
 * `## TODO (auto-stubbed YYYY-MM-DD)` section dates are user-visible
 * annotations next to file pointers; an off-by-one date is noise but
 * still wrong. Using `Date#getFullYear` / `getMonth` / `getDate`
 * reflects the operator's local clock without pulling a tz library.
 * Mirrors `lib/wrap-steps/version-bump.js:_todayIsoLocal`; both call
 * sites should ultimately share a util module, but the duplication
 * is intentional in this chunk to keep the change surgical — the
 * shared-util refactor lands as a follow-up after both #216 (the
 * version-bump fix) and this PR merge.
 *
