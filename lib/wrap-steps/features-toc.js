 ...
 *     range resolves, or when the session diff is empty. The session range opens
 *     at the session's launch baseline, or at `lastWrapSha` when a wrap already
 *     ran inside this session, and runs to HEAD regardless of branch topology
 *     (#465), so drift is captured even when the session merged its PRs and wraps
 *     while checked out on `main`. Falls back to `<base>...HEAD` (`main`, then
 *     `master`) when neither boundary resolves. A trunk merged into the session's
 *     branch contributes no files (#1309). See {@link _resolveSessionRange} and
 *     {@link _sessionTouchedFiles}.
...
async function run(context) {
  ...
  const stoppedProbes = [];
  const { scope } = context;
  const sessionRange = _resolveSessionRange(
    project.path,
    scope ? scope.lastWrapSha : wrapState.readLastWrapSha(configRootOf(project)).sha,
    stoppedProbes,
    scope && scope.baseline ? scope.baseline.sha : null
  );
  if (sessionRange) {
    let touchedFiles = null;
    try {
      touchedFiles = _sessionTouchedFiles(project.path, sessionRange.range, scope);
    } catch (err) {
 ...
 *   base:(string|null), stopped:string[]}|null}
 */
function _resolveSessionRange(cwd, lastWrapSha, stoppedProbes = null, launchSha = null) {
...
/**
 * The files this session changed within `range`.
 *
 * A linear range answers from a plain `git diff`. When the first-parent line
 * carries a merge, the diff would also carry whatever that merge brought in — on
 * a feature branch that pulled the trunk, other sessions' files, which is how six
 * of them were stubbed as "touched in this session" (#1309). Then the files come
 * from the first-parent walk, with trunk syncs left out (`./_git-range`). Deleted
 * paths are included either way, as the diff includes them; `_stillExists` filters
 * on the write path.
 *
 * @param {string} cwd - Project root.
 * @param {string} range - A resolved range.
 * @param {object|null} [scope] - The wrap run's scope, for the trunk position.
 * @returns {string[]} Repo-root-relative paths.
 */
function _sessionTouchedFiles(cwd, range, scope = null) {
  if (!gitRange.hasFirstParentMerges(cwd, range, _internal.execSync)) {
    return _diffNameOnly(cwd, range);
  }
  const commits = gitRange.listSessionCommits(cwd, range, {
    exec: _internal.execSync,
    relative: false,
    trunk: scope ? scope.trunk : undefined
  });
  const net = gitRange.netChanges(commits);
  return [...net.touched, ...net.deleted];
}
