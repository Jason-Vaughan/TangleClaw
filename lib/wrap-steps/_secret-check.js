 ...
 * @returns {{state:'clean', bytes:number}|{state:'gone'}|{state:'flagged', rules:string[], bytes:number}|{state:'skipped', reason:string}}
 *   `bytes` is what was read, so a caller can hold a total-bytes budget.
 */
...
function check(root, classified, decisions, budget) {
  ...
  if (root) {
    for (const p of candidates) {
      if (scannedCount >= maxFiles || bytesRead >= maxBytes) {
        skipped.push({
          path: p,
          reason: `the wrap stopped scanning after ${scannedCount} files (${bytesRead} bytes), so it was not scanned`
        });
        continue;
      }
