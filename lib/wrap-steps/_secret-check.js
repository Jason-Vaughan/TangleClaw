...
/**
 * Largest file scanned. The same cap uploads use: a multi-megabyte file in a
 * working tree is almost always generated or binary, and reading it costs
 * memory for no signal.
 */
const SCAN_SIZE_CAP = 1024 * 1024;
 ...
 * @returns {{state:'clean'}|{state:'gone'}|{state:'flagged', rules:string[]}|{state:'skipped', reason:string}}
 */
function scanFile(root, rel) {
  ...
  if (st.size === 0) return { state: 'clean' };
  let buffer;
  try {
  ...
  if (buffer.length === 0) return { state: 'clean' };
  if (!secretScan.looksLikeText(buffer)) {
    return { state: 'skipped', reason: 'looks binary, so it was not scanned' };
  }
  const result = secretScan.scanText(buffer.toString('utf8'));
  return result.flagged ? { state: 'flagged', rules: result.types } : { state: 'clean' };
}
 ...
 */
function check(root, classified, decisions) {
  ...
  if (root) {
    for (const p of candidates) {
      ...
      if (r.state === 'skipped') { skipped.push({ path: p, reason: r.reason }); continue; }
      scannedCount += 1;
