...
function check(root, classified, decisions, budget) {
  ...
  const foreignUndecided = classified.undecided.map((f) => {
    ...
    // A file that matches a secret rule is never recommended for Include (#1858):
    // the recommendation is what Apply fills in, and one click must not commit a
    // credential. The operator can still choose Include for it explicitly.
    return rules
      ? { ...f, why: `${f.why}; it also ${_rulesPhrase(rules)}`, secretRules: rules, ...(f.recommendation === 'include' ? { recommendation: null, recommendationWhy: null } : {}) }
      : f;
  });
  ...
  return {
    // Through `reclassify`, not a hand-rebuilt `stageable`: narrowing the
    // buckets also changes what `git add` may be handed and can split a rename
    // whose halves were both authorized a moment ago. Rebuilding one selection
    // and spreading the other two would leave them describing the selection
    // this just replaced, with nothing failing (#1513, #1629).
    classified: ownership.reclassify(classified, { owned, included, tangleclawMaintenance }),
