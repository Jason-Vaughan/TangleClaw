...
function todayIsoLocal() {
  return isoLocalDate(new Date().getTime());
}

/**
 * Local-zoned YYYY-MM-DD for a given instant — the same convention as
 * {@link todayIsoLocal}, for a time other than now (a session's start).
 *
 * @param {number} ms - Epoch milliseconds
 * @returns {string} `YYYY-MM-DD` in the host's local timezone.
 */
function isoLocalDate(ms) {
  const d = new Date(ms);
