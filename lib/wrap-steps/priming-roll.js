...
// Read-only fallbacks for projects authored before the move. 48 plan files
// across 12 projects live here, so resolution checks the legacy location when
// the TC-owned one is absent. Writes always target the paths above — the
// fallback lets old plans keep working, it does not keep the old home alive.
...
function _resolvePlansDir(projectPath) {
  const preferred = path.join(projectPath, DEFAULT_PLANS_DIR);
  if (_internal.existsSync(preferred)) {
    return { dir: preferred, relative: DEFAULT_PLANS_DIR, legacy: false };
  }
  const legacy = path.join(projectPath, LEGACY_PLANS_DIR);
  if (_internal.existsSync(legacy)) {
    return { dir: legacy, relative: LEGACY_PLANS_DIR, legacy: true };
  }
  return null;
}
