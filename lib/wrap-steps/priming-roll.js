 ...
 * is preserved byte-for-byte (modulo trailing whitespace at end of file
 * when the block is first appended). If the markers don't exist yet,
 * the handler appends a fresh managed block; if there is exactly one of
 * each but out of order, it repairs them into one well-formed block and
 * keeps the text that was between them below it; any other count is
 * refused, because a marker literal is not proof the marker is ours —
 * the shared policy in `lib/managed-block.js`, which the engine-config
 * layer splices through as well. The handler does NOT
...
const ghIssueState = require('../gh-issue-state');
...
const MARKERS = { begin: BEGIN_MARKER, end: END_MARKER };
...
/**
 * Containment policy for the three plan POINTERS this step resolves
 * (`step.planPath`, `activePlan`, `active_build_plan`).
 *
 * **On the root case, this deliberately does NOT differ from the default.** The
 * hand-rolled checks these three replaced counted the project root as inside,
 * and that was long read as an intentional difference — a pointer validating a
 * DIRECTORY, where the root is a legitimate answer. It is not: every one of the
 * three resolves a plan FILE, which it then reads, and the plans DIRECTORY is
 * resolved elsewhere with no containment check at all. A pointer resolving to
 * the root was therefore never valid; it just failed later and worse, on a read
 * of a directory. So the two predicates converge here rather than preserving a
 * difference neither caller needed.
 *
 * **`followSymlinks: false` is a real difference and stays.** Governance state
 * is routinely symlinked back to a primary checkout when work happens in a git
 * worktree, so `.prawduct/artifacts/build-plan.md` inside a worktree is a
 * symlink whose target is outside that worktree. Following it would refuse
 * every worktree session's plan pointer as an escape. The check stays lexical,
 * which is what it has always been; the cost is bounded because this pointer is
 * read and never written through, and `step.planPath` accepts absolute paths
 * outright — so it is a typo guard on server-authored template JSON, not a
 * boundary holding back untrusted input.
 *
 * @type {{followSymlinks: boolean}}
 */
const PLAN_POINTER_CONTAINMENT = { followSymlinks: false };
...
// A plan's issue field: `**Issue:** #N`, `**Issues:** #A, #B`, `- Tracking issue: **#N**`
// at the start of a line (after an optional list or quote marker). Only this
// line's `#N` references are the plan's own issues — the rest of a plan cites
// history and neighbouring work in prose, and counting those would keep a
// shipped plan alive on an unrelated open issue, or drop a live one because
// the history it cites has closed (#1516).
const ISSUE_FIELD_RE = /^\s*(?:[-*>]\s+)?(?:\*\*)?(?:tracking\s+)?issues?(?:\*\*)?\s*:/i;
// `Fixes #N` / `Closes #N` / `Resolves #N` anywhere: the keyword GitHub itself
// reads as "this work closes that issue".
const CLOSING_KEYWORD_RE = /\b(?:fix(?:es|ed)?|close[sd]?|resolve[sd]?)\s+#(\d+)\b/gi;
const ISSUE_REF_RE = /#(\d+)\b/g;
// A `## Issues` section (typically a chunk → issues table) is the same field at
// section scale: every `#N` from the heading to the next `##` is the plan's own.
const ISSUES_SECTION_RE = /^##\s+Issues\s*$/i;
...
/**
 * Replace, append, or repair the managed block inside `priorContent`.
 *
 * Delegates to the shared managed-block splice so this file and the engine
 * config layer cannot answer "what does a broken marker set mean" differently
 * — they did, and a malformed priming file grew by one stale block per wrap
 * (#1132). A misordered pair is now repaired into one well-formed block with
 * the text between the old markers kept below it, which is idempotent: the
 * next wrap replaces that block rather than adding another.
 *
 ...
 *   `merged` is the new full priming-file content; `error` is set instead when
 *   nothing may be written; `repaired` flags a rewrite outside the block.
 */
function _replaceManagedBlock(priorContent, body) {
  return managedBlock.spliceManagedBlock(priorContent, body, MARKERS, {
    bodySourceHint: 'a chunk heading in the plan carries a priming-roll marker'
  });
}
...
/**
 * The issue numbers a plan names as its own (#1516): every `#N` on an issue
 * field line (`**Issue:**`, `**Issues:**`, `Tracking issue:`) or inside a
 * `## Issues` section, plus every `Fixes|Closes|Resolves #N`. See `ISSUE_FIELD_RE` for why prose references
 * elsewhere in the plan are not counted.
 *
 * @param {string} planContent - Raw plan markdown
 * @returns {number[]} Unique numbers, in order of first appearance
 */
function _citedIssues(planContent) {
  if (!planContent) return [];
  const found = [];
  const add = (n) => { const v = Number(n); if (v > 0 && !found.includes(v)) found.push(v); };
  let inIssuesSection = false;
  for (const line of planContent.split(/\r?\n/)) {
    if (ISSUES_SECTION_RE.test(line)) { inIssuesSection = true; continue; }
    if (inIssuesSection && ANY_H2_RE.test(line)) inIssuesSection = false;
    if (inIssuesSection || ISSUE_FIELD_RE.test(line)) {
      for (const m of line.matchAll(ISSUE_REF_RE)) add(m[1]);
    }
    for (const m of line.matchAll(CLOSING_KEYWORD_RE)) add(m[1]);
  }
  return found;
}

/**
 * Narrow several in-progress plans by the state of the issues each cites
 * (#1516). A plan whose cited issues are ALL closed has shipped and was not
 * archived; it is dropped from the candidates and reported as an archive
 * candidate. Nothing is moved: relocating tracked files is a side effect a
 * wrap step must not have, and the operator decides what gets archived.
 *
 * A plan citing no issues is never "all closed" and stays. An issue GitHub does
 * not know (`unknown`) is not closed either, so its plan stays. When no
 * candidate cites an issue, gh is never called.
 *
 * @param {string} projectPath - Checkout gh reads the repository from
 * @param {Array<{file: string, abs: string, issues: number[]}>} candidates - In-progress plans
 * @returns {Promise<{remaining: Array<{file: string, abs: string, issues: number[]}>,
 *   archiveCandidates: Array<{file: string, issues: number[]}>,
 *   issueCheck: {state: ('checked'|'not-needed'|'unavailable'), reason: (string|null)}}>}
 */
async function _dropShippedPlans(projectPath, candidates) {
  const numbers = [...new Set(candidates.flatMap((c) => c.issues))];
  if (numbers.length === 0) {
    return { remaining: candidates, archiveCandidates: [], issueCheck: { state: 'not-needed', reason: null } };
  }
  const answer = await _internal.lookupIssueStates(projectPath, numbers);
  if (!answer || answer.available !== true) {
    const reason = (answer && answer.reason) || 'the issue lookup gave no answer';
    return { remaining: candidates, archiveCandidates: [], issueCheck: { state: 'unavailable', reason } };
  }
  const remaining = [];
  const archiveCandidates = [];
  for (const c of candidates) {
    const allClosed = c.issues.length > 0 && c.issues.every((n) => answer.states[n] === 'closed');
    if (allClosed) archiveCandidates.push({ file: c.file, issues: c.issues });
    else remaining.push(c);
  }
  return { remaining, archiveCandidates, issueCheck: { state: 'checked', reason: null } };
}

/**
 * One sentence naming the plans dropped as shipped, for the row and the log.
 * @param {Array<{file: string, issues: number[]}>} archiveCandidates - From `_dropShippedPlans`
 * @param {string} plansDirLabel - Project-relative plans directory
 * @returns {string|null} Null when nothing was dropped
 */
function _archiveNote(archiveCandidates, plansDirLabel) {
  if (!archiveCandidates || archiveCandidates.length === 0) return null;
  const list = archiveCandidates
    .map((a) => `${a.file} (${a.issues.map((n) => `#${n}`).join(', ')} closed)`)
    .join('; ');
  return `Archive candidate${archiveCandidates.length === 1 ? '' : 's'}: ${list} — every cited issue is closed, `
    + `so ${archiveCandidates.length === 1 ? 'it was' : 'they were'} not offered; move to ${plansDirLabel}/archive/ when you agree`;
}
 ...
 *      bare filename resolves under the resolved plans dir). A bare filename
 *      whose plan now sits in the plans dir's `archive/` is stale: resolution
 *      falls through to 3-7 and carries `staleActivePlan` (#1516).
 ...
 *
 *   **Where the shipped-plan filter applies (#1516).** `run` drops a candidate
 *   whose cited issues are all closed on the routes where THIS STEP chose the
 *   plan out of several: precedence 5, both when it blocks and when it
 *   auto-disambiguates to one. It does not apply to 1-3, where the operator or
 *   the governing framework named the plan and being second-guessed against
 *   GitHub would be a surprise. It also does not apply to 4, which is a gap
 *   rather than a decision: a lone `.md` plan whose work has shipped is rolled
 *   with no note, exactly the case this fix removes one precedence down.
 ...
 * @param {string} [configRoot=projectPath] - Where `.tangleclaw/project.json` (and
 *   its `activePlan`) lives: the registered checkout when the wrap works on a
 *   worktree, since that file is machine state no worktree carries.
 * @returns {{ok:true, planPath:string}
 *   | {ok:false, skip:true, reason:string}
 *   | {ok:false, error:string, remediation?:string}}
 */
function _resolvePlanPath(projectPath, step, configRoot = projectPath) {
  if (step.planPath) {
    ...
    if (!path.isAbsolute(step.planPath)) {
      const contained = projectPaths.checkContainment(projectPath, abs, PLAN_POINTER_CONTAINMENT);
      if (!contained.inside) {
        return {
          ok: false,
          planPath: null,
          error: `step.planPath "${step.planPath}" ${contained.reason}`
        };
      }
    }
  ...
  const activePlan = _readActivePlan(configRoot);
  if (activePlan) {
    ...
    if (!path.isAbsolute(activePlan)) {
      const contained = projectPaths.checkContainment(projectPath, abs, PLAN_POINTER_CONTAINMENT);
      if (!contained.inside) {
        return { ok: false, error: `activePlan "${activePlan}" (in .tangleclaw/project.json) ${contained.reason}` };
      }
    }
    if (!_internal.existsSync(abs)) {
      // A bare pointer whose plan has since been archived is stale, not a typo:
      // the multi-plan picker writes `activePlan`, and archiving the shipped plan
      // it named is exactly what the archive rule asks for. Blocking every later
      // wrap on that would punish following the rule, so resolution falls through
      // to the pointers and heuristics below, and the row says the pointer is
      // stale (#1516). `project.json` is only read here, never rewritten.
      if (!hasSep && !path.isAbsolute(activePlan)
        && _internal.existsSync(path.join(projectPath, plansRel, 'archive', activePlan))) {
        return { ..._resolveUnpinnedPlan(projectPath), staleActivePlan: activePlan };
      }
      return {
        ok: false,
  ...
  return _resolveUnpinnedPlan(projectPath);
}

/**
 * Plan resolution below the two explicit pointers (`step.planPath`,
 * `activePlan`): the governing framework's declared plan, then the plans
 * directory. Steps 3-7 of `_resolvePlanPath`'s precedence.
 *
 * A multi-plan block carries `candidates` (filenames, for the drawer's picker),
 * plus `candidatePlans` (with absolute paths and cited issues) and
 * `plansDirLabel`, which `run` uses to drop plans whose issues are all closed
 * before it blocks (#1516).
 *
 * @param {string} projectPath - Absolute project root
 * @returns {{ok:true, planPath:string}
 *   | {ok:false, skip:true, reason:string}
 *   | {ok:false, error:string, remediation?:string, candidates?:string[],
 *      candidatePlans?:Array<{file:string, abs:string, issues:number[]}>, plansDirLabel?:string}}
 */
function _resolveUnpinnedPlan(projectPath) {
  ...
  if (governedPlan) {
    ...
    if (!path.isAbsolute(governedPlan)) {
      const contained = projectPaths.checkContainment(projectPath, abs, PLAN_POINTER_CONTAINMENT);
      if (!contained.inside) {
        return { ok: false, error: `active_build_plan "${governedPlan}" (in ${PRAWDUCT_DIR}/project-state.yaml) ${contained.reason}` };
      }
    }
    if (_internal.existsSync(abs)) {
      return { ok: true, planPath: abs };
  ...
  if (entries.length === 1) {
    // KNOWN GAP (#1516): the only plan in the directory is rolled without the
    // shipped check, so a project whose single plan has shipped still rolls it.
    // One precedence down, the same plan beside a completed one IS dropped and
    // the step skips with an archive note — so this is not a reasoned exemption
    // like the operator's own pointer, it is the member the fix did not reach.
  ...
  for (const f of entries) {
    ...
    if (_isPlanInProgress(content)) inProgress.push({ file: f, abs, issues: _citedIssues(content) });
  }
  if (inProgress.length === 1) {
    // The one live candidate still carries its cited issues: a single plan whose
    // work has all shipped is the same stale-pointer bug the multi-plan filter
    // exists for (#1516), and it reached this return before the filter ran.
    return { ok: true, planPath: inProgress[0].abs, candidatePlans: inProgress, plansDirLabel };
  }
...
async function run(context) {
  ...
  let planRes = _resolvePlanPath(project.path, step, configRootOf(project));

  // Notes that ride on whatever the step reports (#1516): plans dropped as
  // shipped, a stale `activePlan`, or why the issue check could not run.
  const notes = { archiveCandidates: [], issueCheck: null, staleActivePlan: planRes.staleActivePlan || null };
  // Runs on every path that knows its candidates — the multi-plan block AND the
  // single live plan resolved without asking. An explicit pointer (`step.planPath`,
  // `activePlan`) carries none: the operator named that plan, so it is rolled as
  // named and never dropped underneath them.
  if (Array.isArray(planRes.candidatePlans) && planRes.candidatePlans.length > 0) {
    const narrowed = await _dropShippedPlans(project.path, planRes.candidatePlans);
    notes.archiveCandidates = narrowed.archiveCandidates;
    notes.issueCheck = narrowed.issueCheck;
    planRes = _afterShippedPlansDropped(planRes, narrowed);
    if (planRes.ok && Array.isArray(planRes.candidatePlans)) delete planRes.candidatePlans;
  }
  const noteFields = _noteFields(notes, planRes.plansDirLabel || DEFAULT_PLANS_DIR);
  if (noteFields.warning) {
    log.warn('priming-roll resolved around stale plan state', {
      project: project.name,
      archiveCandidates: notes.archiveCandidates.map((a) => a.file).join(', ') || null,
      staleActivePlan: notes.staleActivePlan
    });
  }
  ...
  if (!planRes.ok && planRes.skip) {
    ...
    const reason = [planRes.reason, noteFields.note].filter(Boolean).join('. ');
    return {
      ok: true,
      status: 'skipped',
      output: { reason, detail: reason, ...noteFields.fields },
      blockers: []
    };
  }
  if (!planRes.ok) {
    ...
    const blockers = [planRes.error];
    if (notes.issueCheck && notes.issueCheck.state === 'unavailable') {
      // The picker shows as it always did, and says why it could not narrow
      // the list itself — a silent picker would read as "these are all live".
      blockers.push(`Could not check whether these plans' issues are closed (${notes.issueCheck.reason}), so none was ruled out as shipped`);
    }
    return {
      ok: false,
      status: 'blocked',
  ...
  const { merged: newContent, error: mergeError, repaired } = _replaceManagedBlock(priorContent, body);
  if (mergeError) {
    // Blocked, not skipped: a skip reads as "nothing to roll here", and there
    // is something to do — either the plan carries a marker literal that would
    // make a later wrap read the operator's prose as our region and delete it,
    // or the priming file's markers do not say which region is ours.
    return {
      ok: false,
      status: 'blocked',
      output: {
        remediation:
          `Open \`${path.relative(project.path, primingPath) || primingPath}\` and leave exactly one `
          + `\`${BEGIN_MARKER}\` and one \`${END_MARKER}\`, in that order, around the rolled block — `
          + 'or none at all, and the next wrap appends a fresh one. If the marker came from the plan '
          + 'instead, remove it from the chunk heading; a marker inside the rolled body cannot be spliced.'
      },
      blockers: [`Priming roll refused to write ${primingPath}: ${mergeError}`]
    };
  }
  if (repaired) {
    // A repair moves bytes OUTSIDE the managed block, which an ordinary roll
    // never does, so it does not belong under `changed` — that is true of every
    // roll and would say nothing.
    log.warn('priming-roll repaired the managed block\'s markers', {
      project: project.name,
      primingPath,
      howToInvestigate: 'the BEGIN/END markers were out of order; they are now one block in the first '
        + 'marker\'s place, with the text that sat between them kept below it'
    });
  }
  ...
  return {
    ...
    output: {
      ...
      pointer: pointerSummary,
      changed,
      // Reaches the wrap drawer: a repair rewrote the operator's file outside
      // the block, and the row that reports the roll is where they will look.
      repaired,
...
/**
 * The resolution after shipped plans were dropped from a multi-plan block
 * (#1516): exactly one left → that plan, without asking; none left → skip,
 * since every in-progress plan had shipped; several left → the same block,
 * listing only the plans still live.
 *
 * @param {object} planRes - The multi-plan block from `_resolvePlanPath`
 * @param {{remaining: Array<{file:string, abs:string}>, archiveCandidates: Array}} narrowed - From `_dropShippedPlans`
 * @returns {object} A `_resolvePlanPath`-shaped result
 */
function _afterShippedPlansDropped(planRes, narrowed) {
  const label = planRes.plansDirLabel;
  if (narrowed.archiveCandidates.length === 0) return planRes;
  if (narrowed.remaining.length === 1) {
    return { ok: true, planPath: narrowed.remaining[0].abs, plansDirLabel: label };
  }
  if (narrowed.remaining.length === 0) {
    return {
      ok: false,
      skip: true,
      reason: `No live in-progress plan in ${label}: every in-progress plan cites only closed issues — nothing to roll`,
      plansDirLabel: label
    };
  }
  const files = narrowed.remaining.map((p) => p.file);
  return {
    ...planRes,
    error: `Multiple in-progress plans in ${label} (${files.join(', ')}) — cannot pick one automatically`,
    candidates: files,
    candidatePlans: narrowed.remaining
  };
}

/**
 * Output fields and a one-line note for the stale plan state this run worked
 * around (#1516). `warning: true` is the step contract's "ok, but look at
 * this" channel, so the drawer tints the row and the banner counts it.
 *
 * @param {{archiveCandidates: Array<{file:string, issues:number[]}>, issueCheck: object|null, staleActivePlan: string|null}} notes
 * @param {string} plansDirLabel - Project-relative plans directory
 * @returns {{fields: object, note: string|null, warning: boolean}}
 */
function _noteFields(notes, plansDirLabel) {
  const fields = {};
  const sentences = [];
  const archive = _archiveNote(notes.archiveCandidates, plansDirLabel);
  if (archive) {
    fields.archiveCandidates = notes.archiveCandidates;
    sentences.push(archive);
  }
  if (notes.staleActivePlan) {
    fields.staleActivePlan = notes.staleActivePlan;
    sentences.push(`activePlan "${notes.staleActivePlan}" in .tangleclaw/project.json names an archived plan and was ignored`);
  }
  if (notes.issueCheck) fields.issueCheck = notes.issueCheck;
  const warning = sentences.length > 0;
  if (warning) {
    fields.warning = true;
    fields.note = sentences.join('. ');
  }
  return { fields, note: warning ? fields.note : null, warning };
}

const _internal = {
  // Test seam for the GitHub issue lookup (#1516); tests never reach gh.
  lookupIssueStates: (cwd, numbers) => ghIssueState.lookup(cwd, numbers),
