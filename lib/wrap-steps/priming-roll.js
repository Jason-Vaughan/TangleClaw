 ...
 * is preserved byte-for-byte. If the markers don't exist yet, the
 * handler appends a fresh managed block; if they exist but are broken
 * (misordered, unclosed, duplicated) it repairs them into one
 * well-formed block, keeping whatever sat between them as user content
 * outside it — the shared policy in `lib/managed-block.js`, which the
 * engine-config layer splices through as well. The handler does NOT
 ...
 * the prose between the old markers kept outside it, which is idempotent: the
 ...
 * @returns {{merged: string|null, error: string|null}} `merged` is the new full
 *   priming-file content; `error` is set instead when nothing may be written.
 */
...
async function run(context) {
  ...
  const { merged: newContent, error: mergeError } = _replaceManagedBlock(priorContent, body);
  if (mergeError) {
    // Blocked, not skipped: a skip reads as "nothing to roll here", and there
    // is something to do — the plan carries a marker literal that would make a
    // later wrap read the operator's prose as our region and delete it.
    return {
      ok: false,
      status: 'blocked',
      output: null,
