 ...
 *      in-progress → block with `remediation`. `run` then drops plans whose
 *      cited issues are all closed before that block stands (#1516).
...
function _resolveUnpinnedPlan(projectPath) {
  ...
  if (entries.length === 1) {
    // The only plan in the directory is rolled as-is: there is nothing to
    // disambiguate, and dropping it as shipped would leave the project pointing
    // at no plan at all. The shipped-plan filter applies where a choice exists.
