...
async function run(context) {
  ...
  const methodology = context.methodology || governance.methodologyCapability(project.path, {
    engineId: (context.session && context.session.engineId) || project.engineId || null
  });
  ...
  if (methodology.disposition === 'not-applicable' || !_isDir(path.join(project.path, '.prawduct'))) {
    return {
      ok: true,
      ...
      output: {
        engineId: methodology.engineId,
        capability: methodology.capability,
        reason: 'no .prawduct/ directory — not a prawduct-governed project',
        governed: false
      },
      blockers: []
    };
