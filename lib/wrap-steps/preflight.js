 ...
 *
 * How: when the project is onboarded to Prawduct (`.prawduct/` or the
 * committed plugin reference; see `methodologyCapability`), run `prawduct-hook stop` and read
 ...
 * unmet, 0 when clear. A project that is not onboarded is answered without
 ...
 *
 * ENGINE-AWARE (#1738). The hook is a Claude-plugin program, so whether to
 * run it depends on the session's engine, not only on the disk. The run
 * resolves `context.methodology` once (`lib/governance-state.js`
 * `methodologyCapability`) and this step reads it:
 * - not onboarded → `not-applicable`: there is no methodology to measure;
 * - onboarded, but the engine cannot run the plugin → `capability-unavailable`,
 *   and the hook is NEVER spawned, because the probe writes Prawduct's
 *   evidence store and a dormant project's Prawduct state must stay untouched.
 *   The gates were not measured, and the row, the run's withheld authority and
 *   the handoff all say so. It never reads as passed.
 *
 ...
 * @param {object} [context.options] - Run options (reads `skipPreflight`)
 * @param {object} [context.staged] - Run scratch space (an override stages its commit-body line)
...
async function run(context) {
  ...
  // The run resolves this once for every step; a direct call (a test, a
  // caller outside the runner) is answered the same way from the step's own
  // project and session.
  const methodology = context.methodology || governance.methodologyCapability(project.path, {
    engineId: governance.sessionEngineId(context.session, project)
  });
  if (methodology.disposition === 'capability-unavailable') {
    log.info('prawduct methodology unavailable on this engine — preflight not run', { project: project.name, engineId: methodology.engineId });
    return {
      ok: true,
      ...
      output: {
        ...
        reason: `${methodology.reason} — prawduct gates not measured`,
        governed: true,
        measured: false
      },
      blockers: []
  ...
  // One definition of "onboarded": the resolver's. A project carrying only the
  // committed plugin reference (a fresh clone, before `.prawduct/` exists) is
  // onboarded, and on Claude its gates are measured like any other.
  if (methodology.disposition === 'not-applicable') {
    return {
      ok: true,
      ...
      output: {
        engineId: methodology.engineId,
        capability: methodology.capability,
        reason: 'not onboarded to Prawduct (no .prawduct/ directory and no committed plugin reference) — not a prawduct-governed project',
        governed: false
      },
      blockers: []
    };
  }
  // "Wrap anyway" from the drawer, after this step halted a run (#1229). Honoured
  // only where the resolved step allows it, like every operator override. The
  // probe is not re-run: the operator is overriding the verdict they just read,
  // and the commit body records that the gates were passed over, not cleared.
  const options = context.options || {};
  if (step && step.allowOverride === true && options.skipPreflight === true) {
    log.info('preflight skipped via operator override', { project: project.name });
    if (context.staged && step.id) context.staged[step.id] = { preflightOverride: true };
    return {
      ok: true,
      status: 'skipped',
      output: { override: true, measured: false, reason: 'operator chose to wrap anyway — prawduct gates not re-checked' },
      blockers: []
    };
  }
  ...
  if (exec.exitCode !== 0 && exec.exitCode !== BLOCKED_EXIT_CODE) {
    log.warn('prawduct-hook stop exited outside its 0/2 contract — treating as unmeasured', {
      project: project.name, hook: hook.path, exitCode: exec.exitCode,
      stderrTail: _tailChars(exec.stderr, BLOCK_TEXT_MAX_CHARS)
    });
    return _skipped(
      ...
      // NOT `detail`: the drawer's skipped branch renders `output.detail ||
      // output.reason`, so calling it that would show the operator a raw
      // Python traceback and swallow the sentence that explains what happened.
      { governed: true, hook: hook.path, exitCode: exec.exitCode, stderrTail: _tailChars(exec.stderr, BLOCK_TEXT_MAX_CHARS) }
    );
  }
