...
/**
 * How many lines from the BOTTOM of the pane capture the read-only marker is
 * looked for in.
 *
 * The marker is a footer string an engine's TUI draws under the input box, so
 * the bottom of the pane is where it lives — and searching only there is what
 * keeps the check from matching the same words appearing in the conversation
 * above it. That is not hypothetical: the session that BUILT this check had
 * `plan mode on (shift+tab to cycle)` in its own transcript while discussing
 * the marker, and a whole-pane search would have refused its own wrap. Six
 * lines covers the footer even when the input box has grown a line or two.
 */
const READ_ONLY_FOOTER_LINES = 6;
 ...
 *
 * @param {object} [project] - Project record (carries `engineId`)
 * @returns {{marker: string, label: string, exit: string}|null}
 */
function _readOnlyModeMarker(project) {
  ...
  try {
    ...
    if (!field || typeof field.marker !== 'string' || !field.marker.trim()) return null;
    return {
      marker: field.marker,
 ...
 *   - `read-only`  — the marker is on the footer. The caller refuses the step.
 *   - `clear`      — a marker is declared and is NOT on the footer. Measured.
 *   - `unmeasured` — a marker is declared and the pane could not be read.
...
function _readOnlyPrecheck(project, tmuxSession) {
  ...
  let capture;
  try {
    capture = _internal.capturePane(tmuxSession, { lines: READ_ONLY_FOOTER_LINES });
  } catch (err) {
    return {
      state: 'unmeasured',
  ...
  // line count and returns the whole visible pane, so the trailing slice is
  // applied HERE rather than trusted from the capture options.
  const footer = paneLines.slice(-READ_ONLY_FOOTER_LINES).join('\n');
  if (footer.includes(field.marker)) {
    return {
      state: 'read-only',
      reason: `the pane footer reads "${field.marker}"`,
      label: field.label,
      exit: field.exit
    };
  }
  return {
    state: 'clear',
    reason: `the pane footer does not show ${field.label}`,
    label: field.label,
    exit: field.exit
  };
}
...
async function run(context) {
  ...
  // #139 Chunk 11c — empty prompt = pipeline author's intentional
...
function _verifyChangedGate(projectPath, step, beforeSnapshot) {
  ...
  const satisfied = _satisfactionPredicateGate(projectPath, step, paths);
 ...
 */
function _satisfactionPredicateGate(projectPath, step, paths) {
  const name = step.verifySatisfiedBy;
  if (!name) return null;

  if (name !== 'changelog-coverage') {
  ...
  let result;
  try {
    result = _internal.changelogCoverage(projectPath, paths, step.coveragePaths);
  } catch (err) {
...
const _internal = {
  ...
  changelogCoverage: (projectPath, paths, coveragePaths) => changelogCoverage.evaluate(projectPath, paths, coveragePaths),
...
module.exports = { run, _internal, _interpolatePrompt, _readOnlyModeMarker, _readOnlyPrecheck, _readOnlyRemediation, _withPrecheck, _runTmuxCapture, READ_ONLY_FOOTER_LINES, _appendWrapRules, _wrapStepHeader, _parseFields, _normalizeFieldKey, _runGatewayCapture, _resolveEngineConfigFile, _snapshotPaths, _watchedOutputPaths, _verifyChangedGate, SUPPORTED_PROMPT_TOKENS, STABILITY_MS };
