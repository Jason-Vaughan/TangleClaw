...
async function _runTmuxCapture(context) {
  ...
  if (captureFields.length > 0) {
    ...
    const verifyBlock = _verifyChangedGate(project.path, step, beforeSnapshot, context.scope);
    if (verifyBlock) {
      return {
        ok: false,
        status: 'blocked',
 ...
 */
function _verifyChangedGate(projectPath, step, beforeSnapshot, scope = null) {
  ...
  const satisfied = _satisfactionPredicateGate(projectPath, step, paths, scope);
 ...
 */
function _satisfactionPredicateGate(projectPath, step, paths, scope = null) {
...
const _internal = {
  ...
  changelogCoverage: (projectPath, paths, coveragePaths, scope) => changelogCoverage.evaluate(projectPath, paths, coveragePaths, scope),
