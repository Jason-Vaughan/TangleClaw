...
async function run(context) {
  ...
  // #334 — WebUI/OpenClaw sessions have no tmux pane by design
  // (`sessionMode === 'webui'`, `tmuxSession === null`). This step's whole
  // mechanism is send-prompt → poll-idle → capture-pane, which a webui
  // session cannot drive. Skip honestly (`ok:true`) rather than block:
  // prawduct's content ai-content steps are `blocker:true`, so returning
  // `blocked` here would halt EVERY webui wrap before `commit`. AI-authored
  // content (memory/summary) is simply not captured on the webui path for
  // now — bridge-sourced capture is a future enhancement. Mirrors CC-1's
  // honest-empty floor (a flagged skip, never a fabricated capture).
  if (session.sessionMode === 'webui') {
    return {
      ok: true,
      status: 'skipped',
      output: { webui: true, reason: 'webui session has no tmux pane — ai-content capture is N/A' },
      blockers: []
    };
  }
