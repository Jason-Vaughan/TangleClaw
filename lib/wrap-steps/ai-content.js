...
async function _runTmuxCapture(context) {
  ...
  if (captureFields.length > 0 && step.captureFile) {
    try {
      ...
      return {
        ...
        status: 'blocked',
        output: { capturedText },
        blockers: [`captureFile "${step.captureFile}" was captured earlier in this run and consumed on read; it is not available for a replay of this step. This is a pipeline lifecycle issue, not a prompt or AI failure (${err.code || err.message})`]
      };
    }
    _internal.removeCaptureFile(project.path, step.captureFile);
  }
  ...
  if (captureFields.length > 0) {
    ...
    return {
      ok: true,
      status: 'done',
      output: { capturedText, parsedFields: sections, uncapturedOptional },
      blockers: []
    };
  }
...
async function _runGatewayCapture(context) {
  ...
  if (!fileRes.ok) {
    return {
      ...
      status: 'blocked',
      output: null,
      blockers: [`captureFile "${step.captureFile}" was captured earlier in this run and consumed on read; it is not available for a replay of this step. This is a pipeline lifecycle issue, not a prompt or AI failure (${fileRes.error})`]
    };
  }
  ...
  return {
    ok: true,
    status: 'done',
    output: { capturedText: parseSource, parsedFields: sections, uncapturedOptional },
    blockers: []
  };
}
