...
async function run(context) {
  ...
  const prompt = _appendWrapRules(_interpolatePrompt(step.prompt, previousResults || []), project);
...
const _internal = {
  ...
  // every non-empty ai-content prompt. Mockable so tests don't need a live DB.
  listWrapRules: (projectId) => store.sessionRules.list({ projectId, enabled: 1, kind: 'wrap' }),
...
module.exports = { run, _internal, _interpolatePrompt, _appendWrapRules, _parseFields, _normalizeFieldKey, _runGatewayCapture };
