 ...
 *
 * The message used to merge distinct facts into one that named the step prompt
 * as the cause — the one thing that was not wrong — which sent operators and
 * agent sessions after "the AI ignored its instructions". Each variant says only
 ...
 * @param {string} where - "after the AI went idle" | "over the gateway"
 * @returns {string}
 */
    ...
    try {
      ...
      return {
        ...
        blockers: [_captureReadBlocker(step.captureFile,
          err.code === 'ENOENT' ? 'not-written' : 'read-failure', err.code || err.message, 'after the AI went idle')]
      };
    }
    _internal.removeCaptureFile(project.path, step.captureFile);
  }
