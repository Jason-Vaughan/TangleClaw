...
async function _runTmuxCaptureInner(context, carry) {
  ...
  try {
    // The engine locates the composer and the session names the attempt, so a
    // draft cleared before the paste is kept rather than lost (#1507).
    _internal.sendKeys(tmuxSession, prompt, {
      enter: true,
      engineId: governance.sessionEngineId(session, project),
      attemptKey: session.id != null ? draftStore.sessionAttemptKey(session.id) : null
    });
  } catch (err) {
    return {
      ok: false,
...
function _captureAndValidate(context, wait) {
  ...
  if (captureFields.length > 0) {
    ...
    const verifyBlock = _verifyChangedGate(project.path, step, beforeSnapshot, context.scope, context.options, context.previousResults);
    if (verifyBlock) {
      return {
        ok: false,
        status: 'blocked',
 ...
 * @param {Array<object>|null} [previousResults] - Prior step results, handed to the
 *   predicate so it judges files against the upstream facts `session-files` found (#1868)
 * @returns {{blocker: string, remediation: string}|null} Null when the step is satisfied.
 */
function _verifyChangedGate(projectPath, step, beforeSnapshot, scope = null, options = null, previousResults = null) {
  ...
  const satisfied = _satisfactionPredicateGate(projectPath, step, paths, scope, options, previousResults);
 ...
 * @param {Array<object>|null} [previousResults] - Prior step results (#1868)
 * @returns {{ok: true}|{ok: false, block: {blocker: string, remediation: string}}|null}
 */
function _satisfactionPredicateGate(projectPath, step, paths, scope = null, options = null, previousResults = null) {
  ...
  return predicate(projectPath, step, paths, scope, options, previousResults);
}
 ...
 * @returns {{ok: true}|{ok: false, block: {blocker: string, remediation: string}}|null}
 */
function _changelogCoveragePredicate(projectPath, step, paths, scope, options, previousResults = null) {
  let result;
  try {
    result = _internal.changelogCoverage(projectPath, paths, step.coveragePaths, scope, {
      pathDecisions: options ? options.pathDecisions : undefined,
      pathDecisionBasis: options ? options.pathDecisionBasis : undefined,
      previousResults
    });
  } catch (err) {
