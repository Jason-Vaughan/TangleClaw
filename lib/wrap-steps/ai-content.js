...
function _satisfactionPredicateGate(projectPath, step, paths) {
  ...
  let result;
  try {
    result = _internal.changelogCoverage(projectPath, paths);
  } catch (err) {
  ...
  if (result.verdict === changelogCoverage.VERDICTS.UNCOVERED) {
    const names = (step.verifyChanged || []).join(', ');
    ...
    return {
      ...
      block: {
        ...
        // Note what actually clears this: the predicate asks what past commits
        // touched, and no edit made now can change that. What clears it is the
        // mutation route — writing the missing entry changes the file, which
        // satisfies the gate before the predicate is ever consulted on retry.
        remediation: `These commits shipped without a ${names} entry:\n  - ${listed}\nWrite the missing entries into ${names}, then Retry — the edit itself satisfies this step. (Retrying without editing will block again: this check reads what those commits touched, which cannot change now.) If that work genuinely warrants no entry, tick "Skip & note" to record that decision.`
      }
    };
  }
...
const _internal = {
  ...
  changelogCoverage: (projectPath, paths) => changelogCoverage.evaluate(projectPath, paths),
