...
async function run(context) {
  ...
  // #139 — empty prompt = pipeline author's intentional
  // skip marker, not a structural failure. This once returned
...
async function _runTmuxCapture(context) {
  ...
  // (#840).
  //
  // The consumer reads a well-known path with nothing binding the file to the run
  // that should have written it. On the 2026-08-01 wrap the file was already
  // there, carrying the PREVIOUS session's content — three well-formed blocks,
  // correct markdown, real issue numbers. Nothing about it looks wrong, so no
  // validation can catch it; the `## Summary` flows into the wrap commit subject,
  // which makes the blast radius the permanent record.
  //
  // The provenance is established mechanically rather than by asking the AI to
  // stamp a run id, because `wrap-direction.md` commitment 2 requires the
  // mechanical layer to produce identical results on every engine — a stamp only
  // some models write reliably is exactly the capability dependency it forbids.
  //
  // A delete that does not take is a HARD refusal, not a skip: proceeding on a
  // payload whose provenance cannot be established is the same defect one level
  // up, and this meets commitment 3's bright-line — the wrap would otherwise
  // report success while attributing another session's work to this one, which no
  // project preference should be allowed to choose.
  const armCaptureFields = Array.isArray(step.captureFields) ? step.captureFields : [];
...
module.exports = { run, _internal, _interpolatePrompt, _readOnlyModeMarker, _readOnlyPrecheck, _readOnlyRemediation, _withPrecheck, _runTmuxCapture, READ_ONLY_TAIL_LINES, _appendWrapRules, _wrapStepHeader, _parseFields, _normalizeFieldKey, _runGatewayCapture, _resolveEngineConfigFile, _snapshotPaths, _watchedOutputPaths, _verifyChangedGate, SUPPORTED_PROMPT_TOKENS, STABILITY_MS };
