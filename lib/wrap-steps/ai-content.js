...
async function run(context) {
  ...
  if (captureFields.length > 0) {
    ...
    if (verifyBlock && verifyBlock.skipped) {
      return {
        ok: true,
        status: 'skipped',
        output: { capturedText, parsedFields: sections, reason: verifyBlock.reason },
        blockers: []
      };
    }
    if (verifyBlock) {
  ...
  if (verifyBlock && verifyBlock.skipped) {
    return {
      ok: true,
      status: 'skipped',
      output: { capturedText, parsedFields: null, reason: verifyBlock.reason },
      blockers: []
    };
  }
  if (verifyBlock) {
 ...
 * A step may explicitly declare `allowUnchanged: true` when an unchanged file
 * is a valid successful outcome (for example, learnings-capture finding no
 * novel learning worth recording). That contract is consulted only here,
 * after prompt execution and response validation have succeeded, so genuine
 * execution failures remain blocking.
 *
 * @returns {{blocker: string, remediation: string}|{skipped: true, reason: string}|null}
 *   Null when the step is satisfied, or a terminal unchanged outcome.
 */
function _verifyChangedGate(projectPath, step, beforeSnapshot) {
  ...
  if (step.allowUnchanged === true) {
    log.info('ai-content step succeeded without a justified file change', {
      stepId: step.id, verifyChanged: paths
    });
    return {
      skipped: true,
      reason: `${step.id}: completed successfully with no justified change to ${names}`
    };
  }
