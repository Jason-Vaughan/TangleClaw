 ...
 *   - `{overrideBlockingFindings:true, reason, findingCount}` (critic-check, #264)
 *       → "Critic-override: <reason> (<N> blocking finding(s) ignored)".
 *         Audit-trail line for operator overrides of the Critic-halt gate;
 *         must appear in the commit footer so git log / gh pr view make
 *         the override visible to future reviewers without spelunking the
 *         wrap drawer state.
 *   - `{branch, sessionScoped, resolutions}` (pr-check)
 *       → "Open session-scoped PRs: N" + per-PR resolution lines
 *   - `{mapRefresh:true, addedDirs, removedDirs}` (project-map, PIDX slice 3)
 *       → "Project Map: refreshed (+A/-R dir(s))", or
...
function _buildBodyLines(staged) {
  ...
  for (const [stepId, entry] of Object.entries(staged)) {
    ...
    // critic-check blocking override (#264 / ADR 0002 amendment)
    if (entry.overrideBlockingFindings === true
        && typeof entry.findingCount === 'number') {
      const reasonText = entry.reason ? `: ${entry.reason}` : ' (no rationale provided)';
      lines.push(`- Critic-override${reasonText} (${entry.findingCount} blocking finding(s) ignored)`);
      continue;
    }

    // features-toc (#207 Chunk 3) — duck-typed on the trio
    // `{addedCount, addedFiles, todoDate}`. Requires `todoDate` (a
    // string only this stage produces) in addition to the count + list
    // so a future stage shape that happens to carry `addedCount +
    // addedFiles` cannot accidentally trigger this body line.
    if (typeof entry.addedCount === 'number'
        && Array.isArray(entry.addedFiles)
        && typeof entry.todoDate === 'string') {
      if (entry.addedCount > 0) {
        ...
        const suffix = entry.addedFiles.length > 3
          ...
          : '';
        lines.push(`- Feature Index: ${entry.addedCount} stub(s) appended (${filesPreview}${suffix})`);
      }
      continue;
    }

    // project-map (PIDX slice 3) — duck-typed on `mapRefresh:true`. The
    // refreshed file is flushed generically via the `{primingPath,
    // newContent, changed}` trio; this just surfaces the audit line.
