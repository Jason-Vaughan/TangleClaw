...
async function run(context) {
  ...
  if (classified.stageable.length === 0) {
  ...
  let commitRes;
  try {
    const addRes = await _internal.exec('git', ['add', '-A', ...pathspecArgs], { cwd });
    ...
    commitRes = await _internal.exec('git', ['commit', '-m', message, ...pathspecArgs], { cwd });
  } finally {
