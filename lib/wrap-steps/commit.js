...
async function run(context) {
  ...
  if (autoBranched) {
    ...
    // A branch on the remote with no PR is the stranded-wrap condition: the
    // commit is safe, but nothing will ever land it, and the only cue — HEAD
    // left on the wrap branch — is erased by any later checkout on a shared
    // clone. It reads as success unless it is called out, so it warns like an
    // error does. This covers the `gh`-unavailable skip (which carries no
    // `error` and previously logged at info, indistinguishable from a PR that
    // actually opened) and a created PR whose URL could not be parsed.
    // The pre-push refusals — opt-out, no origin — push nothing and stay info.
    const stranded = autoPr.pushed && !autoPr.prUrl;
