 ...
 *   autoMergeArmed:boolean, returnedToBranch:boolean, remote:string|null,
 ...
 */
async function _autoPrCloseLoop({ cwd, branch, originalBranch, staged, configRoot = cwd }) {
  const result = {
    attempted: false,
    pushed: false,
    prUrl: null,
...
async function run(context) {
  ...
  if (autoBranched) {
    try {
      autoPr = await _autoPrCloseLoop({ cwd, branch, originalBranch, staged, configRoot: configRootOf(project) });
    } catch (err) {
      autoPr = {
        attempted: true,
        pushed: false,
