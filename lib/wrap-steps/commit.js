 ...
 * instead of a `HEAD~10..HEAD` guess. The value it REPLACES is reported
 * as `output.previousWrapSha` on every outcome, because the steps that
 * run after this one need the boundary the previous wrap left and can no
 * longer read it from disk (#797).
 *
 ...
 *
 * `projectConfig.load` answers with the documented defaults for an unreadable or
 * malformed config rather than throwing, so a null here means "no boundary
 * recorded" on every reachable path. The catch below is a belt on that contract
 * belonging to another module, not a case this one expects: a throw must not take
 * down a step the pipeline treats as a blocker.
 *
 * @param {string} projectPath
 * @returns {string|null} The recorded base, or null when none is recorded.
 */
function _readLastWrapSha(projectPath) {
  try {
    const cfg = store.projectConfig.load(projectPath);
    return cfg && cfg.lastWrapSha ? String(cfg.lastWrapSha) : null;
  } catch (err) {
    log.warn('Failed to read lastWrapSha from projConfig', { projectPath, error: err.message });
    return null;
  }
}
...
async function run(context) {
  ...
  //    replace it, and report it in every outcome below. `continuity-write` runs
  //    after this step and needs the PREVIOUS boundary to say what the session
  //    changed; once step 8 stamps, it is gone from disk (#797).
  const previousWrapSha = _readLastWrapSha(cwd);
  ...
  return {
    ...
    output: {
      ...
      // can measure the session's own range after the stamp has moved. Null when
      // none was recorded (the project's first wrap) or the config was unreadable.
      previousWrapSha,
