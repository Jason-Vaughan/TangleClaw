...
async function run(context) {
  ...
  // A rename whose halves got different answers is refused before anything is
  // staged. Committing one half is not a smaller version of the change: publish
  // only the destination and the source stays tracked, so the file exists twice
  // and the commit reads as an add; publish only the source and it reads as a
  // deletion of work that was merely moved. Both look like a successful wrap,
  // which is why this is a blocker and not a warning (#1629).
  if (classified.splitRenames && classified.splitRenames.length > 0) {
    return {
      ok: false,
      status: 'blocked',
      output: {
        remediation: 'Nothing was staged or committed. Decide the same way for both halves of each '
          + 'rename — include both to commit the move, or leave both to keep it out of this wrap — '
          + 'then re-run.'
      },
      blockers: classified.splitRenames.map((r) => `a rename would be half-committed: `
        + `${r.staged.join(', ')} would go in but ${r.left.join(', ')} would be left out, which turns `
        + 'the move into an add or a delete rather than a rename')
    };
  }
  ...
  try {
    ...
    commitRes = untrackPaths.length > 0
      ? await _commitWithUntrack({
        ...
        untrackPaths,
        indexRemoved: classified.stageable.filter((p) => (classified.indexRemoved || []).includes(p))
      })
