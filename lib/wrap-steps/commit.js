...
function _releaseCutOf(staged) {
  ...
  for (const entry of Object.values(staged)) {
    if (entry && typeof entry.oldVersion === 'string' && typeof entry.newVersion === 'string') {
...
/**
 * The project's `releasePrepareCommand`, or null when none is usable.
 *
 * A value that is present but not a non-empty string is logged and treated as
 * unset. It is not a reason to stop a wrap: without the command the release PR
 * is exactly what it was before this setting existed, and the step output says
...
async function run(context) {
  ...
  const releasePrepareCommand = releaseCut ? _releasePrepareCommandOf(project) : null;
  const preFlush = releaseCut && releasePrepareCommand ? _snapshotStagedTargets(staged) : null;
  ...
  if (releaseCut && !releasePrepareCommand) {
    releasePrepare = { status: 'skipped', reason: 'no releasePrepareCommand configured' };
  } else if (releaseCut) {
  ...
  let message = _buildMessage(staged, branch);
  const committedCompanions = releasePrepare && releasePrepare.status === 'done'
    ? releasePrepare.paths.filter((p) => classified.stageable.includes(p))
    : [];
  if (committedCompanions.length > 0) {
    message += `${message.includes('\n\n') ? '\n' : '\n\n'}- Release files updated by releasePrepareCommand: ${committedCompanions.join(', ')}`;
  }
