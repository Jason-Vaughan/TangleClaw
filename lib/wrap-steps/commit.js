 ...
 *   - `{branch, sessionScoped, resolutions, applied}` (pr-check)
 *       → "Open session-scoped PRs: N" + per-PR resolution lines, with
 *         "(auto-merge enqueued)" on the merges that were actually applied
    ...
    if (Array.isArray(entry.sessionScoped)) {
      ...
      if (entry.resolutions && typeof entry.resolutions === 'object') {
        ...
        for (const [prNum, action] of Object.entries(entry.resolutions)) {
          // `merge` means the wrap enqueued GitHub auto-merge, not that the
          // PR is merged — the commit body must not overstate what happened.
          const suffix = action === 'merge' && applied[prNum] && applied[prNum].ok === true
            ? ' (auto-merge enqueued)'
            : '';
          lines.push(`  - PR #${prNum}: ${action}${suffix}`);
        }
      }
    }
  }
