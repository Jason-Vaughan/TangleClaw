...
const { redactRemoteOutput, reasonFromFailure } = require('./_remote-output');
...
async function _autoPrCloseLoop({ cwd, branch, originalBranch, staged }) {
  ...
  if (pushRes.exitCode !== 0) {
    const detail = reasonFromFailure(pushRes);
  ...
  if (createRes.exitCode !== 0) {
    const detail = reasonFromFailure(createRes);
  ...
  if (mergeRes.exitCode !== 0) {
    const detail = reasonFromFailure(mergeRes);
