...
const { execFileArgs, execShell } = require('./_exec-shell');
 ...
 * @param {string[]} args.pathspecArgs - The allowed paths, as pathspec-file args.
 * @param {boolean} args.hasStageable - Whether there are allowed paths at all.
 ...
 */
async function _commitWithUntrack({ cwd, message, pathspecArgs, hasStageable, untrackPaths }) {
  ...
  try {
    const steps = [
      ['read-tree', 'HEAD'],
      ...(hasStageable ? [['add', '-A', ...pathspecArgs]] : []),
...
async function run(context) {
  ...
  let commitRes;
  try {
    // An empty pathspec file would make `git add -A` stage the whole tree, so an
    // untrack-only commit skips the add rather than handing git no paths.
    const addRes = classified.stageable.length > 0
      ? await _internal.exec('git', ['add', '-A', ...pathspecArgs], { cwd })
    ...
    commitRes = untrackPaths.length > 0
      ? await _commitWithUntrack({ cwd, message, pathspecArgs, hasStageable: classified.stageable.length > 0, untrackPaths })
