...
const { redactRemoteOutput, detailFromFailure } = require('../remote-output');
...
async function _autoPrCloseLoop({ cwd, branch, originalBranch, staged, configRoot = cwd }) {
  ...
  result.remote = strandedWraps.sanitizeRemote(remoteRes.stdout);
  ...
  if (autoBranched) {
    ...
    store.activity.log({
      ...
      detail: {
        ...
        // this row wants — it is served over `GET /api/activity` and the table
        // has no retention policy, so it must not depend on every current and
        // future producer having remembered.
        error: redactRemoteOutput(autoPr.error)
      }
    });
