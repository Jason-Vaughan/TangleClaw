 ...
 * (`https://user:token@host/...`), so the credential is stripped structurally
 * first; pattern-matching alone would miss it, since a bare `user:password@`
 * matches none of `secret-scan`'s patterns.
 *
...
function _truncateForRecord(text) {
  ...
  // `//user:pass@` → `//***:***@`. Userinfo cannot contain `/`, whitespace or a
  // second `@`, which keeps this from eating the rest of the line.
  const deurled = text.replace(/\/\/[^/\s:@]+:[^/\s@]*@/g, '//***:***@');
