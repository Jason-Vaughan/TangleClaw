 ...
 */
/**
 * Make a raw subprocess error safe to persist and serve.
 *
 * `activity_log` rows are read back over `GET /api/activity`, and this text is
 * whatever `git` or `gh` wrote to stderr — which for a failed push routinely
 * echoes the remote URL. A URL can carry an embedded credential
 * (`https://user:token@host/...`, or the password-less `https://token@host/...`
 * that GitHub's PAT instructions produce), so the credential is stripped
 * structurally first. Pattern-matching alone is not enough: a bare
 * `user:password@` matches none of `secret-scan`'s patterns, and its GitHub
 * rule covers only `ghp_`/`github_pat_` — not `gho_`/`ghs_`/`ghu_`/`ghr_`, and
 * nothing for GitLab, Bitbucket or self-hosted forges.
 *
 * Anything still matching a known secret pattern after that is replaced
 * wholesale rather than trimmed — a truncated secret is still a secret, and
 * `scanText` deliberately returns pattern *types*, never values, so naming the
 * types keeps the row diagnostic without persisting what it found.
 *
 * The length cap mirrors the one `pr-merge.js` `_ensurePushed` applies to the
 * same class of text. It is applied last: it is a size bound, not a redaction.
 *
 * @param {string|null} text - Raw `git`/`gh` stderr, or null
 * @returns {string|null} Redacted, bounded text, or null
 */
function _truncateForRecord(text) {
  if (typeof text !== 'string') return null;
  // Any `//<userinfo>@` → `//***@`, whether or not the userinfo carries a
  // colon. The password-less `https://<token>@github.com/…` form is what
  // GitHub's own PAT-over-HTTPS instructions produce, so requiring a colon
  // would miss the most likely credential of all.
  //
  // It cannot run past the authority: a URL with no credential has no `@`
  // before its first `/`, and userinfo cannot contain `/`, whitespace or a
  // second `@`, so the rest of the line is never eaten.
  //
  // It DOES erase a non-secret userinfo too — `ssh://git@host` becomes
  // `//***@host`. That is the intended trade, not an oversight: the host and
  // the error text carry the diagnostic value, and a username is not worth
  // narrowing the pattern until it misses a token again.
  ...
  return deurled.length > 200 ? `${deurled.slice(0, 200)}…` : deurled;
}
...
async function run(context) {
  ...
  const boundary = _readLastWrapSha(cwd);
  ...
  if (autoBranched) {
    ...
    store.activity.log({
      ...
      detail: {
        ...
        stranded,
        skippedReason: autoPr.skippedReason,
        // Raw `git`/`gh` stderr, bounded the same way `pr-merge.js`
        // `_ensurePushed` bounds it. A row is diagnostic, not an archive, and
        // this table has no retention policy — an unbounded remote error would
        // grow the database with text nobody reads past the first line.
        error: _truncateForRecord(autoPr.error)
      }
    });
  }
