 ...
 * that must not be collapsed: no boundary has ever been recorded, and a config
 * that could not be read. `projectConfig.load` returns its documented defaults
 * for an unreadable or malformed file rather than throwing, so the value alone
 * cannot tell them apart — and the consumer ACTS on the difference: on `absent`
 * it may publish the branch range as the session's work, on `unreadable` it must
 * not, because a boundary is probably sitting on disk unread. That is
 * `architecture.md`'s rule applied at the producer: a read that could not be
 * established names itself rather than passing for a plausible default.
 *
 * @param {string} projectPath
...
function _readLastWrapSha(projectPath) {
  ...
  let cfg;
  try {
    cfg = store.projectConfig.load(projectPath, {
      onError: (err, configPath) => {
        unreadable = true;
        log.warn('project config could not be read; the previous wrap boundary is unknown', {
          projectPath, configPath, error: err.message
        });
      }
    });
  } catch (err) {
    // `load` is documented not to throw; this is a belt on another module's
    // contract, and it lands in the same `unreadable` state for the same reason.
    log.warn('project config read threw; the previous wrap boundary is unknown', {
