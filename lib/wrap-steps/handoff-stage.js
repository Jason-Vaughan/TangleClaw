...
function _worktreeFacts(scope, anchorSha) {
  ...
  // ONE reading, for all three tree facts. `dirty` was measured here already and
  // `branch`/`headSha` were not, which is #1648: `branch` came from the scope's
  // trunk, probed before the commit step ran, and `headSha` fell back to the sha
  // the session LAUNCHED at. The preflight reads all three as one snapshot of
  // one tree at one moment, so the document could claim `main` at a commit that
  // had never been on main — and did, on this install, which is how the defect
  // was found. A launch then computed `movedHead` and refused READY on a sound
  // handoff.
  //
  ...
  return {
    ...
    // Both from the single reading above. NOT `scope.trunk.branch` (probed at
    // wrap-scope resolution, before the commit step) and NOT
    // `scope.baseline.sha` (the sha this session launched at) — those are two
    // other moments, and mixing them is the whole of #1648.
    //
    // `anchorSha` is still preferred for the sha when the commit step produced
    // one, because that commit IS this wrap's head and is the more precise
    // answer to "what did this wrap leave behind". It agrees with the measured
    // head by construction; the measurement is what covers the ordinary clean
    // tree, where the commit step returns null.
    branch,
    headSha: anchorSha || headSha,
    dirty
  };
}
...
/**
 * The rule manifest the handoff carries.
 *
 * Reuses `launchSequence.ruleFingerprints` rather than deriving a second
 * fingerprint: car 21.10 diffs the handoff's manifest against a launch's, and
 * two derivations that drift would report drift that is not there.
 * @param {object} project - Project record
 * @returns {object[]} One fingerprint per project rule
 */
function _ruleManifest(project) {
  try {
    // The same reader the launch snapshot uses, so the two manifests car 21.10
    // diffs cannot disagree about what the rule set was.
    return launchSequence.ruleFingerprints(store.sessionRules.listActiveForProject(project.id));
  } catch (err) {
    log.warn('Could not read the project rules for the handoff manifest', {
