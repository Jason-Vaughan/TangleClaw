 ...
 *
 * A readable tree returns null because the facts themselves are the answer, and
 * a tree with a problem AND a toplevel returns null for the same reason: the
 * problem is only load-bearing where it is the reason a fact is missing.
 *
 * @param {object|null} scope - `lib/wrap-scope.js` scope for this run
 * @returns {string|null} The probe failure, or null
 */
function _worktreeProblem(scope) {
  if (!scope || scope.workToplevel) return null;
  const problem = scope.workTreeProblem;
  return typeof problem === 'string' && problem.trim() ? problem : null;
}

module.exports = {
  run, _missingEvidence, _nextAction, _worktreeFacts, _worktreeProblem, _ruleManifest, _workspaceId
};
