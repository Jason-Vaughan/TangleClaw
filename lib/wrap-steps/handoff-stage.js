...
/**
 * Whether a result is a skip that measured nothing it was required to measure.
 *
 * Two ways in. A step marks it with `output.measured === false` (preflight does,
 * for a hook that could not be found, was killed, broke its contract, or was
 * passed over by the operator). And a `preflight` of a project whose engine CAN
 * run the methodology is a required gate, so any skip of it is unmeasured —
 * including one disabled through `wrapStepOverrides`, whose row says only that
 * it was disabled. An operator may let the wrap checkpoint past a missing
 * measurement; nothing turns that absence into evidence (#1738).
 *
 * @param {object} r - A step result
 * @param {object|null} [methodology] - The run's resolved capability
 * @returns {boolean}
 */
function _unmeasuredSkip(r, methodology) {
  if (r.status !== 'skipped') return false;
  if (r.output && r.output.measured === false) return true;
  return r.kind === 'preflight' && Boolean(methodology) && methodology.disposition === 'available';
}
 ...
 * @param {object|null} [methodology] - The run's resolved capability (#1738)
 * @returns {string[]} `"<stepId>: <status>"` for each step that fell short
 */
function _missingEvidence(previousResults, methodology) {
  return (previousResults || [])
    .filter((r) => r && (!EVIDENCE_PRODUCED.includes(r.status) || _unmeasuredSkip(r, methodology)))
    .map((r) => `${r.stepId || 'step'}: ${_unmeasuredSkip(r, methodology) ? 'skipped (not measured)' : (r.status || 'no status')}`);
}
...
/**
 * The resume text this wrap wrote into the continuity index (#1675), or null
 * when it wrote none — plus, when the step ran and did not write, the evidence
 * that is missing because of it.
 *
 * Taken from the `continuity-write` step's own output rather than read back
 * from the index file: the file is shared and rewritten by every run that
 * reaches the step, so reading it here could freeze another run's words into
 * this publication.
 *
 * A step that ran and did not write — its write threw, or it was skipped —
 * leaves the index describing an older wrap. That is missing evidence, whatever
 * status the step reported: it returns `done` on a failed write so as not to
 * halt the wrap, and a handoff counting that as produced would be `complete`
 * over a resume nobody wrote.
 *
 * @param {object[]} previousResults - Prior step results
 * @returns {{resume: (object|null), missing: (string|null)}}
 */
function _resumeRecord(previousResults) {
  const row = (previousResults || []).find((r) => r && r.kind === 'continuity-write');
  if (!row) return { resume: null, missing: null };
  const out = row.output || {};
  if (row.status !== 'done' || out.written !== true) {
    return { resume: null, missing: 'continuity-write: index not written' };
  }
  // Passed through as the step reported it. The block's shape has one
  // definition, `handoff-publication#normalizeResume`, which the document build
  // applies — a second, more forgiving copy here would turn a malformed value
  // into a quiet null that the one real check never sees.
  return {
    resume: { currentState: out.currentState, nextAction: out.nextAction, freshness: out.freshness },
    missing: null
  };
}
...
function _engineConfigHash(project, session) {
  try {
    const engineId = governance.sessionEngineId(session, project);
...
async function run(context) {
  ...
  const missingEvidence = _missingEvidence(previousResults, methodology);
  const resumeRecord = _resumeRecord(previousResults);
  if (resumeRecord.missing && !missingEvidence.some((m) => m.startsWith('continuity-write:'))) {
    missingEvidence.push(resumeRecord.missing);
  }
  ...
  try {
    ...
    const doc = buildHandoffDocument({
      ...
      wrapRunId,
      engineId: governance.sessionEngineId(session, project) || 'unknown',
      kind,
      ...
      // One canonical next action (#1675): the one the continuity step wrote
      // into the resume, when it wrote one — the document refuses two that
      // disagree. Otherwise the capture's, as before.
      nextAction: resumeRecord.resume ? resumeRecord.resume.nextAction : _nextAction(previousResults),
...
module.exports = {
  run, _missingEvidence, _methodologyRecord, _nextAction, _resumeRecord, _worktreeFacts, _worktreeProblem, _ruleManifest, _workspaceId
};
