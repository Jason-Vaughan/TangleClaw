...
function _worktreeFacts(scope, anchorSha) {
  ...
  return {
    ...
    gitDir: scope.worktreeTarget || null,
    branch: (scope.trunk && scope.trunk.branch) || null,
    headSha: anchorSha || (scope.baseline && scope.baseline.sha) || null,
    dirty: Boolean(scope.baseline && scope.baseline.dirty && (scope.baseline.dirty.paths || []).length > 0)
  };
}
...
module.exports = { run, _missingEvidence, _nextAction, _worktreeFacts, _commitSha, _ruleManifest };
