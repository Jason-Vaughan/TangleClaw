 ...
 *   is the range since the recorded boundary, `branch` is the trunk fallback,
 *   `null` is no range at all.
 */
 ...
 *
 * @param {object} args
 * @param {'session'|'branch'|null} args.kind - Which range answered (`_sessionDelta`).
 ...
 */
function _stampDecision({ kind, commitStepReported, boundaryRead }) {
  ...
  if (kind === 'branch') {
    return { publish: false, why: 'the recorded boundary is not an ancestor of this wrap commit, so the only range that resolved is the whole branch' };
  }
  return { publish: false, why: 'no git range resolved at all — this is not a repository, or it has no trunk branch' };
}
...
async function run(context) {
  ...
  const boundaryRead = commitOut && typeof commitOut.previousWrapShaRead === 'string'
    ...
    : null;
  // A commit step that ran and committed nothing is not a degraded read: the
  // session changed nothing, and the empty set is the honest record. Without
  // this the range would run from the PREVIOUS wrap's base to HEAD and publish
  // that wrap's own commit as this session's work — #797 through a side door.
  const committedNothing = Boolean(commitOut) && commitOut.commitSha === null;
  ...
  try {
    ...
    const decision = committedNothing
      ? { publish: true, why: null }
      : _stampDecision({ kind: delta.kind, commitStepReported: Boolean(commitOut), boundaryRead });
    if (decision.publish) {
      // `committedNothing` publishes the empty set, which is the whole point of
      // taking that branch: nothing landed, so nothing changed.
      touchedFiles = committedNothing ? [] : delta.touched;
    } else {
      log.warn('this wrap records no files: stamp — the session\'s range could not be established', {
        ...
        remediation: 'the next wrap re-stamps the boundary, so the stamp returns on its own; if it '
          + 'does not, check that .tangleclaw/project.json loads and that the recorded lastWrapSha '
          + 'still exists in this clone'
      });
    }
  } catch (err) {
