...
async function run(context) {
  ...
  // CC-8 (#386): the methodology-default layer. When the project has no per-project
  // override (above), fall back to the methodology's declared `wrap_contract.sections`
  // (template.json). Absent ⇒ wrapSections stays null ⇒ all 8 (the deep default).
  // Precedence: per-project wrapSections (CC-6) > methodology wrap_contract (CC-8) >
  // all 8. Best-effort like the per-project read — an unresolvable template never halts.
  if (wrapSections == null && project.methodology) {
    try {
      const tmpl = _internal.getMethodologyTemplate(project.methodology);
      if (tmpl && tmpl.wrap_contract && Array.isArray(tmpl.wrap_contract.sections)) {
        wrapSections = tmpl.wrap_contract.sections;
      }
    } catch (err) {
      log.debug('methodology wrap_contract read skipped', { project: project.name, error: err.message });
    }
  }
...
const _internal = {
  ...
  // CC-8 (#386): indirection so tests can inject a fixture methodology template
  // without registering one — mirrors the exec/today injection pattern above.
  getMethodologyTemplate: (id) => methodologies.getTemplate(id)
};

module.exports = { run, _internal, _resolveCapturedFields, _resolveBase, _mapDelta, _branchType, _deriveTier, _deriveUncapturedReason };
