 ...
 * present.
 *
 * `delta` / `openThreads` / `decisions` / `pointers` feed the four sections
 * that used to be permanently honest-flagged because nothing upstream asked
 * the AI for them (#1379). They arrive from `memory-update`'s
 * `optionalCaptureFields`, so any of them can legitimately be absent on a wrap
 * where the AI omitted the block — each resolves to `''` and
 * `continuity.renderWrapSummary` flags that section rather than the wrap
 * failing. Absent is a normal outcome here, not a degraded one.
 *
 * @param {Array} previousResults - Runner's prior-step results
 * @returns {{currentState:string, nextAction:string, learnings:string,
 *   delta:string, openThreads:string, decisions:string, pointers:string}}
 */
 ...
 * @param {string|null} [options.launchSha] - The session's launch baseline sha;
 *   the range opens there unless the recorded boundary is later.
 * @param {{onTrunk:boolean, trunkRefs:string[]}} [options.trunk] - The run's trunk
 *   position, for classifying merges on the first-parent line.
 * @returns {Promise<{touched:string[], deleted:string[],
 *   kind:('session'|'branch'|'diff-failed'|null), stopped:string[]}>} `kind` says
 *   which range answered: `session` is the range since this session's own start
 *   (its launch baseline or a wrap inside it) or since the recorded boundary,
...
async function _sessionDelta(cwd, options = {}) {
  const { anchor = null, previousWrapSha = null, launchSha = null, trunk = undefined } = options;
  const stopped = [];
  ...
  // `launch` is a session range too: it opens at this session's own start.
  const kind = resolved.kind === 'launch' ? 'session' : resolved.kind;

  // A merge on the first-parent line would put whatever it brought in into the
  // diff; on a branch that pulled the trunk, that is other sessions' files
  // (#1309). Walk instead, with trunk syncs left out.
  if (await gitRange.hasFirstParentMergesAsync(cwd, resolved.range, _internal.exec)) {
    const commits = await gitRange.listSessionCommitsAsync(cwd, resolved.range, {
      exec: _internal.exec,
      relative: false,
      trunk,
      onStopped: (command) => stopped.push(command)
    ...
    if (!commits) {
      log.warn('the session walk did not answer; this wrap records no files: stamp', { cwd, range: resolved.range });
      return { touched: [], deleted: [], kind: 'diff-failed', stopped };
    }
    const net = gitRange.netChanges(commits);
    return { touched: net.touched, deleted: net.deleted, kind, stopped };
  }
  ...
  return { touched, deleted, kind, stopped };
}
...
/**
 * The wrap summary's `Freshness` line for this wrap's release decision (#1492).
 *
 * Found by shape, like the resolvers above: the version-bump output that reached
 * the release gate carries `releaseMode` and `readiness`. A version-bump that
 * skipped before the gate (releaseMode off, nothing to release, a version it
 * can't bump) decided nothing, so there is no line.
 *
 * The line lives in `Freshness` rather than `Decisions` because `Decisions` is
 * the AI's judgment section: a mechanical line there would hide the flag that
 * says the AI wrote none.
 *
 * @param {Array} previousResults - Runner's prior-step results
 * @returns {string|null} One `- release: …` bullet, or null
 */
function _releaseFreshnessLine(previousResults) {
  if (!Array.isArray(previousResults)) return null;
  const result = previousResults.find((r) => r && r.output
    && typeof r.output.releaseMode === 'string' && r.output.readiness && typeof r.output.readiness === 'object');
  if (!result) return null;
  const out = result.output;
  const decider = out.decidedBy === 'operator' ? 'the operator' : 'the release checks';
  const would = out.wouldBump && out.wouldBump.to ? ` (would cut ${out.wouldBump.from} → ${out.wouldBump.to})` : '';
  let line;
  if (out.from && out.to) {
    line = `- release: cut ${out.from} → ${out.to} (${out.bumpLevel}), decided by ${decider}`;
  } else if (result.status === 'needs-operator') {
    line = `- release: undecided${would} — the wrap went on without an answer`;
  } else {
    line = `- release: held${would}, decided by ${decider}`;
  }
  const rec = out.recommendation;
  if (rec && rec.state === 'given') {
    line += `; AI recommended ${rec.value}`;
    if (rec.operatorIntent && rec.operatorIntent !== 'none stated') line += ` (operator: ${rec.operatorIntent})`;
  } else if (rec && rec.state === 'absent' && rec.reason) {
    line += `; no AI recommendation (${rec.reason})`;
  }
  if (out.disagreement === true) line += '; the release checks and the AI disagreed';
  return line;
}
...
async function run(context) {
  ...
  try {
    const prior = continuity.readIndexRaw(configRootOf(project));
    const priorMap = prior && prior.map ? prior.map : '';
    const delta = await _sessionDelta(project.path, {
      anchor, previousWrapSha,
      launchSha: context.scope && context.scope.baseline ? context.scope.baseline.sha : null,
      trunk: context.scope ? context.scope.trunk : undefined
    });
      ...
      log.warn('this wrap records no files: stamp — the session\'s range could not be established', {
        ...
        remediation: delta.stopped.length
          ...
          : 'the next wrap re-stamps the boundary, so the stamp returns on its own; if it does not, '
            + 'check that .tangleclaw/state.json loads and that the recorded lastWrapSha still '
  ...
  let indexFile;
  try {
    indexFile = continuity.writeIndex(configRootOf(project), {
      project: project.name,
  ...
  try {
    const projConfig = store.projectConfig.load(configRootOf(project));
  ...
  if (sid != null) {
    try {
      continuity.appendChangelogEntry(configRootOf(project), {
        date: writtenAt,
        sid,
      ...
      warm.changelog = true;
      continuity.writeWrapSummary(configRootOf(project), sid, {
        ...
        sections: {
          ...
          'Freshness': [
            ...
            _releaseFreshnessLine(previousResults || [])
          ].filter(Boolean).join('\n')
        }
      });
  ...
  return {
    ...
    output: {
      ...
      hadCapture,
      tier,
      // #1675 — the exact fields written into the index, so the handoff can
      // freeze the resume text it vouches for instead of the next launch
      // re-reading a file any later run may rewrite.
      currentState: captured.currentState || null,
      nextAction: captured.nextAction,
      freshness: { sha: facts.sha || null, branch: facts.branch || null, writtenAt, tier },
...
module.exports = { run, _internal, _releaseFreshnessLine, _resolveCapturedFields, _resolveCommitOutput, _resolveCommitAnchor, _sessionDelta, _stampDecision, _gitFacts, _branchType, _deriveTier, _deriveUncapturedReason };
