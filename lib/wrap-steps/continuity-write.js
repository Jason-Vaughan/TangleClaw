...
async function run(context) {
  ...
  try {
    ...
    if (_rangeIsSessionScoped(delta.kind, previousWrapSha)) {
      ...
      log.warn('session range unresolved; the wrap records no files: stamp', {
        project: project.name,
        reason: 'the recorded range base did not resolve as an ancestor of the wrap commit',
        previousWrapSha,
        rangeKind: delta.kind
      });
    }
  } catch (err) {
