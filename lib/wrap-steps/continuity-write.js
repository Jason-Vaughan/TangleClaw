 ...
 *
 * **The `files:` stamp is provenance, so it is the session's own set or it is
 * absent.** The warm tier records which paths a session changed, measured across
 * the session's own commit range and unfiltered — the wrap's commit is mostly
 * `.tangleclaw/` bookkeeping, which no source-file allowlist would keep. When the
 * session's range cannot be established the field is omitted with a logged reason
 * rather than filled from the branch's wider inventory: a provenance record that
 * is confidently wrong is worse than one that is visibly empty (#797).
 *
 ...
 * lists and a null `kind`, so `updateMap` leaves the prior Map untouched.
 *
 * @param {string} cwd - Project root
 * @param {object} [options]
 ...
 *   kind:('session'|'branch'|null)}>} `kind` says which range answered: `session`
 *   is the session's own commits, `branch` is the trunk fallback, `null` is no
 *   range at all.
 */
async function _sessionDelta(cwd, options = {}) {
  const { anchor = null, previousWrapSha = null } = options;
  const empty = { touched: [], deleted: [], kind: null };
  ...
  try {
    ...
    // being stubbed with a thrower to drive the no-repo path. Named anyway: an
    // empty delta and a stopped git look identical downstream.
    ...
    log.debug('session delta unavailable', { cwd, range: resolved.range, error: err.message });
    return empty;
  }
  if (!out || out.exitCode !== 0) {
    ...
    if (out && out.timedOut) _warnStopped(`git diff --name-status ${resolved.range}`, cwd, out);
    return empty;
  }
  ...
  return { touched, deleted, kind: resolved.kind };
}

/**
 * Decide whether this wrap may publish a `files:` provenance stamp.
 *
 * The stamp claims "these are the paths the session changed". The trunk-range
 * fallback answers a different question — every path the BRANCH changed — and
 * publishing that under this name is #797. It is the right answer in exactly one
 * case: a project with no recorded boundary, where the branch's divergence from
 * trunk IS the session's work because no earlier session has wrapped.
 *
 * So a fallback taken because a recorded boundary would not resolve reports
 * nothing rather than a plausible substitute, per `architecture.md`'s rule that a
 * read which could not be established names itself instead of defaulting.
 *
 * @param {'session'|'branch'|null} kind - Which range answered (`_sessionDelta`).
 * @param {string|null} previousWrapSha - The boundary the previous wrap recorded.
 * @returns {boolean} True when the range measured is the session's own work.
 */
function _rangeIsSessionScoped(kind, previousWrapSha) {
  if (kind === 'session') return true;
  return kind === 'branch' && !previousWrapSha;
}
...
async function run(context) {
  ...
  // #797 — the boundary the PREVIOUS wrap recorded. The commit step reports it
  // because its own stamp has already overwritten the on-disk value by now.
  ...
  try {
    ...
    if (_rangeIsSessionScoped(delta.kind, previousWrapSha)) {
      touchedFiles = delta.touched;
    } else if (delta.kind) {
      // The Map still accretes from the wider range (stubbing is idempotent), but
      // the provenance stamp does not get to publish it as this session's work.
      log.warn('this wrap records no files: stamp — the session\'s own range could not be established', {
        project: project.name,
        why: 'the range base the previous wrap recorded is not an ancestor of this wrap commit, '
          + 'so the only range that resolved is the whole branch — which is not what files: means',
        previousWrapSha,
        rangeMeasured: delta.kind,
        remediation: 'usually history rewritten since the last wrap, or a fresh clone missing that '
          + 'object; the next wrap re-stamps the base and the stamp returns on its own'
      });
    }
  } catch (err) {
...
module.exports = { run, _internal, _resolveCapturedFields, _resolveCommitOutput, _resolveCommitAnchor, _sessionDelta, _rangeIsSessionScoped, _gitFacts, _branchType, _deriveTier, _deriveUncapturedReason };
