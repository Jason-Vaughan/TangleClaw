...
function classify(scope, dirty, options = {}) {
  ...
  return {
    ...
    tangleclawState,
    stageable: stageableOf({ owned, included, tangleclawMaintenance })
  };
}
 ...
 *
 * The one definition of "what the wrap may commit". `classify` builds its own
 * `stageable` from this, and so must anything that rebuilds a classification
 * after filtering it — `./_secret-check` withholds flagged paths and needs the
 * same answer. A copy of the rule would silently drop a bucket added later
 * from every wrap commit, with nothing failing (#1513).
 *
