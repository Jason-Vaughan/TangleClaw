...
function parseStatus(stdout) {
  ...
  for (let i = 0; i < fields.length; i += 1) {
    ...
    const pair = isRenameOrCopy ? (pairSeq += 1) : null;
    // The DESTINATION is present on disk and in the index, whatever X says.
    add(entry.slice(3), xy.includes('D'), xy[0] === 'D', pair);
    if (isRenameOrCopy) {
      i += 1;
      // The source. A rename's source is removed from the index by the rename
      // itself; a COPY's source is untouched and stays addable.
...
function selectionOf(buckets, dirty) {
  ...
  return {
    ...
    splitRenames: splitRenamesOf(dirty, stageable)
  };
}
 ...
 */
function splitRenamesOf(dirty, stageable) {
  ...
  return [...pairs.values()].filter((p) => p.staged.length > 0 && p.left.length > 0);
}
