 ...
 *   itself. Committed without asking.
 ...
 *   wrap itself wrote this run; always owned.
 ...
 *   stageable:string[]}}
 *   `stageable` is `owned` plus `included` — exactly what the commit may stage.
 */
function classify(scope, dirty, options = {}) {
  ...
  for (const entry of dirty) {
    ...
    if (launchDirty) {
      ...
      if (m === null || m < startedAtMs) reason = 'predates-launch';
    }
    if (reason) {
  ...
  return { owned, foreign, included, left, undecided, stageable: [...owned, ...included] };
}
...
function remediation(undecided) {
  ...
  return 'These files have uncommitted changes that this session did not make, so the wrap will not commit them without asking:\n'
    ...
    + 'Leave never discards anything. If another session is still working on a file, leave it.';
}
