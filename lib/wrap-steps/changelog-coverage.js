 ...
 * change that the operator has not included — as paths relative to `cwd`, the
 * form `_dirtyPaths` returns.
 *
 * @param {string} cwd - Absolute project root.
...
function _excludedFromCommit(cwd, scope, rawDecisions) {
  ...
  const out = new Set();
  for (const f of classified.foreign) {
    if (included.has(f.path)) continue;
    if (!prefix) out.add(f.path);
    else if (f.path.startsWith(`${prefix}/`)) out.add(f.path.slice(prefix.length + 1));
  }
  return out;
}
