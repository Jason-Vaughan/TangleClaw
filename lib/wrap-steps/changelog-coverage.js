 ...
 *
 * **The working tree counts as part of the session.** An uncommitted edit to a
 * declared path satisfies the predicate outright — the changelog is being
 * maintained, whether by the operator answering a block or by an earlier turn that
 * has not committed yet. Conversely, uncommitted work with NO such edit is its own
 * unaccounted-for unit: it would otherwise be swept into the wrap's own commit,
 * which the next session's range starts after, so it could never be judged at all.
 *
...
/**
 * Stand-in identifier for the working tree in an `uncovered` list. Not a SHA — the
 * pending work has no commit yet, which is exactly why it needs its own unit.
 */
const WORKING_TREE_SHA = '(working tree)';
...
function evaluate(projectPath, paths) {
  ...
  // The working tree is part of the session too. Judging only committed history
  // would leave two holes: an entry written earlier in the session but not yet
  // committed would go unseen, and work still uncommitted at wrap time would be
  // swept into the wrap commit — which the next session's range starts AFTER, so
  // it could never be judged at all.
  let dirty;
  try {
  ...
  const otherDirty = dirty.filter((f) => !wantedSet.has(f));
  ...
  if (checkable.length === 0 && otherDirty.length === 0) {
  ...
  // Uncommitted work with no accompanying entry is one more unaccounted-for unit.
  if (otherDirty.length > 0) {
    uncovered.push({
      sha: WORKING_TREE_SHA,
      subject: `${otherDirty.length} uncommitted change(s) not yet logged`
    });
  }
  const checkedCount = checkable.length + (otherDirty.length > 0 ? 1 : 0);
