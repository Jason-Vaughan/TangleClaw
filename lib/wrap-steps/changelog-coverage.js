 ...
 *
 * **The predicate.** The session range is covered if ANY judged commit in it
 * (not a wrap commit, not a trunk sync) touched the changelog — the obligation is "the changelog was
 ...
 *
 * **The session's commits are its first-parent line (#1309, #1450).** The range
 * opens at the session's launch baseline (or a wrap that ran inside the session),
 * and the listing follows the first parent only. A trunk sync — the trunk merged
 * into the session's branch — is ignored, so another session's commits never
 * reach the verdict. Any other merge on the line (a PR merge on a trunk checkout)
 * is judged as ONE unit, with the paths it brought in. A merge that reports no
 * paths is not judgeable, so a merge never blocks on the absence of evidence.
 * The rule and its known limit live with the walk in `./_git-range`.
 */
 ...
 * @param {object} [scope] - The wrap run's scope (`lib/wrap-scope.js`). Supplies
 *   the launch baseline, the recorded wrap boundary read from the registered
 *   checkout, and the trunk position. Absent in a direct call, which reads the
 *   boundary from `projectPath`'s config and measures with no launch baseline.
 * @param {object} [options]
 * @param {Object<string, string>} [options.pathDecisions] - The operator's
 *   include/leave choices for uncommitted files the session did not change; with
 *   a scope, only files the wrap will commit are judged.
 * @param {Object<string, string>} [options.pathDecisionBasis] - The provenance
 *   verdict each answer was given against (#1868).
 * @param {Array<object>} [options.previousResults] - Prior step results; the
 *   upstream verdicts `session-files` reached are read from them (#1868).
 ...
 */
function evaluate(projectPath, paths, coveragePaths, scope = null, options = {}) {
  ...
  if (scope) {
    lastWrapSha = scope.lastWrapSha || null;
  } else {
    try {
      lastWrapSha = _internal.readLastWrapSha(projectPath).sha;
    } catch (err) {
      // A missing or unreadable wrap boundary is not an error here — it only means
      // no session SHA is recorded, and the base-branch fallback applies.
      log.info('Could not read the wrap boundary; using the trunk range', { error: err.message });
    }
  }
  const launchSha = scope && scope.baseline ? scope.baseline.sha : null;
  ...
  const stoppedProbes = [];
  const range = _resolveLogRange(projectPath, lastWrapSha, stoppedProbes, launchSha);
  if (!range) {
  ...
  let commits;
  try {
    commits = gitRange.markTrunkSyncs(projectPath, _listCommits(projectPath, range), {
      exec: _internal.execSync,
      trunk: scope ? scope.trunk : undefined,
      onStopped: (command) => stoppedProbes.push(command)
    });
  } catch (err) {
  ...
  let dirty;
  let maintenance = new Set();
  try {
    dirty = _dirtyPaths(projectPath);
    // Judge only what this wrap will commit (#1406): an uncommitted file this
    // session did not change, and that the operator has not included, stays out
    // of the commit, so it is neither unlogged work nor a changelog entry.
    if (scope) {
      const sorted = _sortForCommit(projectPath, scope, options.pathDecisions, {
        // #1868 — the upstream verdicts `session-files` reached, so a file
        // upstream already holds is not counted as this wrap's work.
        provenance: upstreamProvenance.fromPreviousResults(options.previousResults),
        decisionBasis: options.pathDecisionBasis
      });
      dirty = dirty.filter((f) => !sorted.excluded.has(f));
      maintenance = sorted.maintenance;
    }
  } catch (err) {
    return _unavailable(wasTimedOut(err)
  ...
  // TangleClaw's own maintenance ships in the commit but is not the session's work
  // (#1509): a managed-block refresh of the engine config is not something the
  // session could log, so it never demands an entry. It can still satisfy coverage
  // above — a declared path is a declared path.
  const dirtyWork = dirty.filter((f) => isSourceFile(f) && !maintenance.has(f));
  ...
  // A commit with no files in scope is not judgeable. Two cases reach here: a merge
  // that reports no paths; and — in a project rooted below its repo root — a commit
  // that touched only paths outside the project, which are not this project's
  // concern. Both would otherwise read as "touched nothing, therefore unlogged".
  // A trunk sync is not this session's work at all (#1309), so it is not judged.
  const checkable = commits.filter(
    (c) => !_isWrapCommit(c.subject) && !c.trunkSync && c.files.length > 0
  );
 ...
 * @param {string|null} lastWrapSha - The recorded wrap boundary (`lib/wrap-state.js`), or null.
 ...
 * @param {string|null} [launchSha] - The session's launch baseline sha, or null.
 * @returns {string|null} A git revision range, or null when none resolves.
 */
function _resolveLogRange(cwd, lastWrapSha, stoppedProbes = null, launchSha = null) {
 ...
 * @returns {Array<{sha:string, parents:string[], subject:string, isMerge:boolean, files:string[]}>}
 */
function _listCommits(cwd, range) {
  const stdout = _internal.execSync(
    `git log ${gitRange.FIRST_PARENT_FLAGS.join(' ')} --format=%x1e%H%x1f%P%x1f%s --name-only --relative ${range}`,
    {
      cwd,
      encoding: 'utf8',
 ...
 * @returns {{sha:string, parents:string[], subject:string, isMerge:boolean, files:string[]}}
 */
function _parseRecord(chunk) {
  ...
  return {
    sha,
    // Kept, not just counted: a merge's second parent is what decides whether it
    // is a trunk sync (`./_git-range` `markTrunkSyncs`).
    parents: parentList,
...
/**
 * The uncommitted paths the wrap will NOT commit — files this session did not
 * change that the operator has not included, and TangleClaw machine state, which a
 * wrap never commits (#1508), and databases it withholds (#1858) — as paths relative to `cwd`, the form `_dirtyPaths`
 * returns.
 *
 * @param {string} cwd - Absolute project root.
 * @param {object} scope - The wrap run's scope.
 * @param {*} rawDecisions - `options.pathDecisions` as received.
 * @returns {Set<string>}
 */
function _excludedFromCommit(cwd, scope, rawDecisions) {
  return _sortForCommit(cwd, scope, rawDecisions).excluded;
}

/**
 * The uncommitted paths the wrap will not commit, and the TangleClaw maintenance
 * it will commit on TangleClaw's behalf, both relative to `cwd`.
 *
 * @param {string} cwd - Absolute project root.
 * @param {object} scope - The wrap run's scope.
 * @param {*} rawDecisions - `options.pathDecisions` as received.
 * @param {object} [extra]
 * @param {object|null} [extra.provenance] - Upstream verdicts (#1868).
 * @param {*} [extra.decisionBasis] - `options.pathDecisionBasis` as received.
 * @returns {{excluded: Set<string>, maintenance: Set<string>}}
 */
function _sortForCommit(cwd, scope, rawDecisions, extra = {}) {
  const stdout = _internal.execSync(`git ${ownership.statusArgs().join(' ')}`, {
    cwd, encoding: 'utf8', timeout: GIT_EXEC_TIMEOUT_MS, maxBuffer: GIT_MAX_BUFFER_BYTES, stdio: ['ignore', 'pipe', 'ignore']
  });
  const classified = ownership.classify(scope, ownership.parseStatus(stdout), {
    decisions: ownership.sanitizeDecisions(rawDecisions),
    decisionBasis: ownership.sanitizeDecisionBasis(extra.decisionBasis),
    provenance: extra.provenance || null
  });
  const included = new Set(classified.included);
  let realCwd = cwd;
  try {
    realCwd = fs.realpathSync(cwd);
  } catch {
    // An unreadable cwd already failed the status read above; keep the given path.
  }
  // The toplevel is symlink-resolved (`lib/wrap-scope.js`), so the project path
  // must be too, or a `/var` → `/private/var` link turns the offset into `../..`.
  const prefix = scope.workToplevel ? path.relative(scope.workToplevel, realCwd) : '';
  const relative = (paths) => {
    const out = new Set();
    for (const p of paths) {
      if (!prefix) out.add(p);
      else if (p.startsWith(`${prefix}/`)) out.add(p.slice(prefix.length + 1));
    }
    return out;
  };
  return {
    excluded: relative([
      ...classified.foreign.map((f) => f.path).filter((p) => !included.has(p)),
      ...classified.tangleclawState,
      // #1858 — a database the wrap withholds is not work, so no changelog
      // entry can be what it is waiting on.
      ...classified.safetyWithheld,
      // #1868 — content upstream already holds is not this wrap's work.
      ...classified.alreadyUpstream
    ]),
    maintenance: relative(classified.tangleclawMaintenance)
  };
}
...
const _internal = {
  execSync,
  readLastWrapSha: (projectPath) => wrapState.readLastWrapSha(projectPath)
};
