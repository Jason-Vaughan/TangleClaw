...
function _buildPrompt(targets) {
  ...
  if (describeTargets.length > 0) {
    ...
    parts.push(
      ...
      fileList,
      '',
      `Replace each literal \`${STUB_MARKER}\` marker with one concise line saying what that directory or feature actually contains (read it if unsure). Leave the rest of the file as it is.`,
      '',
