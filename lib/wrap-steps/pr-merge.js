...
async function run(context) {
  ...
  // methodology grants. On an engine that cannot run it, nothing is enqueued and
  // the row names the PRs the operator resolved, so the decision is not lost:
  // a session on an engine that can run the methodology applies it.
