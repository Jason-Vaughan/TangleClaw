...
async function _ensurePushed(cwd) {
  ...
  const ahead = await _internal.exec('git', ['rev-list', '--count', '@{u}..HEAD'], { cwd });
  if (ahead.exitCode === 0 && ahead.stdout.trim() === '0') {
    return { ok: true, pushed: false, reason: null };
  }

  // Either the branch is ahead, or it has no upstream (`@{u}` fails). Both
  // want the same push; `-u` is harmless when an upstream already exists.
