...
const OUTCOME_MEANINGS = Object.freeze({
  ...
  'control-held': 'the lane is held or stopped by its control assignment, so no kickoff was typed; the launch context still says so'
});
...
function kickoffLine(sequence, stepCount) {
  return '[TangleClaw] Your launch context is waiting and none of it has been read yet: '
    ...
    + 'the command that step prints, then do the read-only freshness checks the task step asks for and attest with '
    + '`tc start ready --verdict <the preflight verdict step 3 stated> --first-action "<what you propose to do first>"`. '
    + 'Then emit the resume proposal the task step describes, and stop there for the operator. '
...
async function kickoff(args) {
  ...
  if (!sent.ok && sent.controlRefusal) {
    // #1861: refused before anything reached the pane; the lane is held.
    log.info('Launch kickoff not typed: the lane is held', { session: sessionId, project: projectName, code: sent.controlRefusal.code });
    return 'control-held';
  }
  if (!sent.ok) {
