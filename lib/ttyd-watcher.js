...
// How long a child must be SEEN exiting (`E`/`Z`) before it counts as wedged.
// It is measured between readings, never from `ps etime`: etime is how long the
// process has existed, so a tab open for hours would count as "old" the moment
// it began to exit, and a burst of long-lived tabs closing together would trip
// the gate on their normal exit. Nothing the kernel exposes says when a process
// began exiting, so the only honest measure is two sightings of the same child,
// exiting, in the same ttyd generation, at least this far apart. A clean exit
// finishes inside one 250 ms sample under the churn harness; the wedged ones it
// reproduced never finished while ttyd lived. The health panel and the watcher
// share one reading store, so two readings can land a second apart; this also
// stops the second of those from "confirming" a child that is one second old.
const DEFAULT_WEDGE_AGE_MS = 30 * 1000;
...
// The environment switches: a kill switch and a bounded orphan-threshold
// override, where an invalid value falls back to the safe default. Read once at
// `start()`.
...
// The newest reading of the current ttyd generation: what `latestReading()`
// serves and what a new generation is compared against to spot a restart. The
// wedge rule keeps its own record (`_exiting`).
let _latest = null;
// When each child of the current ttyd generation has been continuously seen
// exiting since. The only input to "wedged" (see `advanceExiting`).
let _exiting = { generation: null, since: new Map() };
let _lastReceipt = null;
// Set while a kickstart this module issued is waiting for its new ttyd, so the
// reading that first sees the new process is not also recorded as a restart
// nobody made. Keyed by PID, not generation: the reading that triggered the
// kickstart may have had no readable start time.
let _expectingRestartFromPid = null;
 ...
 * states a leaked `tmux attach` child is ever in. In macOS `ps` state strings
 * uppercase E and Z mean nothing else. The churn harness judges children with
 * this same predicate to decide which children are exiting (how long one must
 * stay exiting to count as wedged is set separately by each).
 ...
 *
 * Every caller shares the one measurement, so it always reads the configured
 * label and threshold; there is no per-call override that a joining caller
 * could silently lose.
 *
 * @returns {Promise<{
 *   pid: number|null, generation: string|null, binary: string|null, sampledAt: number,
 ...
 */
async function _measure() {
  const { ttydLabel, ptyThresholdRatio } = _config;
  const errors = [];
  let pid = null;
  try {
  ...
  if (pid === null) {
    return { pid: null, generation: null, binary: null, sampledAt: _clock.now(), children: null, pool: null, errors };
  }
  const [poolResult, psResult, lstartResult, binaryResult] = await Promise.allSettled([
    _readPool(ptyThresholdRatio),
    _run('ps', ['-A', '-o', 'pid=,ppid=,stat=,etime=']),
    _run('ps', ['-o', 'lstart=', '-p', String(pid)]),
    // Which executable launchd is actually running (#1245, ADR 0018): the fix
    // lives in the owned runtime, so a reading that cannot say which binary it
    // came from cannot certify the fix is in force.
    _run('ps', ['-p', String(pid), '-o', 'comm='])
  ]);
  let pool = null;
  ...
  const binary = binaryResult.status === 'fulfilled' ? (String(binaryResult.value).trim() || null) : null;
  if (binary === null) {
    errors.push(`binary: ${binaryResult.status === 'rejected' ? binaryResult.reason && binaryResult.reason.message : 'empty'}`);
  }
  return { pid, generation, binary, sampledAt: _clock.now(), children, pool, errors };
}
...
/**
 * Keep a reading as the newest of its ttyd generation, advance the wedge
 * record, and record a restart this module did not make.
...
function _record(reading) {
  ...
  if (reading.generation && previous && previous.generation && previous.generation !== reading.generation) {
    if (_expectingRestartFromPid !== previous.pid) {
  ...
  if (reading.generation) _latest = reading;
  _exiting = advanceExiting(_exiting, reading);
}

/**
 * Advance the per-child exiting record by one reading. Keyed by ttyd
 * generation plus child PID:
 *
 * - a reading with no children or no generation (a failed or unknown
 *   measurement) changes nothing, so it can neither advance nor confirm;
 * - a new generation discards the whole record;
 * - a child seen exiting keeps the time it was FIRST seen exiting, or starts now;
 * - a child a successful reading shows absent, or no longer exiting, is dropped,
 *   so a later exit starts its clock again.
 *
 * Pure: returns a new record and never mutates `prev`.
 * @param {{generation: string|null, since: Map<number, number>}} prev
 * @param {object} reading - A reading.
 * @returns {{generation: string|null, since: Map<number, number>}}
 */
function advanceExiting(prev, reading) {
  if (!reading.children || !reading.generation) return prev;
  const carry = prev.generation === reading.generation ? prev.since : new Map();
  const since = new Map();
  for (const child of reading.children) {
    if (!_isExiting(child.stat)) continue;
    since.set(child.pid, carry.has(child.pid) ? carry.get(child.pid) : reading.sampledAt);
  }
  return { generation: reading.generation, since };
}

/**
 * Sort a reading's `E`/`Z` children into confirmed wedges and transients.
 *
 * A child is **wedged** when it is exiting now AND it has been seen exiting,
 * continuously, in successful readings of the SAME ttyd generation for at least
 * `wedgeAgeMs` (the record `advanceExiting` keeps). A child's process age says
 * nothing about this (see `DEFAULT_WEDGE_AGE_MS`) and is not used; `ageMs` stays
 * on the reading as a diagnostic only. Everything else exiting is
 * **transient**: the normal cost of a websocket closing, reported but never
 * acted on. That split is what stops a reconnect burst from tripping the gate,
 * while a real wedge is still confirmed by a later reading `wedgeAgeMs` on.
 *
 * The gates are `null` when the measurement they need is missing: a failed
 * measurement, or one with no generation to key the record by, is unknown —
 * never a reason to act and never a reason to call the machine healthy.
 *
 * @param {object} reading - A reading, already folded into `exiting`.
 * @param {{generation: string|null, since: Map<number, number>}} exiting - From `advanceExiting`.
 ...
 * }}
 */
function classifyReading(reading, exiting, opts) {
  ...
  const keyed = !!reading.generation && exiting.generation === reading.generation;
  const wedged = [];
  const transient = [];
  for (const child of reading.children) {
    if (!_isExiting(child.stat)) continue;
    const since = keyed ? exiting.since.get(child.pid) : undefined;
    if (since !== undefined && reading.sampledAt - since >= opts.wedgeAgeMs) wedged.push(child);
    else transient.push(child);
  }
  // Without a generation the record cannot be consulted, so nothing can be
  // confirmed OR cleared: the orphan gate is unknown, not false.
  return { wedged, transient, orphanGate: keyed ? wedged.length >= opts.orphanThreshold : null, poolGate };
}
...
async function _kickstartWithReceipt(reading, reason) {
  ...
  _expectingRestartFromPid = reading.pid;
  try {
...
/**
 * Whether `next` is a different ttyd process from the one `before` read. With a
 * generation on both sides, the generation decides (it also catches a reused
 * PID). When `before` had no readable start time, only a different PID proves a
 * restart: the same PID with its start time now readable is the SAME process.
 * @param {object} before - The reading the kickstart was decided on.
 * @param {object} next - A later reading.
 * @returns {boolean}
 */
function _isNewTtyd(before, next) {
  if (next.pid === null) return false;
  if (before.generation && next.generation) return next.generation !== before.generation;
  return next.pid !== before.pid;
}
...
async function _tick() {
  ...
  try {
    ...
    const classification = classifyReading(reading, _exiting, _config);
...
async function measureLeak() {
  ...
  const c = reading.pid === null
    ...
    : classifyReading(reading, _exiting, _config);
  return {
    pid: reading.pid,
    generation: reading.generation,
 ...
 * @param {number} [options.wedgeAgeMs=30000] - how long a child must be SEEN exiting (between two readings) to count as wedged
...
function _reset() {
  ...
  _latest = null;
  _exiting = { generation: null, since: new Map() };
  _lastReceipt = null;
  _expectingRestartFromPid = null;
