 ...
 *
 * A child is **wedged** when it is exiting now AND the same child was already
 * exiting in an earlier reading of the SAME ttyd generation taken at least
 * `wedgeAgeMs` before — so it has been exiting for at least that long. A
 * child's process age says nothing about this (see `DEFAULT_WEDGE_AGE_MS`) and
 * is not used. Everything else exiting is **transient**: the normal cost of a
 * websocket closing, reported but never acted on. That split is what stops a
 * reconnect burst from tripping the gate, while a real wedge is still confirmed
 * by the next reading at least `wedgeAgeMs` later.
 *
...
function classifyReading(reading, history, opts) {
  ...
  if (reading.generation) {
    for (let i = history.length - 1; i >= 0; i--) {
      ...
      if (reading.sampledAt - h.sampledAt >= opts.wedgeAgeMs) { earlier = h; break; }
    }
  }
  ...
  for (const child of reading.children) {
    ...
    if (seenExiting.has(child.pid)) wedged.push(child);
