...
const DEFAULT_MIN_TTYD_AGE_MS = 3 * DEFAULT_INTERVAL_MS;
...
async function measureLeak(opts = {}) {
  ...
  if (pid === null) {
    return { pid: null, pool: null, orphans: null, orphanThreshold, ptyThresholdRatio, uptimeMs: null, minTtydAgeMs: DEFAULT_MIN_TTYD_AGE_MS };
  }
  ...
  const uptimeMs = etimeResult.status === 'fulfilled'
    ...
    : null;
  // The module default, NOT any per-call override: `server.js` starts the
  // watcher with no options, so the two cannot disagree today. If an override
  // is ever passed to `start`, this has to be threaded through with it — the
  // panel would otherwise quote a threshold the watcher is not using.
  return { pid, pool, orphans, orphanThreshold, ptyThresholdRatio, uptimeMs, minTtydAgeMs: DEFAULT_MIN_TTYD_AGE_MS };
}
 ...
 * cannot be read.
 *
 * The hold is a property of the ttyd process, never of this module's own
 ...
 * refused, ttyd's age is unchanged and old, so nothing is held down and the
 ...
 *
 * The orphan gate stays quiet until ttyd has been running for `minTtydAgeMs`
 * (#1245), because ANY restart makes every terminal reconnect at once and that
 * churn is what leaks — so the gate could otherwise fire on damage a restart
 * caused. Keyed to ttyd's own age rather than to anything this module
 * remembers doing, so the operator's manual `launchctl kickstart` and a launchd
 * respawn count exactly as ours does. The POOL gate is never held down:
 * exhaustion is the #94 incident, and a papercut is not worth trading for it.
 *
 * @param {{ ttydLabel: string, ptyThresholdRatio: number, orphanThreshold?: number, minTtydAgeMs?: number }} opts - ptyThresholdRatio is a 0..1 fraction (not a percent — 0.85, not 85); orphanThreshold is a leaked-child COUNT (defaults to DEFAULT_ORPHAN_THRESHOLD when omitted)
...
function _check(opts) {
  ...
  const orphanThreshold = opts.orphanThreshold ?? DEFAULT_ORPHAN_THRESHOLD;
  const minAgeMs = opts.minTtydAgeMs ?? DEFAULT_MIN_TTYD_AGE_MS;
  try {
    ...
    if (exhausted || orphanGate) {
      ...
      // The minimum-age hold binds the ORPHAN gate only. Pool exhaustion is the #94
      ...
      if (!exhausted && uptimeMs !== null && uptimeMs < minAgeMs) {
        ...
        log.warn('ttyd orphan gate held down — this ttyd is too newly restarted to blame', {
          ...
          ttydUptimeMs: uptimeMs, minTtydAgeMs: minAgeMs, remainingMs: minAgeMs - uptimeMs,
 ...
 * @param {number} [options.minTtydAgeMs=900000] - how long ttyd must have been running before the ORPHAN gate may fire (#1245), counted from ttyd's own start time so every restart producer is covered. The pool gate is never held down.
 */
function start(options = {}) {
  ...
  const minTtydAgeMs = options.minTtydAgeMs ?? DEFAULT_MIN_TTYD_AGE_MS;

  log.debug('Starting ttyd watcher', {
    ttydLabel, intervalMs, ptyThresholdRatio, orphanThreshold, minTtydAgeMs
  });

  _timer = setInterval(() => {
    _check({ ttydLabel, ptyThresholdRatio, orphanThreshold, minTtydAgeMs });
  }, intervalMs);
