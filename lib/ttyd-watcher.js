...
// #1245: a kickstart blanks every open terminal iframe at once, so they all
...
// gate can fire on damage its own previous kickstart caused, blanking the
// operator's terminals two or three times over one underlying leak.
//
// Three poll intervals. Long enough that the post-restart reconnect burst is
// counted as the cost of the restart rather than as a fresh leak, short enough
// that a genuinely fast leak is still recycled several times an hour. Bounds
// the thrash; it does not pretend to fix the leak, which is ttyd's own.
const DEFAULT_KICKSTART_COOLDOWN_MS = 15 * 60 * 1000;
const SHELL_TIMEOUT_MS = 5000;

let _timer = null;
let _disabled = false;
// Epoch ms of the last kickstart this process fired, or null if none yet.
let _lastKickstartAt = null;

/** Injectable clock, so the cooldown is testable without waiting on it. */
const _internal = { now: () => Date.now() };
 ...
 * @returns {{action: 'kickstart'|'suppressed'|'ok'|'skipped'|'measurement-failed'|'error', orphans: number|null, sinceLastKickstartMs: number|null}}
 */
function _check(opts) {
  ...
  try {
    ...
    if (pid === null) {
      ...
      return { action: 'skipped', orphans: null, sinceLastKickstartMs: null };
    }
    ...
    // Carried on every return so the tick AFTER a kickstart says how many
    // orphans survived it. Nothing previously distinguished "the kickstart
    // reclaimed them and the reconnect burst made more" from "the kickstart
    // did not reclaim them", which is what made the thrash inferred rather
    // than diagnosable.
    const sinceLast = _lastKickstartAt === null ? null : _internal.now() - _lastKickstartAt;
    ...
    if (cap === 0 && !orphanGate) {
      ...
      return { action: 'measurement-failed', orphans, sinceLastKickstartMs: sinceLast };
    }
    ...
    if (exhausted || orphanGate) {
      ...
      // exists to prevent.
      if (!exhausted && sinceLast !== null && sinceLast < cooldownMs) {
        ...
        // were not blanking.
        log.warn('ttyd orphan gate held down — still inside the post-kickstart cooldown', {
          ttydLabel, pid, orphans, zombies, reason,
          sinceLastKickstartMs: sinceLast, cooldownMs,
          note: 'a kickstart makes every terminal reconnect at once, and that churn leaks — these orphans may be its cost rather than a new leak'
        });
        return { action: 'suppressed', orphans, sinceLastKickstartMs: sinceLast };
      }

      log.warn('ttyd leak detected, kickstarting', {
        ttydLabel, pid, used, cap, ratio: Number(ratio.toFixed(3)),
        orphans, zombies, reason,
        sinceLastKickstartMs: sinceLast,
        ptyThreshold: ptyThresholdRatio, orphanThreshold
      });
      _kickstartTtyd(ttydLabel);
      _lastKickstartAt = _internal.now();
      return { action: 'kickstart', orphans, sinceLastKickstartMs: sinceLast };
    }

    log.debug('ttyd pool + child population ok', {
      ttydLabel, pid, used, cap, ratio: Number(ratio.toFixed(3)),
      orphans, zombies, sinceLastKickstartMs: sinceLast,
      ptyThreshold: ptyThresholdRatio, orphanThreshold
    });
    return { action: 'ok', orphans, sinceLastKickstartMs: sinceLast };
  } catch (err) {
    log.warn('ttyd watcher check failed', { error: err.message });
    return { action: 'error', orphans: null, sinceLastKickstartMs: null };
  }
}
...
function _reset() {
  stop();
  _disabled = false;
  _lastKickstartAt = null;
  _internal.now = () => Date.now();
  _runnerAsync = null;
