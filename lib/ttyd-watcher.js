...
function measureLeak(opts = {}) {
  ...
  const poolReading = _isPtyPoolExhausted(ptyThresholdRatio);
  // `cap: 0` is the fail-safe sentinel `_isPtyPoolExhausted` returns when the
  // reading broke — a real cap is never zero.
  const pool = poolReading.cap > 0 ? poolReading : null;
  let orphans = null;
  try {
    orphans = _ttydChildStats(pid).filter(
      (stat) => stat.includes('E') || stat.includes('Z')
    ).length;
  } catch (err) {
    log.debug('ps -A failed (health measurement)', { error: err.message });
  }
