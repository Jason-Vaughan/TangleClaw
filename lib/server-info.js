...
// frontend "Restart TangleClaw" button is enabled (#235).
const MACOS_PLIST_PATH = path.join(os.homedir(), 'Library', 'LaunchAgents', 'com.tangleclaw.server.plist');

// Linux systemd USER unit — the equivalent of the per-user launchd plist.
// Only a user unit qualifies: the server runs unprivileged, so a
// system-wide unit (/etc/systemd/system) is one it cannot restart without
// root or a polkit grant. No installer writes this file yet (#1424); until
// one does, an operator who installs the unit by hand gets the button.
const LINUX_SYSTEMD_USER_UNIT_PATH = path.join(os.homedir(), '.config', 'systemd', 'user', 'tangleclaw.service');

// Drop-in directories systemd merges into that unit, highest priority first:
// a same-named file in an earlier directory masks one in a later directory.
// systemd consults further directories (/run, /usr/lib, the type-wide
// `service.d`); an operator-installed unit is configured in these two.
const LINUX_SYSTEMD_USER_DROPIN_DIRS = [
  `${LINUX_SYSTEMD_USER_UNIT_PATH}.d`,
...
/**
 * The `KillMode=` value a unit file's `[Service]` section leaves in force —
 * the last assignment wins, as in systemd — or `undefined` when the text
 * assigns none. An empty assignment is returned as `''` (a reset).
 *
 * @param {string} text - Unit or drop-in file contents
 * @returns {string|undefined}
 */
function _lastServiceKillMode(text) {
  let section = null;
  let value;
  for (const raw of String(text).split(/\r?\n/)) {
    ...
    const m = /^KillMode\s*=\s*(.*)$/.exec(line);
    if (m) value = m[1].trim();
  }
  return value;
}
...
/**
 * The drop-in files systemd would apply to the user unit, in the order it
 * applies them: sorted by file name, a name in a higher-priority directory
 * masking the same name lower down. Returns `null` when a directory exists
 * but cannot be listed, since an override that cannot be seen cannot be
 * ruled out.
 *
 * @returns {string[]|null} Absolute paths, or null when unknowable
 */
function _linuxUserUnitDropins() {
  const byName = new Map();
  for (const dir of LINUX_SYSTEMD_USER_DROPIN_DIRS) {
    let names;
    try {
      names = _internal.readdirSync(dir);
    } catch (err) {
      if (err && err.code === 'ENOENT') continue;
      return null;
    }
    for (const name of names) {
      if (name.endsWith('.conf') && !byName.has(name)) byName.set(name, path.join(dir, name));
    }
  }
  return [...byName.keys()].sort().map((name) => byName.get(name));
}

/**
 * Whether the systemd user unit exists and, with its drop-ins applied,
 * stops only its main process on restart (`KillMode=process`).
 *
 * systemd's default, `control-group`, stops every process in the unit's
 * cgroup — including a tmux server the server started, however detached —
 * while the restart dialog promises that sessions survive. So the effective
 * value must be `process`. Anything that cannot be read counts against the
 * button: a wrong "no" hides it, a wrong "yes" ends every session.
 *
 * @returns {boolean}
 */
function _linuxUserUnitKeepsSessions() {
  if (!_internal.existsSync(LINUX_SYSTEMD_USER_UNIT_PATH)) return false;
  const dropins = _linuxUserUnitDropins();
  if (dropins === null) return false;
  let killMode;
  try {
    for (const file of [LINUX_SYSTEMD_USER_UNIT_PATH, ...dropins]) {
      const value = _lastServiceKillMode(_internal.readFileSync(file, 'utf8'));
      if (value !== undefined) killMode = value;
    }
  } catch { // prawduct:allow prawduct/broad-except -- any unreadable unit or drop-in must hide the button, whatever the error
    return false;
  }
 ...
 * itself (#235): macOS launchd via the per-user plist, or Linux systemd
 * via the per-user unit when its effective `KillMode` is `process`. Each
 * platform checks only its own file, so a stray file from the other
 * platform enables nothing. Other hosts (sysvinit, bare-node, Windows)
 * have no mechanism.
 *
 * Mechanism is detected lazily on first read and cached for the
 * process lifetime (the service file is installed once at setup time
 * and doesn't move). Returns `null` when no mechanism is
...
function detectRestartMechanism() {
  ...
  if (platform === 'darwin' && _internal.existsSync(MACOS_PLIST_PATH)) {
    _restartMechanism = 'launchctl';
  } else if (platform === 'linux' && _linuxUserUnitKeepsSessions()) {
    _restartMechanism = 'systemctl';
  } else {
...
module.exports = {
  ...
  LINUX_SYSTEMD_USER_DROPIN_DIRS
};
