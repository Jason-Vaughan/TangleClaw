 ...
 *
 * Runs `systemctl` without a shell and never throws. `reason` is null when
 * there is simply no such unit, which is the normal case on a host that
 * does not run TangleClaw under systemd and not worth reporting.
 *
 * @returns {{ok: boolean, reason: string|null}}
 */
function probeSystemdUserUnit() {
  let out;
  try {
    out = _internal.execFileSync('systemctl',
      ['--user', 'show', SYSTEMD_USER_UNIT, '-p', 'LoadState', '-p', 'MainPID', '-p', 'KillMode', '-p', 'NeedDaemonReload'],
      { encoding: 'utf8', timeout: SYSTEMCTL_TIMEOUT_MS, stdio: ['ignore', 'pipe', 'pipe'] });
  } catch (err) {
    ...
    return { ok: false, reason: `systemctl --user show failed: ${detail}` };
  }
  const props = {};
 ...
 * itself (#235): macOS launchd via the per-user plist, or a Linux systemd
 * user unit that systemd confirms is safe to restart (see
 * {@link probeSystemdUserUnit}). Other hosts (sysvinit, bare-node,
 * Windows) have no mechanism.
 *
 * Detected once and cached for the process lifetime; `server.js` calls this
 * at boot so the Linux probe never runs on a request. The restart route
 * re-checks with {@link confirmRestartMechanism} before acting, because the
 * unit can change while the server runs. When a Linux unit exists but is not
 * safe, the reason is logged once so the operator can see why the button is
 * disabled.
 *
...
function detectRestartMechanism() {
  ...
  if (platform === 'darwin' && _internal.existsSync(MACOS_PLIST_PATH)) {
    ...
    const probe = probeSystemdUserUnit();
    if (probe.ok) {
...
/**
 * Re-check, at the moment of a restart, that the cached mechanism is still
 * safe to use. launchd needs no re-check. For systemd the unit is queried
 * again; when it no longer qualifies, the cache is cleared so the next
 * `/api/server-info` poll hides the button.
 *
 * @param {string|null} mechanism - Return value of `detectRestartMechanism()`
 * @returns {{ok: boolean, reason: string|null}}
 */
function confirmRestartMechanism(mechanism) {
