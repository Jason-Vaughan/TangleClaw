...
// Linux systemd unit used by the future Linux service installation.
// Detection is intentionally based on the unit file existing rather than
// assuming every Linux host has TangleClaw installed as a systemd service.
const LINUX_SYSTEMD_UNIT_PATH = '/etc/systemd/system/tangleclaw.service';
...
function detectRestartMechanism() {
  ...
  } else if (platform === 'linux' && _internal.existsSync(LINUX_SYSTEMD_UNIT_PATH)) {
...
function buildRestartCommand(mechanism) {
  if (mechanism === 'launchctl') {
    ...
    // current instance and immediately launches a fresh one —
    // the process running this code is the one being killed, which is
  ...
  if (mechanism === 'systemctl') {
    // systemd owns the server process. Restarting the unit causes
    // systemd to terminate this process and start a fresh instance.
    return 'systemctl restart tangleclaw.service';
  }
