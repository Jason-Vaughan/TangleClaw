...
/**
 * The commit the running process started on, or null when it was not read.
 * Staleness itself is `getServerInfo`'s question; this is the one fact a
 * caller that measures the disk side on its own needs, without the
 * synchronous probe `getServerInfo` runs.
 *
 * @returns {string|null}
 */
function getStartupSha() {
  return _startupSha;
}

/**
 * The repository root this server runs from — its own checkout.
 *
 * @returns {string}
 */
function getRepoRoot() {
  return _internal.repoRoot;
}
