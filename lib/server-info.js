...
// systemd's default KillMode=control-group stops every process in the
// unit's cgroup on restart, and a tmux server the server started lives in
// that cgroup however it was detached. The restart dialog promises that
// sessions survive, which holds only when the unit stops the main process
// alone — so the unit must say so. An active (uncommented) line is required.
const KILLMODE_PROCESS_RE = /^[ \t]*KillMode[ \t]*=[ \t]*process[ \t]*$/m;
...
/**
 * Whether the systemd user unit exists and stops only its main process on
 * restart (`KillMode=process`), so tmux sessions outlive the restart.
 * A unit that cannot be read counts as absent.
 *
 * @returns {boolean}
 */
function _linuxUserUnitKeepsSessions() {
  ...
  let text;
  try {
    text = _internal.readFileSync(LINUX_SYSTEMD_USER_UNIT_PATH, 'utf8');
  } catch {
    return false;
  }
  return KILLMODE_PROCESS_RE.test(String(text));
}
 ...
 * via the per-user unit when that unit declares `KillMode=process`. Each
