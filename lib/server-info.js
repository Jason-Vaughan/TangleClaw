 ...
 * via the per-user unit. Each platform checks only its own file, so a
 * stray file from the other platform enables nothing. Other hosts
 * (sysvinit, bare-node, Windows) have no mechanism.
 *
...
function detectRestartMechanism() {
  ...
  } else if (platform === 'linux' && _internal.existsSync(LINUX_SYSTEMD_USER_UNIT_PATH)) {
