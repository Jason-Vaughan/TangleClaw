...
const ECOSYSTEM_ROSTER = [
  ...
  },
  {
    ...
    // are typing. When it cannot be established the item says so: an invented
    // host reads as a fact and sends the operator to a URL that does not answer,
    // which is worse than being told to ask.
    render: (ctx) =>
      ctx.operatorHost
        ...
        : '- **Never hand the operator a `localhost` link** — they are almost never on this machine. '
          + 'TangleClaw could not establish which host they reach this machine on, so **ask them** '
          + 'before composing an operator-facing URL rather than guessing one; `localhost` stays '
          + 'correct only for your own API calls from this host.'
  },
  {
    id: 'project-rules',
    render: (ctx) =>
