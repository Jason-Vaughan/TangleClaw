...
function tcBootstrapLines(format = 'md') {
  ...
  if (format === 'comment') {
    const text = 'Run `tc capabilities` BEFORE concluding a capability is missing — never '
      + `improvise one. tc is normally on PATH in a launched pane (${verbIds.join(', ')}) `
      + 'and reports absence honestly. A capability assumed not checked is how sessions fabricate '
      + 'outcomes. If tc is missing, check TANGLECLAW_API; a renamed install can break PATH. Use the API with '
      + 'verified launch identity. If context is missing or inconsistent, report it and stop identity-dependent '
      + 'actions — absence of both is unavailable context, not proof of being unmanaged. '
      + 'A failed localhost tc or curl is not proof of outage: a sandbox can block loopback while '
      + 'the host service is healthy — ask for a host-context check before '
      + 'reporting the server down.';
    return _wrapAsCommentLines(text);
  }
  return [
    '- **Run `tc capabilities` BEFORE concluding a capability is missing — never improvise '
    + `one.** \`tc\` is normally on PATH in a launched pane (verbs: ${verbIds.map((v) => `\`${v}\``).join(', ')}) `
    + 'and reports absence honestly; a capability assumed not checked is how sessions fabricate '
    + 'outcomes. If `tc` is missing, check `TANGLECLAW_API`; a renamed install can break PATH. Use the API with '
    + 'verified launch identity. If context is missing or inconsistent, report it and stop identity-dependent '
    + 'actions — absence of both is unavailable context, not proof of being unmanaged. '
...
/**
 * The one sentence that tells a session, on any engine, that a plan it wrote
 * has a shareable link and where to get it (#542).
 *
 * A sibling of `tcBootstrapLines`, shipped beside it by every config
 * composer in `lib/engines.js`, and deliberately NOT folded into it: the
 * bootstrap line also rides the session prime, whose budget is capped, and
 * this sentence belongs to the durable config a session reads at leisure.
 * The prime already says "run `tc capabilities`", which reports the same
 * endpoint at runtime. Same words in both forms — a comment carrier cannot
 * render emphasis.
 *
 * @param {('md'|'comment')} [format] - `md` for a markdown bullet, `comment`
 *   for `#`-prefixed plain text
 * @returns {string[]} Lines to append verbatim
 */
function planDocsLine(format = 'md') {
  const text = 'Plan and design docs you write to .tangleclaw/plans/ are served at a shareable URL: '
    + 'GET /api/projects/<projectId>/plans lists each one with the link to hand the operator '
    + '(tc capabilities shows it with your project id) — hand back that link, never a local file path.';
  if (format === 'comment') return _wrapAsCommentLines(text);
  return [`- **Plans are served at a shareable URL.** ${text.slice('Plan and design docs you write to '.length)}`];
}

/**
 * The sentence that tells a session how to get its launch context back after
 * `/clear` or a compaction has dropped it mid-session (#1761).
 *
 * It rides the durable config beside `planDocsLine` because that is the one
 * carrier an engine reloads after a clear; the prime and the pulled launch
 * steps are gone by then, and a session that no longer holds them cannot know
 * to ask. It promises only what `tc start review` does: a re-read for a
 * session that has an attested launch sequence and can run `tc`. Engines
 * without one are told where the rules alone live.
 *
 * @param {('md'|'comment')} [format] - `md` for a markdown bullet, `comment`
 *   for `#`-prefixed plain text
 * @returns {string[]} Lines to append verbatim
 */
function contextReentryLine(format = 'md') {
  const text = 'If your context was cleared or compacted mid-session, run `tc start review` to re-read '
    + 'the launch context you attested, rules included, before acting. It is read-only: do not re-attest or '
    + 're-propose. Without a launch sequence, `tc rules` re-reads the project rules.';
  if (format === 'comment') return _wrapAsCommentLines(text);
  return [`- **Lost your launch context?** ${text}`];
}

/**
 * The sentence that tells a session a HOLD or STOP is durable and binds even
 * where TangleClaw cannot enforce it (#1861).
 *
 * It rides the durable config because a HOLD is exactly the thing a session
 * must still know about after its context is gone. It claims only what the
 * control gate does: TangleClaw-governed mutations are refused while held,
 * and shell git/gh is not, so the session is told to honour it there.
 *
 * @param {('md'|'comment')} [format] - `md` for a markdown bullet, `comment`
 *   for `#`-prefixed plain text
 * @returns {string[]} Lines to append verbatim
 */
function controlStateLine(format = 'md') {
  const text = 'A HOLD or STOP from your assignment\'s authorities is stored by TangleClaw and wins over any earlier '
    + 'go-ahead, even one you read later. `tc control status` shows your lane; acknowledge with `tc control ack '
    + '<generation>`. While held, TangleClaw refuses its own governed mutations (wrap, commit, push, PR, restart); it '
    + 'cannot block shell `git`/`gh`, so honour the hold there too.';
  if (format === 'comment') return _wrapAsCommentLines(text);
  return [`- **Held or stopped?** ${text}`];
}

/**
 * The sentence that tells a session to report its own workload (#1912,
 * ADR 0020 §9), so coordinators can read fleet capacity without parsing
 * pane text. Shared by the durable config line and the `workload` capability.
 *
 * It rides the durable config beside `controlStateLine` because reporting is
 * something a session must keep doing after its context is cleared. The
 * visible STATE / SAFE TO CLEAR lines stay a courtesy to a human reader; this
 * verb is the contract. It claims only what the composition does: no receipt
 * reads UNKNOWN, never available.
 *
 * @returns {string}
 */
function workloadSentence() {
  // Built from the constants the server enforces, so the guidance can never
  // tell a session an expiry or a value the composition no longer uses.
  // Required lazily: this module is loaded by the config composers, which must
  // not pull the store in at require time.
  const { STATES, CLEARANCES, WAIT_KINDS } = require('./workload');
  const { EXPIRY_MS } = require('./workload-compose');
  const minutes = (state) => EXPIRY_MS[state] / 60000;
  const others = STATES.filter((st) => st !== 'working');
  const sameOthers = others.every((st) => minutes(st) === minutes(others[0]));
  const expiries = sameOthers
    ? `${minutes('working')} min working, ${minutes(others[0])} min otherwise`
    : STATES.map((st) => `${minutes(st)} min ${st}`).join(', ');
  return `Report your workload with \`tc workload set <${STATES.join('|')}> `
    + `--clearance <${CLEARANCES.join('|')}> --summary "<one line>"\` at dispatch acceptance, each task `
    + `transition, the start of any external wait (\`--wait ${WAIT_KINDS.join('|')}\`), completion, before `
    + `wrap and before exit, and again before it expires (${expiries}). Coordinators read it `
    + 'with `tc sessions`; a lane with no current receipt reads UNKNOWN, never available.';
}

/**
 * The durable-config form of {@link workloadSentence} (#1912, ADR 0020 §9).
 * @param {('md'|'comment')} [format] - `md` for a markdown bullet, `comment`
 *   for `#`-prefixed plain text
 * @returns {string[]} Lines to append verbatim
 */
function workloadLine(format = 'md') {
  const text = workloadSentence();
  if (format === 'comment') return _wrapAsCommentLines(text);
  return [`- **Report your workload.** ${text}`];
}
