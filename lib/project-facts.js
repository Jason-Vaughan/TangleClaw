 ...
 * and a config parse. 2000ms was right when this only had to cover an
 * `fs.access`, and a cold `git.getInfo` on a large repository can legitimately
 * exceed it. A deadline shorter than the honest work is not a safety margin; it
 * manufactures the failure it is meant to detect, and does so on the path where
 * the answer is "grant Full Disk Access".
 *
 ...
 *   unreadable: string|null, unreadableHint: string|null}}
 */
 ...
 *   config: object|null, unreadable: string|null, unreadableHint: string|null}>}
 */
async function readProjectFacts(project, options = {}) {
  ...
  try {
    ...
    // fields win where present; a clean answer carries nulls.
    return {
      unreadable: null,
      unreadableHint: null,
      unreadableCode: null,
      ...facts,
      unreadableCode: facts.code || facts.unreadableCode || null
    };
  } catch (err) {
    ...
    return _degraded((err && err.message) || 'the directory did not answer', hint,
      (err && err.tcTimedOut) ? 'SCAN_TIMEOUT' : 'SCAN_FAILED');
  }
}
