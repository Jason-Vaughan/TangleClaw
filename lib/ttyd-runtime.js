 ...
 * builds, it refuses and points back to the installer.
 *
 ...
 * point, either the selected pair verifies, or the resolver refuses the partial
 * state and a verified last-known-good pair survives for `rollback`.
...
const REPAIR = 'Re-run deploy/install.sh: it builds the owned ttyd runtime from deploy/ttyd/inputs.json and installs it. '
  + 'By hand: `node scripts/build-ttyd.js --out <stage-dir>`, then `node scripts/ttyd-runtime.js install --from <stage-dir>`. '
  + 'To run the Homebrew ttyd instead — WITHOUT the #1245 leak fix — set TANGLECLAW_TTYD_RUNTIME=homebrew and re-run.';
...
function defaultDeps() {
  return {
    ...
    build: ({ work, out }) => {
      execFileSync(process.execPath, [BUILD_SCRIPT, '--work', work, '--out', out], { stdio: ['ignore', 2, 2] });
    },
    fs: {
...
function expectedInputs() {
  try {
    const raw = fs.readFileSync(INPUTS_FILE);
    const inputs = JSON.parse(raw.toString('utf8'));
    return {
      inputsJsonSha256: crypto.createHash('sha256').update(raw).digest('hex'),
...
function installRuntime({ baseDir, stageDir, deps = defaultDeps() }) {
  ...
  return _recoverably('installation', () => {
    ...
    if (current.ok) {
      deps.fs.copyFileSync(p.ttyd, p.prev);
      deps.fs.chmodSync(p.prev, 0o755);
      deps.fs.copyFileSync(p.manifest, p.prevManifest);
...
function rollbackRuntime({ baseDir, deps = defaultDeps() }) {
  ...
  return _recoverably('rollback', () => {
    ...
    if (fs.existsSync(p.ttyd)) {
      deps.fs.copyFileSync(p.ttyd, p.discarded);
      deps.fs.chmodSync(p.discarded, 0o755);
      if (fs.existsSync(p.manifest)) deps.fs.copyFileSync(p.manifest, p.discardedManifest);
...
function provisionRuntime({ baseDir, env = process.env, deps = defaultDeps(), log = () => {} }) {
  ...
  const r = installRuntime({ baseDir, stageDir, deps });
