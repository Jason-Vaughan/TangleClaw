...
/**
 * Install a staged runtime, failing closed. The staged runtime is verified
 ...
 * verifies, is first copied over the last known good. A failure BEFORE that
 * copy changes nothing; a failure after it leaves the replaced runtime as the
 * last known good and, at worst, a binary/manifest pair the resolver refuses
 * (which `rollback` restores). The pair is not replaced atomically.
 * @param {object} opts
 * @param {string} opts.baseDir - TangleClaw's base directory.
