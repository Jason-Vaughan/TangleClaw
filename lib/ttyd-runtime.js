...
// Mode-aware on purpose: deploy/install.sh rewrites the ttyd plist for direct
// mode, so on a caddy-mode host it is the wrong way to select the runtime.
const REPAIR = 'Run `node scripts/ttyd-runtime.js provision` to build and install the owned ttyd runtime from '
  + 'deploy/ttyd/inputs.json, then select it for your ingress mode — direct mode: `./deploy/install.sh`; '
  + 'caddy mode: `node scripts/ingress-cutover.js --to caddy` (never deploy/install.sh, which rewrites the ttyd plist '
  + 'for direct mode). By hand, provision is `node scripts/build-ttyd.js --out <stage-dir>`, then '
