 ...
 * manifest is readable and names its digest, the digest matches, its whole
 * load graph stays inside its own directory and the macOS system roots, and it
 * runs and reports the version its manifest recorded. Every failing check is
 * reported, so an operator sees the whole problem at once.
 * @param {string} bin - The binary.
 * @param {string} manifestPath - Its manifest.
 * @param {object} deps - From `defaultDeps`.
