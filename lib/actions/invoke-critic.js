 ...
 * to `.tangleclaw/critic-runs.json` so the wrap pipeline's
 * `critic-check` step could detect that an operator had run the
 * Critic externally. That mismatched operator expectation (the "Run
 * Critic" button should run the Critic). #267 closes the gap.
 *
 * **Two execution paths:**
 *
 ...
 *    `ranAt: "ack"` entry without findings. This preserves the
 *    original handler's contract for the wrap pipeline's read side —
 *    any entry on current branch flips `criticRan: true`.
 *
 ...
 * `critic-check.js:defaultLoadCriticRuns` filters by `branchName`
 * presence only — the additive fields don't affect its predicate.
 *
 ...
 * file, malformed JSON, non-array root — all yield `[]`. Symmetric with
 * `critic-check.js`'s reader.
...
async function _attemptRealInvocation(project, session) {
  ...
  const engine = session && (session.engine || (session.engineProfile && session.engineProfile.id));
  if (engine && engine !== 'claude') {
    return { ok: false, reason: `degradedEngine:${engine}` };
  }
 ...
 * invocation path first; on any failure, falls back to ack-only mode
 * so the wrap pipeline's `critic-check` step still has a producer.
 *
