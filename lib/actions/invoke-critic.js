...
async function _attemptRealInvocation(project, session) {
  ...
  try {
    _internal.sendKeys(session.tmuxSession, CRITIC_COMMAND, {
      enter: true,
      // Locates the composer, and names the attempt a cleared draft is kept under (#1507).
      engineId: require('../governance-state').sessionEngineId(session, project),
      attemptKey: session.id != null ? require('../draft-store').sessionAttemptKey(session.id) : null
    });
  } catch (err) {
...
async function run(project, options = {}) {
  ...
  const filePath = path.join(project.path, CRITIC_RUNS_RELPATH);
  const tangleclawDir = path.dirname(filePath);
