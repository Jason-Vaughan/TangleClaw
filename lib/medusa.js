 ...
 *
 * There is deliberately NO `UNKNOWN` member. This probe never throws and always
 * reaches a verdict, so a code for "could not check" would have no producer — and
 * a vocabulary member nothing emits reads as coverage while covering nothing. The
 * "we could not check" state belongs to a CALLER that failed to reach this
 * function at all, and is that caller's to name (`lib/system-health.js` maps it
 * onto its own `unknown`).
 */
const BRIDGE_HEALTH_CODES = Object.freeze({
  /** Both transports answered and the Bridge reported itself healthy. */
  OK: 'BRIDGE_OK',
  ...
  /** The WS port accepts but the HTTP API did not answer. */
  HTTP_ABSENT: 'BRIDGE_HTTP_ABSENT',
  /** Both answered, and the HTTP side did not report itself healthy. */
 ...
 * @returns {Promise<{accepting: boolean, detail: (string|null)}>}
 */
function _probeWsPort(wsUrl, connectImpl) {
  ...
  try {
    ...
    return Promise.resolve({ accepting: false, detail: `unparseable WS URL: ${err.message}` });
  }
  ...
  return new Promise((resolve) => {
    ...
    const done = (accepting, detail) => {
      ...
      resolve({ accepting, detail });
    };
    let socket;
    try {
      socket = connect({ host, port, timeout: BRIDGE_PROBE_TIMEOUT_MS });
    } catch (err) {
      resolve({ accepting: false, detail: err.message });
      return;
    }
    socket.once('connect', () => done(true, null));
    socket.once('timeout', () => done(false, `no answer within ${BRIDGE_PROBE_TIMEOUT_MS}ms`));
 ...
 */
async function checkBridgeHealth({ fetchImpl, connectImpl } = {}) {
  ...
  try {
    ...
    if (res && res.ok) {
      ...
      try {
        ...
        // prawduct:allow prawduct/broad-except -- a health endpoint that answers 200
        // with a body we cannot read is still ANSWERING, which is what this half
        // asks. The version is a nicety; refusing the verdict over it would report
        // a Bridge as absent because its response shape changed.
      }
    } else {
      httpDetail = `HTTP ${res ? res.status : 'no response'}`;
    }
  } catch (err) {
    httpDetail = err.name === 'TimeoutError'
      ? `no answer within ${BRIDGE_PROBE_TIMEOUT_MS}ms`
      : (err.cause && err.cause.code) || err.message;
  }
