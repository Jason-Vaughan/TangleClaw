...
/**
 * @type {Set<string>} Non-loopback URLs already reported, so a misconfigured
 * install logs its refusal once rather than once per session start.
 */
const _reportedNonLoopback = new Set();
 ...
 * a PORT, never a host.
 *
 * @param {string} hostname - A URL hostname (IPv6 may arrive bracketed).
...
/**
 * Refuse a non-loopback Bridge URL, reporting it once.
 * @param {string} url - The rejected URL.
 * @param {string} source - Where it came from, for the log line.
 * @returns {string} The default WS URL to use instead.
 */
function _refuseNonLoopback(url, source) {
  if (!_reportedNonLoopback.has(url)) {
    _reportedNonLoopback.add(url);
    log.error('Refusing a non-loopback Medusa Bridge WS URL; using the loopback default', {
      url, source, using: DEFAULT_BRIDGE_URL,
      hint: 'The Medusa WS path is unauthenticated at the workspace layer, so the override may move the port but not the host. Point it at localhost/127.0.0.1, or tunnel the remote Bridge to loopback.'
    });
  }
 ...
 *   3. `ws://localhost:3010`.
 *
 * A resolved host that is not loopback is refused and the default used — see
 * `_isLoopbackHost` for why that is a constraint rather than a preference.
 *
...
function resolveBridgeWsUrl() {
  if (bridgeWsUrl) {
    let parsed;
    try {
      parsed = new URL(bridgeWsUrl);
    } catch (err) {
      log.error('Unparseable Medusa Bridge WS override; using the loopback default', {
        url: bridgeWsUrl, error: err.message, using: DEFAULT_BRIDGE_URL
      });
      return DEFAULT_BRIDGE_URL;
    }
    if (!_isLoopbackHost(parsed.hostname)) return _refuseNonLoopback(bridgeWsUrl, 'MEDUSA_BRIDGE_WS_URL');
    return bridgeWsUrl;
  }
  ...
  try {
    ...
    log.error('Unparseable Medusa Bridge HTTP base; using the loopback default WS URL', {
      url: bridgeHttpUrl, error: err.message, using: DEFAULT_BRIDGE_URL
    });
    return DEFAULT_BRIDGE_URL;
  }
  if (!_isLoopbackHost(http.hostname)) return _refuseNonLoopback(bridgeHttpUrl, 'MEDUSA_BRIDGE_HTTP_URL');

  // Medusa derives its WS port as `protocolPort + 1` (`medusa-server.js`), so an
  // install that moved the HTTP port moved the WS port with it. An HTTP base with
  // no explicit port carries the scheme's default, which is what `+ 1` applies to.
  const httpPort = http.port ? Number(http.port) : (http.protocol === 'https:' ? 443 : 80);
  const scheme = http.protocol === 'https:' ? 'wss:' : 'ws:';
  // WHATWG `hostname` KEEPS the brackets on an IPv6 literal, so bracket only what
  // is bare. Built from `hostname` rather than by slicing a port off `host`,
  // because `[::1]` and `[::1]:3009` do not share a suffix shape.
  const bare = http.hostname;
  const host = bare.includes(':') && !bare.startsWith('[') ? `[${bare}]` : bare;
  return `${scheme}//${host}:${httpPort + 1}`;
}
