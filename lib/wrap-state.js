 ...
 * - `untrackOffer` — `{decision:'declined', decidedAt, paths}` when the operator
 *   chose to keep TangleClaw state files tracked (#1512), else absent.
 *
 ...
 *
 * `safeToDrop` says whether the key may now leave project.json without losing a
 * boundary. It is false for an unreadable state file: that file is never
 * overwritten with the legacy value (it may hold a newer boundary nobody can
 * read, and a stale value in its place would read as recorded), so the legacy
 * copy is the only one left and must stay where it is.
 *
 * @param {string} projectPath - Absolute path of the registered checkout.
 * @param {*} legacySha - The value found under `lastWrapSha`.
 * @param {object} [opts]
 * @param {() => Date} [opts.now] - Test seam.
 * @returns {{adopted:boolean, safeToDrop:boolean, reason:string}} Throws when the state file cannot be written.
 */
function adoptLegacyLastWrapSha(projectPath, legacySha, opts = {}) {
  const current = readState(projectPath);
  if (current.read === 'unreadable') {
    return { adopted: false, safeToDrop: false, reason: `state file could not be read (${current.error}); left untouched` };
  }
  if (!_isSha(legacySha)) return { adopted: false, safeToDrop: true, reason: 'no legacy boundary to adopt' };
  if (current.read === 'present' && _isSha(current.record.lastWrapSha)) {
    return { adopted: false, safeToDrop: true, reason: 'state file already records a boundary' };
  }
  const now = (opts.now || (() => new Date()))();
  const base = current.read === 'present' ? current.record : {};
  _writeState(projectPath, { ...base, lastWrapSha: legacySha, migratedFromProjectConfigAt: now.toISOString() });
  return { adopted: true, safeToDrop: true, reason: 'adopted from project.json' };
}
 ...
 * written. An unparseable project.json, or a state file that cannot be read, is
 * left untouched.
 *
 * @param {string} projectPath - Absolute path of the registered checkout.
 * @param {object} [opts]
 * @param {() => Date} [opts.now] - Test seam.
 * @returns {{migrated:boolean, failed:boolean, reason:string}} `failed` is true only
 *   when there was a key to move and it could not be moved; nothing to do is not a failure.
 */
function migrateProjectConfig(projectPath, opts = {}) {
  const cfg = _readProjectConfigRaw(projectPath);
  if (cfg.read === 'absent') return { migrated: false, failed: false, reason: 'no project.json' };
  if (cfg.read === 'unreadable') return { migrated: false, failed: true, reason: `project.json could not be read: ${cfg.error}` };
  if (!Object.prototype.hasOwnProperty.call(cfg.config, LEGACY_KEY)) {
    return { migrated: false, failed: false, reason: 'project.json holds no lastWrapSha' };
  }
  try {
    const adopted = adoptLegacyLastWrapSha(projectPath, cfg.config[LEGACY_KEY], opts);
    if (!adopted.safeToDrop) return { migrated: false, failed: true, reason: adopted.reason };
    const rest = { ...cfg.config };
    delete rest[LEGACY_KEY];
    fs.writeFileSync(_projectConfigPath(projectPath), JSON.stringify(rest, null, 2) + '\n');
  } catch (err) {
    return { migrated: false, failed: true, reason: `could not move lastWrapSha: ${err.message}` };
  }
  return { migrated: true, failed: false, reason: 'moved lastWrapSha from project.json to the untracked state file' };
}

/**
 * The TangleClaw state paths the operator chose to keep tracked, from an earlier
 * un-track offer (#1512). An unreadable or absent record reads as none declined,
 * which at worst asks again — never untracks without an answer.
 *
 * @param {string} projectPath - Absolute path of the registered checkout.
 * @returns {string[]}
 */
function readUntrackDeclined(projectPath) {
  const state = readState(projectPath);
  const offer = state.read === 'present' ? state.record.untrackOffer : null;
  if (!offer || offer.decision !== 'declined' || !Array.isArray(offer.paths)) return [];
  return offer.paths.filter((p) => typeof p === 'string' && p);
}

/**
 * Remember that the operator declined to stop tracking these paths, adding them
 * to any earlier decline.
 *
 * An unreadable state file is left alone. Unlike a stamp, a decline is not the
 * newest boundary: writing it would drop the unreadable record's `lastWrapSha`,
 * so the boundary would read as absent instead of unreadable, and the next wrap
 * would publish the whole branch as the session's work (#797). Not remembering
 * costs one repeat question.
 *
 * @param {string} projectPath - Absolute path of the registered checkout.
 * @param {string[]} paths - The paths the declined offer listed.
 * @param {object} [opts]
 * @param {() => Date} [opts.now] - Test seam.
 * @returns {{recorded:boolean, reason:string}} Throws when the file cannot be written.
 */
function recordUntrackDeclined(projectPath, paths, opts = {}) {
  const now = (opts.now || (() => new Date()))();
  const current = readState(projectPath);
  if (current.read === 'unreadable') {
    return { recorded: false, reason: `state file could not be read (${current.error}); the decline was not remembered` };
  }
  const base = current.read === 'present' ? current.record : {};
  const merged = [...new Set([...readUntrackDeclined(projectPath), ...paths])].sort();
  _writeState(projectPath, { ...base, untrackOffer: { decision: 'declined', decidedAt: now.toISOString(), paths: merged } });
  return { recorded: true, reason: 'decline remembered' };
}
