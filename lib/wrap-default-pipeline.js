...
const DEFAULT_WRAP_PIPELINE = {
  ...
  "steps": [
    ...
    },
    {
      ...
      "prompt": "You are at the end of a development session. Capture this session's learnings — non-obvious behaviors, validated patterns, failure modes, or anti-patterns worth remembering for future work — to `.tangleclaw/memories/learnings.md`.\n\nSkip the obvious. Skip routine bug fixes. Capture only what would change how you'd approach a similar task next time.\n\nSteps:\n1. Read `.tangleclaw/memories/learnings.md` if it exists, to match the existing style. If it doesn't exist, create it only when there is a novel learning to record, with a top-level heading like `# Cross-Session Learnings — <project name>`.\n2. Reflect on this session: what surprised you? What broke in an unexpected way? What pattern got validated by shipping? What's worth remembering next time?\n3. If there is a novel learning, append a new entry. Convention: `## YYYY-MM-DD — <one-line title>` followed by a 2-5 sentence body. Link issues / PRs / commits with shortlinks where relevant.\n4. If there is nothing novel this session, leave the file unchanged. Do not fabricate signal or append a no-op marker; an unchanged file is the correct outcome.\n5. Save the file only when you added a justified learning. The commit step will pick it up via `git add -A`.\n\nWhen done, reply with a single `## Result` heading followed by a one-line summary of what you captured (or `no novel learnings; file unchanged` if applicable)."
    },
    {
