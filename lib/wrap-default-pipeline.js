 ...
 *
 * Every project runs this one pipeline. The step list used to live in
 * `data/templates/<methodology>/template.json` as `wrap_pipeline.steps`, one
 * copy per methodology; the wrap no longer reads methodology templates
 * (#538 — methodology sunset) because a per-methodology wrap meant the step
 * list was data that could drift, fork, and silently diverge from the runner
 * that executes it. Code-owning the pipeline makes the step list reviewable
 * in the same diff as the handlers it dispatches to.
 *
 ...
 * `lib/eval-audit.js` scoring) read this instead of re-deriving.
