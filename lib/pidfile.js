...
/**
 * The PID file's directory when a caller names none — TangleClaw's base
 * directory (`lib/tangleclaw-home.js`).
 *
 * Derived on use rather than captured at require, so this module's answer
 * cannot drift from the store's: a PID file landing anywhere else means the
 * liveness check and the running server disagree about which install they are
 * talking about, which is the whole failure `check` exists to prevent.
 *
 * @returns {string} Absolute path to the directory holding the PID file.
 */
function _defaultDir() {
  return tangleclawHome.baseDir();
}
...
function readRecord(dir) {
  const filePath = path.join(dir || _defaultDir(), PID_FILENAME);
  let content;
  try {
