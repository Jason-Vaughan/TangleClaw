...
# Homebrew build leaks a PTY per closed terminal on macOS. The path comes from
# the ONE resolver the ingress cutover also uses, never from PATH, and a missing
# or invalid runtime stops the install HERE, before any plist is written, with
