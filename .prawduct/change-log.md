...
**The change.** On a release promote, the `version-bump` wrap step rewrote `status=merged` tag lines in the project's `.prawduct/change-log.md` to `status=shipped`, and counted statusless tag lines. ADR 0011 says no TangleClaw code writes inside `.prawduct/`, so the flip was a boundary violation. It was also dead: prawduct stopped writing `status=merged` in v2.3.2 and retired its only reader in v3.3.0. The flip helpers, the `version-bump:prawduct-change-log` staging key, the step's `changeLog` / `changeLogWarning` output and the commit body's "Stamped N" line are removed. No fallback is kept for older prawduct (A1).
