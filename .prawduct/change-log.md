...
**Review.** Critic chunk review: 1 blocking finding (the third `classify` caller still demanded a CHANGELOG entry for maintenance), fixed with a regression test. verify-resolutions came back clean. Two known limits are accepted and recorded in the plan: a managed block is judged by location, not content (#1513 covers secrets), and a project below its repo root keeps today's questions. The cumulative review found 2 warnings, both fixed. The hook check now accepts only removals, so an added or edited entry is asked about. State names are read from their writers: a new `lib/tangleclaw-project-files.js` holds the prime, advisory and registry paths, and the other names come from each writer's exported constant, which also covers `.release-recommendation.md`.
