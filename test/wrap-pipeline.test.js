...
describe('wrap-step version-bump — never writes inside .prawduct/ (ADR 0011)', () => {
  ...
  before(() => {
    tmpRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'tc-vb-adr0011-'));
  ...
  beforeEach(() => {
    Object.assign(versionBump._internal, origInternal);
    versionBump._internal.todayIso = () => '2026-09-24';
  });

  it('a promote with a status=merged prawduct ledger stages and flushes only version + CHANGELOG', async () => {
    ...
    fs.writeFileSync(path.join(dir, 'CHANGELOG.md'),
      '## [Unreleased]\n\n### Fixed\n- a fix\n\n## [1.2.3] - 2026-07-01\n');
    const ledgerPath = path.join(dir, '.prawduct', 'change-log.md');
    const ledger = '# Change Log\n<!-- prawduct: type=fix | chunks=42 | scope=demo | status=merged -->\n- body\n';
    fs.mkdirSync(path.dirname(ledgerPath), { recursive: true });
    fs.writeFileSync(ledgerPath, ledger);

    const c = { project: { name: 'adr0011-proj', path: dir }, step: { id: 'version-bump', kind: 'version-bump' }, staged: {}, options: {} };
    const result = await versionBump.run(c);
    assert.equal(result.status, 'done', 'the promote itself still happens');

    assert.deepStrictEqual(Object.keys(c.staged).sort(), ['version-bump:changelog', 'version-bump:version-json']);
    const prawductDir = path.join(dir, '.prawduct') + path.sep;
    for (const [key, entry] of Object.entries(c.staged)) {
      assert.ok(!String(entry.primingPath).startsWith(prawductDir), `${key} stages a write inside .prawduct/`);
    }
    assert.equal(result.output.changeLog, undefined);
    assert.equal(result.output.changeLogWarning, undefined);
    assert.doesNotMatch(result.output.detail, /stamp/i);

    const flushed = commitStep._flushStagedWrites(c.staged);
    assert.equal(flushed.length, 2, 'only version.json + CHANGELOG.md flush');
    assert.equal(fs.readFileSync(ledgerPath, 'utf8'), ledger, 'the prawduct ledger is byte-identical');
    assert.match(fs.readFileSync(path.join(dir, 'CHANGELOG.md'), 'utf8'), /## \[1\.2\.4\] - 2026-09-24/);
  });
});
