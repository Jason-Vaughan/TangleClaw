...
describe('the tmux session listing reports a wedge at the condition\'s cadence, not the poll\'s', () => {
   ...
   * @returns {{stall: () => void, answer: () => void}} Mode switches.
   */
  function shim() {
    ...
    return {
      stall: () => fs.writeFileSync(flag, ''),
      answer: () => fs.rmSync(flag, { force: true })
    };
  }
  ...
  it('says it once, then keeps the repeats at debug', async () => {
    ...
    // one list, so three reads through ONE snapshot would spawn one tmux and
    ...
    const lines = await captured(async () => {
      for (let i = 0; i < 3; i++) {
        await tmux.createSessionNameSnapshot({ timeout: 200 }).get();
      }
    });
  ...
  it('treats a tmux that answered "no server running" as a recovery', async () => {
    ...
    const verdict = await tmux.createSessionNameSnapshot({ timeout: 200 }).get();
