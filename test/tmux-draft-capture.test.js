 ...
 * seams, and drive the clear itself against a real tmux pane.
 */
...
describe('reading the draft through the engine\'s composer profile', () => {
  ...
  it('Claude, empty composer: nothing to record', () => {
    ...
    assert.deepEqual(tmux._readDraft('s', 'claude'), { state: 'empty', text: '', reason: null, draftPresent: false });
  });
...
describe('the clear: records what it read, and still clears when it could not read', () => {
  ...
  it('a located draft is logged with its text, and the prompt is cleared', () => {
    try {
      ...
      const out = logged(() => tmux._clearPromptLine(session, 'claude'));
      assert.match(out, /Cleared a draft from the prompt before injecting/);
      assert.match(out, /draftBeforeClear="half-typed instruction"\n/);
  ...
  it('a wrapped draft stays one log entry', () => {
    try {
      ...
      assert.match(out, /draftBeforeClear="line one\\nline two"/);
    } finally {
      try { tmux.killSession(session); } catch (_) { /* already gone */ }
    }
  });
  ...
  it('an empty composer logs nothing', () => {
    try {
      ...
      assert.equal(logged(() => tmux._clearPromptLine(session, 'claude')), '');
    } finally {
      try { tmux.killSession(session); } catch (_) { /* already gone */ }
    }
  });
  ...
  it('an engine with no composer profile: still cleared (#812), and the log says no draft was captured', () => {
    try {
      ...
      const out = logged(() => tmux._clearPromptLine(session, 'aider'));
      ...
      assert.doesNotMatch(out, /draftBeforeClear|promptBeforeClear/);
  ...
  it('sendKeys hands the clear the engine it was given', () => {
    try {
      ...
      paneIs([DIVIDER, typed, DIVIDER, CLAUDE_FOOTER], cursorAtEnd(typed));
      const out = logged(() => tmux.sendKeys(session, 'true', { enter: false, engineId: 'claude' }));
      assert.match(out, /draftBeforeClear="draft via sendKeys"\n/);
    } finally {
      try { tmux.killSession(session); } catch (_) { /* already gone */ }
    }
  });
