...
describe('antigravity engine (#456)', () => {
  ...
  describe('config generation (AGENTS.md managed block)', () => {
    // Carrier changed 2026-08-31: Antigravity discovers GEMINI.md / AGENTS.md
    // only, so `.antigravity.md` was written and never read. A deliberate
    // contract change — the old assertions pinned a file nothing consumed.
    it('targets AGENTS.md at the project root via the antigravity-md generator', () => {
      const cf = store.engines.get('antigravity').configFormat;
      assert.equal(cf.filename, 'AGENTS.md');
      assert.equal(cf.mergeStrategy, 'managed-block');
      assert.equal(cf.generator, 'antigravity-md');
      assert.equal(cf.syntax, 'markdown');
    });

    it('generates markdown with the managed-block heading and rules content', () => {
      const content = engines.generateConfig('antigravity', PROJECT_CONFIG);
      assert.ok(content, 'generateConfig("antigravity") must not return null — generator missing from switch?');
      assert.ok(
        content.startsWith('## TangleClaw — generated; edits inside the markers are overwritten'),
    ...
    it('REGRESSION: the shared generator default header stays GEMINI.md-flavored (parameterized header)', () => {
      // The gemini PROFILE retired (#457) but the shared markdown builder keeps
      // its default header — custom profiles using generator `gemini-md` rely on it.
      const content = engines._generateGeminiMd(PROJECT_CONFIG);
      assert.ok(content);
      assert.ok(
        content.startsWith('# GEMINI.md — Generated by TangleClaw'),
        `default header drifted: ${content.split('\n')[0]}`
      );
    });
  });
