...
describe('startup session-rule delivery (#595)', () => {
  ...
  describe('rules reach the session on whichever channel the engine has', () => {
    it('delivers rules on a PLUGIN-GOVERNED project, where config generation delivers nothing', () => {
      ...
      // The precondition that made #595 possible, in its #1021 form: TC now
      // writes an OPERATIONAL block into a governed CLAUDE.md, but rules stay
      // off that file deliberately (one delivery path per tier), so anything
      // routed through config generation is still undeliverable for this tier.
      const writeResult = engines.writeEngineConfig('claude', project.path, { id: project.id }, engine, null);
      assert.equal(writeResult.written, true, 'governed projects now get the operational block (#1021)');
      const writtenConfig = require('node:fs').readFileSync(writeResult.configFilePath, 'utf8');
      assert.doesNotMatch(writtenConfig, /governed projects must receive this/,
        'the rule must NOT ride the config file — the prime is its only channel');
