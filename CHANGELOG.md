...
- **Silent prime delivery via Claude Code SessionStart hook — UI surface (#103, chunk 2)** — Per-project toggle that exposes the chunk-1 backend opt-in to users. The Project Settings modal now renders a "Silent prime (hidden context)" checkbox in `public/ui.js:openSettings` when the resolved engine profile advertises `capabilities.supportsSilentPrime === true` (Claude Code today; the gate is on the capability flag rather than on `engine.id === 'claude'` so future engines that gain the same capability light up the toggle without a UI change). The checkbox state mirrors `project.silentPrime`, persists via the existing `PATCH /api/projects/:name` endpoint, and `doSaveSettings` only attaches `silentPrime` to the PATCH body when the element was rendered (matches the capability-gated render so a non-supportive engine never receives a stale `silentPrime: undefined`). New `lib/projects.js:enrichProject` field `silentPrime` (read from `projConfig.silentPrime`, defaulted to `false` when projConfig is unavailable) flows through `GET /api/projects` and `GET /api/projects/:name` so the modal can render the persisted value, and the `PATCH /api/projects/:name` response gains `silentPrime` for symmetry. New `lib/projects.js:updateProject` validation: type-checks `silentPrime` is a boolean, and when set to `true`, gates against the *intended post-PATCH* engine's `capabilities.supportsSilentPrime` (prefers `updates.engine` over `projConfig.engine` over `project.engineId`) so a same-PATCH engine change is honored. Independent Critic on chunk 2 flagged 0 BLOCKERs / 3 MAJORs / 5 MINORs / 5 NITs; the 3 MAJORs addressed in-chunk: (M1) the original silentPrime block sat at the *bottom* of `updateProject` after the engine-config-write side effects, so a same-PATCH `{engine: 'gemini', silentPrime: true}` would write `projConfig.engine` and the new engine config file before the silentPrime gate rejected, leaving DB engine_id and disk projConfig.engine inconsistent — fix hoists the entire silentPrime validation (type check + capability gate) to the top of the function before any mutations run, so a rejection drops cleanly with no state drift; (M2) the same early-return clobbered prior accumulated `errors` and any non-null `methodologySwitch` (e.g., archive paths from a prior methodology-switch step) — moot after the M1 hoist since validation now runs before any errors or methodologySwitch can accumulate; (M3) chunk 2 had no CHANGELOG entry — this bullet. Critic also flagged 1 MINOR (no test for engine+silentPrime PATCH atomicity) addressed in-chunk: new test `updateProject rejects engine+silentPrime PATCH atomically when new engine lacks capability` snapshots projConfig.engine and store engine_id pre-PATCH, attempts the bad PATCH, asserts the rejection, and re-reads to confirm zero disk/DB drift. Critic Mn2 (no negative-render test for the gate's ELSE branch) addressed by adding a regex-bound assertion that the ternary's false branch is `''` (empty string), so a future refactor cannot accidentally emit toggle markup when the engine lacks the capability. Other 4 MINORs and 5 NITs accepted: Mn3 (PATCH response shape stability test for non-silentPrime PATCHes), Mn4 (redundant projConfig load — moot after the M1 hoist removed it), Mn5 (UI re-render of the toggle when engine dropdown changes — backend now rejects gracefully without state corruption regardless, so this is UX polish for chunk 3), N1 (gs-toggle-label class borrow), N2 (catch comment generalization), N3 (test brittleness on character-window walk), N4 (test comment wording), N5 (hint copy density). Symmetric capability gate audit confirmed: UI render gate, updateProject validation gate, `_buildBaselineHooks` gate (chunk 1), and `launchSession` gate (chunk 1) all check `engineProfile.capabilities.supportsSilentPrime === true`, and the two backend-launch gates additionally require `projConfig.silentPrime === true` — all four match the canonical conjunction at their respective scope. 17 new tests: `test/projects.test.js` (8: enrichProject default-false + reflects-projConfig, updateProject persists true/false, rejects when engine lacks capability, rejects non-boolean, allows clearing on unsupported engines, and the M1 atomic-rejection regression), `test/api-projects.test.js` (2: PATCH persists silentPrime + GET round-trip, PATCH rejects non-boolean), `test/settings-modal-silentprime.test.js` (7: capability gate predicate, checkbox tied to project.silentPrime, ternary structure, ternary's false-branch resolves to empty string (Mn2 regression), hint copy mentions SessionStart hook, doSaveSettings reads checkbox + sets body.silentPrime, doSaveSettings guards on element presence). 1673 total tests passing (was 1656 after chunk 1 merge).

- **Silent prime delivery via Claude Code SessionStart hook — backend (#103, chunk 1)** — Opt-in path that delivers the session prime as hidden model context through Claude Code's `SessionStart` hook instead of pasting it into the terminal via `tmux send-keys`. End-state for an enabled project: a launched session shows an empty `>` prompt with no scrollback noise, while the methodology, last-session summary, active learnings, and eval-audit state are all in the AI's context. New `data/hooks/sessionstart-prime.sh` is a 10-line bash script that cats `${CLAUDE_PROJECT_DIR}/.tangleclaw/session-prime.md` and exits 0 silently if the file or env var is missing — failure can never block session startup. New `lib/engines.js` helpers `_buildBaselineHooks(projConfig, engineProfile)` (returns a `SessionStart` entry pointing at the bundled script when `projConfig.silentPrime === true` AND `engineProfile.capabilities.supportsSilentPrime === true`, otherwise `{}`) and `_mergeHookObjects(a, b)` (concatenates entries under each event name, methodology first then baseline, deep-cloned). `syncEngineHooks` now merges methodology hooks with engine-level baseline hooks rather than replacing — Prawduct's existing `SessionStart` governance entry coexists with the new prime entry — and resolves the engine profile via `store.engines.get(projConfig.engine || 'claude')` so the capability gate matches `launchSession`'s exactly. New `lib/sessions.js` helper `_writePrimeFile(projectPath, primeText)` writes `.tangleclaw/session-prime.md` (creates the directory if missing, non-throwing on I/O failure). `launchSession` loads `projConfig` early, derives `silentPrime = projConfig.silentPrime === true && engineProfile.capabilities.supportsSilentPrime === true`, writes the prime file when that's true, and passes a new `silentPrime` arg to `_deferEngineInit` which skips the existing `tmux.sendKeys(primeText)` call when set. New `capabilities.supportsSilentPrime: true` on `data/engines/claude.json`; new `silentPrime: false` default on `DEFAULT_PROJECT_CONFIG` in `lib/store.js` (opt-in until proven). Other engines (Codex, Gemini, Aider) keep the existing typed-prime path. UI surface for the per-project toggle is chunk 2; final polish + Critic ride in chunk 3. Independent Critic flagged 0 BLOCKERs / 2 MAJORs / 4 MINORs / 4 NITs; both MAJORs addressed in-chunk: (M1) `_buildBaselineHooks` was reading `projConfig.silentPrime` only, ignoring the engine capability — would write an orphan SessionStart hook for engines that can't use it; now takes `engineProfile` and gates on both flags symmetrically with `launchSession`; (M2) the existing `syncEngineHooks` describe block in `test/engines.test.js` shared a single `projectDir` across all 7 tests via `before/after`, leaving the new merge behavior load-bearing on an implicit assumption (no leftover `silentPrime: true` `project.json` in the shared dir); flipped to `beforeEach/afterEach` for full per-test isolation. The 4 MINORs and 4 NITs are deferred to chunk 3 (CHANGELOG bullet split, hook-script `set -u`, prime-file cleanup on silent→typed transition, comment polish, profile-validator allow-list verification — already confirmed safe). 29 new tests: `test/engines.test.js` (17: 8 `_buildBaselineHooks` covering the two M1-driven cases of "engine lacks supportsSilentPrime" and "engineProfile omitted", 4 `_mergeHookObjects`, 5 `syncEngineHooks` silentPrime integration covering Prawduct + baseline coexistence, baseline-only when methodology empty, off-by-default writes only methodology hooks, placeholder resolution on disk, non-claude engine skip), `test/sessions.test.js` (7: `_writePrimeFile` round-trip + missing-dir create + non-throwing failure path, `launchSession` writes prime when silentPrime+capability, `launchSession` skips prime when off, `launchSession` skips prime when capability flag patched off, `DEFAULT_PROJECT_CONFIG.silentPrime` defaults to false), and new `test/sessionstart-prime-hook.test.js` (5: script exists + executable, cats prime file via execFileSync with `CLAUDE_PROJECT_DIR` set, silent exit-0 when prime file missing, silent exit-0 when env var unset, byte-exact multi-line preservation as a regression-cousin anchor to #75). 1656 total tests passing (was 1627).
