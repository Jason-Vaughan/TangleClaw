...
- **Bundled methodology template fixes don't reach existing installs — runtime templates at `~/.tangleclaw/templates/` stay stale across upgrades (#136)** — v3.13.7 added `memory-update` to `wrap.steps` in all three bundled methodology templates (`data/templates/{prawduct,tilt,minimal}/template.json`), but `_copyBundledTemplates` in `lib/store.js` is a one-shot at install time — the bundled→runtime copy is gated by `if (target has no templates)`, so subsequent updates never propagate. Existing installs continued to omit the `memory-update` step from every wrap, producing the same UX gap v3.13.7 was meant to fix, just on the install-time-copy path instead of the server-time-read path. Same bug class as #119, fixed via the same shape of self-heal.
  - **Fix in `lib/store.js`**: new `_mergeBundledTemplate(bundledPath, livePath)` runs whenever a runtime `template.json` already exists, in two passes. Pass 1: `_addMissingKeys` (lifted from `_mergeBundledProfile` to module scope so both reconcilers share it) deep-merges keys/sub-keys absent from live — never overwriting user values, never recursing into arrays. Pass 2: `wrap.steps` reconciliation — when the live array is an ordered subsequence of the bundled array (`_isOrderedSubsequence` helper), the user has not added their own steps, so adopt the bundled array to pick up newly-introduced entries. Any user-added step (entry not in bundled) leaves the array untouched. Wired into `_copyBundledTemplates`'s file loop: existing files now branch — copy if missing, merge if `template.json`, no-op for `playbook.md` (already covered by `getPlaybook`'s bundled fallback at `lib/store.js:325-336`).
  - **Additive-only contract (intentional)**: the merger cannot distinguish "stale install" from "user deliberately removed an entry." A user who explicitly trims a top-level key or a `wrap.steps` entry that bundled still ships will see it re-added on next server start. Documented in JSDoc and pinned by a regression test (`re-adds wrap.steps entries that the user deliberately removed`) so the trade-off is deliberate, not emergent. Mirrors `_mergeBundledProfile`'s additive contract from #119; users who genuinely need to disable behavior should do so via project config, not by hand-editing runtime templates.
  - **Tripwire**: a regression test reads `data/templates/{prawduct,tilt,minimal}/template.json` directly and asserts each bundled template still ships `memory-update` in `wrap.steps`. If a future PR drops the step from bundled, this test fires before the gap re-opens — locks in the contract the entire #136 fix exists to maintain.
  - **Idempotency**: a same-content second `init()` does not rewrite the file — the merger only writes when `_addMissingKeys` returns true or `stepsDiffer && _isOrderedSubsequence` is true. Verified by a backdated-mtime test (`fs.utimesSync` to a known past time, second init must not bump it).
  - **Tests**: 7 new in `test/store.test.js` `init` describe block — stale wrap.steps reconciled, user-customized wrap.steps preserved, missing top-level keys backfilled, idempotent across reboots, no-rewrite when live matches bundled, additive-only contract pinned, bundled-tripwire. 1696 total passing (was 1689).
  - **Critic-driven adjustments**: Independent Critic flagged the missing CHANGELOG entry (M1), the missing tripwire test for acceptance criterion (c) (M2), the silent-overwrite-of-deletion edge case (S1) — addressed via JSDoc contract note + pinning regression test, and a flaky busy-wait in the no-rewrite test (S4) — replaced with `fs.utimesSync`. S3 (direct unit tests on the lifted helpers) deferred — would require exporting private helpers solely for testing; integration coverage through `init()` is sufficient.
